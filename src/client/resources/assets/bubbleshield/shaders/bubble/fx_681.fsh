...
// stack: deep=fbm x4 taps | mid=raymarchfog+none+flicker | rim=graze | flourish=echo
...
void main() {
    ...
    float mid = clamp(rfLight * 1.2224 + (1.0 - rfTrans) * 0.2092, 0.0, 1.15);
    ...
    float alpha = vertexColor.a * min(0.0489 + 0.3532 * presence + 0.3996 * pattern + 0.1481 * (1.0 - deepTrans), 0.8685);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2182), 0.8685);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5025 + 0.4975 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
