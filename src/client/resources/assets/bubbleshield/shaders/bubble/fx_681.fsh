...
void main() {
    ...
    mid = clamp(mid * (0.6680 + 0.5297 * texDetail) + 0.3446 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4866), 0.0, 1.0);
    float emit = atlas.a * (0.6287 + 0.2686 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1392 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.7067 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3423 + 0.4132 * presence + 0.3996 * pattern + 0.1481 * (1.0 - deepTrans) + 0.1210 * emit, 0.9285);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2182), 0.9285);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7353 + 0.2647 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0806), 0.0, 1.0);
