...
void main() {
    ...
    mid = clamp(mid * (0.7015 + 0.6421 * texDetail) + 0.1810 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4612), 0.0, 1.0);
    float emit = atlas.a * (0.7355 + 0.2799 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0886 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.7960 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3427 + 0.2968 * presence + 0.4660 * pattern + 0.0899 * (1.0 - deepTrans) + 0.1259 * emit, 0.8876);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2016), 0.8876);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7418 + 0.2582 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0759), 0.0, 1.0);
