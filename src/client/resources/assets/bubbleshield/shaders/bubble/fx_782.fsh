...
void main() {
    ...
    mid = clamp(mid * (0.7104 + 0.4628 * texDetail) + 0.3426 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4008), 0.0, 1.0);
    float emit = atlas.a * (0.7971 + 0.2851 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1555 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.7797 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4264 + 0.4305 * presence + 0.4635 * pattern + 0.1080 * (1.0 - deepTrans) + 0.1564 * emit, 0.9013);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3107), 0.9013);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6938 + 0.3062 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1181), 0.0, 1.0);
