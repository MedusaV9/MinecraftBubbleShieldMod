...
void main() {
    ...
    float mid = clamp((lonLine + latLine) * 0.6815 * holoFlicker + scan * 0.4301 + glint * 0.9, 0.0, 1.25);
    ...
    mid = clamp(mid * (0.6525 + 0.6268 * texDetail) + 0.2192 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4511), 0.0, 1.0);
    float emit = atlas.a * (0.7336 + 0.4754 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1326 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.9188 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2998 + 0.3160 * presence + 0.6076 * pattern + 0.0985 * (1.0 - deepTrans) + 0.1599 * emit, 0.8451);
