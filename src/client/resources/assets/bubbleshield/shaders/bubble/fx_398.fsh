...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7681, 0.0, 1.0);
    ...
    deepCol += 0.4645 * dp * mix(baseCol, deepStop, 0.2114);
    deepPat += 0.4645 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 3.2168) + par * 0.0833, time);
    deepCol += 0.2158 * dp * mix(baseCol, deepStop, 0.4228);
    deepPat += 0.2158 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 3.9444) + par * 0.1249, time);
    deepCol += 0.1002 * dp * mix(baseCol, deepStop, 0.6342);
    deepPat += 0.1002 * dp;
    deepCol *= 0.7069;
    deepPat = pow(clamp(deepPat * 0.5616, 0.0, 1.0), 1.1082);
    ...
    float lonLine = pow(abs(sin(baseUV.x * 3.1415927 * 8.0000)), 14.4189);
    float latLine = pow(abs(sin(baseUV.y * 3.1415927 * 8.0000)), 24.1161);
    float scanPos = fract(time * 0.075000);
    float scan = invsmooth(0.0, 0.1457, abs(baseUV.y - scanPos));
    float glint = step(0.55, cellHash(floor(vec2(baseUV.x * 8.0000, baseUV.y * 8.0000)), 8.0000)) * lonLine * latLine;
    float holoFlicker = 0.82 + 0.18 * step(0.35, hash11(floor(time * 8.0781)));
    float mid = clamp((lonLine + latLine) * 0.4535 * holoFlicker + scan * 0.4301 + glint * 2.2, 0.0, 1.25);
    ...
    rgb += hotStop * 0.3379 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0545 + 0.3635 * presence + 0.4491 * pattern, 0.7851);
