...
void main() {
    ...
    mid = clamp(mid * (0.7283 + 0.6483 * texDetail) + 0.2358 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5096), 0.0, 1.0);
    float emit = atlas.a * (0.6422 + 0.4648 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1768 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.9672 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3571 + 0.2924 * presence + 0.5192 * pattern + 0.0831 * (1.0 - deepTrans) + 0.1252 * emit, 0.8683);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2426), 0.8683);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7124 + 0.2876 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1287), 0.0, 1.0);
