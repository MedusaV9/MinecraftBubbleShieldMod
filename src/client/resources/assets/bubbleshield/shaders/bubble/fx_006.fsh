...
void main() {
    ...
    mid = clamp(mid * (0.6381 + 0.4656 * texDetail) + 0.2476 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5429), 0.0, 1.0);
    float emit = atlas.a * (0.7733 + 0.2956 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0929 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.7287 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4272 + 0.4687 * presence + 0.4315 * pattern + 0.0950 * (1.0 - deepTrans) + 0.2033 * emit, 0.9194);
