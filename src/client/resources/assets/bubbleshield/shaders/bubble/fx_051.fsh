...
void main() {
    ...
    mid = clamp(mid * (0.7083 + 0.4754 * texDetail) + 0.2510 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4829), 0.0, 1.0);
    float emit = atlas.a * (0.6437 + 0.3325 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1491 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.9228 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3344 + 0.4882 * presence + 0.3652 * pattern + 0.1429 * (1.0 - deepTrans) + 0.2013 * emit, 0.8823);
