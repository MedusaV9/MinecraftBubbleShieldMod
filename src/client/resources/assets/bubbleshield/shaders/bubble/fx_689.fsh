...
void main() {
    ...
    mid = clamp(mid * (0.6775 + 0.4864 * texDetail) + 0.1838 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3806), 0.0, 1.0);
    float emit = atlas.a * (0.6629 + 0.2598 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0888 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.8535 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4247 + 0.5322 * presence + 0.4722 * pattern + 0.0949 * (1.0 - deepTrans) + 0.1369 * emit, 0.9230);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2419), 0.9230);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7212 + 0.2788 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1082), 0.0, 1.0);
