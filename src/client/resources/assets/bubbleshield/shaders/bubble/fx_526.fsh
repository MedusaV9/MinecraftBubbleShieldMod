...
void main() {
    ...
    mid = clamp(mid * (0.6591 + 0.6754 * texDetail) + 0.3413 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4326), 0.0, 1.0);
    float emit = atlas.a * (0.7808 + 0.3715 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1077 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.8345 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4016 + 0.3745 * presence + 0.4269 * pattern + 0.0874 * (1.0 - deepTrans) + 0.1785 * emit, 0.9192);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2521), 0.9192);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6379 + 0.3621 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
