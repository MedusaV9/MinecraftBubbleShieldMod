...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.001667, 0.020000) * time, time);
    deep += 0.5231 * deepField(duvBase + rimDir * 0.0312 + vec2(0.001667, 0.032500) * time, time);
    deep += 0.2737 * deepField(duvBase + rimDir * 0.0623 + vec2(0.001667, 0.042500) * time, time);
    deep += 0.1432 * deepField(duvBase + rimDir * 0.0935 + vec2(0.001667, 0.052500) * time, time);
    deep = pow(clamp(deep * 0.5155, 0.0, 1.0), 1.2887);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.435000), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 7.0820);
    float flash = 0.6 + 0.4 * sin(time * 4.476770 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.4849 * deep + 0.9353 * mid + 0.5823 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7819 + pattern * 0.4568);
    vec3 rgb = vertexColor.rgb * (0.5190 + 0.8021 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1159 + 0.8140 * pattern, 0.0, 1.0);
