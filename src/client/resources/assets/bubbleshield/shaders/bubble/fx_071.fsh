...
void main() {
    ...
    mid = clamp(mid * (0.6199 + 0.5620 * texDetail) + 0.2088 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5349), 0.0, 1.0);
    float emit = atlas.a * (0.6385 + 0.3591 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0809 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.8668 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4104 + 0.4969 * presence + 0.4289 * pattern + 0.1579 * (1.0 - deepTrans) + 0.1293 * emit, 0.8574);
