...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6930, 0.0, 1.0);
    ...
    deepCol += 0.5231 * dp * mix(baseCol, deepStop, 0.2667);
    deepPat += 0.5231 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.3265) + par * 0.0623, time);
    deepCol += 0.2737 * dp * mix(baseCol, deepStop, 0.5333);
    deepPat += 0.2737 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 6.6640) + par * 0.0935, time);
    deepCol += 0.1432 * dp * mix(baseCol, deepStop, 0.8000);
    deepPat += 0.1432 * dp;
    deepCol *= 0.6572;
    deepPat = pow(clamp(deepPat * 0.5155, 0.0, 1.0), 1.2887);
    ...
    rgb += hotStop * 0.1629 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0678 + 0.3569 * presence + 0.4289 * pattern, 0.7974);
