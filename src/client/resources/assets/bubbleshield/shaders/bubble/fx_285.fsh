...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6581, 0.0, 1.0);
    ...
    deepCol += 0.3942 * dp * mix(baseCol, deepStop, 0.2940);
    deepPat += 0.3942 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 5.8459) + par * 0.1368, time);
    deepCol += 0.1554 * dp * mix(baseCol, deepStop, 0.5880);
    deepPat += 0.1554 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 7.3104) + par * 0.2052, time);
    deepCol += 0.0612 * dp * mix(baseCol, deepStop, 0.8820);
    deepPat += 0.0612 * dp;
    deepCol *= 0.6551;
    deepPat = pow(clamp(deepPat * 0.6208, 0.0, 1.0), 1.1905);
    ...
    rgb += hotStop * 0.3363 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0745 + 0.3663 * presence + 0.3687 * pattern, 0.7977);
