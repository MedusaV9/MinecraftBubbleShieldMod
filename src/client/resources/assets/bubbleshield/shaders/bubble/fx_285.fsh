...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.044167, -0.031667) * time, time);
    deep += 0.3942 * deepField(duvBase + rimDir * 0.0684 + vec2(-0.068333, -0.050000) * time, time);
    deep += 0.1554 * deepField(duvBase + rimDir * 0.1368 + vec2(-0.092500, -0.068333) * time, time);
    deep += 0.0612 * deepField(duvBase + rimDir * 0.2052 + vec2(-0.116667, -0.085000) * time, time);
    deep = pow(clamp(deep * 0.6208, 0.0, 1.0), 1.1905);
