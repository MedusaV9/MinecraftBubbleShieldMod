...
// stack: deep=ridge x4 taps | mid=curlsmoke+warp2+rotate | rim=graze_sparkle | flourish=glint | tex=marble_ink
// fbm: ridged x5 octaves | seed f14406dcc4085452
// v8 realism: refr 0.4626+0.4463*fres/dist (cap 0.1260) | energy 0.4473+0.4725*p | flow 0.051667 c/s | soft 1.3810m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (0.6968 * atlasPoleW));
    ...
    float refrAmp = min((0.4626 + 0.4463 * fresnel) / max(length(worldPos), 1.0), 0.1260);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0580, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9420, vec2(0.001), vec2(0.999))).b);
