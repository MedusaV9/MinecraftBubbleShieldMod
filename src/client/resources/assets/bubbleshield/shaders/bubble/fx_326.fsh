...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8122, 0.0, 1.0);
    ...
    deepCol += 0.5512 * dp * mix(baseCol, deepStop, 0.2195);
    deepPat += 0.5512 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 3.6773) + par * 0.0728, time);
    deepCol += 0.3038 * dp * mix(baseCol, deepStop, 0.4389);
    deepPat += 0.3038 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.5311) + par * 0.1093, time);
    deepCol += 0.1675 * dp * mix(baseCol, deepStop, 0.6584);
    deepPat += 0.1675 * dp;
    deepCol *= 0.6318;
    deepPat = pow(clamp(deepPat * 0.4945, 0.0, 1.0), 1.7223);
    ...
    rgb += hotStop * 0.1550 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0518 + 0.2854 * presence + 0.3954 * pattern, 0.8325);
