...
void main() {
    ...
    mid = clamp(mid * (0.7032 + 0.6031 * texDetail) + 0.3034 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4956), 0.0, 1.0);
    float emit = atlas.a * (0.6416 + 0.3373 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1660 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.7557 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3630 + 0.4254 * presence + 0.3954 * pattern + 0.1568 * (1.0 - deepTrans) + 0.1972 * emit, 0.8925);
