...
void main() {
    ...
    mid = clamp(mid * (0.6691 + 0.6660 * texDetail) + 0.2397 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3783), 0.0, 1.0);
    float emit = atlas.a * (0.6354 + 0.4019 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0827 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.8160 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2713 + 0.2823 * presence + 0.5235 * pattern + 0.1188 * (1.0 - deepTrans) + 0.1622 * emit, 0.8877);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3004), 0.8877);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7098 + 0.2902 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0755), 0.0, 1.0);
