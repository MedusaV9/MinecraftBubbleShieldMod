...
void main() {
    ...
    mid = clamp(mid * (0.7293 + 0.5111 * texDetail) + 0.2575 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4385), 0.0, 1.0);
    float emit = atlas.a * (0.5756 + 0.2528 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0933 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.9576 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3388 + 0.3688 * presence + 0.4449 * pattern + 0.1378 * (1.0 - deepTrans) + 0.2097 * emit, 0.8574);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2685), 0.8574);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6363 + 0.3637 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0758), 0.0, 1.0);
