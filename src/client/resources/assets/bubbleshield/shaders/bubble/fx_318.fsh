...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.397935);
...
void main() {
    ...
    deep += 0.4095 * deepField(duvBase + rimDir * 0.0413 + vec2(-0.053333, 0.057500) * time, time);
    deep = pow(clamp(deep * 0.7095, 0.0, 1.0), 1.5843);
    ...
    vec2 adv = wuv + 0.2358 * curl2(wuv + vec2(0.0, time * 0.070000), midPer);
    float smoke = fbm2(adv + vec2(time * 0.058333, -time * 0.064167), midPer);
