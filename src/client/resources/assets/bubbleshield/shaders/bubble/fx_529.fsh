...
void main() {
    ...
    mid = clamp(mid * (0.6622 + 0.6769 * texDetail) + 0.3025 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5282), 0.0, 1.0);
    float emit = atlas.a * (0.6632 + 0.4092 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0906 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7396 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3999 + 0.5457 * presence + 0.4183 * pattern + 0.1368 * (1.0 - deepTrans) + 0.1582 * emit, 0.9158);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3311), 0.9158);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7147 + 0.2853 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0996), 0.0, 1.0);
