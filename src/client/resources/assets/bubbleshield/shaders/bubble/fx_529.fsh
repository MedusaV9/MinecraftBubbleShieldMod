...
// stack: deep=caustic x3 taps | mid=stainedglass+warp2+rotate | rim=lat | flourish=shimmer | tex=worley_cells
// fbm: ridged x5 octaves | seed 5944d9397fd12659
// v8 realism: refr 0.7717+1.3261*fres/dist (cap 0.1388) | energy 0.2121+0.4370*p | flow 0.073333 c/s | soft 0.8945m
...
void main() {
    ...
    float refrAmp = min((0.7717 + 1.3261 * fresnel) / max(length(worldPos), 1.0), 0.1388);
