...
void main() {
    ...
    mid = clamp(mid * (0.7127 + 0.5761 * texDetail) + 0.2458 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5203), 0.0, 1.0);
    float emit = atlas.a * (0.6497 + 0.2786 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1593 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.8277 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3518 + 0.3741 * presence + 0.3683 * pattern + 0.0835 * (1.0 - deepTrans) + 0.1302 * emit, 0.9025);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2300), 0.9025);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6762 + 0.3238 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1083), 0.0, 1.0);
