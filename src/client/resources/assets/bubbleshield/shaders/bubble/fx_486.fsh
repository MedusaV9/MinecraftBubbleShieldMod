...
// stack: deep=caustic x3 taps | mid=fluidink+curl+pulse | rim=graze_sparkle | flourish=echo | tex=marble_ink
// fbm: ridged x4 octaves | seed 9c38d5bcdad2af7d
// v8 realism: refr 0.5977+0.5964*fres/dist (cap 0.1297) | energy 0.3147+0.4275*p | flow 0.099167 c/s | soft 0.9654m
...
void main() {
    ...
    float refrAmp = min((0.5977 + 0.5964 * fresnel) / max(length(worldPos), 1.0), 0.1297);
