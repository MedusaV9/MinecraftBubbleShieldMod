...
void main() {
    ...
    mid = clamp(mid * (0.7486 + 0.5785 * texDetail) + 0.2603 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4449), 0.0, 1.0);
    float emit = atlas.a * (0.7668 + 0.2943 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1227 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.8637 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3991 + 0.4069 * presence + 0.3591 * pattern + 0.1179 * (1.0 - deepTrans) + 0.2133 * emit, 0.8688);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2470), 0.8688);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7757 + 0.2243 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1270), 0.0, 1.0);
