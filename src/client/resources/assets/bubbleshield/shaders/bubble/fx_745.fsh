...
void main() {
    ...
    mid = clamp(mid * (0.6849 + 0.4986 * texDetail) + 0.1902 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3506), 0.0, 1.0);
    float emit = atlas.a * (0.6282 + 0.2928 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1402 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.7009 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3723 + 0.4282 * presence + 0.4581 * pattern + 0.1317 * (1.0 - deepTrans) + 0.1881 * emit, 0.9128);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2091), 0.9128);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7694 + 0.2306 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1186), 0.0, 1.0);
