...
void main() {
    ...
    mid = clamp(mid * (0.6478 + 0.4882 * texDetail) + 0.2057 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3669), 0.0, 1.0);
    float emit = atlas.a * (0.7414 + 0.2961 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1239 * (0.5 + 0.5 * sin(time * 0.209440)));
    ...
    float alpha = vertexColor.a * min(0.3442 + 0.2818 * presence + 0.6108 * pattern + 0.1509 * (1.0 - deepTrans) + 0.1401 * emit, 0.8656);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3220), 0.8656);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6879 + 0.3121 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1003), 0.0, 1.0);
