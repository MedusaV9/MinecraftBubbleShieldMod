...
void main() {
    ...
    mid = clamp(mid * (0.7204 + 0.6574 * texDetail) + 0.3086 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.5706 + 0.4702 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0881 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.7084 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2752 + 0.3071 * presence + 0.4701 * pattern + 0.0987 * (1.0 - deepTrans) + 0.1303 * emit, 0.8559);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2718), 0.8559);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7533 + 0.2467 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1030), 0.0, 1.0);
