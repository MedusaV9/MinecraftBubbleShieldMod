...
void main() {
    ...
    mid = clamp(mid * (0.6156 + 0.5785 * texDetail) + 0.2285 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4313), 0.0, 1.0);
    float emit = atlas.a * (0.6082 + 0.2894 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1725 * (0.5 + 0.5 * sin(time * 0.193732)));
    ...
    float alpha = vertexColor.a * min(0.3485 + 0.3038 * presence + 0.4819 * pattern + 0.1227 * (1.0 - deepTrans) + 0.1266 * emit, 0.7943);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2082), 0.7943);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7363 + 0.2637 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0987), 0.0, 1.0);
