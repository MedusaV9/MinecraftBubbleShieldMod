...
void main() {
    ...
    mid = clamp(mid * (0.7396 + 0.4643 * texDetail) + 0.2903 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5389), 0.0, 1.0);
    float emit = atlas.a * (0.6360 + 0.2535 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1313 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.8790 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4065 + 0.4370 * presence + 0.4891 * pattern + 0.1142 * (1.0 - deepTrans) + 0.1567 * emit, 0.8944);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2713), 0.8944);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6439 + 0.3561 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1075), 0.0, 1.0);
