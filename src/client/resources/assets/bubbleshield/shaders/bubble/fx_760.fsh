...
void main() {
    ...
    mid = clamp(mid * (0.7310 + 0.6736 * texDetail) + 0.2714 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5094), 0.0, 1.0);
    float emit = atlas.a * (0.6327 + 0.3967 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0935 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.8545 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3334 + 0.2842 * presence + 0.5157 * pattern + 0.0813 * (1.0 - deepTrans) + 0.1210 * emit, 0.8724);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2849), 0.8724);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6204 + 0.3796 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0728), 0.0, 1.0);
