...
// stack: deep=ridge x2 taps | mid=waves+warp2+pulse | rim=graze | flourish=glint
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.038333, 0.015000) * time, time);
    deep += 0.5418 * deepField(duvBase + rimDir * 0.0494 + vec2(0.058333, 0.022500) * time, time);
    deep = pow(clamp(deep * 0.6486, 0.0, 1.0), 1.2270);
    ...
    float pattern = clamp(0.4000 * deep + 0.7158 * mid + 0.7513 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7232 + pattern * 0.3441);
    vec3 rgb = vertexColor.rgb * (0.3687 + 0.8555 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1430 + 0.6003 * pattern, 0.0, 1.0);
