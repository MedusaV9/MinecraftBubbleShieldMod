...
void main() {
    ...
    mid = clamp(mid * (0.6169 + 0.6430 * texDetail) + 0.1950 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4207), 0.0, 1.0);
    float emit = atlas.a * (0.6336 + 0.3855 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1023 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.7268 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3602 + 0.3563 * presence + 0.4298 * pattern + 0.1144 * (1.0 - deepTrans) + 0.1497 * emit, 0.9013);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3039), 0.9013);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7445 + 0.2555 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0970), 0.0, 1.0);
