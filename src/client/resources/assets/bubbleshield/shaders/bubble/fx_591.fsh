...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8414, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0518, abs(glR - 0.1086));
    float mid = clamp(glStars * (1.0 + glRing * 1.6191) + glRing * 0.5322, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.7256 + 0.6460 * texDetail) + 0.3464 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8352 + 0.4029 * texDetail;
    rgb = satLift(rgb, 1.3065);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5320), 0.0, 1.0);
    float emit = atlas.a * (0.6283 + 0.3582 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1110 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.7838 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3179 + 0.2322 * presence + 0.5066 * pattern + 0.1309 * (1.0 - deepTrans) + 0.1665 * emit, 0.8664);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2352), 0.8664);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7278 + 0.2722 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1397), 0.0, 1.0);
