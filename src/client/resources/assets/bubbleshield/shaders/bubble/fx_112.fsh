...
void main() {
    ...
    mid = clamp(mid * (0.7264 + 0.5845 * texDetail) + 0.2758 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4684), 0.0, 1.0);
    float emit = atlas.a * (0.6902 + 0.4578 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1439 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.7027 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4176 + 0.5061 * presence + 0.4752 * pattern + 0.1511 * (1.0 - deepTrans) + 0.2150 * emit, 0.8689);
