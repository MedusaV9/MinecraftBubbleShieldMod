...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6471, 0.0, 1.0);
    ...
    deepCol += 0.4164 * dp * mix(baseCol, deepStop, 0.2038);
    deepPat += 0.4164 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 5.7275) + par * 0.1461, time);
    deepCol += 0.1734 * dp * mix(baseCol, deepStop, 0.4077);
    deepPat += 0.1734 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 7.2796) + par * 0.2192, time);
    deepCol += 0.0722 * dp * mix(baseCol, deepStop, 0.6115);
    deepPat += 0.0722 * dp;
    deepCol *= 0.7635;
    deepPat = pow(clamp(deepPat * 0.6017, 0.0, 1.0), 1.7903);
    ...
    rgb += hotStop * 0.3034 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0702 + 0.3661 * presence + 0.4752 * pattern, 0.8089);
