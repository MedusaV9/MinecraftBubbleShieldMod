...
// stack: deep=fbm x4 taps | mid=circuit+none+pulse | rim=graze_film | flourish=shimmer | tex=circuit
// fbm: standard x3 octaves | seed f46d82e7a9ffe114
// v8 realism: refr 0.4958+0.7360*fres/dist (cap 0.1303) | energy 0.3133+0.4308*p | flow 0.055000 c/s | soft 1.0307m
...
void main() {
    ...
    float refrAmp = min((0.4958 + 0.7360 * fresnel) / max(length(worldPos), 1.0), 0.1303);
