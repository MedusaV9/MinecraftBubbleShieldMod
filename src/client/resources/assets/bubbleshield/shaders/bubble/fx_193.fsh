...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0495) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0495);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.043333, 0.030000) * time, time);
    deep += 0.5627 * deepField(duvBase + rimDir * 0.0337 + vec2(0.066667, 0.047500) * time, time);
    deep += 0.3167 * deepField(duvBase + rimDir * 0.0674 + vec2(0.091667, 0.062500) * time, time);
    deep += 0.1782 * deepField(duvBase + rimDir * 0.1011 + vec2(0.115000, 0.080000) * time, time);
    deep = pow(clamp(deep * 0.4860, 0.0, 1.0), 1.0563);
