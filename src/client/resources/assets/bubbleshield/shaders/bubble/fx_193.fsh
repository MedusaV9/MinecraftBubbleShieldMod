...
void main() {
    ...
    mid = clamp(mid * (0.6841 + 0.4611 * texDetail) + 0.3091 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5246), 0.0, 1.0);
    float emit = atlas.a * (0.7723 + 0.4009 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1762 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.7926 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3668 + 0.4656 * presence + 0.4502 * pattern + 0.0922 * (1.0 - deepTrans) + 0.1974 * emit, 0.9148);
