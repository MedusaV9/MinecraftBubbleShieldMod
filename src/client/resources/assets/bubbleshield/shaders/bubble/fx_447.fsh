...
void main() {
    ...
    mid = clamp(mid * (0.7056 + 0.5209 * texDetail) + 0.3332 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4477), 0.0, 1.0);
    float emit = atlas.a * (0.7319 + 0.4736 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1486 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.8320 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3030 + 0.3619 * presence + 0.4784 * pattern + 0.1168 * (1.0 - deepTrans) + 0.1956 * emit, 0.8052);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3476), 0.8052);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6755 + 0.3245 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1250), 0.0, 1.0);
