...
void main() {
    ...
    mid = clamp(mid * (0.6385 + 0.6644 * texDetail) + 0.2392 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5138), 0.0, 1.0);
    float emit = atlas.a * (0.7818 + 0.4367 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1211 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.7967 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3438 + 0.4165 * presence + 0.3993 * pattern + 0.1369 * (1.0 - deepTrans) + 0.1213 * emit, 0.8536);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1534), 0.8536);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6333 + 0.3667 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0958), 0.0, 1.0);
