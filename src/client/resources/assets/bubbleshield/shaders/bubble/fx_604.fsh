...
void main() {
    ...
    mid = clamp(mid * (0.6926 + 0.4592 * texDetail) + 0.3442 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4103), 0.0, 1.0);
    float emit = atlas.a * (0.5940 + 0.2845 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1454 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.9055 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3488 + 0.3351 * presence + 0.4911 * pattern + 0.0894 * (1.0 - deepTrans) + 0.1310 * emit, 0.8631);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2267), 0.8631);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7509 + 0.2491 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1212), 0.0, 1.0);
