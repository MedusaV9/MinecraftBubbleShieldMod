...
void main() {
    ...
    mid = clamp(mid * (0.7254 + 0.7316 * texDetail) + 0.3047 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3601), 0.0, 1.0);
    float emit = atlas.a * (0.6731 + 0.3798 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0977 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.9192 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3822 + 0.5559 * presence + 0.3892 * pattern + 0.1312 * (1.0 - deepTrans) + 0.1954 * emit, 0.9381);
