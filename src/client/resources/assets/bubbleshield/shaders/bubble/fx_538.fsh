...
void main() {
    ...
    mid = clamp(mid * (0.6718 + 0.6627 * texDetail) + 0.3064 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5497), 0.0, 1.0);
    float emit = atlas.a * (0.6402 + 0.4426 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1378 * (0.5 + 0.5 * sin(time * 0.251327)));
    ...
    float alpha = vertexColor.a * min(0.3673 + 0.2855 * presence + 0.4120 * pattern + 0.0812 * (1.0 - deepTrans) + 0.1553 * emit, 0.8525);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1643), 0.8525);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7308 + 0.2692 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1248), 0.0, 1.0);
