...
void main() {
    ...
    mid = clamp(mid * (0.6738 + 0.7330 * texDetail) + 0.2398 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4118), 0.0, 1.0);
    float emit = atlas.a * (0.6480 + 0.2544 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1794 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.8501 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4434 + 0.5206 * presence + 0.3598 * pattern + 0.1577 * (1.0 - deepTrans) + 0.1384 * emit, 0.9578);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2451), 0.9578);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7474 + 0.2526 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
