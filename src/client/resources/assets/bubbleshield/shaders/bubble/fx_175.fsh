...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9463) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9463);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.025000, 0.006667) * time, time);
    deep += 0.4785 * deepField(duvBase + rimDir * 0.0409 + vec2(0.037500, 0.013333) * time, time);
    deep += 0.2290 * deepField(duvBase + rimDir * 0.0817 + vec2(0.050000, 0.016667) * time, time);
    deep += 0.1096 * deepField(duvBase + rimDir * 0.1226 + vec2(0.062500, 0.020000) * time, time);
    deep = pow(clamp(deep * 0.5503, 0.0, 1.0), 1.3219);
    ...
    vec2 wuv = auv;
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.346667), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 11.3491);
    float flash = 0.6 + 0.4 * sin(time * 2.068215 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.3598 * deep + 0.7550 * mid + 0.6305 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5808 + pattern * 0.4154);
    vec3 rgb = vertexColor.rgb * (0.4257 + 0.7595 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1749 + 0.6503 * pattern, 0.0, 1.0);
