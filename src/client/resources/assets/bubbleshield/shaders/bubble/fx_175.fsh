...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6981, 0.0, 1.0);
    ...
    deepCol += 0.4785 * dp * mix(baseCol, deepStop, 0.2050);
    deepPat += 0.4785 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 5.8080) + par * 0.0817, time);
    deepCol += 0.2290 * dp * mix(baseCol, deepStop, 0.4099);
    deepPat += 0.2290 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 7.2177) + par * 0.1226, time);
    deepCol += 0.1096 * dp * mix(baseCol, deepStop, 0.6149);
    deepPat += 0.1096 * dp;
    deepCol *= 0.6175;
    deepPat = pow(clamp(deepPat * 0.5503, 0.0, 1.0), 1.3219);
    ...
    rgb += hotStop * 0.2397 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0520 + 0.3602 * presence + 0.3240 * pattern, 0.8556);
