...
void main() {
    ...
    mid = clamp(mid * (0.6343 + 0.6625 * texDetail) + 0.2197 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4336), 0.0, 1.0);
    float emit = atlas.a * (0.6063 + 0.4378 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0887 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.9674 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3269 + 0.3199 * presence + 0.5408 * pattern + 0.1436 * (1.0 - deepTrans) + 0.2058 * emit, 0.8501);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1813), 0.8501);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7324 + 0.2676 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1115), 0.0, 1.0);
