...
void main() {
    ...
    mid = clamp(mid * (0.6370 + 0.5520 * texDetail) + 0.3486 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4156), 0.0, 1.0);
    float emit = atlas.a * (0.6700 + 0.4804 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1793 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.9100 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3114 + 0.2752 * presence + 0.6041 * pattern + 0.0815 * (1.0 - deepTrans) + 0.2050 * emit, 0.8673);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3280), 0.8673);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7522 + 0.2478 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1068), 0.0, 1.0);
