...
// stack: deep=ridge x3 taps | mid=arcs+none+rotate | rim=lat | flourish=shimmer
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1193) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1193);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.020833, 0.016667) * time, time);
    deep += 0.5258 * deepField(duvBase + rimDir * 0.0628 + vec2(-0.033333, 0.026667) * time, time);
    deep += 0.2765 * deepField(duvBase + rimDir * 0.1256 + vec2(-0.045000, 0.035000) * time, time);
    deep = pow(clamp(deep * 0.5548, 0.0, 1.0), 1.3944);
    ...
    vec2 wuv = auv;
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.520000), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 8.9341);
    float flash = 0.6 + 0.4 * sin(time * 2.675590 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.4047 * deep + 0.9029 * mid + 0.5796 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6453 + pattern * 0.3615);
    vec3 rgb = vertexColor.rgb * (0.5360 + 0.7834 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1258 + 0.7580 * pattern, 0.0, 1.0);
