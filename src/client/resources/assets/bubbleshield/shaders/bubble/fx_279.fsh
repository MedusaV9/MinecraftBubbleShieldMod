...
void main() {
    ...
    mid = clamp(mid * (0.6373 + 0.5157 * texDetail) + 0.2600 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7117 * rimLine, 0.0, 1.4);
    ...
    float emit = atlas.a * (0.7358 + 0.3624 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1786 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7276 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3402 + 0.4122 * presence + 0.3990 * pattern + 0.1168 * (1.0 - deepTrans) + 0.1399 * emit, 0.9020);
