...
void main() {
    ...
    mid = clamp(mid * (0.7320 + 0.5426 * texDetail) + 0.2374 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4447), 0.0, 1.0);
    float emit = atlas.a * (0.7130 + 0.3871 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1627 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.8656 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3439 + 0.4193 * presence + 0.3955 * pattern + 0.1203 * (1.0 - deepTrans) + 0.1789 * emit, 0.8589);
