...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8148, 0.0, 1.0);
    ...
    deepCol += 0.5674 * dp * mix(baseCol, deepStop, 0.3348);
    deepPat += 0.5674 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 3.5654) + par * 0.0982, time);
    deepCol += 0.3220 * dp * mix(baseCol, deepStop, 0.6695);
    deepPat += 0.3220 * dp;
    deepCol *= 0.5750;
    deepPat = pow(clamp(deepPat * 0.5293, 0.0, 1.0), 1.3922);
    ...
    float fern = pow(clamp(vein1 * vein2, 0.0, 1.0), 4.2612);
    ...
    rgb = satLift(rgb, 1.2871);
    rgb += hotStop * 0.3417 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0454 + 0.2793 * presence + 0.3955 * pattern, 0.7989);
