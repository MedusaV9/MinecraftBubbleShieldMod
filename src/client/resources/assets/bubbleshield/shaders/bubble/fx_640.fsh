...
void main() {
    ...
    mid = clamp(mid * (0.6363 + 0.7344 * texDetail) + 0.2497 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3682), 0.0, 1.0);
    float emit = atlas.a * (0.5699 + 0.3282 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1470 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7240 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3524 + 0.2480 * presence + 0.3995 * pattern + 0.0943 * (1.0 - deepTrans) + 0.2152 * emit, 0.7827);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2152), 0.7827);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7673 + 0.2327 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1228), 0.0, 1.0);
