...
void main() {
    ...
    deep += 0.4149 * deepField(duvBase + rimDir * 0.0418 + vec2(0.040000, -0.055000) * time, time);
    deep += 0.1722 * deepField(duvBase + rimDir * 0.0836 + vec2(0.053333, -0.075000) * time, time);
    deep += 0.0714 * deepField(duvBase + rimDir * 0.1254 + vec2(0.068333, -0.092500) * time, time);
    deep = pow(clamp(deep * 0.6029, 0.0, 1.0), 1.6286);
