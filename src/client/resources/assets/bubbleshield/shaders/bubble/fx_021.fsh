...
void main() {
    ...
    mid = clamp(mid * (0.7272 + 0.5500 * texDetail) + 0.2887 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.5537 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4184), 0.0, 1.0);
    float emit = atlas.a * (0.7837 + 0.3043 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1718 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.8279 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3463 + 0.4404 * presence + 0.4964 * pattern + 0.0913 * (1.0 - deepTrans) + 0.2179 * emit, 0.9059);
