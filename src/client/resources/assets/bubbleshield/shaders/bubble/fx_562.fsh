...
void main() {
    ...
    mid = clamp(mid * (0.7098 + 0.5426 * texDetail) + 0.2139 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6959 + 0.4371 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1204 * (0.5 + 0.5 * sin(time * 0.151844)));
    ...
    float alpha = vertexColor.a * min(0.3723 + 0.4749 * presence + 0.3560 * pattern + 0.1228 * (1.0 - deepTrans) + 0.1544 * emit, 0.8959);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1804), 0.8959);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0660), 0.0, 1.0);
