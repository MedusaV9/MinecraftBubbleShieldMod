...
void main() {
    ...
    mid = clamp(mid * (0.6600 + 0.5407 * texDetail) + 0.2176 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4839), 0.0, 1.0);
    float emit = atlas.a * (0.7587 + 0.4448 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0896 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.8927 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2809 + 0.2609 * presence + 0.4880 * pattern + 0.1360 * (1.0 - deepTrans) + 0.1456 * emit, 0.8983);
