...
// stack: deep=fbm x3 taps | mid=raymarchfog+curl+flicker | rim=graze_film | flourish=swirl
...
void main() {
    ...
    float mid = clamp(rfLight * 1.0113 + (1.0 - rfTrans) * 0.2207, 0.0, 1.15);
    ...
    float alpha = vertexColor.a * min(0.0401 + 0.3111 * presence + 0.3035 * pattern + 0.1199 * (1.0 - deepTrans), 0.8490);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3311), 0.8490);
    // [layer:v5:ghostalpha]
