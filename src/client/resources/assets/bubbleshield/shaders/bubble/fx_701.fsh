...
void main() {
    ...
    mid = clamp(mid * (0.6468 + 0.6343 * texDetail) + 0.2594 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4847), 0.0, 1.0);
    float emit = atlas.a * (0.6955 + 0.2731 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1468 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.7443 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3192 + 0.3711 * presence + 0.3035 * pattern + 0.1199 * (1.0 - deepTrans) + 0.1837 * emit, 0.9090);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3311), 0.9090);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0668), 0.0, 1.0);
