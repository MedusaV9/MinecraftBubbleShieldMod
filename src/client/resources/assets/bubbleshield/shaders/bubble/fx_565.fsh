...
void main() {
    ...
    mid = clamp(mid * (0.6681 + 0.4748 * texDetail) + 0.3165 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4408), 0.0, 1.0);
    float emit = atlas.a * (0.6482 + 0.3659 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0984 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.9610 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3246 + 0.4159 * presence + 0.4363 * pattern + 0.1006 * (1.0 - deepTrans) + 0.1399 * emit, 0.8482);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2244), 0.8482);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6561 + 0.3439 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0770), 0.0, 1.0);
