...
void main() {
    ...
    mid = clamp(mid * (0.6078 + 0.5222 * texDetail) + 0.2070 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5413), 0.0, 1.0);
    float emit = atlas.a * (0.7218 + 0.4799 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1526 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.7058 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3924 + 0.4253 * presence + 0.4649 * pattern + 0.0853 * (1.0 - deepTrans) + 0.2112 * emit, 0.8782);
