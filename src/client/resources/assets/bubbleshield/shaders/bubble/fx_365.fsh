...
void main() {
    ...
    mid = clamp(mid * (0.6964 + 0.5191 * texDetail) + 0.2229 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4559), 0.0, 1.0);
    float emit = atlas.a * (0.6157 + 0.3550 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1428 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.6690 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4711 + 0.5743 * presence + 0.4688 * pattern + 0.1422 * (1.0 - deepTrans) + 0.1520 * emit, 0.9023);
