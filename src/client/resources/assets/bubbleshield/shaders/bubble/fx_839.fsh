...
void main() {
    ...
    mid = clamp(mid * (0.7294 + 0.5821 * texDetail) + 0.3446 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.5953 + 0.2957 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1689 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.7356 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3798 + 0.3188 * presence + 0.4283 * pattern + 0.1040 * (1.0 - deepTrans) + 0.2063 * emit, 0.8487);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1822), 0.8487);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6231 + 0.3769 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
