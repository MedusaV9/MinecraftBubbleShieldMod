...
// v8 realism: refr 1.1568+1.0966*fres/dist (cap 0.1690) | energy 0.4003+0.4314*p | flow 0.074167 c/s | soft 0.9825m
...
void main() {
    ...
    rim = clamp(rim + 0.8655 * rimLine + 0.3782 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1838 * caustic(wuv + vec2(3.1, 8.7), vec2(0.122500, -0.120000) * time, midPer);
    float grain = 0.0691 * (cellHash(floor(wuv * 32.0000) + vec2(floor(time * 6.0), 0.0), 224.0000) - 0.5);
    ...
    float motifEnv = 0.15 + 0.85 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.565487)) * (0.5 + 0.5 * sin(time * 0.900590 + 1.7)));
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 26.5135) * (0.75 + 0.25 * sin(time * 0.539307 + 4.2268));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 26.5135);
    ...
    float refrAmp = min((1.1568 + 1.0966 * fresnel) / max(length(worldPos), 1.0), 0.1690);
    ...
    float energyW = clamp(0.4003 + 0.4314 * clamp(pattern, 0.0, 1.0) + 0.2007 * fresnel, 0.0, 0.8066);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.5532;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3027) * 0.7872;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3719 * (1.0 - 0.5790 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.4101 + 0.6518 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1051 * (0.5 + 0.5 * sin(time * 0.277507))) * atlasPoleW;
    rgb += emit * 0.9774 * (1.0 - 0.6554 * baseLuma) * emitCol;
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9063) * vertexColor.a;
