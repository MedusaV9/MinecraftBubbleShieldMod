...
void main() {
    ...
    deep += 0.5710 * deepField(duvBase + rimDir * 0.0589 + vec2(0.028333, 0.047500) * time, time);
    deep += 0.3261 * deepField(duvBase + rimDir * 0.1178 + vec2(0.038333, 0.065000) * time, time);
    deep += 0.1862 * deepField(duvBase + rimDir * 0.1767 + vec2(0.048333, 0.080000) * time, time);
    deep = pow(clamp(deep * 0.4800, 0.0, 1.0), 1.0390);
