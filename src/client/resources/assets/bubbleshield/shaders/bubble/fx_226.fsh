...
// stack: deep=fbm x4 taps | mid=petals+warp2+pulse | rim=graze_sparkle | flourish=shimmer | tex=feather_barbs | motif=corex3+sparklegate
// fbm: turb x5 octaves | seed fa558234772bdd80
// v8 realism: refr 1.2889+1.2219*fres/dist (cap 0.1690) | energy 0.2836+0.4314*p | flow 0.074167 c/s | soft 0.9825m
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.7009 * (vec2(fbm2(p + vec2(0.0, t * 0.307500), per),
...
vec2 warp2(vec2 p, vec2 per, float t) {
    ...
    return p + 0.4736 * (vec2(fbm2(q + vec2(1.7, 9.2), per),
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4736, x));
    return mix(g, hotStop, smoothstep(0.4736, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0069, 0.0360, g);
}
...
float sparkle(vec2 p, float t, float px) {
    ...
    return step(0.6872, h) * invsmooth(0.02, 0.22, d) * tw;
}
...
void main() {
    ...
    vec3 spun = clamp(hueSpin(baseCol, 0.3174), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.1927) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.4977, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.6473);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, -2.6000), 0.0, 1.0), 1.7441) * 0.3000, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(0.6143, -0.5841, -0.5306);
    vec3 par = Vt * 1.9394;
    ...
    float deepBreath = 1.0 + 0.0138 * sin(time * 0.785398);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * (2.9986 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4809 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * (4.0836 * deepBreath)) + par * 0.0589, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.4063), deepStop, 0.1490);
    deepTrans *= 1.0 - 0.4809 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * (5.1686 * deepBreath)) + par * 0.1178, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.8126), deepStop, 0.2980);
    deepTrans *= 1.0 - 0.4809 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * (6.2537 * deepBreath)) + par * 0.1767, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4470);
    deepTrans *= 1.0 - 0.4809 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.6846;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0783, 0.0, 1.0), 1.0390);

    // [layer:mid:petals_warp2_pulse]
    // Signature structure of this effect, domain-warped and animated.
    float breathe = 1.0 + 0.0493 * sin(time * 0.984366);
    // only the y axis breathes; the x scale stays fixed so the u wrap
    // stays exact (the y period carries a margin for the stretch)
    vec2 auv = vec2(baseUV.x * 7.0000 + time * 0.000000, (baseUV.y - 0.5) * 7.0000 * breathe + 3.5000 + time * 0.172500);
    ...
    float ang = safeAtan(baseUV.y - 0.5, sin(du * 6.2831853) * 0.5) + time * 0.335103;
    float rad = length(vec2(sin(du * 3.1415927), baseUV.y - 0.5)) * 2.0;
    float petal = pow(abs(cos(ang * 3.0000)), 1.1564);
    float bloom = invsmooth(petal * 0.9 - 0.12, petal * 0.9 + 0.08, rad);
    float veins = 0.5 + 0.5 * sin(rad * 18.6621 - time * 0.769690);
    ...
    vec2 texUV = wuv * 1.0000 + vec2(-0.025000, 0.032500) * time;
    ...
    vec3 hgtW = vec3(0.3771, 0.4093, 0.2136);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * 2.3110;
    ...
    float texDetail = clamp(atlas.r * 0.4776 + atlas.g * 0.5184 + atlas.b * 0.2706, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6450 + 0.7318 * texDetail, atlasPoleW) + 0.1236 * texDetail * (0.2204 + 0.7796 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.8042 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.5418);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0521).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0521).rgb;
    float deepTex = 0.5 * (dot(deepTexA, hgtW) + dot(deepTexB, hgtW));

    // [layer:thick:chord]
    // v11 volumetric thickness: the membrane is a shell of relative
    // thickness rho = 0.1463 (organic material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 1.0416.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.8537 * 0.8537 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 1.0416);
    float chordN = min(chord / 0.1463, 3.0);
    ...
    float rimLine = smoothstep(0.0498, 0.0849, rimSlope);
    ...
    rim = clamp(rim + 0.8655 * rimLine + 0.5838 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1838 * caustic(wuv + vec2(3.1, 8.7), vec2(0.122500, -0.120000) * time, midPer);
    float grain = 0.0691 * (cellHash(floor(wuv * 32.0000) + vec2(floor(time * 2.0), 0.0), 224.0000) - 0.5);
    ...
    float motifEnv = 0.48 + 0.52 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.565487)) * (0.5 + 0.5 * sin(time * 0.900590 + 1.7)));
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 14.3510) * (0.75 + 0.25 * sin(time * 0.539307 + 4.2268));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 14.3510);
    ...
    float pattern = clamp(0.3202 * deepPat * mix(1.0, chordN, 0.2255) + 0.8817 * mid + 0.6574 * rim + flourish + grain + 0.2438 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.7661 + rim * 0.1711, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9889 * mid + 0.4360 * rim, 0.0, 1.0);
    ...
    vec3 softHot = 1.0 - exp(-2.6244 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.2153 * smoothstep(0.55, 1.10, pattern));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.0139 + baseUV.y * 0.3217 + 0.0064) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.2126);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.5172 + baseUV.y * 0.31 + 0.2837) * 6.2831853);
    ...
    rgb *= (1.0 - 0.3140 * washHi) * (1.0 - 0.3294 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.6606, 0.5939, 0.4593);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4613) / 1.4613, 0.0, 1.0);
    rgb *= 0.6335 + 0.5738 * keyDiff * (1.0 - 0.4960 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 14.9584);
    rgb += hotStop * (keySpec * 0.3368 * (1.0 - 0.6282 * washHi));
    ...
    float refrAmp = min((1.2889 + 1.2219 * fresnel) / max(length(worldPos), 1.0), 0.1690);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.5443) * refrAmp;
    ...
    refracted *= exp(-0.6799 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1884);
    float energyW = clamp(0.2836 + 0.4314 * clamp(pattern, 0.0, 1.0) + 0.2975 * fresnel, 0.0, 0.8066);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2268 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (0.8538 * (1.0 - 0.7046 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3027) * 1.1668;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3719 * max(0.0, 1.0 - 0.5790 * baseLuma - 0.5597 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.4101 + 0.6518 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1051 * (0.5 + 0.5 * sin(time * 0.277507))) * atlasPoleW;
    rgb += emit * 0.9774 * (1.0 - 0.6554 * baseLuma) * (1.0 - 0.5345 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.4700));
    ...
    float bodyAlpha = 0.3848 + 0.4456 * presence + 0.4426 * pattern + 0.1111 * (1.0 - deepTrans) + 0.1615 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.1605;
    // [layer:inner:fogbank]
    ...
    // perspective reach.
    float innerFog = clamp(deepField(rotA(spinAxis, time * 0.068068 + 5.2920) * (sdir * 2.6987), time) * 1.0159 + 0.1343, 0.0, 1.0);
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9460) * vertexColor.a;
