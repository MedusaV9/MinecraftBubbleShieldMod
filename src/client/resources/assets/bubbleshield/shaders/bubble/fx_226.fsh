...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6613, 0.0, 1.0);
    ...
    deepCol += 0.5710 * dp * mix(baseCol, deepStop, 0.2709);
    deepPat += 0.5710 * dp;
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * 5.1686) + par * 0.1178, time);
    deepCol += 0.3261 * dp * mix(baseCol, deepStop, 0.5418);
    deepPat += 0.3261 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 6.2537) + par * 0.1767, time);
    deepCol += 0.1862 * dp * mix(baseCol, deepStop, 0.8126);
    deepPat += 0.1862 * dp;
    deepCol *= 0.5083;
    deepPat = pow(clamp(deepPat * 0.4800, 0.0, 1.0), 1.0390);
    ...
    rgb += hotStop * 0.1653 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0592 + 0.3056 * presence + 0.4426 * pattern, 0.8058);
