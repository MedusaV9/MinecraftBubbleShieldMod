...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8439, 0.0, 1.0);
    ...
    deepCol += 0.3921 * dp * mix(baseCol, deepStop, 0.2999);
    deepPat += 0.3921 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 4.2289) + par * 0.1304, time);
    deepCol += 0.1537 * dp * mix(baseCol, deepStop, 0.5998);
    deepPat += 0.1537 * dp;
    deepCol *= 0.7193;
    deepPat = pow(clamp(deepPat * 0.6469, 0.0, 1.0), 1.0184);
    ...
    rgb += hotStop * 0.2616 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0672 + 0.2945 * presence + 0.3942 * pattern, 0.8005);
