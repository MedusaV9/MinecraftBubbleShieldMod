...
void main() {
    ...
    mid = clamp(mid * (0.7442 + 0.6795 * texDetail) + 0.2359 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3549), 0.0, 1.0);
    float emit = atlas.a * (0.6629 + 0.2964 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1266 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9193 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4087 + 0.4345 * presence + 0.3942 * pattern + 0.1459 * (1.0 - deepTrans) + 0.2050 * emit, 0.8605);
