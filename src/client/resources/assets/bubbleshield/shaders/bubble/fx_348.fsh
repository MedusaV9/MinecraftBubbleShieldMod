...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.000000, 0.047500) * time, time);
    deep += 0.3921 * deepField(duvBase + rimDir * 0.0652 + vec2(0.000000, 0.075000) * time, time);
    deep += 0.1537 * deepField(duvBase + rimDir * 0.1304 + vec2(0.000000, 0.102500) * time, time);
    deep = pow(clamp(deep * 0.6469, 0.0, 1.0), 1.0184);
    ...
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.120833, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.110833), midPer)), midPer);
    ...
    float pattern = clamp(0.3189 * deep + 0.8194 * mid + 0.8059 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2415 + pattern * 0.5594);
    vec3 rgb = vertexColor.rgb * (0.3733 + 0.9339 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1955 + 0.7231 * pattern, 0.0, 1.0);
