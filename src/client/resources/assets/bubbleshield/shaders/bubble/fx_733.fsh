...
void main() {
    ...
    mid = clamp(mid * (0.6734 + 0.5200 * texDetail) + 0.1950 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3772), 0.0, 1.0);
    float emit = atlas.a * (0.7749 + 0.4735 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1286 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.9698 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3633 + 0.3036 * presence + 0.4823 * pattern + 0.1552 * (1.0 - deepTrans) + 0.2172 * emit, 0.8577);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1568), 0.8577);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7521 + 0.2479 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1266), 0.0, 1.0);
