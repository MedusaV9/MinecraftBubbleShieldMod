...
void main() {
    ...
    mid = clamp(mid * (0.7020 + 0.6235 * texDetail) + 0.3242 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3699), 0.0, 1.0);
    float emit = atlas.a * (0.7903 + 0.3155 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1430 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.7179 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4251 + 0.4823 * presence + 0.4482 * pattern + 0.1341 * (1.0 - deepTrans) + 0.1363 * emit, 0.9413);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2045), 0.9413);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7493 + 0.2507 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1135), 0.0, 1.0);
