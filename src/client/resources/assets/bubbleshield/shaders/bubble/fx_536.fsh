...
void main() {
    ...
    mid = clamp(mid * (0.6370 + 0.4831 * texDetail) + 0.2626 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4113), 0.0, 1.0);
    float emit = atlas.a * (0.7347 + 0.4767 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1004 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.8818 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3665 + 0.4121 * presence + 0.4477 * pattern + 0.1289 * (1.0 - deepTrans) + 0.1656 * emit, 0.8445);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3106), 0.8445);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7033 + 0.2967 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
