...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6895, 0.0, 1.0);
    ...
    deepCol += 0.4217 * dp * mix(baseCol, deepStop, 0.2847);
    deepPat += 0.4217 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.3039) + par * 0.1362, time);
    deepCol += 0.1778 * dp * mix(baseCol, deepStop, 0.5693);
    deepPat += 0.1778 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.4715) + par * 0.2043, time);
    deepCol += 0.0750 * dp * mix(baseCol, deepStop, 0.8540);
    deepPat += 0.0750 * dp;
    deepCol *= 0.7011;
    deepPat = pow(clamp(deepPat * 0.5972, 0.0, 1.0), 1.1964);
    ...
    for (int i = 0; i < 4; i++) {
        float fi = float(i);
        float dens = clamp(fbm3(mdir * (1.0 + fi * 0.2601) + marchDir * (fi * 0.4532)) * 1.9774 - 0.6163, 0.0, 1.0);
        lightAcc += trans * dens * (1.0 - fi * 0.16);
        trans *= 1.0 - dens * 0.5670;
    }
    float mid = clamp(lightAcc * 1.0166 + (1.0 - trans) * 0.2006, 0.0, 1.25);
    ...
    float pattern = clamp(0.3795 * deepPat + 0.8481 * mid + 0.5336 * rim + flourish + grain, 0.0, 1.5);
    float gpos = clamp(pattern * 0.8208 + rim * 0.2743, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(1.1093 * mid + 0.6947 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.3220);
    rgb += hotStop * 0.1606 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0650 + 0.3105 * presence + 0.4615 * pattern, 0.8542);
