...
// stack: deep=caustic x4 taps | mid=volumecloud+warp2+pulse | rim=graze | flourish=glint | tex=fbm_turbulence
// fbm: ridged x4 octaves | seed c3195c1ebc2b2b4b
// v8 realism: refr 0.4883+0.4660*fres/dist (cap 0.1261) | energy 0.3497+0.4197*p | flow 0.085000 c/s | soft 1.0835m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (0.8959 * atlasPoleW));
    ...
    float refrAmp = min((0.4883 + 0.4660 * fresnel) / max(length(worldPos), 1.0), 0.1261);
