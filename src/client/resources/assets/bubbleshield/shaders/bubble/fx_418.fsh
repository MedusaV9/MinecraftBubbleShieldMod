...
void main() {
    ...
    mid = clamp(mid * (0.6356 + 0.7356 * texDetail) + 0.2010 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4400), 0.0, 1.0);
    float emit = atlas.a * (0.7027 + 0.2926 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0989 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8386 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4295 + 0.5142 * presence + 0.3544 * pattern + 0.1464 * (1.0 - deepTrans) + 0.1607 * emit, 0.9342);
