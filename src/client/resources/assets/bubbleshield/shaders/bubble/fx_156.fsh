...
void main() {
    ...
    mid = clamp(mid * (0.7425 + 0.6191 * texDetail) + 0.3427 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4507), 0.0, 1.0);
    float emit = atlas.a * (0.7074 + 0.4144 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1727 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.8446 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3689 + 0.4700 * presence + 0.3615 * pattern + 0.1381 * (1.0 - deepTrans) + 0.1987 * emit, 0.8871);
