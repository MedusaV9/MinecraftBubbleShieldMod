...
// stack: deep=fbm x2 taps | mid=thinfilm+warp2+flicker | rim=graze_sparkle | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.022500, -0.033333) * time, time);
    deep += 0.4958 * deepField(duvBase + rimDir * 0.0736 + vec2(0.032500, -0.053333) * time, time);
    deep = pow(clamp(deep * 0.6686, 0.0, 1.0), 1.3301);
    ...
    float pattern = clamp(0.4809 * deep + 0.7731 * mid + 0.8320 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.3760 + pattern * 0.6440);
    vec3 rgb = vertexColor.rgb * (0.4810 + 0.7514 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1806 + 0.7695 * pattern, 0.0, 1.0);
