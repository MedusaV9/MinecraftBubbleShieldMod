...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6627, 0.0, 1.0);
    ...
    deepCol += 0.4958 * dp * mix(baseCol, deepStop, 0.4037);
    deepPat += 0.4958 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 6.0737) + par * 0.1472, time);
    deepCol += 0.2458 * dp * mix(baseCol, deepStop, 0.8074);
    deepPat += 0.2458 * dp;
    deepCol *= 0.5603;
    deepPat = pow(clamp(deepPat * 0.5742, 0.0, 1.0), 1.3301);
    ...
    rgb = satLift(rgb, 1.1244);
    rgb += hotStop * 0.1936 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0538 + 0.3300 * presence + 0.3615 * pattern, 0.8271);
