...
// Bubble surface shader fx_414 -- family SHARDTESS
// stack: deep=caustic x3 taps | mid=shardtess+warp2+scroll | rim=graze_film | flourish=echo
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.6355 * (vec2(fbm2(p + vec2(0.0, t * 0.310000), per),
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.2819 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6744, 0.0, 1.0);
    ...
    deepCol += 0.4767 * dp * mix(baseCol, deepStop, 0.3552);
    deepPat += 0.4767 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.6724) + par * 0.1417, time);
    deepCol += 0.2273 * dp * mix(baseCol, deepStop, 0.7105);
    deepPat += 0.2273 * dp;
    deepCol *= 0.5820;
    deepPat = pow(clamp(deepPat * 0.5869, 0.0, 1.0), 1.1566);

    // [layer:mid:shardtess_warp2_scroll]
    // Signature structure of this effect, domain-warped and animated.
    vec2 auv = vec2(baseUV.x * 10.0000, baseUV.y * 10.0000) + vec2(0.241667, -0.200000) * time;
    vec2 wuv = warp2(auv, midPer, time);
    vec3 pane = voro2(wuv, midPer, 0.0);
    vec3 crack = voro2(wuv * 2.0 + vec2(17.3, 8.9), midPer * 2.0, 0.0);
    float lead = invsmooth(0.004, 0.0558, pane.x) + invsmooth(0.003, 0.0344, crack.x) * 0.55;
    float facet = floor(pane.z * 3.0000 + 0.5) / 3.0000;
    float glintPhase = 0.5 + 0.5 * sin(time * 0.633555 + pane.z * 6.2831853);
    float paneGlint = pow(glintPhase, 5.5730);
    float mid = clamp(lead * (0.55 + 0.45 * glintPhase) + facet * 0.3891 + paneGlint * 0.6301 * (1.0 - clamp(lead, 0.0, 1.0)), 0.0, 1.3);
    ...
    float grain = 0.0915 * (cellHash(floor(wuv * 30.0000) + vec2(floor(time * 6.0), 0.0), 300.0000) - 0.5);
    ...
    rgb += hotStop * 0.3017 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0444 + 0.3148 * presence + 0.4080 * pattern, 0.8169);
