...
void main() {
    ...
    mid = clamp(mid * (0.6723 + 0.7155 * texDetail) + 0.2340 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3926), 0.0, 1.0);
    float emit = atlas.a * (0.7550 + 0.4645 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1144 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.8593 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2935 + 0.3260 * presence + 0.5233 * pattern + 0.0977 * (1.0 - deepTrans) + 0.1628 * emit, 0.8769);
