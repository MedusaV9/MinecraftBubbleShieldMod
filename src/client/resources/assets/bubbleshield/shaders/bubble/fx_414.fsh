...
// stack: deep=caustic x3 taps | mid=emberstorm+warp2+scroll | rim=graze_film | flourish=echo | tex=fbm_turbulence
// fbm: ridged x4 octaves | seed 11d573fb76743048
// v8 realism: refr 0.2527+0.3508*fres/dist (cap 0.1141) | energy 0.5140+0.4356*p | flow 0.091667 c/s | soft 0.5546m
...
void main() {
    ...
    float refrAmp = min((0.2527 + 0.3508 * fresnel) / max(length(worldPos), 1.0), 0.1141);
