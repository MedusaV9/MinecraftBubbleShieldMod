...
void main() {
    ...
    mid = clamp(mid * (0.6791 + 0.7085 * texDetail) + 0.2802 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7923 + 0.2842 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0871 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.8205 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3675 + 0.4000 * presence + 0.3339 * pattern + 0.0856 * (1.0 - deepTrans) + 0.1654 * emit, 0.8760);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3368), 0.8760);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6607 + 0.3393 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
