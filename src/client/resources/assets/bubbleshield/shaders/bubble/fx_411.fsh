...
void main() {
    ...
    mid = clamp(mid * (0.6459 + 0.4903 * texDetail) + 0.2109 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4654), 0.0, 1.0);
    float emit = atlas.a * (0.5671 + 0.3045 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1229 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.8657 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3744 + 0.5009 * presence + 0.3571 * pattern + 0.1054 * (1.0 - deepTrans) + 0.1945 * emit, 0.9642);
