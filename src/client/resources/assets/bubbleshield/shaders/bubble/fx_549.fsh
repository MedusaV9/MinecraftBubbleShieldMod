...
void main() {
    ...
    mid = clamp(mid * (0.6247 + 0.5839 * texDetail) + 0.3481 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3521), 0.0, 1.0);
    float emit = atlas.a * (0.6135 + 0.4226 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1269 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.8337 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4282 + 0.4510 * presence + 0.3610 * pattern + 0.1284 * (1.0 - deepTrans) + 0.1624 * emit, 0.9565);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2744), 0.9565);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6534 + 0.3466 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0968), 0.0, 1.0);
