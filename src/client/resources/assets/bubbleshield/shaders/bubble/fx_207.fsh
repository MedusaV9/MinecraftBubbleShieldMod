...
void main() {
    ...
    mid = clamp(mid * (0.7165 + 0.7378 * texDetail) + 0.2752 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5374), 0.0, 1.0);
    float emit = atlas.a * (0.6962 + 0.4586 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0944 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7645 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4323 + 0.4875 * presence + 0.3606 * pattern + 0.0927 * (1.0 - deepTrans) + 0.1820 * emit, 0.9135);
