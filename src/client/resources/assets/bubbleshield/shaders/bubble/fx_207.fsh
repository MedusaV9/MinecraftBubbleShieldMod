...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8002, 0.0, 1.0);
    ...
    deepCol += 0.5180 * dp * mix(baseCol, deepStop, 0.2801);
    deepPat += 0.5180 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 3.1164) + par * 0.1498, time);
    deepCol += 0.2683 * dp * mix(baseCol, deepStop, 0.5602);
    deepPat += 0.2683 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 3.7788) + par * 0.2247, time);
    deepCol += 0.1390 * dp * mix(baseCol, deepStop, 0.8404);
    deepPat += 0.1390 * dp;
    deepCol *= 0.6325;
    deepPat = pow(clamp(deepPat * 0.5194, 0.0, 1.0), 1.2148);
    ...
    rgb = satLift(rgb, 1.2711);
    rgb += hotStop * 0.2339 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0695 + 0.3018 * presence + 0.3722 * pattern, 0.8135);
