...
void main() {
    ...
    mid = clamp(mid * (0.6866 + 0.4986 * texDetail) + 0.2478 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4396), 0.0, 1.0);
    float emit = atlas.a * (0.7702 + 0.3640 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0908 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.8006 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3599 + 0.3303 * presence + 0.5075 * pattern + 0.1011 * (1.0 - deepTrans) + 0.1425 * emit, 0.8761);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2993), 0.8761);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6489 + 0.3511 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0661), 0.0, 1.0);
