...
// stack: deep=voro x3 taps | mid=sparkle+curl+rotate | rim=graze_film | flourish=shimmer | tex=starfield
// fbm: ridged x5 octaves | seed 750a65c241279748
// v8 realism: refr 0.6877+0.8094*fres/dist (cap 0.1204) | energy 0.3741+0.4379*p | flow 0.119167 c/s | soft 0.9296m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.3853 + 0.2379, 0.0, 1.0);
    ...
    float refrAmp = min((0.6877 + 0.8094 * fresnel) / max(length(worldPos), 1.0), 0.1204);
    ...
    float emitMask = clamp(atlas.a * 1.9551, 0.0, 1.0);
    float emit = emitMask * (0.3943 + 0.6362 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1255 * (0.5 + 0.5 * sin(time * 0.225147))) * atlasPoleW;
