...
// stack: deep=voro x2 taps | mid=sparkle+curl+rotate | rim=graze_film | flourish=shimmer
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3525 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.481711);
...
void main() {
    ...
    deep += 0.3975 * deepField(duvBase + rimDir * 0.0710 + vec2(-0.048333, 0.037500) * time, time);
    deep = pow(clamp(deep * 0.7156, 0.0, 1.0), 1.0571);
    ...
    float pattern = clamp(0.4353 * deep + 0.9002 * mid + 0.6745 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7451 + pattern * 0.5435);
    vec3 rgb = vertexColor.rgb * (0.3626 + 0.8392 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2009 + 0.7426 * pattern, 0.0, 1.0);
