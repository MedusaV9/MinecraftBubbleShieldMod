...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7933, 0.0, 1.0);
    ...
    deepCol += 0.4632 * dp * mix(baseCol, deepStop, 0.2793);
    deepPat += 0.4632 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 4.9064) + par * 0.1411, time);
    deepCol += 0.2146 * dp * mix(baseCol, deepStop, 0.5586);
    deepPat += 0.2146 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 6.0850) + par * 0.2116, time);
    deepCol += 0.0994 * dp * mix(baseCol, deepStop, 0.8379);
    deepPat += 0.0994 * dp;
    deepCol *= 0.5988;
    deepPat = pow(clamp(deepPat * 0.5627, 0.0, 1.0), 1.2387);
    ...
    rgb += hotStop * 0.3474 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0602 + 0.2635 * presence + 0.3856 * pattern, 0.8573);
