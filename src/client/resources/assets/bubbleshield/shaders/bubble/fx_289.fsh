...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.2805 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.340339);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.006667, -0.002500) * time, time);
    deep += 0.4632 * deepField(duvBase + rimDir * 0.0705 + vec2(-0.010000, -0.002500) * time, time);
    deep += 0.2146 * deepField(duvBase + rimDir * 0.1411 + vec2(-0.013333, -0.002500) * time, time);
    deep += 0.0994 * deepField(duvBase + rimDir * 0.2116 + vec2(-0.016667, -0.002500) * time, time);
    deep = pow(clamp(deep * 0.5627, 0.0, 1.0), 1.2387);
    ...
    float pattern = clamp(0.4735 * deep + 0.7905 * mid + 0.5190 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7528 + pattern * 0.6595);
    vec3 rgb = vertexColor.rgb * (0.4695 + 0.6251 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1947 + 0.6137 * pattern, 0.0, 1.0);
