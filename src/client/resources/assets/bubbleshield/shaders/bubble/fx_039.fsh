...
// v8 realism: refr 1.0099+0.9563*fres/dist (cap 0.1884) | energy 0.4044+0.4378*p | flow 0.113333 c/s | soft 0.6689m
...
void main() {
    ...
    rim = clamp(rim + 0.8409 * rimLine + 0.4544 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1202 * caustic(wuv + vec2(3.1, 8.7), vec2(0.116667, -0.120000) * time, midPer);
    float grain = 0.0996 * (cellHash(floor(wuv * 28.0000) + vec2(floor(time * 6.0), 0.0), 112.0000) - 0.5);
    ...
    float motifEnv = 0.15 + 0.85 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.429351)) * (0.5 + 0.5 * sin(time * 0.481711 + 1.7)));
    float motifA = baseUV.x * 9.0000 + 1.0 * time * 0.105833 + 0.8648;
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 20.2076);
    float motifDy = (baseUV.y - 0.4864) * 15.9168;
    ...
    float refrAmp = min((1.0099 + 0.9563 * fresnel) / max(length(worldPos), 1.0), 0.1884);
    ...
    float energyW = clamp(0.4044 + 0.4378 * clamp(pattern, 0.0, 1.0) + 0.1563 * fresnel, 0.0, 0.8720);
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.1694) * 0.6648;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2871 * (1.0 - 0.5850 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.3649 + 0.5259 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1166 * (0.5 + 0.5 * sin(time * 0.293215))) * atlasPoleW;
    rgb += emit * 0.7818 * (1.0 - 0.7925 * baseLuma) * emitCol;
