...
// stack: deep=fbm x4 taps | mid=vortex+warp2+rotate | rim=graze_sparkle | flourish=shimmer | tex=marble_ink | motif=sigilx4+sparklegate
// fbm: turb x5 octaves | seed 1dfc1f5b36cc4d07
// v8 realism: refr 1.1237+1.0640*fres/dist (cap 0.1884) | energy 0.2870+0.4378*p | flow 0.113333 c/s | soft 0.6689m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9400) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9400);
        amplitude *= 0.5;
    }
    return value;
}
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.7286 * (vec2(fbm2(p + vec2(0.0, t * 0.310000), per),
...
vec2 warp2(vec2 p, vec2 per, float t) {
    ...
    return p + 0.3645 * (vec2(fbm2(q + vec2(1.7, 9.2), per),
...
vec2 spiralWarp(vec2 uv, float t) {
    ...
    float a = safeAtan(dv, sin(du * 6.2831853) * 0.5) + 3.7554 * r - t;
...
float rimGraze() {
    ...
    return smoothstep(0.0035, 0.0475, g);
}
...
float sparkle(vec2 p, float t, float px) {
    ...
    return step(0.7036, h) * invsmooth(0.02, 0.22, d) * tw;
}
...
void main() {
    ...
    vec3 spun = clamp(hueSpin(baseCol, 0.4761), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2247) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.4716, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.5783);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, -0.0161), 0.0, 1.0), 1.2764) * 0.4215, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(0.0487, 0.6556, -0.7535);
    vec3 par = Vt * 2.3878;
    ...
    float deepBreath = 1.0 + 0.0140 * sin(time * 1.911136);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * (2.6383 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.5603 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * (3.5842 * deepBreath)) + par * 0.0373, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.2935), deepStop, 0.1076);
    deepTrans *= 1.0 - 0.5603 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * (4.5302 * deepBreath)) + par * 0.0747, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.5870), deepStop, 0.2152);
    deepTrans *= 1.0 - 0.5603 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * (5.4762 * deepBreath)) + par * 0.1120, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.8805), deepStop, 0.3228);
    deepTrans *= 1.0 - 0.5603 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.7435;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0388, 0.0, 1.0), 1.3074);
    ...
    float sway = 0.6138 * sin(time * 0.146608);
    vec2 auv = vec2(baseUV.x * 4.0000 + (baseUV.y - 0.5) * sway + time * -0.013333, baseUV.y * 4.0000);
    vec2 wuv = warp2(auv, midPer, time);
    float latDist = 1.0 - abs(baseUV.y * 2.0 - 1.0);
    float twist = baseUV.x * 6.2831853 + latDist * 5.7525 - time * 0.984366;
    float band = smoothstep(0.15, 0.9, sin(twist * 4.0000));
    float arms = smoothstep(0.45, 0.95, sin(baseUV.x * 6.2831853 - latDist * 5.1034 + time * 0.691150));
    float eye = clamp(pow(clamp(1.0 - latDist, 0.0, 1.0), 3.0) * (0.6 + 0.4 * sin(time * 1.701696)), 0.0, 1.0);
    vec2 sv = spiralWarp(baseUV, time * 0.130900);
    // sv is periodic in u by construction, so the noise needs no wrap
    float mid = clamp(band * 0.8 + arms * 0.5 + eye + fbm2(sv * 3.8795, vec2(64.0000, 64.0000)) * 0.18, 0.0, 1.3);
    ...
    vec2 texUV = wuv * 3.0000 + vec2(0.023333, -0.072500) * time;
    ...
    vec3 hgtW = vec3(0.4199, 0.3836, 0.1964);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * 2.4374;
    ...
    float texDetail = clamp(atlas.r * 0.5337 + atlas.g * 0.4876 + atlas.b * 0.2497, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6044 + 0.4908 * texDetail, atlasPoleW) + 0.1726 * texDetail * (0.2041 + 0.7959 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.8057 * atlasPoleW));
    ...
    // thickness rho = 0.0689 (geo material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.7296.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9311 * 0.9311 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.7296);
    float chordN = min(chord / 0.0689, 3.0);
    ...
    float rimLine = smoothstep(0.0607, 0.0863, rimSlope);
    ...
    rim = clamp(rim + 0.8409 * rimLine + 0.6753 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1202 * caustic(wuv + vec2(3.1, 8.7), vec2(0.116667, -0.120000) * time, midPer);
    float grain = 0.0996 * (cellHash(floor(wuv * 28.0000) + vec2(floor(time * 2.0), 0.0), 112.0000) - 0.5);
    ...
    float motifEnv = 0.48 + 0.52 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.429351)) * (0.5 + 0.5 * sin(time * 0.481711 + 1.7)));
    float motifA = baseUV.x * 9.0000 + 1.0 * time * 0.105833 + 0.8648;
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 14.1384);
    float motifDy = (baseUV.y - 0.4864) * 11.0629;
    ...
    float pattern = clamp(0.3505 * deepPat * mix(1.0, chordN, 0.3131) + 0.9028 * mid + 0.7939 * rim + flourish + grain + 0.2123 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.9084 + rim * 0.1215, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9307 * mid + 0.5775 * rim, 0.0, 1.0);
    ...
    float texBody = 0.8238 + 0.4306 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4220 + 0.5780 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.1934);
    ...
    vec3 softHot = 1.0 - exp(-2.3203 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.3332 * smoothstep(0.55, 1.10, pattern));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.0697 + baseUV.y * 0.7002 + 0.8847) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1412);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 1.0296 + baseUV.y * 0.31 + 0.6973) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2323);
    ...
    rgb *= (1.0 - 0.3248 * washHi) * (1.0 - 0.2941 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.4094, 0.8505, -0.3301);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.5514) / 1.5514, 0.0, 1.0);
    rgb *= 0.6052 + 0.5441 * keyDiff * (1.0 - 0.5185 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 22.4304);
    rgb += hotStop * (keySpec * 0.4275 * (1.0 - 0.7138 * washHi));
    ...
    float refrAmp = min((1.1237 + 1.0640 * fresnel) / max(length(worldPos), 1.0), 0.1884);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.7365) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0613, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9387, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.4191 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.2744);
    float energyW = clamp(0.2870 + 0.4378 * clamp(pattern, 0.0, 1.0) + 0.2457 * fresnel, 0.0, 0.8720);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2587 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (1.0674 * (1.0 - 0.6930 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.1694) * 1.0291;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2871 * max(0.0, 1.0 - 0.5850 * baseLuma - 0.5920 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3649 + 0.5259 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1166 * (0.5 + 0.5 * sin(time * 0.293215))) * atlasPoleW;
    rgb += emit * 0.7818 * (1.0 - 0.7925 * baseLuma) * (1.0 - 0.5865 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2147 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.1444));
    ...
    float bodyAlpha = 0.4246 + 0.4218 * presence + 0.3490 * pattern + 0.1558 * (1.0 - deepTrans) + 0.2116 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.1933;
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 0.9864 + hgtC * 0.5017, 0.0, 1.0);
    ...
    innerScaf = clamp(innerScaf + 0.3439 * pow(clamp(motifM, 0.0, 1.0), 0.25), 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.2079);
    bodyAlpha *= 1.0 + innerFace * 0.1402;
    ...
    bodyAlpha *= smoothstep(0.0, 0.6689, sceneVDist - fragVDist);
