...
void main() {
    ...
    mid = clamp(mid * (0.6044 + 0.4908 * texDetail) + 0.3035 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3888), 0.0, 1.0);
    float emit = atlas.a * (0.6582 + 0.3132 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1166 * (0.5 + 0.5 * sin(time * 0.293215)));
    ...
    float alpha = vertexColor.a * min(0.4246 + 0.4218 * presence + 0.3490 * pattern + 0.1558 * (1.0 - deepTrans) + 0.2116 * emit, 0.8469);
