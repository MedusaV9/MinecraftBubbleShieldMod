...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6997, 0.0, 1.0);
    ...
    deepCol += 0.4066 * dp * mix(baseCol, deepStop, 0.1957);
    deepPat += 0.4066 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 4.5302) + par * 0.0747, time);
    deepCol += 0.1653 * dp * mix(baseCol, deepStop, 0.3913);
    deepPat += 0.1653 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.4762) + par * 0.1120, time);
    deepCol += 0.0672 * dp * mix(baseCol, deepStop, 0.5870);
    deepPat += 0.0672 * dp;
    deepCol *= 0.6524;
    deepPat = pow(clamp(deepPat * 0.6101, 0.0, 1.0), 1.3074);
    ...
    rgb += hotStop * 0.2832 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0726 + 0.2818 * presence + 0.3490 * pattern, 0.7869);
