...
void main() {
    ...
    mid = clamp(mid * (0.6179 + 0.5911 * texDetail) + 0.3225 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3527), 0.0, 1.0);
    float emit = atlas.a * (0.7405 + 0.2726 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1090 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.6827 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3087 + 0.2392 * presence + 0.6190 * pattern + 0.1499 * (1.0 - deepTrans) + 0.1608 * emit, 0.8705);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3243), 0.8705);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7187 + 0.2813 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1315), 0.0, 1.0);
