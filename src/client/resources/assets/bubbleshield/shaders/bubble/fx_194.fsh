...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1332) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1332);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    deep += 0.4478 * deepField(duvBase + rimDir * 0.0512 + vec2(-0.005000, -0.037500) * time, time);
    deep += 0.2005 * deepField(duvBase + rimDir * 0.1025 + vec2(-0.006667, -0.052500) * time, time);
    deep += 0.0898 * deepField(duvBase + rimDir * 0.1537 + vec2(-0.010000, -0.065000) * time, time);
    deep = pow(clamp(deep * 0.5753, 0.0, 1.0), 1.0108);
    ...
    vec2 wuv = auv;
    vec2 adv = wuv + 0.2081 * curl2(wuv + vec2(0.0, time * 0.062500), midPer);
    float smoke = fbm2(adv + vec2(time * 0.050000, -time * 0.104167), midPer);
