...
void main() {
    ...
    mid = clamp(mid * (0.6037 + 0.5705 * texDetail) + 0.2962 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4082), 0.0, 1.0);
    float emit = atlas.a * (0.6330 + 0.2680 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1697 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.8148 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3885 + 0.4141 * presence + 0.4632 * pattern + 0.1356 * (1.0 - deepTrans) + 0.1448 * emit, 0.8956);
