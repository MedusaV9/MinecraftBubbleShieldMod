...
void main() {
    ...
    mid = clamp(mid * (0.7418 + 0.7361 * texDetail) + 0.2149 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3730), 0.0, 1.0);
    float emit = atlas.a * (0.7514 + 0.4195 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0989 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.8661 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4168 + 0.4792 * presence + 0.4723 * pattern + 0.1466 * (1.0 - deepTrans) + 0.1299 * emit, 0.9283);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2221), 0.9283);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6668 + 0.3332 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
