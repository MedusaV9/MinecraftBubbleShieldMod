...
void main() {
    ...
    mid = clamp(mid * (0.7032 + 0.5240 * texDetail) + 0.2273 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6939 + 0.2782 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0808 * (0.5 + 0.5 * sin(time * 0.099484)));
    rgb += emit * 0.8416 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4350 + 0.4446 * presence + 0.4414 * pattern + 0.1110 * (1.0 - deepTrans) + 0.2099 * emit, 0.9507);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2260), 0.9507);
    // [layer:v5:ghostalpha]
