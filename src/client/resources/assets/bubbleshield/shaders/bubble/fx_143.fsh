...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8267, 0.0, 1.0);
    ...
    deepCol += 0.4589 * dp * mix(baseCol, deepStop, 0.2393);
    deepPat += 0.4589 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 5.8453) + par * 0.0949, time);
    deepCol += 0.2106 * dp * mix(baseCol, deepStop, 0.4785);
    deepPat += 0.2106 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 7.3866) + par * 0.1424, time);
    deepCol += 0.0967 * dp * mix(baseCol, deepStop, 0.7178);
    deepPat += 0.0967 * dp;
    deepCol *= 0.7337;
    deepPat = pow(clamp(deepPat * 0.5662, 0.0, 1.0), 1.0528);
    ...
    rgb += hotStop * 0.2855 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0485 + 0.2509 * presence + 0.3691 * pattern, 0.8522);
