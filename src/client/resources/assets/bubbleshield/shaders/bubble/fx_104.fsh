...
void main() {
    ...
    mid = clamp(mid * (0.6536 + 0.5674 * texDetail) + 0.2039 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4024), 0.0, 1.0);
    float emit = atlas.a * (0.6602 + 0.2582 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0931 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.6940 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3426 + 0.4715 * presence + 0.3337 * pattern + 0.1536 * (1.0 - deepTrans) + 0.2039 * emit, 0.8573);
