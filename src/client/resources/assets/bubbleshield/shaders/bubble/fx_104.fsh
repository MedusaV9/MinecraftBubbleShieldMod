...
// stack: deep=fbm x2 taps | mid=interference+warp2+rotate | rim=lat | flourish=glint
...
void main() {
    ...
    deep += 0.3870 * deepField(duvBase + rimDir * 0.0432 + vec2(0.066667, 0.045000) * time, time);
    deep = pow(clamp(deep * 0.7210, 0.0, 1.0), 1.7962);
    ...
    float pattern = clamp(0.4009 * deep + 0.8336 * mid + 0.8186 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8548 + pattern * 0.3419);
    vec3 rgb = vertexColor.rgb * (0.4472 + 0.5810 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2487 + 0.7934 * pattern, 0.0, 1.0);
