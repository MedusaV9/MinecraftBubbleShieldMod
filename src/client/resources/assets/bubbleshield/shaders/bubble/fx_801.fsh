...
void main() {
    ...
    mid = clamp(mid * (0.7341 + 0.6464 * texDetail) + 0.2756 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4302), 0.0, 1.0);
    float emit = atlas.a * (0.6193 + 0.2518 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1723 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.9108 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3840 + 0.4039 * presence + 0.3424 * pattern + 0.1430 * (1.0 - deepTrans) + 0.1337 * emit, 0.8684);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3269), 0.8684);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6856 + 0.3144 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1364), 0.0, 1.0);
