...
void main() {
    ...
    mid = clamp(mid * (0.6311 + 0.5146 * texDetail) + 0.3100 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3795), 0.0, 1.0);
    float emit = atlas.a * (0.7640 + 0.2901 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1743 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.6603 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3733 + 0.4234 * presence + 0.3688 * pattern + 0.1016 * (1.0 - deepTrans) + 0.1466 * emit, 0.9044);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1755), 0.9044);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7244 + 0.2756 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
