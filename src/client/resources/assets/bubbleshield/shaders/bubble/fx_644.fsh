...
void main() {
    ...
    mid = clamp(mid * (0.6200 + 0.7412 * texDetail) + 0.2184 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7460 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4813), 0.0, 1.0);
    float emit = atlas.a * (0.5612 + 0.4481 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1443 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.7143 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3555 + 0.2915 * presence + 0.5553 * pattern + 0.1345 * (1.0 - deepTrans) + 0.1514 * emit, 0.8668);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2442), 0.8668);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7384 + 0.2616 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
