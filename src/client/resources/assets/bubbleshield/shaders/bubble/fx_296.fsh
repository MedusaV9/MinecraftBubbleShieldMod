...
void main() {
    ...
    mid = clamp(mid * (0.6644 + 0.7309 * texDetail) + 0.2953 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5068), 0.0, 1.0);
    float emit = atlas.a * (0.6542 + 0.4918 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0985 * (0.5 + 0.5 * sin(time * 0.235619)));
    ...
    float alpha = vertexColor.a * min(0.4189 + 0.5308 * presence + 0.4487 * pattern + 0.1545 * (1.0 - deepTrans) + 0.2091 * emit, 0.8857);
