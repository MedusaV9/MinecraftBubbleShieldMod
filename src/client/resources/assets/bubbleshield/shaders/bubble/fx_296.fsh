...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5974, 0.0, 1.0);
    ...
    deepCol += 0.5511 * dp * mix(baseCol, deepStop, 0.2607);
    deepPat += 0.5511 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.0575) + par * 0.1307, time);
    deepCol += 0.3037 * dp * mix(baseCol, deepStop, 0.5214);
    deepPat += 0.3037 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.1885) + par * 0.1961, time);
    deepCol += 0.1673 * dp * mix(baseCol, deepStop, 0.7821);
    deepPat += 0.1673 * dp;
    deepCol *= 0.4967;
    deepPat = pow(clamp(deepPat * 0.4945, 0.0, 1.0), 1.5972);
    ...
    float midCover = clamp(0.9738 * mid + 0.6055 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.2165);
    rgb += hotStop * 0.1724 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0609 + 0.3321 * presence + 0.4597 * pattern, 0.7857);
