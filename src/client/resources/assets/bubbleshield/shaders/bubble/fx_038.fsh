...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6828, 0.0, 1.0);
    ...
    deepCol += 0.4489 * dp * mix(baseCol, deepStop, 0.1878);
    deepPat += 0.4489 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 4.8463) + par * 0.1308, time);
    deepCol += 0.2015 * dp * mix(baseCol, deepStop, 0.3756);
    deepPat += 0.2015 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.9561) + par * 0.1962, time);
    deepCol += 0.0905 * dp * mix(baseCol, deepStop, 0.5633);
    deepPat += 0.0905 * dp;
    deepCol *= 0.6781;
    deepPat = pow(clamp(deepPat * 0.5744, 0.0, 1.0), 1.5982);
    ...
    rgb += hotStop * 0.2106 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0645 + 0.3307 * presence + 0.4697 * pattern, 0.7860);
