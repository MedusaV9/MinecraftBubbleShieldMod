...
void main() {
    ...
    mid = clamp(mid * (0.7384 + 0.5049 * texDetail) + 0.2404 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4228), 0.0, 1.0);
    float emit = atlas.a * (0.7388 + 0.3081 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0873 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.6657 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4004 + 0.4707 * presence + 0.4697 * pattern + 0.1089 * (1.0 - deepTrans) + 0.1310 * emit, 0.8460);
