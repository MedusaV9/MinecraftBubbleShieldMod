...
void main() {
    ...
    mid = clamp(mid * (0.6136 + 0.5416 * texDetail) + 0.3414 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5097), 0.0, 1.0);
    float emit = atlas.a * (0.7194 + 0.4940 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1000 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.8488 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3181 + 0.2853 * presence + 0.4742 * pattern + 0.0886 * (1.0 - deepTrans) + 0.1698 * emit, 0.9014);
