...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5709, 0.0, 1.0);
    ...
    deepCol += 0.5255 * dp * mix(baseCol, deepStop, 0.3015);
    deepPat += 0.5255 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.2540) + par * 0.0953, time);
    deepCol += 0.2762 * dp * mix(baseCol, deepStop, 0.6029);
    deepPat += 0.2762 * dp;
    deepCol *= 0.6662;
    deepPat = pow(clamp(deepPat * 0.5550, 0.0, 1.0), 1.4241);
    ...
    rgb += hotStop * 0.1665 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0655 + 0.3564 * presence + 0.4777 * pattern, 0.8013);
