...
void main() {
    ...
    mid = clamp(mid * (0.7100 + 0.5401 * texDetail) + 0.3107 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4514), 0.0, 1.0);
    float emit = atlas.a * (0.7696 + 0.2640 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1413 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.7770 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4037 + 0.4964 * presence + 0.4777 * pattern + 0.1107 * (1.0 - deepTrans) + 0.1247 * emit, 0.8613);
