...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.011667, 0.020000) * time, time);
    deep += 0.5255 * deepField(duvBase + rimDir * 0.0477 + vec2(-0.020000, 0.032500) * time, time);
    deep += 0.2762 * deepField(duvBase + rimDir * 0.0953 + vec2(-0.026667, 0.045000) * time, time);
    deep = pow(clamp(deep * 0.5550, 0.0, 1.0), 1.4241);
    ...
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.150000, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.086667), midPer)), midPer);
