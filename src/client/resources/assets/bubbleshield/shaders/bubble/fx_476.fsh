...
void main() {
    ...
    mid = clamp(mid * (0.6806 + 0.6644 * texDetail) + 0.1935 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4098), 0.0, 1.0);
    float emit = atlas.a * (0.5678 + 0.3895 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1414 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.9997 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4158 + 0.4147 * presence + 0.3401 * pattern + 0.1470 * (1.0 - deepTrans) + 0.2112 * emit, 0.8476);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1948), 0.8476);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7167 + 0.2833 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1255), 0.0, 1.0);
