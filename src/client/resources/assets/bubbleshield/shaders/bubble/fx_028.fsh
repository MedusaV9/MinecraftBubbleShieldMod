...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6948, 0.0, 1.0);
    ...
    deepCol += 0.4932 * dp * mix(baseCol, deepStop, 0.3407);
    deepPat += 0.4932 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.1167) + par * 0.1520, time);
    deepCol += 0.2433 * dp * mix(baseCol, deepStop, 0.6815);
    deepPat += 0.2433 * dp;
    deepCol *= 0.6540;
    deepPat = pow(clamp(deepPat * 0.5759, 0.0, 1.0), 1.7176);
    ...
    rgb += hotStop * 0.2148 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0591 + 0.3631 * presence + 0.3738 * pattern, 0.7938);
