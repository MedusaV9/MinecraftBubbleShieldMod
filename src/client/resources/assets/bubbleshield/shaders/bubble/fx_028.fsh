...
void main() {
    ...
    mid = clamp(mid * (0.6148 + 0.6956 * texDetail) + 0.3077 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4640), 0.0, 1.0);
    float emit = atlas.a * (0.7381 + 0.2861 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1016 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.6658 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3847 + 0.5031 * presence + 0.3738 * pattern + 0.1074 * (1.0 - deepTrans) + 0.1842 * emit, 0.8538);
