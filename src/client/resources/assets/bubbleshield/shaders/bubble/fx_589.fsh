...
void main() {
    ...
    mid = clamp(mid * (0.6311 + 0.5991 * texDetail) + 0.3173 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4771), 0.0, 1.0);
    float emit = atlas.a * (0.7533 + 0.4735 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1019 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.8956 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3557 + 0.4838 * presence + 0.4505 * pattern + 0.1045 * (1.0 - deepTrans) + 0.2160 * emit, 0.9404);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2540), 0.9404);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7578 + 0.2422 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1020), 0.0, 1.0);
