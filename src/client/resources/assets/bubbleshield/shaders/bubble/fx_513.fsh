...
void main() {
    ...
    mid = clamp(mid * (0.6554 + 0.5329 * texDetail) + 0.2009 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5049), 0.0, 1.0);
    float emit = atlas.a * (0.7263 + 0.3991 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1029 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9863 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3043 + 0.2713 * presence + 0.4650 * pattern + 0.1485 * (1.0 - deepTrans) + 0.1734 * emit, 0.8485);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2046), 0.8485);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7266 + 0.2734 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0772), 0.0, 1.0);
