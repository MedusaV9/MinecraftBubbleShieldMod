...
void main() {
    ...
    mid = clamp(mid * (0.6921 + 0.5200 * texDetail) + 0.2198 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4214), 0.0, 1.0);
    float emit = atlas.a * (0.6473 + 0.4687 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1656 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9194 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3954 + 0.3966 * presence + 0.4362 * pattern + 0.1180 * (1.0 - deepTrans) + 0.2105 * emit, 0.8592);
