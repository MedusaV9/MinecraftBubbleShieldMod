...
void main() {
    ...
    mid = clamp(mid * (0.6618 + 0.5644 * texDetail) + 0.2887 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4617), 0.0, 1.0);
    float emit = atlas.a * (0.7984 + 0.3700 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1513 * (0.5 + 0.5 * sin(time * 0.225147)));
    ...
    float alpha = vertexColor.a * min(0.3434 + 0.3556 * presence + 0.5166 * pattern + 0.0936 * (1.0 - deepTrans) + 0.1657 * emit, 0.9156);
