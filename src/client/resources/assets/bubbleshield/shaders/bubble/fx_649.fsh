...
void main() {
    ...
    mid = clamp(mid * (0.6116 + 0.7236 * texDetail) + 0.2260 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5293), 0.0, 1.0);
    float emit = atlas.a * (0.7065 + 0.4063 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1703 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.8209 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4687 + 0.5259 * presence + 0.4091 * pattern + 0.1315 * (1.0 - deepTrans) + 0.1748 * emit, 0.9324);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2141), 0.9324);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6306 + 0.3694 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1215), 0.0, 1.0);
