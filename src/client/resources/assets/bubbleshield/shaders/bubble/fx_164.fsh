...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7486, 0.0, 1.0);
    ...
    deepCol += 0.5032 * dp * mix(baseCol, deepStop, 0.2439);
    deepPat += 0.5032 * dp;
    dp = deepField(rotA(spinAxis, time * 0.094248) * (sdir * 4.7920) + par * 0.0865, time);
    deepCol += 0.2532 * dp * mix(baseCol, deepStop, 0.4877);
    deepPat += 0.2532 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 5.8243) + par * 0.1298, time);
    deepCol += 0.1274 * dp * mix(baseCol, deepStop, 0.7316);
    deepPat += 0.1274 * dp;
    deepCol *= 0.6832;
    deepPat = pow(clamp(deepPat * 0.5308, 0.0, 1.0), 1.2403);
    ...
    float midCover = clamp(0.9183 * mid + 0.6456 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.1034);
    rgb += hotStop * 0.2763 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0671 + 0.3511 * presence + 0.3977 * pattern, 0.8334);
