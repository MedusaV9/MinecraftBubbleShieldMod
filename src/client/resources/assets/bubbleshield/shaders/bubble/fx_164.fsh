...
void main() {
    ...
    mid = clamp(mid * (0.6941 + 0.4767 * texDetail) + 0.2257 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4896), 0.0, 1.0);
    float emit = atlas.a * (0.7174 + 0.4412 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1438 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8065 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4350 + 0.5455 * presence + 0.4004 * pattern + 0.1275 * (1.0 - deepTrans) + 0.1243 * emit, 0.9334);
