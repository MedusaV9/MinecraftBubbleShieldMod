...
void main() {
    ...
    mid = clamp(mid * (0.7118 + 0.4791 * texDetail) + 0.2397 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5115), 0.0, 1.0);
    float emit = atlas.a * (0.5948 + 0.2606 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1612 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.7030 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4413 + 0.4866 * presence + 0.3227 * pattern + 0.0834 * (1.0 - deepTrans) + 0.2192 * emit, 0.8712);
