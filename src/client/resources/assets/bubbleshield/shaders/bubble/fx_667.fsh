...
void main() {
    ...
    mid = clamp(mid * (0.6551 + 0.5647 * texDetail) + 0.3345 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5147), 0.0, 1.0);
    float emit = atlas.a * (0.7375 + 0.4945 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0882 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.9782 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3529 + 0.3360 * presence + 0.3964 * pattern + 0.1575 * (1.0 - deepTrans) + 0.1413 * emit, 0.8018);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3019), 0.8018);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6772 + 0.3228 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1312), 0.0, 1.0);
