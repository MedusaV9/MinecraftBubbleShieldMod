...
void main() {
    ...
    mid = clamp(mid * (0.6781 + 0.6466 * texDetail) + 0.2674 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3660), 0.0, 1.0);
    float emit = atlas.a * (0.7790 + 0.3095 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1540 * (0.5 + 0.5 * sin(time * 0.214675)));
    ...
    float alpha = vertexColor.a * min(0.3281 + 0.3501 * presence + 0.4343 * pattern + 0.1445 * (1.0 - deepTrans) + 0.1606 * emit, 0.8969);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2725), 0.8969);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6881 + 0.3119 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1394), 0.0, 1.0);
