...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8379, 0.0, 1.0);
    ...
    deepCol += 0.4440 * dp * mix(baseCol, deepStop, 0.4436);
    deepPat += 0.4440 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.1546) + par * 0.1083, time);
    deepCol += 0.1972 * dp * mix(baseCol, deepStop, 0.8873);
    deepPat += 0.1972 * dp;
    deepCol *= 0.7012;
    deepPat = pow(clamp(deepPat * 0.6093, 0.0, 1.0), 1.0409);
    ...
    rgb += hotStop * 0.2703 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0549 + 0.2679 * presence + 0.4231 * pattern, 0.7827);
