...
void main() {
    ...
    mid = clamp(mid * (0.6556 + 0.6287 * texDetail) + 0.2448 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3702), 0.0, 1.0);
    float emit = atlas.a * (0.6232 + 0.4213 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1090 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.6549 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3723 + 0.4079 * presence + 0.4231 * pattern + 0.1360 * (1.0 - deepTrans) + 0.1420 * emit, 0.8427);
