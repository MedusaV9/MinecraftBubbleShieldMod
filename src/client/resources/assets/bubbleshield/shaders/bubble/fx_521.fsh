...
void main() {
    ...
    mid = clamp(mid * (0.6150 + 0.6422 * texDetail) + 0.3126 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4781), 0.0, 1.0);
    float emit = atlas.a * (0.5643 + 0.3954 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1366 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.8910 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4195 + 0.3835 * presence + 0.3585 * pattern + 0.0829 * (1.0 - deepTrans) + 0.1316 * emit, 0.8913);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2955), 0.8913);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7624 + 0.2376 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1334), 0.0, 1.0);
