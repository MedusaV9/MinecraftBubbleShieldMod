...
void main() {
    ...
    mid = clamp(mid * (0.7034 + 0.5606 * texDetail) + 0.3313 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5179), 0.0, 1.0);
    float emit = atlas.a * (0.7623 + 0.3466 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1193 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.9044 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4091 + 0.3968 * presence + 0.3529 * pattern + 0.1332 * (1.0 - deepTrans) + 0.2056 * emit, 0.8674);
