...
// stack: deep=voro x2 taps | mid=petals+warp1+rotate | rim=graze_sparkle | flourish=swirl
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3842 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.013333, 0.012500) * time, time);
    deep += 0.4276 * deepField(duvBase + rimDir * 0.0544 + vec2(-0.021667, 0.017500) * time, time);
    deep = pow(clamp(deep * 0.7005, 0.0, 1.0), 1.0381);
    ...
    float pattern = clamp(0.4663 * deep + 0.9581 * mid + 0.8764 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.0845 + pattern * 0.6882);
    vec3 rgb = vertexColor.rgb * (0.5135 + 0.5787 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1958 + 0.7768 * pattern, 0.0, 1.0);
