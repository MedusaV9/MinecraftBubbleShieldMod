...
void main() {
    ...
    mid = clamp(mid * (0.6279 + 0.7036 * texDetail) + 0.2034 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4519), 0.0, 1.0);
    float emit = atlas.a * (0.6284 + 0.3482 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1329 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.8824 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4389 + 0.4960 * presence + 0.4714 * pattern + 0.1010 * (1.0 - deepTrans) + 0.1538 * emit, 0.8696);
