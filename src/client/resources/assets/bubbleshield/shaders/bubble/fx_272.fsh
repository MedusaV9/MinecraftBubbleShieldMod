...
void main() {
    ...
    mid = clamp(mid * (0.6947 + 0.4732 * texDetail) + 0.2934 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3780), 0.0, 1.0);
    float emit = atlas.a * (0.7366 + 0.4364 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0861 * (0.5 + 0.5 * sin(time * 0.109956)));
    ...
    float alpha = vertexColor.a * min(0.4107 + 0.4987 * presence + 0.4138 * pattern + 0.1431 * (1.0 - deepTrans) + 0.2043 * emit, 0.8936);
