...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6316, 0.0, 1.0);
    ...
    deepCol += 0.5744 * dp * mix(baseCol, deepStop, 0.2309);
    deepPat += 0.5744 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 4.4146) + par * 0.0829, time);
    deepCol += 0.3299 * dp * mix(baseCol, deepStop, 0.4617);
    deepPat += 0.3299 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 5.6037) + par * 0.1244, time);
    deepCol += 0.1895 * dp * mix(baseCol, deepStop, 0.6926);
    deepPat += 0.1895 * dp;
    deepCol *= 0.4913;
    deepPat = pow(clamp(deepPat * 0.4776, 0.0, 1.0), 1.1617);
    ...
    rgb = satLift(rgb, 1.2264);
    rgb += hotStop * 0.2403 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0679 + 0.3587 * presence + 0.4138 * pattern, 0.8336);
