...
void main() {
    ...
    mid = clamp(mid * (0.6653 + 0.5221 * texDetail) + 0.1996 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4164), 0.0, 1.0);
    float emit = atlas.a * (0.6111 + 0.4096 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1446 * (0.5 + 0.5 * sin(time * 0.209440)));
    ...
    float alpha = vertexColor.a * min(0.3078 + 0.3259 * presence + 0.4902 * pattern + 0.1259 * (1.0 - deepTrans) + 0.2198 * emit, 0.9132);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3180), 0.9132);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0875), 0.0, 1.0);
