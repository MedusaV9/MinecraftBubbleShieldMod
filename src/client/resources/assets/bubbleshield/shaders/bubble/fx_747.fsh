...
void main() {
    ...
    mid = clamp(mid * (0.6821 + 0.4710 * texDetail) + 0.3408 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4292), 0.0, 1.0);
    float emit = atlas.a * (0.7037 + 0.4687 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1279 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.8426 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3091 + 0.3403 * presence + 0.4924 * pattern + 0.1257 * (1.0 - deepTrans) + 0.1299 * emit, 0.8476);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1855), 0.8476);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0716), 0.0, 1.0);
