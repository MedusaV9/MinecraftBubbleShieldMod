...
// stack: deep=voro x4 taps | mid=irisfilm+none+flicker | rim=graze | flourish=echo | tex=chrome
// fbm: ridged x5 octaves | seed 1ae64b5956d9dc65
// v8 realism: refr 1.0302+1.5369*fres/dist (cap 0.1337) | energy 0.2690+0.4472*p | flow 0.087500 c/s | soft 0.6507m
...
void main() {
    ...
    float refrAmp = min((1.0302 + 1.5369 * fresnel) / max(length(worldPos), 1.0), 0.1337);
