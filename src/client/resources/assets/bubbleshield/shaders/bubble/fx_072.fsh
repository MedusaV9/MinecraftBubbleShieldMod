...
// stack: deep=voro x3 taps | mid=aurora+warp2+scroll | rim=graze | flourish=glint
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3212 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.403171);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.046667, -0.025000) * time, time);
    deep += 0.5641 * deepField(duvBase + rimDir * 0.0608 + vec2(-0.073333, -0.037500) * time, time);
    deep += 0.3182 * deepField(duvBase + rimDir * 0.1217 + vec2(-0.098333, -0.052500) * time, time);
    deep = pow(clamp(deep * 0.5313, 0.0, 1.0), 1.2139);
    ...
    float pattern = clamp(0.3182 * deep + 0.7147 * mid + 0.6514 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2567 + pattern * 0.3410);
    vec3 rgb = vertexColor.rgb * (0.4358 + 0.7625 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2074 + 0.7215 * pattern, 0.0, 1.0);
