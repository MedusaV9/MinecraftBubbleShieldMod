...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5789, 0.0, 1.0);
    ...
    deepCol += 0.5641 * dp * mix(baseCol, deepStop, 0.2280);
    deepPat += 0.5641 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.7333) + par * 0.1217, time);
    deepCol += 0.3182 * dp * mix(baseCol, deepStop, 0.4559);
    deepPat += 0.3182 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.6230) + par * 0.1825, time);
    deepCol += 0.1795 * dp * mix(baseCol, deepStop, 0.6839);
    deepPat += 0.1795 * dp;
    deepCol *= 0.4970;
    deepPat = pow(clamp(deepPat * 0.4850, 0.0, 1.0), 1.2139);
    ...
    rgb += hotStop * 0.1777 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0625 + 0.2839 * presence + 0.4175 * pattern, 0.8482);
