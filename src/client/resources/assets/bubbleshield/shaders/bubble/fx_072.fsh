...
void main() {
    ...
    mid = clamp(mid * (0.7251 + 0.6567 * texDetail) + 0.3455 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4420), 0.0, 1.0);
    float emit = atlas.a * (0.6606 + 0.4899 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1358 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7113 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3946 + 0.4239 * presence + 0.4175 * pattern + 0.1280 * (1.0 - deepTrans) + 0.1205 * emit, 0.9082);
