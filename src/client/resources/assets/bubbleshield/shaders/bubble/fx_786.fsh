...
void main() {
    ...
    mid = clamp(mid * (0.6285 + 0.6293 * texDetail) + 0.1804 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5149), 0.0, 1.0);
    float emit = atlas.a * (0.6216 + 0.2567 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1200 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.9434 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4166 + 0.4219 * presence + 0.4541 * pattern + 0.0983 * (1.0 - deepTrans) + 0.1492 * emit, 0.8685);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1816), 0.8685);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6682 + 0.3318 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1096), 0.0, 1.0);
