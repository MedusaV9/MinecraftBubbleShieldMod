...
void main() {
    ...
    mid = clamp(mid * (0.7338 + 0.7461 * texDetail) + 0.2010 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4456), 0.0, 1.0);
    float emit = atlas.a * (0.6587 + 0.3470 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1782 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8391 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4283 + 0.5006 * presence + 0.3598 * pattern + 0.1501 * (1.0 - deepTrans) + 0.1736 * emit, 0.9314);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2132), 0.9314);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6854 + 0.3146 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0656), 0.0, 1.0);
