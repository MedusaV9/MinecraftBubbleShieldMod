...
void main() {
    ...
    mid = clamp(mid * (0.6925 + 0.4555 * texDetail) + 0.3441 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5404), 0.0, 1.0);
    float emit = atlas.a * (0.7553 + 0.4798 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1598 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.9095 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3584 + 0.4593 * presence + 0.3552 * pattern + 0.1523 * (1.0 - deepTrans) + 0.1899 * emit, 0.8960);
