...
void main() {
    ...
    mid = clamp(mid * (0.7494 + 0.5732 * texDetail) + 0.2067 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4498), 0.0, 1.0);
    float emit = atlas.a * (0.7737 + 0.3253 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1290 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.9281 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3387 + 0.2514 * presence + 0.5012 * pattern + 0.1549 * (1.0 - deepTrans) + 0.2059 * emit, 0.9032);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2594), 0.9032);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7262 + 0.2738 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0638), 0.0, 1.0);
