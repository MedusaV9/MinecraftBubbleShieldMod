...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9124) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9124);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.005000, 0.023333) * time, time);
    deep += 0.4136 * deepField(duvBase + rimDir * 0.0483 + vec2(-0.007500, 0.036667) * time, time);
    deep += 0.1711 * deepField(duvBase + rimDir * 0.0967 + vec2(-0.012500, 0.050000) * time, time);
    deep += 0.0708 * deepField(duvBase + rimDir * 0.1450 + vec2(-0.015000, 0.060000) * time, time);
    deep = pow(clamp(deep * 0.6041, 0.0, 1.0), 1.4140);
