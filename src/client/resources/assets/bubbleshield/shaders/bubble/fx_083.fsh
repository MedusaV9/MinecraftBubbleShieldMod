...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6416, 0.0, 1.0);
    ...
    deepCol += 0.4136 * dp * mix(baseCol, deepStop, 0.2166);
    deepPat += 0.4136 * dp;
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * 3.9847) + par * 0.0967, time);
    deepCol += 0.1711 * dp * mix(baseCol, deepStop, 0.4332);
    deepPat += 0.1711 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 4.9056) + par * 0.1450, time);
    deepCol += 0.0708 * dp * mix(baseCol, deepStop, 0.6499);
    deepPat += 0.0708 * dp;
    deepCol *= 0.6189;
    deepPat = pow(clamp(deepPat * 0.6041, 0.0, 1.0), 1.4140);
    ...
    rgb += hotStop * 0.3043 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0738 + 0.3637 * presence + 0.3784 * pattern, 0.7978);
