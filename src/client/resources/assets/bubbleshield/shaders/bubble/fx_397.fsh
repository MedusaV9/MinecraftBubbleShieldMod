...
void main() {
    ...
    mid = clamp(mid * (0.7421 + 0.6036 * texDetail) + 0.2873 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.6927 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5037), 0.0, 1.0);
    float emit = atlas.a * (0.6237 + 0.3868 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0882 * (0.5 + 0.5 * sin(time * 0.109956)));
    ...
    float alpha = vertexColor.a * min(0.3483 + 0.2834 * presence + 0.5137 * pattern + 0.1449 * (1.0 - deepTrans) + 0.1557 * emit, 0.9392);
