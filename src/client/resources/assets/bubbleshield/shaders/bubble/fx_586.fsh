...
void main() {
    ...
    mid = clamp(mid * (0.7290 + 0.4717 * texDetail) + 0.2278 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4722), 0.0, 1.0);
    float emit = atlas.a * (0.5962 + 0.2947 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1054 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.9202 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3296 + 0.4263 * presence + 0.3468 * pattern + 0.1541 * (1.0 - deepTrans) + 0.2033 * emit, 0.8452);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1666), 0.8452);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7499 + 0.2501 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
