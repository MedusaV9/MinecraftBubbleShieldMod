...
void main() {
    ...
    mid = clamp(mid * (0.6577 + 0.5463 * texDetail) + 0.2924 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4970), 0.0, 1.0);
    float emit = atlas.a * (0.7879 + 0.4491 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0812 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.9888 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4347 + 0.5537 * presence + 0.2798 * pattern + 0.1351 * (1.0 - deepTrans) + 0.1238 * emit, 0.9112);
