...
void main() {
    ...
    mid = clamp(mid * (0.6048 + 0.6618 * texDetail) + 0.2402 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5039), 0.0, 1.0);
    float emit = atlas.a * (0.7260 + 0.2615 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1320 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8295 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2615 + 0.2863 * presence + 0.5554 * pattern + 0.0993 * (1.0 - deepTrans) + 0.1676 * emit, 0.8268);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2887), 0.8268);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7455 + 0.2545 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0909), 0.0, 1.0);
