...
// stack: deep=fbm x3 taps | mid=holoparallax+curl+flicker | rim=lat | flourish=echo
...
void main() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        hpAcc += (hpGrid * 0.5500 + hpScan * 0.1248) * (1.0 - fi * 0.1825);
    }
    ...
    float mid = clamp(hpAcc * hpFlick + hpSync * 0.4937, 0.0, 1.25);
    ...
    float alpha = vertexColor.a * min(0.0784 + 0.1074 * presence + 0.4939 * pattern + 0.0817 * (1.0 - deepTrans), 0.8131);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2694), 0.8131);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5409 + 0.4591 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
