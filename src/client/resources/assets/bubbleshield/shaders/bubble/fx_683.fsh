...
void main() {
    ...
    mid = clamp(mid * (0.7184 + 0.5630 * texDetail) + 0.3079 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4241), 0.0, 1.0);
    float emit = atlas.a * (0.5595 + 0.4373 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1533 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.8830 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3571 + 0.2474 * presence + 0.4939 * pattern + 0.0817 * (1.0 - deepTrans) + 0.1506 * emit, 0.8731);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2694), 0.8731);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7714 + 0.2286 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
