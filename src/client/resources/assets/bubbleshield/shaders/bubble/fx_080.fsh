...
void main() {
    ...
    deep += 0.5538 * deepField(duvBase + rimDir * 0.0777 + vec2(0.060000, 0.007500) * time, time);
    deep += 0.3067 * deepField(duvBase + rimDir * 0.1555 + vec2(0.080000, 0.007500) * time, time);
    deep += 0.1698 * deepField(duvBase + rimDir * 0.2332 + vec2(0.101667, 0.010000) * time, time);
    deep = pow(clamp(deep * 0.4925, 0.0, 1.0), 1.1249);
