...
void main() {
    ...
    mid = clamp(mid * (0.6394 + 0.6544 * texDetail) + 0.1984 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3611), 0.0, 1.0);
    float emit = atlas.a * (0.7664 + 0.4289 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0909 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.8146 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4053 + 0.5058 * presence + 0.3477 * pattern + 0.0872 * (1.0 - deepTrans) + 0.1437 * emit, 0.9162);
