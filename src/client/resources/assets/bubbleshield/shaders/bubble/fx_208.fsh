...
void main() {
    ...
    mid = clamp(mid * (0.6330 + 0.5970 * texDetail) + 0.3374 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3719), 0.0, 1.0);
    float emit = atlas.a * (0.7649 + 0.4766 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1232 * (0.5 + 0.5 * sin(time * 0.219911)));
    ...
    float alpha = vertexColor.a * min(0.3452 + 0.4606 * presence + 0.4823 * pattern + 0.1317 * (1.0 - deepTrans) + 0.2162 * emit, 0.9117);
