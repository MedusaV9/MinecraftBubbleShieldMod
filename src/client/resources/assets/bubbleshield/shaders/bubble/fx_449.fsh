...
void main() {
    ...
    mid = clamp(mid * (0.7173 + 0.6724 * texDetail) + 0.3215 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5261), 0.0, 1.0);
    float emit = atlas.a * (0.7058 + 0.4271 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1050 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.7312 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3860 + 0.4774 * presence + 0.4613 * pattern + 0.1254 * (1.0 - deepTrans) + 0.1475 * emit, 0.9282);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3279), 0.9282);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6366 + 0.3634 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0810), 0.0, 1.0);
