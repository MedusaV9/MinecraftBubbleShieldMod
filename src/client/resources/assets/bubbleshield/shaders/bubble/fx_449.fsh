...
// stack: deep=caustic x4 taps | mid=stainedglass+warp1+flicker | rim=graze_sparkle | flourish=shimmer | tex=worley_cells
// fbm: ridged x3 octaves | seed 86422a30888a3116
// v8 realism: refr 1.0007+1.6686*fres/dist (cap 0.1515) | energy 0.2710+0.4216*p | flow 0.083333 c/s | soft 0.7733m
...
vec4 atlasTile(vec2 p) {
    return texture(Sampler0, (vec2(1.0000, 0.0000) + clamp(fract(p), 0.004, 0.996)) * 0.25);
}
...
void main() {
    ...
    float refrAmp = min((1.0007 + 1.6686 * fresnel) / max(length(worldPos), 1.0), 0.1515);
