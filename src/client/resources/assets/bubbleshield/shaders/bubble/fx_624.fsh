...
void main() {
    ...
    mid = clamp(mid * (0.6331 + 0.6362 * texDetail) + 0.3230 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3553), 0.0, 1.0);
    float emit = atlas.a * (0.7621 + 0.3032 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1758 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.7088 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3116 + 0.3160 * presence + 0.4572 * pattern + 0.0956 * (1.0 - deepTrans) + 0.1927 * emit, 0.9059);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2353), 0.9059);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6893 + 0.3107 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1310), 0.0, 1.0);
