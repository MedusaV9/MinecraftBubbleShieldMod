...
void main() {
    ...
    mid = clamp(mid * (0.6933 + 0.5570 * texDetail) + 0.2166 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4409), 0.0, 1.0);
    float emit = atlas.a * (0.7453 + 0.3337 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1524 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9031 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4144 + 0.4490 * presence + 0.4518 * pattern + 0.1079 * (1.0 - deepTrans) + 0.1541 * emit, 0.9073);
