...
void main() {
    ...
    mid = clamp(mid * (0.6469 + 0.5186 * texDetail) + 0.3112 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4332), 0.0, 1.0);
    float emit = atlas.a * (0.7713 + 0.3521 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1636 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.9955 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2977 + 0.2547 * presence + 0.5434 * pattern + 0.1034 * (1.0 - deepTrans) + 0.1810 * emit, 0.9055);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2115), 0.9055);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7686 + 0.2314 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1055), 0.0, 1.0);
