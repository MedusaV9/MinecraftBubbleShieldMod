...
void main() {
    ...
    mid = clamp(mid * (0.6656 + 0.5760 * texDetail) + 0.2535 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4824), 0.0, 1.0);
    float emit = atlas.a * (0.7370 + 0.3649 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1441 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.7890 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4354 + 0.4006 * presence + 0.4221 * pattern + 0.1312 * (1.0 - deepTrans) + 0.2152 * emit, 0.8767);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2365), 0.8767);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7739 + 0.2261 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1275), 0.0, 1.0);
