...
void main() {
    ...
    mid = clamp(mid * (0.6809 + 0.5135 * texDetail) + 0.2390 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4903), 0.0, 1.0);
    float emit = atlas.a * (0.6858 + 0.2689 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1550 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.9489 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3880 + 0.4129 * presence + 0.4309 * pattern + 0.1159 * (1.0 - deepTrans) + 0.2193 * emit, 0.9010);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1955), 0.9010);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6639 + 0.3361 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0854), 0.0, 1.0);
