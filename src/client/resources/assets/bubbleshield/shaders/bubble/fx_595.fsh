...
void main() {
    ...
    mid = clamp(mid * (0.6262 + 0.5304 * texDetail) + 0.3307 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3641), 0.0, 1.0);
    float emit = atlas.a * (0.7000 + 0.3987 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0959 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8115 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3144 + 0.3069 * presence + 0.5987 * pattern + 0.1574 * (1.0 - deepTrans) + 0.1322 * emit, 0.8653);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2389), 0.8653);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7539 + 0.2461 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0965), 0.0, 1.0);
