...
// stack: deep=fbm x4 taps | mid=runecircuit+warp1+rotate | rim=graze_film | flourish=glint | tex=runes
// fbm: turb x6 octaves | seed 604aac53e8ac87fc
// v8 realism: refr 0.3775+0.4260*fres/dist (cap 0.1319) | energy 0.5325+0.4138*p | flow 0.104167 c/s | soft 0.8821m
...
void main() {
    ...
    float refrAmp = min((0.3775 + 0.4260 * fresnel) / max(length(worldPos), 1.0), 0.1319);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0542, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9458, vec2(0.001), vec2(0.999))).b);
