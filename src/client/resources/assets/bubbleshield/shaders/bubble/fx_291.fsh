...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5643, 0.0, 1.0);
    ...
    deepCol += 0.4846 * dp * mix(baseCol, deepStop, 0.2087);
    deepPat += 0.4846 * dp;
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * 4.9150) + par * 0.0938, time);
    deepCol += 0.2349 * dp * mix(baseCol, deepStop, 0.4173);
    deepPat += 0.2349 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 6.0307) + par * 0.1408, time);
    deepCol += 0.1138 * dp * mix(baseCol, deepStop, 0.6260);
    deepPat += 0.1138 * dp;
    deepCol *= 0.5709;
    deepPat = pow(clamp(deepPat * 0.5455, 0.0, 1.0), 1.2894);
    ...
    rgb += hotStop * 0.3061 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0607 + 0.2639 * presence + 0.3889 * pattern, 0.8235);
