...
void main() {
    ...
    mid = clamp(mid * (0.6314 + 0.6040 * texDetail) + 0.3175 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4496), 0.0, 1.0);
    float emit = atlas.a * (0.6174 + 0.4974 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1563 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.7436 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3892 + 0.4039 * presence + 0.3889 * pattern + 0.1323 * (1.0 - deepTrans) + 0.2115 * emit, 0.8835);
