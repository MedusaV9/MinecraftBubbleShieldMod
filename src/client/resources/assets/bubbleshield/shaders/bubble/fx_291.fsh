...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9107) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9107);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.048333, 0.050000) * time, time);
    deep += 0.4846 * deepField(duvBase + rimDir * 0.0469 + vec2(0.073333, 0.075000) * time, time);
    deep += 0.2349 * deepField(duvBase + rimDir * 0.0938 + vec2(0.100000, 0.102500) * time, time);
    deep = pow(clamp(deep * 0.5816, 0.0, 1.0), 1.2894);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.380000), midPer) - 1.0);
