...
void main() {
    ...
    mid = clamp(mid * (0.7477 + 0.6333 * texDetail) + 0.3452 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5003), 0.0, 1.0);
    float emit = atlas.a * (0.7720 + 0.3239 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0882 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9795 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2814 + 0.2877 * presence + 0.6271 * pattern + 0.1015 * (1.0 - deepTrans) + 0.1474 * emit, 0.9058);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2717), 0.9058);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6939 + 0.3061 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0691), 0.0, 1.0);
