...
void main() {
    ...
    mid = clamp(mid * (0.7290 + 0.7470 * texDetail) + 0.2923 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5151), 0.0, 1.0);
    float emit = atlas.a * (0.7975 + 0.4438 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1716 * (0.5 + 0.5 * sin(time * 0.099484)));
    rgb += emit * 0.8776 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3649 + 0.4581 * presence + 0.3926 * pattern + 0.1040 * (1.0 - deepTrans) + 0.1581 * emit, 0.9363);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1906), 0.9363);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0677), 0.0, 1.0);
