...
void main() {
    ...
    mid = clamp(mid * (0.7121 + 0.6068 * texDetail) + 0.2600 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4554), 0.0, 1.0);
    float emit = atlas.a * (0.7633 + 0.4095 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1466 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.8245 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3350 + 0.3466 * presence + 0.5054 * pattern + 0.0863 * (1.0 - deepTrans) + 0.1411 * emit, 0.9021);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2432), 0.9021);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6758 + 0.3242 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0779), 0.0, 1.0);
