...
void main() {
    ...
    mid = clamp(mid * (0.7028 + 0.6399 * texDetail) + 0.2763 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3644), 0.0, 1.0);
    float emit = atlas.a * (0.7461 + 0.2741 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1350 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8307 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3574 + 0.4931 * presence + 0.3667 * pattern + 0.1442 * (1.0 - deepTrans) + 0.2103 * emit, 0.8767);
