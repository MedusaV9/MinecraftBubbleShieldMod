...
// stack: deep=caustic x3 taps | mid=sparkle+warp1+rotate | rim=graze | flourish=swirl | tex=starfield
// fbm: ridged x6 octaves | seed d02bacfce1076a94
// v8 realism: refr 0.6454+0.8729*fres/dist (cap 0.1509) | energy 0.4053+0.3817*p | flow 0.077500 c/s | soft 0.6999m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.3770 + 0.1952, 0.0, 1.0);
    ...
    float refrAmp = min((0.6454 + 0.8729 * fresnel) / max(length(worldPos), 1.0), 0.1509);
    ...
    float emitMask = clamp(atlas.a * 2.3211, 0.0, 1.0);
    float emit = emitMask * (0.4177 + 0.4789 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1350 * (0.5 + 0.5 * sin(time * 0.178024))) * atlasPoleW;
