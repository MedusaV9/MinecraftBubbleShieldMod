...
void main() {
    ...
    deep += 0.4419 * deepField(duvBase + rimDir * 0.0515 + vec2(-0.028333, -0.070000) * time, time);
    deep += 0.1952 * deepField(duvBase + rimDir * 0.1030 + vec2(-0.038333, -0.095000) * time, time);
    deep += 0.0863 * deepField(duvBase + rimDir * 0.1545 + vec2(-0.048333, -0.117500) * time, time);
    deep = pow(clamp(deep * 0.5803, 0.0, 1.0), 1.5496);
    ...
    float pattern = clamp(0.4340 * deep + 0.9785 * mid + 0.5822 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1554 + pattern * 0.3814);
    vec3 rgb = vertexColor.rgb * (0.5444 + 0.5653 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1909 + 0.8472 * pattern, 0.0, 1.0);
