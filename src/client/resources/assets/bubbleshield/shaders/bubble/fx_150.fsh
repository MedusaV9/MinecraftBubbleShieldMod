...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7903, 0.0, 1.0);
    ...
    deepCol += 0.4419 * dp * mix(baseCol, deepStop, 0.2192);
    deepPat += 0.4419 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 5.0686) + par * 0.1030, time);
    deepCol += 0.1952 * dp * mix(baseCol, deepStop, 0.4383);
    deepPat += 0.1952 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 6.1395) + par * 0.1545, time);
    deepCol += 0.0863 * dp * mix(baseCol, deepStop, 0.6575);
    deepPat += 0.0863 * dp;
    deepCol *= 0.5950;
    deepPat = pow(clamp(deepPat * 0.5803, 0.0, 1.0), 1.5496);
    ...
    rgb += hotStop * 0.2491 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0633 + 0.2443 * presence + 0.4715 * pattern, 0.8054);
