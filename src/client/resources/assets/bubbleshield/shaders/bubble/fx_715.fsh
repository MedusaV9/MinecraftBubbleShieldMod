...
void main() {
    ...
    mid = clamp(mid * (0.6096 + 0.6684 * texDetail) + 0.2165 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5154), 0.0, 1.0);
    float emit = atlas.a * (0.6491 + 0.4343 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1565 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6957 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3002 + 0.2792 * presence + 0.4830 * pattern + 0.1122 * (1.0 - deepTrans) + 0.2150 * emit, 0.8444);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3223), 0.8444);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7318 + 0.2682 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
