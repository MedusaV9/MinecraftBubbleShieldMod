...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6495, 0.0, 1.0);
    ...
    deepCol += 0.4916 * dp * mix(baseCol, deepStop, 0.2729);
    deepPat += 0.4916 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.7683) + par * 0.1211, time);
    deepCol += 0.2417 * dp * mix(baseCol, deepStop, 0.5459);
    deepPat += 0.2417 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 5.7969) + par * 0.1817, time);
    deepCol += 0.1188 * dp * mix(baseCol, deepStop, 0.8188);
    deepPat += 0.1188 * dp;
    deepCol *= 0.5356;
    deepPat = pow(clamp(deepPat * 0.5399, 0.0, 1.0), 1.7444);
    ...
    rgb += hotStop * 0.3382 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0436 + 0.2571 * presence + 0.3489 * pattern, 0.7885);
