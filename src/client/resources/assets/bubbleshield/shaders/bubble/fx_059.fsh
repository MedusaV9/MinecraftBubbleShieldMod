...
// stack: deep=caustic x4 taps | mid=starfield+warp2+rotate | rim=graze_film | flourish=glint | tex=starfield
// fbm: ridged x4 octaves | seed d641c844c05666f9
// v8 realism: refr 0.3608+0.4526*fres/dist (cap 0.1034) | energy 0.5069+0.4435*p | flow 0.054167 c/s | soft 0.9930m
...
void main() {
    ...
    float refrAmp = min((0.3608 + 0.4526 * fresnel) / max(length(worldPos), 1.0), 0.1034);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0312, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9688, vec2(0.001), vec2(0.999))).b);
