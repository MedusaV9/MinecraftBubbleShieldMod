...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6036, 0.0, 1.0);
    ...
    deepCol += 0.3909 * dp * mix(baseCol, deepStop, 0.2885);
    deepPat += 0.3909 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.1024) + par * 0.1410, time);
    deepCol += 0.1528 * dp * mix(baseCol, deepStop, 0.5770);
    deepPat += 0.1528 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 6.4780) + par * 0.2115, time);
    deepCol += 0.0597 * dp * mix(baseCol, deepStop, 0.8655);
    deepPat += 0.0597 * dp;
    deepCol *= 0.7894;
    deepPat = pow(clamp(deepPat * 0.6237, 0.0, 1.0), 1.7750);
    ...
    rgb += hotStop * 0.2837 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0570 + 0.3048 * presence + 0.4005 * pattern, 0.8480);
