...
void main() {
    ...
    mid = clamp(mid * (0.7094 + 0.5860 * texDetail) + 0.3469 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6218 + 0.2626 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0824 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.9248 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2776 + 0.2436 * presence + 0.5850 * pattern + 0.1259 * (1.0 - deepTrans) + 0.2120 * emit, 0.8410);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3255), 0.8410);
    // [layer:v5:ghostalpha]
