...
// stack: deep=fbm x4 taps | mid=phantomecho+warp1+scroll | rim=graze_sparkle | flourish=shimmer | tex=foam
// fbm: ridged x5 octaves | seed 9ae4f6ee967bfbd8
// v8 realism: refr 0.4525+0.6701*fres/dist (cap 0.1280) | energy 0.3601+0.5324*p | flow 0.110833 c/s | soft 1.1478m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.1002 * atlasPoleW));
    ...
    float refrAmp = min((0.4525 + 0.6701 * fresnel) / max(length(worldPos), 1.0), 0.1280);
