...
void main() {
    ...
    mid = clamp(mid * (0.7381 + 0.7416 * texDetail) + 0.2329 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4515), 0.0, 1.0);
    float emit = atlas.a * (0.7777 + 0.2803 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0881 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.9106 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3826 + 0.4253 * presence + 0.3469 * pattern + 0.1227 * (1.0 - deepTrans) + 0.1822 * emit, 0.9069);
