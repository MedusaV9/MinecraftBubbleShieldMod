...
void main() {
    ...
    mid = clamp(mid * (0.7496 + 0.5492 * texDetail) + 0.2575 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.8369 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5006), 0.0, 1.0);
    float emit = atlas.a * (0.7152 + 0.2673 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1330 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.8651 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3493 + 0.3884 * presence + 0.4622 * pattern + 0.1002 * (1.0 - deepTrans) + 0.1537 * emit, 0.8444);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1988), 0.8444);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6464 + 0.3536 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
