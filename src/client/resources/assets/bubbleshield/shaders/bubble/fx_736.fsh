...
void main() {
    ...
    mid = clamp(mid * (0.6640 + 0.7163 * texDetail) + 0.3031 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5300), 0.0, 1.0);
    float emit = atlas.a * (0.7471 + 0.3461 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1369 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.8210 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3607 + 0.4491 * presence + 0.4575 * pattern + 0.1462 * (1.0 - deepTrans) + 0.1611 * emit, 0.8628);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2365), 0.8628);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7755 + 0.2245 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
