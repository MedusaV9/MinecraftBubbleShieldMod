...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8289, 0.0, 1.0);
    ...
    deepCol += 0.4300 * dp * mix(baseCol, deepStop, 0.2543);
    deepPat += 0.4300 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 3.3612) + par * 0.0604, time);
    deepCol += 0.1849 * dp * mix(baseCol, deepStop, 0.5086);
    deepPat += 0.1849 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.1758) + par * 0.0907, time);
    deepCol += 0.0795 * dp * mix(baseCol, deepStop, 0.7628);
    deepPat += 0.0795 * dp;
    deepCol *= 0.6140;
    deepPat = pow(clamp(deepPat * 0.5901, 0.0, 1.0), 1.3418);
    ...
    float fern = pow(clamp(vein1 * vein2, 0.0, 1.0), 3.4565);
    ...
    rgb += hotStop * 0.2278 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0776 + 0.3178 * presence + 0.4847 * pattern, 0.8576);
