...
void main() {
    ...
    mid = clamp(mid * (0.6221 + 0.7357 * texDetail) + 0.3465 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4905), 0.0, 1.0);
    float emit = atlas.a * (0.7049 + 0.3673 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1222 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.8960 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3786 + 0.4867 * presence + 0.4674 * pattern + 0.1118 * (1.0 - deepTrans) + 0.1764 * emit, 0.9487);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2909), 0.9487);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7766 + 0.2234 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0850), 0.0, 1.0);
