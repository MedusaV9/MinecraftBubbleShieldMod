...
// v8 realism: refr 1.3449+1.6516*fres/dist (cap 0.1408) | energy 0.1978+0.3909*p | flow 0.072500 c/s | soft 0.8937m
...
void main() {
    ...
    float jump = hash11(floor(time * 3.1438));
    ...
    vec2 tuv = vec2(baseUV.x * 7.0000, baseUV.y * 7.0000) + vec2(-0.233333, 0.210000) * time;
    ...
    vec2 texUV = tuv * 1.0000 + vec2(-0.005833, -0.057500) * time;
    ...
    rim = clamp(rim + 0.8669 * rimLine + 0.5881 * fresnel, 0.0, 1.4);
    ...
    float motifEnv = 0.15 + 0.85 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.780162)) * (0.5 + 0.5 * sin(time * 0.455531 + 1.7)));
    float motifM = 0.0;
    ...
    motifM += exp(-dot(motifD0, motifD0) * 308.3642);
    ...
    float refrAmp = min((1.3449 + 1.6516 * fresnel) / max(length(worldPos), 1.0), 0.1408);
    ...
    float energyW = clamp(0.1978 + 0.3909 * clamp(pattern, 0.0, 1.0) + 0.1085 * fresnel, 0.0, 0.8856);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.6609;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3127) * 0.7009;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3584 * (1.0 - 0.4667 * baseLuma)), 0.0, 1.0);
    // strobe-free emission drive (see the v7 note above): only smooth,
    // non-gated terms may modulate the emissive add
    float emitDrive = clamp(0.2960 * deepPat + 0.8723 * texDetail + 0.6045 * rim + 0.2103 * v5Par, 0.0, 1.0);
    float emit = atlas.a * (0.3930 + 0.5907 * emitDrive) * (1.0 - 0.1222 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
    rgb += emit * 0.8960 * (1.0 - 0.7999 * baseLuma) * emitCol;
    // [layer:v5:softknee]
