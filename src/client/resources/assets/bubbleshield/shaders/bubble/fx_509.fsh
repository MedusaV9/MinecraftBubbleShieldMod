...
// Bubble surface shader fx_509 -- family STAINEDGLASS
// stack: deep=ridge x3 taps | mid=stainedglass+curl+flicker | rim=lat | flourish=glint | tex=basalt_columns | motif=orbitx1+sparklegate
// fbm: ridged x4 octaves | seed 3da13551745ad6bb
// v8 realism: refr 1.5055+1.8488*fres/dist (cap 0.1408) | energy 0.1542+0.3909*p | flow 0.072500 c/s | soft 0.8937m
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3438 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.5682, 1.0528, 0.5454) * t
        + vec3(0.3532, 0.6161, 0.4497)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4859, x));
    return mix(g, hotStop, smoothstep(0.4859, 1.0, x));
}
...
float rimLat(vec2 uv) {
    ...
    float poles = smoothstep(0.7196, 1.0, lat);
    float belt = invsmooth(0.0, 0.3346, lat);
    return clamp(poles + 0.3878 * belt, 0.0, 1.0);
}
...
float deepField(vec3 p, float t) {
    float n = fbm3(p);
    return pow(1.0 - abs(2.0 * n - 1.0), 2.9338);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.8047;
    vec3 spun = clamp(hueSpin(baseCol, 0.3517), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.1810) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5653, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.8462);
    ...
    vec3 spinAxis = vec3(0.7418, 0.6055, 0.2884);
    vec3 par = Vt * 1.7278;
    ...
    float deepBreath = 1.0 + 0.0145 * sin(time * 1.204277);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * (2.6674 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4752 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * (3.8489 * deepBreath)) + par * 0.0456, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.4969), deepStop, 0.1822);
    deepTrans *= 1.0 - 0.4752 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * (5.0305 * deepBreath)) + par * 0.0913, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.9939), deepStop, 0.3644);
    deepTrans *= 1.0 - 0.4752 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.8816;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.1690, 0.0, 1.0), 1.2367);
    ...
    float ft = time + 0.3249 * (0.6 * sin(time * 3.366740) + 0.4 * sin(time * 10.744247 + 4.1741));
    ...
    vec2 texUV = wuv * 1.0000 + vec2(-0.005833, -0.057500) * time;
    ...
    vec3 hgtW = vec3(0.3922, 0.3124, 0.2955);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -3.8777;
    ...
    float texDetail = clamp(atlas.r * 0.4656 + atlas.g * 0.3709 + atlas.b * 0.3508, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6221 + 0.7357 * texDetail, atlasPoleW) + 0.1979 * texDetail * (0.2096 + 0.7904 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (2.0318 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.2372);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0731).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0731).rgb;
    float deepTex = 0.5 * (dot(deepTexA, hgtW) + dot(deepTexB, hgtW));

    // [layer:thick:chord]
    // v11 volumetric thickness: the membrane is a shell of relative
    // thickness rho = 0.0920 (crystalline material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.8379.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9080 * 0.9080 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.8379);
    float chordN = min(chord / 0.0920, 3.0);
    ...
    float rimLine = smoothstep(0.0452, 0.0772, rimSlope);
    float rim = rimLat(baseUV) * 0.8416;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.8669 * rimLine + 0.8357 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2685 * sparkle(wuv * 2.0 + 7.7, time * 2.0, midPer.x * 2.0);
    float grain = 0.0913 * (cellHash(floor(wuv * 43.0000) + vec2(floor(time * 2.0), 0.0), 301.0000) - 0.5);
    ...
    float motifEnv = 0.48 + 0.52 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.780162)) * (0.5 + 0.5 * sin(time * 0.455531 + 1.7)));
    float motifM = 0.0;
    float motifO0 = fract(0.9204 + 1.0 * time * 0.056667);
    float motifL0 = 0.4286 + 0.1230 * sin(time * 0.376991 + 0.0000);
    vec2 motifD0 = vec2(sin((baseUV.x - motifO0) * 3.1415927) * 0.6366, baseUV.y - motifL0);
    motifM += exp(-dot(motifD0, motifD0) * 137.3546);
    ...
    float v5Par = deepField(sdir * 1.8482 + par * 0.0686, time) * 0.5;
    v5Par += deepField(sdir * 2.4951 + par * 0.1372, time) * 0.3;
    v5Par += deepField(sdir * 3.2344 + par * 0.2058, time) * 0.2;
    float pattern = clamp(0.2960 * deepPat * mix(1.0, chordN, 0.2298) + 0.8723 * mid + 0.6045 * rim + flourish + grain + 0.2103 * v5Par + 0.1518 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.7995 + rim * 0.1994, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9430 * mid + 0.5856 * rim, 0.0, 1.0);
    ...
    rgb *= mix(1.0, texBody, atlasPoleW * (0.5109 + 0.4891 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.4285);
    ...
    vec3 softHot = 1.0 - exp(-2.4831 * hotStop * (pattern + 0.35 * rim));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 0.7646 + baseUV.y * 0.6755 + 0.9206) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1183);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.7443 + baseUV.y * 0.31 + 0.3319) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2807);
    ...
    rgb *= (1.0 - 0.2667 * washHi) * (1.0 - 0.2662 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.6476, 0.7191, -0.2519);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4445) / 1.4445, 0.0, 1.0);
    rgb *= 0.6229 + 0.6711 * keyDiff * (1.0 - 0.5377 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 29.2565);
    rgb += hotStop * (keySpec * 0.5556 * (1.0 - 0.7419 * washHi));
    ...
    float refrAmp = min((1.5055 + 1.8488 * fresnel) / max(length(worldPos), 1.0), 0.1408);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.1504) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.1356, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.8644, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.5101 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1133);
    float energyW = clamp(0.1542 + 0.3909 * clamp(pattern, 0.0, 1.0) + 0.1899 * fresnel, 0.0, 0.8856);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2629 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (1.0531 * (1.0 - 0.7450 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3127) * 1.0697;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3584 * max(0.0, 1.0 - 0.4667 * baseLuma - 0.6924 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3930 + 0.5907 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1222 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
    rgb += emit * 0.8960 * (1.0 - 0.7999 * baseLuma) * (1.0 - 0.4706 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    rgb = min(rgb, vec3(0.6316)) + 0.3684 * (1.0 - exp(-v5Over * 1.9650));
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2634 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.4542));
    ...
    float bodyAlpha = 0.3786 + 0.4867 * presence + 0.4674 * pattern + 0.1118 * (1.0 - deepTrans) + 0.1764 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.1991;
    // [layer:v5:backface]
    ...
    rgb = mix(rgb, deepStop, v5Back * 0.3243);
    bodyAlpha *= 1.0 + v5Back * 0.2909;
    // [layer:v5:ghostalpha]
    ...
    bodyAlpha *= 0.7766 + 0.2234 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    bodyAlpha *= 1.0 + (v5Dither - 0.5) * 0.0850;
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.0690 + hgtC * 0.3746, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.2548);
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9700) * vertexColor.a;
