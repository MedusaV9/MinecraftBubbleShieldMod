...
// stack: deep=fbm x4 taps | mid=galaxyswirl+curl+scroll | rim=graze_film | flourish=shimmer | tex=starfield
// fbm: standard x4 octaves | seed fd351fc65a047117
// v8 realism: refr 0.3369+0.4677*fres/dist (cap 0.1235) | energy 0.4440+0.4753*p | flow 0.094167 c/s | soft 0.6441m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.3339 + 0.2572, 0.0, 1.0);
    ...
    float refrAmp = min((0.3369 + 0.4677 * fresnel) / max(length(worldPos), 1.0), 0.1235);
    ...
    float emitMask = clamp(atlas.a * 1.8628, 0.0, 1.0);
    float emit = emitMask * (0.3621 + 0.5104 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1113 * (0.5 + 0.5 * sin(time * 0.146608))) * atlasPoleW;
