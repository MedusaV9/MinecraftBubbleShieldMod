...
// stack: deep=fbm x3 taps | mid=sparkle+warp2+rotate | rim=graze | flourish=echo | tex=starfield
// fbm: turb x5 octaves | seed b24d696cca3c4b6f
// v8 realism: refr 0.5428+0.6305*fres/dist (cap 0.1540) | energy 0.3191+0.4693*p | flow 0.080000 c/s | soft 1.0862m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.0688 + 0.1751, 0.0, 1.0);
    ...
    float refrAmp = min((0.5428 + 0.6305 * fresnel) / max(length(worldPos), 1.0), 0.1540);
    ...
    float emitMask = clamp(atlas.a * 1.8677, 0.0, 1.0);
    float emit = emitMask * (0.3731 + 0.6770 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1274 * (0.5 + 0.5 * sin(time * 0.157080))) * atlasPoleW;
