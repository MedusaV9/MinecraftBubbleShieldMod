...
// v8 realism: refr 1.0238+1.1891*fres/dist (cap 0.1940) | energy 0.3191+0.4693*p | flow 0.080000 c/s | soft 1.0862m
...
void main() {
    ...
    rim = clamp(rim + 0.7158 * rimLine + 0.3949 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2997 * ringPulse(baseUV + vec2(0.13, 0.31), time);
    float grain = 0.0924 * (cellHash(floor(wuv * 34.0000) + vec2(floor(time * 6.0), 0.0), 374.0000) - 0.5);
    ...
    float motifEnv = 0.55 + 0.45 * sin(time * 0.869174 + 4.1150);
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 40.2998) * (0.75 + 0.25 * sin(time * 0.408407 + 6.1211));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 40.2998);
    ...
    float refrAmp = min((1.0238 + 1.1891 * fresnel) / max(length(worldPos), 1.0), 0.1940);
    ...
    float energyW = clamp(0.3191 + 0.4693 * clamp(pattern, 0.0, 1.0) + 0.1274 * fresnel, 0.0, 0.8785);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.8500;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2731) * 0.9047;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3083 * (1.0 - 0.5075 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.3731 + 0.6770 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1274 * (0.5 + 0.5 * sin(time * 0.157080))) * atlasPoleW;
    rgb += emit * 0.9197 * (1.0 - 0.7856 * baseLuma) * emitCol;
