...
void main() {
    ...
    mid = clamp(mid * (0.6769 + 0.5150 * texDetail) + 0.2079 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4190), 0.0, 1.0);
    float emit = atlas.a * (0.6718 + 0.4392 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1274 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.9197 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3808 + 0.5073 * presence + 0.3924 * pattern + 0.1559 * (1.0 - deepTrans) + 0.1834 * emit, 0.9116);
