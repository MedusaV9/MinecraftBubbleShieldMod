...
// stack: deep=fbm x3 taps | mid=sparkle+warp2+rotate | rim=graze | flourish=echo | tex=solar_granulation | motif=corex3+breath
// fbm: turb x5 octaves | seed b24d696cca3c4b6f
// v8 realism: refr 1.1392+1.3232*fres/dist (cap 0.1940) | energy 0.2159+0.4693*p | flow 0.080000 c/s | soft 1.0862m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9142) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9142);
        amplitude *= 0.5;
    }
    return value;
}
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.5365 * (vec2(fbm2(p + vec2(0.0, t * 0.314167), per),
...
vec2 warp2(vec2 p, vec2 per, float t) {
    ...
    return p + 0.6701 * (vec2(fbm2(q + vec2(1.7, 9.2), per),
...
float rimGraze() {
    ...
    return smoothstep(0.0041, 0.0466, g);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.7561;
    vec3 spun = clamp(hueSpin(baseCol, -0.3854), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2175) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5711, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.4724);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, -0.0030), 0.0, 1.0), 1.5689) * 0.3922, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(-0.7555, -0.5268, -0.3894);
    vec3 par = Vt * 1.6417;
    ...
    float deepBreath = 1.0 + 0.0197 * sin(time * 1.497492);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * (1.9431 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.5884 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * (2.7047 * deepBreath)) + par * 0.0510, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.4410), deepStop, 0.1617);
    deepTrans *= 1.0 - 0.5884 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * (3.4663 * deepBreath)) + par * 0.1019, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.8821), deepStop, 0.3234);
    deepTrans *= 1.0 - 0.5884 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.8410;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0750, 0.0, 1.0), 1.6520);
    ...
    float sway = 0.6853 * sin(time * 0.115192);
    vec2 auv = vec2(baseUV.x * 11.0000 + (baseUV.y - 0.5) * sway + time * -0.064167, baseUV.y * 11.0000);
    vec2 wuv = warp2(auv, midPer, time);
    float s1 = sparkle(wuv, time, midPer.x);
    float s2 = sparkle(wuv * 2.0 + 31.7, time * 1.3497, midPer.x * 2.0);
    float haze = voronoise(wuv + vec2(time * 0.064167, 0.0), midPer, 0.7445, 0.4289);
    ...
    vec2 texUV = wuv * 1.0000 + vec2(-0.037500, -0.014167) * time;
    ...
    vec3 hgtW = vec3(0.3340, 0.4415, 0.2245);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -2.6388;
    ...
    float texDetail = clamp(atlas.r * 0.3774 + atlas.g * 0.4988 + atlas.b * 0.2536, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6769 + 0.5150 * texDetail, atlasPoleW) + 0.1164 * texDetail * (0.2451 + 0.7549 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.1171 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 2.8264);
    ...
    // thickness rho = 0.0444 (energy material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.5891.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9556 * 0.9556 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.5891);
    float chordN = min(chord / 0.0444, 3.0);
    ...
    float rimLine = smoothstep(0.0456, 0.0789, rimSlope);
    ...
    rim = clamp(rim + 0.7158 * rimLine + 0.6039 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2997 * ringPulse(baseUV + vec2(0.13, 0.31), time);
    float grain = 0.0924 * (cellHash(floor(wuv * 34.0000) + vec2(floor(time * 2.0), 0.0), 374.0000) - 0.5);
    ...
    float motifEnv = 0.72 + 0.28 * sin(time * 0.869174 + 4.1150);
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 21.0190) * (0.75 + 0.25 * sin(time * 0.408407 + 6.1211));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 21.0190);
    ...
    float pattern = clamp(0.4915 * deepPat * mix(1.0, chordN, 0.1592) + 0.8367 * mid + 0.8120 * rim + flourish + grain + 0.1735 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.8190 + rim * 0.1277, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9339 * mid + 0.5413 * rim, 0.0, 1.0);
    ...
    float texBody = 0.8190 + 0.4424 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4912 + 0.5088 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.1256);
    ...
    vec3 softHot = 1.0 - exp(-2.7090 * hotStop * (pattern + 0.35 * rim));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.0097 + baseUV.y * 0.4472 + 0.9881) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1378);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 1.1054 + baseUV.y * 0.31 + 0.4808) * 6.2831853);
    ...
    rgb *= (1.0 - 0.2931 * washHi) * (1.0 - 0.3013 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.4070, 0.6784, 0.6117);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4734) / 1.4734, 0.0, 1.0);
    rgb *= 0.6658 + 0.6768 * keyDiff * (1.0 - 0.5853 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 14.0999);
    rgb += hotStop * (keySpec * 0.4746 * (1.0 - 0.6522 * washHi));
    ...
    float refrAmp = min((1.1392 + 1.3232 * fresnel) / max(length(worldPos), 1.0), 0.1940);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 4.0460) * refrAmp;
    ...
    refracted *= exp(-1.0042 * chordN * (1.0 - baseCol));
    ...
    float energyW = clamp(0.2159 + 0.4693 * clamp(pattern, 0.0, 1.0) + 0.2119 * fresnel, 0.0, 0.8785);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.3261 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (1.2000 * (1.0 - 0.7148 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2731) * 1.2991;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3083 * max(0.0, 1.0 - 0.5075 * baseLuma - 0.6328 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3731 + 0.6770 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1274 * (0.5 + 0.5 * sin(time * 0.157080))) * atlasPoleW;
    rgb += emit * 0.9197 * (1.0 - 0.7856 * baseLuma) * (1.0 - 0.5161 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.7527));
    ...
    float bodyAlpha = 0.3808 + 0.5073 * presence + 0.3924 * pattern + 0.1559 * (1.0 - deepTrans) + 0.1834 * emit;
    ...
    float innerGlow = deepField(rotA(spinAxis, -time * 0.052360 + 3.8437) * (sdir * 2.1244), time);
    vec3 innerCol = mix(hotStop, secCol, clamp(innerGlow * 1.0589, 0.0, 1.0));
    rgb = mix(rgb, innerCol, innerFace * 0.2633);
    ...
    bodyAlpha *= smoothstep(0.0, 1.0862, sceneVDist - fragVDist);
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9511) * vertexColor.a;
