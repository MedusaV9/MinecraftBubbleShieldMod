...
void main() {
    ...
    mid = clamp(mid * (0.7403 + 0.5089 * texDetail) + 0.3471 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4967), 0.0, 1.0);
    float emit = atlas.a * (0.7509 + 0.3695 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0898 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.8252 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3390 + 0.3602 * presence + 0.3889 * pattern + 0.1389 * (1.0 - deepTrans) + 0.1259 * emit, 0.8503);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2614), 0.8503);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6236 + 0.3764 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0919), 0.0, 1.0);
