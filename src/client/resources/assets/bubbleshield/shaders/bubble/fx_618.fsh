...
void main() {
    ...
    mid = clamp(mid * (0.6211 + 0.4579 * texDetail) + 0.2335 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3867), 0.0, 1.0);
    float emit = atlas.a * (0.7395 + 0.2924 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1567 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.7445 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3715 + 0.3514 * presence + 0.4677 * pattern + 0.1442 * (1.0 - deepTrans) + 0.1258 * emit, 0.8455);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2980), 0.8455);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7179 + 0.2821 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1102), 0.0, 1.0);
