...
void main() {
    ...
    mid = clamp(mid * (0.7324 + 0.6572 * texDetail) + 0.3044 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4430), 0.0, 1.0);
    float emit = atlas.a * (0.6373 + 0.4571 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1433 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7978 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3655 + 0.4026 * presence + 0.3927 * pattern + 0.0826 * (1.0 - deepTrans) + 0.1928 * emit, 0.8774);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3181), 0.8774);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6332 + 0.3668 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0655), 0.0, 1.0);
