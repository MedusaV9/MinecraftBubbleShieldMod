...
void main() {
    ...
    mid = clamp(mid * (0.7165 + 0.4979 * texDetail) + 0.2359 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4603), 0.0, 1.0);
    float emit = atlas.a * (0.6173 + 0.3662 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1160 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.8799 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4069 + 0.4092 * presence + 0.3913 * pattern + 0.1409 * (1.0 - deepTrans) + 0.1648 * emit, 0.9165);
