...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8247, 0.0, 1.0);
    ...
    deepCol += 0.4409 * dp * mix(baseCol, deepStop, 0.2852);
    deepPat += 0.4409 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 3.6671) + par * 0.1129, time);
    deepCol += 0.1944 * dp * mix(baseCol, deepStop, 0.5703);
    deepPat += 0.1944 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 4.4562) + par * 0.1693, time);
    deepCol += 0.0857 * dp * mix(baseCol, deepStop, 0.8555);
    deepPat += 0.0857 * dp;
    deepCol *= 0.5586;
    deepPat = pow(clamp(deepPat * 0.5811, 0.0, 1.0), 1.3029);
    ...
    rgb += hotStop * 0.3195 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0666 + 0.2692 * presence + 0.3913 * pattern, 0.8565);
