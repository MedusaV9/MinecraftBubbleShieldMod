...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.025000, -0.015000) * time, time);
    deep += 0.4409 * deepField(duvBase + rimDir * 0.0564 + vec2(-0.036667, -0.022500) * time, time);
    deep += 0.1944 * deepField(duvBase + rimDir * 0.1129 + vec2(-0.050000, -0.030000) * time, time);
    deep = pow(clamp(deep * 0.6115, 0.0, 1.0), 1.3029);
