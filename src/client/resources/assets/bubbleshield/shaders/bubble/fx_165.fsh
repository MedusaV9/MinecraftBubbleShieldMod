...
void main() {
    ...
    mid = clamp(mid * (0.7242 + 0.5968 * texDetail) + 0.2651 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4358), 0.0, 1.0);
    float emit = atlas.a * (0.6659 + 0.3093 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1770 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.7279 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3988 + 0.4094 * presence + 0.4019 * pattern + 0.1042 * (1.0 - deepTrans) + 0.1957 * emit, 0.8975);
