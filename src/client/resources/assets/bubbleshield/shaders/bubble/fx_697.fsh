...
void main() {
    ...
    mid = clamp(mid * (0.6517 + 0.5465 * texDetail) + 0.3207 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4694), 0.0, 1.0);
    float emit = atlas.a * (0.6417 + 0.3959 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1584 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.8579 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2723 + 0.2389 * presence + 0.5482 * pattern + 0.1011 * (1.0 - deepTrans) + 0.1754 * emit, 0.8624);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2008), 0.8624);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6884 + 0.3116 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
