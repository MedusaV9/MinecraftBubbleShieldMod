...
// stack: deep=voro x4 taps | mid=raymarchfog+curl+flicker | rim=graze_film | flourish=swirl
...
void main() {
    ...
    float mid = clamp(rfLight * 0.9142 + (1.0 - rfTrans) * 0.2093, 0.0, 1.15);
    ...
    float alpha = vertexColor.a * min(0.0444 + 0.3831 * presence + 0.3391 * pattern + 0.1357 * (1.0 - deepTrans), 0.8578);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2428), 0.8578);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4552 + 0.5448 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
