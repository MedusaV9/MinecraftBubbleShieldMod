...
void main() {
    ...
    mid = clamp(mid * (0.6995 + 0.5184 * texDetail) + 0.3385 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4918), 0.0, 1.0);
    float emit = atlas.a * (0.7725 + 0.4526 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0863 * (0.5 + 0.5 * sin(time * 0.178024)));
    ...
    float alpha = vertexColor.a * min(0.3306 + 0.4431 * presence + 0.3391 * pattern + 0.1357 * (1.0 - deepTrans) + 0.1295 * emit, 0.9178);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2428), 0.9178);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6908 + 0.3092 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
