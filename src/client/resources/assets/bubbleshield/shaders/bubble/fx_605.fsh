...
void main() {
    ...
    mid = clamp(mid * (0.6143 + 0.5753 * texDetail) + 0.3186 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5095), 0.0, 1.0);
    float emit = atlas.a * (0.6610 + 0.4065 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1431 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.9647 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3735 + 0.3828 * presence + 0.5184 * pattern + 0.1012 * (1.0 - deepTrans) + 0.2133 * emit, 0.8842);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2078), 0.8842);
    // [layer:v5:ghostalpha]
