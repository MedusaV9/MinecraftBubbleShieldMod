...
void main() {
    ...
    mid = clamp(mid * (0.7027 + 0.6608 * texDetail) + 0.2531 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5053), 0.0, 1.0);
    float emit = atlas.a * (0.7906 + 0.3994 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1190 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.6788 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4247 + 0.4298 * presence + 0.4967 * pattern + 0.1257 * (1.0 - deepTrans) + 0.1799 * emit, 0.8674);
