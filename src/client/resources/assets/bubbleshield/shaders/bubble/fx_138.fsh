...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.011667, -0.005000) * time, time);
    deep += 0.4689 * deepField(duvBase + rimDir * 0.0612 + vec2(0.018333, -0.010000) * time, time);
    deep += 0.2199 * deepField(duvBase + rimDir * 0.1224 + vec2(0.025000, -0.012500) * time, time);
    deep += 0.1031 * deepField(duvBase + rimDir * 0.1836 + vec2(0.031667, -0.015000) * time, time);
    deep = pow(clamp(deep * 0.5581, 0.0, 1.0), 1.1887);
    ...
    float pattern = clamp(0.4499 * deep + 0.8057 * mid + 0.7160 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7064 + pattern * 0.4351);
    vec3 rgb = vertexColor.rgb * (0.4718 + 0.8402 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1724 + 0.6478 * pattern, 0.0, 1.0);
