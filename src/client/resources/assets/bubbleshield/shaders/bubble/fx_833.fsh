...
void main() {
    ...
    mid = clamp(mid * (0.7094 + 0.4810 * texDetail) + 0.3281 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4437), 0.0, 1.0);
    float emit = atlas.a * (0.6106 + 0.4849 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1359 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.8350 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3038 + 0.3141 * presence + 0.5123 * pattern + 0.1163 * (1.0 - deepTrans) + 0.1429 * emit, 0.8720);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2387), 0.8720);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6867 + 0.3133 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1022), 0.0, 1.0);
