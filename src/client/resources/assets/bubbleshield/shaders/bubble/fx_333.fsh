...
void main() {
    ...
    mid = clamp(mid * (0.7081 + 0.5841 * texDetail) + 0.1865 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5125), 0.0, 1.0);
    float emit = atlas.a * (0.5960 + 0.4348 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0960 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.7103 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3609 + 0.3094 * presence + 0.5873 * pattern + 0.1523 * (1.0 - deepTrans) + 0.1339 * emit, 0.8841);
