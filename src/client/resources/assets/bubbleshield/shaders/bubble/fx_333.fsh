...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7066, 0.0, 1.0);
    ...
    deepCol += 0.4575 * dp * mix(baseCol, deepStop, 0.2935);
    deepPat += 0.4575 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 3.4456) + par * 0.1009, time);
    deepCol += 0.2093 * dp * mix(baseCol, deepStop, 0.5870);
    deepPat += 0.2093 * dp;
    deepCol *= 0.7745;
    deepPat = pow(clamp(deepPat * 0.6000, 0.0, 1.0), 1.0813);
    ...
    rgb += hotStop * 0.2137 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0800 + 0.3529 * presence + 0.4848 * pattern, 0.8241);
