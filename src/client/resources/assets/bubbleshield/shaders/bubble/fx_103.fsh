...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6404, 0.0, 1.0);
    ...
    deepCol += 0.4319 * dp * mix(baseCol, deepStop, 0.2839);
    deepPat += 0.4319 * dp;
    dp = deepField(rotA(spinAxis, time * 0.089012) * (sdir * 3.4739) + par * 0.1371, time);
    deepCol += 0.1865 * dp * mix(baseCol, deepStop, 0.5677);
    deepPat += 0.1865 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 4.1569) + par * 0.2057, time);
    deepCol += 0.0806 * dp * mix(baseCol, deepStop, 0.8516);
    deepPat += 0.0806 * dp;
    deepCol *= 0.6284;
    deepPat = pow(clamp(deepPat * 0.5886, 0.0, 1.0), 1.0003);
    ...
    rgb += hotStop * 0.3084 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0517 + 0.3282 * presence + 0.4703 * pattern, 0.8194);
