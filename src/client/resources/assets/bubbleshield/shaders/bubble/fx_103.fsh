...
void main() {
    ...
    mid = clamp(mid * (0.7028 + 0.6352 * texDetail) + 0.2354 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7258 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4320), 0.0, 1.0);
    float emit = atlas.a * (0.7288 + 0.4136 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1433 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.8896 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3627 + 0.4682 * presence + 0.4703 * pattern + 0.0884 * (1.0 - deepTrans) + 0.1708 * emit, 0.8794);
