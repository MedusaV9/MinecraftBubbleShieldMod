...
void main() {
    ...
    mid = clamp(mid * (0.6650 + 0.6420 * texDetail) + 0.2920 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5431), 0.0, 1.0);
    float emit = atlas.a * (0.7723 + 0.4344 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1338 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.7536 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2910 + 0.3299 * presence + 0.4664 * pattern + 0.0989 * (1.0 - deepTrans) + 0.1821 * emit, 0.9103);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1688), 0.9103);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6765 + 0.3235 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
