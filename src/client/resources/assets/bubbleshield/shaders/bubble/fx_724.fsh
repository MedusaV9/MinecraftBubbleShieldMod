...
void main() {
    ...
    mid = clamp(mid * (0.6893 + 0.4768 * texDetail) + 0.3184 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5194), 0.0, 1.0);
    float emit = atlas.a * (0.6052 + 0.4620 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1218 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.9484 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3373 + 0.3037 * presence + 0.4851 * pattern + 0.0819 * (1.0 - deepTrans) + 0.1774 * emit, 0.9033);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3252), 0.9033);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6524 + 0.3476 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0787), 0.0, 1.0);
