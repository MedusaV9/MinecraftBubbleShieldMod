...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7379, 0.0, 1.0);
    ...
    deepCol += 0.5624 * dp * mix(baseCol, deepStop, 0.2690);
    deepPat += 0.5624 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.8482) + par * 0.0694, time);
    deepCol += 0.3163 * dp * mix(baseCol, deepStop, 0.5381);
    deepPat += 0.3163 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 4.5995) + par * 0.1041, time);
    deepCol += 0.1778 * dp * mix(baseCol, deepStop, 0.8071);
    deepPat += 0.1778 * dp;
    deepCol *= 0.4992;
    deepPat = pow(clamp(deepPat * 0.4863, 0.0, 1.0), 1.1509);
    ...
    rgb += hotStop * 0.2512 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0409 + 0.3127 * presence + 0.4580 * pattern, 0.8542);
