...
void main() {
    ...
    mid = clamp(mid * (0.7214 + 0.5861 * texDetail) + 0.2943 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.8040 * rimLine, 0.0, 1.4);
    ...
    float emit = atlas.a * (0.7327 + 0.3495 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1409 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.7379 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3308 + 0.4527 * presence + 0.4580 * pattern + 0.1506 * (1.0 - deepTrans) + 0.1476 * emit, 0.9142);
