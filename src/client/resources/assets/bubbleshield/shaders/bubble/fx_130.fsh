...
void main() {
    ...
    mid = clamp(mid * (0.7055 + 0.7343 * texDetail) + 0.2207 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5048), 0.0, 1.0);
    float emit = atlas.a * (0.7815 + 0.4684 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1243 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.7259 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4282 + 0.4021 * presence + 0.4023 * pattern + 0.0824 * (1.0 - deepTrans) + 0.1407 * emit, 0.8815);
