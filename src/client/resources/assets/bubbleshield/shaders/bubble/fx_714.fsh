...
void main() {
    ...
    mid = clamp(mid * (0.6295 + 0.6267 * texDetail) + 0.3007 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4116), 0.0, 1.0);
    float emit = atlas.a * (0.6500 + 0.4048 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1761 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.7076 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3553 + 0.4610 * presence + 0.4537 * pattern + 0.1283 * (1.0 - deepTrans) + 0.1938 * emit, 0.8974);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1576), 0.8974);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7732 + 0.2268 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1107), 0.0, 1.0);
