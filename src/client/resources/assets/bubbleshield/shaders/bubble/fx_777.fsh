...
void main() {
    ...
    mid = clamp(mid * (0.6058 + 0.5274 * texDetail) + 0.3165 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5143), 0.0, 1.0);
    float emit = atlas.a * (0.5530 + 0.2908 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1713 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.8409 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3339 + 0.2366 * presence + 0.5632 * pattern + 0.1430 * (1.0 - deepTrans) + 0.1727 * emit, 0.8605);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2696), 0.8605);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1342), 0.0, 1.0);
