...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.036667, -0.010000) * time, time);
    deep += 0.5763 * deepField(duvBase + rimDir * 0.0734 + vec2(-0.056667, -0.015000) * time, time);
    deep += 0.3321 * deepField(duvBase + rimDir * 0.1469 + vec2(-0.076667, -0.020000) * time, time);
    deep = pow(clamp(deep * 0.5240, 0.0, 1.0), 1.0267);
