...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.020833, -0.025000) * time, time);
    deep += 0.4863 * deepField(duvBase + rimDir * 0.0379 + vec2(0.032500, -0.040000) * time, time);
    deep += 0.2365 * deepField(duvBase + rimDir * 0.0759 + vec2(0.044167, -0.053333) * time, time);
    deep += 0.1150 * deepField(duvBase + rimDir * 0.1138 + vec2(0.056667, -0.068333) * time, time);
    deep = pow(clamp(deep * 0.5441, 0.0, 1.0), 1.5071);
