...
void main() {
    ...
    mid = clamp(mid * (0.7383 + 0.5205 * texDetail) + 0.3068 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3760), 0.0, 1.0);
    float emit = atlas.a * (0.7447 + 0.3986 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0944 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.6833 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4010 + 0.4858 * presence + 0.4613 * pattern + 0.1070 * (1.0 - deepTrans) + 0.2006 * emit, 0.9568);
