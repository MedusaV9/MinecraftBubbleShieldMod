...
void main() {
    ...
    mid = clamp(mid * (0.6507 + 0.5071 * texDetail) + 0.2380 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5175), 0.0, 1.0);
    float emit = atlas.a * (0.6882 + 0.4512 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0980 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7823 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4029 + 0.3965 * presence + 0.3647 * pattern + 0.0824 * (1.0 - deepTrans) + 0.1421 * emit, 0.9104);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1694), 0.9104);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6621 + 0.3379 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1158), 0.0, 1.0);
