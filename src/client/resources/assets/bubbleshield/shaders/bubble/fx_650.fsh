...
void main() {
    ...
    mid = clamp(mid * (0.7430 + 0.5936 * texDetail) + 0.2685 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4604), 0.0, 1.0);
    float emit = atlas.a * (0.5751 + 0.3675 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0822 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.8846 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2695 + 0.2998 * presence + 0.6127 * pattern + 0.1070 * (1.0 - deepTrans) + 0.1491 * emit, 0.8891);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2824), 0.8891);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6420 + 0.3580 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
