...
// stack: deep=fbm x4 taps | mid=deepice+warp2+scroll | rim=graze_sparkle | flourish=swirl
...
void main() {
    ...
    rgb = mix(rgb, mix(secCol, deepStop, 0.5), clamp(crackB * (1.0 - crackA), 0.0, 1.0) * 0.3694);
    ...
    float alpha = vertexColor.a * min(0.0578 + 0.3522 * presence + 0.4106 * pattern + 0.1189 * (1.0 - deepTrans), 0.8470);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2430), 0.8470);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5497 + 0.4503 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
