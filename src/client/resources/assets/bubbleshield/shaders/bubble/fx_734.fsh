...
void main() {
    ...
    mid = clamp(mid * (0.7493 + 0.6801 * texDetail) + 0.2309 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.8545 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3840), 0.0, 1.0);
    float emit = atlas.a * (0.5989 + 0.4192 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1040 * (0.5 + 0.5 * sin(time * 0.178024)));
    ...
    float alpha = vertexColor.a * min(0.3966 + 0.4922 * presence + 0.4106 * pattern + 0.1189 * (1.0 - deepTrans) + 0.1772 * emit, 0.9070);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2430), 0.9070);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7797 + 0.2203 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1052), 0.0, 1.0);
