...
void main() {
    ...
    mid = clamp(mid * (0.6341 + 0.4765 * texDetail) + 0.2559 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4294), 0.0, 1.0);
    float emit = atlas.a * (0.7037 + 0.4357 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1004 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.8796 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4063 + 0.4247 * presence + 0.4339 * pattern + 0.1228 * (1.0 - deepTrans) + 0.1565 * emit, 0.8451);
