...
// stack: deep=ridge x3 taps | mid=aethersmoke+curl+rotate | rim=lat | flourish=glint
...
void main() {
    ...
    float flourish = 0.2499 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0639 + 0.2674 * presence + 0.4248 * pattern + 0.0944 * (1.0 - deepTrans), 0.7993);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3343), 0.7993);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4660 + 0.5340 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
