...
void main() {
    ...
    mid = clamp(mid * (0.6567 + 0.4712 * texDetail) + 0.2301 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3911), 0.0, 1.0);
    float emit = atlas.a * (0.6900 + 0.4872 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1208 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.8747 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3638 + 0.3474 * presence + 0.4248 * pattern + 0.0944 * (1.0 - deepTrans) + 0.2074 * emit, 0.8593);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3343), 0.8593);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7010 + 0.2990 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1091), 0.0, 1.0);
