...
void main() {
    ...
    mid = clamp(mid * (0.6425 + 0.5128 * texDetail) + 0.2612 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4950), 0.0, 1.0);
    float emit = atlas.a * (0.6936 + 0.4330 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1682 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.8510 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3479 + 0.3858 * presence + 0.4564 * pattern + 0.1031 * (1.0 - deepTrans) + 0.1678 * emit, 0.8644);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3490), 0.8644);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6980 + 0.3020 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1306), 0.0, 1.0);
