...
void main() {
    ...
    mid = clamp(mid * (0.6585 + 0.5922 * texDetail) + 0.3210 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3755), 0.0, 1.0);
    float emit = atlas.a * (0.7013 + 0.2949 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1589 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.9704 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3408 + 0.3469 * presence + 0.4169 * pattern + 0.1156 * (1.0 - deepTrans) + 0.1703 * emit, 0.9044);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3072), 0.9044);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7335 + 0.2665 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1253), 0.0, 1.0);
