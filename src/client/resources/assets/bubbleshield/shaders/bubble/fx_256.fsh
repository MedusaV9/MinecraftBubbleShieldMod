...
void main() {
    ...
    mid = clamp(mid * (0.7074 + 0.6212 * texDetail) + 0.2229 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5345), 0.0, 1.0);
    float emit = atlas.a * (0.6342 + 0.3176 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0871 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.7777 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3512 + 0.4075 * presence + 0.4292 * pattern + 0.1202 * (1.0 - deepTrans) + 0.2016 * emit, 0.8674);
