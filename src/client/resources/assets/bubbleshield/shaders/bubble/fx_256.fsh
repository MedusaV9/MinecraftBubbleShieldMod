...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3216 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.335103);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.001667, -0.012500) * time, time);
    deep += 0.4602 * deepField(duvBase + rimDir * 0.0681 + vec2(0.003333, -0.020000) * time, time);
    deep += 0.2118 * deepField(duvBase + rimDir * 0.1361 + vec2(0.005000, -0.025000) * time, time);
    deep = pow(clamp(deep * 0.5981, 0.0, 1.0), 1.6345);
    ...
    float rn = fbm2(wuv + vec2(time * 0.058333, time * 0.046667), midPer);
    ...
    float pattern = clamp(0.4543 * deep + 0.9801 * mid + 0.8197 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.3772 + pattern * 0.6183);
    vec3 rgb = vertexColor.rgb * (0.4142 + 0.6829 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2434 + 0.6601 * pattern, 0.0, 1.0);
