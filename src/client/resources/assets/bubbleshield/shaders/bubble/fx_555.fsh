...
void main() {
    ...
    mid = clamp(mid * (0.6006 + 0.6865 * texDetail) + 0.2883 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4812), 0.0, 1.0);
    float emit = atlas.a * (0.6778 + 0.4705 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1349 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7651 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3112 + 0.3062 * presence + 0.4926 * pattern + 0.1347 * (1.0 - deepTrans) + 0.1742 * emit, 0.8707);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2151), 0.8707);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7051 + 0.2949 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
