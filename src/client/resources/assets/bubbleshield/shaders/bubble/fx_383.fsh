...
void main() {
    ...
    mid = clamp(mid * (0.6786 + 0.6899 * texDetail) + 0.2937 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7239 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5032), 0.0, 1.0);
    float emit = atlas.a * (0.5861 + 0.4225 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1187 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.9422 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3584 + 0.4280 * presence + 0.3885 * pattern + 0.1120 * (1.0 - deepTrans) + 0.2004 * emit, 0.8832);
