...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7351, 0.0, 1.0);
    ...
    deepCol += 0.4478 * dp * mix(baseCol, deepStop, 0.1960);
    deepPat += 0.4478 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 3.7534) + par * 0.0811, time);
    deepCol += 0.2005 * dp * mix(baseCol, deepStop, 0.3921);
    deepPat += 0.2005 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 4.7263) + par * 0.1216, time);
    deepCol += 0.0898 * dp * mix(baseCol, deepStop, 0.5881);
    deepPat += 0.0898 * dp;
    deepCol *= 0.7258;
    deepPat = pow(clamp(deepPat * 0.5753, 0.0, 1.0), 1.2995);
    ...
    rgb = satLift(rgb, 1.1909);
    rgb += hotStop * 0.3421 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0700 + 0.2981 * presence + 0.3370 * pattern, 0.8422);
