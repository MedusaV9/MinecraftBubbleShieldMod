...
void main() {
    ...
    mid = clamp(mid * (0.7215 + 0.5001 * texDetail) + 0.2059 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4362), 0.0, 1.0);
    float emit = atlas.a * (0.7736 + 0.3383 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1747 * (0.5 + 0.5 * sin(time * 0.235619)));
    ...
    float alpha = vertexColor.a * min(0.3553 + 0.3313 * presence + 0.5560 * pattern + 0.1231 * (1.0 - deepTrans) + 0.1917 * emit, 0.9036);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1628), 0.9036);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7241 + 0.2759 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1367), 0.0, 1.0);
