...
void main() {
    ...
    mid = clamp(mid * (0.7419 + 0.6572 * texDetail) + 0.1934 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6615 + 0.4532 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1322 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.6623 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3389 + 0.3588 * presence + 0.5282 * pattern + 0.1336 * (1.0 - deepTrans) + 0.1666 * emit, 0.8752);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2991), 0.8752);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0701), 0.0, 1.0);
