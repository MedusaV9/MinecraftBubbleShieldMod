...
void main() {
    ...
    mid = clamp(mid * (0.6322 + 0.7153 * texDetail) + 0.2398 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3789), 0.0, 1.0);
    float emit = atlas.a * (0.7588 + 0.4215 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1112 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.7092 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3650 + 0.3288 * presence + 0.4835 * pattern + 0.0900 * (1.0 - deepTrans) + 0.1944 * emit, 0.8798);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2903), 0.8798);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6681 + 0.3319 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1133), 0.0, 1.0);
