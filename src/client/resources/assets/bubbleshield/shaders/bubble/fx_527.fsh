...
void main() {
    ...
    mid = clamp(mid * (0.6940 + 0.5449 * texDetail) + 0.2016 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4072), 0.0, 1.0);
    float emit = atlas.a * (0.7208 + 0.3117 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1411 * (0.5 + 0.5 * sin(time * 0.193732)));
    ...
    float alpha = vertexColor.a * min(0.3147 + 0.3919 * presence + 0.4116 * pattern + 0.0887 * (1.0 - deepTrans) + 0.1712 * emit, 0.8298);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2604), 0.8298);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7197 + 0.2803 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1219), 0.0, 1.0);
