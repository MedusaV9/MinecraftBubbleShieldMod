...
void main() {
    ...
    mid = clamp(mid * (0.7020 + 0.5781 * texDetail) + 0.1868 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4394), 0.0, 1.0);
    float emit = atlas.a * (0.6600 + 0.4557 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1284 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.6613 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3418 + 0.3014 * presence + 0.4635 * pattern + 0.0876 * (1.0 - deepTrans) + 0.1579 * emit, 0.8750);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1709), 0.8750);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7728 + 0.2272 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1131), 0.0, 1.0);
