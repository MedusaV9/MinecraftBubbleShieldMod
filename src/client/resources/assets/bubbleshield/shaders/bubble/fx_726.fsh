...
void main() {
    ...
    mid = clamp(mid * (0.6871 + 0.7028 * texDetail) + 0.3420 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4050), 0.0, 1.0);
    float emit = atlas.a * (0.7631 + 0.3912 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1060 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.8573 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3266 + 0.4546 * presence + 0.4528 * pattern + 0.0999 * (1.0 - deepTrans) + 0.1517 * emit, 0.8625);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2369), 0.8625);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7514 + 0.2486 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0734), 0.0, 1.0);
