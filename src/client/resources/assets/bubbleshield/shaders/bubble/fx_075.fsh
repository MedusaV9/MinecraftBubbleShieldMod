...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6711, 0.0, 1.0);
    ...
    deepCol += 0.4669 * dp * mix(baseCol, deepStop, 0.2711);
    deepPat += 0.4669 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 3.9921) + par * 0.0907, time);
    deepCol += 0.2180 * dp * mix(baseCol, deepStop, 0.5423);
    deepPat += 0.2180 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 5.0718) + par * 0.1360, time);
    deepCol += 0.1018 * dp * mix(baseCol, deepStop, 0.8134);
    deepPat += 0.1018 * dp;
    deepCol *= 0.6464;
    deepPat = pow(clamp(deepPat * 0.5597, 0.0, 1.0), 1.1379);
    ...
    float midCover = clamp(1.1409 * mid + 0.4273 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.0910);
    rgb += hotStop * 0.2270 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0659 + 0.2603 * presence + 0.3340 * pattern, 0.8256);
