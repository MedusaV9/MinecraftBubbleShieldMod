...
// stack: deep=fbm x3 taps | mid=spectralveil+none+rotate | rim=graze | flourish=glint
...
void main() {
    ...
    float flourish = 0.2659 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0707 + 0.1596 * presence + 0.4904 * pattern + 0.1541 * (1.0 - deepTrans), 0.7509);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1693), 0.7509);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5008 + 0.4992 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
