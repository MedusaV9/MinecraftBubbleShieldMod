...
void main() {
    ...
    mid = clamp(mid * (0.6098 + 0.6362 * texDetail) + 0.3041 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.6078 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5045), 0.0, 1.0);
    float emit = atlas.a * (0.5714 + 0.2894 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1412 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.8381 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3388 + 0.2996 * presence + 0.4904 * pattern + 0.1541 * (1.0 - deepTrans) + 0.1977 * emit, 0.8109);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1693), 0.8109);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7337 + 0.2663 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0626), 0.0, 1.0);
