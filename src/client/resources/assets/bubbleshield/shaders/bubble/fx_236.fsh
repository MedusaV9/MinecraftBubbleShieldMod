...
// stack: deep=fbm x3 taps | mid=plasma+none+flicker | rim=graze | flourish=swirl
...
void main() {
    ...
    deep += 0.5362 * deepField(duvBase + rimDir * 0.0667 + vec2(0.023333, -0.020000) * time, time);
    deep += 0.2876 * deepField(duvBase + rimDir * 0.1334 + vec2(0.030000, -0.025000) * time, time);
    deep = pow(clamp(deep * 0.5483, 0.0, 1.0), 1.0727);
    ...
    vec2 wuv = auv;
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.105000, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.126667), midPer)), midPer);
    ...
    float pattern = clamp(0.4751 * deep + 0.8996 * mid + 0.7338 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.3696 + pattern * 0.5910);
    vec3 rgb = vertexColor.rgb * (0.4076 + 0.8924 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1380 + 0.6484 * pattern, 0.0, 1.0);
