...
void main() {
    ...
    mid = clamp(mid * (0.6218 + 0.6373 * texDetail) + 0.3253 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.5100 * rimLine, 0.0, 1.4);
    ...
    float emit = atlas.a * (0.5503 + 0.3682 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1019 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.8187 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3131 + 0.3326 * presence + 0.5824 * pattern + 0.0812 * (1.0 - deepTrans) + 0.1225 * emit, 0.9123);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2742), 0.9123);
    // [layer:v5:ghostalpha]
