...
void main() {
    ...
    mid = clamp(mid * (0.6718 + 0.4818 * texDetail) + 0.3251 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3966), 0.0, 1.0);
    float emit = atlas.a * (0.6221 + 0.4316 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1596 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.9480 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2856 + 0.2321 * presence + 0.6314 * pattern + 0.1090 * (1.0 - deepTrans) + 0.1621 * emit, 0.8878);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2674), 0.8878);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7005 + 0.2995 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0674), 0.0, 1.0);
