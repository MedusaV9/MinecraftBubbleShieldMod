...
void main() {
    ...
    mid = clamp(mid * (0.6499 + 0.7236 * texDetail) + 0.2857 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4909), 0.0, 1.0);
    float emit = atlas.a * (0.6404 + 0.3725 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1178 * (0.5 + 0.5 * sin(time * 0.277507)));
    ...
    float alpha = vertexColor.a * min(0.3695 + 0.4824 * presence + 0.3468 * pattern + 0.1163 * (1.0 - deepTrans) + 0.1735 * emit, 0.8785);
