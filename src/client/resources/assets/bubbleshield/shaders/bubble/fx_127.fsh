...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.030000, 0.045000) * time, time);
    deep += 0.4807 * deepField(duvBase + rimDir * 0.0551 + vec2(0.046667, 0.070000) * time, time);
    deep += 0.2311 * deepField(duvBase + rimDir * 0.1102 + vec2(0.063333, 0.095000) * time, time);
    deep = pow(clamp(deep * 0.5842, 0.0, 1.0), 1.3318);
    ...
    float n = fbm2(wuv + vec2(0.0, time * 0.510000), midPer);
