...
void main() {
    ...
    mid = clamp(mid * (0.6964 + 0.6504 * texDetail) + 0.3126 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3979), 0.0, 1.0);
    float emit = atlas.a * (0.7819 + 0.3944 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1364 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9091 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3458 + 0.2843 * presence + 0.4866 * pattern + 0.0978 * (1.0 - deepTrans) + 0.1205 * emit, 0.8923);
