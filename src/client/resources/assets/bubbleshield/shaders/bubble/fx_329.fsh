...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6743, 0.0, 1.0);
    ...
    deepCol += 0.4222 * dp * mix(baseCol, deepStop, 0.1863);
    deepPat += 0.4222 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 3.4840) + par * 0.0822, time);
    deepCol += 0.1783 * dp * mix(baseCol, deepStop, 0.3725);
    deepPat += 0.1783 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.1538) + par * 0.1233, time);
    deepCol += 0.0753 * dp * mix(baseCol, deepStop, 0.5588);
    deepPat += 0.0753 * dp;
    deepCol *= 0.7686;
    deepPat = pow(clamp(deepPat * 0.5967, 0.0, 1.0), 1.7927);
    ...
    rgb += hotStop * 0.3409 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0737 + 0.3120 * presence + 0.3639 * pattern, 0.8323);
