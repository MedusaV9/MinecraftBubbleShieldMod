...
void main() {
    ...
    mid = clamp(mid * (0.6950 + 0.5943 * texDetail) + 0.3125 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4739), 0.0, 1.0);
    float emit = atlas.a * (0.5912 + 0.3874 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1233 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.8355 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3831 + 0.3764 * presence + 0.3724 * pattern + 0.0940 * (1.0 - deepTrans) + 0.1716 * emit, 0.9176);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2049), 0.9176);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7256 + 0.2744 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0921), 0.0, 1.0);
