...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.025000, -0.007500) * time, time);
    deep += 0.3868 * deepField(duvBase + rimDir * 0.0786 + vec2(0.038333, -0.012500) * time, time);
    deep += 0.1496 * deepField(duvBase + rimDir * 0.1573 + vec2(0.053333, -0.017500) * time, time);
    deep += 0.0579 * deepField(duvBase + rimDir * 0.2359 + vec2(0.066667, -0.022500) * time, time);
    deep = pow(clamp(deep * 0.6272, 0.0, 1.0), 1.0875);
    ...
    float pattern = clamp(0.3069 * deep + 0.8174 * mid + 0.5785 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6882 + pattern * 0.5595);
    vec3 rgb = vertexColor.rgb * (0.4489 + 0.7227 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2354 + 0.8497 * pattern, 0.0, 1.0);
