...
void main() {
    ...
    mid = clamp(mid * (0.6940 + 0.7339 * texDetail) + 0.2660 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4555), 0.0, 1.0);
    float emit = atlas.a * (0.6106 + 0.4309 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0808 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.7155 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3850 + 0.5066 * presence + 0.3805 * pattern + 0.1363 * (1.0 - deepTrans) + 0.1389 * emit, 0.9196);
