...
void main() {
    ...
    mid = clamp(mid * (0.6032 + 0.4695 * texDetail) + 0.2604 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4790), 0.0, 1.0);
    float emit = atlas.a * (0.5811 + 0.3462 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1146 * (0.5 + 0.5 * sin(time * 0.167552)));
    ...
    float alpha = vertexColor.a * min(0.3533 + 0.4854 * presence + 0.3248 * pattern + 0.1468 * (1.0 - deepTrans) + 0.1477 * emit, 0.8796);
