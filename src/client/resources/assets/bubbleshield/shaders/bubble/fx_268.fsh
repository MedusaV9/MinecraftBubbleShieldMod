...
// stack: deep=ridge x2 taps | mid=thinfilm+warp1+rotate | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.033333, 0.045000) * time, time);
    deep += 0.5669 * deepField(duvBase + rimDir * 0.0360 + vec2(0.050000, 0.070000) * time, time);
    deep = pow(clamp(deep * 0.6382, 0.0, 1.0), 1.3795);
    ...
    float pattern = clamp(0.3872 * deep + 0.8079 * mid + 0.7168 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.0890 + pattern * 0.3192);
    vec3 rgb = vertexColor.rgb * (0.4476 + 0.9243 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1255 + 0.6089 * pattern, 0.0, 1.0);
