...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8190, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0370, abs(glR - 0.1480));
    float mid = clamp(glStars * (1.0 + glRing * 1.8614) + glRing * 0.3663, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6148 + 0.6479 * texDetail) + 0.2972 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8631 + 0.3347 * texDetail;
    rgb = satLift(rgb, 1.2449);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5176), 0.0, 1.0);
    float emit = atlas.a * (0.7743 + 0.4259 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1080 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7207 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2673 + 0.2742 * presence + 0.4837 * pattern + 0.1574 * (1.0 - deepTrans) + 0.1658 * emit, 0.9050);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2024), 0.9050);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6945 + 0.3055 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
