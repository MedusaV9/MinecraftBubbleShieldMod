...
// stack: deep=caustic x3 taps | mid=gravlens+warp2+rotate | rim=graze_sparkle | flourish=swirl | tex=starfield
// fbm: standard x6 octaves | seed 70f527ceb4f41fb1
// v8 realism: refr 1.4785+2.0228*fres/dist (cap 0.1030) | energy 0.2079+0.3524*p | flow 0.115000 c/s | soft 0.9344m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.0092 + 0.1618, 0.0, 1.0);
    ...
    float refrAmp = min((1.4785 + 2.0228 * fresnel) / max(length(worldPos), 1.0), 0.1030);
    ...
    float emitMask = clamp(atlas.a * 2.0647, 0.0, 1.0);
    float emit = emitMask * (0.4346 + 0.6611 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1080 * (0.5 + 0.5 * sin(time * 0.198968))) * atlasPoleW;
