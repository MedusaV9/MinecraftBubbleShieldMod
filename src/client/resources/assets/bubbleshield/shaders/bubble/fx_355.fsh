...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7850, 0.0, 1.0);
    ...
    deepCol += 0.5443 * dp * mix(baseCol, deepStop, 0.4123);
    deepPat += 0.5443 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 4.6453) + par * 0.1483, time);
    deepCol += 0.2962 * dp * mix(baseCol, deepStop, 0.8245);
    deepPat += 0.2962 * dp;
    deepCol *= 0.5897;
    deepPat = pow(clamp(deepPat * 0.5433, 0.0, 1.0), 1.0285);
    ...
    vec2 wuv = auv;
    float curve1 = 0.5 + 0.1798 * sin(baseUV.x * 6.2831853 * 1.0000 + time * 0.329867) + 0.0582 * sin(baseUV.x * 6.2831853 * 3.0000 - time * 0.157080);
    float slope1 = cos(baseUV.x * 6.2831853 * 1.0000 + time * 0.329867);
    float band1 = invsmooth(0.005, 0.0632, abs(baseUV.y - curve1));
    float curve2 = 0.5 + 0.2084 * sin(baseUV.x * 6.2831853 * 2.0000 - time * 0.350811 + 2.1) + 0.0237;
    float band2 = invsmooth(0.005, 0.0559, abs(baseUV.y - curve2));
    float foldGlow = 0.6 + 0.4 * slope1 * slope1;
    float shimmer = fbm2(wuv, midPer) * 0.2270;
    ...
    rgb = satLift(rgb, 1.1685);
    rgb += hotStop * 0.2318 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0731 + 0.2796 * presence + 0.3988 * pattern, 0.8482);
