...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5871, 0.0, 1.0);
    ...
    deepCol += 0.5160 * dp * mix(baseCol, deepStop, 0.2383);
    deepPat += 0.5160 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.9248) + par * 0.1234, time);
    deepCol += 0.2662 * dp * mix(baseCol, deepStop, 0.4765);
    deepPat += 0.2662 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.0309) + par * 0.1851, time);
    deepCol += 0.1374 * dp * mix(baseCol, deepStop, 0.7148);
    deepPat += 0.1374 * dp;
    deepCol *= 0.5762;
    deepPat = pow(clamp(deepPat * 0.5209, 0.0, 1.0), 1.3975);
    ...
    rgb += hotStop * 0.2883 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0514 + 0.3444 * presence + 0.4156 * pattern, 0.8236);
