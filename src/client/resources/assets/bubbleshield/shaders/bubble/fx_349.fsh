...
void main() {
    ...
    mid = clamp(mid * (0.7057 + 0.6311 * texDetail) + 0.2223 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3989), 0.0, 1.0);
    float emit = atlas.a * (0.6361 + 0.2736 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1577 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.9326 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3710 + 0.4346 * presence + 0.3569 * pattern + 0.1097 * (1.0 - deepTrans) + 0.1820 * emit, 0.9110);
