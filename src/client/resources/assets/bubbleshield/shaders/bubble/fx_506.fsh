...
void main() {
    ...
    mid = clamp(mid * (0.6553 + 0.6503 * texDetail) + 0.2683 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.5610 + 0.2935 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0936 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.6640 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3928 + 0.3653 * presence + 0.4403 * pattern + 0.1022 * (1.0 - deepTrans) + 0.1875 * emit, 0.8938);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3285), 0.8938);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6887 + 0.3113 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1379), 0.0, 1.0);
