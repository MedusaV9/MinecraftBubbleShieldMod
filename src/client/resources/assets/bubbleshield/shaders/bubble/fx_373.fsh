...
// Bubble surface shader fx_373 -- family EMBERSTORM
// stack: deep=ridge x3 taps | mid=emberstorm+warp1+flicker | rim=graze_sparkle | flourish=shimmer
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.6022 * (vec2(fbm2(p + vec2(0.0, t * 0.311667), per),
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6937, 0.0, 1.0);
    ...
    deepCol += 0.4890 * dp * mix(baseCol, deepStop, 0.3475);
    deepPat += 0.4890 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.5138) + par * 0.1339, time);
    deepCol += 0.2392 * dp * mix(baseCol, deepStop, 0.6950);
    deepPat += 0.2392 * dp;
    deepCol *= 0.6595;
    deepPat = pow(clamp(deepPat * 0.5786, 0.0, 1.0), 1.4708);

    // [layer:mid:emberstorm_warp1_flicker]
    // Signature structure of this effect, domain-warped and animated.
    float jump = hash11(floor(time * 3.5791));
    float ft = time + 0.3183 * jump;
    vec2 auv = vec2(baseUV.x * 9.0000, baseUV.y * 9.0000) + vec2(0.435000, 0.146667) * ft;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        vec2 eu = vec2(wuv.x * (fi + 1.0), wuv.y * (fi + 1.0) - time * 0.293333 * (fi + 1.0)) + vec2(fi * 13.7, fi * 5.3);
        vec2 ecell = floor(eu);
        ...
        vec2 ed = ef - eoff * 0.5;
        float streak = invsmooth(0.01, 0.0791, abs(ed.x)) * invsmooth(0.04, 0.3429, abs(ed.y));
        float heat = 0.5 + 0.5 * sin(time * (3.0 + 4.0 * eh) + eh * 47.0);
        embers += step(0.6722, eh) * streak * (0.4 + 0.6 * heat) * (1.0 - fi * 0.22);
    }
    float shimmer = fbm2(wuv + vec2(0.0, -time * 0.293333), midPer) * 0.1827;
    ...
    float flourish = 0.1045 * caustic(wuv + vec2(3.1, 8.7), vec2(0.195000, -0.119167) * time, midPer);
    float grain = 0.0903 * (cellHash(floor(wuv * 57.0000) + vec2(floor(time * 6.0), 0.0), 513.0000) - 0.5);
    ...
    float pattern = clamp(0.3299 * deepPat + 0.9119 * mid + 0.5441 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb = satLift(rgb, 1.1155);
    rgb += hotStop * 0.2978 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0744 + 0.2528 * presence + 0.3650 * pattern, 0.8521);
