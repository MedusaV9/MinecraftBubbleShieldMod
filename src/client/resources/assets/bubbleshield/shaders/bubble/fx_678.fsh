...
void main() {
    ...
    mid = clamp(mid * (0.7138 + 0.6982 * texDetail) + 0.2632 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4104), 0.0, 1.0);
    float emit = atlas.a * (0.7098 + 0.3715 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1076 * (0.5 + 0.5 * sin(time * 0.219911)));
    ...
    float alpha = vertexColor.a * min(0.2901 + 0.3333 * presence + 0.4530 * pattern + 0.1278 * (1.0 - deepTrans) + 0.1641 * emit, 0.8588);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3340), 0.8588);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7616 + 0.2384 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
