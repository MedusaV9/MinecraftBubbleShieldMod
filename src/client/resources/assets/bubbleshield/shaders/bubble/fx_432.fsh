...
void main() {
    ...
    mid = clamp(mid * (0.6998 + 0.6581 * texDetail) + 0.3393 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5423), 0.0, 1.0);
    float emit = atlas.a * (0.5912 + 0.4927 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0905 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.8653 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3009 + 0.2766 * presence + 0.4916 * pattern + 0.0817 * (1.0 - deepTrans) + 0.1704 * emit, 0.8776);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1805), 0.8776);
    // [layer:v5:ghostalpha]
