...
// stack: deep=ridge x2 taps | mid=hex+curl+rotate | rim=lat | flourish=shimmer
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9157) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9157);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.020000, 0.046667) * time, time);
    deep += 0.4992 * deepField(duvBase + rimDir * 0.0752 + vec2(-0.030000, 0.070000) * time, time);
    deep = pow(clamp(deep * 0.6670, 0.0, 1.0), 1.3014);
    ...
    float pattern = clamp(0.4133 * deep + 0.9828 * mid + 0.6146 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8244 + pattern * 0.4136);
    vec3 rgb = vertexColor.rgb * (0.4546 + 0.8214 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1789 + 0.7172 * pattern, 0.0, 1.0);
