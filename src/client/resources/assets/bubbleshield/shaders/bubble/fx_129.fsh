...
void main() {
    ...
    mid = clamp(mid * (0.7498 + 0.4749 * texDetail) + 0.2889 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4908), 0.0, 1.0);
    float emit = atlas.a * (0.6453 + 0.3472 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1666 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.7002 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4434 + 0.3995 * presence + 0.3279 * pattern + 0.1020 * (1.0 - deepTrans) + 0.2177 * emit, 0.8540);
