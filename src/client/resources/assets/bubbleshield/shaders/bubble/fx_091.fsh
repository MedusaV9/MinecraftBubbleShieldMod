...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6519, 0.0, 1.0);
    ...
    deepCol += 0.4278 * dp * mix(baseCol, deepStop, 0.2497);
    deepPat += 0.4278 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 3.1995) + par * 0.1589, time);
    deepCol += 0.1830 * dp * mix(baseCol, deepStop, 0.4993);
    deepPat += 0.1830 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 3.8992) + par * 0.2383, time);
    deepCol += 0.0783 * dp * mix(baseCol, deepStop, 0.7490);
    deepPat += 0.0783 * dp;
    deepCol *= 0.6611;
    deepPat = pow(clamp(deepPat * 0.5920, 0.0, 1.0), 1.2878);
    ...
    rgb += hotStop * 0.3270 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0594 + 0.3653 * presence + 0.4392 * pattern, 0.8444);
