...
void main() {
    ...
    mid = clamp(mid * (0.6320 + 0.5307 * texDetail) + 0.2109 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4340), 0.0, 1.0);
    float emit = atlas.a * (0.6783 + 0.4619 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1168 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.6648 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3854 + 0.5053 * presence + 0.4392 * pattern + 0.1100 * (1.0 - deepTrans) + 0.1299 * emit, 0.9044);
