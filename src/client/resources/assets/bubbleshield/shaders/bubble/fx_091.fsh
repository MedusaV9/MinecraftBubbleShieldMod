...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.020000, 0.025000) * time, time);
    deep += 0.4278 * deepField(duvBase + rimDir * 0.0794 + vec2(0.030000, 0.037500) * time, time);
    deep += 0.1830 * deepField(duvBase + rimDir * 0.1589 + vec2(0.040000, 0.050000) * time, time);
    deep += 0.0783 * deepField(duvBase + rimDir * 0.2383 + vec2(0.051667, 0.065000) * time, time);
    deep = pow(clamp(deep * 0.5920, 0.0, 1.0), 1.2878);
    ...
    float n = fbm2(wuv + vec2(0.0, time * 0.688333), midPer);
    ...
    float pattern = clamp(0.3323 * deep + 0.9603 * mid + 0.6112 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2580 + pattern * 0.5632);
    vec3 rgb = vertexColor.rgb * (0.4860 + 0.6132 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1653 + 0.7764 * pattern, 0.0, 1.0);
