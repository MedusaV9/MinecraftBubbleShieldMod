...
// stack: deep=ridge x4 taps | mid=lightning+warp1+pulse | rim=graze_sparkle | flourish=echo | tex=filaments
// fbm: ridged x3 octaves | seed 728adc3509511032
// v8 realism: refr 0.5564+0.6018*fres/dist (cap 0.1457) | energy 0.3164+0.4891*p | flow 0.069167 c/s | soft 0.8777m
...
void main() {
    ...
    float refrAmp = min((0.5564 + 0.6018 * fresnel) / max(length(worldPos), 1.0), 0.1457);
