...
void main() {
    ...
    mid = clamp(mid * (0.6497 + 0.6076 * texDetail) + 0.2485 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4552), 0.0, 1.0);
    float emit = atlas.a * (0.6207 + 0.4839 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1615 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.9699 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3687 + 0.3521 * presence + 0.5463 * pattern + 0.0848 * (1.0 - deepTrans) + 0.1430 * emit, 0.9143);
