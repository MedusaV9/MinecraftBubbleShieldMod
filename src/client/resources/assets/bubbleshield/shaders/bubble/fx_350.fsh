...
// stack: deep=caustic x4 taps | mid=kaliset+curl+pulse | rim=graze_sparkle | flourish=echo | tex=nebula
// fbm: ridged x6 octaves | seed ef8acb41580c693c
// v8 realism: refr 0.3214+0.2797*fres/dist (cap 0.1098) | energy 0.4929+0.3766*p | flow 0.071667 c/s | soft 0.8044m
...
void main() {
    ...
    float refrAmp = min((0.3214 + 0.2797 * fresnel) / max(length(worldPos), 1.0), 0.1098);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0406, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9594, vec2(0.001), vec2(0.999))).b);
