...
// v8 realism: refr 1.4805+2.3631*fres/dist (cap 0.1542) | energy 0.1907+0.3794*p | flow 0.103333 c/s | soft 0.9447m
...
void main() {
    ...
    vec2 tuv = vec2(baseUV.x * 4.0000, baseUV.y * 4.0000) + vec2(-0.076667, 0.350000) * time;
    ...
    vec2 texUV = tuv * 2.0000 + vec2(0.009167, -0.033333) * time;
    ...
    rim = clamp(rim + 0.7601 * rimLine + 0.4151 * fresnel, 0.0, 1.4);
    ...
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 10.6270);
    float motifDy = (baseUV.y - 0.5308) * 14.4627;
    ...
    float refrAmp = min((1.4805 + 2.3631 * fresnel) / max(length(worldPos), 1.0), 0.1542);
    ...
    float energyW = clamp(0.1907 + 0.3794 * clamp(pattern, 0.0, 1.0) + 0.2083 * fresnel, 0.0, 0.8711);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.5260;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3458) * 0.5555;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3061 * (1.0 - 0.4623 * baseLuma)), 0.0, 1.0);
    // strobe-free emission drive (see the v7 note above): only smooth,
    // non-gated terms may modulate the emissive add
    float emitDrive = clamp(0.4791 * deepPat + 0.9245 * texDetail + 0.7170 * rim, 0.0, 1.0);
    float emit = atlas.a * (0.3818 + 0.6797 * emitDrive) * (1.0 - 0.1249 * (0.5 + 0.5 * sin(time * 0.183260))) * atlasPoleW;
    rgb += emit * 0.8109 * (1.0 - 0.7234 * baseLuma) * emitCol;
