...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6173, 0.0, 1.0);
    ...
    deepCol += 0.5705 * dp * mix(baseCol, deepStop, 0.3859);
    deepPat += 0.5705 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 4.3150) + par * 0.0965, time);
    deepCol += 0.3255 * dp * mix(baseCol, deepStop, 0.7717);
    deepPat += 0.3255 * dp;
    deepCol *= 0.6475;
    deepPat = pow(clamp(deepPat * 0.5274, 0.0, 1.0), 1.3049);
    ...
    rgb += hotStop * 0.2727 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0458 + 0.2788 * presence + 0.3913 * pattern, 0.8139);
