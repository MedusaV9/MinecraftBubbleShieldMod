...
// Bubble surface shader fx_092 -- family WAVES
// stack: deep=fbm x3 taps | mid=waves+warp1+flicker | rim=graze_film | flourish=swirl | tex=dune_ripples | motif=sigilx1+doublepulse
// fbm: ridged x6 octaves | seed c3ae668eaccbf4d5
// v8 realism: refr 1.6561+2.6434*fres/dist (cap 0.1542) | energy 0.1486+0.3794*p | flow 0.103333 c/s | soft 0.9447m
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.6619, 0.9788, 0.8919) * t
        + vec3(0.1102, 0.5601, 0.3524)));
}
...
void main() {
    ...
    vec3 spun = clamp(hueSpin(baseCol, -0.0092), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.1561) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.4626, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.5276);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, 2.6000), 0.0, 1.0), 0.6656) * 1.7832, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(0.4223, -0.2876, -0.8596);
    vec3 par = Vt * 1.4391;
    ...
    float deepBreath = 1.0 + 0.0129 * sin(time * 1.204277);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * (2.0026 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4079 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * (3.1588 * deepBreath)) + par * 0.0483, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.5788), deepStop, 0.2122);
    deepTrans *= 1.0 - 0.4079 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * (4.3150 * deepBreath)) + par * 0.0965, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4244);
    deepTrans *= 1.0 - 0.4079 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.8798;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.2619, 0.0, 1.0), 1.3049);
    ...
    float ft = time + 0.3633 * (0.6 * sin(time * 3.921755) + 0.4 * sin(time * 7.408923 + 4.1990));
    ...
    vec2 texUV = wuv * 2.0000 + vec2(0.009167, -0.033333) * time;
    ...
    vec3 hgtW = vec3(0.3492, 0.4393, 0.2115);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * 3.6252;
    ...
    float texDetail = clamp(atlas.r * 0.3874 + atlas.g * 0.4873 + atlas.b * 0.2346, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.7249 + 0.5428 * texDetail, atlasPoleW) + 0.1903 * texDetail * (0.3327 + 0.6673 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (2.2325 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.5899);
    ...
    float rimLine = smoothstep(0.0508, 0.0817, rimSlope);
    ...
    rim = clamp(rim + 0.7601 * rimLine + 0.6281 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1197 * pow(clamp(fbm2(wuv + vec2(-time * 0.083333, time * 0.070000), midPer), 0.0, 1.0), 2.0);
    float grain = 0.0720 * (cellHash(floor(wuv * 35.0000) + vec2(floor(time * 2.0), 0.0), 140.0000) - 0.5);
    ...
    float motifP = fract(time * 0.294167 + 0.3615);
    float motifEnv = 0.52 + 0.75 * exp(-140.0 * (motifP - 0.16) * (motifP - 0.16)) + 0.60 * exp(-140.0 * (motifP - 0.38) * (motifP - 0.38));
    float motifA = baseUV.x * 3.0000 + 1.0 * time * 0.076667 + 0.0094;
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 7.7514);
    float motifDy = (baseUV.y - 0.5308) * 10.2070;
    ...
    float pattern = clamp(0.4791 * deepPat * mix(1.0, chordN, 0.2357) + 0.9245 * mid + 0.7170 * rim + flourish + grain + 0.1726 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.7707 + rim * 0.2047, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9787 * mid + 0.4519 * rim, 0.0, 1.0);
    ...
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4425 + 0.5575 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.1260);
    ...
    vec3 softHot = 1.0 - exp(-3.3365 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.3227 * smoothstep(0.55, 1.10, pattern));
    ...
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 1.0162 + baseUV.y * 0.31 + 0.1204) * 6.2831853);
    ...
    rgb *= (1.0 - 0.2873 * washHi) * (1.0 - 0.3898 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.4699, 0.8146, -0.3401);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.5019) / 1.5019, 0.0, 1.0);
    rgb *= 0.6745 + 0.6487 * keyDiff * (1.0 - 0.4535 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 40.6388);
    rgb += hotStop * (keySpec * 0.4943 * (1.0 - 0.6638 * washHi));
    ...
    float refrAmp = min((1.6561 + 2.6434 * fresnel) / max(length(worldPos), 1.0), 0.1542);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.7335) * refrAmp;
    ...
    refracted *= exp(-0.7767 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1418);
    float energyW = clamp(0.1486 + 0.3794 * clamp(pattern, 0.0, 1.0) + 0.3063 * fresnel, 0.0, 0.8711);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2309 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (0.8912 * (1.0 - 0.6875 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3458) * 0.9062;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3061 * max(0.0, 1.0 - 0.4623 * baseLuma - 0.6768 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3818 + 0.6797 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1249 * (0.5 + 0.5 * sin(time * 0.183260))) * atlasPoleW;
    rgb += emit * 0.8109 * (1.0 - 0.7234 * baseLuma) * (1.0 - 0.5229 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2256 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.2351));
    ...
    float bodyAlpha = 0.3451 + 0.4188 * presence + 0.3913 * pattern + 0.1335 * (1.0 - deepTrans) + 0.1732 * emit;
    ...
    // perspective reach.
    float innerFog = clamp(deepField(rotA(spinAxis, time * 0.047124 + 1.3142) * (sdir * 1.8023), time) * 0.9149 + 0.2322, 0.0, 1.0);
    vec3 innerCol = mix(mix(baseCol, secCol, 0.55), deepStop, 0.9 * innerFog);
    rgb = mix(rgb, innerCol, innerFace * 0.3132);
    ...
    bodyAlpha *= smoothstep(0.0, 0.9447, sceneVDist - fragVDist);
