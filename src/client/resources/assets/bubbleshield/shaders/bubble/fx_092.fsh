...
void main() {
    ...
    mid = clamp(mid * (0.7249 + 0.5428 * texDetail) + 0.3336 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4158), 0.0, 1.0);
    float emit = atlas.a * (0.6864 + 0.4414 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1249 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.8109 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3451 + 0.4188 * presence + 0.3913 * pattern + 0.1335 * (1.0 - deepTrans) + 0.1732 * emit, 0.8739);
