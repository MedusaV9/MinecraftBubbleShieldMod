...
void main() {
    ...
    mid = clamp(mid * (0.7005 + 0.6302 * texDetail) + 0.2596 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3781), 0.0, 1.0);
    float emit = atlas.a * (0.7767 + 0.4684 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1460 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.7366 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3018 + 0.2698 * presence + 0.5783 * pattern + 0.1401 * (1.0 - deepTrans) + 0.1953 * emit, 0.8498);
