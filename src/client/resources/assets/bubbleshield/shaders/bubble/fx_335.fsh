...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6100, 0.0, 1.0);
    ...
    deepCol += 0.4700 * dp * mix(baseCol, deepStop, 0.2610);
    deepPat += 0.4700 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 4.2491) + par * 0.0663, time);
    deepCol += 0.2209 * dp * mix(baseCol, deepStop, 0.5220);
    deepPat += 0.2209 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 5.0690) + par * 0.0994, time);
    deepCol += 0.1039 * dp * mix(baseCol, deepStop, 0.7830);
    deepPat += 0.1039 * dp;
    deepCol *= 0.7076;
    deepPat = pow(clamp(deepPat * 0.5571, 0.0, 1.0), 1.4851);
    ...
    rgb = satLift(rgb, 1.1953);
    rgb += hotStop * 0.2829 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0730 + 0.2579 * presence + 0.4059 * pattern, 0.7896);
