...
void main() {
    ...
    mid = clamp(mid * (0.6693 + 0.5570 * texDetail) + 0.3111 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4954), 0.0, 1.0);
    float emit = atlas.a * (0.7957 + 0.3835 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1741 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.7334 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4416 + 0.4537 * presence + 0.3868 * pattern + 0.0941 * (1.0 - deepTrans) + 0.1785 * emit, 0.8896);
