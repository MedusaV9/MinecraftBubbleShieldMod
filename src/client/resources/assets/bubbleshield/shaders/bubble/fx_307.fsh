...
void main() {
    ...
    mid = clamp(mid * (0.6102 + 0.6454 * texDetail) + 0.1853 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4959), 0.0, 1.0);
    float emit = atlas.a * (0.7883 + 0.2979 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1425 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.7918 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3984 + 0.4923 * presence + 0.4825 * pattern + 0.1252 * (1.0 - deepTrans) + 0.2170 * emit, 0.8461);
