...
void main() {
    ...
    mid = clamp(mid * (0.6929 + 0.6315 * texDetail) + 0.2488 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4070), 0.0, 1.0);
    float emit = atlas.a * (0.6399 + 0.4223 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1790 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9550 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3528 + 0.2714 * presence + 0.3945 * pattern + 0.0918 * (1.0 - deepTrans) + 0.1298 * emit, 0.8345);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2000), 0.8345);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7047 + 0.2953 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0795), 0.0, 1.0);
