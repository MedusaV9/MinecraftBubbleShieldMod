...
void main() {
    ...
    mid = clamp(mid * (0.6435 + 0.5961 * texDetail) + 0.2876 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4893), 0.0, 1.0);
    float emit = atlas.a * (0.6237 + 0.2948 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0922 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.8247 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3494 + 0.3367 * presence + 0.4029 * pattern + 0.1217 * (1.0 - deepTrans) + 0.1415 * emit, 0.8827);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1924), 0.8827);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6562 + 0.3438 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
