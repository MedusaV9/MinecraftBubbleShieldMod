...
void main() {
    ...
    mid = clamp(mid * (0.7274 + 0.7030 * texDetail) + 0.3491 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3734), 0.0, 1.0);
    float emit = atlas.a * (0.6020 + 0.4666 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1596 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.8060 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3482 + 0.4457 * presence + 0.4369 * pattern + 0.0951 * (1.0 - deepTrans) + 0.2015 * emit, 0.8644);
