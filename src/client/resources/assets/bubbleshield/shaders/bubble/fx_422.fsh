...
void main() {
    ...
    mid = clamp(mid * (0.7105 + 0.6027 * texDetail) + 0.2833 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7962 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3972), 0.0, 1.0);
    float emit = atlas.a * (0.7901 + 0.3039 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1683 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.7516 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3768 + 0.4267 * presence + 0.4328 * pattern + 0.1573 * (1.0 - deepTrans) + 0.2116 * emit, 0.9131);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2612), 0.9131);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6260 + 0.3740 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1369), 0.0, 1.0);
