...
void main() {
    ...
    mid = clamp(mid * (0.7245 + 0.5812 * texDetail) + 0.2448 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5371), 0.0, 1.0);
    float emit = atlas.a * (0.6910 + 0.4636 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1067 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7381 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3468 + 0.2548 * presence + 0.5860 * pattern + 0.1230 * (1.0 - deepTrans) + 0.1368 * emit, 0.8610);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2700), 0.8610);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7048 + 0.2952 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
