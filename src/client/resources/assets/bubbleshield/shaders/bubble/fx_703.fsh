...
void main() {
    ...
    mid = clamp(mid * (0.7320 + 0.4748 * texDetail) + 0.2773 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4733), 0.0, 1.0);
    float emit = atlas.a * (0.7825 + 0.4038 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0934 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.9879 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3285 + 0.2695 * presence + 0.5697 * pattern + 0.1446 * (1.0 - deepTrans) + 0.1765 * emit, 0.9126);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1555), 0.9126);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7432 + 0.2568 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0912), 0.0, 1.0);
