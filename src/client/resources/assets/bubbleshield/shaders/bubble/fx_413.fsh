...
// Bubble surface shader fx_413 -- family EMBERSTORM
// stack: deep=fbm x3 taps | mid=emberstorm+warp1+pulse | rim=graze | flourish=shimmer
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.7310 * (vec2(fbm2(p + vec2(0.0, t * 0.308333), per),
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7539, 0.0, 1.0);
    ...
    deepCol += 0.4070 * dp * mix(baseCol, deepStop, 0.3235);
    deepPat += 0.4070 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.8883) + par * 0.0943, time);
    deepCol += 0.1656 * dp * mix(baseCol, deepStop, 0.6470);
    deepPat += 0.1656 * dp;
    deepCol *= 0.6938;
    deepPat = pow(clamp(deepPat * 0.6359, 0.0, 1.0), 1.7563);

    // [layer:mid:emberstorm_warp1_pulse]
    ...
    vec2 auv = vec2(baseUV.x * 8.0000 + time * -0.126667, (baseUV.y - 0.5) * 8.0000 * breathe + 4.0000 + time * 0.300000);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        vec2 eu = vec2(wuv.x * (fi + 1.0), wuv.y * (fi + 1.0) - time * 0.183333 * (fi + 1.0)) + vec2(fi * 13.7, fi * 5.3);
        vec2 ecell = floor(eu);
        ...
        vec2 ed = ef - eoff * 0.5;
        float streak = invsmooth(0.01, 0.0841, abs(ed.x)) * invsmooth(0.04, 0.2470, abs(ed.y));
        float heat = 0.5 + 0.5 * sin(time * (3.0 + 4.0 * eh) + eh * 47.0);
        embers += step(0.7149, eh) * streak * (0.4 + 0.6 * heat) * (1.0 - fi * 0.22);
    }
    float shimmer = fbm2(wuv + vec2(0.0, -time * 0.183333), midPer) * 0.1690;
    ...
    float flourish = 0.1327 * caustic(wuv + vec2(3.1, 8.7), vec2(0.146667, -0.116667) * time, midPer);
    float grain = 0.0470 * (cellHash(floor(wuv * 41.0000) + vec2(floor(time * 6.0), 0.0), 328.0000) - 0.5);
    ...
    rgb += hotStop * 0.2390 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0630 + 0.2731 * presence + 0.3338 * pattern, 0.8342);
