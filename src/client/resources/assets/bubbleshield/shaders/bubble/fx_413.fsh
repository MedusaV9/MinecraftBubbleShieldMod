...
void main() {
    ...
    mid = clamp(mid * (0.6281 + 0.5513 * texDetail) + 0.2867 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5248), 0.0, 1.0);
    float emit = atlas.a * (0.7017 + 0.3729 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1722 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.6797 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3960 + 0.4131 * presence + 0.3338 * pattern + 0.1172 * (1.0 - deepTrans) + 0.1357 * emit, 0.8942);
