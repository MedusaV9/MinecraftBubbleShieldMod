...
// stack: deep=ridge x4 taps | mid=sparkle+curl+flicker | rim=graze_film | flourish=swirl | tex=solar_granulation | motif=lissajousx2+breath
// fbm: standard x6 octaves | seed fc3c1c346485c89d
// v8 realism: refr 1.1526+1.6043*fres/dist (cap 0.1584) | energy 0.2044+0.4210*p | flow 0.077500 c/s | soft 0.9258m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0235) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0235);
        amplitude *= 0.5;
    }
    return value;
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4307, x));
    return mix(g, hotStop, smoothstep(0.4307, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0052, 0.0447, g);
}
...
float sparkle(vec2 p, float t, float px) {
    ...
    return step(0.6988, h) * invsmooth(0.02, 0.22, d) * tw;
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.6319;
    vec3 spun = clamp(hueSpin(baseCol, 0.3163), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2007) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5955, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.4528);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, 0.2480), 0.0, 1.0), 0.3500) * 1.6786, 0.0, 1.0);
    ...
    deepTrans *= 1.0 - 0.3945 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.5825;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.1552, 0.0, 1.0), 1.3266);
    ...
    float ft = time + 0.2184 * (0.6 * sin(time * 5.188864) + 0.4 * sin(time * 7.199483 + 4.4173));
    vec2 auv = vec2(baseUV.x * 10.0000, baseUV.y * 10.0000) + vec2(0.391667, -0.010000) * ft;
    vec2 wuv = auv + 0.2584 * curl2(auv + vec2(0.0, time * 0.070000), midPer);
    float s1 = sparkle(wuv, time, midPer.x);
    float s2 = sparkle(wuv * 2.0 + 31.7, time * 1.5650, midPer.x * 2.0);
    float haze = voronoise(wuv + vec2(time * 0.066667, 0.0), midPer, 0.7187, 0.6263);
    ...
    vec2 texUV = wuv * 1.0000 + vec2(-0.044167, -0.015000) * time;
    ...
    vec3 hgtW = vec3(0.4242, 0.2822, 0.2935);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -4.8306;
    ...
    float texDetail = clamp(atlas.r * 0.4533 + atlas.g * 0.3016 + atlas.b * 0.3136, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6872 + 0.5763 * texDetail, atlasPoleW) + 0.1383 * texDetail * (0.2126 + 0.7874 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.6447.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9466 * 0.9466 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.6447);
    ...
    float rimLine = smoothstep(0.0626, 0.0981, rimSlope);
    ...
    rim = clamp(rim + 0.5808 * rimLine + 0.7455 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1846 * pow(clamp(fbm2(wuv + vec2(-time * 0.133333, time * 0.070000), midPer), 0.0, 1.0), 2.0);
    float grain = 0.0735 * (cellHash(floor(wuv * 25.0000) + vec2(floor(time * 2.0), 0.0), 250.0000) - 0.5);
    ...
    float motifEnv = 0.72 + 0.28 * sin(time * 0.895354 + 5.4351);
    float motifM = 0.0;
    float motifU0 = 0.5 + 0.34 * sin(time * 0.345575 + 3.7570);
    float motifV0 = 0.5 + 0.2228 * sin(time * 0.246091 + 1.9429);
    vec2 motifD0 = vec2(sin((baseUV.x - motifU0) * 3.1415927) * 0.6366, baseUV.y - motifV0);
    motifM += exp(-dot(motifD0, motifD0) * 69.9702);
    float motifU1 = 0.5 + 0.34 * sin(time * 0.345575 + 6.1569);
    float motifV1 = 0.5 + 0.2228 * sin(time * 0.246091 + 3.5137);
    vec2 motifD1 = vec2(sin((baseUV.x - motifU1) * 3.1415927) * 0.6366, baseUV.y - motifV1);
    motifM += exp(-dot(motifD1, motifD1) * 69.9702);
    ...
    vec3 softHot = 1.0 - exp(-2.7983 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.2926 * smoothstep(0.55, 1.10, pattern));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.0894 + baseUV.y * 0.4237 + 0.9637) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1506);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 1.1729 + baseUV.y * 0.31 + 0.6998) * 6.2831853);
    ...
    rgb *= (1.0 - 0.3472 * washHi) * (1.0 - 0.3227 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.3952, 0.6794, 0.6182);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4943) / 1.4943, 0.0, 1.0);
    rgb *= 0.6427 + 0.6521 * keyDiff * (1.0 - 0.6093 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 23.4729);
    rgb += hotStop * (keySpec * 0.3153 * (1.0 - 0.7229 * washHi));
    ...
    float refrAmp = min((1.1526 + 1.6043 * fresnel) / max(length(worldPos), 1.0), 0.1584);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.7443) * refrAmp;
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.2430);
    float energyW = clamp(0.2044 + 0.4210 * clamp(pattern, 0.0, 1.0) + 0.3011 * fresnel, 0.0, 0.8675);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2540 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (1.0965 * (1.0 - 0.6610 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2557) * 1.2053;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2938 * max(0.0, 1.0 - 0.5172 * baseLuma - 0.5872 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3904 + 0.5037 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1112 * (0.5 + 0.5 * sin(time * 0.198968))) * atlasPoleW;
    rgb += emit * 0.9017 * (1.0 - 0.6516 * baseLuma) * (1.0 - 0.5468 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2798 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.3882));
    ...
    float bodyAlpha = 0.3817 + 0.4989 * presence + 0.4347 * pattern + 0.1302 * (1.0 - deepTrans) + 0.1293 * emit;
    ...
    bodyAlpha *= smoothstep(0.0, 0.9258, sceneVDist - fragVDist);
