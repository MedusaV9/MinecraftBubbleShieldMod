...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6708, 0.0, 1.0);
    ...
    deepCol += 0.5390 * dp * mix(baseCol, deepStop, 0.2188);
    deepPat += 0.5390 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.7166) + par * 0.0638, time);
    deepCol += 0.2905 * dp * mix(baseCol, deepStop, 0.4376);
    deepPat += 0.2905 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.8220) + par * 0.0956, time);
    deepCol += 0.1566 * dp * mix(baseCol, deepStop, 0.6564);
    deepPat += 0.1566 * dp;
    deepCol *= 0.4910;
    deepPat = pow(clamp(deepPat * 0.5035, 0.0, 1.0), 1.3266);
    ...
    rgb += hotStop * 0.2426 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0581 + 0.3589 * presence + 0.4347 * pattern, 0.8459);
