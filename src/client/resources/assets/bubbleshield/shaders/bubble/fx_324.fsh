...
// stack: deep=ridge x4 taps | mid=sparkle+curl+flicker | rim=graze_film | flourish=swirl | tex=starfield
// fbm: standard x6 octaves | seed fc3c1c346485c89d
// v8 realism: refr 0.5517+0.7680*fres/dist (cap 0.1184) | energy 0.3053+0.4210*p | flow 0.077500 c/s | soft 0.9258m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.0705 + 0.2521, 0.0, 1.0);
    ...
    float refrAmp = min((0.5517 + 0.7680 * fresnel) / max(length(worldPos), 1.0), 0.1184);
    ...
    float emitMask = clamp(atlas.a * 1.9988, 0.0, 1.0);
    ...
    float emit = emitMask * (0.3904 + 0.5037 * emitDrive) * (1.0 - 0.1112 * (0.5 + 0.5 * sin(time * 0.198968))) * atlasPoleW;
