...
void main() {
    ...
    mid = clamp(mid * (0.6872 + 0.5763 * texDetail) + 0.2450 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3982), 0.0, 1.0);
    float emit = atlas.a * (0.7006 + 0.2947 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1112 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.9017 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3817 + 0.4989 * presence + 0.4347 * pattern + 0.1302 * (1.0 - deepTrans) + 0.1293 * emit, 0.9059);
