...
// v8 realism: refr 1.0357+1.4416*fres/dist (cap 0.1584) | energy 0.3053+0.4210*p | flow 0.077500 c/s | soft 0.9258m
...
void main() {
    ...
    vec2 tuv = vec2(baseUV.x * 10.0000, baseUV.y * 10.0000) + vec2(0.391667, -0.010000) * time;
    ...
    vec2 texUV = tuv * 1.0000 + vec2(-0.044167, -0.015000) * time;
    ...
    rim = clamp(rim + 0.5808 * rimLine + 0.5130 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1846 * pow(clamp(fbm2(wuv + vec2(-time * 0.133333, time * 0.070000), midPer), 0.0, 1.0), 2.0);
    float grain = 0.0735 * (cellHash(floor(wuv * 25.0000) + vec2(floor(time * 6.0), 0.0), 250.0000) - 0.5);
    ...
    float motifEnv = 0.55 + 0.45 * sin(time * 0.895354 + 5.4351);
    float motifM = 0.0;
    ...
    motifM += exp(-dot(motifD0, motifD0) * 140.6332);
    ...
    motifM += exp(-dot(motifD1, motifD1) * 140.6332);
    ...
    float refrAmp = min((1.0357 + 1.4416 * fresnel) / max(length(worldPos), 1.0), 0.1584);
    ...
    float energyW = clamp(0.3053 + 0.4210 * clamp(pattern, 0.0, 1.0) + 0.2038 * fresnel, 0.0, 0.8675);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.7613;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2557) * 0.8213;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2938 * (1.0 - 0.5172 * baseLuma)), 0.0, 1.0);
    // strobe-free emission drive (see the v7 note above): only smooth,
    // non-gated terms may modulate the emissive add
    float emitDrive = clamp(0.3455 * deepPat + 0.9775 * texDetail + 0.7852 * rim, 0.0, 1.0);
    float emit = atlas.a * (0.3904 + 0.5037 * emitDrive) * (1.0 - 0.1112 * (0.5 + 0.5 * sin(time * 0.198968))) * atlasPoleW;
    rgb += emit * 0.9017 * (1.0 - 0.6516 * baseLuma) * emitCol;
