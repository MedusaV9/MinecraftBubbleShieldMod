...
void main() {
    ...
    mid = clamp(mid * (0.6595 + 0.6429 * texDetail) + 0.1916 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5102), 0.0, 1.0);
    float emit = atlas.a * (0.6360 + 0.2759 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1014 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.9384 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2757 + 0.2945 * presence + 0.5291 * pattern + 0.1352 * (1.0 - deepTrans) + 0.1984 * emit, 0.8929);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2761), 0.8929);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7097 + 0.2903 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1160), 0.0, 1.0);
