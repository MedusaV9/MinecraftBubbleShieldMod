...
void main() {
    ...
    mid = clamp(mid * (0.7032 + 0.7434 * texDetail) + 0.2501 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4570), 0.0, 1.0);
    float emit = atlas.a * (0.7125 + 0.4444 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0806 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.9430 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4421 + 0.4801 * presence + 0.4399 * pattern + 0.0821 * (1.0 - deepTrans) + 0.1456 * emit, 0.8557);
