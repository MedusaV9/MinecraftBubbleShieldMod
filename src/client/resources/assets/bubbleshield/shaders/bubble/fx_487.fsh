...
void main() {
    ...
    mid = clamp(mid * (0.7103 + 0.4557 * texDetail) + 0.2496 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3712), 0.0, 1.0);
    float emit = atlas.a * (0.6524 + 0.3366 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1658 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.8470 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3687 + 0.3169 * presence + 0.4511 * pattern + 0.1276 * (1.0 - deepTrans) + 0.1524 * emit, 0.8870);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2539), 0.8870);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7617 + 0.2383 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0990), 0.0, 1.0);
