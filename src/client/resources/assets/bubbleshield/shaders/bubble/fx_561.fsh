...
void main() {
    ...
    mid = clamp(mid * (0.6970 + 0.5057 * texDetail) + 0.2042 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5211), 0.0, 1.0);
    float emit = atlas.a * (0.6577 + 0.3149 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1040 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.9339 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4080 + 0.4300 * presence + 0.3101 * pattern + 0.1067 * (1.0 - deepTrans) + 0.2072 * emit, 0.8747);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2087), 0.8747);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6898 + 0.3102 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
