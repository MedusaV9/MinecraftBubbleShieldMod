...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6491, 0.0, 1.0);
    ...
    deepCol += 0.4344 * dp * mix(baseCol, deepStop, 0.4434);
    deepPat += 0.4344 * dp;
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * 4.7040) + par * 0.1554, time);
    deepCol += 0.1887 * dp * mix(baseCol, deepStop, 0.8867);
    deepPat += 0.1887 * dp;
    deepCol *= 0.7841;
    deepPat = pow(clamp(deepPat * 0.6161, 0.0, 1.0), 1.3896);
    ...
    float lonLine = pow(abs(sin(baseUV.x * 3.1415927 * 12.0000)), 21.6900);
    float latLine = pow(abs(sin(baseUV.y * 3.1415927 * 8.0000)), 24.7118);
    float scanPos = fract(time * 0.054167);
    float scan = invsmooth(0.0, 0.1210, abs(baseUV.y - scanPos));
    float glint = step(0.55, cellHash(floor(vec2(baseUV.x * 12.0000, baseUV.y * 8.0000)), 12.0000)) * lonLine * latLine;
    float holoFlicker = 0.82 + 0.18 * step(0.35, hash11(floor(time * 10.6728)));
    float mid = clamp((lonLine + latLine) * 0.4335 * holoFlicker + scan * 0.4533 + glint * 2.2, 0.0, 1.25);
    ...
    rgb += hotStop * 0.3299 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0434 + 0.2536 * presence + 0.3834 * pattern, 0.7992);
