...
void main() {
    ...
    mid = clamp(mid * (0.6140 + 0.5990 * texDetail) + 0.1925 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4807), 0.0, 1.0);
    float emit = atlas.a * (0.7067 + 0.2893 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1711 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.7037 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3924 + 0.4652 * presence + 0.3554 * pattern + 0.1399 * (1.0 - deepTrans) + 0.1748 * emit, 0.9033);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2440), 0.9033);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6597 + 0.3403 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1216), 0.0, 1.0);
