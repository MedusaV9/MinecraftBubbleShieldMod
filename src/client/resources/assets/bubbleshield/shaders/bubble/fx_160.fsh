...
void main() {
    ...
    mid = clamp(mid * (0.7030 + 0.6755 * texDetail) + 0.2665 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4188), 0.0, 1.0);
    float emit = atlas.a * (0.6698 + 0.4044 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1200 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.9315 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3749 + 0.4957 * presence + 0.4473 * pattern + 0.1591 * (1.0 - deepTrans) + 0.2102 * emit, 0.8619);
