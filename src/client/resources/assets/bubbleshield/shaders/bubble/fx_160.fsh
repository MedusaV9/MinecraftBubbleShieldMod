...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3739 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.502655);
...
void main() {
    ...
    deep += 0.4621 * deepField(duvBase + rimDir * 0.0552 + vec2(-0.051667, 0.037500) * time, time);
    deep += 0.2136 * deepField(duvBase + rimDir * 0.1103 + vec2(-0.070000, 0.050000) * time, time);
    deep += 0.0987 * deepField(duvBase + rimDir * 0.1655 + vec2(-0.086667, 0.062500) * time, time);
    deep = pow(clamp(deep * 0.5636, 0.0, 1.0), 1.3899);
    ...
    float pattern = clamp(0.3942 * deep + 0.7481 * mid + 0.5540 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9947 + pattern * 0.4813);
    vec3 rgb = vertexColor.rgb * (0.3756 + 0.6387 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1205 + 0.6138 * pattern, 0.0, 1.0);
