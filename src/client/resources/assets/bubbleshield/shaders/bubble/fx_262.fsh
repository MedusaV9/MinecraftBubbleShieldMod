...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.319395);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.033333, -0.017500) * time, time);
    deep += 0.5091 * deepField(duvBase + rimDir * 0.0611 + vec2(0.053333, -0.025000) * time, time);
    deep += 0.2592 * deepField(duvBase + rimDir * 0.1223 + vec2(0.071667, -0.035000) * time, time);
    deep = pow(clamp(deep * 0.5655, 0.0, 1.0), 1.0074);
    ...
    float pattern = clamp(0.4137 * deep + 0.7910 * mid + 0.8354 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5582 + pattern * 0.5328);
    vec3 rgb = vertexColor.rgb * (0.3888 + 0.6954 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1305 + 0.7740 * pattern, 0.0, 1.0);
