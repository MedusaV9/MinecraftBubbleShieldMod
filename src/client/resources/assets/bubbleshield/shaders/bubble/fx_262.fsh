...
void main() {
    ...
    mid = clamp(mid * (0.6247 + 0.6512 * texDetail) + 0.2192 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5227), 0.0, 1.0);
    float emit = atlas.a * (0.7768 + 0.3908 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1377 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9331 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3575 + 0.3853 * presence + 0.3978 * pattern + 0.1044 * (1.0 - deepTrans) + 0.1821 * emit, 0.8530);
