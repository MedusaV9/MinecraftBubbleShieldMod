...
void main() {
    ...
    mid = clamp(mid * (0.7178 + 0.6176 * texDetail) + 0.3198 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4065), 0.0, 1.0);
    float emit = atlas.a * (0.5876 + 0.2612 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0957 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.9957 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3508 + 0.3856 * presence + 0.4548 * pattern + 0.1413 * (1.0 - deepTrans) + 0.1932 * emit, 0.8937);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1731), 0.8937);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7693 + 0.2307 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
