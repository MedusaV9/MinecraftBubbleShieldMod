...
void main() {
    ...
    mid = clamp(mid * (0.6757 + 0.6467 * texDetail) + 0.2552 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5457), 0.0, 1.0);
    float emit = atlas.a * (0.5590 + 0.4251 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1435 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.9306 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3023 + 0.3529 * presence + 0.4114 * pattern + 0.1243 * (1.0 - deepTrans) + 0.1976 * emit, 0.8910);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2008), 0.8910);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7118 + 0.2882 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0813), 0.0, 1.0);
