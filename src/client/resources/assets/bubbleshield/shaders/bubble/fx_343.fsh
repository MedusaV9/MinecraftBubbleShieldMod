...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8487, 0.0, 1.0);
    ...
    deepCol += 0.4071 * dp * mix(baseCol, deepStop, 0.2497);
    deepPat += 0.4071 * dp;
    dp = deepField(rotA(spinAxis, time * 0.089012) * (sdir * 2.9186) + par * 0.1588, time);
    deepCol += 0.1657 * dp * mix(baseCol, deepStop, 0.4994);
    deepPat += 0.1657 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 3.4772) + par * 0.2383, time);
    deepCol += 0.0675 * dp * mix(baseCol, deepStop, 0.7491);
    deepPat += 0.0675 * dp;
    deepCol *= 0.7124;
    deepPat = pow(clamp(deepPat * 0.6097, 0.0, 1.0), 1.3620);
    ...
    rgb += hotStop * 0.2554 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0736 + 0.3626 * presence + 0.4058 * pattern, 0.8189);
