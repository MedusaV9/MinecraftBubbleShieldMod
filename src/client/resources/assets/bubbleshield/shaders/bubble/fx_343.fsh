...
void main() {
    ...
    mid = clamp(mid * (0.6497 + 0.4544 * texDetail) + 0.3064 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3519), 0.0, 1.0);
    float emit = atlas.a * (0.5937 + 0.3230 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0878 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.6760 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4274 + 0.5026 * presence + 0.4058 * pattern + 0.1202 * (1.0 - deepTrans) + 0.1631 * emit, 0.8789);
