...
// stack: deep=caustic x3 taps | mid=truchet+warp1+rotate | rim=graze | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.040000, -0.032500) * time, time);
    deep += 0.4071 * deepField(duvBase + rimDir * 0.0794 + vec2(-0.061667, -0.050000) * time, time);
    deep += 0.1657 * deepField(duvBase + rimDir * 0.1588 + vec2(-0.083333, -0.070000) * time, time);
    deep = pow(clamp(deep * 0.6358, 0.0, 1.0), 1.3620);
    ...
    float pattern = clamp(0.4120 * deep + 0.8768 * mid + 0.7776 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8004 + pattern * 0.4801);
    vec3 rgb = vertexColor.rgb * (0.3666 + 0.8822 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1944 + 0.6725 * pattern, 0.0, 1.0);
