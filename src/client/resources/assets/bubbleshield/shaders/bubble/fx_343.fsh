...
// Bubble surface shader fx_343 -- family TRUCHET
// stack: deep=caustic x4 taps | mid=truchet+warp1+rotate | rim=graze | flourish=shimmer | tex=coral | motif=corex1+doublepulse
// fbm: turb x4 octaves | seed 45bfd328317b260a
// v8 realism: refr 1.0550+0.8800*fres/dist (cap 0.1978) | energy 0.2484+0.4678*p | flow 0.073333 c/s | soft 0.8615m
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.8877 * (vec2(fbm2(p + vec2(0.0, t * 0.311667), per),
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(1.1645, 0.8148, 0.4001) * t
        + vec3(0.6865, 0.6808, 0.6752)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.5459, x));
    return mix(g, hotStop, smoothstep(0.5459, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0034, 0.0509, g);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.7169;
    vec3 spun = clamp(hueSpin(baseCol, 0.6174), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.3489) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5706, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.6608);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, 1.9116), 0.0, 1.0), 2.2000) * 1.5234, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(0.3481, -0.9355, 0.0603);
    vec3 par = Vt * 2.0000;
    ...
    float deepBreath = 1.0 + 0.0155 * sin(time * 1.424189);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.089012) * (sdir * (1.8015 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.3709 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * (2.3601 * deepBreath)) + par * 0.0794, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.3745), deepStop, 0.1373);
    deepTrans *= 1.0 - 0.3709 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * (2.9186 * deepBreath)) + par * 0.1588, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.7491), deepStop, 0.2747);
    deepTrans *= 1.0 - 0.3709 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * (3.4772 * deepBreath)) + par * 0.2383, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4120);
    deepTrans *= 1.0 - 0.3709 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.6828;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.1858, 0.0, 1.0), 1.3620);

    // [layer:mid:truchet_warp1_rotate]
    ...
    float sway = 0.9760 * sin(time * 0.219911);
    vec2 auv = vec2(baseUV.x * 9.0000 + (baseUV.y - 0.5) * sway + time * -0.007500, baseUV.y * 9.0000);
    vec2 wuv = warp1(auv, midPer, time);
    float t1 = truchet(wuv, 0.0808, midPer.x);
    float t2 = truchet(wuv * 2.0 + vec2(7.3, 3.1), 0.0777, midPer.x * 2.0) * 0.5;
    // the flow phase rides one full longitude turn: seam-aligned
    float flow = 0.5 + 0.5 * sin(baseUV.x * 6.2831853 + wuv.y * 1.7 + time * 1.780236);
    ...
    vec2 texUV = wuv * 1.0000 + vec2(-0.001667, 0.025000) * time;
    ...
    vec3 hgtW = vec3(0.3680, 0.2887, 0.3433);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -2.2633;
    ...
    float texDetail = clamp(atlas.r * 0.4696 + atlas.g * 0.3684 + atlas.b * 0.4381, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6497 + 0.4544 * texDetail, atlasPoleW) + 0.1744 * texDetail * (0.2270 + 0.7730 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.2076 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.4093);
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.7059.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9356 * 0.9356 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.7059);
    ...
    float rimLine = smoothstep(0.0535, 0.0884, rimSlope);
    float rim = rimGraze() * 0.8245;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.5927 * rimLine + 0.6986 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1784 * caustic(wuv + vec2(3.1, 8.7), vec2(0.127500, -0.119167) * time, midPer);
    float grain = 0.0677 * (cellHash(floor(wuv * 63.0000) + vec2(floor(time * 2.0), 0.0), 567.0000) - 0.5);
    ...
    float motifP = fract(time * 0.338333 + 0.9269);
    ...
    float motifM = exp(-motifR * motifR * 12.9741) * (0.75 + 0.25 * sin(time * 0.659734 + 1.9503));
    ...
    float pattern = clamp(0.4120 * deepPat * mix(1.0, chordN, 0.1928) + 0.8768 * mid + 0.7776 * rim + flourish + grain + 0.2013 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.8734 + rim * 0.2408, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(1.0209 * mid + 0.6790 * rim, 0.0, 1.0);
    ...
    vec3 softHot = 1.0 - exp(-2.6929 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.3054 * smoothstep(0.55, 1.10, pattern));
    vec3 accent = accentPalette(0.8004 + pattern * 0.4801);
    rgb = mix(rgb, rgb * (0.55 + 0.9 * accent), 0.4189);
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.0634 + baseUV.y * 0.5372 + 0.7819) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1361);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.7236 + baseUV.y * 0.31 + 0.8479) * 6.2831853);
    ...
    rgb *= (1.0 - 0.2728 * washHi) * (1.0 - 0.2811 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.6117, 0.7357, -0.2907);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4554) / 1.4554, 0.0, 1.0);
    rgb *= 0.6772 + 0.6011 * keyDiff * (1.0 - 0.6227 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 17.5837);
    rgb += hotStop * (keySpec * 0.4115 * (1.0 - 0.5906 * washHi));
    ...
    float refrAmp = min((1.0550 + 0.8800 * fresnel) / max(length(worldPos), 1.0), 0.1978);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.9561) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0881, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9119, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.4336 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.2434);
    float energyW = clamp(0.2484 + 0.4678 * clamp(pattern, 0.0, 1.0) + 0.3042 * fresnel, 0.0, 0.8544);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2444 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (1.1200 * (1.0 - 0.6057 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4483) * 1.2658;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2613 * max(0.0, 1.0 - 0.4516 * baseLuma - 0.5807 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3262 + 0.5376 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0878 * (0.5 + 0.5 * sin(time * 0.267035))) * atlasPoleW;
    rgb += emit * 0.6760 * (1.0 - 0.7199 * baseLuma) * (1.0 - 0.5434 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.6983));
    ...
    float bodyAlpha = 0.4274 + 0.5026 * presence + 0.4058 * pattern + 0.1202 * (1.0 - deepTrans) + 0.1631 * emit;
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.0697 + hgtC * 0.3024, 0.0, 1.0);
    ...
    innerScaf = clamp(innerScaf + 0.3735 * pow(clamp(motifM, 0.0, 1.0), 0.25), 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.2969);
    ...
    bodyAlpha *= smoothstep(0.0, 0.8615, sceneVDist - fragVDist);
