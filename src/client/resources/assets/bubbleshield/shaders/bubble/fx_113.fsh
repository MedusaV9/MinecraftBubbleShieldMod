...
// stack: deep=fbm x3 taps | mid=rings+curl+scroll | rim=graze | flourish=shimmer | tex=ridged
// fbm: ridged x4 octaves | seed ea11a761828ade32
// v8 realism: refr 0.7091+1.0556*fres/dist (cap 0.1423) | energy 0.4029+0.4023*p | flow 0.099167 c/s | soft 0.7778m
...
void main() {
    ...
    float refrAmp = min((0.7091 + 1.0556 * fresnel) / max(length(worldPos), 1.0), 0.1423);
    ...
    float emit = atlas.a * (0.3433 + 0.5797 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0990 * (0.5 + 0.5 * sin(time * 0.209440))) * atlasPoleW;
