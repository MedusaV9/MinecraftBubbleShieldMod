...
// stack: deep=fbm x2 taps | mid=rings+curl+scroll | rim=graze | flourish=shimmer
...
void main() {
    ...
    deep += 0.4523 * deepField(duvBase + rimDir * 0.0683 + vec2(-0.008333, 0.065000) * time, time);
    deep = pow(clamp(deep * 0.6886, 0.0, 1.0), 1.6323);
    ...
    float pattern = clamp(0.4194 * deep + 0.7768 * mid + 0.5733 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2044 + pattern * 0.5903);
    vec3 rgb = vertexColor.rgb * (0.3658 + 0.6005 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1752 + 0.7906 * pattern, 0.0, 1.0);
