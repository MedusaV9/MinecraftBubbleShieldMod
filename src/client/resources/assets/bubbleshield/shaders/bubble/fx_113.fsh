...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6183, 0.0, 1.0);
    ...
    deepCol += 0.4523 * dp * mix(baseCol, deepStop, 0.3016);
    deepPat += 0.4523 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 3.1443) + par * 0.1366, time);
    deepCol += 0.2046 * dp * mix(baseCol, deepStop, 0.6033);
    deepPat += 0.2046 * dp;
    deepCol *= 0.7447;
    deepPat = pow(clamp(deepPat * 0.6035, 0.0, 1.0), 1.6323);
    ...
    rgb += hotStop * 0.1691 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0624 + 0.3186 * presence + 0.4913 * pattern, 0.8490);
