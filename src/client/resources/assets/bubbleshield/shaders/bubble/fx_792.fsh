...
void main() {
    ...
    mid = clamp(mid * (0.6551 + 0.5933 * texDetail) + 0.2836 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5162), 0.0, 1.0);
    float emit = atlas.a * (0.7844 + 0.3764 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1592 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.8195 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3324 + 0.3282 * presence + 0.5314 * pattern + 0.0887 * (1.0 - deepTrans) + 0.1275 * emit, 0.8629);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2487), 0.8629);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6575 + 0.3425 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0783), 0.0, 1.0);
