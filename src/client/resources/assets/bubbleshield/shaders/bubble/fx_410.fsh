...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5576, 0.0, 1.0);
    ...
    deepCol += 0.4681 * dp * mix(baseCol, deepStop, 0.2057);
    deepPat += 0.4681 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 5.3942) + par * 0.0857, time);
    deepCol += 0.2191 * dp * mix(baseCol, deepStop, 0.4114);
    deepPat += 0.2191 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 6.6974) + par * 0.1286, time);
    deepCol += 0.1026 * dp * mix(baseCol, deepStop, 0.6171);
    deepPat += 0.1026 * dp;
    deepCol *= 0.7201;
    deepPat = pow(clamp(deepPat * 0.5587, 0.0, 1.0), 1.1440);
    ...
    float blob = invsmooth(0.05, 0.4738, bio.y);
    ...
    float haze = fbm2(wuv + vec2(3.1, 6.7), midPer) * 0.1401;
    float mid = clamp(blob * (0.2946 + 0.6701 * waveGlow) + haze, 0.0, 1.2);
    ...
    float pattern = clamp(0.4983 * deepPat + 0.9319 * mid + 0.7007 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb = satLift(rgb, 1.1851);
    rgb += hotStop * 0.2625 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0787 + 0.2946 * presence + 0.4627 * pattern, 0.7944);
