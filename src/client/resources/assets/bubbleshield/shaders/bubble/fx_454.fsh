...
void main() {
    ...
    mid = clamp(mid * (0.6264 + 0.5381 * texDetail) + 0.2584 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3951), 0.0, 1.0);
    float emit = atlas.a * (0.7101 + 0.2794 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1608 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.8497 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3612 + 0.5028 * presence + 0.4234 * pattern + 0.1401 * (1.0 - deepTrans) + 0.1699 * emit, 0.9472);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3202), 0.9472);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6679 + 0.3321 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
