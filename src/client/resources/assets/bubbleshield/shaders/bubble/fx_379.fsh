...
void main() {
    ...
    mid = clamp(mid * (0.6104 + 0.6648 * texDetail) + 0.3313 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4743), 0.0, 1.0);
    float emit = atlas.a * (0.7264 + 0.4699 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0995 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.6743 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3754 + 0.5172 * presence + 0.4066 * pattern + 0.0936 * (1.0 - deepTrans) + 0.2045 * emit, 0.9462);
