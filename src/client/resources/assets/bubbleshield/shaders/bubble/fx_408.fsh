...
// stack: deep=caustic x3 taps | mid=galaxyswirl+none+scroll | rim=lat | flourish=shimmer | tex=starfield
// fbm: ridged x6 octaves | seed 6b35c584d37fdd9c
// v8 realism: refr 0.3025+0.2882*fres/dist (cap 0.1592) | energy 0.4443+0.4760*p | flow 0.060833 c/s | soft 0.9874m
...
void main() {
    ...
    texDetail = clamp(texDetail * 1.9412 + 0.2101, 0.0, 1.0);
    ...
    float refrAmp = min((0.3025 + 0.2882 * fresnel) / max(length(worldPos), 1.0), 0.1592);
    ...
    float emitMask = clamp(atlas.a * 1.8133, 0.0, 1.0);
    float emit = emitMask * (0.3566 + 0.6753 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1683 * (0.5 + 0.5 * sin(time * 0.183260))) * atlasPoleW;
