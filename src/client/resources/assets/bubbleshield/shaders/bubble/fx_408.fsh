...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6407, 0.0, 1.0);
    ...
    deepCol += 0.4220 * dp * mix(baseCol, deepStop, 0.4410);
    deepPat += 0.4220 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 4.7793) + par * 0.0705, time);
    deepCol += 0.1781 * dp * mix(baseCol, deepStop, 0.8819);
    deepPat += 0.1781 * dp;
    deepCol *= 0.7043;
    deepPat = pow(clamp(deepPat * 0.6250, 0.0, 1.0), 1.1467);
    ...
    float arm = 0.5 + 0.5 * cos(gang * 2.0000 + log(max(grad, 0.05)) * 3.3353 - time * 0.392699);
    float arms = pow(clamp(arm, 0.0, 1.0), 4.5712) * smoothstep(0.08, 0.45, grad) * invsmooth(0.55, 1.15, grad);
    float core = invsmooth(0.02, 0.3308, grad);
    float dust = voronoise(wuv, midPer, 0.7171, 0.4407);
    float mid = clamp(arms * 0.9408 + core + dust * 0.2486 * arms, 0.0, 1.3);
    ...
    float pattern = clamp(0.3675 * deepPat + 0.9943 * mid + 0.8397 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.3490 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0699 + 0.3121 * presence + 0.4214 * pattern, 0.7942);
