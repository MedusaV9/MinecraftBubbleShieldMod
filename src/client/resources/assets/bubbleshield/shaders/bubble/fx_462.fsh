...
void main() {
    ...
    mid = clamp(mid * (0.7481 + 0.6453 * texDetail) + 0.3153 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4522), 0.0, 1.0);
    float emit = atlas.a * (0.6850 + 0.3947 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1083 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.8633 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3712 + 0.4593 * presence + 0.4688 * pattern + 0.1539 * (1.0 - deepTrans) + 0.2050 * emit, 0.8763);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2713), 0.8763);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7422 + 0.2578 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0616), 0.0, 1.0);
