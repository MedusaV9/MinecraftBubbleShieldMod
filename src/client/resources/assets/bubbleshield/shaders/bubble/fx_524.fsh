...
void main() {
    ...
    mid = clamp(mid * (0.6514 + 0.7376 * texDetail) + 0.2435 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7263 + 0.3033 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1295 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.8933 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3128 + 0.3126 * presence + 0.5442 * pattern + 0.1117 * (1.0 - deepTrans) + 0.1212 * emit, 0.8869);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1734), 0.8869);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7046 + 0.2954 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0663), 0.0, 1.0);
