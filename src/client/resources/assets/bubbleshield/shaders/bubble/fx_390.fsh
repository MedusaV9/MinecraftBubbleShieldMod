...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7400, 0.0, 1.0);
    ...
    deepCol += 0.5557 * dp * mix(baseCol, deepStop, 0.2011);
    deepPat += 0.5557 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 4.5152) + par * 0.1552, time);
    deepCol += 0.3088 * dp * mix(baseCol, deepStop, 0.4022);
    deepPat += 0.3088 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.7186) + par * 0.2328, time);
    deepCol += 0.1716 * dp * mix(baseCol, deepStop, 0.6033);
    deepPat += 0.1716 * dp;
    deepCol *= 0.5187;
    deepPat = pow(clamp(deepPat * 0.4911, 0.0, 1.0), 1.3546);
    ...
    float nest = pow(clamp(kacc * 0.1856 - 0.1319, 0.0, 1.0), 1.5289);
    float mid = nest * (0.72 + 0.28 * sin(time * 0.900590 + kacc * 2.9321));
    ...
    float pattern = clamp(0.4741 * deepPat + 0.8294 * mid + 0.5421 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb = satLift(rgb, 1.2853);
    rgb += hotStop * 0.3254 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0664 + 0.3087 * presence + 0.4842 * pattern, 0.8548);
