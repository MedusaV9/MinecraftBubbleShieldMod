...
void main() {
    ...
    mid = clamp(mid * (0.6758 + 0.7175 * texDetail) + 0.3042 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4210), 0.0, 1.0);
    float emit = atlas.a * (0.5897 + 0.3739 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1688 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.7605 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3771 + 0.3066 * presence + 0.4871 * pattern + 0.1226 * (1.0 - deepTrans) + 0.1870 * emit, 0.8902);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1728), 0.8902);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6909 + 0.3091 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0817), 0.0, 1.0);
