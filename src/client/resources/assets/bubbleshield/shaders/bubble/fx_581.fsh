...
void main() {
    ...
    mid = clamp(mid * (0.6831 + 0.7203 * texDetail) + 0.2367 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5357), 0.0, 1.0);
    float emit = atlas.a * (0.5758 + 0.4341 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1536 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9902 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4133 + 0.4112 * presence + 0.3539 * pattern + 0.0922 * (1.0 - deepTrans) + 0.2028 * emit, 0.9092);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1636), 0.9092);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0723), 0.0, 1.0);
