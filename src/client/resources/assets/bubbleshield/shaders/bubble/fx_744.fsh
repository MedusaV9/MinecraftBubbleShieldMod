...
void main() {
    ...
    mid = clamp(mid * (0.7138 + 0.6968 * texDetail) + 0.3375 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4097), 0.0, 1.0);
    float emit = atlas.a * (0.7162 + 0.4993 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1146 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.9421 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3111 + 0.2747 * presence + 0.5365 * pattern + 0.1416 * (1.0 - deepTrans) + 0.1417 * emit, 0.8438);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2444), 0.8438);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6656 + 0.3344 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0884), 0.0, 1.0);
