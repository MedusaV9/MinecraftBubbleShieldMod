...
void main() {
    ...
    mid = clamp(mid * (0.6373 + 0.6551 * texDetail) + 0.2474 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4547), 0.0, 1.0);
    float emit = atlas.a * (0.5654 + 0.3014 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1090 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.6728 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3183 + 0.3056 * presence + 0.5884 * pattern + 0.1314 * (1.0 - deepTrans) + 0.2192 * emit, 0.9111);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2700), 0.9111);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7181 + 0.2819 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0894), 0.0, 1.0);
