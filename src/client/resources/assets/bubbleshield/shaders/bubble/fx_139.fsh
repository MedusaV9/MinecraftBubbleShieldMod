...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7996, 0.0, 1.0);
    ...
    deepCol += 0.4727 * dp * mix(baseCol, deepStop, 0.2881);
    deepPat += 0.4727 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.3337) + par * 0.1471, time);
    deepCol += 0.2234 * dp * mix(baseCol, deepStop, 0.5762);
    deepPat += 0.2234 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 5.2304) + par * 0.2206, time);
    deepCol += 0.1056 * dp * mix(baseCol, deepStop, 0.8643);
    deepPat += 0.1056 * dp;
    deepCol *= 0.5815;
    deepPat = pow(clamp(deepPat * 0.5550, 0.0, 1.0), 1.5678);
    ...
    rgb += hotStop * 0.3422 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0723 + 0.3664 * presence + 0.3728 * pattern, 0.8282);
