...
// stack: deep=caustic x3 taps | mid=interference+warp1+flicker | rim=lat | flourish=glint
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.011667, -0.032500) * time, time);
    deep += 0.4727 * deepField(duvBase + rimDir * 0.0735 + vec2(-0.018333, -0.050000) * time, time);
    deep += 0.2234 * deepField(duvBase + rimDir * 0.1471 + vec2(-0.025000, -0.067500) * time, time);
    deep = pow(clamp(deep * 0.5896, 0.0, 1.0), 1.5678);
    ...
    float pattern = clamp(0.3910 * deep + 0.9496 * mid + 0.7122 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8740 + pattern * 0.3999);
    vec3 rgb = vertexColor.rgb * (0.5286 + 0.7223 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1338 + 0.7232 * pattern, 0.0, 1.0);
