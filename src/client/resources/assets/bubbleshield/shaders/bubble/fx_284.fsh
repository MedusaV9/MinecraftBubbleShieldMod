...
void main() {
    ...
    mid = clamp(mid * (0.7496 + 0.5806 * texDetail) + 0.3478 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3532), 0.0, 1.0);
    float emit = atlas.a * (0.7823 + 0.3091 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0910 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9680 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3909 + 0.5436 * presence + 0.4642 * pattern + 0.1600 * (1.0 - deepTrans) + 0.1937 * emit, 0.9442);
