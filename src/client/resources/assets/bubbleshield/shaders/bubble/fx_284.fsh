...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8277, 0.0, 1.0);
    ...
    deepCol += 0.3959 * dp * mix(baseCol, deepStop, 0.2735);
    deepPat += 0.3959 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.3847) + par * 0.1122, time);
    deepCol += 0.1568 * dp * mix(baseCol, deepStop, 0.5469);
    deepPat += 0.1568 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.3133) + par * 0.1683, time);
    deepCol += 0.0621 * dp * mix(baseCol, deepStop, 0.8204);
    deepPat += 0.0621 * dp;
    deepCol *= 0.7451;
    deepPat = pow(clamp(deepPat * 0.6193, 0.0, 1.0), 1.6436);
    ...
    float midCover = clamp(0.8893 * mid + 0.4714 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.3279);
    rgb += hotStop * 0.2029 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0503 + 0.3486 * presence + 0.4797 * pattern, 0.8442);
