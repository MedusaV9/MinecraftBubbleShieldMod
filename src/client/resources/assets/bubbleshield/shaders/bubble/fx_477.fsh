...
void main() {
    ...
    mid = clamp(mid * (0.7268 + 0.5851 * texDetail) + 0.2162 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3825), 0.0, 1.0);
    float emit = atlas.a * (0.5752 + 0.4725 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1290 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.6731 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2997 + 0.2136 * presence + 0.5644 * pattern + 0.1277 * (1.0 - deepTrans) + 0.1902 * emit, 0.9388);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2642), 0.9388);
    // [layer:v5:ghostalpha]
