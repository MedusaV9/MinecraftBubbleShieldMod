...
void main() {
    ...
    mid = clamp(mid * (0.6468 + 0.5400 * texDetail) + 0.2121 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4442), 0.0, 1.0);
    float emit = atlas.a * (0.6154 + 0.2537 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1483 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8365 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4430 + 0.4680 * presence + 0.3551 * pattern + 0.1343 * (1.0 - deepTrans) + 0.1303 * emit, 0.9180);
