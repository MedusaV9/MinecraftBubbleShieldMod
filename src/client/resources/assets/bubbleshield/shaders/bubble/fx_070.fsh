...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6987, 0.0, 1.0);
    ...
    deepCol += 0.4898 * dp * mix(baseCol, deepStop, 0.2589);
    deepPat += 0.4898 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 4.8823) + par * 0.1295, time);
    deepCol += 0.2399 * dp * mix(baseCol, deepStop, 0.5178);
    deepPat += 0.2399 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 5.9650) + par * 0.1943, time);
    deepCol += 0.1175 * dp * mix(baseCol, deepStop, 0.7767);
    deepPat += 0.1175 * dp;
    deepCol *= 0.6086;
    deepPat = pow(clamp(deepPat * 0.5414, 0.0, 1.0), 1.3003);
    ...
    rgb += hotStop * 0.2356 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0731 + 0.2783 * presence + 0.3403 * pattern, 0.8190);
