...
void main() {
    ...
    mid = clamp(mid * (0.7059 + 0.6730 * texDetail) + 0.3461 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4244), 0.0, 1.0);
    float emit = atlas.a * (0.7892 + 0.2509 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0994 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.6517 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4260 + 0.4183 * presence + 0.3403 * pattern + 0.0882 * (1.0 - deepTrans) + 0.1432 * emit, 0.8790);
