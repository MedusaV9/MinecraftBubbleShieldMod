...
void main() {
    ...
    mid = clamp(mid * (0.6696 + 0.5134 * texDetail) + 0.2918 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4827), 0.0, 1.0);
    float emit = atlas.a * (0.6052 + 0.2867 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1268 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8944 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3511 + 0.4056 * presence + 0.4817 * pattern + 0.1517 * (1.0 - deepTrans) + 0.1995 * emit, 0.8765);
