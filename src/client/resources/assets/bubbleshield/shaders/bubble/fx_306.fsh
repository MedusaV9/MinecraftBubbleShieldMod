...
// stack: deep=ridge x3 taps | mid=triweave+warp1+flicker | rim=lat | flourish=shimmer | tex=hex_lattice
// fbm: ridged x6 octaves | seed a59509452bc024bc
// v8 realism: refr 0.7047+0.8956*fres/dist (cap 0.1291) | energy 0.3326+0.5141*p | flow 0.060000 c/s | soft 0.8220m
...
void main() {
    ...
    float refrAmp = min((0.7047 + 0.8956 * fresnel) / max(length(worldPos), 1.0), 0.1291);
