...
void main() {
    ...
    mid = clamp(mid * (0.6431 + 0.6859 * texDetail) + 0.2009 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5114), 0.0, 1.0);
    float emit = atlas.a * (0.7730 + 0.2910 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1299 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.9598 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3442 + 0.4335 * presence + 0.4740 * pattern + 0.0922 * (1.0 - deepTrans) + 0.2066 * emit, 0.8910);
