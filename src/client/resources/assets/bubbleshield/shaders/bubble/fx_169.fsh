...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6470, 0.0, 1.0);
    ...
    deepCol += 0.4135 * dp * mix(baseCol, deepStop, 0.2426);
    deepPat += 0.4135 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 4.2790) + par * 0.1403, time);
    deepCol += 0.1710 * dp * mix(baseCol, deepStop, 0.4851);
    deepPat += 0.1710 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 5.1480) + par * 0.2104, time);
    deepCol += 0.0707 * dp * mix(baseCol, deepStop, 0.7277);
    deepPat += 0.0707 * dp;
    deepCol *= 0.6065;
    deepPat = pow(clamp(deepPat * 0.6042, 0.0, 1.0), 1.6890);
    ...
    rgb += hotStop * 0.3024 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0455 + 0.2935 * presence + 0.4740 * pattern, 0.8310);
