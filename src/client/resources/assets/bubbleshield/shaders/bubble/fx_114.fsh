...
void main() {
    ...
    mid = clamp(mid * (0.6228 + 0.5871 * texDetail) + 0.3453 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4021), 0.0, 1.0);
    float emit = atlas.a * (0.6696 + 0.4350 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1798 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.6855 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3532 + 0.4343 * presence + 0.4819 * pattern + 0.1307 * (1.0 - deepTrans) + 0.1833 * emit, 0.8872);
