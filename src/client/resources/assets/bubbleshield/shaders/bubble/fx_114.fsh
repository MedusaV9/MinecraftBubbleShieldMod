...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7440, 0.0, 1.0);
    ...
    deepCol += 0.5208 * dp * mix(baseCol, deepStop, 0.2157);
    deepPat += 0.5208 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 5.1692) + par * 0.0619, time);
    deepCol += 0.2712 * dp * mix(baseCol, deepStop, 0.4313);
    deepPat += 0.2712 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 6.4835) + par * 0.0929, time);
    deepCol += 0.1413 * dp * mix(baseCol, deepStop, 0.6470);
    deepPat += 0.1413 * dp;
    deepCol *= 0.5909;
    deepPat = pow(clamp(deepPat * 0.5173, 0.0, 1.0), 1.6408);
    ...
    rgb += hotStop * 0.3204 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0485 + 0.2943 * presence + 0.4819 * pattern, 0.8272);
