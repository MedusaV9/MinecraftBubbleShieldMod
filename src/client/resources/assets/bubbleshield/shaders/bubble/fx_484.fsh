...
void main() {
    ...
    mid = clamp(mid * (0.6585 + 0.6911 * texDetail) + 0.2913 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3879), 0.0, 1.0);
    float emit = atlas.a * (0.6188 + 0.3627 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1147 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.9403 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3177 + 0.2982 * presence + 0.5259 * pattern + 0.1574 * (1.0 - deepTrans) + 0.2152 * emit, 0.8634);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1872), 0.8634);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1335), 0.0, 1.0);
