...
void main() {
    ...
    mid = clamp(mid * (0.7009 + 0.5473 * texDetail) + 0.2560 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4046), 0.0, 1.0);
    float emit = atlas.a * (0.5789 + 0.3701 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1552 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.7763 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3217 + 0.3322 * presence + 0.4565 * pattern + 0.1226 * (1.0 - deepTrans) + 0.1913 * emit, 0.8420);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2640), 0.8420);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6804 + 0.3196 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0935), 0.0, 1.0);
