...
void main() {
    ...
    mid = clamp(mid * (0.7308 + 0.4524 * texDetail) + 0.2408 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3813), 0.0, 1.0);
    float emit = atlas.a * (0.5710 + 0.4903 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1263 * (0.5 + 0.5 * sin(time * 0.146608)));
    ...
    float alpha = vertexColor.a * min(0.4131 + 0.4633 * presence + 0.4227 * pattern + 0.1434 * (1.0 - deepTrans) + 0.2151 * emit, 0.8474);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2434), 0.8474);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7667 + 0.2333 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
