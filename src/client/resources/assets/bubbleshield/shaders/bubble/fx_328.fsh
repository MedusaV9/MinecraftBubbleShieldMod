...
// stack: deep=voro x4 taps | mid=sparkle+curl+pulse | rim=graze | flourish=echo | tex=starfield
// fbm: standard x6 octaves | seed cf3e0fd49ddf6f6c
// v8 realism: refr 0.7381+0.7228*fres/dist (cap 0.1543) | energy 0.3217+0.4450*p | flow 0.066667 c/s | soft 0.9401m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.0303 + 0.1832, 0.0, 1.0);
    ...
    float refrAmp = min((0.7381 + 0.7228 * fresnel) / max(length(worldPos), 1.0), 0.1543);
    ...
    float emitMask = clamp(atlas.a * 1.7723, 0.0, 1.0);
    float emit = emitMask * (0.3685 + 0.5745 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1259 * (0.5 + 0.5 * sin(time * 0.130900))) * atlasPoleW;
