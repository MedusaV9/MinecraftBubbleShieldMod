...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6148, 0.0, 1.0);
    ...
    deepCol += 0.5169 * dp * mix(baseCol, deepStop, 0.2603);
    deepPat += 0.5169 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.7289) + par * 0.0913, time);
    deepCol += 0.2671 * dp * mix(baseCol, deepStop, 0.5207);
    deepPat += 0.2671 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 7.2523) + par * 0.1370, time);
    deepCol += 0.1381 * dp * mix(baseCol, deepStop, 0.7810);
    deepPat += 0.1381 * dp;
    deepCol *= 0.5423;
    deepPat = pow(clamp(deepPat * 0.5203, 0.0, 1.0), 1.5930);
    ...
    rgb += hotStop * 0.1954 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0471 + 0.3203 * presence + 0.4265 * pattern, 0.8052);
