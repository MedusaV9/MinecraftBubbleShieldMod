...
void main() {
    ...
    mid = clamp(mid * (0.6630 + 0.4750 * texDetail) + 0.3144 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4007), 0.0, 1.0);
    float emit = atlas.a * (0.7388 + 0.3489 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1494 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9210 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4178 + 0.4060 * presence + 0.4869 * pattern + 0.1563 * (1.0 - deepTrans) + 0.1740 * emit, 0.8789);
