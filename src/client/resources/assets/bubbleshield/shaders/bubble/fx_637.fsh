...
void main() {
    ...
    mid = clamp(mid * (0.7207 + 0.6610 * texDetail) + 0.2236 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4888), 0.0, 1.0);
    float emit = atlas.a * (0.6936 + 0.2673 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0949 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.8223 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3299 + 0.2358 * presence + 0.6136 * pattern + 0.0887 * (1.0 - deepTrans) + 0.1650 * emit, 0.9317);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2793), 0.9317);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6661 + 0.3339 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1070), 0.0, 1.0);
