...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6586, 0.0, 1.0);
    ...
    deepCol += 0.4695 * dp * mix(baseCol, deepStop, 0.2960);
    deepPat += 0.4695 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.9032) + par * 0.0632, time);
    deepCol += 0.2204 * dp * mix(baseCol, deepStop, 0.5921);
    deepPat += 0.2204 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 5.9998) + par * 0.0948, time);
    deepCol += 0.1035 * dp * mix(baseCol, deepStop, 0.8881);
    deepPat += 0.1035 * dp;
    deepCol *= 0.6352;
    deepPat = pow(clamp(deepPat * 0.5576, 0.0, 1.0), 1.1242);
    ...
    rgb += hotStop * 0.2956 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0710 + 0.3149 * presence + 0.4990 * pattern, 0.7947);
