...
void main() {
    ...
    mid = clamp(mid * (0.7226 + 0.5773 * texDetail) + 0.2334 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7888 + 0.3942 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0823 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9425 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2722 + 0.2427 * presence + 0.6008 * pattern + 0.1101 * (1.0 - deepTrans) + 0.1518 * emit, 0.8711);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3315), 0.8711);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6921 + 0.3079 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1340), 0.0, 1.0);
