...
void main() {
    ...
    mid = clamp(mid * (0.6139 + 0.4511 * texDetail) + 0.2265 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5255), 0.0, 1.0);
    float emit = atlas.a * (0.5568 + 0.2539 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1091 * (0.5 + 0.5 * sin(time * 0.287979)));
    ...
    float alpha = vertexColor.a * min(0.3557 + 0.4688 * presence + 0.3654 * pattern + 0.1108 * (1.0 - deepTrans) + 0.1899 * emit, 0.9570);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1903), 0.9570);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7250 + 0.2750 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1294), 0.0, 1.0);
