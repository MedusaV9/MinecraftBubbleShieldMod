...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6885, 0.0, 1.0);
    ...
    deepCol += 0.3916 * dp * mix(baseCol, deepStop, 0.2161);
    deepPat += 0.3916 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.0485) + par * 0.1406, time);
    deepCol += 0.1533 * dp * mix(baseCol, deepStop, 0.4321);
    deepPat += 0.1533 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 3.6223) + par * 0.2109, time);
    deepCol += 0.0600 * dp * mix(baseCol, deepStop, 0.6482);
    deepPat += 0.0600 * dp;
    deepCol *= 0.7349;
    deepPat = pow(clamp(deepPat * 0.6231, 0.0, 1.0), 1.7859);
    ...
    rgb += hotStop * 0.3362 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0734 + 0.3179 * presence + 0.4982 * pattern, 0.8545);
