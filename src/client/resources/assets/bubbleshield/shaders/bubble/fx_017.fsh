...
void main() {
    ...
    mid = clamp(mid * (0.6431 + 0.5950 * texDetail) + 0.2693 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5289), 0.0, 1.0);
    float emit = atlas.a * (0.6299 + 0.3858 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0863 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.6551 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4270 + 0.4579 * presence + 0.4982 * pattern + 0.0822 * (1.0 - deepTrans) + 0.1587 * emit, 0.9145);
