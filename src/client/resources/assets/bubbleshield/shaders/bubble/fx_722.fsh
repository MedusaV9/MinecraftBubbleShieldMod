...
void main() {
    ...
    mid = clamp(mid * (0.6245 + 0.6386 * texDetail) + 0.3483 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4467), 0.0, 1.0);
    float emit = atlas.a * (0.7906 + 0.3158 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1380 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.7162 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3428 + 0.4345 * presence + 0.4197 * pattern + 0.1167 * (1.0 - deepTrans) + 0.1313 * emit, 0.8578);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2510), 0.8578);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6438 + 0.3562 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0969), 0.0, 1.0);
