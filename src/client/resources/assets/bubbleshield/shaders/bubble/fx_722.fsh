...
// stack: deep=fbm x3 taps | mid=prismdisperse+curl+scroll | rim=graze_film | flourish=glint
...
void main() {
    ...
    float flourish = 0.2791 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0456 + 0.2945 * presence + 0.4197 * pattern + 0.1167 * (1.0 - deepTrans), 0.7978);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2510), 0.7978);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4053 + 0.5947 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
