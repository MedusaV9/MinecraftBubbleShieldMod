...
void main() {
    ...
    mid = clamp(mid * (0.6168 + 0.5122 * texDetail) + 0.3186 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4734), 0.0, 1.0);
    float emit = atlas.a * (0.6523 + 0.4637 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1408 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9080 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3505 + 0.3262 * presence + 0.5037 * pattern + 0.1301 * (1.0 - deepTrans) + 0.1563 * emit, 0.8602);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2925), 0.8602);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6299 + 0.3701 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
