...
// stack: deep=voro x3 taps | mid=rings+none+scroll | rim=graze | flourish=glint | tex=ridged
// fbm: turb x5 octaves | seed edaa370f9f7c926e
// v8 realism: refr 0.5258+0.7110*fres/dist (cap 0.1138) | energy 0.3149+0.4553*p | flow 0.086667 c/s | soft 0.9862m
...
void main() {
    ...
    float refrAmp = min((0.5258 + 0.7110 * fresnel) / max(length(worldPos), 1.0), 0.1138);
    ...
    float emit = atlas.a * (0.3318 + 0.6053 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1694 * (0.5 + 0.5 * sin(time * 0.225147))) * atlasPoleW;
