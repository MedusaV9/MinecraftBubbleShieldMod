...
void main() {
    ...
    mid = clamp(mid * (0.6526 + 0.4546 * texDetail) + 0.3397 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4675), 0.0, 1.0);
    float emit = atlas.a * (0.6031 + 0.3794 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1694 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.7402 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3794 + 0.5031 * presence + 0.4017 * pattern + 0.1182 * (1.0 - deepTrans) + 0.1678 * emit, 0.8940);
