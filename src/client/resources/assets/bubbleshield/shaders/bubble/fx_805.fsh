...
void main() {
    ...
    mid = clamp(mid * (0.7232 + 0.7255 * texDetail) + 0.2463 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5283), 0.0, 1.0);
    float emit = atlas.a * (0.6390 + 0.3498 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1695 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9861 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3830 + 0.3728 * presence + 0.4002 * pattern + 0.1559 * (1.0 - deepTrans) + 0.1782 * emit, 0.8426);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3220), 0.8426);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6743 + 0.3257 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1360), 0.0, 1.0);
