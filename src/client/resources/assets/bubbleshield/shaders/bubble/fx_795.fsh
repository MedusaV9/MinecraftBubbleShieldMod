...
void main() {
    ...
    mid = clamp(mid * (0.6181 + 0.6432 * texDetail) + 0.3037 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5275), 0.0, 1.0);
    float emit = atlas.a * (0.6980 + 0.2918 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1438 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.7412 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3330 + 0.2690 * presence + 0.5847 * pattern + 0.1515 * (1.0 - deepTrans) + 0.1948 * emit, 0.8702);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2872), 0.8702);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6353 + 0.3647 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1011), 0.0, 1.0);
