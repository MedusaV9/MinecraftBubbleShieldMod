...
void main() {
    ...
    mid = clamp(mid * (0.6137 + 0.4909 * texDetail) + 0.3427 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5346), 0.0, 1.0);
    float emit = atlas.a * (0.6714 + 0.4899 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1771 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.6586 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3486 + 0.4751 * presence + 0.4584 * pattern + 0.1386 * (1.0 - deepTrans) + 0.2167 * emit, 0.8982);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1533), 0.8982);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7603 + 0.2397 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
