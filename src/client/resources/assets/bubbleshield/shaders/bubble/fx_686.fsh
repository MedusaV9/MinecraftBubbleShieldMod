...
void main() {
    ...
    mid = clamp(mid * (0.6787 + 0.5358 * texDetail) + 0.3359 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4731), 0.0, 1.0);
    float emit = atlas.a * (0.7419 + 0.3113 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1573 * (0.5 + 0.5 * sin(time * 0.183260)));
    ...
    float alpha = vertexColor.a * min(0.4141 + 0.4144 * presence + 0.4462 * pattern + 0.1114 * (1.0 - deepTrans) + 0.1671 * emit, 0.8758);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3482), 0.8758);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7333 + 0.2667 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1026), 0.0, 1.0);
