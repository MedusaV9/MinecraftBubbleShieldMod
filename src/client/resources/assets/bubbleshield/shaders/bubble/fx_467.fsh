...
void main() {
    ...
    mid = clamp(mid * (0.6707 + 0.6731 * texDetail) + 0.2919 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3964), 0.0, 1.0);
    float emit = atlas.a * (0.6634 + 0.2666 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1021 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.7323 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3690 + 0.3542 * presence + 0.4598 * pattern + 0.0919 * (1.0 - deepTrans) + 0.1512 * emit, 0.8997);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3246), 0.8997);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6954 + 0.3046 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
