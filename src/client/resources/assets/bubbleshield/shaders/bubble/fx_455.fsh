...
void main() {
    ...
    mid = clamp(mid * (0.6146 + 0.5682 * texDetail) + 0.3015 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7946 + 0.3633 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1331 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9393 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3289 + 0.2710 * presence + 0.4842 * pattern + 0.1097 * (1.0 - deepTrans) + 0.1347 * emit, 0.9060);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1977), 0.9060);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0750), 0.0, 1.0);
