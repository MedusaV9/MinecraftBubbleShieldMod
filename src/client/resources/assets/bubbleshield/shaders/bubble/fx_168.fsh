...
void main() {
    ...
    mid = clamp(mid * (0.6450 + 0.7353 * texDetail) + 0.1820 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5272), 0.0, 1.0);
    float emit = atlas.a * (0.7170 + 0.3577 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1115 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8441 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4654 + 0.5132 * presence + 0.3447 * pattern + 0.1029 * (1.0 - deepTrans) + 0.1686 * emit, 0.9450);
