...
void main() {
    ...
    mid = clamp(mid * (0.6229 + 0.6463 * texDetail) + 0.2511 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7852 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4596), 0.0, 1.0);
    float emit = atlas.a * (0.7188 + 0.2610 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0963 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8810 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3691 + 0.4681 * presence + 0.3724 * pattern + 0.0813 * (1.0 - deepTrans) + 0.1983 * emit, 0.8507);
