...
void main() {
    ...
    mid = clamp(mid * (0.7052 + 0.7170 * texDetail) + 0.2754 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3675), 0.0, 1.0);
    float emit = atlas.a * (0.7717 + 0.4833 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1325 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.7404 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2731 + 0.3173 * presence + 0.5885 * pattern + 0.0933 * (1.0 - deepTrans) + 0.1697 * emit, 0.9186);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3060), 0.9186);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6393 + 0.3607 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1177), 0.0, 1.0);
