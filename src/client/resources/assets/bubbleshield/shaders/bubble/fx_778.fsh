...
void main() {
    ...
    mid = clamp(mid * (0.6844 + 0.5561 * texDetail) + 0.2048 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.5157 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5257), 0.0, 1.0);
    float emit = atlas.a * (0.7393 + 0.3711 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0892 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.7555 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3771 + 0.3507 * presence + 0.5190 * pattern + 0.1371 * (1.0 - deepTrans) + 0.1793 * emit, 0.9077);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1816), 0.9077);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6892 + 0.3108 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1185), 0.0, 1.0);
