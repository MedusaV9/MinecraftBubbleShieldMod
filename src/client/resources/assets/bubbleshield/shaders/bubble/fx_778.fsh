...
// stack: deep=ridge x3 taps | mid=ectoplasm+curl+scroll | rim=graze_sparkle | flourish=shimmer
...
void main() {
    ...
    rgb = mix(rgb, hotStop, clamp(ecEdge, 0.0, 1.0) * 0.2513);
    ...
    float alpha = vertexColor.a * min(0.0792 + 0.2683 * presence + 0.5190 * pattern + 0.1371 * (1.0 - deepTrans), 0.8477);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1816), 0.8477);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4535 + 0.5465 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
