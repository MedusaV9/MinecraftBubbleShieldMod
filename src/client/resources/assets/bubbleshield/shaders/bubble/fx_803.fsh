...
void main() {
    ...
    mid = clamp(mid * (0.7159 + 0.5067 * texDetail) + 0.2121 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4337), 0.0, 1.0);
    float emit = atlas.a * (0.7698 + 0.4292 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1434 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.9129 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3551 + 0.2908 * presence + 0.4983 * pattern + 0.1483 * (1.0 - deepTrans) + 0.1344 * emit, 0.9018);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1769), 0.9018);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7205 + 0.2795 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
