...
void main() {
    ...
    mid = clamp(mid * (0.6749 + 0.7351 * texDetail) + 0.3316 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6150 + 0.4427 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1111 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.9665 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2767 + 0.3113 * presence + 0.4978 * pattern + 0.0849 * (1.0 - deepTrans) + 0.1555 * emit, 0.8579);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2436), 0.8579);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7786 + 0.2214 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0749), 0.0, 1.0);
