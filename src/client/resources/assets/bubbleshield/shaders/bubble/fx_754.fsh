...
void main() {
    ...
    mid = clamp(mid * (0.6422 + 0.6585 * texDetail) + 0.3468 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5273), 0.0, 1.0);
    float emit = atlas.a * (0.7311 + 0.3381 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1727 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.9481 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3724 + 0.4639 * presence + 0.4429 * pattern + 0.1283 * (1.0 - deepTrans) + 0.1523 * emit, 0.9563);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2638), 0.9563);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7302 + 0.2698 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
