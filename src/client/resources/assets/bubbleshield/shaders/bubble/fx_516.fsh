...
void main() {
    ...
    mid = clamp(mid * (0.6718 + 0.6299 * texDetail) + 0.3248 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4636), 0.0, 1.0);
    float emit = atlas.a * (0.7434 + 0.4231 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1455 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.7876 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4007 + 0.4107 * presence + 0.3464 * pattern + 0.0876 * (1.0 - deepTrans) + 0.1409 * emit, 0.8993);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2369), 0.8993);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7420 + 0.2580 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0872), 0.0, 1.0);
