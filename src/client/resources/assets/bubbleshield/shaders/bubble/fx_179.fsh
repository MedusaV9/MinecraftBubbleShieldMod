...
void main() {
    ...
    mid = clamp(mid * (0.6789 + 0.6490 * texDetail) + 0.2276 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5120), 0.0, 1.0);
    float emit = atlas.a * (0.5578 + 0.2717 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1347 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.7070 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3390 + 0.4757 * presence + 0.4783 * pattern + 0.1544 * (1.0 - deepTrans) + 0.2059 * emit, 0.9040);
