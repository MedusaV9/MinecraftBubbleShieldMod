...
void main() {
    ...
    mid = clamp(mid * (0.6513 + 0.6881 * texDetail) + 0.2123 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5133), 0.0, 1.0);
    float emit = atlas.a * (0.7687 + 0.2571 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0888 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.8321 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3652 + 0.5025 * presence + 0.4030 * pattern + 0.1229 * (1.0 - deepTrans) + 0.2001 * emit, 0.9429);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1576), 0.9429);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1046), 0.0, 1.0);
