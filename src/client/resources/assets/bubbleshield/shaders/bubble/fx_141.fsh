...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.031667, -0.045000) * time, time);
    deep += 0.4194 * deepField(duvBase + rimDir * 0.0485 + vec2(-0.050000, -0.070000) * time, time);
    deep += 0.1759 * deepField(duvBase + rimDir * 0.0969 + vec2(-0.066667, -0.095000) * time, time);
    deep = pow(clamp(deep * 0.6268, 0.0, 1.0), 1.0068);
