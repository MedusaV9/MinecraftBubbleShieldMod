...
void main() {
    ...
    mid = clamp(mid * (0.7316 + 0.4718 * texDetail) + 0.3124 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5016), 0.0, 1.0);
    float emit = atlas.a * (0.6160 + 0.3301 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0971 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.6550 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3048 + 0.3614 * presence + 0.3870 * pattern + 0.1344 * (1.0 - deepTrans) + 0.2197 * emit, 0.8897);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2613), 0.8897);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7502 + 0.2498 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1140), 0.0, 1.0);
