...
void main() {
    ...
    mid = clamp(mid * (0.6543 + 0.5436 * texDetail) + 0.3474 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4726), 0.0, 1.0);
    float emit = atlas.a * (0.7006 + 0.4537 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1034 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.9818 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2719 + 0.2658 * presence + 0.5604 * pattern + 0.1161 * (1.0 - deepTrans) + 0.1395 * emit, 0.8515);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2863), 0.8515);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7023 + 0.2977 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1321), 0.0, 1.0);
