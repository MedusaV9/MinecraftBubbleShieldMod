...
void main() {
    ...
    mid = clamp(mid * (0.6132 + 0.6270 * texDetail) + 0.2999 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4932), 0.0, 1.0);
    float emit = atlas.a * (0.7897 + 0.3454 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1411 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.9938 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4093 + 0.4264 * presence + 0.3452 * pattern + 0.1212 * (1.0 - deepTrans) + 0.2015 * emit, 0.8904);
