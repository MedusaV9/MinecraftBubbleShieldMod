...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(1.0000, 2.0000), t * 0.675442);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.032500, 0.026667) * time, time);
    deep += 0.5201 * deepField(duvBase + rimDir * 0.0748 + vec2(-0.050833, 0.040000) * time, time);
    deep = pow(clamp(deep * 0.6578, 0.0, 1.0), 1.5044);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.473333), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 9.7587);
    float flash = 0.6 + 0.4 * sin(time * 4.183554 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
