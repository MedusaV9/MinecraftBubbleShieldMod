...
void main() {
    ...
    mid = clamp(mid * (0.7455 + 0.4568 * texDetail) + 0.3030 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3894), 0.0, 1.0);
    float emit = atlas.a * (0.5610 + 0.4076 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0889 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.8041 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4028 + 0.3538 * presence + 0.3918 * pattern + 0.1215 * (1.0 - deepTrans) + 0.1321 * emit, 0.9054);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3026), 0.9054);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6337 + 0.3663 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0908), 0.0, 1.0);
