...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7865, 0.0, 1.0);
    ...
    deepCol += 0.4460 * dp * mix(baseCol, deepStop, 0.2260);
    deepPat += 0.4460 * dp;
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * 2.6045) + par * 0.1013, time);
    deepCol += 0.1989 * dp * mix(baseCol, deepStop, 0.4520);
    deepPat += 0.1989 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 3.1044) + par * 0.1519, time);
    deepCol += 0.0887 * dp * mix(baseCol, deepStop, 0.6780);
    deepPat += 0.0887 * dp;
    deepCol *= 0.6750;
    deepPat = pow(clamp(deepPat * 0.5768, 0.0, 1.0), 1.3205);
    ...
    for (int i = 0; i < 4; i++) {
        float fi = float(i);
        float fa = fi * 1.5707963 + 0.7697 * sin(time * 0.214675 + fi * 1.9);
        ...
        float fil = pow(clamp(1.0 - dAng * 3.3526, 0.0, 1.0), 8.1811);
        net += fil * smoothstep(0.06, 0.25, trad) * (0.55 + 0.45 * sin(time * 1.685988 + fi * 2.3));
    }
    ...
    float pattern = clamp(0.3426 * deepPat + 0.7159 * mid + 0.8467 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.1791 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0459 + 0.2781 * presence + 0.4335 * pattern, 0.8153);
