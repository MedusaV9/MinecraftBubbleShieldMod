...
void main() {
    ...
    mid = clamp(mid * (0.7476 + 0.4896 * texDetail) + 0.3083 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5331), 0.0, 1.0);
    float emit = atlas.a * (0.7322 + 0.4817 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0921 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.8142 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2938 + 0.3141 * presence + 0.4817 * pattern + 0.0978 * (1.0 - deepTrans) + 0.1263 * emit, 0.8709);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1802), 0.8709);
    // [layer:v5:ghostalpha]
