...
// stack: deep=caustic x3 taps | mid=sparkle+warp2+scroll | rim=graze_film | flourish=shimmer | tex=starfield
// fbm: turb x4 octaves | seed 0b6a4b4f80e2110f
// v8 realism: refr 0.5056+0.5299*fres/dist (cap 0.1119) | energy 0.3035+0.4475*p | flow 0.075000 c/s | soft 0.5783m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.4577 + 0.1685, 0.0, 1.0);
    ...
    float refrAmp = min((0.5056 + 0.5299 * fresnel) / max(length(worldPos), 1.0), 0.1119);
    ...
    float emitMask = clamp(atlas.a * 1.8177, 0.0, 1.0);
    float emit = emitMask * (0.3016 + 0.4973 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1547 * (0.5 + 0.5 * sin(time * 0.146608))) * atlasPoleW;
