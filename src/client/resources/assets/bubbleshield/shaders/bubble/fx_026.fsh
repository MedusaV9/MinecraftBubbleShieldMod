...
// stack: deep=caustic x2 taps | mid=sparkle+warp2+scroll | rim=graze_film | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.015000, -0.027500) * time, time);
    deep += 0.5695 * deepField(duvBase + rimDir * 0.0531 + vec2(-0.023333, -0.045000) * time, time);
    deep = pow(clamp(deep * 0.6372, 0.0, 1.0), 1.2387);
    ...
    float pattern = clamp(0.3923 * deep + 0.8048 * mid + 0.7042 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6424 + pattern * 0.5759);
    vec3 rgb = vertexColor.rgb * (0.3987 + 0.9173 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1823 + 0.7401 * pattern, 0.0, 1.0);
