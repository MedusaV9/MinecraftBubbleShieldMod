...
void main() {
    ...
    mid = clamp(mid * (0.6916 + 0.5013 * texDetail) + 0.3167 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3653), 0.0, 1.0);
    float emit = atlas.a * (0.7485 + 0.4570 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1600 * (0.5 + 0.5 * sin(time * 0.298451)));
    ...
    float alpha = vertexColor.a * min(0.3585 + 0.3269 * presence + 0.4678 * pattern + 0.0952 * (1.0 - deepTrans) + 0.1978 * emit, 0.8860);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2420), 0.8860);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7239 + 0.2761 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
