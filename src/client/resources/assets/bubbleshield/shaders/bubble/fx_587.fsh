...
// v8 realism: refr 1.3046+1.8599*fres/dist (cap 0.1750) | energy 0.2257+0.4241*p | flow 0.117500 c/s | soft 1.0636m
...
void main() {
    ...
    rim = clamp(rim + 0.7507 * rimLine + 0.5532 * fresnel, 0.0, 1.4);
    ...
    float motifEnv = 0.55 + 0.45 * sin(time * 0.345575 + 1.9443);
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 29.8949) * (0.75 + 0.25 * sin(time * 0.555015 + 1.2900));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 29.8949);
    ...
    float refrAmp = min((1.3046 + 1.8599 * fresnel) / max(length(worldPos), 1.0), 0.1750);
    ...
    float energyW = clamp(0.2257 + 0.4241 * clamp(pattern, 0.0, 1.0) + 0.1327 * fresnel, 0.0, 0.8655);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.6049;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4490) * 0.9435;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2707 * (1.0 - 0.4919 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.4191 + 0.6984 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1600 * (0.5 + 0.5 * sin(time * 0.298451))) * atlasPoleW;
    rgb += emit * 0.7100 * (1.0 - 0.7690 * baseLuma) * emitCol;
    // [layer:v5:softknee]
