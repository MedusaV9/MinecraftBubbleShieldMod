...
// Bubble surface shader fx_587 -- family IRISFILM
// stack: deep=caustic x4 taps | mid=irisfilm+warp1+rotate | rim=graze_sparkle | flourish=glint | tex=iris_eye | motif=corex4+breath
// fbm: ridged x4 octaves | seed d49cf1def53037ce
// v8 realism: refr 1.4606+2.0823*fres/dist (cap 0.1750) | energy 0.1766+0.4241*p | flow 0.117500 c/s | soft 1.0636m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9365) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9365);
        amplitude *= 0.5;
    }
    return value;
}
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.7990 * (vec2(fbm2(p + vec2(0.0, t * 0.309167), per),
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.6956, 0.9917, 0.9826) * t
        + vec3(0.1618, 0.6602, 0.3551)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4210, x));
    return mix(g, hotStop, smoothstep(0.4210, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0049, 0.0422, g);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.7321;
    vec3 spun = clamp(hueSpin(baseCol, 0.2782), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2822) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.4649, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.7506);
    ...
    vec3 spinAxis = vec3(-0.6141, -0.7818, 0.1080);
    vec3 par = Vt * 2.3377;
    ...
    float deepBreath = 1.0 + 0.0175 * sin(time * 1.361357);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * (1.8903 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4975 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * (2.7440 * deepBreath)) + par * 0.0622, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.4245), deepStop, 0.1557);
    deepTrans *= 1.0 - 0.4975 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * (3.5977 * deepBreath)) + par * 0.1245, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.8491), deepStop, 0.3113);
    deepTrans *= 1.0 - 0.4975 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * (4.4514 * deepBreath)) + par * 0.1867, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4670);
    deepTrans *= 1.0 - 0.4975 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.7035;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0681, 0.0, 1.0), 1.5430);
    ...
    vec2 texUV = wuv * 2.0000 + vec2(0.051667, -0.010833) * time;
    ...
    vec3 hgtW = vec3(0.4702, 0.3186, 0.2112);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * 1.9114;
    ...
    float texDetail = clamp(atlas.r * 0.5313 + atlas.g * 0.3599 + atlas.b * 0.2386, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6916 + 0.5013 * texDetail, atlasPoleW) + 0.1804 * texDetail * (0.3025 + 0.6975 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (2.1675 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 2.6237);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0736).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0736).rgb;
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.9131.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.8897 * 0.8897 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.9131);
    ...
    float motifEnv = 0.72 + 0.28 * sin(time * 0.345575 + 1.9443);
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 16.0216) * (0.75 + 0.25 * sin(time * 0.555015 + 1.2900));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 16.0216);
    float motifPing = fract(time * 0.202500 + 0.0180);
    float motifPd = (motifR - motifPing * 0.5322) * 21.0894;
    motifM += 0.7474 * exp(-motifPd * motifPd) * (1.0 - motifPing);
    ...
    float v5Par = deepField(sdir * 2.3817 + par * 0.0658, time) * 0.5;
    v5Par += deepField(sdir * 3.2153 + par * 0.1316, time) * 0.3;
    v5Par += deepField(sdir * 4.1679 + par * 0.1974, time) * 0.2;
    float pattern = clamp(0.3505 * deepPat * mix(1.0, chordN, 0.2370) + 0.7893 * mid + 0.6101 * rim + flourish + grain + 0.1920 * v5Par + 0.1702 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.7459 + rim * 0.2662, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.8991 * mid + 0.5513 * rim, 0.0, 1.0);
    ...
    vec3 softHot = 1.0 - exp(-3.0717 * hotStop * (pattern + 0.35 * rim));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 0.8245 + baseUV.y * 0.5108 + 0.2981) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1234);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.6466 + baseUV.y * 0.31 + 0.9863) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2272);
    ...
    rgb *= (1.0 - 0.3312 * washHi) * (1.0 - 0.2956 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.6107, 0.4273, 0.6666);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.3513) / 1.3513, 0.0, 1.0);
    rgb *= 0.6362 + 0.5913 * keyDiff * (1.0 - 0.4828 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 32.5259);
    rgb += hotStop * (keySpec * 0.5718 * (1.0 - 0.7454 * washHi));
    ...
    float refrAmp = min((1.4606 + 2.0823 * fresnel) / max(length(worldPos), 1.0), 0.1750);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.6722) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.1105, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.8895, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.4420 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1680);
    float energyW = clamp(0.1766 + 0.4241 * clamp(pattern, 0.0, 1.0) + 0.2182 * fresnel, 0.0, 0.8655);
    ...
    rgb += fresnel * hotStop * (0.9859 * (1.0 - 0.5556 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4490) * 1.3426;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2707 * max(0.0, 1.0 - 0.4919 * baseLuma - 0.5347 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.4191 + 0.6984 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1600 * (0.5 + 0.5 * sin(time * 0.298451))) * atlasPoleW;
    rgb += emit * 0.7100 * (1.0 - 0.7690 * baseLuma) * (1.0 - 0.5476 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    vec3 v5Over = max(rgb - vec3(0.7469), vec3(0.0));
    rgb = min(rgb, vec3(0.7469)) + 0.2531 * (1.0 - exp(-v5Over * 2.4525));
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.0901));
    ...
    float bodyAlpha = 0.3585 + 0.3269 * presence + 0.4678 * pattern + 0.0952 * (1.0 - deepTrans) + 0.1978 * emit;
    ...
    rgb = mix(rgb, deepStop, v5Back * 0.3583);
    bodyAlpha *= 1.0 + v5Back * 0.2420;
    // [layer:v5:ghostalpha]
    ...
    bodyAlpha *= 0.7239 + 0.2761 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.2994 + hgtC * 0.5085, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.1955);
    bodyAlpha *= 1.0 + innerFace * 0.1275;
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9735) * vertexColor.a;
