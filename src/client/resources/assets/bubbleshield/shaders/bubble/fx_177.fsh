...
void main() {
    ...
    mid = clamp(mid * (0.6615 + 0.6860 * texDetail) + 0.3336 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7543 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4453), 0.0, 1.0);
    float emit = atlas.a * (0.6283 + 0.4019 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1235 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.7338 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3814 + 0.4120 * presence + 0.3909 * pattern + 0.1231 * (1.0 - deepTrans) + 0.2059 * emit, 0.9084);
