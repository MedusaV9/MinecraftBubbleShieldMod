...
// Bubble surface shader fx_375 -- family VOIDTENDRIL
// stack: deep=voro x4 taps | mid=voidtendril+warp1+pulse | rim=lat | flourish=swirl
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.6907 * (vec2(fbm2(p + vec2(0.0, t * 0.309167), per),
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7906, 0.0, 1.0);
    ...
    deepCol += 0.4599 * dp * mix(baseCol, deepStop, 0.2336);
    deepPat += 0.4599 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.0403) + par * 0.0983, time);
    deepCol += 0.2115 * dp * mix(baseCol, deepStop, 0.4671);
    deepPat += 0.2115 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.9954) + par * 0.1474, time);
    deepCol += 0.0973 * dp * mix(baseCol, deepStop, 0.7007);
    deepPat += 0.0973 * dp;
    deepCol *= 0.7286;
    deepPat = pow(clamp(deepPat * 0.5654, 0.0, 1.0), 1.7761);

    // [layer:mid:voidtendril_warp1_pulse]
    ...
    vec3 mdir = rotA(spinAxis, time * 0.057596) * (sdir * 1.4618);
    ...
    vec2 auv = vec2(baseUV.x * 5.0000 + time * -0.145833, (baseUV.y - 0.5) * 5.0000 * breathe + 2.5000 + time * 0.116667);
    vec2 wuv = warp1(auv, midPer, time);
    vec3 tdir = vec3(mdir.x, mdir.y * 0.2882, mdir.z);
    float tnoise = fbm3(tdir + vec3(0.0, 0.1688 * sin(time * 0.094248), 0.0));
    float tendril = pow(clamp(1.0 - abs(2.0 * tnoise - 1.0), 0.0, 1.0), 6.6622);
    float lat = abs(baseUV.y * 2.0 - 1.0);
    float reach = smoothstep(0.1398, 0.7824, lat + 0.2098 * sin(time * 0.104720));
    float murk = fbm3(mdir * 0.6 + vec3(7.7, 2.2, 5.5)) * 0.1277;
    float mid = clamp(tendril * reach * 1.1026 + murk, 0.0, 1.2);
    ...
    float flourish = 0.2561 * pow(clamp(fbm2(wuv + vec2(-time * 0.108333, time * 0.070000), midPer), 0.0, 1.0), 2.0);
    float grain = 0.0593 * (cellHash(floor(wuv * 36.0000) + vec2(floor(time * 6.0), 0.0), 180.0000) - 0.5);
    ...
    rgb = satLift(rgb, 1.2847);
    rgb += hotStop * 0.3333 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0750 + 0.3324 * presence + 0.4435 * pattern, 0.8292);
