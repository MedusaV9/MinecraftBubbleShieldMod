...
void main() {
    ...
    mid = clamp(mid * (0.6273 + 0.4665 * texDetail) + 0.2462 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4148), 0.0, 1.0);
    float emit = atlas.a * (0.7762 + 0.4187 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0807 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.9985 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4727 + 0.5653 * presence + 0.4435 * pattern + 0.1317 * (1.0 - deepTrans) + 0.1672 * emit, 0.9508);
