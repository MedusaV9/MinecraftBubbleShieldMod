...
void main() {
    ...
    mid = clamp(mid * (0.7110 + 0.6367 * texDetail) + 0.3424 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3728), 0.0, 1.0);
    float emit = atlas.a * (0.6757 + 0.2719 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1313 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.9190 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3259 + 0.3052 * presence + 0.3863 * pattern + 0.1257 * (1.0 - deepTrans) + 0.1454 * emit, 0.7816);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1529), 0.7816);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6741 + 0.3259 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1333), 0.0, 1.0);
