...
void main() {
    ...
    mid = clamp(mid * (0.6535 + 0.6678 * texDetail) + 0.2118 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3992), 0.0, 1.0);
    float emit = atlas.a * (0.6917 + 0.3485 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1048 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.7916 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3579 + 0.3825 * presence + 0.4139 * pattern + 0.1455 * (1.0 - deepTrans) + 0.2170 * emit, 0.8356);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1680), 0.8356);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7589 + 0.2411 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
