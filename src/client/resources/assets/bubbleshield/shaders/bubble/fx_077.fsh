...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.022500, -0.031667) * time, time);
    deep += 0.5344 * deepField(duvBase + rimDir * 0.0693 + vec2(0.034167, -0.050000) * time, time);
    deep += 0.2855 * deepField(duvBase + rimDir * 0.1386 + vec2(0.046667, -0.066667) * time, time);
    deep += 0.1526 * deepField(duvBase + rimDir * 0.2080 + vec2(0.059167, -0.085000) * time, time);
    deep = pow(clamp(deep * 0.5070, 0.0, 1.0), 1.5022);
    ...
    float pattern = clamp(0.3661 * deep + 0.7263 * mid + 0.7650 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2029 + pattern * 0.4500);
    vec3 rgb = vertexColor.rgb * (0.4332 + 0.8023 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2122 + 0.6928 * pattern, 0.0, 1.0);
