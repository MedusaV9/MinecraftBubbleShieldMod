...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8290, 0.0, 1.0);
    ...
    deepCol += 0.5344 * dp * mix(baseCol, deepStop, 0.1954);
    deepPat += 0.5344 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 3.4975) + par * 0.1386, time);
    deepCol += 0.2855 * dp * mix(baseCol, deepStop, 0.3908);
    deepPat += 0.2855 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.3753) + par * 0.2080, time);
    deepCol += 0.1526 * dp * mix(baseCol, deepStop, 0.5861);
    deepPat += 0.1526 * dp;
    deepCol *= 0.4958;
    deepPat = pow(clamp(deepPat * 0.5070, 0.0, 1.0), 1.5022);
    ...
    rgb += hotStop * 0.1996 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0689 + 0.3261 * presence + 0.4243 * pattern, 0.8394);
