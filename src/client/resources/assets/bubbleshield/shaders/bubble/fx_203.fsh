...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5910, 0.0, 1.0);
    ...
    deepCol += 0.4440 * dp * mix(baseCol, deepStop, 0.2764);
    deepPat += 0.4440 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 4.3225) + par * 0.1582, time);
    deepCol += 0.1971 * dp * mix(baseCol, deepStop, 0.5528);
    deepPat += 0.1971 * dp;
    deepCol *= 0.6794;
    deepPat = pow(clamp(deepPat * 0.6093, 0.0, 1.0), 1.5570);
    ...
    rgb = satLift(rgb, 1.1683);
    rgb += hotStop * 0.1949 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0759 + 0.3197 * presence + 0.4776 * pattern, 0.7950);
