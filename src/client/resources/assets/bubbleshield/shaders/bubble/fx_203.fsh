...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9635) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9635);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.016667, 0.037500) * time, time);
    deep += 0.4440 * deepField(duvBase + rimDir * 0.0791 + vec2(0.026667, 0.060000) * time, time);
    deep += 0.1971 * deepField(duvBase + rimDir * 0.1582 + vec2(0.036667, 0.080000) * time, time);
    deep = pow(clamp(deep * 0.6093, 0.0, 1.0), 1.5570);
    ...
    float neb = fbm2(wuv + 0.8805 * vec2(fbm2(wuv + vec2(time * 0.050000, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.035000), midPer)), midPer);
    float lanes = 1.0 - pow(clamp(1.0 - abs(2.0 * fbm2(wuv * 2.0 + vec2(4.2, 1.1), midPer * 2.0) - 1.0), 0.0, 1.0), 3.0) * 0.5921;
