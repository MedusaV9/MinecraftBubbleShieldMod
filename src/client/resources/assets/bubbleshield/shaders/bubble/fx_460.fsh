...
void main() {
    ...
    mid = clamp(mid * (0.7354 + 0.5455 * texDetail) + 0.2094 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4293), 0.0, 1.0);
    float emit = atlas.a * (0.6466 + 0.2724 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1435 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6554 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2921 + 0.2525 * presence + 0.4688 * pattern + 0.1573 * (1.0 - deepTrans) + 0.1650 * emit, 0.8799);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3042), 0.8799);
    // [layer:v5:ghostalpha]
