...
// stack: deep=caustic x3 taps | mid=spectralveil+warp1+scroll | rim=graze | flourish=glint
...
void main() {
    ...
    float flourish = 0.3151 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0513 + 0.1125 * presence + 0.4688 * pattern + 0.1573 * (1.0 - deepTrans), 0.8199);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3042), 0.8199);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5069 + 0.4931 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
