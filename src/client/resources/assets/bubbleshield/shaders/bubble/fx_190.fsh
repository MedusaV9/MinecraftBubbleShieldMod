...
// stack: deep=caustic x3 taps | mid=triweave+warp1+flicker | rim=graze | flourish=echo
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.011667, -0.015000) * time, time);
    deep += 0.4837 * deepField(duvBase + rimDir * 0.0523 + vec2(-0.018333, -0.025000) * time, time);
    deep += 0.2339 * deepField(duvBase + rimDir * 0.1046 + vec2(-0.025000, -0.035000) * time, time);
    deep = pow(clamp(deep * 0.5822, 0.0, 1.0), 1.2500);
    ...
    float pattern = clamp(0.4877 * deep + 0.8503 * mid + 0.6345 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2329 + pattern * 0.4171);
    vec3 rgb = vertexColor.rgb * (0.3643 + 0.6062 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1911 + 0.8064 * pattern, 0.0, 1.0);
