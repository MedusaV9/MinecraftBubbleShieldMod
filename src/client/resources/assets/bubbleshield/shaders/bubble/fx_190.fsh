...
// stack: deep=caustic x4 taps | mid=triweave+warp1+flicker | rim=graze | flourish=echo | tex=hex_lattice
// fbm: standard x3 octaves | seed 72150944f8383246
// v8 realism: refr 0.6855+0.8841*fres/dist (cap 0.1126) | energy 0.3004+0.4626*p | flow 0.110833 c/s | soft 0.7256m
...
vec4 atlasTile(vec2 p) {
    return texture(Sampler0, (vec2(3.0000, 0.0000) + clamp(fract(p), 0.004, 0.996)) * 0.25);
}
...
void main() {
    ...
    float refrAmp = min((0.6855 + 0.8841 * fresnel) / max(length(worldPos), 1.0), 0.1126);
