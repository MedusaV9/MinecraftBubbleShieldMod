...
void main() {
    ...
    mid = clamp(mid * (0.6092 + 0.5165 * texDetail) + 0.2916 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4187), 0.0, 1.0);
    float emit = atlas.a * (0.7332 + 0.2666 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1688 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.9996 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3430 + 0.2670 * presence + 0.4802 * pattern + 0.0971 * (1.0 - deepTrans) + 0.1591 * emit, 0.9049);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2551), 0.9049);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6207 + 0.3793 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
