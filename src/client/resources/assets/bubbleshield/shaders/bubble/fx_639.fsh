...
void main() {
    ...
    mid = clamp(mid * (0.6453 + 0.4580 * texDetail) + 0.3464 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4557), 0.0, 1.0);
    float emit = atlas.a * (0.6436 + 0.2725 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1592 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.7328 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3157 + 0.3688 * presence + 0.4639 * pattern + 0.0806 * (1.0 - deepTrans) + 0.1991 * emit, 0.8849);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2075), 0.8849);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6687 + 0.3313 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1028), 0.0, 1.0);
