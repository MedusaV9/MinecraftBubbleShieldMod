...
void main() {
    ...
    mid = clamp(mid * (0.6787 + 0.6251 * texDetail) + 0.3398 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5128), 0.0, 1.0);
    float emit = atlas.a * (0.6421 + 0.3809 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1077 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7832 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3583 + 0.3005 * presence + 0.5514 * pattern + 0.1224 * (1.0 - deepTrans) + 0.1489 * emit, 0.8694);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2134), 0.8694);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7690 + 0.2310 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1329), 0.0, 1.0);
