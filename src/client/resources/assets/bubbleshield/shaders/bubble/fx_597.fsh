...
void main() {
    ...
    mid = clamp(mid * (0.7034 + 0.5394 * texDetail) + 0.2508 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3936), 0.0, 1.0);
    float emit = atlas.a * (0.6669 + 0.3739 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1173 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9220 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2626 + 0.2569 * presence + 0.6345 * pattern + 0.1102 * (1.0 - deepTrans) + 0.1348 * emit, 0.9056);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3416), 0.9056);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7159 + 0.2841 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0778), 0.0, 1.0);
