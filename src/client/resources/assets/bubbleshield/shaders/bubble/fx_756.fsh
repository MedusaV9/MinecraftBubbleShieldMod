...
void main() {
    ...
    mid = clamp(mid * (0.6210 + 0.7053 * texDetail) + 0.2514 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3847), 0.0, 1.0);
    float emit = atlas.a * (0.5731 + 0.3217 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1646 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.7630 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4394 + 0.4391 * presence + 0.3906 * pattern + 0.1265 * (1.0 - deepTrans) + 0.1758 * emit, 0.8966);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2373), 0.8966);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6286 + 0.3714 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1077), 0.0, 1.0);
