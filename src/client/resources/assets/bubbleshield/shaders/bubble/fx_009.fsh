...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7552, 0.0, 1.0);
    ...
    deepCol += 0.4294 * dp * mix(baseCol, deepStop, 0.2255);
    deepPat += 0.4294 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 6.5141) + par * 0.0847, time);
    deepCol += 0.1844 * dp * mix(baseCol, deepStop, 0.4510);
    deepPat += 0.1844 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 8.2894) + par * 0.1271, time);
    deepCol += 0.0792 * dp * mix(baseCol, deepStop, 0.6765);
    deepPat += 0.0792 * dp;
    deepCol *= 0.6262;
    deepPat = pow(clamp(deepPat * 0.5907, 0.0, 1.0), 1.6648);
    ...
    rgb += hotStop * 0.2964 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0758 + 0.2974 * presence + 0.3848 * pattern, 0.8395);
