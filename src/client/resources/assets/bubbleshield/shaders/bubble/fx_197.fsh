...
// stack: deep=ridge x3 taps | mid=circuit+curl+rotate | rim=graze | flourish=glint | tex=circuit
// fbm: standard x4 octaves | seed 59fdbb9f5e98031e
// v8 realism: refr 0.7108+0.8323*fres/dist (cap 0.1529) | energy 0.3676+0.4655*p | flow 0.080833 c/s | soft 0.7312m
...
void main() {
    ...
    float refrAmp = min((0.7108 + 0.8323 * fresnel) / max(length(worldPos), 1.0), 0.1529);
