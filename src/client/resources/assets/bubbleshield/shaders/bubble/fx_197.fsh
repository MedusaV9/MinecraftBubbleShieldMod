...
void main() {
    ...
    mid = clamp(mid * (0.6372 + 0.6140 * texDetail) + 0.2175 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3596), 0.0, 1.0);
    float emit = atlas.a * (0.6696 + 0.4028 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0804 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.9533 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4299 + 0.3894 * presence + 0.3324 * pattern + 0.1175 * (1.0 - deepTrans) + 0.1431 * emit, 0.8764);
