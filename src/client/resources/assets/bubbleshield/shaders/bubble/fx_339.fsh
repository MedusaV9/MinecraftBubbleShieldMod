...
void main() {
    ...
    mid = clamp(mid * (0.6218 + 0.6849 * texDetail) + 0.3180 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3526), 0.0, 1.0);
    float emit = atlas.a * (0.6121 + 0.3075 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1200 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9223 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4294 + 0.5021 * presence + 0.3228 * pattern + 0.1200 * (1.0 - deepTrans) + 0.1806 * emit, 0.8428);
