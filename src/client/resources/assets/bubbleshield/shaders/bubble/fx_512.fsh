...
// v8 realism: refr 0.7683+0.8665*fres/dist (cap 0.1779) | energy 0.4993+0.4070*p | flow 0.089167 c/s | soft 0.6748m
...
void main() {
    ...
    rim = clamp(rim + 0.5028 * rimLine + 0.3925 * fresnel, 0.0, 1.4);
    ...
    float motifEnv = 0.55 + 0.45 * sin(time * 0.319395 + 5.5929);
    float motifM = 0.0;
    ...
    motifM += exp(-dot(motifD0, motifD0) * 168.6281);
    ...
    motifM += exp(-dot(motifD1, motifD1) * 168.6281);
    ...
    float refrAmp = min((0.7683 + 0.8665 * fresnel) / max(length(worldPos), 1.0), 0.1779);
    ...
    float energyW = clamp(0.4993 + 0.4070 * clamp(pattern, 0.0, 1.0) + 0.1088 * fresnel, 0.0, 0.8167);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.5852;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4077) * 0.7491;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3679 * (1.0 - 0.4755 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.3316 + 0.6796 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1743 * (0.5 + 0.5 * sin(time * 0.277507))) * atlasPoleW;
    rgb += emit * 0.9922 * (1.0 - 0.7415 * baseLuma) * emitCol;
    // [layer:v5:softknee]
    ...
    bodyAlpha = max(bodyAlpha, 0.8643);
