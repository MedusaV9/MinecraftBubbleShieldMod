...
// stack: deep=caustic x4 taps | mid=mycelia+curl+scroll | rim=graze_film | flourish=glint | tex=filaments
// fbm: standard x5 octaves | seed 9b337f09b374c352
// v8 realism: refr 0.2494+0.2813*fres/dist (cap 0.1379) | energy 0.4993+0.4070*p | flow 0.089167 c/s | soft 0.6748m
...
void main() {
    ...
    float refrAmp = min((0.2494 + 0.2813 * fresnel) / max(length(worldPos), 1.0), 0.1379);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0433, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9567, vec2(0.001), vec2(0.999))).b);
