...
void main() {
    ...
    mid = clamp(mid * (0.7271 + 0.5159 * texDetail) + 0.3058 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5042), 0.0, 1.0);
    float emit = atlas.a * (0.6027 + 0.4413 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1743 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.9922 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2993 + 0.2880 * presence + 0.5451 * pattern + 0.1172 * (1.0 - deepTrans) + 0.1339 * emit, 0.8773);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1538), 0.8773);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7200 + 0.2800 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
