...
void main() {
    ...
    mid = clamp(mid * (0.6219 + 0.5806 * texDetail) + 0.1820 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5498), 0.0, 1.0);
    float emit = atlas.a * (0.7535 + 0.2558 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1283 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9749 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4208 + 0.4897 * presence + 0.3945 * pattern + 0.1044 * (1.0 - deepTrans) + 0.2127 * emit, 0.9061);
