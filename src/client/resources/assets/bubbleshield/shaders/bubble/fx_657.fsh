...
void main() {
    ...
    mid = clamp(mid * (0.6321 + 0.6559 * texDetail) + 0.2915 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7200 + 0.2528 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0848 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.9972 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3268 + 0.2010 * presence + 0.6051 * pattern + 0.0932 * (1.0 - deepTrans) + 0.2092 * emit, 0.8983);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2531), 0.8983);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6959 + 0.3041 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0856), 0.0, 1.0);
