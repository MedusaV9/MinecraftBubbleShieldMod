...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.010000, 0.017500) * time, time);
    deep += 0.5669 * deepField(duvBase + rimDir * 0.0430 + vec2(0.015000, 0.027500) * time, time);
    deep += 0.3213 * deepField(duvBase + rimDir * 0.0859 + vec2(0.020000, 0.035000) * time, time);
    deep = pow(clamp(deep * 0.5296, 0.0, 1.0), 1.5056);
    ...
    float pattern = clamp(0.4989 * deep + 0.7587 * mid + 0.5154 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1901 + pattern * 0.5366);
    vec3 rgb = vertexColor.rgb * (0.5037 + 0.8450 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1031 + 0.6426 * pattern, 0.0, 1.0);
