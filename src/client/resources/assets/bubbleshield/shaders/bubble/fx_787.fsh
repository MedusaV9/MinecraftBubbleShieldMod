...
void main() {
    ...
    mid = clamp(mid * (0.6976 + 0.5048 * texDetail) + 0.3106 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5031), 0.0, 1.0);
    float emit = atlas.a * (0.7659 + 0.3343 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1538 * (0.5 + 0.5 * sin(time * 0.167552)));
    ...
    float alpha = vertexColor.a * min(0.3429 + 0.3894 * presence + 0.5119 * pattern + 0.1265 * (1.0 - deepTrans) + 0.1535 * emit, 0.8314);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3061), 0.8314);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7106 + 0.2894 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1098), 0.0, 1.0);
