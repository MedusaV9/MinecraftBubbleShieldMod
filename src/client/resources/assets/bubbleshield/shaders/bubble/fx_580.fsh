...
void main() {
    ...
    mid = clamp(mid * (0.7115 + 0.5647 * texDetail) + 0.2818 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4238), 0.0, 1.0);
    float emit = atlas.a * (0.6157 + 0.4968 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1638 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.9906 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2857 + 0.2576 * presence + 0.5124 * pattern + 0.1123 * (1.0 - deepTrans) + 0.2140 * emit, 0.8118);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3272), 0.8118);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7481 + 0.2519 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0725), 0.0, 1.0);
