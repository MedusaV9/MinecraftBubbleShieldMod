...
void main() {
    ...
    mid = clamp(mid * (0.7156 + 0.5901 * texDetail) + 0.2653 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3838), 0.0, 1.0);
    float emit = atlas.a * (0.5965 + 0.4190 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1185 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.9877 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4191 + 0.4180 * presence + 0.4725 * pattern + 0.1412 * (1.0 - deepTrans) + 0.1246 * emit, 0.8513);
