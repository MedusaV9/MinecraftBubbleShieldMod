...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5803, 0.0, 1.0);
    ...
    deepCol += 0.4565 * dp * mix(baseCol, deepStop, 0.2739);
    deepPat += 0.4565 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.1001) + par * 0.0918, time);
    deepCol += 0.2084 * dp * mix(baseCol, deepStop, 0.5478);
    deepPat += 0.2084 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 5.1066) + par * 0.1378, time);
    deepCol += 0.0951 * dp * mix(baseCol, deepStop, 0.8217);
    deepPat += 0.0951 * dp;
    deepCol *= 0.5956;
    deepPat = pow(clamp(deepPat * 0.5682, 0.0, 1.0), 1.0320);
    ...
    rgb += hotStop * 0.1556 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0708 + 0.2780 * presence + 0.4725 * pattern, 0.7913);
