...
void main() {
    ...
    mid = clamp(mid * (0.6672 + 0.7355 * texDetail) + 0.2794 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3842), 0.0, 1.0);
    float emit = atlas.a * (0.7886 + 0.3056 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1099 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.7282 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2858 + 0.2010 * presence + 0.5400 * pattern + 0.1145 * (1.0 - deepTrans) + 0.2114 * emit, 0.9171);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1566), 0.9171);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6274 + 0.3726 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
