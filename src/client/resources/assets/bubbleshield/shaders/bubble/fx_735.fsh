...
void main() {
    ...
    mid = clamp(mid * (0.6108 + 0.5633 * texDetail) + 0.3290 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4850), 0.0, 1.0);
    float emit = atlas.a * (0.6812 + 0.3452 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1115 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.6813 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3549 + 0.2957 * presence + 0.5748 * pattern + 0.0828 * (1.0 - deepTrans) + 0.1986 * emit, 0.8522);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3274), 0.8522);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7717 + 0.2283 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0798), 0.0, 1.0);
