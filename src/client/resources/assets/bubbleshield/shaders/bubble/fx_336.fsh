...
// v8 realism: refr 0.5565+0.7794*fres/dist (cap 0.1294) | energy 0.3724+0.5140*p | flow 0.080000 c/s | soft 0.5622m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.4587 + 0.2067, 0.0, 1.0);
    ...
    float refrAmp = min((0.5565 + 0.7794 * fresnel) / max(length(worldPos), 1.0), 0.1294);
    ...
    float emitMask = clamp(atlas.a * 1.7208, 0.0, 1.0);
    float emit = emitMask * (0.3435 + 0.4730 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1411 * (0.5 + 0.5 * sin(time * 0.240855))) * atlasPoleW;
