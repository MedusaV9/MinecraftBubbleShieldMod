...
// stack: deep=fbm x3 taps | mid=ectoplasm+warp1+pulse | rim=lat | flourish=shimmer
...
void main() {
    ...
    rgb = mix(rgb, hotStop, clamp(ecEdge, 0.0, 1.0) * 0.3817);
    ...
    float alpha = vertexColor.a * min(0.0592 + 0.2219 * presence + 0.5453 * pattern + 0.1017 * (1.0 - deepTrans), 0.7910);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1775), 0.7910);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5356 + 0.4644 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
