...
void main() {
    ...
    mid = clamp(mid * (0.6693 + 0.4603 * texDetail) + 0.2280 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.6571 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3885), 0.0, 1.0);
    float emit = atlas.a * (0.5936 + 0.4745 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1058 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.9490 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3292 + 0.3135 * presence + 0.5453 * pattern + 0.1017 * (1.0 - deepTrans) + 0.2100 * emit, 0.8510);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1775), 0.8510);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7664 + 0.2336 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
