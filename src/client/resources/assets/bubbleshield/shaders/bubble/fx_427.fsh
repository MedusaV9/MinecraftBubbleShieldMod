...
// Bubble surface shader fx_427 -- family IRISFILM
// stack: deep=ridge x4 taps | mid=irisfilm+curl+flicker | rim=lat | flourish=shimmer | tex=iris_eye | motif=corex3+doublepulse
// fbm: standard x4 octaves | seed 152a79e666bc0d68
// v8 realism: refr 1.6621+1.8709*fres/dist (cap 0.1565) | energy 0.1995+0.4145*p | flow 0.107500 c/s | soft 1.0115m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0029) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0029);
        amplitude *= 0.5;
    }
    return value;
}
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.6596, 0.7371, 1.0679) * t
        + vec3(0.3627, 0.5187, 0.0480)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4272, x));
    return mix(g, hotStop, smoothstep(0.4272, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0064, 0.0576, g);
}

// latitude band rim: pole caps plus a soft equator belt
float rimLat(vec2 uv) {
    float lat = abs(uv.y * 2.0 - 1.0);
    float poles = smoothstep(0.6856, 1.0, lat);
    float belt = invsmooth(0.0, 0.3208, lat);
    return clamp(poles + 0.3276 * belt, 0.0, 1.0);
}
...
float deepField(vec3 p, float t) {
    float n = fbm3(p);
    return pow(1.0 - abs(2.0 * n - 1.0), 2.9838);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.6217;
    vec3 spun = clamp(hueSpin(baseCol, 0.5242), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.3263) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5071, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.8235);
    ...
    vec3 spinAxis = vec3(-0.3110, -0.3341, -0.8898);
    vec3 par = Vt * 2.2764;
    ...
    float deepBreath = 1.0 + 0.0139 * sin(time * 1.596976);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.089012) * (sdir * (2.4318 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.5799 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * (3.5433 * deepBreath)) + par * 0.0623, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.3694), deepStop, 0.1355);
    deepTrans *= 1.0 - 0.5799 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * (4.6549 * deepBreath)) + par * 0.1247, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.7389), deepStop, 0.2709);
    deepTrans *= 1.0 - 0.5799 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * (5.7664 * deepBreath)) + par * 0.1870, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4064);
    deepTrans *= 1.0 - 0.5799 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.8408;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0321, 0.0, 1.0), 1.7644);
    ...
    float ft = time + 0.2769 * (0.6 * sin(time * 5.167920) + 0.4 * sin(time * 10.503391 + 3.6936));
    ...
    vec2 texUV = wuv * 4.0000 + vec2(0.055833, -0.005000) * time;
    ...
    vec3 hgtW = vec3(0.3133, 0.3866, 0.3001);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -3.1540;
    ...
    float texDetail = clamp(atlas.r * 0.3555 + atlas.g * 0.4386 + atlas.b * 0.3405, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.7008 + 0.5703 * texDetail, atlasPoleW) + 0.1711 * texDetail * (0.2067 + 0.7933 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.4672 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 2.6543);
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.8518.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9048 * 0.9048 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.8518);
    ...
    float rimLine = smoothstep(0.0626, 0.0829, rimSlope);
    float rim = rimLat(baseUV) * 0.7836;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.5056 * rimLine + 0.8037 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1583 * caustic(wuv + vec2(3.1, 8.7), vec2(0.153333, -0.120000) * time, midPer);
    float grain = 0.0730 * (cellHash(floor(wuv * 57.0000) + vec2(floor(time * 2.0), 0.0), 228.0000) - 0.5);
    ...
    float motifP = fract(time * 0.393333 + 0.3017);
    ...
    float motifM = exp(-motifR * motifR * 14.0070) * (0.75 + 0.25 * sin(time * 0.680678 + 4.9444));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 14.0070);
    ...
    float v5Par = deepField(sdir * 2.1022 + par * 0.1004, time) * 0.5;
    v5Par += deepField(sdir * 2.8380 + par * 0.2009, time) * 0.3;
    v5Par += deepField(sdir * 3.6789 + par * 0.3013, time) * 0.2;
    float pattern = clamp(0.3478 * deepPat * mix(1.0, chordN, 0.2495) + 0.8531 * mid + 0.6496 * rim + flourish + grain + 0.2148 * v5Par + 0.2277 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.9418 + rim * 0.1490, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(1.1068 * mid + 0.6466 * rim, 0.0, 1.0);
    ...
    float texBody = 0.8647 + 0.3307 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4873 + 0.5127 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.3698);
    ...
    vec3 softHot = 1.0 - exp(-2.9207 * hotStop * (pattern + 0.35 * rim));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 0.8386 + baseUV.y * 0.6499 + 0.0476) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1788);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.5626 + baseUV.y * 0.31 + 0.6703) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2149);
    ...
    rgb *= (1.0 - 0.3108 * washHi) * (1.0 - 0.3629 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.4446, 0.8416, -0.3068);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4181) / 1.4181, 0.0, 1.0);
    rgb *= 0.6967 + 0.5357 * keyDiff * (1.0 - 0.4736 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 27.0693);
    rgb += hotStop * (keySpec * 0.5823 * (1.0 - 0.5840 * washHi));
    ...
    float refrAmp = min((1.6621 + 1.8709 * fresnel) / max(length(worldPos), 1.0), 0.1565);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.5698) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0827, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9173, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.4523 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1409);
    float energyW = clamp(0.1995 + 0.4145 * clamp(pattern, 0.0, 1.0) + 0.2780 * fresnel, 0.0, 0.8175);
    ...
    rgb += fresnel * hotStop * (0.8933 * (1.0 - 0.6317 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2420) * 1.2086;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2711 * max(0.0, 1.0 - 0.5976 * baseLuma - 0.6116 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3060 + 0.6865 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1094 * (0.5 + 0.5 * sin(time * 0.172788))) * atlasPoleW;
    rgb += emit * 0.8565 * (1.0 - 0.7637 * baseLuma) * (1.0 - 0.4779 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    rgb = min(rgb, vec3(0.6501)) + 0.3499 * (1.0 - exp(-v5Over * 1.6378));
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.2817));
    ...
    float bodyAlpha = 0.3125 + 0.3560 * presence + 0.4205 * pattern + 0.0939 * (1.0 - deepTrans) + 0.1824 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.2312;
    // [layer:v5:backface]
    ...
    rgb = mix(rgb, deepStop, v5Back * 0.2887);
    ...
    bodyAlpha *= 0.6492 + 0.3508 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.0361 + hgtC * 0.2833, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.2512);
    bodyAlpha *= 1.0 + innerFace * 0.1869;
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9677) * vertexColor.a;
