...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.040000, 0.043333) * time, time);
    deep += 0.4838 * deepField(duvBase + rimDir * 0.0376 + vec2(0.062500, 0.066667) * time, time);
    deep = pow(clamp(deep * 0.6739, 0.0, 1.0), 1.1973);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.525000), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 9.1332);
    float flash = 0.6 + 0.4 * sin(time * 2.853613 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
