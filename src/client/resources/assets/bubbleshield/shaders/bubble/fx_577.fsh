...
void main() {
    ...
    mid = clamp(mid * (0.6983 + 0.5364 * texDetail) + 0.2464 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3768), 0.0, 1.0);
    float emit = atlas.a * (0.6964 + 0.3963 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1101 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.9989 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3060 + 0.2045 * presence + 0.5687 * pattern + 0.1223 * (1.0 - deepTrans) + 0.1284 * emit, 0.8981);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1908), 0.8981);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6920 + 0.3080 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0650), 0.0, 1.0);
