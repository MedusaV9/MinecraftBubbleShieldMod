...
// stack: deep=caustic x4 taps | mid=galaxyswirl+warp1+flicker | rim=graze_sparkle | flourish=echo | tex=starfield
// fbm: turb x6 octaves | seed b5212c657a854b04
// v8 realism: refr 0.2732+0.3941*fres/dist (cap 0.1449) | energy 0.4665+0.4575*p | flow 0.075833 c/s | soft 0.6546m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.4700 + 0.2094, 0.0, 1.0);
    ...
    float refrAmp = min((0.2732 + 0.3941 * fresnel) / max(length(worldPos), 1.0), 0.1449);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0344, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9656, vec2(0.001), vec2(0.999))).b);
    ...
    float emitMask = clamp(atlas.a * 2.0811, 0.0, 1.0);
    ...
    float emit = emitMask * (0.3971 + 0.5717 * emitDrive) * (1.0 - 0.1421 * (0.5 + 0.5 * sin(time * 0.130900))) * atlasPoleW;
