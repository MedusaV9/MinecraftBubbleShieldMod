...
void main() {
    ...
    mid = clamp(mid * (0.7217 + 0.6843 * texDetail) + 0.2911 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4579), 0.0, 1.0);
    float emit = atlas.a * (0.7118 + 0.3514 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1421 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.9049 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3015 + 0.3268 * presence + 0.5321 * pattern + 0.0925 * (1.0 - deepTrans) + 0.2171 * emit, 0.8683);
