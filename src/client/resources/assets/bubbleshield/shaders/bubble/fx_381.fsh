...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6362, 0.0, 1.0);
    ...
    deepCol += 0.4808 * dp * mix(baseCol, deepStop, 0.2352);
    deepPat += 0.4808 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.3133) + par * 0.1120, time);
    deepCol += 0.2312 * dp * mix(baseCol, deepStop, 0.4703);
    deepPat += 0.2312 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.3809) + par * 0.1680, time);
    deepCol += 0.1111 * dp * mix(baseCol, deepStop, 0.7055);
    deepPat += 0.1111 * dp;
    deepCol *= 0.5551;
    deepPat = pow(clamp(deepPat * 0.5485, 0.0, 1.0), 1.7111);
    ...
    float arms = pow(clamp(arm, 0.0, 1.0), 4.9641) * smoothstep(0.08, 0.45, grad) * invsmooth(0.55, 1.15, grad);
    float core = invsmooth(0.02, 0.3261, grad);
    float dust = voronoise(wuv, midPer, 0.9655, 0.1517);
    float mid = clamp(arms * 0.9653 + core + dust * 0.2014 * arms, 0.0, 1.3);
    ...
    float pattern = clamp(0.3028 * deepPat + 0.7150 * mid + 0.5105 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.2388 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0515 + 0.3486 * presence + 0.4011 * pattern, 0.8083);
