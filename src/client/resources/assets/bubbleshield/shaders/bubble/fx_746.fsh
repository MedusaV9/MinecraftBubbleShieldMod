...
void main() {
    ...
    mid = clamp(mid * (0.7268 + 0.6986 * texDetail) + 0.2201 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5083), 0.0, 1.0);
    float emit = atlas.a * (0.7411 + 0.3374 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1722 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.8613 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3580 + 0.4402 * presence + 0.3302 * pattern + 0.1443 * (1.0 - deepTrans) + 0.1647 * emit, 0.8406);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1965), 0.8406);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7674 + 0.2326 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0786), 0.0, 1.0);
