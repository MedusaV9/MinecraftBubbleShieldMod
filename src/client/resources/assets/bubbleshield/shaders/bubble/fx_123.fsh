...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0419) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0419);
        amplitude *= 0.5;
    }
    return value;
}
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(1.0000, 2.0000), t * 0.303687);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.022500, 0.025000) * time, time);
    deep += 0.4614 * deepField(duvBase + rimDir * 0.0449 + vec2(-0.035000, 0.038333) * time, time);
    deep += 0.2129 * deepField(duvBase + rimDir * 0.0898 + vec2(-0.046667, 0.051667) * time, time);
    deep += 0.0983 * deepField(duvBase + rimDir * 0.1348 + vec2(-0.059167, 0.065000) * time, time);
    deep = pow(clamp(deep * 0.5641, 0.0, 1.0), 1.6781);
    ...
    float n = fbm2(wuv + vec2(0.0, time * 0.647500), midPer);
