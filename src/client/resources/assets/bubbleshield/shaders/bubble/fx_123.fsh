...
void main() {
    ...
    mid = clamp(mid * (0.6661 + 0.5440 * texDetail) + 0.3472 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4045), 0.0, 1.0);
    float emit = atlas.a * (0.5673 + 0.4755 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1145 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.7985 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4408 + 0.4482 * presence + 0.4904 * pattern + 0.1288 * (1.0 - deepTrans) + 0.1962 * emit, 0.8905);
