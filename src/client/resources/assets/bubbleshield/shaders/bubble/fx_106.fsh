...
// Bubble surface shader fx_106 -- family PETALS
// stack: deep=fbm x3 taps | mid=petals+curl+flicker | rim=graze_sparkle | flourish=shimmer | tex=feather_barbs | motif=orbitx1+sparklegate
// fbm: turb x6 octaves | seed 4dbc6c250a73d7b5
// v8 realism: refr 1.4254+1.6356*fres/dist (cap 0.1469) | energy 0.2976+0.3996*p | flow 0.104167 c/s | soft 0.8818m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0793) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0793);
        amplitude *= 0.5;
    }
    return value;
}
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(1.0350, 0.9137, 1.1829) * t
        + vec3(0.6857, 0.4552, 0.4531)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.5291, x));
    return mix(g, hotStop, smoothstep(0.5291, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0060, 0.0340, g);
}
...
float sparkle(vec2 p, float t, float px) {
    ...
    return step(0.6817, h) * invsmooth(0.02, 0.22, d) * tw;
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.5712;
    vec3 spun = clamp(hueSpin(baseCol, -0.5747), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2416) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5211, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.7196);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, 0.2686), 0.0, 1.0), 1.8281) * 0.6392, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(0.6161, 0.6384, -0.4614);
    vec3 par = Vt * 1.8274;
    ...
    float deepBreath = 1.0 + 0.0102 * sin(time * 2.382374);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * (2.2733 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.5033 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * (3.6323 * deepBreath)) + par * 0.0637, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.5280), deepStop, 0.1936);
    deepTrans *= 1.0 - 0.5033 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * (4.9913 * deepBreath)) + par * 0.1273, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.3872);
    deepTrans *= 1.0 - 0.5033 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.7658;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.1396, 0.0, 1.0), 1.2837);

    // [layer:mid:petals_curl_flicker]
    ...
    float ft = time + 0.2138 * (0.6 * sin(time * 4.309218) + 0.4 * sin(time * 10.822787 + 1.3578));
    vec2 auv = vec2(baseUV.x * 6.0000, baseUV.y * 6.0000) + vec2(0.400000, -0.466667) * ft;
    vec2 wuv = auv + 0.2782 * curl2(auv + vec2(0.0, time * 0.073333), midPer);
    ...
    float ang = safeAtan(baseUV.y - 0.5, sin(du * 6.2831853) * 0.5) + time * 0.198968;
    float rad = length(vec2(sin(du * 3.1415927), baseUV.y - 0.5)) * 2.0;
    float petal = pow(abs(cos(ang * 3.0000)), 1.2872);
    float bloom = invsmooth(petal * 0.9 - 0.12, petal * 0.9 + 0.08, rad);
    float veins = 0.5 + 0.5 * sin(rad * 15.7133 - time * 0.664970);
    ...
    vec3 hgtW = vec3(0.3701, 0.3424, 0.2875);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -2.5808;
    ...
    float texDetail = clamp(atlas.r * 0.5177 + atlas.g * 0.4789 + atlas.b * 0.4020, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.7146 + 0.6153 * texDetail, atlasPoleW) + 0.1129 * texDetail * (0.3310 + 0.6690 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 2.8585);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0467).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0467).rgb;
    float deepTex = 0.5 * (dot(deepTexA, hgtW) + dot(deepTexB, hgtW));

    // [layer:thick:chord]
    // v11 volumetric thickness: the membrane is a shell of relative
    // thickness rho = 0.1339 (organic material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.9996.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.8661 * 0.8661 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.9996);
    float chordN = min(chord / 0.1339, 3.0);
    ...
    float rimLine = smoothstep(0.0700, 0.0968, rimSlope);
    float rimGlint = sparkle(baseUV * 18.0000, time, 18.0000);
    float rim = rimGraze() * 0.7436 * (0.7 + 0.6 * rimGlint) + 0.4049 * rimGlint;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.5751 * rimLine + 0.8192 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1030 * caustic(wuv + vec2(3.1, 8.7), vec2(0.140000, -0.120000) * time, midPer);
    float grain = 0.0960 * (cellHash(floor(wuv * 33.0000) + vec2(floor(time * 2.0), 0.0), 198.0000) - 0.5);
    ...
    float motifEnv = 0.48 + 0.52 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.581195)) * (0.5 + 0.5 * sin(time * 0.769690 + 1.7)));
    float motifM = 0.0;
    float motifO0 = fract(0.0601 + 1.0 * time * 0.065833);
    float motifL0 = 0.5550 + 0.1353 * sin(time * 0.256563 + 0.0000);
    vec2 motifD0 = vec2(sin((baseUV.x - motifO0) * 3.1415927) * 0.6366, baseUV.y - motifL0);
    motifM += exp(-dot(motifD0, motifD0) * 90.6458);
    ...
    float pattern = clamp(0.4041 * deepPat * mix(1.0, chordN, 0.2058) + 0.9810 * mid + 0.5279 * rim + flourish + grain + 0.1048 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.8450 + rim * 0.2139, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(1.0706 * mid + 0.5855 * rim, 0.0, 1.0);
    ...
    float texBody = 0.8230 + 0.4326 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.5469 + 0.4531 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.1394);
    ...
    vec3 softHot = 1.0 - exp(-2.4952 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.2505 * smoothstep(0.55, 1.10, pattern));
    vec3 accent = accentPalette(0.8540 + pattern * 0.4933);
    rgb = mix(rgb, rgb * (0.55 + 0.9 * accent), 0.1864);
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.0393 + baseUV.y * 0.7751 + 0.8287) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.2122);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.7719 + baseUV.y * 0.31 + 0.5298) * 6.2831853);
    ...
    rgb *= (1.0 - 0.3767 * washHi) * (1.0 - 0.3977 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.5661, 0.6124, -0.5517);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.5539) / 1.5539, 0.0, 1.0);
    rgb *= 0.5904 + 0.6593 * keyDiff * (1.0 - 0.4900 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 23.6468);
    rgb += hotStop * (keySpec * 0.4671 * (1.0 - 0.7391 * washHi));
    ...
    float refrAmp = min((1.4254 + 1.6356 * fresnel) / max(length(worldPos), 1.0), 0.1469);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.3762) * refrAmp;
    ...
    refracted *= exp(-0.6212 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.3142);
    float energyW = clamp(0.2976 + 0.3996 * clamp(pattern, 0.0, 1.0) + 0.2200 * fresnel, 0.0, 0.8972);
    ...
    rgb += fresnel * hotStop * (1.0856 * (1.0 - 0.6044 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4417) * 1.2989;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3318 * max(0.0, 1.0 - 0.5933 * baseLuma - 0.6063 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.4451 + 0.6027 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1385 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
    rgb += emit * 0.7383 * (1.0 - 0.7533 * baseLuma) * (1.0 - 0.5247 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.3438));
    ...
    float bodyAlpha = 0.4332 + 0.4635 * presence + 0.4579 * pattern + 0.1260 * (1.0 - deepTrans) + 0.1539 * emit;
    ...
    // perspective reach.
    float innerFog = clamp(deepField(rotA(spinAxis, time * 0.052360 + 2.6831) * (sdir * 2.0460), time) * 1.3605 + 0.1799, 0.0, 1.0);
    ...
    bodyAlpha *= smoothstep(0.0, 0.8818, sceneVDist - fragVDist);
