...
// stack: deep=fbm x2 taps | mid=petals+curl+flicker | rim=graze_sparkle | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.046667, 0.045000) * time, time);
    deep += 0.4594 * deepField(duvBase + rimDir * 0.0637 + vec2(0.072500, 0.070000) * time, time);
    deep = pow(clamp(deep * 0.6852, 0.0, 1.0), 1.2837);
    ...
    float pattern = clamp(0.4041 * deep + 0.9810 * mid + 0.5279 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8540 + pattern * 0.4933);
    vec3 rgb = vertexColor.rgb * (0.4641 + 0.6122 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1071 + 0.7445 * pattern, 0.0, 1.0);
