...
void main() {
    ...
    mid = clamp(mid * (0.7146 + 0.6153 * texDetail) + 0.2019 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4525), 0.0, 1.0);
    float emit = atlas.a * (0.7919 + 0.3773 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1385 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.7383 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4332 + 0.4635 * presence + 0.4579 * pattern + 0.1260 * (1.0 - deepTrans) + 0.1539 * emit, 0.8613);
