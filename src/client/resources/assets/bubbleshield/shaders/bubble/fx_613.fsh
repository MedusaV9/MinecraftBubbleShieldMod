...
void main() {
    ...
    mid = clamp(mid * (0.6081 + 0.6916 * texDetail) + 0.1823 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3548), 0.0, 1.0);
    float emit = atlas.a * (0.6269 + 0.2844 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1053 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.7261 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3607 + 0.3229 * presence + 0.4918 * pattern + 0.1477 * (1.0 - deepTrans) + 0.1900 * emit, 0.8625);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3408), 0.8625);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6577 + 0.3423 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
