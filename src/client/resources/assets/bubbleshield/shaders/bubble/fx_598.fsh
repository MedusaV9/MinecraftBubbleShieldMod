...
// v8 realism: refr 1.0723+1.0546*fres/dist (cap 0.1858) | energy 0.3489+0.4181*p | flow 0.054167 c/s | soft 1.0795m
...
void main() {
    ...
    rim = clamp(rim + 0.8300 * rimLine + 0.5255 * fresnel, 0.0, 1.4);
    ...
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 17.0052);
    float motifDy = (baseUV.y - 0.4677) * 18.4250;
    ...
    float refrAmp = min((1.0723 + 1.0546 * fresnel) / max(length(worldPos), 1.0), 0.1858);
    ...
    float energyW = clamp(0.3489 + 0.4181 * clamp(pattern, 0.0, 1.0) + 0.2134 * fresnel, 0.0, 0.8042);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.6553;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3281) * 0.7723;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3583 * (1.0 - 0.5036 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.4443 + 0.5459 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1236 * (0.5 + 0.5 * sin(time * 0.125664))) * atlasPoleW;
    rgb += emit * 0.8728 * (1.0 - 0.6156 * baseLuma) * emitCol;
    // [layer:v5:softknee]
