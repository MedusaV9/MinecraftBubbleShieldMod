...
// stack: deep=fbm x3 taps | mid=ectoplasm+warp1+rotate | rim=graze | flourish=glint | tex=foam
// fbm: ridged x4 octaves | seed 6e7cfc6404677a27
// v8 realism: refr 0.4556+0.4481*fres/dist (cap 0.1458) | energy 0.3489+0.4181*p | flow 0.054167 c/s | soft 1.0795m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (0.9372 * atlasPoleW));
    ...
    float refrAmp = min((0.4556 + 0.4481 * fresnel) / max(length(worldPos), 1.0), 0.1458);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0371, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9629, vec2(0.001), vec2(0.999))).b);
