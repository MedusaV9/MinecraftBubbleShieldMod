...
void main() {
    ...
    mid = clamp(mid * (0.6309 + 0.5913 * texDetail) + 0.3390 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4904), 0.0, 1.0);
    float emit = atlas.a * (0.7905 + 0.3299 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1236 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.8728 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3167 + 0.2850 * presence + 0.5577 * pattern + 0.1138 * (1.0 - deepTrans) + 0.2083 * emit, 0.8441);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2073), 0.8441);
    // [layer:v5:ghostalpha]
