...
// Bubble surface shader fx_598 -- family ECTOPLASM
// stack: deep=fbm x3 taps | mid=ectoplasm+warp1+rotate | rim=graze | flourish=glint | tex=worley_cells | motif=sigilx2+doublepulse
// fbm: ridged x4 octaves | seed 6e7cfc6404677a27
// v8 realism: refr 1.1723+1.1529*fres/dist (cap 0.1858) | energy 0.2674+0.4181*p | flow 0.054167 c/s | soft 1.0795m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0078) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0078);
        amplitude *= 0.5;
    }
    return value;
}
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.7315 * (vec2(fbm2(p + vec2(0.0, t * 0.309167), per),
        fbm2(p + vec2(5.2, 1.3) - vec2(t * 0.270833, 0.0), per)) - 0.5);
}

// iq cosine palette, baked per effect; used ONLY for bounded accents
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.6339, 1.1873, 1.0295) * t
        + vec3(0.5005, 0.9250, 0.8983)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4825, x));
    return mix(g, hotStop, smoothstep(0.4825, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0066, 0.0400, g);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.5534;
    vec3 spun = clamp(hueSpin(baseCol, -0.6065), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.3485) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5384, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.7686);
    ...
    vec3 spinAxis = vec3(-0.5184, 0.8106, 0.2724);
    vec3 par = Vt * 1.8175;
    ...
    float deepBreath = 1.0 + 0.0151 * sin(time * 1.659808);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * (2.6506 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4978 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * (3.9330 * deepBreath)) + par * 0.0717, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.5125), deepStop, 0.1879);
    deepTrans *= 1.0 - 0.4978 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * (5.2154 * deepBreath)) + par * 0.1434, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.3758);
    deepTrans *= 1.0 - 0.4978 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.9084;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.1450, 0.0, 1.0), 1.0615);
    ...
    float ecMask = smoothstep(0.4158, 0.7043, ecBody * 0.55 + ecDrip * 0.45 + (1.0 - baseUV.y) * 0.2216);
    ...
    vec2 texUV = wuv * 2.0000 + vec2(0.045000, 0.065833) * time;
    ...
    vec3 hgtW = vec3(0.4003, 0.3914, 0.2083);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * 3.8128;
    ...
    float texDetail = clamp(atlas.r * 0.4577 + atlas.g * 0.4474 + atlas.b * 0.2381, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6309 + 0.5913 * texDetail, atlasPoleW) + 0.1935 * texDetail * (0.3293 + 0.6707 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.2934 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 2.6603);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0758).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0758).rgb;
    ...
    // 2*sqrt(1 - (1-rho)^2) = 1.0419.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.8536 * 0.8536 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 1.0419);
    ...
    float rimLine = smoothstep(0.0469, 0.0820, rimSlope);
    float rim = rimGraze() * 0.7272;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.8300 * rimLine + 0.7606 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2302 * sparkle(wuv * 2.0 + 7.7, time * 2.0, midPer.x * 2.0);
    float grain = 0.0568 * (cellHash(floor(wuv * 53.0000) + vec2(floor(time * 2.0), 0.0), 265.0000) - 0.5);
    ...
    float motifP = fract(time * 0.286667 + 0.4942);
    float motifEnv = 0.52 + 0.75 * exp(-140.0 * (motifP - 0.16) * (motifP - 0.16)) + 0.60 * exp(-140.0 * (motifP - 0.38) * (motifP - 0.38));
    float motifA = baseUV.x * 5.0000 + -1.0 * time * 0.070833 + 0.7292;
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 12.0035);
    float motifDy = (baseUV.y - 0.4677) * 12.4799;
    ...
    float v5Par = deepField(sdir * 1.5919 + par * 0.0877, time) * 0.5;
    v5Par += deepField(sdir * 2.1490 + par * 0.1755, time) * 0.3;
    v5Par += deepField(sdir * 2.7858 + par * 0.2632, time) * 0.2;
    float pattern = clamp(0.4437 * deepPat * mix(1.0, chordN, 0.3010) + 0.9672 * mid + 0.7718 * rim + flourish + grain + 0.1229 * v5Par + 0.1765 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.9258 + rim * 0.2177, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(1.1328 * mid + 0.6559 * rim, 0.0, 1.0);
    ...
    float texBody = 0.8485 + 0.3704 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4431 + 0.5569 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.3218);
    ...
    vec3 softHot = 1.0 - exp(-3.1356 * hotStop * (pattern + 0.35 * rim));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 0.8383 + baseUV.y * 0.5562 + 0.5025) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1061);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.5966 + baseUV.y * 0.31 + 0.4475) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.1994);
    ...
    rgb *= (1.0 - 0.3794 * washHi) * (1.0 - 0.3214 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.4371, 0.8922, -0.1139);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.5409) / 1.5409, 0.0, 1.0);
    rgb *= 0.5825 + 0.5688 * keyDiff * (1.0 - 0.4518 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 10.4466);
    rgb += hotStop * (keySpec * 0.2896 * (1.0 - 0.5972 * washHi));
    ...
    float refrAmp = min((1.1723 + 1.1529 * fresnel) / max(length(worldPos), 1.0), 0.1858);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.5111) * refrAmp;
    ...
    refracted *= exp(-0.7133 * chordN * (1.0 - baseCol));
    ...
    float energyW = clamp(0.2674 + 0.4181 * clamp(pattern, 0.0, 1.0) + 0.3123 * fresnel, 0.0, 0.8042);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2925 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (0.9728 * (1.0 - 0.5874 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3281) * 1.1501;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3583 * max(0.0, 1.0 - 0.5036 * baseLuma - 0.6750 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.4443 + 0.5459 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1236 * (0.5 + 0.5 * sin(time * 0.125664))) * atlasPoleW;
    rgb += emit * 0.8728 * (1.0 - 0.6156 * baseLuma) * (1.0 - 0.5612 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    vec3 v5Over = max(rgb - vec3(0.6351), vec3(0.0));
    rgb = min(rgb, vec3(0.6351)) + 0.3649 * (1.0 - exp(-v5Over * 2.3463));
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2465 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.3597));
    ...
    float bodyAlpha = 0.3167 + 0.2850 * presence + 0.5577 * pattern + 0.1138 * (1.0 - deepTrans) + 0.2083 * emit;
    ...
    // perspective reach.
    float innerFog = clamp(deepField(rotA(spinAxis, time * 0.068068 + 2.6014) * (sdir * 2.3855), time) * 1.2694 + 0.1582, 0.0, 1.0);
    vec3 innerCol = mix(mix(baseCol, secCol, 0.55), deepStop, 0.9 * innerFog);
    rgb = mix(rgb, innerCol, innerFace * 0.2083);
    bodyAlpha *= 1.0 + innerFace * 0.1320;
    ...
    bodyAlpha *= smoothstep(0.0, 1.0795, sceneVDist - fragVDist);
