...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8365, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0382, abs(glR - 0.1177));
    float mid = clamp(glStars * (1.0 + glRing * 1.9064) + glRing * 0.4131, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6679 + 0.5406 * texDetail) + 0.2591 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.5739 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5121), 0.0, 1.0);
    float emit = atlas.a * (0.5686 + 0.4573 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1538 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9154 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2849 + 0.2444 * presence + 0.5170 * pattern + 0.1550 * (1.0 - deepTrans) + 0.2112 * emit, 0.8459);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2418), 0.8459);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6988 + 0.3012 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
