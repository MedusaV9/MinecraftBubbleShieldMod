...
void main() {
    ...
    mid = clamp(mid * (0.6501 + 0.7291 * texDetail) + 0.1842 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5324), 0.0, 1.0);
    float emit = atlas.a * (0.6768 + 0.4380 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1045 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.6867 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3434 + 0.2883 * presence + 0.5816 * pattern + 0.0973 * (1.0 - deepTrans) + 0.1694 * emit, 0.8770);
