...
void main() {
    ...
    mid = clamp(mid * (0.6395 + 0.4724 * texDetail) + 0.2146 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5056), 0.0, 1.0);
    float emit = atlas.a * (0.6333 + 0.4175 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1423 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.7842 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4104 + 0.4339 * presence + 0.3385 * pattern + 0.1475 * (1.0 - deepTrans) + 0.1985 * emit, 0.8487);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3343), 0.8487);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7636 + 0.2364 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
