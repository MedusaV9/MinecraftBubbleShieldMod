...
void main() {
    ...
    mid = clamp(mid * (0.6045 + 0.5856 * texDetail) + 0.2082 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4944), 0.0, 1.0);
    float emit = atlas.a * (0.5622 + 0.3627 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0866 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.7996 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4244 + 0.4290 * presence + 0.3439 * pattern + 0.1081 * (1.0 - deepTrans) + 0.2064 * emit, 0.8888);
