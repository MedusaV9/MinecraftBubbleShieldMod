...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.050000, 0.025000) * time, time);
    deep += 0.4213 * deepField(duvBase + rimDir * 0.0655 + vec2(-0.076667, 0.037500) * time, time);
    deep += 0.1775 * deepField(duvBase + rimDir * 0.1310 + vec2(-0.103333, 0.052500) * time, time);
    deep += 0.0748 * deepField(duvBase + rimDir * 0.1965 + vec2(-0.131667, 0.065000) * time, time);
    deep = pow(clamp(deep * 0.5975, 0.0, 1.0), 1.5373);
    ...
    float pattern = clamp(0.4155 * deep + 0.8201 * mid + 0.8443 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7202 + pattern * 0.3078);
    vec3 rgb = vertexColor.rgb * (0.4556 + 0.8321 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1446 + 0.8229 * pattern, 0.0, 1.0);
