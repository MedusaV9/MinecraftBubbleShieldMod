...
void main() {
    ...
    mid = clamp(mid * (0.7268 + 0.6233 * texDetail) + 0.2027 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5446), 0.0, 1.0);
    float emit = atlas.a * (0.7182 + 0.3656 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1446 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.7707 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3943 + 0.5086 * presence + 0.4482 * pattern + 0.1136 * (1.0 - deepTrans) + 0.1928 * emit, 0.8706);
