...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.020833, -0.040000) * time, time);
    deep += 0.5596 * deepField(duvBase + rimDir * 0.0707 + vec2(-0.032500, -0.061667) * time, time);
    deep = pow(clamp(deep * 0.6412, 0.0, 1.0), 1.6643);
    ...
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.133333, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.140000), midPer)), midPer);
