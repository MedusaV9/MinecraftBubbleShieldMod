...
void main() {
    ...
    mid = clamp(mid * (0.6154 + 0.5719 * texDetail) + 0.2625 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5368), 0.0, 1.0);
    float emit = atlas.a * (0.7805 + 0.4244 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0975 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.9927 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2669 + 0.2941 * presence + 0.6002 * pattern + 0.0928 * (1.0 - deepTrans) + 0.2176 * emit, 0.8751);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3369), 0.8751);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7178 + 0.2822 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0852), 0.0, 1.0);
