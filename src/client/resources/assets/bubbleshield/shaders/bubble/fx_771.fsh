...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8669, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0321, abs(glR - 0.1134));
    float mid = clamp(glStars * (1.0 + glRing * 1.4128) + glRing * 0.3640, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6313 + 0.7227 * texDetail) + 0.3488 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8095 + 0.4656 * texDetail;
    rgb = satLift(rgb, 1.0950);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4256), 0.0, 1.0);
    float emit = atlas.a * (0.6138 + 0.2692 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1419 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7100 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2643 + 0.2360 * presence + 0.5804 * pattern + 0.0924 * (1.0 - deepTrans) + 0.2089 * emit, 0.9171);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3001), 0.9171);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6232 + 0.3768 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1256), 0.0, 1.0);
