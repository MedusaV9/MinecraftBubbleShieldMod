...
void main() {
    ...
    mid = clamp(mid * (0.6970 + 0.7019 * texDetail) + 0.2174 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4792), 0.0, 1.0);
    float emit = atlas.a * (0.5708 + 0.2771 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1405 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.9607 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4292 + 0.4169 * presence + 0.4395 * pattern + 0.1301 * (1.0 - deepTrans) + 0.2106 * emit, 0.9164);
