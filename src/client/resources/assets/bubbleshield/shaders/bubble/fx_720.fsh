...
void main() {
    ...
    mid = clamp(mid * (0.6652 + 0.4883 * texDetail) + 0.2054 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4303), 0.0, 1.0);
    float emit = atlas.a * (0.7047 + 0.4644 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1468 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.9267 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3342 + 0.2711 * presence + 0.3863 * pattern + 0.1404 * (1.0 - deepTrans) + 0.1239 * emit, 0.8722);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1998), 0.8722);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7633 + 0.2367 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
