...
void main() {
    ...
    mid = clamp(mid * (0.6679 + 0.6674 * texDetail) + 0.2411 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4591), 0.0, 1.0);
    float emit = atlas.a * (0.5540 + 0.3183 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1022 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.6593 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3260 + 0.4120 * presence + 0.3116 * pattern + 0.0898 * (1.0 - deepTrans) + 0.1532 * emit, 0.8992);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2876), 0.8992);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6572 + 0.3428 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
