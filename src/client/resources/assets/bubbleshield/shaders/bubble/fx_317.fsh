...
void main() {
    ...
    mid = clamp(mid * (0.7394 + 0.4848 * texDetail) + 0.2559 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3843), 0.0, 1.0);
    float emit = atlas.a * (0.6291 + 0.2550 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1512 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.6567 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3603 + 0.4320 * presence + 0.3733 * pattern + 0.1059 * (1.0 - deepTrans) + 0.1871 * emit, 0.8554);
