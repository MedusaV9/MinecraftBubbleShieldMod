...
// stack: deep=ridge x3 taps | mid=circuit+warp2+scroll | rim=graze_sparkle | flourish=echo | tex=circuit
// fbm: turb x3 octaves | seed 6075e36381412db2
// v8 realism: refr 0.6719+0.9075*fres/dist (cap 0.1508) | energy 0.4077+0.5061*p | flow 0.080000 c/s | soft 0.5409m
...
void main() {
    ...
    float refrAmp = min((0.6719 + 0.9075 * fresnel) / max(length(worldPos), 1.0), 0.1508);
