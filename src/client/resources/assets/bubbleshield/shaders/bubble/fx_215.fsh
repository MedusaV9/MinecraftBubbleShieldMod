...
void main() {
    ...
    mid = clamp(mid * (0.6520 + 0.6669 * texDetail) + 0.2915 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3892), 0.0, 1.0);
    float emit = atlas.a * (0.6759 + 0.2660 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1163 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.7069 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4331 + 0.4646 * presence + 0.4515 * pattern + 0.0938 * (1.0 - deepTrans) + 0.1910 * emit, 0.9383);
