...
void main() {
    ...
    mid = clamp(mid * (0.6907 + 0.5895 * texDetail) + 0.2535 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4873), 0.0, 1.0);
    float emit = atlas.a * (0.6128 + 0.3805 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0890 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.6721 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3822 + 0.5063 * presence + 0.4892 * pattern + 0.0885 * (1.0 - deepTrans) + 0.1748 * emit, 0.9088);
