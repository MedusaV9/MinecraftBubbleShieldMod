...
void main() {
    ...
    mid = clamp(mid * (0.7310 + 0.4912 * texDetail) + 0.2476 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.8789 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3785), 0.0, 1.0);
    float emit = atlas.a * (0.6488 + 0.3337 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1295 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.9514 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3829 + 0.4082 * presence + 0.3287 * pattern + 0.1410 * (1.0 - deepTrans) + 0.1733 * emit, 0.8965);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1934), 0.8965);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6593 + 0.3407 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0815), 0.0, 1.0);
