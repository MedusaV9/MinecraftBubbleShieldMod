...
void main() {
    ...
    mid = clamp(mid * (0.6371 + 0.4741 * texDetail) + 0.3174 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5103), 0.0, 1.0);
    float emit = atlas.a * (0.5648 + 0.2511 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1193 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9593 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3311 + 0.3924 * presence + 0.4914 * pattern + 0.0965 * (1.0 - deepTrans) + 0.1690 * emit, 0.9160);
