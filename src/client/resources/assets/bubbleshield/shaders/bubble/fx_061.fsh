...
// stack: deep=caustic x2 taps | mid=waves+none+rotate | rim=graze_film | flourish=echo
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.035000, -0.006667) * time, time);
    deep += 0.3880 * deepField(duvBase + rimDir * 0.0457 + vec2(0.054167, -0.010000) * time, time);
    deep = pow(clamp(deep * 0.7205, 0.0, 1.0), 1.5906);
    ...
    float pattern = clamp(0.4494 * deep + 0.8284 * mid + 0.5458 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9524 + pattern * 0.6637);
    vec3 rgb = vertexColor.rgb * (0.5362 + 0.8266 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1790 + 0.6415 * pattern, 0.0, 1.0);
