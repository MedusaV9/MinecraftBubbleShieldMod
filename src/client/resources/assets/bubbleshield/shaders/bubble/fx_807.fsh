...
void main() {
    ...
    mid = clamp(mid * (0.7405 + 0.7350 * texDetail) + 0.1996 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4762), 0.0, 1.0);
    float emit = atlas.a * (0.5886 + 0.3600 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0829 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.9613 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3666 + 0.3664 * presence + 0.4356 * pattern + 0.1322 * (1.0 - deepTrans) + 0.1990 * emit, 0.8934);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2706), 0.8934);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7018 + 0.2982 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
