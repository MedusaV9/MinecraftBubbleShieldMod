...
void main() {
    ...
    mid = clamp(mid * (0.6818 + 0.5134 * texDetail) + 0.2563 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4633), 0.0, 1.0);
    float emit = atlas.a * (0.6652 + 0.2632 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1728 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.8493 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4108 + 0.4494 * presence + 0.4578 * pattern + 0.1001 * (1.0 - deepTrans) + 0.1512 * emit, 0.9084);
