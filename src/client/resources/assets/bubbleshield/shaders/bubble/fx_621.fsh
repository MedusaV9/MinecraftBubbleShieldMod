...
void main() {
    ...
    mid = clamp(mid * (0.6068 + 0.7083 * texDetail) + 0.2946 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4250), 0.0, 1.0);
    float emit = atlas.a * (0.7349 + 0.2720 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1290 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.8770 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3923 + 0.4155 * presence + 0.3559 * pattern + 0.0922 * (1.0 - deepTrans) + 0.1979 * emit, 0.8775);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2496), 0.8775);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6935 + 0.3065 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1240), 0.0, 1.0);
