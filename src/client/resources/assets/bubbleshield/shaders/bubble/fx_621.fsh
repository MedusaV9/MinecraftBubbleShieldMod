...
// stack: deep=voro x4 taps | mid=raymarchfog+warp2+scroll | rim=lat | flourish=shimmer
...
void main() {
    ...
    float mid = clamp(rfLight * 0.9510 + (1.0 - rfTrans) * 0.2324, 0.0, 1.15);
    ...
    float alpha = vertexColor.a * min(0.0679 + 0.3555 * presence + 0.3559 * pattern + 0.0922 * (1.0 - deepTrans), 0.8175);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2496), 0.8175);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4581 + 0.5419 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
