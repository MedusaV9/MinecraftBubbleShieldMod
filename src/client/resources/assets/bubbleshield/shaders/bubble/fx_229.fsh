...
void main() {
    ...
    mid = clamp(mid * (0.7210 + 0.7244 * texDetail) + 0.2697 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3581), 0.0, 1.0);
    float emit = atlas.a * (0.6526 + 0.2850 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1003 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.7887 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4004 + 0.4656 * presence + 0.4621 * pattern + 0.1126 * (1.0 - deepTrans) + 0.1823 * emit, 0.8455);
