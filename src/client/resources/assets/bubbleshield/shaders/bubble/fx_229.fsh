...
// stack: deep=fbm x4 taps | mid=rings+curl+rotate | rim=graze_sparkle | flourish=swirl | tex=ridged
// fbm: turb x6 octaves | seed 686d59b89dd92a08
// v8 realism: refr 0.5008+0.6340*fres/dist (cap 0.1091) | energy 0.3420+0.5134*p | flow 0.059167 c/s | soft 0.6987m
...
void main() {
    ...
    float refrAmp = min((0.5008 + 0.6340 * fresnel) / max(length(worldPos), 1.0), 0.1091);
    ...
    float emit = atlas.a * (0.3616 + 0.4920 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1003 * (0.5 + 0.5 * sin(time * 0.109956))) * atlasPoleW;
