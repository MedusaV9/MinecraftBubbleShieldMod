...
void main() {
    ...
    mid = clamp(mid * (0.7104 + 0.4987 * texDetail) + 0.2350 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5073), 0.0, 1.0);
    float emit = atlas.a * (0.6115 + 0.2518 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1095 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.6535 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3728 + 0.3969 * presence + 0.3794 * pattern + 0.0881 * (1.0 - deepTrans) + 0.1828 * emit, 0.9183);
