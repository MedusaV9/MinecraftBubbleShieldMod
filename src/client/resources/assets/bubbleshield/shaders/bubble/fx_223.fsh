...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6896, 0.0, 1.0);
    ...
    deepCol += 0.4043 * dp * mix(baseCol, deepStop, 0.2968);
    deepPat += 0.4043 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 3.7302) + par * 0.0902, time);
    deepCol += 0.1634 * dp * mix(baseCol, deepStop, 0.5937);
    deepPat += 0.1634 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.5101) + par * 0.1354, time);
    deepCol += 0.0661 * dp * mix(baseCol, deepStop, 0.8905);
    deepPat += 0.0661 * dp;
    deepCol *= 0.7521;
    deepPat = pow(clamp(deepPat * 0.6121, 0.0, 1.0), 1.3817);
    ...
    rgb += hotStop * 0.2984 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0551 + 0.2569 * presence + 0.3794 * pattern, 0.8583);
