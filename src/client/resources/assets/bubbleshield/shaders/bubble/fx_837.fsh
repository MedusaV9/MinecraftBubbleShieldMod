...
void main() {
    ...
    mid = clamp(mid * (0.6608 + 0.6340 * texDetail) + 0.3003 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5035), 0.0, 1.0);
    float emit = atlas.a * (0.5588 + 0.3848 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1013 * (0.5 + 0.5 * sin(time * 0.193732)));
    ...
    float alpha = vertexColor.a * min(0.3075 + 0.2501 * presence + 0.6133 * pattern + 0.1562 * (1.0 - deepTrans) + 0.1856 * emit, 0.8636);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2025), 0.8636);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7684 + 0.2316 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
