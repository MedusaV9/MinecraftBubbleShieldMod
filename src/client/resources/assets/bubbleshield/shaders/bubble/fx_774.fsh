...
void main() {
    ...
    mid = clamp(mid * (0.6135 + 0.4613 * texDetail) + 0.2046 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5400), 0.0, 1.0);
    float emit = atlas.a * (0.6981 + 0.3954 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1598 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9950 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3733 + 0.4483 * presence + 0.3431 * pattern + 0.0995 * (1.0 - deepTrans) + 0.1444 * emit, 0.8893);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3318), 0.8893);
    // [layer:v5:ghostalpha]
