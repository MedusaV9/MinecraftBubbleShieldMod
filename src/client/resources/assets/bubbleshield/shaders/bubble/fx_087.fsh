...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1522) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1522);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.000000, -0.050000) * time, time);
    deep += 0.4009 * deepField(duvBase + rimDir * 0.0341 + vec2(0.000000, -0.077500) * time, time);
    deep += 0.1607 * deepField(duvBase + rimDir * 0.0681 + vec2(0.000000, -0.105000) * time, time);
    deep = pow(clamp(deep * 0.6404, 0.0, 1.0), 1.5467);
    ...
    vec2 wuv = auv;
    float n = fbm2(wuv + vec2(0.0, time * 0.449167), midPer);
