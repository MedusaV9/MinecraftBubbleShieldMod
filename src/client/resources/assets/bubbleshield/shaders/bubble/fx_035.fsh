...
void main() {
    ...
    mid = clamp(mid * (0.6140 + 0.5937 * texDetail) + 0.2632 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.8994 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4179), 0.0, 1.0);
    float emit = atlas.a * (0.6545 + 0.3898 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1644 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.9402 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3416 + 0.4438 * presence + 0.4414 * pattern + 0.1546 * (1.0 - deepTrans) + 0.1346 * emit, 0.8682);
