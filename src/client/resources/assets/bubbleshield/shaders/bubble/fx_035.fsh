...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7530, 0.0, 1.0);
    ...
    deepCol += 0.4611 * dp * mix(baseCol, deepStop, 0.2690);
    deepPat += 0.4611 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 5.3562) + par * 0.1124, time);
    deepCol += 0.2126 * dp * mix(baseCol, deepStop, 0.5380);
    deepPat += 0.2126 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.6272) + par * 0.1686, time);
    deepCol += 0.0980 * dp * mix(baseCol, deepStop, 0.8070);
    deepPat += 0.0980 * dp;
    deepCol *= 0.5751;
    deepPat = pow(clamp(deepPat * 0.5644, 0.0, 1.0), 1.6763);
    ...
    rgb += hotStop * 0.2735 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0446 + 0.3038 * presence + 0.4414 * pattern, 0.8082);
