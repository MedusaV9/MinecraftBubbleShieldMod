...
void main() {
    ...
    deep += 0.5624 * deepField(duvBase + rimDir * 0.0391 + vec2(-0.008333, 0.040000) * time, time);
    deep += 0.3162 * deepField(duvBase + rimDir * 0.0782 + vec2(-0.010000, 0.055000) * time, time);
    deep = pow(clamp(deep * 0.5323, 0.0, 1.0), 1.7073);
    ...
    float pattern = clamp(0.4411 * deep + 0.7926 * mid + 0.8185 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8781 + pattern * 0.4104);
    vec3 rgb = vertexColor.rgb * (0.5479 + 0.9446 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1634 + 0.6175 * pattern, 0.0, 1.0);
