...
void main() {
    ...
    mid = clamp(mid * (0.6781 + 0.6115 * texDetail) + 0.2015 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4711), 0.0, 1.0);
    float emit = atlas.a * (0.7211 + 0.4964 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1656 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.9827 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2680 + 0.2924 * presence + 0.4850 * pattern + 0.1157 * (1.0 - deepTrans) + 0.1825 * emit, 0.8746);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2977), 0.8746);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7583 + 0.2417 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
