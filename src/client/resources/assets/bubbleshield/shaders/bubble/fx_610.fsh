...
void main() {
    ...
    mid = clamp(mid * (0.7242 + 0.4794 * texDetail) + 0.3476 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3957), 0.0, 1.0);
    float emit = atlas.a * (0.5755 + 0.3881 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1281 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.9886 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3235 + 0.2822 * presence + 0.5934 * pattern + 0.1254 * (1.0 - deepTrans) + 0.1432 * emit, 0.8945);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2896), 0.8945);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7700 + 0.2300 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0887), 0.0, 1.0);
