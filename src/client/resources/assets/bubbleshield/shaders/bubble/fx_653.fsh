...
void main() {
    ...
    mid = clamp(mid * (0.6847 + 0.5546 * texDetail) + 0.1832 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5393), 0.0, 1.0);
    float emit = atlas.a * (0.7921 + 0.4924 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0927 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.9759 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3602 + 0.3215 * presence + 0.5258 * pattern + 0.1197 * (1.0 - deepTrans) + 0.1548 * emit, 0.9015);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1792), 0.9015);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6426 + 0.3574 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0895), 0.0, 1.0);
