...
void main() {
    ...
    mid = clamp(mid * (0.7440 + 0.6080 * texDetail) + 0.2682 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3507), 0.0, 1.0);
    float emit = atlas.a * (0.7308 + 0.4068 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1434 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.7558 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3593 + 0.4511 * presence + 0.4324 * pattern + 0.1286 * (1.0 - deepTrans) + 0.1238 * emit, 0.8869);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3211), 0.8869);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6549 + 0.3451 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
