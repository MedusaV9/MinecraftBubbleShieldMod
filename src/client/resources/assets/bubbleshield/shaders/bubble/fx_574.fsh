...
void main() {
    ...
    mid = clamp(mid * (0.6075 + 0.6890 * texDetail) + 0.3162 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5479), 0.0, 1.0);
    float emit = atlas.a * (0.5542 + 0.4728 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1144 * (0.5 + 0.5 * sin(time * 0.261799)));
    ...
    float alpha = vertexColor.a * min(0.3658 + 0.4356 * presence + 0.3210 * pattern + 0.1530 * (1.0 - deepTrans) + 0.1354 * emit, 0.9180);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1742), 0.9180);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6537 + 0.3463 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0713), 0.0, 1.0);
