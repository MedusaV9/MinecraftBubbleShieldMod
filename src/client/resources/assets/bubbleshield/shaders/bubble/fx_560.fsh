...
void main() {
    ...
    mid = clamp(mid * (0.7032 + 0.6228 * texDetail) + 0.2048 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7242 + 0.2622 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1410 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.9482 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3265 + 0.2958 * presence + 0.5099 * pattern + 0.0982 * (1.0 - deepTrans) + 0.1800 * emit, 0.8288);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3315), 0.8288);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6876 + 0.3124 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0900), 0.0, 1.0);
