...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6860, 0.0, 1.0);
    ...
    deepCol += 0.4404 * dp * mix(baseCol, deepStop, 0.3780);
    deepPat += 0.4404 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 3.9137) + par * 0.1292, time);
    deepCol += 0.1939 * dp * mix(baseCol, deepStop, 0.7560);
    deepPat += 0.1939 * dp;
    deepCol *= 0.6657;
    deepPat = pow(clamp(deepPat * 0.6119, 0.0, 1.0), 1.2510);
    ...
    float midCover = clamp(0.8837 * mid + 0.6203 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.3461);
    rgb += hotStop * 0.2683 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0684 + 0.3221 * presence + 0.4075 * pattern, 0.8274);
