...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6229, 0.0, 1.0);
    ...
    deepCol += 0.5761 * dp * mix(baseCol, deepStop, 0.2093);
    deepPat += 0.5761 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.4457) + par * 0.1165, time);
    deepCol += 0.3319 * dp * mix(baseCol, deepStop, 0.4186);
    deepPat += 0.3319 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 5.6211) + par * 0.1747, time);
    deepCol += 0.1912 * dp * mix(baseCol, deepStop, 0.6279);
    deepPat += 0.1912 * dp;
    deepCol *= 0.5187;
    deepPat = pow(clamp(deepPat * 0.4764, 0.0, 1.0), 1.6706);
    ...
    float curve1 = 0.5 + 0.1123 * sin(baseUV.x * 6.2831853 * 2.0000 + time * 0.371755) + 0.0935 * sin(baseUV.x * 6.2831853 * 6.0000 - time * 0.298451);
    float slope1 = cos(baseUV.x * 6.2831853 * 2.0000 + time * 0.371755);
    float band1 = invsmooth(0.005, 0.0723, abs(baseUV.y - curve1));
    float curve2 = 0.5 + 0.1328 * sin(baseUV.x * 6.2831853 * 3.0000 - time * 0.361283 + 2.1) + 0.0776;
    float band2 = invsmooth(0.005, 0.0584, abs(baseUV.y - curve2));
    float foldGlow = 0.6 + 0.4 * slope1 * slope1;
    float shimmer = fbm2(wuv, midPer) * 0.2102;
    ...
    rgb = satLift(rgb, 1.2245);
    rgb += hotStop * 0.3497 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0747 + 0.3070 * presence + 0.4254 * pattern, 0.8292);
