...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6461, 0.0, 1.0);
    ...
    deepCol += 0.5766 * dp * mix(baseCol, deepStop, 0.3116);
    deepPat += 0.5766 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.0532) + par * 0.0744, time);
    deepCol += 0.3325 * dp * mix(baseCol, deepStop, 0.6233);
    deepPat += 0.3325 * dp;
    deepCol *= 0.5882;
    deepPat = pow(clamp(deepPat * 0.5238, 0.0, 1.0), 1.5447);
    ...
    vec3 chromeN = normalize(vec3(chromeX - chromeBase, chromeY - chromeBase, chromeZ - chromeBase) * 2.2944 + sdir);
    float env1 = 0.5 + 0.5 * sin(dot(chromeN, vec3(0.0299, 0.6143, -0.4944)) * 8.6177 + time * 0.350811);
    float env2 = 0.5 + 0.5 * sin(dot(chromeN, vec3(0.7237, -0.8050, 0.6036)) * 4.1233);
    float mid = pow(clamp(max(env1, env2 * 0.85), 0.0, 1.0), 3.8813);
    ...
    float midCover = clamp(0.9765 * mid + 0.6400 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.2051);
    rgb += hotStop * 0.2476 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0484 + 0.2811 * presence + 0.3352 * pattern, 0.8500);
