...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7874, 0.0, 1.0);
    ...
    deepCol += 0.5451 * dp * mix(baseCol, deepStop, 0.2597);
    deepPat += 0.5451 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 3.4289) + par * 0.1092, time);
    deepCol += 0.2972 * dp * mix(baseCol, deepStop, 0.5193);
    deepPat += 0.2972 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.1435) + par * 0.1638, time);
    deepCol += 0.1620 * dp * mix(baseCol, deepStop, 0.7790);
    deepPat += 0.1620 * dp;
    deepCol *= 0.5787;
    deepPat = pow(clamp(deepPat * 0.4989, 0.0, 1.0), 1.7934);
    ...
    float midCover = clamp(0.9022 * mid + 0.5504 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.2775);
    rgb += hotStop * 0.2290 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0488 + 0.2449 * presence + 0.3817 * pattern, 0.7966);
