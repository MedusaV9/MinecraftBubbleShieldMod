...
// stack: deep=caustic x3 taps | mid=kaleido+warp2+pulse | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.025000, 0.020000) * time, time);
    deep += 0.5451 * deepField(duvBase + rimDir * 0.0546 + vec2(-0.038333, 0.032500) * time, time);
    deep += 0.2972 * deepField(duvBase + rimDir * 0.1092 + vec2(-0.053333, 0.042500) * time, time);
    deep = pow(clamp(deep * 0.5428, 0.0, 1.0), 1.7934);
    ...
    float pattern = clamp(0.4332 * deep + 0.8721 * mid + 0.7282 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6568 + pattern * 0.4413);
    vec3 rgb = vertexColor.rgb * (0.4350 + 0.7032 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1744 + 0.7836 * pattern, 0.0, 1.0);
