...
void main() {
    ...
    mid = clamp(mid * (0.7341 + 0.6099 * texDetail) + 0.2283 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5416), 0.0, 1.0);
    float emit = atlas.a * (0.7897 + 0.3979 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1112 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.9832 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3308 + 0.3352 * presence + 0.5908 * pattern + 0.0937 * (1.0 - deepTrans) + 0.1878 * emit, 0.9148);
