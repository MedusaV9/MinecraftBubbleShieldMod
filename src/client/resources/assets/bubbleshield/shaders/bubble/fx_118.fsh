...
void main() {
    ...
    mid = clamp(mid * (0.6921 + 0.6469 * texDetail) + 0.2315 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4416), 0.0, 1.0);
    float emit = atlas.a * (0.6491 + 0.4742 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1346 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.9740 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3487 + 0.4781 * presence + 0.4442 * pattern + 0.1209 * (1.0 - deepTrans) + 0.1750 * emit, 0.8839);
