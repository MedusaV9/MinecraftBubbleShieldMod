...
void main() {
    ...
    mid = clamp(mid * (0.6298 + 0.6129 * texDetail) + 0.3318 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.5384 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4261), 0.0, 1.0);
    float emit = atlas.a * (0.5937 + 0.4820 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1208 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.8434 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3490 + 0.4838 * presence + 0.3529 * pattern + 0.1096 * (1.0 - deepTrans) + 0.2070 * emit, 0.9074);
