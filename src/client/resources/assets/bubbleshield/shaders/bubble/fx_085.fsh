...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7409, 0.0, 1.0);
    ...
    deepCol += 0.4095 * dp * mix(baseCol, deepStop, 0.1854);
    deepPat += 0.4095 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 3.5319) + par * 0.0802, time);
    deepCol += 0.1677 * dp * mix(baseCol, deepStop, 0.3708);
    deepPat += 0.1677 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.4472) + par * 0.1203, time);
    deepCol += 0.0687 * dp * mix(baseCol, deepStop, 0.5562);
    deepPat += 0.0687 * dp;
    deepCol *= 0.6406;
    deepPat = pow(clamp(deepPat * 0.6076, 0.0, 1.0), 1.1163);
    ...
    float alpha = vertexColor.a * min(0.0522 + 0.2409 * presence + 0.3648 * pattern, 0.8444);
