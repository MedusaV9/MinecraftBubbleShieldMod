...
void main() {
    ...
    mid = clamp(mid * (0.6970 + 0.7272 * texDetail) + 0.2408 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4114), 0.0, 1.0);
    float emit = atlas.a * (0.7209 + 0.4996 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1697 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.9200 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3642 + 0.3809 * presence + 0.3648 * pattern + 0.0842 * (1.0 - deepTrans) + 0.1463 * emit, 0.9044);
