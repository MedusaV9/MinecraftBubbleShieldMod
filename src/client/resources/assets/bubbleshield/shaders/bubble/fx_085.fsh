...
// stack: deep=fbm x4 taps | mid=circuit+curl+rotate | rim=lat | flourish=echo | tex=circuit
// fbm: turb x3 octaves | seed abb8f3b6847ce48a
// v8 realism: refr 0.6782+0.8896*fres/dist (cap 0.1575) | energy 0.3191+0.4185*p | flow 0.117500 c/s | soft 0.7234m
...
void main() {
    ...
    float refrAmp = min((0.6782 + 0.8896 * fresnel) / max(length(worldPos), 1.0), 0.1575);
