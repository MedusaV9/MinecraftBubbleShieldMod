...
// stack: deep=fbm x2 taps | mid=lightning+curl+flicker | rim=lat | flourish=swirl
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.031667, -0.015000) * time, time);
    deep += 0.4727 * deepField(duvBase + rimDir * 0.0349 + vec2(0.048333, -0.022500) * time, time);
    deep = pow(clamp(deep * 0.6790, 0.0, 1.0), 1.4308);
    ...
    float n = fbm2(wuv + vec2(0.0, time * 0.583333), midPer);
    ...
    float pattern = clamp(0.4653 * deep + 0.9945 * mid + 0.5467 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.4445 + pattern * 0.3252);
    vec3 rgb = vertexColor.rgb * (0.4405 + 0.9359 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2379 + 0.8116 * pattern, 0.0, 1.0);
