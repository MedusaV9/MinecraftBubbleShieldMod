...
// stack: deep=ridge x4 taps | mid=stainedglass+curl+scroll | rim=lat | flourish=shimmer
...
void main() {
    ...
    float alpha = vertexColor.a * min(0.0442 + 0.3932 * presence + 0.4022 * pattern + 0.1310 * (1.0 - deepTrans), 0.8451);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3085), 0.8451);
    // [layer:v5:ghostalpha]
