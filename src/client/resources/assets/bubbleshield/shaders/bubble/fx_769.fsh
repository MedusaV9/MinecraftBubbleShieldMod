...
void main() {
    ...
    mid = clamp(mid * (0.6150 + 0.5072 * texDetail) + 0.3110 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.6294 * rimLine, 0.0, 1.4);
    ...
    float emit = atlas.a * (0.6222 + 0.4114 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1617 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.7149 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3671 + 0.5332 * presence + 0.4022 * pattern + 0.1310 * (1.0 - deepTrans) + 0.1853 * emit, 0.9051);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3085), 0.9051);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1297), 0.0, 1.0);
