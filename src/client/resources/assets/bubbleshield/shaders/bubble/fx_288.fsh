...
// v8 realism: refr 1.6145+2.2730*fres/dist (cap 0.1899) | energy 0.2206+0.4494*p | flow 0.100833 c/s | soft 0.7003m
...
void main() {
    ...
    rim = clamp(rim + 0.6435 * rimLine + 0.4821 * fresnel, 0.0, 1.4);
    ...
    float motifEnv = 0.15 + 0.85 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.743510)) * (0.5 + 0.5 * sin(time * 0.874410 + 1.7)));
    float motifBand = smoothstep(0.06, 0.22, baseUV.y) * smoothstep(0.06, 0.22, 1.0 - baseUV.y);
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.y * 3.0000 - 1.0 * time * 0.115833 + 0.2003)), 12.5154) * motifBand;
    ...
    float refrAmp = min((1.6145 + 2.2730 * fresnel) / max(length(worldPos), 1.0), 0.1899);
    ...
    float energyW = clamp(0.2206 + 0.4494 * clamp(pattern, 0.0, 1.0) + 0.1621 * fresnel, 0.0, 0.8688);
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4444) * 0.5685;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3706 * (1.0 - 0.4532 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.3673 + 0.4638 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1256 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
    rgb += emit * 0.9725 * (1.0 - 0.7247 * baseLuma) * emitCol;
