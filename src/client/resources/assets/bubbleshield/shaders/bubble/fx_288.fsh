...
// stack: deep=fbm x4 taps | mid=kaleido+curl+pulse | rim=graze | flourish=swirl | tex=mandala | motif=scanx4+sparklegate
// fbm: turb x5 octaves | seed 554ecc35816457d8
// v8 realism: refr 1.8050+2.5412*fres/dist (cap 0.1899) | energy 0.1725+0.4494*p | flow 0.100833 c/s | soft 0.7003m
...
void main() {
    ...
    float lumaCap = max(0.4545, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.7076);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, 0.1586), 0.0, 1.0), 1.3932) * 0.7882, 0.0, 1.0);
    ...
    float deepBreath = 1.0 + 0.0179 * sin(time * 1.130973);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * (2.3771 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.5263 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * (3.3464 * deepBreath)) + par * 0.0451, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.3761), deepStop, 0.1379);
    deepTrans *= 1.0 - 0.5263 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * (4.3158 * deepBreath)) + par * 0.0901, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.7521), deepStop, 0.2758);
    deepTrans *= 1.0 - 0.5263 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * (5.2851 * deepBreath)) + par * 0.1352, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4137);
    deepTrans *= 1.0 - 0.5263 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.8440;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0530, 0.0, 1.0), 1.0588);
    ...
    vec2 texUV = wuv * 3.0000 + vec2(-0.022500, -0.014167) * time;
    ...
    vec3 hgtW = vec3(0.3528, 0.3892, 0.2580);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * 2.6145;
    ...
    float texDetail = clamp(atlas.r * 0.4500 + atlas.g * 0.4963 + atlas.b * 0.3291, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.7076 + 0.5904 * texDetail, atlasPoleW) + 0.1986 * texDetail * (0.2441 + 0.7559 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.5192 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.4825);
    ...
    // thickness rho = 0.1038 (crystalline material group) on the unit
    ...
    float chordDisc = 0.8962 * 0.8962 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.8875);
    float chordN = min(chord / 0.1038, 3.0);
    ...
    rim = clamp(rim + 0.6435 * rimLine + 0.7085 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1310 * pow(clamp(fbm2(wuv + vec2(-time * 0.112500, time * 0.070000), midPer), 0.0, 1.0), 2.0);
    float grain = 0.0754 * (cellHash(floor(wuv * 28.0000) + vec2(floor(time * 2.0), 0.0), 140.0000) - 0.5);
    ...
    float motifEnv = 0.48 + 0.52 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.743510)) * (0.5 + 0.5 * sin(time * 0.874410 + 1.7)));
    float motifBand = smoothstep(0.06, 0.22, baseUV.y) * smoothstep(0.06, 0.22, 1.0 - baseUV.y);
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.y * 3.0000 - 1.0 * time * 0.115833 + 0.2003)), 8.6350) * motifBand;
    ...
    float pattern = clamp(0.4059 * deepPat * mix(1.0, chordN, 0.1745) + 0.9270 * mid + 0.6585 * rim + flourish + grain + 0.1045 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.9351 + rim * 0.2053, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(1.1072 * mid + 0.4182 * rim, 0.0, 1.0);
    ...
    float texBody = 0.8438 + 0.3817 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4534 + 0.5466 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.3827);
    ...
    vec3 softHot = 1.0 - exp(-2.4366 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.3151 * smoothstep(0.55, 1.10, pattern));
    ...
    rgb *= (1.0 - 0.3127 * washHi) * (1.0 - 0.3437 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.2491, 0.7743, 0.5818);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.3746) / 1.3746, 0.0, 1.0);
    rgb *= 0.6590 + 0.5687 * keyDiff * (1.0 - 0.4714 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 38.2705);
    rgb += hotStop * (keySpec * 0.6647 * (1.0 - 0.6440 * washHi));
    ...
    float refrAmp = min((1.8050 + 2.5412 * fresnel) / max(length(worldPos), 1.0), 0.1899);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.0313) * refrAmp;
    ...
    refracted *= exp(-0.5102 * chordN * (1.0 - baseCol));
    ...
    float energyW = clamp(0.1725 + 0.4494 * clamp(pattern, 0.0, 1.0) + 0.2524 * fresnel, 0.0, 0.8688);
    ...
    rgb += fresnel * hotStop * (0.9316 * (1.0 - 0.6823 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4444) * 0.9209;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3706 * max(0.0, 1.0 - 0.4532 * baseLuma - 0.5029 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3673 + 0.4638 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1256 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
    rgb += emit * 0.9725 * (1.0 - 0.7247 * baseLuma) * (1.0 - 0.5313 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.1476));
    ...
    float bodyAlpha = 0.4643 + 0.5522 * presence + 0.4236 * pattern + 0.1284 * (1.0 - deepTrans) + 0.2180 * emit;
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 0.9681 + hgtC * 0.4121, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.1862);
    bodyAlpha *= 1.0 + innerFace * 0.1536;
    ...
    bodyAlpha *= smoothstep(0.0, 0.7003, sceneVDist - fragVDist);
