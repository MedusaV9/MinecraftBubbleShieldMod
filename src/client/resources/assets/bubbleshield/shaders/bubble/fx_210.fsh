...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6007, 0.0, 1.0);
    ...
    deepCol += 0.4484 * dp * mix(baseCol, deepStop, 0.2515);
    deepPat += 0.4484 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.2114) + par * 0.1124, time);
    deepCol += 0.2010 * dp * mix(baseCol, deepStop, 0.5030);
    deepPat += 0.2010 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 5.1051) + par * 0.1687, time);
    deepCol += 0.0901 * dp * mix(baseCol, deepStop, 0.7544);
    deepPat += 0.0901 * dp;
    deepCol *= 0.5794;
    deepPat = pow(clamp(deepPat * 0.5749, 0.0, 1.0), 1.0821);
    ...
    rgb += hotStop * 0.1918 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0470 + 0.3191 * presence + 0.3379 * pattern, 0.8366);
