...
// stack: deep=caustic x4 taps | mid=curlsmoke+warp2+rotate | rim=graze_sparkle | flourish=glint | tex=marble_ink
// fbm: turb x5 octaves | seed 1d0af95ab48fe146
// v8 realism: refr 0.3578+0.5221*fres/dist (cap 0.1373) | energy 0.4599+0.4839*p | flow 0.061667 c/s | soft 1.2069m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.1068 * atlasPoleW));
    ...
    float refrAmp = min((0.3578 + 0.5221 * fresnel) / max(length(worldPos), 1.0), 0.1373);
