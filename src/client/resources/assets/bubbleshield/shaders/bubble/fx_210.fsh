...
void main() {
    ...
    mid = clamp(mid * (0.6657 + 0.5243 * texDetail) + 0.3436 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4189), 0.0, 1.0);
    float emit = atlas.a * (0.6399 + 0.3019 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1158 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.6651 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3487 + 0.4591 * presence + 0.3379 * pattern + 0.0813 * (1.0 - deepTrans) + 0.2006 * emit, 0.8966);
