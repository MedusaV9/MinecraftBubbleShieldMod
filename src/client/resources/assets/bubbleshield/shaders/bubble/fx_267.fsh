...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.038333, -0.005000) * time, time);
    deep += 0.4710 * deepField(duvBase + rimDir * 0.0729 + vec2(0.060000, -0.007500) * time, time);
    deep += 0.2219 * deepField(duvBase + rimDir * 0.1458 + vec2(0.080000, -0.010000) * time, time);
    deep += 0.1045 * deepField(duvBase + rimDir * 0.2187 + vec2(0.101667, -0.012500) * time, time);
    deep = pow(clamp(deep * 0.5564, 0.0, 1.0), 1.4329);
