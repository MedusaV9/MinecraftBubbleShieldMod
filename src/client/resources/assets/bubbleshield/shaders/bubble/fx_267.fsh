...
void main() {
    ...
    mid = clamp(mid * (0.7318 + 0.4699 * texDetail) + 0.3130 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3818), 0.0, 1.0);
    float emit = atlas.a * (0.6397 + 0.2956 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1445 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.7239 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4119 + 0.4677 * presence + 0.3864 * pattern + 0.1250 * (1.0 - deepTrans) + 0.1271 * emit, 0.8599);
