...
// stack: deep=caustic x4 taps | mid=interference+warp2+flicker | rim=graze_sparkle | flourish=echo | tex=caustic_web
// fbm: turb x3 octaves | seed 7a7dd08c562eded5
// v8 realism: refr 1.0023+1.5839*fres/dist (cap 0.1058) | energy 0.2208+0.3699*p | flow 0.111667 c/s | soft 0.7101m
...
vec4 atlasTile(vec2 p) {
    return texture(Sampler0, (vec2(0.0000, 1.0000) + clamp(fract(p), 0.004, 0.996)) * 0.25);
}
...
void main() {
    ...
    float refrAmp = min((1.0023 + 1.5839 * fresnel) / max(length(worldPos), 1.0), 0.1058);
