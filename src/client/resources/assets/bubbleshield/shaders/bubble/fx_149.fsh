...
// stack: deep=caustic x3 taps | mid=moire+warp1+pulse | rim=lat | flourish=glint
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.041667, -0.045000) * time, time);
    deep += 0.4640 * deepField(duvBase + rimDir * 0.0529 + vec2(-0.065000, -0.070000) * time, time);
    deep += 0.2153 * deepField(duvBase + rimDir * 0.1058 + vec2(-0.088333, -0.092500) * time, time);
    deep = pow(clamp(deep * 0.5955, 0.0, 1.0), 1.2134);
    ...
    float pattern = clamp(0.4145 * deep + 0.9878 * mid + 0.8235 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9812 + pattern * 0.5866);
    vec3 rgb = vertexColor.rgb * (0.3883 + 0.8930 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1674 + 0.6103 * pattern, 0.0, 1.0);
