...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6484, 0.0, 1.0);
    ...
    deepCol += 0.4640 * dp * mix(baseCol, deepStop, 0.2735);
    deepPat += 0.4640 * dp;
    dp = deepField(rotA(spinAxis, time * 0.094248) * (sdir * 2.7568) + par * 0.1058, time);
    deepCol += 0.2153 * dp * mix(baseCol, deepStop, 0.5470);
    deepPat += 0.2153 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 3.3195) + par * 0.1588, time);
    deepCol += 0.0999 * dp * mix(baseCol, deepStop, 0.8205);
    deepPat += 0.0999 * dp;
    deepCol *= 0.5556;
    deepPat = pow(clamp(deepPat * 0.5620, 0.0, 1.0), 1.2134);
    ...
    rgb += hotStop * 0.1750 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0621 + 0.2593 * presence + 0.3509 * pattern, 0.8341);
