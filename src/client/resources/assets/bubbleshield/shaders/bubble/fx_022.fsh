...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7250, 0.0, 1.0);
    ...
    deepCol += 0.4115 * dp * mix(baseCol, deepStop, 0.2948);
    deepPat += 0.4115 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 3.7842) + par * 0.0957, time);
    deepCol += 0.1693 * dp * mix(baseCol, deepStop, 0.5897);
    deepPat += 0.1693 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.7783) + par * 0.1435, time);
    deepCol += 0.0697 * dp * mix(baseCol, deepStop, 0.8845);
    deepPat += 0.0697 * dp;
    deepCol *= 0.7115;
    deepPat = pow(clamp(deepPat * 0.6059, 0.0, 1.0), 1.6275);
    ...
    rgb += hotStop * 0.2508 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0607 + 0.3450 * presence + 0.3792 * pattern, 0.7837);
