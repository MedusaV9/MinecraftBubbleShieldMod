...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.392699);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.011667, -0.007500) * time, time);
    deep += 0.4115 * deepField(duvBase + rimDir * 0.0478 + vec2(-0.016667, -0.012500) * time, time);
    deep += 0.1693 * deepField(duvBase + rimDir * 0.0957 + vec2(-0.023333, -0.015000) * time, time);
    deep += 0.0697 * deepField(duvBase + rimDir * 0.1435 + vec2(-0.028333, -0.020000) * time, time);
    deep = pow(clamp(deep * 0.6059, 0.0, 1.0), 1.6275);
    ...
    float pattern = clamp(0.4163 * deep + 0.9378 * mid + 0.8012 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.4672 + pattern * 0.6801);
    vec3 rgb = vertexColor.rgb * (0.4531 + 0.9007 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1161 + 0.6010 * pattern, 0.0, 1.0);
