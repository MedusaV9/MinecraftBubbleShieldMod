...
void main() {
    ...
    mid = clamp(mid * (0.7230 + 0.6539 * texDetail) + 0.2118 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4573), 0.0, 1.0);
    float emit = atlas.a * (0.6588 + 0.3386 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0822 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.7807 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3894 + 0.4850 * presence + 0.3792 * pattern + 0.1229 * (1.0 - deepTrans) + 0.2078 * emit, 0.8437);
