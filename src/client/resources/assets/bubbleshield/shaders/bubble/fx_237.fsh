...
// stack: deep=caustic x4 taps | mid=rings+warp1+pulse | rim=graze | flourish=echo | tex=ridged
// fbm: ridged x3 octaves | seed 0b52770456e7110d
// v8 realism: refr 0.4505+0.4500*fres/dist (cap 0.1474) | energy 0.4188+0.4043*p | flow 0.050000 c/s | soft 1.0746m
...
void main() {
    ...
    float refrAmp = min((0.4505 + 0.4500 * fresnel) / max(length(worldPos), 1.0), 0.1474);
    ...
    float emit = atlas.a * (0.3968 + 0.4862 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1179 * (0.5 + 0.5 * sin(time * 0.193732))) * atlasPoleW;
