...
void main() {
    ...
    mid = clamp(mid * (0.6535 + 0.5404 * texDetail) + 0.3023 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4737), 0.0, 1.0);
    float emit = atlas.a * (0.7113 + 0.2802 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1179 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.9307 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3423 + 0.4805 * presence + 0.3861 * pattern + 0.0848 * (1.0 - deepTrans) + 0.1482 * emit, 0.8750);
