...
void main() {
    ...
    deepCol += 0.4528 * dp * mix(baseCol, deepStop, 0.2006);
    deepPat += 0.4528 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 4.1733) + par * 0.1384, time);
    deepCol += 0.2050 * dp * mix(baseCol, deepStop, 0.4012);
    deepPat += 0.2050 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.2548) + par * 0.2075, time);
    deepCol += 0.0928 * dp * mix(baseCol, deepStop, 0.6017);
    deepPat += 0.0928 * dp;
    deepCol *= 0.6655;
    deepPat = pow(clamp(deepPat * 0.5712, 0.0, 1.0), 1.4153);
    ...
    float alpha = vertexColor.a * min(0.0448 + 0.3405 * presence + 0.3861 * pattern, 0.8150);
