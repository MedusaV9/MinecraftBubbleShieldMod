...
void main() {
    ...
    mid = clamp(mid * (0.6757 + 0.6153 * texDetail) + 0.2958 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4611), 0.0, 1.0);
    float emit = atlas.a * (0.7648 + 0.4426 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1716 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.8612 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3674 + 0.4969 * presence + 0.3270 * pattern + 0.1181 * (1.0 - deepTrans) + 0.1251 * emit, 0.8860);
