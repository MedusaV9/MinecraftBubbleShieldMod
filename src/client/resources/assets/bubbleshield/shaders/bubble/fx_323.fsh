...
// stack: deep=ridge x3 taps | mid=nebula+warp2+pulse | rim=graze | flourish=swirl
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.041667, -0.012500) * time, time);
    deep += 0.4328 * deepField(duvBase + rimDir * 0.0648 + vec2(0.063333, -0.017500) * time, time);
    deep += 0.1873 * deepField(duvBase + rimDir * 0.1296 + vec2(0.086667, -0.025000) * time, time);
    deep = pow(clamp(deep * 0.6172, 0.0, 1.0), 1.0203);
    ...
    float neb = fbm2(wuv + 0.7568 * vec2(fbm2(wuv + vec2(time * 0.037500, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.023333), midPer)), midPer);
    float lanes = 1.0 - pow(clamp(1.0 - abs(2.0 * fbm2(wuv * 2.0 + vec2(4.2, 1.1), midPer * 2.0) - 1.0), 0.0, 1.0), 3.0) * 0.4366;
    ...
    float pattern = clamp(0.4117 * deep + 0.7191 * mid + 0.5921 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2098 + pattern * 0.6106);
    vec3 rgb = vertexColor.rgb * (0.3590 + 0.8112 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1112 + 0.7687 * pattern, 0.0, 1.0);
