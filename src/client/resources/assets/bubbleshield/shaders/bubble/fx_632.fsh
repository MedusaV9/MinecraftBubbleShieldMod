...
void main() {
    ...
    mid = clamp(mid * (0.6811 + 0.5498 * texDetail) + 0.2197 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4319), 0.0, 1.0);
    float emit = atlas.a * (0.5609 + 0.4131 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1633 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.8869 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3545 + 0.2689 * presence + 0.5172 * pattern + 0.1527 * (1.0 - deepTrans) + 0.1238 * emit, 0.8526);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1703), 0.8526);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7389 + 0.2611 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
