...
void main() {
    ...
    mid = clamp(mid * (0.7100 + 0.4698 * texDetail) + 0.3164 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4283), 0.0, 1.0);
    float emit = atlas.a * (0.6266 + 0.2991 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1750 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.8230 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3599 + 0.2453 * presence + 0.5667 * pattern + 0.1362 * (1.0 - deepTrans) + 0.1997 * emit, 0.8871);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2516), 0.8871);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7537 + 0.2463 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
