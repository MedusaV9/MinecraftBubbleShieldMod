...
void main() {
    ...
    mid = clamp(mid * (0.6842 + 0.5363 * texDetail) + 0.2510 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5210), 0.0, 1.0);
    float emit = atlas.a * (0.7786 + 0.4132 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1424 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7722 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4041 + 0.3527 * presence + 0.4143 * pattern + 0.1192 * (1.0 - deepTrans) + 0.1778 * emit, 0.8684);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2224), 0.8684);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6896 + 0.3104 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0642), 0.0, 1.0);
