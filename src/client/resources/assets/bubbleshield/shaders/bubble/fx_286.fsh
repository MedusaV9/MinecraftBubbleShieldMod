...
void main() {
    ...
    mid = clamp(mid * (0.6973 + 0.6760 * texDetail) + 0.2065 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4279), 0.0, 1.0);
    float emit = atlas.a * (0.7079 + 0.3260 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1544 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.9159 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3299 + 0.4322 * presence + 0.3357 * pattern + 0.1347 * (1.0 - deepTrans) + 0.1792 * emit, 0.8861);
