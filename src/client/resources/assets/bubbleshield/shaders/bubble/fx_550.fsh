...
void main() {
    ...
    mid = clamp(mid * (0.7107 + 0.7010 * texDetail) + 0.3470 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4578), 0.0, 1.0);
    float emit = atlas.a * (0.7452 + 0.2636 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0811 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.9926 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3065 + 0.2639 * presence + 0.5875 * pattern + 0.1256 * (1.0 - deepTrans) + 0.1384 * emit, 0.8439);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2963), 0.8439);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6474 + 0.3526 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0858), 0.0, 1.0);
