...
void main() {
    ...
    mid = clamp(mid * (0.6051 + 0.6079 * texDetail) + 0.3337 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3713), 0.0, 1.0);
    float emit = atlas.a * (0.6654 + 0.2776 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1246 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7291 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3450 + 0.2815 * presence + 0.5641 * pattern + 0.1368 * (1.0 - deepTrans) + 0.1570 * emit, 0.9160);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1724), 0.9160);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6950 + 0.3050 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1398), 0.0, 1.0);
