...
void main() {
    ...
    mid = clamp(mid * (0.6402 + 0.7390 * texDetail) + 0.3479 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5247), 0.0, 1.0);
    float emit = atlas.a * (0.5975 + 0.2867 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1141 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.8949 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3418 + 0.2444 * presence + 0.4844 * pattern + 0.1175 * (1.0 - deepTrans) + 0.1940 * emit, 0.8633);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2718), 0.8633);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6233 + 0.3767 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1376), 0.0, 1.0);
