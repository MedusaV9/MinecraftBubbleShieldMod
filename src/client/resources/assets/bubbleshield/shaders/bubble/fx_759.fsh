...
void main() {
    ...
    mid = clamp(mid * (0.7292 + 0.6931 * texDetail) + 0.2440 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4571), 0.0, 1.0);
    float emit = atlas.a * (0.6893 + 0.4323 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1444 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.7026 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3634 + 0.2808 * presence + 0.4692 * pattern + 0.1108 * (1.0 - deepTrans) + 0.1464 * emit, 0.8766);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2547), 0.8766);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1124), 0.0, 1.0);
