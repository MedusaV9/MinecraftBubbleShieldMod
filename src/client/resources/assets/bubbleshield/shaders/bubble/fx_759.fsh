...
// stack: deep=fbm x3 taps | mid=voidrift+warp1+flicker | rim=lat | flourish=swirl
...
void main() {
    ...
    float vrStar = step(0.7558, vrH) * invsmooth(0.08, 0.42, length(fract(vrS) - 0.5)) * (0.6 + 0.4 * sin(time * (1.5 + 2.0 * vrH) + vrH * 31.0));
    ...
    float alpha = vertexColor.a * min(0.0706 + 0.1408 * presence + 0.4692 * pattern + 0.1108 * (1.0 - deepTrans), 0.8166);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2547), 0.8166);
    // [layer:v5:ghostalpha]
