...
void main() {
    ...
    mid = clamp(mid * (0.6616 + 0.6532 * texDetail) + 0.1843 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5474), 0.0, 1.0);
    float emit = atlas.a * (0.6067 + 0.4503 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1164 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.7473 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3547 + 0.4533 * presence + 0.3605 * pattern + 0.0824 * (1.0 - deepTrans) + 0.1542 * emit, 0.8858);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2256), 0.8858);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7585 + 0.2415 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
