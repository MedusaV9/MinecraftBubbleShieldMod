...
void main() {
    ...
    mid = clamp(mid * (0.7040 + 0.4607 * texDetail) + 0.3231 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5279), 0.0, 1.0);
    float emit = atlas.a * (0.5719 + 0.4001 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0831 * (0.5 + 0.5 * sin(time * 0.151844)));
    ...
    float alpha = vertexColor.a * min(0.4629 + 0.5058 * presence + 0.3625 * pattern + 0.1075 * (1.0 - deepTrans) + 0.1276 * emit, 0.9149);
