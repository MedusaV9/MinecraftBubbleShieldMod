...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7512, 0.0, 1.0);
    ...
    deepCol += 0.4392 * dp * mix(baseCol, deepStop, 0.2964);
    deepPat += 0.4392 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 3.5750) + par * 0.1583, time);
    deepCol += 0.1929 * dp * mix(baseCol, deepStop, 0.5929);
    deepPat += 0.1929 * dp;
    deepCol *= 0.6519;
    deepPat = pow(clamp(deepPat * 0.6127, 0.0, 1.0), 1.5852);
    ...
    float midCover = clamp(1.1477 * mid + 0.6897 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.0884);
    rgb += hotStop * 0.1719 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0777 + 0.2996 * presence + 0.3490 * pattern, 0.8149);
