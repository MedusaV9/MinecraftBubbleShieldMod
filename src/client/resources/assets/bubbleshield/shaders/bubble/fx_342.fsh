...
void main() {
    ...
    mid = clamp(mid * (0.6020 + 0.4779 * texDetail) + 0.2534 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3613), 0.0, 1.0);
    float emit = atlas.a * (0.5588 + 0.3430 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1126 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.9214 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4290 + 0.5044 * presence + 0.4567 * pattern + 0.0885 * (1.0 - deepTrans) + 0.1229 * emit, 0.8452);
