...
void main() {
    ...
    mid = clamp(mid * (0.7032 + 0.7136 * texDetail) + 0.3355 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3994), 0.0, 1.0);
    float emit = atlas.a * (0.6116 + 0.2566 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1060 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.6645 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2797 + 0.2688 * presence + 0.6066 * pattern + 0.1525 * (1.0 - deepTrans) + 0.1777 * emit, 0.8016);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3294), 0.8016);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7751 + 0.2249 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0609), 0.0, 1.0);
