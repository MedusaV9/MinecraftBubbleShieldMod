...
void main() {
    ...
    mid = clamp(mid * (0.6922 + 0.4985 * texDetail) + 0.2744 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5178), 0.0, 1.0);
    float emit = atlas.a * (0.6903 + 0.2799 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1355 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6913 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3137 + 0.3499 * presence + 0.4914 * pattern + 0.1229 * (1.0 - deepTrans) + 0.2054 * emit, 0.8517);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1518), 0.8517);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1153), 0.0, 1.0);
