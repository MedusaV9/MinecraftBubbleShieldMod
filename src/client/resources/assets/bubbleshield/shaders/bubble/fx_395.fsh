...
void main() {
    ...
    mid = clamp(mid * (0.6499 + 0.6213 * texDetail) + 0.2808 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4354), 0.0, 1.0);
    float emit = atlas.a * (0.7152 + 0.4908 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1570 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.8943 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3578 + 0.2903 * presence + 0.4484 * pattern + 0.0890 * (1.0 - deepTrans) + 0.1893 * emit, 0.8681);
