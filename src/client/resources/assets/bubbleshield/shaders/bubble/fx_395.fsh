...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6574, 0.0, 1.0);
    ...
    deepCol += 0.5394 * dp * mix(baseCol, deepStop, 0.4195);
    deepPat += 0.5394 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.3425) + par * 0.0667, time);
    deepCol += 0.2909 * dp * mix(baseCol, deepStop, 0.8390);
    deepPat += 0.2909 * dp;
    deepCol *= 0.5835;
    deepPat = pow(clamp(deepPat * 0.5464, 0.0, 1.0), 1.3295);
    ...
    float curve1 = 0.5 + 0.1312 * sin(baseUV.x * 6.2831853 * 2.0000 + time * 0.345575) + 0.1027 * sin(baseUV.x * 6.2831853 * 6.0000 - time * 0.277507);
    float slope1 = cos(baseUV.x * 6.2831853 * 2.0000 + time * 0.345575);
    float band1 = invsmooth(0.005, 0.0579, abs(baseUV.y - curve1));
    float curve2 = 0.5 + 0.1563 * sin(baseUV.x * 6.2831853 * 3.0000 - time * 0.272271 + 2.1) + 0.0734;
    float band2 = invsmooth(0.005, 0.0773, abs(baseUV.y - curve2));
    float foldGlow = 0.6 + 0.4 * slope1 * slope1;
    float shimmer = fbm2(wuv, midPer) * 0.2266;
    ...
    rgb = satLift(rgb, 1.2896);
    rgb += hotStop * 0.2383 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0749 + 0.2892 * presence + 0.3295 * pattern, 0.8081);
