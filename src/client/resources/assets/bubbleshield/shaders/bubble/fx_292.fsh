...
// v8 realism: refr 1.2564+1.9223*fres/dist (cap 0.1583) | energy 0.2283+0.4214*p | flow 0.085833 c/s | soft 0.7238m
...
void main() {
    ...
    rim = clamp(rim + 0.6026 * rimLine + 0.4852 * fresnel, 0.0, 1.4);
    ...
    motifM += exp(-dot(motifD0, motifD0) * 140.9502);
    ...
    motifM += exp(-dot(motifD1, motifD1) * 140.9502);
    ...
    motifM += exp(-dot(motifD2, motifD2) * 140.9502);
    ...
    float refrAmp = min((1.2564 + 1.9223 * fresnel) / max(length(worldPos), 1.0), 0.1583);
    ...
    float energyW = clamp(0.2283 + 0.4214 * clamp(pattern, 0.0, 1.0) + 0.1108 * fresnel, 0.0, 0.8089);
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.1511) * 0.8295;
    // [layer:emit:atlas_a]
