...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7495, 0.0, 1.0);
    ...
    deepCol += 0.5345 * dp * mix(baseCol, deepStop, 0.2252);
    deepPat += 0.5345 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.6318) + par * 0.1003, time);
    deepCol += 0.2857 * dp * mix(baseCol, deepStop, 0.4504);
    deepPat += 0.2857 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 6.9697) + par * 0.1505, time);
    deepCol += 0.1527 * dp * mix(baseCol, deepStop, 0.6757);
    deepPat += 0.1527 * dp;
    deepCol *= 0.5825;
    deepPat = pow(clamp(deepPat * 0.5069, 0.0, 1.0), 1.0316);
    ...
    float midCover = clamp(0.9899 * mid + 0.4343 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.0953);
    rgb += hotStop * 0.2607 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0567 + 0.3385 * presence + 0.3461 * pattern, 0.8175);
