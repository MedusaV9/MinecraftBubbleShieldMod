...
// stack: deep=caustic x4 taps | mid=kaleido+curl+pulse | rim=graze_film | flourish=swirl | tex=mandala | motif=orbitx3+heartbeat
// fbm: ridged x4 octaves | seed a111d015292b3c4d
// v8 realism: refr 1.4071+2.1529*fres/dist (cap 0.1583) | energy 0.1786+0.4214*p | flow 0.085833 c/s | soft 0.7238m
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4089, x));
    return mix(g, hotStop, smoothstep(0.4089, 1.0, x));
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.5856;
    vec3 spun = clamp(hueSpin(baseCol, -0.0350), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2663) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5624, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.5312);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, 0.9327), 0.0, 1.0), 0.3500) * 1.1644, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(-0.7242, 0.4937, 0.4814);
    vec3 par = Vt * 2.3024;
    ...
    float deepBreath = 1.0 + 0.0145 * sin(time * 0.743510);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * (2.9559 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4428 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * (4.2938 * deepBreath)) + par * 0.0502, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.3378), deepStop, 0.1239);
    deepTrans *= 1.0 - 0.4428 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * (5.6318 * deepBreath)) + par * 0.1003, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.6757), deepStop, 0.2477);
    deepTrans *= 1.0 - 0.4428 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * (6.9697 * deepBreath)) + par * 0.1505, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.3716);
    deepTrans *= 1.0 - 0.4428 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.7175;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.1067, 0.0, 1.0), 1.0316);
    ...
    vec2 texUV = wuv * 2.0000 + vec2(-0.017500, -0.014167) * time;
    ...
    vec3 hgtW = vec3(0.3158, 0.3663, 0.3180);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -2.5985;
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.6771 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 2.7011);
    ...
    // thickness rho = 0.1088 (crystalline material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.9073.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.8912 * 0.8912 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.9073);
    float chordN = min(chord / 0.1088, 3.0);
    ...
    float rimLine = smoothstep(0.0504, 0.0713, rimSlope);
    ...
    rim = clamp(rim + 0.6026 * rimLine + 0.7123 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2304 * pow(clamp(fbm2(wuv + vec2(-time * 0.073333, time * 0.070000), midPer), 0.0, 1.0), 2.0);
    float grain = 0.0862 * (cellHash(floor(wuv * 33.0000) + vec2(floor(time * 2.0), 0.0), 132.0000) - 0.5);
    ...
    float motifP = fract(time * 0.333333 + 0.1223);
    float motifEnv = 0.50 + 0.80 * pow(max(sin(motifP * 6.2831853), 0.0), 6.0) + 0.52 * pow(max(sin((motifP - 0.14) * 6.2831853), 0.0), 10.0);
    float motifM = 0.0;
    float motifO0 = fract(0.4162 + 1.0 * time * 0.025833);
    float motifL0 = 0.4220 + 0.0606 * sin(time * 0.507891 + 0.0000);
    vec2 motifD0 = vec2(sin((baseUV.x - motifO0) * 3.1415927) * 0.6366, baseUV.y - motifL0);
    motifM += exp(-dot(motifD0, motifD0) * 70.1092);
    float motifO1 = fract(0.7495 + 1.0 * time * 0.025833);
    float motifL1 = 0.4220 + 0.0606 * sin(time * 0.507891 + 2.4000);
    vec2 motifD1 = vec2(sin((baseUV.x - motifO1) * 3.1415927) * 0.6366, baseUV.y - motifL1);
    motifM += exp(-dot(motifD1, motifD1) * 70.1092);
    float motifO2 = fract(1.0828 + 1.0 * time * 0.025833);
    float motifL2 = 0.4220 + 0.0606 * sin(time * 0.507891 + 4.8000);
    vec2 motifD2 = vec2(sin((baseUV.x - motifO2) * 3.1415927) * 0.6366, baseUV.y - motifL2);
    motifM += exp(-dot(motifD2, motifD2) * 70.1092);
    ...
    float pattern = clamp(0.3705 * deepPat * mix(1.0, chordN, 0.1706) + 0.7771 * mid + 0.6277 * rim + flourish + grain + 0.1878 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.7728 + rim * 0.2395, 0.0, 1.0);
    ...
    vec3 softHot = 1.0 - exp(-3.2280 * hotStop * (pattern + 0.35 * rim));
    ...
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.8018 + baseUV.y * 0.31 + 0.1908) * 6.2831853);
    ...
    rgb *= (1.0 - 0.3051 * washHi) * (1.0 - 0.3557 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.5796, 0.7332, 0.3557);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4632) / 1.4632, 0.0, 1.0);
    rgb *= 0.6126 + 0.6065 * keyDiff * (1.0 - 0.5367 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 35.3082);
    rgb += hotStop * (keySpec * 0.5674 * (1.0 - 0.5820 * washHi));
    ...
    float refrAmp = min((1.4071 + 2.1529 * fresnel) / max(length(worldPos), 1.0), 0.1583);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.2847) * refrAmp;
    ...
    refracted *= exp(-0.5472 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1413);
    float energyW = clamp(0.1786 + 0.4214 * clamp(pattern, 0.0, 1.0) + 0.1926 * fresnel, 0.0, 0.8089);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2352 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (0.8239 * (1.0 - 0.7099 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.1511) * 1.2145;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3062 * max(0.0, 1.0 - 0.5366 * baseLuma - 0.6683 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3651 + 0.4670 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1164 * (0.5 + 0.5 * sin(time * 0.136136))) * atlasPoleW;
    rgb += emit * 0.9929 * (1.0 - 0.6752 * baseLuma) * (1.0 - 0.4791 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.9405));
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 0.9520 + hgtC * 0.5273, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.1963);
