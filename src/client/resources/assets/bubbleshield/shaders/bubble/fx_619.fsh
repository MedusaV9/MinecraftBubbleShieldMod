...
void main() {
    ...
    mid = clamp(mid * (0.6491 + 0.6134 * texDetail) + 0.3463 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3648), 0.0, 1.0);
    float emit = atlas.a * (0.5702 + 0.3275 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0963 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.8116 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3789 + 0.3495 * presence + 0.4331 * pattern + 0.1002 * (1.0 - deepTrans) + 0.1454 * emit, 0.8653);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2868), 0.8653);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7243 + 0.2757 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
