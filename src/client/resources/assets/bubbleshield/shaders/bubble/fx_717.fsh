...
void main() {
    ...
    mid = clamp(mid * (0.7066 + 0.5739 * texDetail) + 0.2236 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5428), 0.0, 1.0);
    float emit = atlas.a * (0.7035 + 0.3974 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1575 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.8961 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2761 + 0.2236 * presence + 0.5489 * pattern + 0.1581 * (1.0 - deepTrans) + 0.1839 * emit, 0.8797);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1979), 0.8797);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6581 + 0.3419 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1218), 0.0, 1.0);
