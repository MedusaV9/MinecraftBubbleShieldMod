...
void main() {
    ...
    mid = clamp(mid * (0.6761 + 0.6126 * texDetail) + 0.1944 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4752), 0.0, 1.0);
    float emit = atlas.a * (0.6680 + 0.4964 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1273 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.8686 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3637 + 0.4287 * presence + 0.3224 * pattern + 0.1068 * (1.0 - deepTrans) + 0.1770 * emit, 0.8729);
