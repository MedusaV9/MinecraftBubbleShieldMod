...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8375, 0.0, 1.0);
    ...
    deepCol += 0.5082 * dp * mix(baseCol, deepStop, 0.2776);
    deepPat += 0.5082 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 5.9344) + par * 0.0643, time);
    deepCol += 0.2583 * dp * mix(baseCol, deepStop, 0.5552);
    deepPat += 0.2583 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 7.5150) + par * 0.0964, time);
    deepCol += 0.1313 * dp * mix(baseCol, deepStop, 0.8327);
    deepPat += 0.1313 * dp;
    deepCol *= 0.5880;
    deepPat = pow(clamp(deepPat * 0.5269, 0.0, 1.0), 1.4640);
    ...
    rgb += hotStop * 0.2597 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0592 + 0.2948 * presence + 0.3517 * pattern, 0.7884);
