...
// stack: deep=fbm x3 taps | mid=ridged+curl+rotate | rim=graze | flourish=glint
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0304) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0304);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.033333, 0.035000) * time, time);
    deep += 0.5082 * deepField(duvBase + rimDir * 0.0321 + vec2(-0.051667, 0.055000) * time, time);
    deep += 0.2583 * deepField(duvBase + rimDir * 0.0643 + vec2(-0.071667, 0.075000) * time, time);
    deep = pow(clamp(deep * 0.5661, 0.0, 1.0), 1.4640);
    ...
    float rn = fbm2(wuv + vec2(time * 0.113333, time * 0.030000), midPer);
    ...
    float pattern = clamp(0.4707 * deep + 0.8557 * mid + 0.7868 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8560 + pattern * 0.6917);
    vec3 rgb = vertexColor.rgb * (0.3608 + 0.6745 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2373 + 0.7234 * pattern, 0.0, 1.0);
