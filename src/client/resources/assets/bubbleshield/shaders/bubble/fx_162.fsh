...
void main() {
    ...
    mid = clamp(mid * (0.6763 + 0.5780 * texDetail) + 0.3281 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4405), 0.0, 1.0);
    float emit = atlas.a * (0.5580 + 0.3043 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1171 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.8564 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3346 + 0.4713 * presence + 0.4482 * pattern + 0.0920 * (1.0 - deepTrans) + 0.1221 * emit, 0.8428);
