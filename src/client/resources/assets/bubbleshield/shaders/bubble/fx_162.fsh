...
// stack: deep=fbm x3 taps | mid=waves+warp1+rotate | rim=graze | flourish=shimmer
...
void main() {
    ...
    deep += 0.4687 * deepField(duvBase + rimDir * 0.0366 + vec2(-0.021667, -0.067500) * time, time);
    deep += 0.2196 * deepField(duvBase + rimDir * 0.0731 + vec2(-0.030000, -0.092500) * time, time);
    deep = pow(clamp(deep * 0.5923, 0.0, 1.0), 1.7141);
    ...
    float pattern = clamp(0.3154 * deep + 0.7279 * mid + 0.8088 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1489 + pattern * 0.6811);
    vec3 rgb = vertexColor.rgb * (0.3916 + 0.6439 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1857 + 0.7443 * pattern, 0.0, 1.0);
