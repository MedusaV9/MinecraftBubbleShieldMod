...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6417, 0.0, 1.0);
    ...
    deepCol += 0.4687 * dp * mix(baseCol, deepStop, 0.2102);
    deepPat += 0.4687 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 4.2206) + par * 0.0731, time);
    deepCol += 0.2196 * dp * mix(baseCol, deepStop, 0.4204);
    deepPat += 0.2196 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 5.2937) + par * 0.1097, time);
    deepCol += 0.1029 * dp * mix(baseCol, deepStop, 0.6306);
    deepPat += 0.1029 * dp;
    deepCol *= 0.6319;
    deepPat = pow(clamp(deepPat * 0.5583, 0.0, 1.0), 1.7141);
    ...
    float alpha = vertexColor.a * min(0.0422 + 0.3313 * presence + 0.4482 * pattern, 0.7828);
