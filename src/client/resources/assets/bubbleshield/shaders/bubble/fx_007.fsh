...
void main() {
    ...
    mid = clamp(mid * (0.6161 + 0.4986 * texDetail) + 0.2963 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4411), 0.0, 1.0);
    float emit = atlas.a * (0.6438 + 0.4318 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1347 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.7682 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3286 + 0.4217 * presence + 0.3618 * pattern + 0.1453 * (1.0 - deepTrans) + 0.1887 * emit, 0.9200);
