...
void main() {
    ...
    mid = clamp(mid * (0.6081 + 0.5752 * texDetail) + 0.3154 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4703), 0.0, 1.0);
    float emit = atlas.a * (0.5978 + 0.3543 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1260 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.8904 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3641 + 0.5078 * presence + 0.3239 * pattern + 0.0965 * (1.0 - deepTrans) + 0.1486 * emit, 0.8460);
