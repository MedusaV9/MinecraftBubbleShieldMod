...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.043333, 0.043333) * time, time);
    deep += 0.5507 * deepField(duvBase + rimDir * 0.0498 + vec2(0.066667, 0.066667) * time, time);
    deep += 0.3032 * deepField(duvBase + rimDir * 0.0996 + vec2(0.090000, 0.091667) * time, time);
    deep += 0.1670 * deepField(duvBase + rimDir * 0.1493 + vec2(0.114167, 0.115000) * time, time);
    deep = pow(clamp(deep * 0.4948, 0.0, 1.0), 1.7962);
    ...
    float pattern = clamp(0.3835 * deep + 0.8658 * mid + 0.6999 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6743 + pattern * 0.6139);
    vec3 rgb = vertexColor.rgb * (0.5172 + 0.8190 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1042 + 0.6310 * pattern, 0.0, 1.0);
