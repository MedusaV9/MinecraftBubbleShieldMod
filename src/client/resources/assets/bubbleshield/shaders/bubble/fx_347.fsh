...
void main() {
    ...
    mid = clamp(mid * (0.7188 + 0.6270 * texDetail) + 0.2703 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4014), 0.0, 1.0);
    float emit = atlas.a * (0.7807 + 0.4402 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1757 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.9559 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3541 + 0.4655 * presence + 0.3737 * pattern + 0.1413 * (1.0 - deepTrans) + 0.1680 * emit, 0.9029);
