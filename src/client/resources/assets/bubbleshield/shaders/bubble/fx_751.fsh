...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.7951, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0437, abs(glR - 0.1040));
    float mid = clamp(glStars * (1.0 + glRing * 1.3307) + glRing * 0.5002, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.7009 + 0.5991 * texDetail) + 0.3368 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3667), 0.0, 1.0);
    float emit = atlas.a * (0.6697 + 0.2696 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1742 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.9165 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3373 + 0.2430 * presence + 0.4902 * pattern + 0.1097 * (1.0 - deepTrans) + 0.1459 * emit, 0.8455);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3190), 0.8455);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7388 + 0.2612 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1079), 0.0, 1.0);
