...
// stack: deep=caustic x3 taps | mid=deepice+warp2+flicker | rim=lat | flourish=shimmer
...
void main() {
    ...
    rgb = mix(rgb, mix(secCol, deepStop, 0.5), clamp(crackB * (1.0 - crackA), 0.0, 1.0) * 0.4416);
    ...
    float alpha = vertexColor.a * min(0.0674 + 0.3291 * presence + 0.3435 * pattern + 0.0992 * (1.0 - deepTrans), 0.8757);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3232), 0.8757);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4028 + 0.5972 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
