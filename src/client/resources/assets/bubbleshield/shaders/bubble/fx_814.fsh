...
void main() {
    ...
    mid = clamp(mid * (0.6796 + 0.5656 * texDetail) + 0.2786 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3561), 0.0, 1.0);
    float emit = atlas.a * (0.7175 + 0.4292 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1191 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.9956 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4242 + 0.4691 * presence + 0.3435 * pattern + 0.0992 * (1.0 - deepTrans) + 0.2163 * emit, 0.9357);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3232), 0.9357);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6415 + 0.3585 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0929), 0.0, 1.0);
