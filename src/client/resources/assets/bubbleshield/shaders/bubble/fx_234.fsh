...
// stack: deep=fbm x2 taps | mid=vortex+curl+scroll | rim=graze | flourish=glint
...
void main() {
    ...
    deep += 0.4697 * deepField(duvBase + rimDir * 0.0518 + vec2(0.002500, -0.030000) * time, time);
    deep = pow(clamp(deep * 0.6804, 0.0, 1.0), 1.4894);
    ...
    float pattern = clamp(0.3301 * deep + 0.8102 * mid + 0.6126 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2707 + pattern * 0.6655);
    vec3 rgb = vertexColor.rgb * (0.3843 + 0.6036 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1232 + 0.8270 * pattern, 0.0, 1.0);
