...
void main() {
    ...
    mid = clamp(mid * (0.6295 + 0.6023 * texDetail) + 0.2691 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4402), 0.0, 1.0);
    float emit = atlas.a * (0.5579 + 0.2760 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0927 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.9928 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3028 + 0.3741 * presence + 0.4931 * pattern + 0.1387 * (1.0 - deepTrans) + 0.2143 * emit, 0.9032);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2797), 0.9032);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6823 + 0.3177 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0999), 0.0, 1.0);
