...
void main() {
    ...
    mid = clamp(mid * (0.6611 + 0.5893 * texDetail) + 0.2100 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3812), 0.0, 1.0);
    float emit = atlas.a * (0.6454 + 0.3959 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1064 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.8743 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3551 + 0.4078 * presence + 0.4201 * pattern + 0.1528 * (1.0 - deepTrans) + 0.1861 * emit, 0.8757);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2519), 0.8757);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7216 + 0.2784 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1021), 0.0, 1.0);
