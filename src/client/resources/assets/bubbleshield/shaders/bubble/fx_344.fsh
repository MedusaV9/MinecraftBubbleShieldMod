...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6793, 0.0, 1.0);
    ...
    deepCol += 0.4730 * dp * mix(baseCol, deepStop, 0.2729);
    deepPat += 0.4730 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 4.5470) + par * 0.0690, time);
    deepCol += 0.2237 * dp * mix(baseCol, deepStop, 0.5458);
    deepPat += 0.2237 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 5.5090) + par * 0.1034, time);
    deepCol += 0.1058 * dp * mix(baseCol, deepStop, 0.8188);
    deepPat += 0.1058 * dp;
    deepCol *= 0.5511;
    deepPat = pow(clamp(deepPat * 0.5548, 0.0, 1.0), 1.7395);
    ...
    rgb += hotStop * 0.2422 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0660 + 0.2768 * presence + 0.3361 * pattern, 0.8311);
