...
void main() {
    ...
    mid = clamp(mid * (0.6682 + 0.6324 * texDetail) + 0.3205 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3726), 0.0, 1.0);
    float emit = atlas.a * (0.6184 + 0.3269 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1046 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8432 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3384 + 0.3905 * presence + 0.3968 * pattern + 0.1146 * (1.0 - deepTrans) + 0.1645 * emit, 0.9154);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2946), 0.9154);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7081 + 0.2919 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0631), 0.0, 1.0);
