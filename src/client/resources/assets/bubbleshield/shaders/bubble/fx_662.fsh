...
// stack: deep=ridge x3 taps | mid=prismdisperse+curl+pulse | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    float flourish = 0.2610 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0440 + 0.2505 * presence + 0.3968 * pattern + 0.1146 * (1.0 - deepTrans), 0.8554);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2946), 0.8554);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4736 + 0.5264 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
