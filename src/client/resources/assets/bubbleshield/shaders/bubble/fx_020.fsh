...
// stack: deep=caustic x2 taps | mid=starfield+warp2+pulse | rim=graze_sparkle | flourish=echo
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.018333, 0.027500) * time, time);
    deep += 0.4069 * deepField(duvBase + rimDir * 0.0426 + vec2(-0.028333, 0.040000) * time, time);
    deep = pow(clamp(deep * 0.7108, 0.0, 1.0), 1.3115);
    ...
    float pattern = clamp(0.4296 * deep + 0.9105 * mid + 0.8682 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5405 + pattern * 0.6622);
    vec3 rgb = vertexColor.rgb * (0.4286 + 0.6419 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1944 + 0.6207 * pattern, 0.0, 1.0);
