...
void main() {
    ...
    mid = clamp(mid * (0.6651 + 0.6956 * texDetail) + 0.3287 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4793), 0.0, 1.0);
    float emit = atlas.a * (0.7533 + 0.4816 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1624 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9448 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2974 + 0.3480 * presence + 0.4855 * pattern + 0.1197 * (1.0 - deepTrans) + 0.1238 * emit, 0.9162);
