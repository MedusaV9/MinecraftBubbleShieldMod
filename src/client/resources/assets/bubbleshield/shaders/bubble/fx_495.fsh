...
void main() {
    ...
    mid = clamp(mid * (0.6251 + 0.6478 * texDetail) + 0.2515 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3770), 0.0, 1.0);
    float emit = atlas.a * (0.7659 + 0.3906 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1570 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.9954 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3062 + 0.3068 * presence + 0.5611 * pattern + 0.1530 * (1.0 - deepTrans) + 0.1822 * emit, 0.8873);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2731), 0.8873);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7721 + 0.2279 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0740), 0.0, 1.0);
