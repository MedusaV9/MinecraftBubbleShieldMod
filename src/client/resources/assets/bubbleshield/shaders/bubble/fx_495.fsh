...
// stack: deep=fbm x4 taps | mid=runecircuit+warp1+rotate | rim=graze | flourish=glint | tex=runes
// fbm: ridged x5 octaves | seed 46e47882e0bc0d1e
// v8 realism: refr 0.3757+0.4418*fres/dist (cap 0.1056) | energy 0.4433+0.4336*p | flow 0.082500 c/s | soft 0.9374m
...
void main() {
    ...
    float refrAmp = min((0.3757 + 0.4418 * fresnel) / max(length(worldPos), 1.0), 0.1056);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0376, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9624, vec2(0.001), vec2(0.999))).b);
