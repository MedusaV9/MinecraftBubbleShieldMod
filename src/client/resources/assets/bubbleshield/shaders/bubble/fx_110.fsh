...
void main() {
    ...
    mid = clamp(mid * (0.6544 + 0.4823 * texDetail) + 0.2069 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4395), 0.0, 1.0);
    float emit = atlas.a * (0.5655 + 0.3449 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1380 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.6784 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4420 + 0.4627 * presence + 0.4084 * pattern + 0.1110 * (1.0 - deepTrans) + 0.1807 * emit, 0.9020);
