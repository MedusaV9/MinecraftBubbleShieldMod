...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8025, 0.0, 1.0);
    ...
    deepCol += 0.5702 * dp * mix(baseCol, deepStop, 0.2835);
    deepPat += 0.5702 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.1309) + par * 0.0978, time);
    deepCol += 0.3251 * dp * mix(baseCol, deepStop, 0.5670);
    deepPat += 0.3251 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.5003) + par * 0.1467, time);
    deepCol += 0.1854 * dp * mix(baseCol, deepStop, 0.8505);
    deepPat += 0.1854 * dp;
    deepCol *= 0.4801;
    deepPat = pow(clamp(deepPat * 0.4806, 0.0, 1.0), 1.5664);
    ...
    rgb += hotStop * 0.2449 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0785 + 0.3227 * presence + 0.4084 * pattern, 0.8420);
