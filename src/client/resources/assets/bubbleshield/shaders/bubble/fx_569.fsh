...
void main() {
    ...
    mid = clamp(mid * (0.6502 + 0.7492 * texDetail) + 0.2732 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4146), 0.0, 1.0);
    float emit = atlas.a * (0.5783 + 0.3098 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1211 * (0.5 + 0.5 * sin(time * 0.287979)));
    ...
    float alpha = vertexColor.a * min(0.4650 + 0.4609 * presence + 0.3987 * pattern + 0.1010 * (1.0 - deepTrans) + 0.1681 * emit, 0.9210);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1800), 0.9210);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6323 + 0.3677 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
