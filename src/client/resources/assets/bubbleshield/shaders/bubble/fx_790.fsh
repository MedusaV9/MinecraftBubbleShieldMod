...
void main() {
    ...
    mid = clamp(mid * (0.6059 + 0.4787 * texDetail) + 0.1919 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4484), 0.0, 1.0);
    float emit = atlas.a * (0.7130 + 0.4872 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0906 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.7286 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3237 + 0.2272 * presence + 0.5123 * pattern + 0.1041 * (1.0 - deepTrans) + 0.1622 * emit, 0.8435);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3334), 0.8435);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7752 + 0.2248 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1318), 0.0, 1.0);
