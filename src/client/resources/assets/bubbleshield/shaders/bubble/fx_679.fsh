...
void main() {
    ...
    mid = clamp(mid * (0.6889 + 0.7073 * texDetail) + 0.3062 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3659), 0.0, 1.0);
    float emit = atlas.a * (0.5552 + 0.4571 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1587 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7252 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2901 + 0.3316 * presence + 0.5530 * pattern + 0.0944 * (1.0 - deepTrans) + 0.1506 * emit, 0.8656);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3175), 0.8656);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6777 + 0.3223 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
