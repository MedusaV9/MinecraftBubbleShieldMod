...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9304) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9304);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.016667, -0.017500) * time, time);
    deep += 0.5350 * deepField(duvBase + rimDir * 0.0478 + vec2(0.025000, -0.027500) * time, time);
    deep += 0.2862 * deepField(duvBase + rimDir * 0.0957 + vec2(0.035000, -0.037500) * time, time);
    deep += 0.1531 * deepField(duvBase + rimDir * 0.1435 + vec2(0.043333, -0.047500) * time, time);
    deep = pow(clamp(deep * 0.5065, 0.0, 1.0), 1.3183);
