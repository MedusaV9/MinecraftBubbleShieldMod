...
void main() {
    ...
    mid = clamp(mid * (0.6878 + 0.6210 * texDetail) + 0.2800 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5150), 0.0, 1.0);
    float emit = atlas.a * (0.6933 + 0.4197 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1748 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.8916 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3549 + 0.4188 * presence + 0.4688 * pattern + 0.1566 * (1.0 - deepTrans) + 0.1710 * emit, 0.9091);
