...
// stack: deep=ridge x4 taps | mid=rings+warp1+pulse | rim=graze_film | flourish=shimmer | tex=ridged
// fbm: ridged x4 octaves | seed c92bc309c3cac433
// v8 realism: refr 0.6310+0.6933*fres/dist (cap 0.1007) | energy 0.4135+0.3980*p | flow 0.097500 c/s | soft 0.7110m
...
void main() {
    ...
    float refrAmp = min((0.6310 + 0.6933 * fresnel) / max(length(worldPos), 1.0), 0.1007);
    ...
    float emit = atlas.a * (0.3860 + 0.6536 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1748 * (0.5 + 0.5 * sin(time * 0.172788))) * atlasPoleW;
