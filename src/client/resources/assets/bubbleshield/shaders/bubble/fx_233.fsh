...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7517, 0.0, 1.0);
    ...
    deepCol += 0.4749 * dp * mix(baseCol, deepStop, 0.2029);
    deepPat += 0.4749 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 3.1858) + par * 0.1399, time);
    deepCol += 0.2255 * dp * mix(baseCol, deepStop, 0.4059);
    deepPat += 0.2255 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 3.9725) + par * 0.2099, time);
    deepCol += 0.1071 * dp * mix(baseCol, deepStop, 0.6088);
    deepPat += 0.1071 * dp;
    deepCol *= 0.7083;
    deepPat = pow(clamp(deepPat * 0.5532, 0.0, 1.0), 1.4152);
    ...
    rgb += hotStop * 0.2184 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0491 + 0.2788 * presence + 0.4688 * pattern, 0.8491);
