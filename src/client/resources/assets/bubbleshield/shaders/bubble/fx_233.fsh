...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.030000, 0.012500) * time, time);
    deep += 0.4749 * deepField(duvBase + rimDir * 0.0700 + vec2(0.045000, 0.020000) * time, time);
    deep += 0.2255 * deepField(duvBase + rimDir * 0.1399 + vec2(0.061667, 0.027500) * time, time);
    deep += 0.1071 * deepField(duvBase + rimDir * 0.2099 + vec2(0.078333, 0.035000) * time, time);
    deep = pow(clamp(deep * 0.5532, 0.0, 1.0), 1.4152);
    ...
    float pattern = clamp(0.3597 * deep + 0.7528 * mid + 0.8523 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6331 + pattern * 0.5381);
    vec3 rgb = vertexColor.rgb * (0.4959 + 0.7934 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1976 + 0.6459 * pattern, 0.0, 1.0);
