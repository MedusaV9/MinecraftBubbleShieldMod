...
void main() {
    ...
    float mid = clamp((lonLine + latLine) * 0.7295 * holoFlicker + scan * 0.3726 + glint * 0.9, 0.0, 1.25);
    ...
    mid = clamp(mid * (0.6333 + 0.7056 * texDetail) + 0.3049 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3754), 0.0, 1.0);
    float emit = atlas.a * (0.5659 + 0.4724 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0846 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.6636 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3506 + 0.2727 * presence + 0.5233 * pattern + 0.1118 * (1.0 - deepTrans) + 0.1954 * emit, 0.8496);
