...
// Bubble surface shader fx_366 -- family LAVAFLOW
// stack: deep=ridge x3 taps | mid=lavaflow+warp1+rotate | rim=graze_sparkle | flourish=glint
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.7679 * (vec2(fbm2(p + vec2(0.0, t * 0.306667), per),
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8119, 0.0, 1.0);
    ...
    deepCol += 0.5301 * dp * mix(baseCol, deepStop, 0.3982);
    deepPat += 0.5301 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 3.6304) + par * 0.1127, time);
    deepCol += 0.2810 * dp * mix(baseCol, deepStop, 0.7964);
    deepPat += 0.2810 * dp;
    deepCol *= 0.5428;
    deepPat = pow(clamp(deepPat * 0.5521, 0.0, 1.0), 1.0366);

    // [layer:mid:lavaflow_warp1_rotate]
    ...
    vec2 lavaUV = wuv + vec2(0.0, time * 0.166667);
    float molten = fbm2(lavaUV, midPer);
    float crust = smoothstep(0.3986, 0.7130, fbm2(lavaUV * 2.0 + vec2(9.1, 3.7), midPer * 2.0));
    float vein = pow(clamp(1.0 - abs(2.0 * fbm2(lavaUV + vec2(4.3, 7.9), midPer) - 1.0), 0.0, 1.0), 8.5609);
    float throb = 0.8 + 0.2 * sin(time * 0.539307 + molten * 2.6199);
    float mid = clamp(molten * (1.0 - crust * 0.6533) * throb + vein * 0.7905 * (1.0 - crust * 0.5), 0.0, 1.3);
    ...
    float grain = 0.0774 * (cellHash(floor(wuv * 26.0000) + vec2(floor(time * 6.0), 0.0), 156.0000) - 0.5);
    ...
    rgb += hotStop * 0.2672 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0757 + 0.2931 * presence + 0.3480 * pattern, 0.7896);
