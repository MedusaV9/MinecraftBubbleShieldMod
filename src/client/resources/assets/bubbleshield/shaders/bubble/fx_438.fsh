...
void main() {
    ...
    mid = clamp(mid * (0.7250 + 0.5834 * texDetail) + 0.3358 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7202 + 0.4401 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1652 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.9222 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2862 + 0.3039 * presence + 0.4507 * pattern + 0.1244 * (1.0 - deepTrans) + 0.1450 * emit, 0.9095);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2323), 0.9095);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0757), 0.0, 1.0);
