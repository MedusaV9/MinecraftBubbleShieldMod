...
// stack: deep=fbm x3 taps | mid=mycelia+warp2+scroll | rim=graze_film | flourish=echo | tex=filaments
// fbm: ridged x5 octaves | seed d8821e387078d787
// v8 realism: refr 0.3084+0.3122*fres/dist (cap 0.1273) | energy 0.4624+0.3688*p | flow 0.118333 c/s | soft 0.6417m
...
void main() {
    ...
    float refrAmp = min((0.3084 + 0.3122 * fresnel) / max(length(worldPos), 1.0), 0.1273);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0538, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9462, vec2(0.001), vec2(0.999))).b);
