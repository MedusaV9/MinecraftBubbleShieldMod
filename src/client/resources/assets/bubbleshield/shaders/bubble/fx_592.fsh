...
void main() {
    ...
    mid = clamp(mid * (0.7337 + 0.5791 * texDetail) + 0.3343 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4221), 0.0, 1.0);
    float emit = atlas.a * (0.7709 + 0.3908 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1451 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.9539 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2923 + 0.3301 * presence + 0.5832 * pattern + 0.0979 * (1.0 - deepTrans) + 0.1414 * emit, 0.9015);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1643), 0.9015);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7451 + 0.2549 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0914), 0.0, 1.0);
