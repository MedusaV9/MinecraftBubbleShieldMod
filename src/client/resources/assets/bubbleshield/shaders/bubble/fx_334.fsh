...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.036667, -0.023333) * time, time);
    deep += 0.5620 * deepField(duvBase + rimDir * 0.0458 + vec2(-0.057500, -0.036667) * time, time);
    deep += 0.3158 * deepField(duvBase + rimDir * 0.0916 + vec2(-0.077500, -0.050000) * time, time);
    deep = pow(clamp(deep * 0.5326, 0.0, 1.0), 1.2162);
