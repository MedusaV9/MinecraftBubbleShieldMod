...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7978, 0.0, 1.0);
    ...
    deepCol += 0.5193 * dp * mix(baseCol, deepStop, 0.2998);
    deepPat += 0.5193 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 3.9349) + par * 0.0822, time);
    deepCol += 0.2697 * dp * mix(baseCol, deepStop, 0.5995);
    deepPat += 0.2697 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.8053) + par * 0.1233, time);
    deepCol += 0.1401 * dp * mix(baseCol, deepStop, 0.8993);
    deepPat += 0.1401 * dp;
    deepCol *= 0.6351;
    deepPat = pow(clamp(deepPat * 0.5184, 0.0, 1.0), 1.0350);
    ...
    rgb += hotStop * 0.3479 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0505 + 0.3032 * presence + 0.4329 * pattern, 0.8004);
