...
// stack: deep=fbm x3 taps | mid=voronoi+curl+flicker | rim=graze | flourish=echo
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.050000, -0.007500) * time, time);
    deep += 0.5193 * deepField(duvBase + rimDir * 0.0411 + vec2(0.076667, -0.012500) * time, time);
    deep += 0.2697 * deepField(duvBase + rimDir * 0.0822 + vec2(0.105000, -0.017500) * time, time);
    deep = pow(clamp(deep * 0.5590, 0.0, 1.0), 1.0350);
    ...
    float pattern = clamp(0.4643 * deep + 0.9907 * mid + 0.5339 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6934 + pattern * 0.5639);
    vec3 rgb = vertexColor.rgb * (0.4015 + 0.6473 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2053 + 0.6922 * pattern, 0.0, 1.0);
