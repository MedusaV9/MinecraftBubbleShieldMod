...
void main() {
    ...
    mid = clamp(mid * (0.7445 + 0.7080 * texDetail) + 0.2295 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8417 + 0.3869 * texDetail;
    rgb = satLift(rgb, 1.2300);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5183), 0.0, 1.0);
    float emit = atlas.a * (0.6309 + 0.2633 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0957 * (0.5 + 0.5 * sin(time * 0.183260)));
    ...
    float alpha = vertexColor.a * min(0.3044 + 0.2766 * presence + 0.5263 * pattern + 0.1034 * (1.0 - deepTrans) + 0.1409 * emit, 0.8984);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2410), 0.8984);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7157 + 0.2843 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
