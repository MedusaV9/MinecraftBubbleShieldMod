...
void main() {
    ...
    mid = clamp(mid * (0.6618 + 0.6754 * texDetail) + 0.2571 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4808), 0.0, 1.0);
    float emit = atlas.a * (0.7452 + 0.4497 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0973 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.9241 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4644 + 0.4473 * presence + 0.4606 * pattern + 0.1426 * (1.0 - deepTrans) + 0.2186 * emit, 0.9030);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2142), 0.9030);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6465 + 0.3535 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
