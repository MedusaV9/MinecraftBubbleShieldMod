...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8413, 0.0, 1.0);
    ...
    deepCol += 0.4532 * dp * mix(baseCol, deepStop, 0.2601);
    deepPat += 0.4532 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 4.3151) + par * 0.1332, time);
    deepCol += 0.2054 * dp * mix(baseCol, deepStop, 0.5202);
    deepPat += 0.2054 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.4385) + par * 0.1998, time);
    deepCol += 0.0931 * dp * mix(baseCol, deepStop, 0.7803);
    deepPat += 0.0931 * dp;
    deepCol *= 0.6210;
    deepPat = pow(clamp(deepPat * 0.5709, 0.0, 1.0), 1.2087);
    ...
    rgb += hotStop * 0.2416 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0743 + 0.3254 * presence + 0.3302 * pattern, 0.7864);
