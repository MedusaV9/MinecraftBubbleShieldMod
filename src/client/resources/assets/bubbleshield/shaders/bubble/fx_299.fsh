...
void main() {
    ...
    mid = clamp(mid * (0.7407 + 0.4754 * texDetail) + 0.3245 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7225 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4237), 0.0, 1.0);
    float emit = atlas.a * (0.6301 + 0.3134 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0846 * (0.5 + 0.5 * sin(time * 0.261799)));
    ...
    float alpha = vertexColor.a * min(0.4295 + 0.4654 * presence + 0.3302 * pattern + 0.0892 * (1.0 - deepTrans) + 0.1933 * emit, 0.8464);
