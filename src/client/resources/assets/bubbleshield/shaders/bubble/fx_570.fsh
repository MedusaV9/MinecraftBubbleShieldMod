...
void main() {
    ...
    mid = clamp(mid * (0.6702 + 0.5483 * texDetail) + 0.2534 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5090), 0.0, 1.0);
    float emit = atlas.a * (0.7482 + 0.4337 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1647 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.6948 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3350 + 0.2328 * presence + 0.5267 * pattern + 0.1225 * (1.0 - deepTrans) + 0.2172 * emit, 0.8713);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3147), 0.8713);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0761), 0.0, 1.0);
