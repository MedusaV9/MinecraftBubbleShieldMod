...
// stack: deep=caustic x3 taps | mid=sparkle+curl+flicker | rim=graze_film | flourish=glint | tex=starfield
// fbm: standard x4 octaves | seed 3387984bfc243d21
// v8 realism: refr 0.5296+0.7846*fres/dist (cap 0.1145) | energy 0.3704+0.5216*p | flow 0.091667 c/s | soft 0.8473m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.5073 + 0.2462, 0.0, 1.0);
    ...
    float refrAmp = min((0.5296 + 0.7846 * fresnel) / max(length(worldPos), 1.0), 0.1145);
    ...
    float emitMask = clamp(atlas.a * 1.8175, 0.0, 1.0);
    ...
    float emit = emitMask * (0.3285 + 0.6112 * emitDrive) * (1.0 - 0.1777 * (0.5 + 0.5 * sin(time * 0.136136))) * atlasPoleW;
