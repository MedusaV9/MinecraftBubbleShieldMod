...
void main() {
    ...
    mid = clamp(mid * (0.6434 + 0.5467 * texDetail) + 0.3201 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4049), 0.0, 1.0);
    float emit = atlas.a * (0.5975 + 0.3843 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1777 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.8293 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3809 + 0.4940 * presence + 0.4492 * pattern + 0.1244 * (1.0 - deepTrans) + 0.1679 * emit, 0.8839);
