...
void main() {
    ...
    mid = clamp(mid * (0.6660 + 0.6519 * texDetail) + 0.1858 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3846), 0.0, 1.0);
    float emit = atlas.a * (0.6817 + 0.3951 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0865 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.8889 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4380 + 0.4903 * presence + 0.4957 * pattern + 0.0923 * (1.0 - deepTrans) + 0.1259 * emit, 0.9073);
