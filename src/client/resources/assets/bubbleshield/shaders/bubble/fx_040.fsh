...
void main() {
    ...
    mid = clamp(mid * (0.6719 + 0.5123 * texDetail) + 0.3005 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4053), 0.0, 1.0);
    float emit = atlas.a * (0.6787 + 0.3870 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1574 * (0.5 + 0.5 * sin(time * 0.225147)));
    ...
    float alpha = vertexColor.a * min(0.3686 + 0.3805 * presence + 0.3411 * pattern + 0.1318 * (1.0 - deepTrans) + 0.1731 * emit, 0.8813);
