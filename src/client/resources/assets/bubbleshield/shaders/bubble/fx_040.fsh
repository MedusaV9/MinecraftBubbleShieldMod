...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5848, 0.0, 1.0);
    ...
    deepCol += 0.5252 * dp * mix(baseCol, deepStop, 0.2286);
    deepPat += 0.5252 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 3.3475) + par * 0.1383, time);
    deepCol += 0.2759 * dp * mix(baseCol, deepStop, 0.4572);
    deepPat += 0.2759 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.1429) + par * 0.2074, time);
    deepCol += 0.1449 * dp * mix(baseCol, deepStop, 0.6857);
    deepPat += 0.1449 * dp;
    deepCol *= 0.5994;
    deepPat = pow(clamp(deepPat * 0.5139, 0.0, 1.0), 1.5893);
    ...
    rgb += hotStop * 0.3128 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0537 + 0.2405 * presence + 0.3411 * pattern, 0.8213);
