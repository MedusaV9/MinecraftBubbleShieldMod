...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.045000, 0.002500) * time, time);
    deep += 0.5252 * deepField(duvBase + rimDir * 0.0691 + vec2(0.070000, 0.002500) * time, time);
    deep += 0.2759 * deepField(duvBase + rimDir * 0.1383 + vec2(0.095000, 0.005000) * time, time);
    deep += 0.1449 * deepField(duvBase + rimDir * 0.2074 + vec2(0.118333, 0.005000) * time, time);
    deep = pow(clamp(deep * 0.5139, 0.0, 1.0), 1.5893);
    ...
    float pattern = clamp(0.3156 * deep + 0.8952 * mid + 0.7176 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5837 + pattern * 0.5224);
    vec3 rgb = vertexColor.rgb * (0.5471 + 0.8504 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2484 + 0.6208 * pattern, 0.0, 1.0);
