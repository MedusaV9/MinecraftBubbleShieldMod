...
// stack: deep=caustic x3 taps | mid=scales+warp2+flicker | rim=graze_sparkle | flourish=echo | tex=scales | motif=scanx2+heartbeat
// fbm: ridged x5 octaves | seed ee86c9085f921ed2
// v8 realism: refr 1.3646+1.1519*fres/dist (cap 0.1707) | energy 0.2425+0.4815*p | flow 0.073333 c/s | soft 0.5580m
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4659, x));
    return mix(g, hotStop, smoothstep(0.4659, 1.0, x));
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.8393;
    vec3 spun = clamp(hueSpin(baseCol, -0.4473), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.1799) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.4898, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.6155);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, -0.4890), 0.0, 1.0), 1.3245) * 0.5061, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(-0.5616, -0.5716, 0.5983);
    vec3 par = Vt * 2.1819;
    ...
    float dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * (2.8953 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4280 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * (4.4218 * deepBreath)) + par * 0.0752, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.4932), deepStop, 0.1808);
    deepTrans *= 1.0 - 0.4280 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * (5.9482 * deepBreath)) + par * 0.1503, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.9864), deepStop, 0.3617);
    deepTrans *= 1.0 - 0.4280 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.7611;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.2302, 0.0, 1.0), 1.5175);
    ...
    float ft = time + 0.4310 * (0.6 * sin(time * 3.309144) + 0.4 * sin(time * 10.210176 + 1.8959));
    ...
    vec2 texUV = wuv * 1.0000 + vec2(0.052500, 0.029167) * time;
    ...
    vec3 hgtW = vec3(0.3482, 0.4156, 0.2362);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -3.3170;
    ...
    float texDetail = clamp(atlas.r * 0.4581 + atlas.g * 0.5467 + atlas.b * 0.3108, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.7010 + 0.7497 * texDetail, atlasPoleW) + 0.1628 * texDetail * (0.2501 + 0.7499 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.4046 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.3541);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0576).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0576).rgb;
    float deepTex = 0.5 * (dot(deepTexA, hgtW) + dot(deepTexB, hgtW));

    // [layer:thick:chord]
    // v11 volumetric thickness: the membrane is a shell of relative
    // thickness rho = 0.0686 (geo material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.7278.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9314 * 0.9314 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.7278);
    float chordN = min(chord / 0.0686, 3.0);
    ...
    float rimLine = smoothstep(0.0483, 0.0826, rimSlope);
    ...
    rim = clamp(rim + 0.6761 * rimLine + 0.7788 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2369 * ringPulse(baseUV + vec2(0.13, 0.31), time);
    float grain = 0.0726 * (cellHash(floor(wuv * 60.0000) + vec2(floor(time * 2.0), 0.0), 720.0000) - 0.5);
    ...
    float motifP = fract(time * 0.427500 + 0.0471);
    ...
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.y * 1.0000 - -1.0 * time * 0.097500 + 0.0957)), 15.3316) * motifBand;
    ...
    float pattern = clamp(0.3850 * deepPat * mix(1.0, chordN, 0.1765) + 0.7805 * mid + 0.8120 * rim + flourish + grain + 0.1795 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.8712 + rim * 0.2488, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9945 * mid + 0.5233 * rim, 0.0, 1.0);
    ...
    vec3 softHot = 1.0 - exp(-3.3233 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.2201 * smoothstep(0.55, 1.10, pattern));
    ...
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.9110 + baseUV.y * 0.31 + 0.8351) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2465);
    ...
    rgb *= (1.0 - 0.3231 * washHi) * (1.0 - 0.2692 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.6246, 0.5992, -0.5008);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4921) / 1.4921, 0.0, 1.0);
    rgb *= 0.5809 + 0.5954 * keyDiff * (1.0 - 0.6115 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 25.9187);
    rgb += hotStop * (keySpec * 0.3061 * (1.0 - 0.6844 * washHi));
    ...
    float refrAmp = min((1.3646 + 1.1519 * fresnel) / max(length(worldPos), 1.0), 0.1707);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.3859) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0599, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9401, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.4397 * chordN * (1.0 - baseCol));
    ...
    float energyW = clamp(0.2425 + 0.4815 * clamp(pattern, 0.0, 1.0) + 0.2363 * fresnel, 0.0, 0.8839);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2837 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (0.9996 * (1.0 - 0.6745 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2299) * 1.1984;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3636 * max(0.0, 1.0 - 0.5389 * baseLuma - 0.5266 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.4259 + 0.6457 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1338 * (0.5 + 0.5 * sin(time * 0.282743))) * atlasPoleW;
    rgb += emit * 0.7307 * (1.0 - 0.6161 * baseLuma) * (1.0 - 0.5882 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.4411));
    ...
    float bodyAlpha = 0.4125 + 0.4182 * presence + 0.4123 * pattern + 0.0992 * (1.0 - deepTrans) + 0.1312 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.1414;
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.2631 + hgtC * 0.3685, 0.0, 1.0);
    ...
    innerScaf = clamp(innerScaf + 0.3174 * pow(clamp(motifM, 0.0, 1.0), 0.25), 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.2945);
