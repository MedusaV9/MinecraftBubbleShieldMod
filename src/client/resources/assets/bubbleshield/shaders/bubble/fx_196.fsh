...
void main() {
    ...
    vec3 par = rimDir.x * vec3(0.5025, 0.0823, 0.8607) + rimDir.y * vec3(0.9062, 0.0889, 0.4134);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.089012) * (sdir * 2.8953), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4280 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.4218) + par * 0.0752, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.4932), deepStop, 0.1808);
    deepTrans *= 1.0 - 0.4280 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 5.9482) + par * 0.1503, time);
    ...
    float pattern = clamp(0.3850 * deepPat + 0.7805 * mid + 0.8120 * rim + flourish + grain, 0.0, 1.5);
