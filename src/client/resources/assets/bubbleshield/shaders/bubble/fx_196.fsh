...
// v8 realism: refr 1.2241+1.0333*fres/dist (cap 0.1707) | energy 0.3510+0.4815*p | flow 0.073333 c/s | soft 0.5580m
...
void main() {
    ...
    vec2 tuv = vec2(baseUV.x * 12.0000, baseUV.y * 12.0000) + vec2(0.410000, 0.116667) * time;
    ...
    vec2 texUV = tuv * 1.0000 + vec2(0.052500, 0.029167) * time;
    ...
    rim = clamp(rim + 0.6761 * rimLine + 0.5407 * fresnel, 0.0, 1.4);
    ...
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.y * 1.0000 - -1.0 * time * 0.097500 + 0.0957)), 22.8179) * motifBand;
    ...
    float refrAmp = min((1.2241 + 1.0333 * fresnel) / max(length(worldPos), 1.0), 0.1707);
    ...
    float energyW = clamp(0.3510 + 0.4815 * clamp(pattern, 0.0, 1.0) + 0.1482 * fresnel, 0.0, 0.8839);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.6782;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2299) * 0.8153;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3636 * (1.0 - 0.5389 * baseLuma)), 0.0, 1.0);
    // strobe-free emission drive (see the v7 note above): only smooth,
    // non-gated terms may modulate the emissive add
    float emitDrive = clamp(0.3850 * deepPat + 0.7805 * texDetail + 0.8120 * rim, 0.0, 1.0);
    float emit = atlas.a * (0.4259 + 0.6457 * emitDrive) * (1.0 - 0.1338 * (0.5 + 0.5 * sin(time * 0.282743))) * atlasPoleW;
    rgb += emit * 0.7307 * (1.0 - 0.6161 * baseLuma) * emitCol;
