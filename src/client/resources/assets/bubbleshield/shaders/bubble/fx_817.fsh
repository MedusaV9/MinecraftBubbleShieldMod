...
// stack: deep=fbm x4 taps | mid=plasmaglobe+warp2+rotate | rim=graze_sparkle | flourish=echo | tex=circuit | motif=sigilx4+doublepulse
// fbm: ridged x5 octaves | seed 20532560e34c3fc2
// v8 realism: refr 1.4497+1.7806*fres/dist (cap 0.1523) | energy 0.2309+0.4165*p | flow 0.091667 c/s | soft 0.7261m
...
void main() {
    ...
    float deepBreath = 1.0 + 0.0181 * sin(time * 1.806416);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.094248) * (sdir * (1.6333 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.3566 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.089012) * (sdir * (2.2469 * deepBreath)) + par * 0.0729, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.3849), deepStop, 0.1411);
    deepTrans *= 1.0 - 0.3566 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * (2.8605 * deepBreath)) + par * 0.1459, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.7699), deepStop, 0.2823);
    deepTrans *= 1.0 - 0.3566 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * (3.4740 * deepBreath)) + par * 0.2188, time);
    ...
    vec2 texUV = wuv * 3.0000 + vec2(-0.000833, -0.035833) * time;
    ...
    vec3 hgtW = vec3(0.4001, 0.3424, 0.2576);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -1.6102;
    ...
    mid = clamp(mid * mix(1.0, 0.6774 + 0.6777 * texDetail, atlasPoleW) + 0.1798 * texDetail * (0.2750 + 0.7250 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.5769 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.4346);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0795).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0795).rgb;
    float deepTex = 0.5 * (dot(deepTexA, hgtW) + dot(deepTexB, hgtW));

    // [layer:thick:chord]
    // v11 volumetric thickness: the membrane is a shell of relative
    // thickness rho = 0.0515 (energy material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.6335.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9485 * 0.9485 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.6335);
    float chordN = min(chord / 0.0515, 3.0);
    ...
    rim = clamp(rim + 0.8072 * rimLine + 0.6690 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2176 * ringPulse(baseUV + vec2(0.13, 0.31), time);
    float grain = 0.0709 * (cellHash(floor(wuv * 42.0000) + vec2(floor(time * 2.0), 0.0), 168.0000) - 0.5);
    ...
    float motifP = fract(time * 0.250000 + 0.0141);
    float motifEnv = 0.52 + 0.75 * exp(-140.0 * (motifP - 0.16) * (motifP - 0.16)) + 0.60 * exp(-140.0 * (motifP - 0.38) * (motifP - 0.38));
    float motifA = baseUV.x * 9.0000 + -1.0 * time * 0.056667 + 0.3099;
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 9.2514);
    float motifDy = (baseUV.y - 0.5410) * 8.5094;
    ...
    float pattern = clamp(0.1785 * deepPat * mix(1.0, chordN, 0.2200) + 0.7939 * mid + 0.5365 * rim + flourish + grain + 0.1622 * v5Par + 0.1431 * deepTex, 0.0, 1.5);
    ...
    float texBody = 0.8572 + 0.3490 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.5367 + 0.4633 * clamp(pattern, 0.0, 1.0)));
    ...
    rgb *= (1.0 - 0.3361 * washHi) * (1.0 - 0.2787 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.2489, 0.9073, 0.3390);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.5536) / 1.5536, 0.0, 1.0);
    rgb *= 0.5992 + 0.5251 * keyDiff * (1.0 - 0.5047 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 20.7363);
    rgb += hotStop * (keySpec * 0.3267 * (1.0 - 0.6665 * washHi));
    ...
    float refrAmp = min((1.4497 + 1.7806 * fresnel) / max(length(worldPos), 1.0), 0.1523);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.8152) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0717, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9283, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.8162 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.3068);
    float energyW = clamp(0.2309 + 0.4165 * clamp(pattern, 0.0, 1.0) + 0.3030 * fresnel, 0.0, 0.8412);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2770 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (0.8824 * (1.0 - 0.6214 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2803) * 1.0593;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2896 * max(0.0, 1.0 - 0.5211 * baseLuma - 0.5445 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3485 + 0.5399 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1704 * (0.5 + 0.5 * sin(time * 0.261799))) * atlasPoleW;
    rgb += emit * 0.8022 * (1.0 - 0.6446 * baseLuma) * (1.0 - 0.4789 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    float washKnee = 1.0 - 0.2074 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.9237));
    ...
    float bodyAlpha = 0.3225 + 0.2326 * presence + 0.5058 * pattern + 0.1097 * (1.0 - deepTrans) + 0.1901 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.2210;
    // [layer:v5:backface]
    ...
    bodyAlpha *= 0.6962 + 0.3038 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    bodyAlpha *= 1.0 + (v5Dither - 0.5) * 0.1208;
    // [layer:inner:current]
    ...
    float innerGlow = deepField(rotA(spinAxis, -time * 0.073304 + 2.1064) * (sdir * 1.8763), time);
    vec3 innerCol = mix(hotStop, secCol, clamp(innerGlow * 1.4893, 0.0, 1.0));
    rgb = mix(rgb, innerCol, innerFace * 0.2567);
    bodyAlpha *= 1.0 + innerFace * 0.1051;
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9651) * vertexColor.a;
