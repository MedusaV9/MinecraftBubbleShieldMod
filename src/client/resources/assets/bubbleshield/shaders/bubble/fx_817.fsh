...
// v8 realism: refr 1.2997+1.5964*fres/dist (cap 0.1523) | energy 0.3371+0.4165*p | flow 0.091667 c/s | soft 0.7261m
...
void main() {
    ...
    rim = clamp(rim + 0.8072 * rimLine + 0.4492 * fresnel, 0.0, 1.4);
    ...
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 12.8771);
    float motifDy = (baseUV.y - 0.5410) * 11.7053;
    ...
    float refrAmp = min((1.2997 + 1.5964 * fresnel) / max(length(worldPos), 1.0), 0.1523);
    ...
    float energyW = clamp(0.3371 + 0.4165 * clamp(pattern, 0.0, 1.0) + 0.2054 * fresnel, 0.0, 0.8412);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.5778;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2803) * 0.6916;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2896 * (1.0 - 0.5211 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.3485 + 0.5399 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1704 * (0.5 + 0.5 * sin(time * 0.261799))) * atlasPoleW;
    rgb += emit * 0.8022 * (1.0 - 0.6446 * baseLuma) * emitCol;
    // [layer:v5:softknee]
