...
void main() {
    ...
    mid = clamp(mid * (0.6780 + 0.4604 * texDetail) + 0.2320 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5295), 0.0, 1.0);
    float emit = atlas.a * (0.7430 + 0.3969 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1003 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8687 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3452 + 0.5073 * presence + 0.3345 * pattern + 0.1166 * (1.0 - deepTrans) + 0.1580 * emit, 0.9074);
