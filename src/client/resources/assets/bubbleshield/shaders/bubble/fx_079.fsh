...
// stack: deep=fbm x3 taps | mid=rings+curl+pulse | rim=graze_film | flourish=shimmer | tex=ridged
// fbm: ridged x5 octaves | seed b1d209dc8d09594c
// v8 realism: refr 0.7065+1.0230*fres/dist (cap 0.1527) | energy 0.4193+0.5480*p | flow 0.108333 c/s | soft 0.7054m
...
void main() {
    ...
    float refrAmp = min((0.7065 + 1.0230 * fresnel) / max(length(worldPos), 1.0), 0.1527);
    ...
    float emit = atlas.a * (0.4158 + 0.6262 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1003 * (0.5 + 0.5 * sin(time * 0.178024))) * atlasPoleW;
