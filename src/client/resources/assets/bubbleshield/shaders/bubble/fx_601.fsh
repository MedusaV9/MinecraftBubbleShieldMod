...
void main() {
    ...
    mid = clamp(mid * (0.6091 + 0.6615 * texDetail) + 0.3378 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3817), 0.0, 1.0);
    float emit = atlas.a * (0.7509 + 0.3578 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0933 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.7481 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3206 + 0.3884 * presence + 0.3091 * pattern + 0.1473 * (1.0 - deepTrans) + 0.1880 * emit, 0.9332);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1837), 0.9332);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7409 + 0.2591 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1214), 0.0, 1.0);
