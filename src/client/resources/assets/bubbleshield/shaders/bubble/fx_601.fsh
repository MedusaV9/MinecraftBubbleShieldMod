...
// stack: deep=caustic x4 taps | mid=raymarchfog+none+rotate | rim=graze_sparkle | flourish=swirl
...
void main() {
    ...
    float mid = clamp(rfLight * 0.9101 + (1.0 - rfTrans) * 0.1736, 0.0, 1.15);
    ...
    float alpha = vertexColor.a * min(0.0406 + 0.3284 * presence + 0.3091 * pattern + 0.1473 * (1.0 - deepTrans), 0.8732);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1837), 0.8732);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5084 + 0.4916 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
