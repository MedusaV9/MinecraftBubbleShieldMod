...
void main() {
    ...
    mid = clamp(mid * (0.6123 + 0.5592 * texDetail) + 0.2170 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4345), 0.0, 1.0);
    float emit = atlas.a * (0.7746 + 0.4897 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0942 * (0.5 + 0.5 * sin(time * 0.293215)));
    ...
    float alpha = vertexColor.a * min(0.2921 + 0.2631 * presence + 0.4951 * pattern + 0.1421 * (1.0 - deepTrans) + 0.1465 * emit, 0.8841);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3343), 0.8841);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6394 + 0.3606 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
