...
void main() {
    ...
    mid = clamp(mid * (0.6188 + 0.7234 * texDetail) + 0.2080 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4368), 0.0, 1.0);
    float emit = atlas.a * (0.7254 + 0.4621 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1381 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9142 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3255 + 0.3354 * presence + 0.5998 * pattern + 0.0909 * (1.0 - deepTrans) + 0.1634 * emit, 0.8658);
