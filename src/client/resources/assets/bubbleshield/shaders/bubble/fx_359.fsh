...
// Bubble surface shader fx_359 -- family PORTALVOID
// stack: deep=ridge x3 taps | mid=portalvoid+warp2+scroll | rim=lat | flourish=glint
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.6192 * (vec2(fbm2(p + vec2(0.0, t * 0.310000), per),
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7706, 0.0, 1.0);
    ...
    deepCol += 0.4766 * dp * mix(baseCol, deepStop, 0.4225);
    deepPat += 0.4766 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.8145) + par * 0.1055, time);
    deepCol += 0.2271 * dp * mix(baseCol, deepStop, 0.8450);
    deepPat += 0.2271 * dp;
    deepCol *= 0.6575;
    deepPat = pow(clamp(deepPat * 0.5870, 0.0, 1.0), 1.0944);

    // [layer:mid:portalvoid_warp2_scroll]
    // Signature structure of this effect, domain-warped and animated.
    vec2 auv = vec2(baseUV.x * 4.0000, baseUV.y * 4.0000) + vec2(-0.476667, -0.100000) * time;
    ...
    float infall = 0.5 + 0.5 * sin(prad * 14.8411 + time * 1.513200);
    float suck = pow(clamp(0.5 + 0.5 * sin(pang * 3.0000 + prad * 7.0723 + time * 0.769690), 0.0, 1.0), 3.5325);
    float voidCore = invsmooth(0.2180, 0.3251, prad);
    float accretion = invsmooth(0.0, 0.0661, abs(prad - 0.4784));
    ...
    float grain = 0.0878 * (cellHash(floor(wuv * 51.0000) + vec2(floor(time * 6.0), 0.0), 204.0000) - 0.5);
    ...
    rgb += hotStop * 0.1664 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0577 + 0.3300 * presence + 0.4998 * pattern, 0.8058);
