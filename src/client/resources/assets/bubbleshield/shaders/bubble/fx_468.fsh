...
void main() {
    ...
    mid = clamp(mid * (0.7143 + 0.4657 * texDetail) + 0.2033 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4937), 0.0, 1.0);
    float emit = atlas.a * (0.7992 + 0.4386 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1216 * (0.5 + 0.5 * sin(time * 0.293215)));
    ...
    float alpha = vertexColor.a * min(0.3862 + 0.3320 * presence + 0.3856 * pattern + 0.1057 * (1.0 - deepTrans) + 0.1367 * emit, 0.9073);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2584), 0.9073);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7394 + 0.2606 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
