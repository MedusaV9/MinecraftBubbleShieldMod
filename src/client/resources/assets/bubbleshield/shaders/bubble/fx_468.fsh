...
// stack: deep=ridge x3 taps | mid=aethersmoke+curl+rotate | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    float flourish = 0.2650 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0724 + 0.2520 * presence + 0.3856 * pattern + 0.1057 * (1.0 - deepTrans), 0.8473);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2584), 0.8473);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5068 + 0.4932 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
