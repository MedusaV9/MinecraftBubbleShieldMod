...
// stack: deep=fbm x4 taps | mid=volumecloud+warp1+rotate | rim=graze_sparkle | flourish=shimmer | tex=fbm_turbulence
// fbm: turb x6 octaves | seed 9db4ef4e5c0cb9fb
// v8 realism: refr 0.3086+0.3251*fres/dist (cap 0.1513) | energy 0.3490+0.4440*p | flow 0.115000 c/s | soft 1.5715m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (0.6878 * atlasPoleW));
    ...
    float refrAmp = min((0.3086 + 0.3251 * fresnel) / max(length(worldPos), 1.0), 0.1513);
