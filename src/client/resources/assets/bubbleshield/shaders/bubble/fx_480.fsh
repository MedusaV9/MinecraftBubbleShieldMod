...
void main() {
    ...
    mid = clamp(mid * (0.6237 + 0.4998 * texDetail) + 0.2786 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.6822 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4858), 0.0, 1.0);
    float emit = atlas.a * (0.7590 + 0.3414 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1665 * (0.5 + 0.5 * sin(time * 0.162316)));
    ...
    float alpha = vertexColor.a * min(0.3155 + 0.2746 * presence + 0.4043 * pattern + 0.1349 * (1.0 - deepTrans) + 0.1314 * emit, 0.8251);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3282), 0.8251);
    // [layer:v5:ghostalpha]
