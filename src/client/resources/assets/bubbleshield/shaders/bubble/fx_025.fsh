...
void main() {
    ...
    mid = clamp(mid * (0.7476 + 0.6574 * texDetail) + 0.2631 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.8813 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5201), 0.0, 1.0);
    float emit = atlas.a * (0.7809 + 0.3133 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1268 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.8124 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3293 + 0.4440 * presence + 0.4569 * pattern + 0.1322 * (1.0 - deepTrans) + 0.2006 * emit, 0.8809);
