...
// stack: deep=voro x4 taps | mid=rings+curl+scroll | rim=graze_sparkle | flourish=glint | tex=ridged
// fbm: turb x5 octaves | seed f8d39d0eb63b4065
// v8 realism: refr 0.6157+0.8222*fres/dist (cap 0.1391) | energy 0.4189+0.5389*p | flow 0.074167 c/s | soft 0.6196m
...
void main() {
    ...
    float refrAmp = min((0.6157 + 0.8222 * fresnel) / max(length(worldPos), 1.0), 0.1391);
    ...
    float emit = atlas.a * (0.4386 + 0.5260 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1268 * (0.5 + 0.5 * sin(time * 0.214675))) * atlasPoleW;
