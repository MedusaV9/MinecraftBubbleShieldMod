...
// stack: deep=caustic x3 taps | mid=circuit+curl+flicker | rim=graze | flourish=echo | tex=circuit
// fbm: turb x5 octaves | seed d48d1d489cfb5131
// v8 realism: refr 0.5201+0.4658*fres/dist (cap 0.1384) | energy 0.3661+0.4437*p | flow 0.050833 c/s | soft 0.8266m
...
void main() {
    ...
    float refrAmp = min((0.5201 + 0.4658 * fresnel) / max(length(worldPos), 1.0), 0.1384);
