...
void main() {
    ...
    mid = clamp(mid * (0.6168 + 0.5135 * texDetail) + 0.2280 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3997), 0.0, 1.0);
    float emit = atlas.a * (0.7190 + 0.2735 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1532 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.8025 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2911 + 0.2928 * presence + 0.5199 * pattern + 0.1119 * (1.0 - deepTrans) + 0.1247 * emit, 0.8973);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1619), 0.8973);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6433 + 0.3567 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0873), 0.0, 1.0);
