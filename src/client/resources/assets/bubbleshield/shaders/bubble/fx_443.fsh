...
// stack: deep=fbm x4 taps | mid=holoparallax+curl+scroll | rim=graze_film | flourish=shimmer
...
void main() {
    ...
    float alpha = vertexColor.a * min(0.0509 + 0.1528 * presence + 0.5199 * pattern + 0.1119 * (1.0 - deepTrans), 0.8373);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1619), 0.8373);
    // [layer:v5:ghostalpha]
