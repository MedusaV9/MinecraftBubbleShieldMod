...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8733, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0525, abs(glR - 0.1470));
    float mid = clamp(glStars * (1.0 + glRing * 1.2211) + glRing * 0.3855, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.7152 + 0.4620 * texDetail) + 0.3160 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8584 + 0.3460 * texDetail;
    rgb = satLift(rgb, 1.2697);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4170), 0.0, 1.0);
    float emit = atlas.a * (0.7250 + 0.2977 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0954 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.9340 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3268 + 0.2746 * presence + 0.5329 * pattern + 0.1149 * (1.0 - deepTrans) + 0.1774 * emit, 0.8886);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2876), 0.8886);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6609 + 0.3391 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1309), 0.0, 1.0);
