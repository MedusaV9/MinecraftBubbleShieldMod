...
void main() {
    ...
    mid = clamp(mid * (0.6856 + 0.5722 * texDetail) + 0.3156 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4309), 0.0, 1.0);
    float emit = atlas.a * (0.7400 + 0.3502 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1073 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.8287 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4329 + 0.5552 * presence + 0.3682 * pattern + 0.1171 * (1.0 - deepTrans) + 0.2089 * emit, 0.9471);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2502), 0.9471);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6821 + 0.3179 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0898), 0.0, 1.0);
