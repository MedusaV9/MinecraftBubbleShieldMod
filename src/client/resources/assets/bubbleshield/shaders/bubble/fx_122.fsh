...
void main() {
    ...
    mid = clamp(mid * (0.6020 + 0.6776 * texDetail) + 0.1890 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4526), 0.0, 1.0);
    float emit = atlas.a * (0.6990 + 0.4428 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0995 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9052 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3820 + 0.4740 * presence + 0.4848 * pattern + 0.1155 * (1.0 - deepTrans) + 0.1796 * emit, 0.8419);
