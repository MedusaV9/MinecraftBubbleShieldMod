...
void main() {
    ...
    mid = clamp(mid * (0.6315 + 0.6480 * texDetail) + 0.2867 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4978), 0.0, 1.0);
    float emit = atlas.a * (0.6118 + 0.3111 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1276 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.9152 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3727 + 0.4456 * presence + 0.3229 * pattern + 0.1290 * (1.0 - deepTrans) + 0.1988 * emit, 0.9064);
