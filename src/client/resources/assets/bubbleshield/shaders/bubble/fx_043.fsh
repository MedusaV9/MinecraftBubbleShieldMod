...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5972, 0.0, 1.0);
    ...
    deepCol += 0.4756 * dp * mix(baseCol, deepStop, 0.2290);
    deepPat += 0.4756 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.8581) + par * 0.0842, time);
    deepCol += 0.2262 * dp * mix(baseCol, deepStop, 0.4579);
    deepPat += 0.2262 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 6.0817) + par * 0.1263, time);
    deepCol += 0.1076 * dp * mix(baseCol, deepStop, 0.6869);
    deepPat += 0.1076 * dp;
    deepCol *= 0.5677;
    deepPat = pow(clamp(deepPat * 0.5527, 0.0, 1.0), 1.4161);
    ...
    rgb += hotStop * 0.2596 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0551 + 0.3056 * presence + 0.3229 * pattern, 0.8464);
