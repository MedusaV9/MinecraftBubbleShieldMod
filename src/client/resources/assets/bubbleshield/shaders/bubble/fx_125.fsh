...
// stack: deep=fbm x3 taps | mid=rings+warp2+rotate | rim=lat | flourish=shimmer | tex=ridged
// fbm: turb x5 octaves | seed b18e17733da7e1e1
// v8 realism: refr 0.6953+0.8928*fres/dist (cap 0.1204) | energy 0.4046+0.4330*p | flow 0.070833 c/s | soft 0.8007m
...
void main() {
    ...
    float refrAmp = min((0.6953 + 0.8928 * fresnel) / max(length(worldPos), 1.0), 0.1204);
    ...
    float emit = atlas.a * (0.3813 + 0.5206 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1734 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
