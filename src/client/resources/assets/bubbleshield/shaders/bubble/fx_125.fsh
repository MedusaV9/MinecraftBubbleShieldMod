...
// stack: deep=fbm x2 taps | mid=rings+warp2+rotate | rim=lat | flourish=shimmer
...
void main() {
    ...
    deep += 0.4648 * deepField(duvBase + rimDir * 0.0776 + vec2(-0.049167, 0.046667) * time, time);
    deep = pow(clamp(deep * 0.6827, 0.0, 1.0), 1.6433);
    ...
    float pattern = clamp(0.4040 * deep + 0.8568 * mid + 0.7185 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.3807 + pattern * 0.5352);
    vec3 rgb = vertexColor.rgb * (0.3881 + 0.9094 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1626 + 0.6412 * pattern, 0.0, 1.0);
