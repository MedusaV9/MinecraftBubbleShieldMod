...
void main() {
    ...
    mid = clamp(mid * (0.6298 + 0.4521 * texDetail) + 0.2468 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4181), 0.0, 1.0);
    float emit = atlas.a * (0.6520 + 0.2528 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1626 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.6929 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4418 + 0.4870 * presence + 0.3545 * pattern + 0.1008 * (1.0 - deepTrans) + 0.1587 * emit, 0.9129);
