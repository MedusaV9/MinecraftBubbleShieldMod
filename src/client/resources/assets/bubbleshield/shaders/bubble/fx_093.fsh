...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8048, 0.0, 1.0);
    ...
    deepCol += 0.4966 * dp * mix(baseCol, deepStop, 0.2134);
    deepPat += 0.4966 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.6938) + par * 0.1384, time);
    deepCol += 0.2466 * dp * mix(baseCol, deepStop, 0.4268);
    deepPat += 0.2466 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 5.6892) + par * 0.2076, time);
    deepCol += 0.1225 * dp * mix(baseCol, deepStop, 0.6402);
    deepPat += 0.1225 * dp;
    deepCol *= 0.5265;
    deepPat = pow(clamp(deepPat * 0.5360, 0.0, 1.0), 1.3030);
    ...
    rgb += hotStop * 0.2092 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0784 + 0.3470 * presence + 0.3545 * pattern, 0.8529);
