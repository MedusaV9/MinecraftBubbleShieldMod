...
float deepField(vec2 p, float t) {
    return caustic(p, vec2(0.0, 0.0), vec2(3.0000, 4.0000));
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.002500, -0.020000) * time, time);
    deep += 0.4966 * deepField(duvBase + rimDir * 0.0692 + vec2(-0.005000, -0.033333) * time, time);
    deep += 0.2466 * deepField(duvBase + rimDir * 0.1384 + vec2(-0.007500, -0.043333) * time, time);
    deep += 0.1225 * deepField(duvBase + rimDir * 0.2076 + vec2(-0.007500, -0.053333) * time, time);
    deep = pow(clamp(deep * 0.5360, 0.0, 1.0), 1.3030);
    ...
    float pattern = clamp(0.4254 * deep + 0.8796 * mid + 0.5901 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2867 + pattern * 0.5648);
    vec3 rgb = vertexColor.rgb * (0.4685 + 0.7834 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1583 + 0.7388 * pattern, 0.0, 1.0);
