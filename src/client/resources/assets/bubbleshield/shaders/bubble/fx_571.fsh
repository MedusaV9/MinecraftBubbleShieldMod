...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8301, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0391, abs(glR - 0.1552));
    float mid = clamp(glStars * (1.0 + glRing * 1.3313) + glRing * 0.3964, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6430 + 0.5642 * texDetail) + 0.1839 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8550 + 0.3545 * texDetail;
    rgb = satLift(rgb, 1.3334);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3598), 0.0, 1.0);
    float emit = atlas.a * (0.7108 + 0.3602 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1272 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7531 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2562 + 0.2485 * presence + 0.5171 * pattern + 0.1400 * (1.0 - deepTrans) + 0.1791 * emit, 0.8768);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2007), 0.8768);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6673 + 0.3327 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1006), 0.0, 1.0);
