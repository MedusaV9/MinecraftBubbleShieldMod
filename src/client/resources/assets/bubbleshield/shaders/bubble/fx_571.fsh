...
// stack: deep=ridge x4 taps | mid=gravlens+warp2+rotate | rim=graze_sparkle | flourish=echo | tex=starfield
// fbm: ridged x5 octaves | seed ee67a1873c7eb4ab
// v8 realism: refr 1.3907+1.7929*fres/dist (cap 0.1188) | energy 0.2273+0.4082*p | flow 0.103333 c/s | soft 0.5923m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.0549 + 0.2257, 0.0, 1.0);
    ...
    float refrAmp = min((1.3907 + 1.7929 * fresnel) / max(length(worldPos), 1.0), 0.1188);
    ...
    float emitMask = clamp(atlas.a * 2.2338, 0.0, 1.0);
    float emit = emitMask * (0.3965 + 0.5823 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1272 * (0.5 + 0.5 * sin(time * 0.178024))) * atlasPoleW;
