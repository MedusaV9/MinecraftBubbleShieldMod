...
void main() {
    ...
    mid = clamp(mid * (0.6067 + 0.4834 * texDetail) + 0.2199 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3803), 0.0, 1.0);
    float emit = atlas.a * (0.5505 + 0.3284 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1083 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.7166 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3446 + 0.4823 * presence + 0.3967 * pattern + 0.0932 * (1.0 - deepTrans) + 0.2012 * emit, 0.8779);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2919), 0.8779);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6834 + 0.3166 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
