...
void main() {
    ...
    mid = clamp(mid * (0.6778 + 0.5348 * texDetail) + 0.2309 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4154), 0.0, 1.0);
    float emit = atlas.a * (0.6719 + 0.4133 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1410 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.8329 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3688 + 0.4884 * presence + 0.4583 * pattern + 0.1556 * (1.0 - deepTrans) + 0.1331 * emit, 0.8458);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3003), 0.8458);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6870 + 0.3130 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
