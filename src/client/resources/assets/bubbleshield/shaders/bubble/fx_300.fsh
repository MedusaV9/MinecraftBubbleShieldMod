...
void main() {
    ...
    mid = clamp(mid * (0.6652 + 0.6448 * texDetail) + 0.3466 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5159), 0.0, 1.0);
    float emit = atlas.a * (0.6101 + 0.2962 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1046 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.9862 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4351 + 0.5214 * presence + 0.3539 * pattern + 0.1408 * (1.0 - deepTrans) + 0.2013 * emit, 0.8806);
