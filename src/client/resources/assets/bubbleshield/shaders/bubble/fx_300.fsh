...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7061, 0.0, 1.0);
    ...
    deepCol += 0.4766 * dp * mix(baseCol, deepStop, 0.4354);
    deepPat += 0.4766 * dp;
    dp = deepField(rotA(spinAxis, time * 0.094248) * (sdir * 3.9525) + par * 0.1595, time);
    deepCol += 0.2271 * dp * mix(baseCol, deepStop, 0.8708);
    deepPat += 0.2271 * dp;
    deepCol *= 0.6579;
    deepPat = pow(clamp(deepPat * 0.5870, 0.0, 1.0), 1.4206);
    ...
    float midCover = clamp(0.8931 * mid + 0.5462 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.3130);
    rgb += hotStop * 0.1974 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0671 + 0.3198 * presence + 0.3378 * pattern, 0.7806);
