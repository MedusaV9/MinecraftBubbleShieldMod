...
void main() {
    ...
    mid = clamp(mid * (0.6343 + 0.6979 * texDetail) + 0.1836 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4786), 0.0, 1.0);
    float emit = atlas.a * (0.6724 + 0.4774 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0956 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.9127 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3470 + 0.4001 * presence + 0.4015 * pattern + 0.1380 * (1.0 - deepTrans) + 0.1944 * emit, 0.8565);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2310), 0.8565);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6257 + 0.3743 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1348), 0.0, 1.0);
