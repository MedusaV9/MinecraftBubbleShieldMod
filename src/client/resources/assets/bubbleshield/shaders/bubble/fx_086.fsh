...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7672, 0.0, 1.0);
    ...
    deepCol += 0.4583 * dp * mix(baseCol, deepStop, 0.2997);
    deepPat += 0.4583 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 4.4721) + par * 0.1285, time);
    deepCol += 0.2100 * dp * mix(baseCol, deepStop, 0.5994);
    deepPat += 0.2100 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 5.4369) + par * 0.1928, time);
    deepCol += 0.0963 * dp * mix(baseCol, deepStop, 0.8991);
    deepPat += 0.0963 * dp;
    deepCol *= 0.5834;
    deepPat = pow(clamp(deepPat * 0.5667, 0.0, 1.0), 1.6025);
    ...
    rgb += hotStop * 0.2081 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0642 + 0.3014 * presence + 0.3314 * pattern, 0.8484);
