...
// stack: deep=fbm x2 taps | mid=vortex+curl+flicker | rim=graze_sparkle | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.030000, 0.025000) * time, time);
    deep += 0.4148 * deepField(duvBase + rimDir * 0.0359 + vec2(-0.046667, 0.037500) * time, time);
    deep = pow(clamp(deep * 0.7068, 0.0, 1.0), 1.5785);
    ...
    float pattern = clamp(0.3452 * deep + 0.8197 * mid + 0.5306 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5302 + pattern * 0.6419);
    vec3 rgb = vertexColor.rgb * (0.3954 + 0.8995 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2068 + 0.6029 * pattern, 0.0, 1.0);
