...
// Bubble surface shader fx_126 -- family VORTEX
// stack: deep=fbm x3 taps | mid=vortex+curl+flicker | rim=graze_sparkle | flourish=shimmer | tex=marble_ink | motif=scanx1+sparklegate
// fbm: ridged x6 octaves | seed 329c9cc2aad85983
// v8 realism: refr 1.3376+1.8234*fres/dist (cap 0.1876) | energy 0.2382+0.4632*p | flow 0.055833 c/s | soft 0.8107m
...
vec2 spiralWarp(vec2 uv, float t) {
    ...
    float a = safeAtan(dv, sin(du * 6.2831853) * 0.5) + 5.2990 * r - t;
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.5483, 1.0398, 1.2354) * t
        + vec3(0.5916, 0.5243, 0.8770)));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0045, 0.0526, g);
}
...
float sparkle(vec2 p, float t, float px) {
    ...
    return step(0.7522, h) * invsmooth(0.02, 0.22, d) * tw;
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.6906;
    vec3 spun = clamp(hueSpin(baseCol, -0.6760), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.3197) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5374, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.7280);
    ...
    vec3 secCol = clamp(satLift(clamp(hueSpin(baseCol, 1.9850), 0.0, 1.0), 1.2500) * 0.8392, 0.0, 1.0);
    ...
    vec3 spinAxis = vec3(0.6477, 0.1209, 0.7522);
    vec3 par = Vt * 2.3574;
    ...
    float deepBreath = 1.0 + 0.0178 * sin(time * 1.267109);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * (1.7561 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.5839 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * (2.6201 * deepBreath)) + par * 0.0359, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.5411), deepStop, 0.1984);
    deepTrans *= 1.0 - 0.5839 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * (3.4840 * deepBreath)) + par * 0.0717, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.3968);
    deepTrans *= 1.0 - 0.5839 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.9510;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0776, 0.0, 1.0), 1.5785);
    ...
    float ft = time + 0.4549 * (0.6 * sin(time * 5.319764) + 0.4 * sin(time * 8.262389 + 6.1148));
    vec2 auv = vec2(baseUV.x * 4.0000, baseUV.y * 4.0000) + vec2(0.256667, -0.440000) * ft;
    vec2 wuv = auv + 0.2373 * curl2(auv + vec2(0.0, time * 0.070000), midPer);
    float latDist = 1.0 - abs(baseUV.y * 2.0 - 1.0);
    float twist = baseUV.x * 6.2831853 + latDist * 8.8697 - time * 1.120501;
    float band = smoothstep(0.15, 0.9, sin(twist * 3.0000));
    float arms = smoothstep(0.45, 0.95, sin(baseUV.x * 6.2831853 - latDist * 4.7948 + time * 0.471239));
    float eye = clamp(pow(clamp(1.0 - latDist, 0.0, 1.0), 3.0) * (0.6 + 0.4 * sin(time * 1.701696)), 0.0, 1.0);
    vec2 sv = spiralWarp(baseUV, time * 0.120428);
    // sv is periodic in u by construction, so the noise needs no wrap
    float mid = clamp(band * 0.8 + arms * 0.5 + eye + fbm2(sv * 4.4871, vec2(64.0000, 64.0000)) * 0.18, 0.0, 1.3);
    ...
    vec2 texUV = wuv * 4.0000 + vec2(0.020833, -0.078333) * time;
    ...
    vec3 hgtW = vec3(0.3743, 0.3448, 0.2809);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * 2.4990;
    ...
    float texDetail = clamp(atlas.r * 0.4588 + atlas.g * 0.4227 + atlas.b * 0.3443, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6314 + 0.6385 * texDetail, atlasPoleW) + 0.1642 * texDetail * (0.3244 + 0.6756 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.4706 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.5537);
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.7384.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9293 * 0.9293 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.7384);
    ...
    float rimLine = smoothstep(0.0641, 0.0983, rimSlope);
    float rimGlint = sparkle(baseUV * 10.0000, time, 10.0000);
    float rim = rimGraze() * 0.8552 * (0.7 + 0.6 * rimGlint) + 0.3760 * rimGlint;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.5203 * rimLine + 0.8316 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1223 * caustic(wuv + vec2(3.1, 8.7), vec2(0.143333, -0.120000) * time, midPer);
    float grain = 0.0575 * (cellHash(floor(wuv * 43.0000) + vec2(floor(time * 2.0), 0.0), 172.0000) - 0.5);
    ...
    float motifEnv = 0.48 + 0.52 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.518363)) * (0.5 + 0.5 * sin(time * 0.738274 + 1.7)));
    float motifBand = smoothstep(0.06, 0.22, baseUV.y) * smoothstep(0.06, 0.22, 1.0 - baseUV.y);
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.x * 2.0000 - -1.0 * time * 0.147500 + 0.2911)), 9.0141) * motifBand;
    ...
    float pattern = clamp(0.3452 * deepPat * mix(1.0, chordN, 0.2124) + 0.8197 * mid + 0.5306 * rim + flourish + grain + 0.1902 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.9474 + rim * 0.1832, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9194 * mid + 0.4432 * rim, 0.0, 1.0);
    ...
    float texBody = 0.7867 + 0.5214 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4203 + 0.5797 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.3018);
    ...
    vec3 softHot = 1.0 - exp(-2.4057 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.3262 * smoothstep(0.55, 1.10, pattern));
    vec3 accent = accentPalette(0.5302 + pattern * 0.6419);
    rgb = mix(rgb, rgb * (0.55 + 0.9 * accent), 0.2546);
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.0325 + baseUV.y * 0.5353 + 0.5971) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1129);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.5487 + baseUV.y * 0.31 + 0.8810) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2090);
    ...
    rgb *= (1.0 - 0.2920 * washHi) * (1.0 - 0.2657 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.0073, 0.7164, -0.6977);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.3608) / 1.3608, 0.0, 1.0);
    rgb *= 0.6698 + 0.5150 * keyDiff * (1.0 - 0.5878 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 22.1674);
    rgb += hotStop * (keySpec * 0.4271 * (1.0 - 0.6235 * washHi));
    ...
    float refrAmp = min((1.3376 + 1.8234 * fresnel) / max(length(worldPos), 1.0), 0.1876);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.0362) * refrAmp;
    ...
    refracted *= exp(-0.3976 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.2087);
    float energyW = clamp(0.2382 + 0.4632 * clamp(pattern, 0.0, 1.0) + 0.3049 * fresnel, 0.0, 0.8115);
    // v10 wash guard on the overlay: white palettes show MORE refracted
    // scene instead of piling white energy on white glass.
    energyW *= 1.0 - 0.2163 * washHi;
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * (0.9459 * (1.0 - 0.6011 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2148) * 0.9896;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3706 * max(0.0, 1.0 - 0.5744 * baseLuma - 0.5951 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.4336 + 0.6069 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0990 * (0.5 + 0.5 * sin(time * 0.198968))) * atlasPoleW;
    rgb += emit * 0.7123 * (1.0 - 0.6065 * baseLuma) * (1.0 - 0.5479 * washHi) * emitCol;
    // [layer:wash:knee]
    ...
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.3953));
    ...
    float bodyAlpha = 0.3976 + 0.4463 * presence + 0.4062 * pattern + 0.1441 * (1.0 - deepTrans) + 0.1260 * emit;
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 0.9045 + hgtC * 0.4156, 0.0, 1.0);
    ...
    bodyAlpha *= 1.0 + innerFace * 0.2034;
    ...
    bodyAlpha *= smoothstep(0.0, 0.8107, sceneVDist - fragVDist);
