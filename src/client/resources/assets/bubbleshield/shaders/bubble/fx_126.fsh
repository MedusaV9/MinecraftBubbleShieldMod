...
void main() {
    ...
    mid = clamp(mid * (0.6314 + 0.6385 * texDetail) + 0.2891 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5080), 0.0, 1.0);
    float emit = atlas.a * (0.7727 + 0.3808 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0990 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7123 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3976 + 0.4463 * presence + 0.4062 * pattern + 0.1441 * (1.0 - deepTrans) + 0.1260 * emit, 0.9110);
