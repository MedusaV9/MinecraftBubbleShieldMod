...
void main() {
    ...
    mid = clamp(mid * (0.6982 + 0.6394 * texDetail) + 0.2037 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4657), 0.0, 1.0);
    float emit = atlas.a * (0.5813 + 0.2707 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1253 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.7232 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3971 + 0.4128 * presence + 0.3464 * pattern + 0.0823 * (1.0 - deepTrans) + 0.1448 * emit, 0.8747);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3394), 0.8747);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6800 + 0.3200 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0673), 0.0, 1.0);
