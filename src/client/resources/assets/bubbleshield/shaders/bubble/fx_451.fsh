...
// stack: deep=fbm x3 taps | mid=gravlens+curl+rotate | rim=graze_sparkle | flourish=echo | tex=starfield
// fbm: ridged x4 octaves | seed afc517b3749c408b
// v8 realism: refr 1.1662+2.0773*fres/dist (cap 0.1148) | energy 0.2181+0.3641*p | flow 0.068333 c/s | soft 1.0816m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.3443 + 0.1939, 0.0, 1.0);
    ...
    float refrAmp = min((1.1662 + 2.0773 * fresnel) / max(length(worldPos), 1.0), 0.1148);
    ...
    float emitMask = clamp(atlas.a * 1.7502, 0.0, 1.0);
    float emit = emitMask * (0.4305 + 0.6801 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1086 * (0.5 + 0.5 * sin(time * 0.287979))) * atlasPoleW;
