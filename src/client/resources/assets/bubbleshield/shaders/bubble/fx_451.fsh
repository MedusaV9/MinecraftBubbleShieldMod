...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.7913, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0428, abs(glR - 0.1046));
    float mid = clamp(glStars * (1.0 + glRing * 1.3182) + glRing * 0.3946, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6353 + 0.6376 * texDetail) + 0.2924 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5454), 0.0, 1.0);
    float emit = atlas.a * (0.7675 + 0.4417 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1086 * (0.5 + 0.5 * sin(time * 0.287979)));
    ...
    float alpha = vertexColor.a * min(0.2645 + 0.2797 * presence + 0.4889 * pattern + 0.0828 * (1.0 - deepTrans) + 0.1803 * emit, 0.9183);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2939), 0.9183);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6657 + 0.3343 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
