...
void main() {
    ...
    mid = clamp(mid * (0.7301 + 0.4674 * texDetail) + 0.3418 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4273), 0.0, 1.0);
    float emit = atlas.a * (0.5813 + 0.3500 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1478 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7597 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3527 + 0.3165 * presence + 0.5208 * pattern + 0.1205 * (1.0 - deepTrans) + 0.1486 * emit, 0.8587);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2000), 0.8587);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7485 + 0.2515 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
