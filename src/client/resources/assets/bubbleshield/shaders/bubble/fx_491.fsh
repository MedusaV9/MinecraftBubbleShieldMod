...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8589, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0377, abs(glR - 0.1785));
    float mid = clamp(glStars * (1.0 + glRing * 1.8664) + glRing * 0.4102, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6296 + 0.6470 * texDetail) + 0.2434 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8421 + 0.3859 * texDetail;
    rgb = satLift(rgb, 1.0838);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3586), 0.0, 1.0);
    float emit = atlas.a * (0.6360 + 0.3974 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0925 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.7317 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2984 + 0.2589 * presence + 0.5347 * pattern + 0.1278 * (1.0 - deepTrans) + 0.1925 * emit, 0.8854);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2626), 0.8854);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6222 + 0.3778 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0980), 0.0, 1.0);
