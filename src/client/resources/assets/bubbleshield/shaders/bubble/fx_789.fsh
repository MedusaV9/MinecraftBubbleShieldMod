...
void main() {
    ...
    mid = clamp(mid * (0.7337 + 0.5508 * texDetail) + 0.1982 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4720), 0.0, 1.0);
    float emit = atlas.a * (0.6451 + 0.4062 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1693 * (0.5 + 0.5 * sin(time * 0.162316)));
    ...
    float alpha = vertexColor.a * min(0.4470 + 0.5247 * presence + 0.3850 * pattern + 0.1096 * (1.0 - deepTrans) + 0.1235 * emit, 0.9137);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1841), 0.9137);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0853), 0.0, 1.0);
