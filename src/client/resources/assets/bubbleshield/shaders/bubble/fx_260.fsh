...
void main() {
    ...
    mid = clamp(mid * (0.7038 + 0.4646 * texDetail) + 0.2059 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4777), 0.0, 1.0);
    float emit = atlas.a * (0.6988 + 0.4931 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1375 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9338 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3343 + 0.4176 * presence + 0.3234 * pattern + 0.1244 * (1.0 - deepTrans) + 0.1741 * emit, 0.9005);
