...
// stack: deep=ridge x3 taps | mid=petals+curl+pulse | rim=graze_film | flourish=shimmer | tex=scales
// fbm: ridged x6 octaves | seed 2890664b97fe7170
// v8 realism: refr 0.4902+0.7001*fres/dist (cap 0.1298) | energy 0.3397+0.4263*p | flow 0.055833 c/s | soft 0.5319m
...
void main() {
    ...
    float refrAmp = min((0.4902 + 0.7001 * fresnel) / max(length(worldPos), 1.0), 0.1298);
