...
// Bubble surface shader fx_364 -- family KALISET
// stack: deep=voro x3 taps | mid=kaliset+warp1+rotate | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5796, 0.0, 1.0);
    ...
    deepCol += 0.4483 * dp * mix(baseCol, deepStop, 0.3553);
    deepPat += 0.4483 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.2643) + par * 0.1560, time);
    deepCol += 0.2009 * dp * mix(baseCol, deepStop, 0.7106);
    deepPat += 0.2009 * dp;
    deepCol *= 0.5929;
    deepPat = pow(clamp(deepPat * 0.6064, 0.0, 1.0), 1.0897);
    ...
    vec3 mdir = rotA(spinAxis, time * 0.052360) * (sdir * 1.3229);
    ...
    vec3 kp = mdir * 1.1734;
    vec3 kc = vec3(0.5729, 0.5617, 0.5251);
    float kacc = 0.0;
    float kprev = 0.0;
    for (int i = 0; i < 5; i++) {
    ...
    float nest = pow(clamp(kacc * 0.1649 - 0.1856, 0.0, 1.0), 1.8769);
    float mid = nest * (0.72 + 0.28 * sin(time * 0.727802 + kacc * 1.7131));
    ...
    float pattern = clamp(0.3269 * deepPat + 0.8890 * mid + 0.8964 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.1994 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0594 + 0.3591 * presence + 0.3236 * pattern, 0.7976);
