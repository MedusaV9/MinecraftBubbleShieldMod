...
void main() {
    ...
    mid = clamp(mid * (0.6202 + 0.6947 * texDetail) + 0.3202 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7558 + 0.4444 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1333 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.9478 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2858 + 0.3381 * presence + 0.4756 * pattern + 0.1312 * (1.0 - deepTrans) + 0.1257 * emit, 0.8632);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2480), 0.8632);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7310 + 0.2690 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0735), 0.0, 1.0);
