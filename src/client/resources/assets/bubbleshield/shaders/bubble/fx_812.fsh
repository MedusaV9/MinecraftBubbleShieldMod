...
void main() {
    ...
    mid = clamp(mid * (0.6039 + 0.5523 * texDetail) + 0.2955 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4517), 0.0, 1.0);
    float emit = atlas.a * (0.7264 + 0.4096 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1799 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.9971 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3182 + 0.2907 * presence + 0.5529 * pattern + 0.0836 * (1.0 - deepTrans) + 0.1570 * emit, 0.8591);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3057), 0.8591);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6820 + 0.3180 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0931), 0.0, 1.0);
