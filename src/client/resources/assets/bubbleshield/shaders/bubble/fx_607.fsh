...
void main() {
    ...
    mid = clamp(mid * (0.6815 + 0.5970 * texDetail) + 0.2303 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5208), 0.0, 1.0);
    float emit = atlas.a * (0.7625 + 0.2905 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0831 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.7904 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3366 + 0.3172 * presence + 0.4595 * pattern + 0.1529 * (1.0 - deepTrans) + 0.1973 * emit, 0.8688);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3310), 0.8688);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7286 + 0.2714 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0979), 0.0, 1.0);
