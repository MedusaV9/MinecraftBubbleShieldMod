...
void main() {
    ...
    mid = clamp(mid * (0.7220 + 0.6695 * texDetail) + 0.2805 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3692), 0.0, 1.0);
    float emit = atlas.a * (0.6311 + 0.4435 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0910 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9673 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3711 + 0.4162 * presence + 0.3790 * pattern + 0.1241 * (1.0 - deepTrans) + 0.1262 * emit, 0.8772);
