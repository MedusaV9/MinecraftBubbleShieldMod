...
void main() {
    ...
    mid = clamp(mid * (0.6381 + 0.5649 * texDetail) + 0.2993 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4892), 0.0, 1.0);
    float emit = atlas.a * (0.6000 + 0.4406 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1776 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.9683 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4239 + 0.3653 * presence + 0.3775 * pattern + 0.1371 * (1.0 - deepTrans) + 0.1768 * emit, 0.8464);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2769), 0.8464);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7326 + 0.2674 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1198), 0.0, 1.0);
