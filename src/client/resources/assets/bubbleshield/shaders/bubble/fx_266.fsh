...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8260, 0.0, 1.0);
    ...
    deepCol += 0.5617 * dp * mix(baseCol, deepStop, 0.2138);
    deepPat += 0.5617 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 3.1252) + par * 0.0723, time);
    deepCol += 0.3155 * dp * mix(baseCol, deepStop, 0.4275);
    deepPat += 0.3155 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 3.8150) + par * 0.1085, time);
    deepCol += 0.1772 * dp * mix(baseCol, deepStop, 0.6413);
    deepPat += 0.1772 * dp;
    deepCol *= 0.5705;
    deepPat = pow(clamp(deepPat * 0.4868, 0.0, 1.0), 1.5629);
    ...
    rgb += hotStop * 0.1981 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0639 + 0.3342 * presence + 0.3797 * pattern, 0.8142);
