...
void main() {
    ...
    mid = clamp(mid * (0.7262 + 0.5439 * texDetail) + 0.2671 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4482), 0.0, 1.0);
    float emit = atlas.a * (0.6461 + 0.2522 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0854 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.7461 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3989 + 0.4742 * presence + 0.3797 * pattern + 0.1137 * (1.0 - deepTrans) + 0.1771 * emit, 0.8742);
