...
void main() {
    ...
    mid = clamp(mid * (0.7460 + 0.6392 * texDetail) + 0.2552 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3683), 0.0, 1.0);
    float emit = atlas.a * (0.6432 + 0.2816 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0979 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.8402 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3640 + 0.4097 * presence + 0.3986 * pattern + 0.1493 * (1.0 - deepTrans) + 0.2036 * emit, 0.8425);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3292), 0.8425);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6216 + 0.3784 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1267), 0.0, 1.0);
