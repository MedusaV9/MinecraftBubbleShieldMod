...
void main() {
    ...
    mid = clamp(mid * (0.6357 + 0.5749 * texDetail) + 0.2093 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4951), 0.0, 1.0);
    float emit = atlas.a * (0.6970 + 0.4372 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1637 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.7949 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2892 + 0.3239 * presence + 0.4904 * pattern + 0.1457 * (1.0 - deepTrans) + 0.1432 * emit, 0.8628);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2635), 0.8628);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6655 + 0.3345 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
