...
void main() {
    ...
    mid = clamp(mid * (0.6640 + 0.4516 * texDetail) + 0.2314 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4989), 0.0, 1.0);
    float emit = atlas.a * (0.6073 + 0.4558 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1725 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.9211 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4126 + 0.4051 * presence + 0.4362 * pattern + 0.0809 * (1.0 - deepTrans) + 0.1350 * emit, 0.8768);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2473), 0.8768);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7456 + 0.2544 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1206), 0.0, 1.0);
