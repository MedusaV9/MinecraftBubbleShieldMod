...
void main() {
    ...
    mid = clamp(mid * (0.7066 + 0.6325 * texDetail) + 0.2054 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4868), 0.0, 1.0);
    float emit = atlas.a * (0.6422 + 0.4863 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1064 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.8129 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3906 + 0.4091 * presence + 0.3677 * pattern + 0.1082 * (1.0 - deepTrans) + 0.1422 * emit, 0.8950);
