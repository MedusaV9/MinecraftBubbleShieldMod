...
// stack: deep=caustic x4 taps | mid=sparkle+none+pulse | rim=graze | flourish=swirl | tex=starfield
// fbm: standard x3 octaves | seed 75e637062138a6de
// v8 realism: refr 0.4889+0.6503*fres/dist (cap 0.1538) | energy 0.3055+0.3976*p | flow 0.073333 c/s | soft 0.7499m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.1942 + 0.2235, 0.0, 1.0);
    ...
    float refrAmp = min((0.4889 + 0.6503 * fresnel) / max(length(worldPos), 1.0), 0.1538);
    ...
    float emitMask = clamp(atlas.a * 2.0942, 0.0, 1.0);
    float emit = emitMask * (0.3553 + 0.7335 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1064 * (0.5 + 0.5 * sin(time * 0.298451))) * atlasPoleW;
