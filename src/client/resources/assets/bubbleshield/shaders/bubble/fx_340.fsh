...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6474, 0.0, 1.0);
    ...
    deepCol += 0.4718 * dp * mix(baseCol, deepStop, 0.1998);
    deepPat += 0.4718 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 3.2515) + par * 0.1292, time);
    deepCol += 0.2226 * dp * mix(baseCol, deepStop, 0.3996);
    deepPat += 0.2226 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 4.0465) + par * 0.1938, time);
    deepCol += 0.1050 * dp * mix(baseCol, deepStop, 0.5993);
    deepPat += 0.1050 * dp;
    deepCol *= 0.7140;
    deepPat = pow(clamp(deepPat * 0.5557, 0.0, 1.0), 1.6292);
    ...
    rgb += hotStop * 0.3000 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0611 + 0.2691 * presence + 0.3677 * pattern, 0.8350);
