...
void main() {
    ...
    mid = clamp(mid * (0.6140 + 0.5750 * texDetail) + 0.2061 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4985), 0.0, 1.0);
    float emit = atlas.a * (0.7940 + 0.3074 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0978 * (0.5 + 0.5 * sin(time * 0.183260)));
    ...
    float alpha = vertexColor.a * min(0.3811 + 0.4338 * presence + 0.4981 * pattern + 0.1543 * (1.0 - deepTrans) + 0.1272 * emit, 0.8453);
