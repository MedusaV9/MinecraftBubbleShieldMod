...
// stack: deep=ridge x2 taps | mid=petals+warp2+scroll | rim=graze | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.035000, -0.017500) * time, time);
    deep += 0.4438 * deepField(duvBase + rimDir * 0.0724 + vec2(-0.055000, -0.027500) * time, time);
    deep = pow(clamp(deep * 0.6926, 0.0, 1.0), 1.6189);
    ...
    float pattern = clamp(0.3826 * deep + 0.8825 * mid + 0.6388 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5379 + pattern * 0.6698);
    vec3 rgb = vertexColor.rgb * (0.3938 + 0.7117 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1912 + 0.8168 * pattern, 0.0, 1.0);
