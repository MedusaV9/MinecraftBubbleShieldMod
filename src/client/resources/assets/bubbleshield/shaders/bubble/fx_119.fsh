...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1821) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1821);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.023333, 0.020000) * time, time);
    deep += 0.4038 * deepField(duvBase + rimDir * 0.0301 + vec2(0.035000, 0.030000) * time, time);
    deep += 0.1631 * deepField(duvBase + rimDir * 0.0602 + vec2(0.048333, 0.042500) * time, time);
    deep += 0.0658 * deepField(duvBase + rimDir * 0.0903 + vec2(0.060000, 0.052500) * time, time);
    deep = pow(clamp(deep * 0.6125, 0.0, 1.0), 1.1168);
    ...
    vec2 wuv = auv;
    float n = fbm2(wuv + vec2(0.0, time * 0.425833), midPer);
