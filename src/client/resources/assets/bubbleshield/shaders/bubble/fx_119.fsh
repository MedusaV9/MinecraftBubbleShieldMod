...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6869, 0.0, 1.0);
    ...
    deepCol += 0.4038 * dp * mix(baseCol, deepStop, 0.1972);
    deepPat += 0.4038 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.7421) + par * 0.0602, time);
    deepCol += 0.1631 * dp * mix(baseCol, deepStop, 0.3944);
    deepPat += 0.1631 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 7.2529) + par * 0.0903, time);
    deepCol += 0.0658 * dp * mix(baseCol, deepStop, 0.5916);
    deepPat += 0.0658 * dp;
    deepCol *= 0.7153;
    deepPat = pow(clamp(deepPat * 0.6125, 0.0, 1.0), 1.1168);
    ...
    rgb += hotStop * 0.2518 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0682 + 0.2890 * presence + 0.3644 * pattern, 0.8311);
