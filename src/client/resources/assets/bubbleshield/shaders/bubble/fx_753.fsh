...
void main() {
    ...
    mid = clamp(mid * (0.6227 + 0.6607 * texDetail) + 0.2019 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4314), 0.0, 1.0);
    float emit = atlas.a * (0.6895 + 0.4445 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1720 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.6801 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3170 + 0.3054 * presence + 0.5149 * pattern + 0.1433 * (1.0 - deepTrans) + 0.2038 * emit, 0.8494);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1778), 0.8494);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7588 + 0.2412 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1059), 0.0, 1.0);
