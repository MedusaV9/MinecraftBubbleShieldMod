...
void main() {
    ...
    mid = clamp(mid * (0.6885 + 0.5583 * texDetail) + 0.2261 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.5616 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3819), 0.0, 1.0);
    float emit = atlas.a * (0.7709 + 0.2640 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1319 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9569 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3299 + 0.3141 * presence + 0.5098 * pattern + 0.0966 * (1.0 - deepTrans) + 0.1366 * emit, 0.8977);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2569), 0.8977);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6749 + 0.3251 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
