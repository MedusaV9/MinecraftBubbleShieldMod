...
// stack: deep=caustic x4 taps | mid=voidrift+warp2+rotate | rim=graze | flourish=shimmer
...
void main() {
    ...
    float vrStar = step(0.7656, vrH) * invsmooth(0.08, 0.42, length(fract(vrS) - 0.5)) * (0.6 + 0.4 * sin(time * (1.5 + 2.0 * vrH) + vrH * 31.0));
    ...
    float alpha = vertexColor.a * min(0.0579 + 0.1741 * presence + 0.5098 * pattern + 0.0966 * (1.0 - deepTrans), 0.8377);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2569), 0.8377);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4383 + 0.5617 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
