...
void main() {
    ...
    mid = clamp(mid * (0.6277 + 0.6462 * texDetail) + 0.2106 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3931), 0.0, 1.0);
    float emit = atlas.a * (0.5878 + 0.3570 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0972 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.7016 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3062 + 0.3041 * presence + 0.4804 * pattern + 0.1398 * (1.0 - deepTrans) + 0.1667 * emit, 0.9010);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3476), 0.9010);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6542 + 0.3458 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
