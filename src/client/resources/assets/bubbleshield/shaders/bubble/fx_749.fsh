...
void main() {
    ...
    mid = clamp(mid * (0.6770 + 0.6823 * texDetail) + 0.2043 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4887), 0.0, 1.0);
    float emit = atlas.a * (0.7803 + 0.3582 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1782 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8026 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4483 + 0.4484 * presence + 0.4207 * pattern + 0.1093 * (1.0 - deepTrans) + 0.2073 * emit, 0.9419);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2111), 0.9419);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6752 + 0.3248 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1327), 0.0, 1.0);
