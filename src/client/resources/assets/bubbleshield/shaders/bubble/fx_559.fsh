...
void main() {
    ...
    mid = clamp(mid * (0.6072 + 0.4772 * texDetail) + 0.2784 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4835), 0.0, 1.0);
    float emit = atlas.a * (0.6690 + 0.3999 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0960 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9984 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3676 + 0.3722 * presence + 0.4260 * pattern + 0.1449 * (1.0 - deepTrans) + 0.1716 * emit, 0.9092);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1609), 0.9092);
    // [layer:v5:ghostalpha]
