...
// stack: deep=caustic x3 taps | mid=runecircuit+warp2+scroll | rim=lat | flourish=glint | tex=runes
// fbm: ridged x6 octaves | seed 26c1286970c92086
// v8 realism: refr 0.3642+0.2960*fres/dist (cap 0.1001) | energy 0.4398+0.3506*p | flow 0.050000 c/s | soft 0.5219m
...
void main() {
    ...
    float refrAmp = min((0.3642 + 0.2960 * fresnel) / max(length(worldPos), 1.0), 0.1001);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0333, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9667, vec2(0.001), vec2(0.999))).b);
