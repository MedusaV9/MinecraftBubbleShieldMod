...
void main() {
    ...
    mid = clamp(mid * (0.6858 + 0.4957 * texDetail) + 0.2696 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3981), 0.0, 1.0);
    float emit = atlas.a * (0.7528 + 0.2755 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0986 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.8458 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3476 + 0.2981 * presence + 0.5742 * pattern + 0.1144 * (1.0 - deepTrans) + 0.1522 * emit, 0.8530);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3439), 0.8530);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6392 + 0.3608 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
