...
void main() {
    ...
    mid = clamp(mid * (0.6111 + 0.7354 * texDetail) + 0.3374 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4073), 0.0, 1.0);
    float emit = atlas.a * (0.6704 + 0.3236 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1683 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.9675 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4184 + 0.4099 * presence + 0.2851 * pattern + 0.1411 * (1.0 - deepTrans) + 0.1320 * emit, 0.9361);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3321), 0.9361);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7761 + 0.2239 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0726), 0.0, 1.0);
