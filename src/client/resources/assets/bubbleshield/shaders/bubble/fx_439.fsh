...
// stack: deep=fbm x4 taps | mid=voidrift+warp2+pulse | rim=graze_sparkle | flourish=swirl | tex=cracked_glass
// fbm: ridged x6 octaves | seed e438380888fe13de
// v8 realism: refr 0.2852+0.3748*fres/dist (cap 0.1478) | energy 0.4231+0.4331*p | flow 0.116667 c/s | soft 0.9239m
...
void main() {
    ...
    float refrAmp = min((0.2852 + 0.3748 * fresnel) / max(length(worldPos), 1.0), 0.1478);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0429, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9571, vec2(0.001), vec2(0.999))).b);
