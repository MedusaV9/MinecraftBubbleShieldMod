...
void main() {
    ...
    mid = clamp(mid * (0.6834 + 0.7157 * texDetail) + 0.2526 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4029), 0.0, 1.0);
    float emit = atlas.a * (0.5522 + 0.3098 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1035 * (0.5 + 0.5 * sin(time * 0.240855)));
    ...
    float alpha = vertexColor.a * min(0.3460 + 0.3349 * presence + 0.4728 * pattern + 0.1254 * (1.0 - deepTrans) + 0.1756 * emit, 0.9018);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3071), 0.9018);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6230 + 0.3770 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0885), 0.0, 1.0);
