...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8196, 0.0, 1.0);
    ...
    deepCol += 0.4455 * dp * mix(baseCol, deepStop, 0.4220);
    deepPat += 0.4455 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 6.3351) + par * 0.0911, time);
    deepCol += 0.1985 * dp * mix(baseCol, deepStop, 0.8439);
    deepPat += 0.1985 * dp;
    deepCol *= 0.6293;
    deepPat = pow(clamp(deepPat * 0.6083, 0.0, 1.0), 1.0774);
    ...
    rgb += hotStop * 0.2332 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0775 + 0.2907 * presence + 0.4913 * pattern, 0.8015);
