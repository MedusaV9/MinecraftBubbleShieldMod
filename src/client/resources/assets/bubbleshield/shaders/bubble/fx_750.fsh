...
void main() {
    ...
    mid = clamp(mid * (0.6206 + 0.6594 * texDetail) + 0.1914 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4441), 0.0, 1.0);
    float emit = atlas.a * (0.5846 + 0.4465 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1204 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.9602 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2700 + 0.2954 * presence + 0.4932 * pattern + 0.1070 * (1.0 - deepTrans) + 0.1845 * emit, 0.8092);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2296), 0.8092);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6523 + 0.3477 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0689), 0.0, 1.0);
