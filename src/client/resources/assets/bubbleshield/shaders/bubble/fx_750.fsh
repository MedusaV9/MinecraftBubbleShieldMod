...
// stack: deep=ridge x4 taps | mid=phantomecho+warp1+rotate | rim=graze_sparkle | flourish=glint | tex=foam
// fbm: ridged x5 octaves | seed 29f87162a62c2c07
// v8 realism: refr 0.3801+0.5316*fres/dist (cap 0.1347) | energy 0.3742+0.5177*p | flow 0.084167 c/s | soft 1.4437m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (0.9365 * atlasPoleW));
    ...
    float refrAmp = min((0.3801 + 0.5316 * fresnel) / max(length(worldPos), 1.0), 0.1347);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0659, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9341, vec2(0.001), vec2(0.999))).b);
