...
void main() {
    ...
    mid = clamp(mid * (0.6316 + 0.6731 * texDetail) + 0.2496 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3962), 0.0, 1.0);
    float emit = atlas.a * (0.6947 + 0.2517 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1788 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.8867 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4305 + 0.5143 * presence + 0.3467 * pattern + 0.0945 * (1.0 - deepTrans) + 0.2056 * emit, 0.9029);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1590), 0.9029);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7419 + 0.2581 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1123), 0.0, 1.0);
