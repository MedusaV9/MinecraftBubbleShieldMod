...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6205, 0.0, 1.0);
    ...
    deepCol += 0.5099 * dp * mix(baseCol, deepStop, 0.2102);
    deepPat += 0.5099 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 5.4163) + par * 0.1338, time);
    deepCol += 0.2600 * dp * mix(baseCol, deepStop, 0.4203);
    deepPat += 0.2600 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.8005) + par * 0.2008, time);
    deepCol += 0.1326 * dp * mix(baseCol, deepStop, 0.6305);
    deepPat += 0.1326 * dp;
    deepCol *= 0.5075;
    deepPat = pow(clamp(deepPat * 0.5256, 0.0, 1.0), 1.7813);
    ...
    rgb += hotStop * 0.3356 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0777 + 0.2804 * presence + 0.4916 * pattern, 0.7958);
