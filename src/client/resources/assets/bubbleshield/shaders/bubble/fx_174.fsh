...
void main() {
    ...
    mid = clamp(mid * (0.6976 + 0.4958 * texDetail) + 0.2975 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7524 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4093), 0.0, 1.0);
    float emit = atlas.a * (0.6972 + 0.3569 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1601 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7582 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4396 + 0.4204 * presence + 0.4916 * pattern + 0.1254 * (1.0 - deepTrans) + 0.1811 * emit, 0.8558);
