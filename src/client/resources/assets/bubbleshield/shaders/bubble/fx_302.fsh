...
// stack: deep=fbm x3 taps | mid=triweave+none+rotate | rim=graze | flourish=shimmer | tex=hex_lattice
// fbm: turb x4 octaves | seed ba57acc549470b1b
// v8 realism: refr 0.4678+0.5661*fres/dist (cap 0.1355) | energy 0.4027+0.4562*p | flow 0.059167 c/s | soft 1.0832m
...
void main() {
    ...
    float refrAmp = min((0.4678 + 0.5661 * fresnel) / max(length(worldPos), 1.0), 0.1355);
