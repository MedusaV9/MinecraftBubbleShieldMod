...
// stack: deep=fbm x2 taps | mid=triweave+none+rotate | rim=graze | flourish=shimmer
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1514) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1514);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.050000, 0.026667) * time, time);
    deep += 0.4191 * deepField(duvBase + rimDir * 0.0388 + vec2(0.075000, 0.040000) * time, time);
    deep = pow(clamp(deep * 0.7047, 0.0, 1.0), 1.1024);
    ...
    float pattern = clamp(0.3938 * deep + 0.7432 * mid + 0.5803 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5716 + pattern * 0.4379);
    vec3 rgb = vertexColor.rgb * (0.5117 + 0.6750 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2113 + 0.8390 * pattern, 0.0, 1.0);
