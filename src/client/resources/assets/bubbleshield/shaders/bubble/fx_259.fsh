...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8340, 0.0, 1.0);
    ...
    deepCol += 0.4412 * dp * mix(baseCol, deepStop, 0.2912);
    deepPat += 0.4412 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.1814) + par * 0.0994, time);
    deepCol += 0.1947 * dp * mix(baseCol, deepStop, 0.5823);
    deepPat += 0.1947 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 5.0125) + par * 0.1491, time);
    deepCol += 0.0859 * dp * mix(baseCol, deepStop, 0.8735);
    deepPat += 0.0859 * dp;
    deepCol *= 0.6844;
    deepPat = pow(clamp(deepPat * 0.5808, 0.0, 1.0), 1.4586);
    ...
    rgb += hotStop * 0.1831 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0731 + 0.3225 * presence + 0.4436 * pattern, 0.7824);
