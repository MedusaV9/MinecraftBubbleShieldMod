...
void main() {
    ...
    mid = clamp(mid * (0.6313 + 0.5572 * texDetail) + 0.3423 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3782), 0.0, 1.0);
    float emit = atlas.a * (0.6512 + 0.4434 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1636 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.6756 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3649 + 0.3277 * presence + 0.5994 * pattern + 0.0850 * (1.0 - deepTrans) + 0.1228 * emit, 0.8844);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2115), 0.8844);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6364 + 0.3636 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1273), 0.0, 1.0);
