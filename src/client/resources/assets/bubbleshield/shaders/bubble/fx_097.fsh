...
// stack: deep=ridge x3 taps | mid=scales+curl+rotate | rim=lat | flourish=glint | tex=scales
// fbm: ridged x5 octaves | seed 68d2cec5f3ab66c0
// v8 realism: refr 0.5357+0.7423*fres/dist (cap 0.1025) | energy 0.3965+0.4633*p | flow 0.104167 c/s | soft 0.6565m
...
void main() {
    ...
    float refrAmp = min((0.5357 + 0.7423 * fresnel) / max(length(worldPos), 1.0), 0.1025);
