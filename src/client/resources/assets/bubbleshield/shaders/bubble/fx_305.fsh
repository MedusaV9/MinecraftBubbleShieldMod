...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6667, 0.0, 1.0);
    ...
    deepCol += 0.3889 * dp * mix(baseCol, deepStop, 0.2632);
    deepPat += 0.3889 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 5.1328) + par * 0.0922, time);
    deepCol += 0.1513 * dp * mix(baseCol, deepStop, 0.5263);
    deepPat += 0.1513 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 6.4403) + par * 0.1383, time);
    deepCol += 0.0588 * dp * mix(baseCol, deepStop, 0.7895);
    deepPat += 0.0588 * dp;
    deepCol *= 0.7092;
    deepPat = pow(clamp(deepPat * 0.6254, 0.0, 1.0), 1.4193);
    ...
    rgb += hotStop * 0.3476 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0681 + 0.2990 * presence + 0.4074 * pattern, 0.8194);
