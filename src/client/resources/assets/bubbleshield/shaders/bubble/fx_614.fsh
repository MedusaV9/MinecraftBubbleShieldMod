...
void main() {
    ...
    mid = clamp(mid * (0.7424 + 0.7122 * texDetail) + 0.2675 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4160), 0.0, 1.0);
    float emit = atlas.a * (0.5691 + 0.2821 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1103 * (0.5 + 0.5 * sin(time * 0.146608)));
    ...
    float alpha = vertexColor.a * min(0.3891 + 0.5269 * presence + 0.3714 * pattern + 0.1147 * (1.0 - deepTrans) + 0.2146 * emit, 0.8886);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2117), 0.8886);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7217 + 0.2783 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1230), 0.0, 1.0);
