...
void main() {
    ...
    mid = clamp(mid * (0.7073 + 0.5440 * texDetail) + 0.3353 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3531), 0.0, 1.0);
    float emit = atlas.a * (0.7745 + 0.2825 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1608 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.9409 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3497 + 0.3843 * presence + 0.5341 * pattern + 0.1185 * (1.0 - deepTrans) + 0.1420 * emit, 0.8663);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3141), 0.8663);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7698 + 0.2302 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0892), 0.0, 1.0);
