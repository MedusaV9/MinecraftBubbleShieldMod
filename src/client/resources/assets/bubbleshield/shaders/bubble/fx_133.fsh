...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6887, 0.0, 1.0);
    ...
    deepCol += 0.4451 * dp * mix(baseCol, deepStop, 0.1869);
    deepPat += 0.4451 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.8340) + par * 0.1369, time);
    deepCol += 0.1981 * dp * mix(baseCol, deepStop, 0.3738);
    deepPat += 0.1981 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 5.8463) + par * 0.2054, time);
    deepCol += 0.0882 * dp * mix(baseCol, deepStop, 0.5607);
    deepPat += 0.0882 * dp;
    deepCol *= 0.5729;
    deepPat = pow(clamp(deepPat * 0.5776, 0.0, 1.0), 1.2228);
    ...
    rgb += hotStop * 0.2957 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0669 + 0.2574 * presence + 0.4642 * pattern, 0.8543);
