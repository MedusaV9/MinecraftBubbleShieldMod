...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3280 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.523599);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.046667, 0.042500) * time, time);
    deep += 0.4451 * deepField(duvBase + rimDir * 0.0685 + vec2(0.071667, 0.067500) * time, time);
    deep += 0.1981 * deepField(duvBase + rimDir * 0.1369 + vec2(0.098333, 0.090000) * time, time);
    deep += 0.0882 * deepField(duvBase + rimDir * 0.2054 + vec2(0.123333, 0.112500) * time, time);
    deep = pow(clamp(deep * 0.5776, 0.0, 1.0), 1.2228);
    ...
    float pattern = clamp(0.4268 * deep + 0.9511 * mid + 0.5646 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1932 + pattern * 0.3713);
    vec3 rgb = vertexColor.rgb * (0.4034 + 0.5866 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1153 + 0.7784 * pattern, 0.0, 1.0);
