...
void main() {
    ...
    mid = clamp(mid * (0.7025 + 0.5089 * texDetail) + 0.2412 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4622), 0.0, 1.0);
    float emit = atlas.a * (0.6196 + 0.2695 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1163 * (0.5 + 0.5 * sin(time * 0.293215)));
    ...
    float alpha = vertexColor.a * min(0.4078 + 0.3974 * presence + 0.4642 * pattern + 0.0883 * (1.0 - deepTrans) + 0.1789 * emit, 0.9143);
