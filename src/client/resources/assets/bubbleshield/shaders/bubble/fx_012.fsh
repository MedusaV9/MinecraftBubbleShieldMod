...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.018333, 0.041667) * time, time);
    deep += 0.4022 * deepField(duvBase + rimDir * 0.0473 + vec2(-0.028333, 0.065000) * time, time);
    deep += 0.1618 * deepField(duvBase + rimDir * 0.0945 + vec2(-0.038333, 0.088333) * time, time);
    deep += 0.0651 * deepField(duvBase + rimDir * 0.1418 + vec2(-0.049167, 0.111667) * time, time);
    deep = pow(clamp(deep * 0.6138, 0.0, 1.0), 1.0325);
    ...
    float pattern = clamp(0.4101 * deep + 0.8395 * mid + 0.6877 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8234 + pattern * 0.4827);
    vec3 rgb = vertexColor.rgb * (0.4805 + 0.8551 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1891 + 0.7518 * pattern, 0.0, 1.0);
