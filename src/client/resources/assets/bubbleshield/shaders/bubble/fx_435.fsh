...
void main() {
    ...
    mid = clamp(mid * (0.6201 + 0.4731 * texDetail) + 0.3286 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4914), 0.0, 1.0);
    float emit = atlas.a * (0.7896 + 0.2859 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1289 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.9766 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3035 + 0.3056 * presence + 0.5047 * pattern + 0.0817 * (1.0 - deepTrans) + 0.2136 * emit, 0.9072);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2433), 0.9072);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6911 + 0.3089 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0610), 0.0, 1.0);
