...
void main() {
    ...
    mid = clamp(mid * (0.7219 + 0.7479 * texDetail) + 0.3172 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4664), 0.0, 1.0);
    float emit = atlas.a * (0.7621 + 0.3469 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1039 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.7717 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3830 + 0.4707 * presence + 0.3760 * pattern + 0.1449 * (1.0 - deepTrans) + 0.2068 * emit, 0.9426);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2921), 0.9426);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6494 + 0.3506 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0607), 0.0, 1.0);
