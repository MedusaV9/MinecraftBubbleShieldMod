...
// stack: deep=fbm x4 taps | mid=triweave+curl+pulse | rim=graze | flourish=glint | tex=hex_lattice
// fbm: ridged x3 octaves | seed b494c611a30eac1f
// v8 realism: refr 0.4830+0.6675*fres/dist (cap 0.1084) | energy 0.3831+0.4370*p | flow 0.087500 c/s | soft 0.9081m
...
void main() {
    ...
    float refrAmp = min((0.4830 + 0.6675 * fresnel) / max(length(worldPos), 1.0), 0.1084);
