...
void main() {
    ...
    mid = clamp(mid * (0.6092 + 0.7293 * texDetail) + 0.2570 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3917), 0.0, 1.0);
    float emit = atlas.a * (0.5734 + 0.4462 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1772 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.8487 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3410 + 0.4729 * presence + 0.4549 * pattern + 0.1346 * (1.0 - deepTrans) + 0.1601 * emit, 0.8720);
