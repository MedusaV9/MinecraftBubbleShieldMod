...
// stack: deep=voro x4 taps | mid=rings+curl+rotate | rim=graze_sparkle | flourish=glint | tex=ridged
// fbm: standard x3 octaves | seed a4cb5f37626e1904
// v8 realism: refr 0.4556+0.4233*fres/dist (cap 0.1355) | energy 0.3950+0.4497*p | flow 0.080833 c/s | soft 0.5628m
...
void main() {
    ...
    float refrAmp = min((0.4556 + 0.4233 * fresnel) / max(length(worldPos), 1.0), 0.1355);
    ...
    float emit = atlas.a * (0.3572 + 0.4904 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1421 * (0.5 + 0.5 * sin(time * 0.193732))) * atlasPoleW;
