...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3783 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.382227);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.021667, -0.017500) * time, time);
    deep += 0.5617 * deepField(duvBase + rimDir * 0.0522 + vec2(-0.035000, -0.030000) * time, time);
    deep += 0.3155 * deepField(duvBase + rimDir * 0.1044 + vec2(-0.046667, -0.040000) * time, time);
    deep += 0.1772 * deepField(duvBase + rimDir * 0.1566 + vec2(-0.058333, -0.050000) * time, time);
    deep = pow(clamp(deep * 0.4868, 0.0, 1.0), 1.7547);
    ...
    float pattern = clamp(0.4168 * deep + 0.7665 * mid + 0.6133 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.0319 + pattern * 0.3416);
    vec3 rgb = vertexColor.rgb * (0.4985 + 0.6280 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1630 + 0.8205 * pattern, 0.0, 1.0);
