...
void main() {
    ...
    mid = clamp(mid * (0.7458 + 0.6459 * texDetail) + 0.1974 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3637), 0.0, 1.0);
    float emit = atlas.a * (0.6453 + 0.2836 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1421 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.8722 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3483 + 0.4823 * presence + 0.4919 * pattern + 0.1441 * (1.0 - deepTrans) + 0.1627 * emit, 0.9129);
