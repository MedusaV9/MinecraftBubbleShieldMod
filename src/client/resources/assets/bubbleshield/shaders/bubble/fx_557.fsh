...
void main() {
    ...
    mid = clamp(mid * (0.6145 + 0.6910 * texDetail) + 0.2153 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3558), 0.0, 1.0);
    float emit = atlas.a * (0.7570 + 0.2953 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1667 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.8310 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3338 + 0.2395 * presence + 0.6321 * pattern + 0.1253 * (1.0 - deepTrans) + 0.1375 * emit, 0.9342);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3021), 0.9342);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1384), 0.0, 1.0);
