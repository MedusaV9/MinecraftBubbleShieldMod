...
void main() {
    ...
    mid = clamp(mid * (0.7318 + 0.5551 * texDetail) + 0.2318 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4869), 0.0, 1.0);
    float emit = atlas.a * (0.7560 + 0.4163 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1454 * (0.5 + 0.5 * sin(time * 0.230383)));
    ...
    float alpha = vertexColor.a * min(0.4422 + 0.4132 * presence + 0.4733 * pattern + 0.1527 * (1.0 - deepTrans) + 0.2013 * emit, 0.9153);
