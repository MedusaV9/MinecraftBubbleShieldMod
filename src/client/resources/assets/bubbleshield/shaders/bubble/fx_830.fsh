...
void main() {
    ...
    mid = clamp(mid * (0.6795 + 0.6677 * texDetail) + 0.2164 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4476), 0.0, 1.0);
    float emit = atlas.a * (0.7594 + 0.3410 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0955 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.7998 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3420 + 0.2864 * presence + 0.5748 * pattern + 0.1301 * (1.0 - deepTrans) + 0.2142 * emit, 0.8327);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2240), 0.8327);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6241 + 0.3759 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1037), 0.0, 1.0);
