...
// stack: deep=fbm x2 taps | mid=plasma+none+rotate | rim=graze_film | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.023333, -0.001667) * time, time);
    deep += 0.5206 * deepField(duvBase + rimDir * 0.0495 + vec2(0.035833, -0.003333) * time, time);
    deep = pow(clamp(deep * 0.6576, 0.0, 1.0), 1.2020);
    ...
    vec2 wuv = auv;
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.090000, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.100000), midPer)), midPer);
    ...
    float pattern = clamp(0.4734 * deep + 0.8462 * mid + 0.8511 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8247 + pattern * 0.3535);
    vec3 rgb = vertexColor.rgb * (0.3728 + 0.9039 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1989 + 0.8396 * pattern, 0.0, 1.0);
