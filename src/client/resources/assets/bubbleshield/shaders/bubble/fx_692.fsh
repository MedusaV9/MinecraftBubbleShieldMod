...
void main() {
    ...
    mid = clamp(mid * (0.7500 + 0.6849 * texDetail) + 0.1877 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7812 + 0.3174 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1358 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.7599 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3466 + 0.3194 * presence + 0.5224 * pattern + 0.1032 * (1.0 - deepTrans) + 0.1555 * emit, 0.8655);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3325), 0.8655);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6329 + 0.3671 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0924), 0.0, 1.0);
