...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.045000, 0.007500) * time, time);
    deep += 0.4803 * deepField(duvBase + rimDir * 0.0562 + vec2(-0.070000, 0.012500) * time, time);
    deep += 0.2307 * deepField(duvBase + rimDir * 0.1123 + vec2(-0.095000, 0.015000) * time, time);
    deep = pow(clamp(deep * 0.5845, 0.0, 1.0), 1.1846);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.455000), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 12.5241);
    float flash = 0.6 + 0.4 * sin(time * 2.591814 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
