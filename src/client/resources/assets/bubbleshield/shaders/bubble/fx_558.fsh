...
void main() {
    ...
    mid = clamp(mid * (0.6491 + 0.6089 * texDetail) + 0.2947 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4669), 0.0, 1.0);
    float emit = atlas.a * (0.7767 + 0.3901 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1764 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.8206 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3237 + 0.3281 * presence + 0.5257 * pattern + 0.0979 * (1.0 - deepTrans) + 0.1836 * emit, 0.8812);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1978), 0.8812);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6981 + 0.3019 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
