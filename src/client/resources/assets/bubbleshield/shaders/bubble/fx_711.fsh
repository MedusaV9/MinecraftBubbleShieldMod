...
void main() {
    ...
    mid = clamp(mid * (0.6265 + 0.6319 * texDetail) + 0.2636 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4906), 0.0, 1.0);
    float emit = atlas.a * (0.7035 + 0.2620 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1742 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.7652 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2615 + 0.2615 * presence + 0.4897 * pattern + 0.1399 * (1.0 - deepTrans) + 0.2049 * emit, 0.8798);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2421), 0.8798);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6704 + 0.3296 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
