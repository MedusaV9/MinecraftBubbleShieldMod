...
void main() {
    ...
    mid = clamp(mid * (0.6684 + 0.6002 * texDetail) + 0.3400 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4440), 0.0, 1.0);
    float emit = atlas.a * (0.5949 + 0.3328 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1354 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.6947 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3641 + 0.3752 * presence + 0.4882 * pattern + 0.0849 * (1.0 - deepTrans) + 0.1557 * emit, 0.8583);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2483), 0.8583);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7555 + 0.2445 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0704), 0.0, 1.0);
