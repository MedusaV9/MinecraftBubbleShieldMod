...
void main() {
    ...
    mid = clamp(mid * (0.7099 + 0.6686 * texDetail) + 0.2684 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5124), 0.0, 1.0);
    float emit = atlas.a * (0.6761 + 0.4145 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1768 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8202 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3182 + 0.2729 * presence + 0.4601 * pattern + 0.1136 * (1.0 - deepTrans) + 0.1524 * emit, 0.8783);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3136), 0.8783);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7534 + 0.2466 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0657), 0.0, 1.0);
