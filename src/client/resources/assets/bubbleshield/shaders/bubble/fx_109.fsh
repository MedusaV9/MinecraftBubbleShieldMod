...
// stack: deep=fbm x3 taps | mid=rings+warp2+pulse | rim=graze | flourish=swirl | tex=ridged
// fbm: ridged x6 octaves | seed 57bd3552614a7774
// v8 realism: refr 0.6674+0.8713*fres/dist (cap 0.1420) | energy 0.3645+0.4712*p | flow 0.095833 c/s | soft 0.9916m
...
void main() {
    ...
    float refrAmp = min((0.6674 + 0.8713 * fresnel) / max(length(worldPos), 1.0), 0.1420);
    ...
    float emit = atlas.a * (0.4458 + 0.5753 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1185 * (0.5 + 0.5 * sin(time * 0.277507))) * atlasPoleW;
