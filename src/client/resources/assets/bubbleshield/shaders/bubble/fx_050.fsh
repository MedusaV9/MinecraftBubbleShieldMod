...
void main() {
    ...
    mid = clamp(mid * (0.6969 + 0.7374 * texDetail) + 0.1857 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4335), 0.0, 1.0);
    float emit = atlas.a * (0.6846 + 0.3783 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1217 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.8423 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2867 + 0.2541 * presence + 0.5020 * pattern + 0.1557 * (1.0 - deepTrans) + 0.1209 * emit, 0.8654);
