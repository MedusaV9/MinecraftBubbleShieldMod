...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6707, 0.0, 1.0);
    ...
    deepCol += 0.5566 * dp * mix(baseCol, deepStop, 0.2293);
    deepPat += 0.5566 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 3.5269) + par * 0.0955, time);
    deepCol += 0.3098 * dp * mix(baseCol, deepStop, 0.4586);
    deepPat += 0.3098 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 4.3671) + par * 0.1433, time);
    deepCol += 0.1724 * dp * mix(baseCol, deepStop, 0.6879);
    deepPat += 0.1724 * dp;
    deepCol *= 0.5116;
    deepPat = pow(clamp(deepPat * 0.4905, 0.0, 1.0), 1.3316);
    ...
    rgb += hotStop * 0.1843 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0490 + 0.2630 * presence + 0.3825 * pattern, 0.8054);
