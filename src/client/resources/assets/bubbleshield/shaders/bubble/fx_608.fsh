...
void main() {
    ...
    mid = clamp(mid * (0.6816 + 0.6019 * texDetail) + 0.1842 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5266), 0.0, 1.0);
    float emit = atlas.a * (0.6594 + 0.4781 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1567 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.9604 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3441 + 0.3469 * presence + 0.3355 * pattern + 0.1334 * (1.0 - deepTrans) + 0.1278 * emit, 0.8814);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1519), 0.8814);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6458 + 0.3542 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0679), 0.0, 1.0);
