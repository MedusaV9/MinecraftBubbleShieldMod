...
// stack: deep=fbm x2 taps | mid=aurora+warp2+rotate | rim=graze_sparkle | flourish=shimmer
...
void main() {
    ...
    deep += 0.4697 * deepField(duvBase + rimDir * 0.0638 + vec2(0.003333, -0.030000) * time, time);
    deep = pow(clamp(deep * 0.6804, 0.0, 1.0), 1.4228);
    ...
    float pattern = clamp(0.4563 * deep + 0.7169 * mid + 0.8171 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.4033 + pattern * 0.5985);
    vec3 rgb = vertexColor.rgb * (0.4503 + 0.7949 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1145 + 0.6176 * pattern, 0.0, 1.0);
