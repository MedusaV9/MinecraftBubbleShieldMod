...
void main() {
    ...
    mid = clamp(mid * (0.6912 + 0.5855 * texDetail) + 0.2826 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4080), 0.0, 1.0);
    float emit = atlas.a * (0.7200 + 0.4827 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1787 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7759 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4443 + 0.4823 * presence + 0.4453 * pattern + 0.1152 * (1.0 - deepTrans) + 0.2090 * emit, 0.8971);
