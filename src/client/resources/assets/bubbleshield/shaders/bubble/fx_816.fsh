...
void main() {
    ...
    mid = clamp(mid * (0.6927 + 0.6522 * texDetail) + 0.2752 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6138 + 0.4074 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0943 * (0.5 + 0.5 * sin(time * 0.246091)));
    ...
    float alpha = vertexColor.a * min(0.3981 + 0.4140 * presence + 0.3350 * pattern + 0.1465 * (1.0 - deepTrans) + 0.2173 * emit, 0.8912);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3187), 0.8912);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6696 + 0.3304 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1317), 0.0, 1.0);
