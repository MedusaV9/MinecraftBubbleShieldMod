...
// stack: deep=fbm x3 taps | mid=mycelia+curl+flicker | rim=graze_sparkle | flourish=echo | tex=filaments
// fbm: standard x5 octaves | seed 2c8e8618a23c4980
// v8 realism: refr 0.3204+0.3016*fres/dist (cap 0.1517) | energy 0.4214+0.3600*p | flow 0.100000 c/s | soft 0.9733m
...
void main() {
    ...
    float refrAmp = min((0.3204 + 0.3016 * fresnel) / max(length(worldPos), 1.0), 0.1517);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0435, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9565, vec2(0.001), vec2(0.999))).b);
