...
void main() {
    ...
    mid = clamp(mid * (0.6082 + 0.5300 * texDetail) + 0.2933 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3511), 0.0, 1.0);
    float emit = atlas.a * (0.6944 + 0.3176 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1730 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.9583 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3183 + 0.2685 * presence + 0.5336 * pattern + 0.1130 * (1.0 - deepTrans) + 0.1543 * emit, 0.8448);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2894), 0.8448);
    // [layer:v5:ghostalpha]
