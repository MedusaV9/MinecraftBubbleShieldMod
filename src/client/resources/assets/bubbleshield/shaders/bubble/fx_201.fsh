...
void main() {
    ...
    mid = clamp(mid * (0.6864 + 0.6112 * texDetail) + 0.2181 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5285), 0.0, 1.0);
    float emit = atlas.a * (0.5763 + 0.2719 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1136 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.7132 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3511 + 0.4768 * presence + 0.4251 * pattern + 0.0964 * (1.0 - deepTrans) + 0.1638 * emit, 0.9026);
