...
void main() {
    ...
    mid = clamp(mid * (0.6174 + 0.5924 * texDetail) + 0.2855 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3987), 0.0, 1.0);
    float emit = atlas.a * (0.7634 + 0.2847 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1274 * (0.5 + 0.5 * sin(time * 0.188496)));
    ...
    float alpha = vertexColor.a * min(0.3497 + 0.4061 * presence + 0.4800 * pattern + 0.1281 * (1.0 - deepTrans) + 0.1582 * emit, 0.8948);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2704), 0.8948);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1099), 0.0, 1.0);
