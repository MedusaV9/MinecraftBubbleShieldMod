...
void main() {
    ...
    mid = clamp(mid * (0.7063 + 0.4519 * texDetail) + 0.2007 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5225), 0.0, 1.0);
    float emit = atlas.a * (0.5616 + 0.2878 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1138 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.6798 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3647 + 0.3126 * presence + 0.4679 * pattern + 0.1580 * (1.0 - deepTrans) + 0.2139 * emit, 0.8768);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3154), 0.8768);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7259 + 0.2741 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0941), 0.0, 1.0);
