...
void main() {
    ...
    mid = clamp(mid * (0.7360 + 0.6177 * texDetail) + 0.2438 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4655), 0.0, 1.0);
    float emit = atlas.a * (0.6952 + 0.3439 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1016 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.6715 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3349 + 0.2691 * presence + 0.5731 * pattern + 0.1042 * (1.0 - deepTrans) + 0.1410 * emit, 0.8618);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1990), 0.8618);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7066 + 0.2934 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1092), 0.0, 1.0);
