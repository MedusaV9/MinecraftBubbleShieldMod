...
void main() {
    ...
    mid = clamp(mid * (0.6729 + 0.6095 * texDetail) + 0.2422 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3651), 0.0, 1.0);
    float emit = atlas.a * (0.7927 + 0.4616 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1766 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.9161 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3524 + 0.3106 * presence + 0.4742 * pattern + 0.1177 * (1.0 - deepTrans) + 0.1519 * emit, 0.9007);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2634), 0.9007);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6350 + 0.3650 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0916), 0.0, 1.0);
