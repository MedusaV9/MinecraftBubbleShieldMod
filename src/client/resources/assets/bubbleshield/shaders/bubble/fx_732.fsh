...
void main() {
    ...
    mid = clamp(mid * (0.6658 + 0.6144 * texDetail) + 0.2919 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3919), 0.0, 1.0);
    float emit = atlas.a * (0.7466 + 0.3387 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1765 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.8991 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3198 + 0.3194 * presence + 0.5809 * pattern + 0.0819 * (1.0 - deepTrans) + 0.1741 * emit, 0.8808);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1635), 0.8808);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6940 + 0.3060 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
