...
void main() {
    ...
    mid = clamp(mid * (0.6176 + 0.4564 * texDetail) + 0.3382 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.5576 + 0.4530 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1648 * (0.5 + 0.5 * sin(time * 0.240855)));
    ...
    float alpha = vertexColor.a * min(0.3788 + 0.4697 * presence + 0.3215 * pattern + 0.1089 * (1.0 - deepTrans) + 0.2172 * emit, 0.8850);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2117), 0.8850);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6245 + 0.3755 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
