...
void main() {
    ...
    mid = clamp(mid * (0.7464 + 0.6238 * texDetail) + 0.3285 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4830), 0.0, 1.0);
    float emit = atlas.a * (0.6218 + 0.4657 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1521 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.8862 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2957 + 0.3151 * presence + 0.5784 * pattern + 0.1342 * (1.0 - deepTrans) + 0.2167 * emit, 0.8665);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2703), 0.8665);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7753 + 0.2247 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
