...
void main() {
    ...
    mid = clamp(mid * (0.6342 + 0.6881 * texDetail) + 0.2326 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3552), 0.0, 1.0);
    float emit = atlas.a * (0.5706 + 0.3180 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1034 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.7882 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2720 + 0.2695 * presence + 0.5227 * pattern + 0.0962 * (1.0 - deepTrans) + 0.1258 * emit, 0.8989);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1874), 0.8989);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7435 + 0.2565 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
