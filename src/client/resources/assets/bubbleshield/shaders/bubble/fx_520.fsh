...
void main() {
    ...
    mid = clamp(mid * (0.6045 + 0.6881 * texDetail) + 0.3125 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6426 + 0.4512 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1280 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.9371 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3317 + 0.2936 * presence + 0.4549 * pattern + 0.1338 * (1.0 - deepTrans) + 0.1768 * emit, 0.8426);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1590), 0.8426);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7799 + 0.2201 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0727), 0.0, 1.0);
