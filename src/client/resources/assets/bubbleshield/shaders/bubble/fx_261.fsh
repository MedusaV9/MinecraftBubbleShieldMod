...
void main() {
    ...
    mid = clamp(mid * (0.7013 + 0.4859 * texDetail) + 0.2499 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3574), 0.0, 1.0);
    float emit = atlas.a * (0.5930 + 0.3364 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1683 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.7421 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4083 + 0.4971 * presence + 0.4646 * pattern + 0.1429 * (1.0 - deepTrans) + 0.1596 * emit, 0.9030);
