...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6307, 0.0, 1.0);
    ...
    deepCol += 0.3918 * dp * mix(baseCol, deepStop, 0.2291);
    deepPat += 0.3918 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 4.6927) + par * 0.1380, time);
    deepCol += 0.1535 * dp * mix(baseCol, deepStop, 0.4581);
    deepPat += 0.1535 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 5.8420) + par * 0.2070, time);
    deepCol += 0.0602 * dp * mix(baseCol, deepStop, 0.6872);
    deepPat += 0.0602 * dp;
    deepCol *= 0.7023;
    deepPat = pow(clamp(deepPat * 0.6228, 0.0, 1.0), 1.5941);
    ...
    rgb += hotStop * 0.2682 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0671 + 0.3571 * presence + 0.4646 * pattern, 0.8430);
