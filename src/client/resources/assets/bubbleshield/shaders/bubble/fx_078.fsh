...
// stack: deep=fbm x3 taps | mid=scales+curl+pulse | rim=graze_film | flourish=echo | tex=scales
// fbm: ridged x6 octaves | seed 40c85e4e776a7413
// v8 realism: refr 0.4600+0.5174*fres/dist (cap 0.1565) | energy 0.3768+0.5493*p | flow 0.115833 c/s | soft 0.8470m
...
void main() {
    ...
    float refrAmp = min((0.4600 + 0.5174 * fresnel) / max(length(worldPos), 1.0), 0.1565);
