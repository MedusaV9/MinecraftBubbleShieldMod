...
void main() {
    ...
    mid = clamp(mid * (0.6803 + 0.5728 * texDetail) + 0.2260 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3858), 0.0, 1.0);
    float emit = atlas.a * (0.7300 + 0.3218 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1015 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.7747 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3390 + 0.3886 * presence + 0.3811 * pattern + 0.0823 * (1.0 - deepTrans) + 0.1804 * emit, 0.8774);
