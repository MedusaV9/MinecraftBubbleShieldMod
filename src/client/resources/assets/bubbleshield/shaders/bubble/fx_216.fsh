...
void main() {
    ...
    mid = clamp(mid * (0.6177 + 0.7298 * texDetail) + 0.2276 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4901), 0.0, 1.0);
    float emit = atlas.a * (0.6771 + 0.2764 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1210 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.8582 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4424 + 0.4366 * presence + 0.4277 * pattern + 0.1063 * (1.0 - deepTrans) + 0.1481 * emit, 0.8861);
