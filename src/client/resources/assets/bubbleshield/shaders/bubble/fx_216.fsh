...
// stack: deep=caustic x3 taps | mid=sparkle+warp1+scroll | rim=lat | flourish=echo | tex=starfield
// fbm: ridged x5 octaves | seed 0d60486a4b238b24
// v8 realism: refr 0.4936+0.4007*fres/dist (cap 0.1361) | energy 0.3724+0.4060*p | flow 0.066667 c/s | soft 0.9255m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.1372 + 0.2510, 0.0, 1.0);
    ...
    float refrAmp = min((0.4936 + 0.4007 * fresnel) / max(length(worldPos), 1.0), 0.1361);
    ...
    float emitMask = clamp(atlas.a * 2.2474, 0.0, 1.0);
    float emit = emitMask * (0.3763 + 0.4817 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1210 * (0.5 + 0.5 * sin(time * 0.188496))) * atlasPoleW;
