...
void main() {
    ...
    mid = clamp(mid * (0.6340 + 0.7173 * texDetail) + 0.2893 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4842), 0.0, 1.0);
    float emit = atlas.a * (0.5890 + 0.2770 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1796 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.7266 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3102 + 0.3003 * presence + 0.5287 * pattern + 0.1568 * (1.0 - deepTrans) + 0.2021 * emit, 0.8534);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2230), 0.8534);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7242 + 0.2758 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1043), 0.0, 1.0);
