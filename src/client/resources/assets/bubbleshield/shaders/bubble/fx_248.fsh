...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6252, 0.0, 1.0);
    ...
    deepCol += 0.3923 * dp * mix(baseCol, deepStop, 0.3149);
    deepPat += 0.3923 * dp;
    dp = deepField(rotA(spinAxis, time * 0.083776) * (sdir * 4.8354) + par * 0.0876, time);
    deepCol += 0.1539 * dp * mix(baseCol, deepStop, 0.6298);
    deepPat += 0.1539 * dp;
    deepCol *= 0.7050;
    deepPat = pow(clamp(deepPat * 0.6468, 0.0, 1.0), 1.2177);
    ...
    rgb += hotStop * 0.2440 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0704 + 0.2871 * presence + 0.4104 * pattern, 0.8257);
