...
void main() {
    ...
    mid = clamp(mid * (0.6247 + 0.5250 * texDetail) + 0.3016 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4379), 0.0, 1.0);
    float emit = atlas.a * (0.7329 + 0.2919 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1520 * (0.5 + 0.5 * sin(time * 0.157080)));
    ...
    float alpha = vertexColor.a * min(0.4181 + 0.4271 * presence + 0.4104 * pattern + 0.1250 * (1.0 - deepTrans) + 0.1737 * emit, 0.8857);
