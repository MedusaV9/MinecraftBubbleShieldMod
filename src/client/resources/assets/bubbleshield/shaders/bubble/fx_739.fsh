...
void main() {
    ...
    mid = clamp(mid * (0.6097 + 0.6526 * texDetail) + 0.2683 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4344), 0.0, 1.0);
    float emit = atlas.a * (0.7342 + 0.3508 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1794 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.9386 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3136 + 0.2838 * presence + 0.4454 * pattern + 0.1069 * (1.0 - deepTrans) + 0.2017 * emit, 0.8803);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1748), 0.8803);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7777 + 0.2223 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1292), 0.0, 1.0);
