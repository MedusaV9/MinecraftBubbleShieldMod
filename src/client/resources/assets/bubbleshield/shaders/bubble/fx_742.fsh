...
// stack: deep=caustic x3 taps | mid=prismdisperse+warp2+pulse | rim=graze_film | flourish=glint
...
void main() {
    ...
    float flourish = 0.2478 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0645 + 0.2604 * presence + 0.4907 * pattern + 0.0988 * (1.0 - deepTrans), 0.7923);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2082), 0.7923);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4680 + 0.5320 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
