...
void main() {
    ...
    mid = clamp(mid * (0.6340 + 0.5108 * texDetail) + 0.2136 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4056), 0.0, 1.0);
    float emit = atlas.a * (0.6754 + 0.3196 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1384 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.7778 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3924 + 0.4004 * presence + 0.4907 * pattern + 0.0988 * (1.0 - deepTrans) + 0.1894 * emit, 0.8523);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2082), 0.8523);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7028 + 0.2972 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0764), 0.0, 1.0);
