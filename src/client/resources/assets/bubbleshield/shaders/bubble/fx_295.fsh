...
void main() {
    ...
    mid = clamp(mid * (0.6809 + 0.7291 * texDetail) + 0.2739 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4716), 0.0, 1.0);
    float emit = atlas.a * (0.6251 + 0.3801 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1532 * (0.5 + 0.5 * sin(time * 0.172788)));
    ...
    float alpha = vertexColor.a * min(0.4062 + 0.4891 * presence + 0.4546 * pattern + 0.0999 * (1.0 - deepTrans) + 0.1801 * emit, 0.8846);
