...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.005833, 0.046667) * time, time);
    deep += 0.5217 * deepField(duvBase + rimDir * 0.0641 + vec2(0.009167, 0.071667) * time, time);
    deep += 0.2722 * deepField(duvBase + rimDir * 0.1282 + vec2(0.013333, 0.098333) * time, time);
    deep = pow(clamp(deep * 0.5575, 0.0, 1.0), 1.0665);
    ...
    float flash = 0.6 + 0.4 * sin(time * 3.858923 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.3234 * deep + 0.9178 * mid + 0.5473 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8783 + pattern * 0.5238);
    vec3 rgb = vertexColor.rgb * (0.4525 + 0.9470 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1811 + 0.6393 * pattern, 0.0, 1.0);
