...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7435, 0.0, 1.0);
    ...
    deepCol += 0.5217 * dp * mix(baseCol, deepStop, 0.2832);
    deepPat += 0.5217 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 5.2663) + par * 0.1282, time);
    deepCol += 0.2722 * dp * mix(baseCol, deepStop, 0.5665);
    deepPat += 0.2722 * dp;
    deepCol *= 0.6590;
    deepPat = pow(clamp(deepPat * 0.5575, 0.0, 1.0), 1.0665);
    ...
    rgb += hotStop * 0.2615 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0664 + 0.3491 * presence + 0.4546 * pattern, 0.8246);
