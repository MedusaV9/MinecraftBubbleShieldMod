...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5565, 0.0, 1.0);
    ...
    deepCol += 0.4772 * dp * mix(baseCol, deepStop, 0.2439);
    deepPat += 0.4772 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.4389) + par * 0.1008, time);
    deepCol += 0.2277 * dp * mix(baseCol, deepStop, 0.4879);
    deepPat += 0.2277 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 6.6857) + par * 0.1513, time);
    deepCol += 0.1087 * dp * mix(baseCol, deepStop, 0.7318);
    deepPat += 0.1087 * dp;
    deepCol *= 0.7008;
    deepPat = pow(clamp(deepPat * 0.5514, 0.0, 1.0), 1.2972);
    ...
    rgb += hotStop * 0.1862 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0630 + 0.3669 * presence + 0.4614 * pattern, 0.8192);
