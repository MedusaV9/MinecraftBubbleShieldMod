...
void main() {
    ...
    mid = clamp(mid * (0.7453 + 0.5890 * texDetail) + 0.3255 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4750), 0.0, 1.0);
    float emit = atlas.a * (0.7606 + 0.4586 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1619 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.8433 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3962 + 0.5069 * presence + 0.4614 * pattern + 0.0926 * (1.0 - deepTrans) + 0.1427 * emit, 0.8792);
