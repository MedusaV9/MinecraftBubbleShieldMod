...
// stack: deep=caustic x4 taps | mid=rings+warp2+scroll | rim=lat | flourish=shimmer | tex=ridged
// fbm: turb x4 octaves | seed 64dea5c80bc2183e
// v8 realism: refr 0.5935+0.7136*fres/dist (cap 0.1143) | energy 0.3118+0.4144*p | flow 0.080000 c/s | soft 0.9117m
...
void main() {
    ...
    float refrAmp = min((0.5935 + 0.7136 * fresnel) / max(length(worldPos), 1.0), 0.1143);
    ...
    float emit = atlas.a * (0.4263 + 0.7004 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1619 * (0.5 + 0.5 * sin(time * 0.188496))) * atlasPoleW;
