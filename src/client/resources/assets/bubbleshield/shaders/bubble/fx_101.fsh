...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3317 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
void main() {
    ...
    deep += 0.4985 * deepField(duvBase + rimDir * 0.0618 + vec2(0.008333, -0.045000) * time, time);
    deep += 0.2485 * deepField(duvBase + rimDir * 0.1236 + vec2(0.011667, -0.060000) * time, time);
    deep += 0.1239 * deepField(duvBase + rimDir * 0.1854 + vec2(0.015000, -0.075000) * time, time);
    deep = pow(clamp(deep * 0.5345, 0.0, 1.0), 1.5929);
    ...
    float pattern = clamp(0.4012 * deep + 0.8131 * mid + 0.8750 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9620 + pattern * 0.4205);
    vec3 rgb = vertexColor.rgb * (0.4220 + 0.6308 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1361 + 0.8014 * pattern, 0.0, 1.0);
