...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8117, 0.0, 1.0);
    ...
    deepCol += 0.4985 * dp * mix(baseCol, deepStop, 0.2368);
    deepPat += 0.4985 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 5.5989) + par * 0.1236, time);
    deepCol += 0.2485 * dp * mix(baseCol, deepStop, 0.4737);
    deepPat += 0.2485 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 7.1154) + par * 0.1854, time);
    deepCol += 0.1239 * dp * mix(baseCol, deepStop, 0.7105);
    deepPat += 0.1239 * dp;
    deepCol *= 0.5900;
    deepPat = pow(clamp(deepPat * 0.5345, 0.0, 1.0), 1.5929);
    ...
    rgb += hotStop * 0.2659 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0527 + 0.3323 * presence + 0.4929 * pattern, 0.8473);
