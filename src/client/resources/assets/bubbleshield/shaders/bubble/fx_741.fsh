...
void main() {
    ...
    mid = clamp(mid * (0.6002 + 0.5160 * texDetail) + 0.2975 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4575), 0.0, 1.0);
    float emit = atlas.a * (0.5595 + 0.4440 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1683 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8121 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3800 + 0.3991 * presence + 0.3891 * pattern + 0.0975 * (1.0 - deepTrans) + 0.1870 * emit, 0.9086);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2702), 0.9086);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6629 + 0.3371 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1362), 0.0, 1.0);
