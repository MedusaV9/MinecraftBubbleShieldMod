...
// stack: deep=voro x3 taps | mid=hex+none+scroll | rim=graze_sparkle | flourish=shimmer | tex=hex_lattice
// fbm: ridged x4 octaves | seed 332eed8f8c939137
// v8 realism: refr 0.5563+0.4616*fres/dist (cap 0.1599) | energy 0.3455+0.4654*p | flow 0.058333 c/s | soft 1.0225m
...
void main() {
    ...
    float refrAmp = min((0.5563 + 0.4616 * fresnel) / max(length(worldPos), 1.0), 0.1599);
