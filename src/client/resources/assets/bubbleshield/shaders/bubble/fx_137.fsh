...
void main() {
    ...
    mid = clamp(mid * (0.7273 + 0.7404 * texDetail) + 0.2682 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4751), 0.0, 1.0);
    float emit = atlas.a * (0.7402 + 0.4360 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1544 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.9824 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3701 + 0.3835 * presence + 0.3686 * pattern + 0.1212 * (1.0 - deepTrans) + 0.1625 * emit, 0.8681);
