...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7004, 0.0, 1.0);
    ...
    deepCol += 0.5128 * dp * mix(baseCol, deepStop, 0.2017);
    deepPat += 0.5128 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.1560) + par * 0.0644, time);
    deepCol += 0.2630 * dp * mix(baseCol, deepStop, 0.4034);
    deepPat += 0.2630 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 5.2346) + par * 0.0966, time);
    deepCol += 0.1348 * dp * mix(baseCol, deepStop, 0.6051);
    deepPat += 0.1348 * dp;
    deepCol *= 0.6702;
    deepPat = pow(clamp(deepPat * 0.5234, 0.0, 1.0), 1.4954);
    ...
    rgb = satLift(rgb, 1.2437);
    rgb += hotStop * 0.2364 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0423 + 0.2986 * presence + 0.4185 * pattern, 0.8201);
