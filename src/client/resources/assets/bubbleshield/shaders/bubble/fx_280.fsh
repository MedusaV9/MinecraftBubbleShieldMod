...
void main() {
    ...
    mid = clamp(mid * (0.7375 + 0.6404 * texDetail) + 0.2482 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4381), 0.0, 1.0);
    float emit = atlas.a * (0.6149 + 0.3935 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1029 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.9865 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3349 + 0.4386 * presence + 0.4185 * pattern + 0.1204 * (1.0 - deepTrans) + 0.1819 * emit, 0.8801);
