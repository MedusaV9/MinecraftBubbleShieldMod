...
void main() {
    ...
    mid = clamp(mid * (0.7325 + 0.6881 * texDetail) + 0.3244 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4222), 0.0, 1.0);
    float emit = atlas.a * (0.7270 + 0.2645 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1518 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.9015 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3569 + 0.5086 * presence + 0.4551 * pattern + 0.1444 * (1.0 - deepTrans) + 0.1837 * emit, 0.8942);
