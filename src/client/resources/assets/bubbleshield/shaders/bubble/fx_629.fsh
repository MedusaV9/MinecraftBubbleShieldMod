...
void main() {
    ...
    mid = clamp(mid * (0.6182 + 0.6372 * texDetail) + 0.2810 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4186), 0.0, 1.0);
    float emit = atlas.a * (0.7499 + 0.4564 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1118 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.7426 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4056 + 0.5081 * presence + 0.3818 * pattern + 0.0872 * (1.0 - deepTrans) + 0.1874 * emit, 0.9085);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1508), 0.9085);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7371 + 0.2629 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
