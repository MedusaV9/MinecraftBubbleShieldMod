...
void main() {
    ...
    mid = clamp(mid * (0.7458 + 0.6826 * texDetail) + 0.2704 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4746), 0.0, 1.0);
    float emit = atlas.a * (0.6258 + 0.3890 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1193 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8625 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3221 + 0.3128 * presence + 0.5667 * pattern + 0.1551 * (1.0 - deepTrans) + 0.1656 * emit, 0.8771);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2097), 0.8771);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7214 + 0.2786 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1227), 0.0, 1.0);
