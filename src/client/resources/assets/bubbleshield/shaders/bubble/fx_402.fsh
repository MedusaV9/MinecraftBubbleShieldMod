...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6448, 0.0, 1.0);
    ...
    deepCol += 0.5174 * dp * mix(baseCol, deepStop, 0.3170);
    deepPat += 0.5174 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 4.5073) + par * 0.0697, time);
    deepCol += 0.2677 * dp * mix(baseCol, deepStop, 0.6341);
    deepPat += 0.2677 * dp;
    deepCol *= 0.6767;
    deepPat = pow(clamp(deepPat * 0.5602, 0.0, 1.0), 1.6904);
    ...
    float tendril = pow(clamp(1.0 - abs(2.0 * tnoise - 1.0), 0.0, 1.0), 7.4000);
    float lat = abs(baseUV.y * 2.0 - 1.0);
    float reach = smoothstep(0.1202, 0.9060, lat + 0.1532 * sin(time * 0.115192));
    float murk = fbm3(mdir * 0.6 + vec3(7.7, 2.2, 5.5)) * 0.1546;
    float mid = clamp(tendril * reach * 1.0675 + murk, 0.0, 1.2);
    ...
    float pattern = clamp(0.4385 * deepPat + 0.8429 * mid + 0.5969 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.3410 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0680 + 0.3622 * presence + 0.4099 * pattern, 0.8105);
