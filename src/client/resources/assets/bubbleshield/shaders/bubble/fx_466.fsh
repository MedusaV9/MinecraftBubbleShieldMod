...
void main() {
    ...
    mid = clamp(mid * (0.7402 + 0.6423 * texDetail) + 0.3042 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4506), 0.0, 1.0);
    float emit = atlas.a * (0.7448 + 0.4887 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1025 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.9898 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3371 + 0.3749 * presence + 0.4540 * pattern + 0.0802 * (1.0 - deepTrans) + 0.1726 * emit, 0.8779);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3077), 0.8779);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6931 + 0.3069 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
