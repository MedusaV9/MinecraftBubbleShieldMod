...
void main() {
    ...
    mid = clamp(mid * (0.7104 + 0.6457 * texDetail) + 0.2416 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7207 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4298), 0.0, 1.0);
    float emit = atlas.a * (0.7708 + 0.3327 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1519 * (0.5 + 0.5 * sin(time * 0.235619)));
    ...
    float alpha = vertexColor.a * min(0.3738 + 0.3315 * presence + 0.4808 * pattern + 0.0872 * (1.0 - deepTrans) + 0.1819 * emit, 0.9093);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2684), 0.9093);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7696 + 0.2304 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
