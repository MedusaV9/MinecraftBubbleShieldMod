...
void main() {
    ...
    mid = clamp(mid * (0.6373 + 0.6840 * texDetail) + 0.2653 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5473), 0.0, 1.0);
    float emit = atlas.a * (0.6471 + 0.2873 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1309 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.9820 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3165 + 0.3364 * presence + 0.5190 * pattern + 0.1113 * (1.0 - deepTrans) + 0.1516 * emit, 0.8513);
