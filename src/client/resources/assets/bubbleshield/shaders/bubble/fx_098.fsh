...
void main() {
    ...
    deepCol += 0.4703 * dp * mix(baseCol, deepStop, 0.2481);
    deepPat += 0.4703 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.5578) + par * 0.0628, time);
    deepCol += 0.2212 * dp * mix(baseCol, deepStop, 0.4961);
    deepPat += 0.2212 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 4.2455) + par * 0.0942, time);
    deepCol += 0.1040 * dp * mix(baseCol, deepStop, 0.7442);
    deepPat += 0.1040 * dp;
    deepCol *= 0.6024;
    deepPat = pow(clamp(deepPat * 0.5569, 0.0, 1.0), 1.7055);
    ...
    rgb += hotStop * 0.1881 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0784 + 0.2696 * presence + 0.4498 * pattern, 0.8501);
