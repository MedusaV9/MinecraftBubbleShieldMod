...
void main() {
    ...
    mid = clamp(mid * (0.6713 + 0.5328 * texDetail) + 0.2396 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4789), 0.0, 1.0);
    float emit = atlas.a * (0.6302 + 0.4561 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1598 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.9391 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4418 + 0.4096 * presence + 0.4498 * pattern + 0.1582 * (1.0 - deepTrans) + 0.1516 * emit, 0.9101);
