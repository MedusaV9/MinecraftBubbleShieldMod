...
void main() {
    ...
    mid = clamp(mid * (0.6865 + 0.5158 * texDetail) + 0.3022 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5401), 0.0, 1.0);
    float emit = atlas.a * (0.6063 + 0.3944 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1312 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.6734 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2814 + 0.2863 * presence + 0.5453 * pattern + 0.1526 * (1.0 - deepTrans) + 0.1238 * emit, 0.8907);
