...
void main() {
    ...
    deep += 0.4040 * deepField(duvBase + rimDir * 0.0711 + vec2(0.035000, -0.027500) * time, time);
    deep += 0.1632 * deepField(duvBase + rimDir * 0.1422 + vec2(0.046667, -0.037500) * time, time);
    deep += 0.0659 * deepField(duvBase + rimDir * 0.2133 + vec2(0.058333, -0.047500) * time, time);
    deep = pow(clamp(deep * 0.6123, 0.0, 1.0), 1.5088);
    ...
    float rn = fbm2(wuv + vec2(time * 0.116667, time * 0.035000), midPer);
