...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6912, 0.0, 1.0);
    ...
    deepCol += 0.5080 * dp * mix(baseCol, deepStop, 0.2524);
    deepPat += 0.5080 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 3.5626) + par * 0.0989, time);
    deepCol += 0.2581 * dp * mix(baseCol, deepStop, 0.5048);
    deepPat += 0.2581 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.4237) + par * 0.1484, time);
    deepCol += 0.1311 * dp * mix(baseCol, deepStop, 0.7573);
    deepPat += 0.1311 * dp;
    deepCol *= 0.6706;
    deepPat = pow(clamp(deepPat * 0.5271, 0.0, 1.0), 1.7485);
    ...
    rgb += hotStop * 0.2343 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0614 + 0.3452 * presence + 0.4383 * pattern, 0.8382);
