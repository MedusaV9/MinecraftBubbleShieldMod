...
// stack: deep=caustic x4 taps | mid=starfield+warp1+flicker | rim=graze_film | flourish=shimmer | tex=starfield
// fbm: ridged x6 octaves | seed b137363b2ff2a796
// v8 realism: refr 0.3229+0.4133*fres/dist (cap 0.1248) | energy 0.5116+0.4174*p | flow 0.072500 c/s | soft 0.6005m
...
void main() {
    ...
    float refrAmp = min((0.3229 + 0.4133 * fresnel) / max(length(worldPos), 1.0), 0.1248);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0369, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9631, vec2(0.001), vec2(0.999))).b);
