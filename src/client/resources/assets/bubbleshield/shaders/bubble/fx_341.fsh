...
void main() {
    ...
    deep += 0.5080 * deepField(duvBase + rimDir * 0.0495 + vec2(0.065000, -0.055000) * time, time);
    deep += 0.2581 * deepField(duvBase + rimDir * 0.0989 + vec2(0.088333, -0.075000) * time, time);
    deep += 0.1311 * deepField(duvBase + rimDir * 0.1484 + vec2(0.110000, -0.095000) * time, time);
    deep = pow(clamp(deep * 0.5271, 0.0, 1.0), 1.7485);
