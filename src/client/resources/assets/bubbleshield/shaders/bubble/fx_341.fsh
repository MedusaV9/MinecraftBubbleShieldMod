...
void main() {
    ...
    mid = clamp(mid * (0.6638 + 0.6699 * texDetail) + 0.2838 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4163), 0.0, 1.0);
    float emit = atlas.a * (0.7453 + 0.3811 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1036 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.7351 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3163 + 0.3048 * presence + 0.5486 * pattern + 0.1464 * (1.0 - deepTrans) + 0.1651 * emit, 0.8982);
