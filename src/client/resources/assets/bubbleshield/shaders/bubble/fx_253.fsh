...
void main() {
    ...
    mid = clamp(mid * (0.6188 + 0.7367 * texDetail) + 0.2441 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4764), 0.0, 1.0);
    float emit = atlas.a * (0.6538 + 0.4086 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1176 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.9363 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4081 + 0.3968 * presence + 0.4114 * pattern + 0.0842 * (1.0 - deepTrans) + 0.1742 * emit, 0.8688);
