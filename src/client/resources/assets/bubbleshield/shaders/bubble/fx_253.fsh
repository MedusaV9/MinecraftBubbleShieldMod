...
// stack: deep=ridge x4 taps | mid=hex+warp2+pulse | rim=lat | flourish=echo | tex=hex_lattice
// fbm: standard x6 octaves | seed bb0825f77de2f0f3
// v8 realism: refr 0.7178+0.8782*fres/dist (cap 0.1384) | energy 0.3219+0.4142*p | flow 0.050833 c/s | soft 0.6422m
...
void main() {
    ...
    float refrAmp = min((0.7178 + 0.8782 * fresnel) / max(length(worldPos), 1.0), 0.1384);
