...
void main() {
    ...
    mid = clamp(mid * (0.6488 + 0.6742 * texDetail) + 0.3351 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5075), 0.0, 1.0);
    float emit = atlas.a * (0.7306 + 0.4150 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1001 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.9562 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3147 + 0.3333 * presence + 0.4750 * pattern + 0.1509 * (1.0 - deepTrans) + 0.1238 * emit, 0.9155);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3443), 0.9155);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7358 + 0.2642 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0629), 0.0, 1.0);
