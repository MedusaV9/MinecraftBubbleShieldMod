...
void main() {
    ...
    mid = clamp(mid * (0.6943 + 0.4547 * texDetail) + 0.2067 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4832), 0.0, 1.0);
    float emit = atlas.a * (0.7236 + 0.4027 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1258 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.9187 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3943 + 0.3951 * presence + 0.4035 * pattern + 0.1233 * (1.0 - deepTrans) + 0.1498 * emit, 0.8753);
