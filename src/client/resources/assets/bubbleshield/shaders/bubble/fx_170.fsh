...
// stack: deep=fbm x2 taps | mid=waves+curl+flicker | rim=graze_sparkle | flourish=swirl
...
void main() {
    ...
    deep += 0.5578 * deepField(duvBase + rimDir * 0.0554 + vec2(-0.063333, -0.067500) * time, time);
    deep = pow(clamp(deep * 0.6419, 0.0, 1.0), 1.2299);
    ...
    float pattern = clamp(0.4959 * deep + 0.8158 * mid + 0.6457 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.0241 + pattern * 0.5560);
    vec3 rgb = vertexColor.rgb * (0.4966 + 0.5608 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1442 + 0.7959 * pattern, 0.0, 1.0);
