...
void main() {
    ...
    mid = clamp(mid * (0.7471 + 0.5169 * texDetail) + 0.3182 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4334), 0.0, 1.0);
    float emit = atlas.a * (0.5691 + 0.4430 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1517 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.6681 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3548 + 0.3129 * presence + 0.5999 * pattern + 0.1112 * (1.0 - deepTrans) + 0.2119 * emit, 0.8886);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2841), 0.8886);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7145 + 0.2855 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
