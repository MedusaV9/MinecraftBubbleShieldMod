...
void main() {
    ...
    mid = clamp(mid * (0.6089 + 0.7341 * texDetail) + 0.2128 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3833), 0.0, 1.0);
    float emit = atlas.a * (0.6678 + 0.3530 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1067 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.8315 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2921 + 0.2874 * presence + 0.4822 * pattern + 0.1414 * (1.0 - deepTrans) + 0.1602 * emit, 0.8914);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2500), 0.8914);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7211 + 0.2789 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
