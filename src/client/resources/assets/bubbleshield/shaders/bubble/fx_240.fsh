...
void main() {
    ...
    mid = clamp(mid * (0.6131 + 0.6554 * texDetail) + 0.3306 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4325), 0.0, 1.0);
    float emit = atlas.a * (0.5875 + 0.4389 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1008 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.8354 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3540 + 0.4515 * presence + 0.3355 * pattern + 0.0808 * (1.0 - deepTrans) + 0.1791 * emit, 0.8997);
