...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.015000, 0.017500) * time, time);
    deep += 0.4275 * deepField(duvBase + rimDir * 0.0700 + vec2(0.023333, 0.025000) * time, time);
    deep = pow(clamp(deep * 0.7005, 0.0, 1.0), 1.2613);
    ...
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.150000, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.070000), midPer)), midPer);
