...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8617, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0426, abs(glR - 0.1291));
    float mid = clamp(glStars * (1.0 + glRing * 1.3606) + glRing * 0.5230, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6591 + 0.5773 * texDetail) + 0.2151 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8300 + 0.4155 * texDetail;
    rgb = satLift(rgb, 1.2722);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3597), 0.0, 1.0);
    float emit = atlas.a * (0.7332 + 0.3583 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1618 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.9445 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3101 + 0.2754 * presence + 0.5153 * pattern + 0.1240 * (1.0 - deepTrans) + 0.1982 * emit, 0.8974);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1755), 0.8974);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7798 + 0.2202 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0938), 0.0, 1.0);
