...
void main() {
    ...
    mid = clamp(mid * (0.7074 + 0.4816 * texDetail) + 0.2023 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5306), 0.0, 1.0);
    float emit = atlas.a * (0.6042 + 0.3133 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0846 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.9762 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3479 + 0.4991 * presence + 0.3568 * pattern + 0.1311 * (1.0 - deepTrans) + 0.1649 * emit, 0.8962);
