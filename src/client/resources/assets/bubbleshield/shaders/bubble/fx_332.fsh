...
// stack: deep=ridge x2 taps | mid=sparkle+warp2+flicker | rim=graze_sparkle | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.025000, -0.042500) * time, time);
    deep += 0.4408 * deepField(duvBase + rimDir * 0.0433 + vec2(-0.038333, -0.065000) * time, time);
    deep = pow(clamp(deep * 0.6941, 0.0, 1.0), 1.1208);
    ...
    float pattern = clamp(0.3706 * deep + 0.9420 * mid + 0.6657 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.4057 + pattern * 0.3808);
    vec3 rgb = vertexColor.rgb * (0.5363 + 0.7084 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1589 + 0.7436 * pattern, 0.0, 1.0);
