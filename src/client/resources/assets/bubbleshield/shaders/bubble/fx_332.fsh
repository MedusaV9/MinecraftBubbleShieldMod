...
// stack: deep=ridge x3 taps | mid=sparkle+warp2+flicker | rim=graze_sparkle | flourish=shimmer | tex=starfield
// fbm: turb x4 octaves | seed 01209a1e1a923097
// v8 realism: refr 0.7396+0.9626*fres/dist (cap 0.1111) | energy 0.3104+0.4382*p | flow 0.097500 c/s | soft 0.7261m
...
void main() {
    ...
    texDetail = clamp(texDetail * 1.9463 + 0.2570, 0.0, 1.0);
    ...
    float refrAmp = min((0.7396 + 0.9626 * fresnel) / max(length(worldPos), 1.0), 0.1111);
    ...
    float emitMask = clamp(atlas.a * 2.3790, 0.0, 1.0);
    ...
    float emit = emitMask * (0.3325 + 0.5260 * emitDrive) * (1.0 - 0.0846 * (0.5 + 0.5 * sin(time * 0.230383))) * atlasPoleW;
