...
void main() {
    ...
    mid = clamp(mid * (0.6438 + 0.5486 * texDetail) + 0.2217 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4920), 0.0, 1.0);
    float emit = atlas.a * (0.5794 + 0.4806 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1251 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.8375 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2858 + 0.2857 * presence + 0.4944 * pattern + 0.1458 * (1.0 - deepTrans) + 0.1541 * emit, 0.8850);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2390), 0.8850);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7008 + 0.2992 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1338), 0.0, 1.0);
