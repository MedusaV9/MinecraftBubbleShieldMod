...
void main() {
    ...
    mid = clamp(mid * (0.6991 + 0.6672 * texDetail) + 0.3208 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4180), 0.0, 1.0);
    float emit = atlas.a * (0.6114 + 0.3461 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1718 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.9890 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3928 + 0.4787 * presence + 0.3218 * pattern + 0.1539 * (1.0 - deepTrans) + 0.1558 * emit, 0.8843);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1705), 0.8843);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7504 + 0.2496 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
