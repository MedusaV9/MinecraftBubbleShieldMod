...
void main() {
    ...
    mid = clamp(mid * (0.6912 + 0.6317 * texDetail) + 0.2485 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5107), 0.0, 1.0);
    float emit = atlas.a * (0.6069 + 0.2728 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1268 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.8058 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3612 + 0.4471 * presence + 0.4267 * pattern + 0.1082 * (1.0 - deepTrans) + 0.1555 * emit, 0.8412);
