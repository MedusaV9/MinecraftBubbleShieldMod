...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6184, 0.0, 1.0);
    ...
    deepCol += 0.4943 * dp * mix(baseCol, deepStop, 0.2589);
    deepPat += 0.4943 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 3.0695) + par * 0.0672, time);
    deepCol += 0.2444 * dp * mix(baseCol, deepStop, 0.5177);
    deepPat += 0.2444 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 3.7863) + par * 0.1008, time);
    deepCol += 0.1208 * dp * mix(baseCol, deepStop, 0.7766);
    deepPat += 0.1208 * dp;
    deepCol *= 0.5493;
    deepPat = pow(clamp(deepPat * 0.5378, 0.0, 1.0), 1.4672);
    ...
    rgb += hotStop * 0.2341 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0548 + 0.3512 * presence + 0.4595 * pattern, 0.8560);
