...
void main() {
    ...
    mid = clamp(mid * (0.6939 + 0.5613 * texDetail) + 0.2560 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4234), 0.0, 1.0);
    float emit = atlas.a * (0.7816 + 0.3080 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1013 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.8541 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3679 + 0.3078 * presence + 0.4682 * pattern + 0.1370 * (1.0 - deepTrans) + 0.1690 * emit, 0.8875);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2074), 0.8875);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7709 + 0.2291 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1155), 0.0, 1.0);
