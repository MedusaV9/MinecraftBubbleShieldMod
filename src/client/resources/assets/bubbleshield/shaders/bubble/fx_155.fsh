...
void main() {
    ...
    mid = clamp(mid * (0.6653 + 0.7211 * texDetail) + 0.2707 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4085), 0.0, 1.0);
    float emit = atlas.a * (0.5783 + 0.2749 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1187 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.8130 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4241 + 0.4558 * presence + 0.4695 * pattern + 0.0964 * (1.0 - deepTrans) + 0.1277 * emit, 0.9062);
