...
// stack: deep=ridge x2 taps | mid=interference+none+flicker | rim=lat | flourish=shimmer
...
void main() {
    ...
    deep += 0.4400 * deepField(duvBase + rimDir * 0.0378 + vec2(-0.053333, -0.060000) * time, time);
    deep = pow(clamp(deep * 0.6944, 0.0, 1.0), 1.2700);
    ...
    float pattern = clamp(0.4628 * deep + 0.8779 * mid + 0.7428 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5772 + pattern * 0.4927);
    vec3 rgb = vertexColor.rgb * (0.3880 + 0.8827 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2149 + 0.7297 * pattern, 0.0, 1.0);
