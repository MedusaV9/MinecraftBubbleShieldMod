...
// stack: deep=fbm x2 taps | mid=arcs+none+rotate | rim=graze_sparkle | flourish=swirl
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.005833, -0.036667) * time, time);
    deep += 0.3910 * deepField(duvBase + rimDir * 0.0773 + vec2(-0.009167, -0.058333) * time, time);
    deep = pow(clamp(deep * 0.7189, 0.0, 1.0), 1.3417);
    ...
    float flash = 0.6 + 0.4 * sin(time * 2.230531 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.4879 * deep + 0.9113 * mid + 0.8938 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1562 + pattern * 0.5539);
    vec3 rgb = vertexColor.rgb * (0.5096 + 0.7854 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2109 + 0.6076 * pattern, 0.0, 1.0);
