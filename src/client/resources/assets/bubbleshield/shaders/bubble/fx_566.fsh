...
void main() {
    ...
    mid = clamp(mid * (0.7115 + 0.6844 * texDetail) + 0.2909 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4800), 0.0, 1.0);
    float emit = atlas.a * (0.7356 + 0.3301 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1544 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.6714 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3980 + 0.4343 * presence + 0.4331 * pattern + 0.1065 * (1.0 - deepTrans) + 0.1609 * emit, 0.8723);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2614), 0.8723);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7785 + 0.2215 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1233), 0.0, 1.0);
