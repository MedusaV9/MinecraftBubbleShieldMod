...
void main() {
    ...
    mid = clamp(mid * (0.6738 + 0.5309 * texDetail) + 0.3114 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4138), 0.0, 1.0);
    float emit = atlas.a * (0.6992 + 0.4370 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1088 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.9546 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3581 + 0.3212 * presence + 0.5434 * pattern + 0.1014 * (1.0 - deepTrans) + 0.1615 * emit, 0.8909);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1918), 0.8909);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7032 + 0.2968 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0621), 0.0, 1.0);
