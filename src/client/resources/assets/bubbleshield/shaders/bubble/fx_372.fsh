...
void main() {
    ...
    mid = clamp(mid * (0.6597 + 0.5684 * texDetail) + 0.1905 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5344), 0.0, 1.0);
    float emit = atlas.a * (0.7984 + 0.3477 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1369 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.9889 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2897 + 0.3290 * presence + 0.4539 * pattern + 0.1409 * (1.0 - deepTrans) + 0.1300 * emit, 0.9580);
