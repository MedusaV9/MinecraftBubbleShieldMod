...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5885, 0.0, 1.0);
    ...
    deepCol += 0.4939 * dp * mix(baseCol, deepStop, 0.3957);
    deepPat += 0.4939 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 5.4476) + par * 0.0903, time);
    deepCol += 0.2439 * dp * mix(baseCol, deepStop, 0.7913);
    deepPat += 0.2439 * dp;
    deepCol *= 0.7381;
    deepPat = pow(clamp(deepPat * 0.5754, 0.0, 1.0), 1.4314);
    ...
    float accretion = invsmooth(0.0, 0.0420, abs(prad - 0.4503));
    float mid = clamp((infall * 0.45 + suck * 0.65) * (1.0 - voidCore) * smoothstep(0.10, 0.55, prad) + accretion * (0.8 + 0.2 * sin(time * 2.000147)), 0.0, 1.3);
    ...
    float pattern = clamp(0.3435 * deepPat + 0.9308 * mid + 0.6455 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.2767 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0428 + 0.3197 * presence + 0.3581 * pattern, 0.8580);
