...
// stack: deep=fbm x4 taps | mid=mycelia+curl+pulse | rim=graze_sparkle | flourish=glint | tex=filaments
// fbm: ridged x3 octaves | seed bcbe65e60bd6c2e8
// v8 realism: refr 0.3052+0.4005*fres/dist (cap 0.1282) | energy 0.5514+0.3925*p | flow 0.100000 c/s | soft 0.8871m
...
void main() {
    ...
    float refrAmp = min((0.3052 + 0.4005 * fresnel) / max(length(worldPos), 1.0), 0.1282);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0555, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9445, vec2(0.001), vec2(0.999))).b);
