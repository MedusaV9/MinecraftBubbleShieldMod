...
void main() {
    ...
    mid = clamp(mid * (0.6299 + 0.7303 * texDetail) + 0.2352 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3874), 0.0, 1.0);
    float emit = atlas.a * (0.7899 + 0.4557 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1082 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.8410 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3546 + 0.2762 * presence + 0.4978 * pattern + 0.1132 * (1.0 - deepTrans) + 0.1499 * emit, 0.9136);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2746), 0.9136);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6380 + 0.3620 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1005), 0.0, 1.0);
