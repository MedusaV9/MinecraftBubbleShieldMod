...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9948) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9948);
        amplitude *= 0.5;
    }
    return value;
}
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.539307);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.005000, -0.012500) * time, time);
    deep += 0.4380 * deepField(duvBase + rimDir * 0.0466 + vec2(-0.008333, -0.020000) * time, time);
    deep = pow(clamp(deep * 0.6954, 0.0, 1.0), 1.4464);
    ...
    float rn = fbm2(wuv + vec2(time * 0.083333, time * 0.075833), midPer);
