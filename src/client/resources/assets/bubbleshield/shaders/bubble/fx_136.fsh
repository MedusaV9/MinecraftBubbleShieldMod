...
void main() {
    ...
    mid = clamp(mid * (0.7043 + 0.7048 * texDetail) + 0.3040 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4879), 0.0, 1.0);
    float emit = atlas.a * (0.5949 + 0.3208 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0973 * (0.5 + 0.5 * sin(time * 0.141372)));
    ...
    float alpha = vertexColor.a * min(0.4019 + 0.4122 * presence + 0.3346 * pattern + 0.0861 * (1.0 - deepTrans) + 0.1382 * emit, 0.8779);
