...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7131, 0.0, 1.0);
    ...
    deepCol += 0.4088 * dp * mix(baseCol, deepStop, 0.2383);
    deepPat += 0.4088 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 4.7794) + par * 0.0873, time);
    deepCol += 0.1671 * dp * mix(baseCol, deepStop, 0.4767);
    deepPat += 0.1671 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.7317) + par * 0.1309, time);
    deepCol += 0.0683 * dp * mix(baseCol, deepStop, 0.7150);
    deepPat += 0.0683 * dp;
    deepCol *= 0.6414;
    deepPat = pow(clamp(deepPat * 0.6082, 0.0, 1.0), 1.5755);
    ...
    rgb += hotStop * 0.3163 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0752 + 0.3347 * presence + 0.3610 * pattern, 0.8563);
