...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0182) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0182);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.013333, 0.022500) * time, time);
    deep += 0.3990 * deepField(duvBase + rimDir * 0.0339 + vec2(0.020000, 0.035000) * time, time);
    deep = pow(clamp(deep * 0.7148, 0.0, 1.0), 1.0856);
