...
void main() {
    ...
    mid = clamp(mid * (0.6823 + 0.7068 * texDetail) + 0.3220 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4981), 0.0, 1.0);
    float emit = atlas.a * (0.7441 + 0.4773 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1340 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.8451 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3203 + 0.4365 * presence + 0.3415 * pattern + 0.0978 * (1.0 - deepTrans) + 0.1942 * emit, 0.9025);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2212), 0.9025);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1121), 0.0, 1.0);
