...
// stack: deep=fbm x2 taps | mid=interference+warp2+scroll | rim=graze_film | flourish=echo
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.045000, -0.035000) * time, time);
    deep += 0.3944 * deepField(duvBase + rimDir * 0.0593 + vec2(0.070000, -0.055000) * time, time);
    deep = pow(clamp(deep * 0.7171, 0.0, 1.0), 1.5471);
    ...
    float pattern = clamp(0.4810 * deep + 0.9434 * mid + 0.6516 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6414 + pattern * 0.3981);
    vec3 rgb = vertexColor.rgb * (0.4437 + 0.6938 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2354 + 0.7213 * pattern, 0.0, 1.0);
