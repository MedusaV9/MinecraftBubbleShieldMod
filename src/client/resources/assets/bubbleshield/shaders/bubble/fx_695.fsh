...
void main() {
    ...
    mid = clamp(mid * (0.7338 + 0.5778 * texDetail) + 0.3096 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3895), 0.0, 1.0);
    float emit = atlas.a * (0.5770 + 0.3630 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0974 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.8120 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2978 + 0.3162 * presence + 0.5950 * pattern + 0.1074 * (1.0 - deepTrans) + 0.1306 * emit, 0.8524);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2225), 0.8524);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7130 + 0.2870 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
