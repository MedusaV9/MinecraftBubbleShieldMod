...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 1.9997) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 1.9997);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    deep += 0.3978 * deepField(duvBase + rimDir * 0.0576 + vec2(-0.063333, -0.027500) * time, time);
    deep += 0.1582 * deepField(duvBase + rimDir * 0.1152 + vec2(-0.086667, -0.035000) * time, time);
    deep += 0.0630 * deepField(duvBase + rimDir * 0.1729 + vec2(-0.108333, -0.045000) * time, time);
    deep = pow(clamp(deep * 0.6177, 0.0, 1.0), 1.5100);

    // [layer:mid:arcs_none_scroll]
    // Signature structure of this effect, domain-warped and animated.
    vec2 auv = vec2(baseUV.x * 5.0000, baseUV.y * 5.0000) + vec2(-0.412500, 0.367500) * time;
    vec2 wuv = auv;
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.320833), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 12.7194);
    float flash = 0.6 + 0.4 * sin(time * 4.194026 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
