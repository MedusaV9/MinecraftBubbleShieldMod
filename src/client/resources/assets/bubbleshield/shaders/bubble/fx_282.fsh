...
// stack: deep=voro x3 taps | mid=waves+curl+scroll | rim=lat | flourish=shimmer
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.560251);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.041667, 0.005000) * time, time);
    deep += 0.3937 * deepField(duvBase + rimDir * 0.0307 + vec2(0.065000, 0.007500) * time, time);
    deep += 0.1550 * deepField(duvBase + rimDir * 0.0614 + vec2(0.088333, 0.010000) * time, time);
    deep = pow(clamp(deep * 0.6457, 0.0, 1.0), 1.7518);
    ...
    float pattern = clamp(0.4092 * deep + 0.7911 * mid + 0.8932 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1632 + pattern * 0.4880);
    vec3 rgb = vertexColor.rgb * (0.4208 + 0.6651 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1643 + 0.7888 * pattern, 0.0, 1.0);
