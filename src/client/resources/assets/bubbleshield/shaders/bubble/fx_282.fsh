...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7829, 0.0, 1.0);
    ...
    deepCol += 0.3937 * dp * mix(baseCol, deepStop, 0.1898);
    deepPat += 0.3937 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 3.9916) + par * 0.0614, time);
    deepCol += 0.1550 * dp * mix(baseCol, deepStop, 0.3795);
    deepPat += 0.1550 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 4.8780) + par * 0.0920, time);
    deepCol += 0.0610 * dp * mix(baseCol, deepStop, 0.5693);
    deepPat += 0.0610 * dp;
    deepCol *= 0.6331;
    deepPat = pow(clamp(deepPat * 0.6213, 0.0, 1.0), 1.7518);
    ...
    rgb += hotStop * 0.1516 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0789 + 0.2907 * presence + 0.4833 * pattern, 0.8427);
