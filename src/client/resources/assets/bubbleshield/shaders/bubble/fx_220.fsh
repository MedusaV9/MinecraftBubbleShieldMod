...
void main() {
    ...
    mid = clamp(mid * (0.7225 + 0.4935 * texDetail) + 0.2363 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5426), 0.0, 1.0);
    float emit = atlas.a * (0.5573 + 0.3582 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0803 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.6963 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3714 + 0.4795 * presence + 0.3533 * pattern + 0.0838 * (1.0 - deepTrans) + 0.1299 * emit, 0.8701);
