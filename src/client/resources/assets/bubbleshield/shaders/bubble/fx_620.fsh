...
void main() {
    ...
    mid = clamp(mid * (0.6713 + 0.4942 * texDetail) + 0.3264 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3622), 0.0, 1.0);
    float emit = atlas.a * (0.5634 + 0.4057 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1177 * (0.5 + 0.5 * sin(time * 0.141372)));
    rgb += emit * 0.8833 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2989 + 0.3018 * presence + 0.4207 * pattern + 0.0939 * (1.0 - deepTrans) + 0.1838 * emit, 0.8075);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3480), 0.8075);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6256 + 0.3744 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
