...
void main() {
    ...
    mid = clamp(mid * (0.6767 + 0.5867 * texDetail) + 0.2930 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4925), 0.0, 1.0);
    float emit = atlas.a * (0.7948 + 0.4389 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1455 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.6935 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2956 + 0.3025 * presence + 0.4300 * pattern + 0.1215 * (1.0 - deepTrans) + 0.1440 * emit, 0.8633);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2197), 0.8633);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7515 + 0.2485 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
