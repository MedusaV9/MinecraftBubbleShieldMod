...
void main() {
    ...
    mid = clamp(mid * (0.7389 + 0.5280 * texDetail) + 0.2116 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4463), 0.0, 1.0);
    float emit = atlas.a * (0.6136 + 0.4796 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0820 * (0.5 + 0.5 * sin(time * 0.104720)));
    ...
    float alpha = vertexColor.a * min(0.4559 + 0.5279 * presence + 0.3738 * pattern + 0.1113 * (1.0 - deepTrans) + 0.1655 * emit, 0.9258);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2149), 0.9258);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7221 + 0.2779 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
