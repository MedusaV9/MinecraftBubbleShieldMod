...
// Bubble surface shader fx_794 -- family DEEPICE
// stack: deep=caustic x4 taps | mid=deepice+warp2+rotate | rim=lat | flourish=echo | tex=topo_contours | motif=corex4+sparklegate
// fbm: turb x3 octaves | seed a9e5cb8ed41d4858
// v8 realism: refr 1.7154+1.9792*fres/dist (cap 0.1542) | energy 0.2051+0.4645*p | flow 0.097500 c/s | soft 0.5892m
...
vec2 warp1(vec2 p, vec2 per, float t) {
    return p + 0.5594 * (vec2(fbm2(p + vec2(0.0, t * 0.306667), per),
...
vec2 warp2(vec2 p, vec2 per, float t) {
    ...
    return p + 0.6338 * (vec2(fbm2(q + vec2(1.7, 9.2), per),
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3453 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(1.1482, 1.1648, 1.1335) * t
        + vec3(0.4176, 0.3493, 0.2163)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4586, x));
    return mix(g, hotStop, smoothstep(0.4586, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0058, 0.0493, g);
}

// latitude band rim: pole caps plus a soft equator belt
float rimLat(vec2 uv) {
    float lat = abs(uv.y * 2.0 - 1.0);
    float poles = smoothstep(0.6984, 1.0, lat);
    float belt = invsmooth(0.0, 0.2777, lat);
...
float ringPulse(vec2 uv, float t) {
    ...
    for (int i = 0; i < 3; i++) {
        float fi = float(i);
        vec2 c = hash22(vec2(fi * 7.31 + 29.2126, fi * 2.97));
        float phase = fract(t * 0.164167 + fi * 0.37);
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.6681;
    vec3 spun = clamp(hueSpin(baseCol, -0.2835), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2649) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5588, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.6220);
    ...
    vec3 spinAxis = vec3(-0.0739, -0.9408, -0.3307);
    vec3 par = Vt * 2.3846;
    ...
    float deepBreath = 1.0 + 0.0126 * sin(time * 0.869174);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * (1.7723 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.3535 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * (2.3236 * deepBreath)) + par * 0.0533, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.3779), deepStop, 0.1386);
    deepTrans *= 1.0 - 0.3535 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * (2.8749 * deepBreath)) + par * 0.1065, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.7558), deepStop, 0.2771);
    deepTrans *= 1.0 - 0.3535 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * (3.4263 * deepBreath)) + par * 0.1598, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4157);
    deepTrans *= 1.0 - 0.3535 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.5906;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.2117, 0.0, 1.0), 1.5298);

    // [layer:mid:deepice_warp2_rotate]
    ...
    vec2 iceOff = rimDir * (0.2066 + 0.9266 * rimGraze());
    vec3 iceB = voro2(wuv * 2.0 + iceOff + vec2(13.7, 5.9), midPer * 2.0, 0.0);
    float crackA = invsmooth(0.004, 0.0598, iceA.x);
    float crackB = invsmooth(0.004, 0.0302, iceB.x);
    ...
    vec2 texUV = wuv * 2.0000 + vec2(-0.040000, -0.034167) * time;
    ...
    vec3 hgtW = vec3(0.4183, 0.3573, 0.2244);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -3.8727;
    ...
    float texDetail = clamp(atlas.r * 0.3777 + atlas.g * 0.3227 + atlas.b * 0.2026, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.7389 + 0.5280 * texDetail, atlasPoleW) + 0.1186 * texDetail * (0.2004 + 0.7996 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.3476 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 3.4757);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0555).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0555).rgb;
    float deepTex = 0.5 * (dot(deepTexA, hgtW) + dot(deepTexB, hgtW));

    // [layer:thick:chord]
    // v11 volumetric thickness: the membrane is a shell of relative
    // thickness rho = 0.0932 (crystalline material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.8430.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9068 * 0.9068 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.8430);
    float chordN = min(chord / 0.0932, 3.0);
    ...
    float rimLine = smoothstep(0.0545, 0.0875, rimSlope);
    float rim = rimLat(baseUV) * 0.7740;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.6008 * rimLine + 0.8472 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2085 * ringPulse(baseUV + vec2(0.13, 0.31), time);
    float grain = 0.0435 * (cellHash(floor(wuv * 57.0000) + vec2(floor(time * 2.0), 0.0), 342.0000) - 0.5);
    ...
    float motifEnv = 0.48 + 0.52 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.712094)) * (0.5 + 0.5 * sin(time * 0.623083 + 1.7)));
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 9.6441) * (0.75 + 0.25 * sin(time * 0.644026 + 1.5082));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 9.6441);
    float motifPing = fract(time * 0.129167 + 0.8303);
    float motifPd = (motifR - motifPing * 0.4737) * 25.6949;
    motifM += 0.4787 * exp(-motifPd * motifPd) * (1.0 - motifPing);
    ...
    float v5Par = deepField(sdir * 1.7306 + par * 0.0517, time) * 0.5;
    v5Par += deepField(sdir * 2.3364 + par * 0.1034, time) * 0.3;
    v5Par += deepField(sdir * 3.0286 + par * 0.1551, time) * 0.2;
    float pattern = clamp(0.4267 * deepPat * mix(1.0, chordN, 0.2005) + 0.7881 * mid + 0.8730 * rim + flourish + grain + 0.1584 * v5Par + 0.1546 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.8084 + rim * 0.2337, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9603 * mid + 0.6174 * rim, 0.0, 1.0);
    ...
    rgb *= mix(1.0, texBody, atlasPoleW * (0.5377 + 0.4623 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.2754);
    ...
    vec3 softHot = 1.0 - exp(-2.9765 * hotStop * (pattern + 0.35 * rim));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 1.1753 + baseUV.y * 0.7018 + 0.7027) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1822);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.7563 + baseUV.y * 0.31 + 0.0003) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2656);
    ...
    rgb *= (1.0 - 0.2873 * washHi) * (1.0 - 0.3958 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(0.6457, 0.5981, -0.4747);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.4308) / 1.4308, 0.0, 1.0);
    rgb *= 0.6308 + 0.6595 * keyDiff * (1.0 - 0.5263 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 26.1698);
    rgb += hotStop * (keySpec * 0.6869 * (1.0 - 0.6751 * washHi));
    ...
    float refrAmp = min((1.7154 + 1.9792 * fresnel) / max(length(worldPos), 1.0), 0.1542);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 2.8063) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.1308, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.8692, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.5228 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1399);
    float energyW = clamp(0.2051 + 0.4645 * clamp(pattern, 0.0, 1.0) + 0.2097 * fresnel, 0.0, 0.8306);
    ...
    rgb += fresnel * hotStop * (1.0337 * (1.0 - 0.7128 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.1788) * 1.3087;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3274 * max(0.0, 1.0 - 0.5323 * baseLuma - 0.5478 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3381 + 0.7256 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0820 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
    rgb += emit * 0.8399 * (1.0 - 0.6070 * baseLuma) * (1.0 - 0.5187 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    vec3 v5Over = max(rgb - vec3(0.6412), vec3(0.0));
    rgb = min(rgb, vec3(0.6412)) + 0.3588 * (1.0 - exp(-v5Over * 2.5896));
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2035 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.7307));
    ...
    float bodyAlpha = 0.4559 + 0.5279 * presence + 0.3738 * pattern + 0.1113 * (1.0 - deepTrans) + 0.1655 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.2081;
    // [layer:v5:backface]
    ...
    rgb = mix(rgb, deepStop, v5Back * 0.3884);
    bodyAlpha *= 1.0 + v5Back * 0.2149;
    // [layer:v5:ghostalpha]
    ...
    bodyAlpha *= 0.7221 + 0.2779 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.1958 + hgtC * 0.2728, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.1823);
    bodyAlpha *= 1.0 + innerFace * 0.1478;
    ...
    bodyAlpha *= smoothstep(0.0, 0.5892, sceneVDist - fragVDist);
