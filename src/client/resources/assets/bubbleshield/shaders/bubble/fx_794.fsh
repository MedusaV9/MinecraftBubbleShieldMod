...
// v8 realism: refr 1.5339+1.7698*fres/dist (cap 0.1542) | energy 0.2613+0.4645*p | flow 0.097500 c/s | soft 0.5892m
...
void main() {
    ...
    rim = clamp(rim + 0.6008 * rimLine + 0.5977 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2085 * ringPulse(baseUV + vec2(0.13, 0.31), time);
    float grain = 0.0435 * (cellHash(floor(wuv * 57.0000) + vec2(floor(time * 6.0), 0.0), 342.0000) - 0.5);
    ...
    float motifEnv = 0.15 + 0.85 * smoothstep(0.30, 0.72, (0.5 + 0.5 * sin(time * 0.712094)) * (0.5 + 0.5 * sin(time * 0.623083 + 1.7)));
    vec2 motifC = vec2(sin((baseUV.x - 0.5) * 3.1415927) * 0.6366, baseUV.y - 0.5);
    float motifR = length(motifC);
    float motifM = exp(-motifR * motifR * 17.2693) * (0.75 + 0.25 * sin(time * 0.644026 + 1.5082));
    vec2 motifC2 = vec2(sin(baseUV.x * 3.1415927) * 0.6366, baseUV.y - 0.5);
    motifM += 0.8 * exp(-dot(motifC2, motifC2) * 17.2693);
    ...
    float refrAmp = min((1.5339 + 1.7698 * fresnel) / max(length(worldPos), 1.0), 0.1542);
    ...
    float energyW = clamp(0.2613 + 0.4645 * clamp(pattern, 0.0, 1.0) + 0.1255 * fresnel, 0.0, 0.8306);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.6447;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.1788) * 0.9133;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3274 * (1.0 - 0.5323 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.3381 + 0.7256 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0820 * (0.5 + 0.5 * sin(time * 0.104720))) * atlasPoleW;
    rgb += emit * 0.8399 * (1.0 - 0.6070 * baseLuma) * emitCol;
    // [layer:v5:softknee]
    ...
    bodyAlpha = max(bodyAlpha, 0.9196);
