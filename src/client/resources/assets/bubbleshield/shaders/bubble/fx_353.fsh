...
void main() {
    ...
    mid = clamp(mid * (0.6283 + 0.5847 * texDetail) + 0.2043 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4141), 0.0, 1.0);
    float emit = atlas.a * (0.7510 + 0.3172 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1215 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.9909 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3668 + 0.3493 * presence + 0.4642 * pattern + 0.1391 * (1.0 - deepTrans) + 0.1935 * emit, 0.9101);
