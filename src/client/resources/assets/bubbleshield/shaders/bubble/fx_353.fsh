...
// Bubble surface shader fx_353 -- family LAVAFLOW
// stack: deep=fbm x4 taps | mid=lavaflow+none+pulse | rim=lat | flourish=shimmer
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1887) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1887);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7503, 0.0, 1.0);
    ...
    deepCol += 0.4304 * dp * mix(baseCol, deepStop, 0.2943);
    deepPat += 0.4304 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.9209) + par * 0.0911, time);
    deepCol += 0.1852 * dp * mix(baseCol, deepStop, 0.5886);
    deepPat += 0.1852 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 5.9338) + par * 0.1367, time);
    deepCol += 0.0797 * dp * mix(baseCol, deepStop, 0.8830);
    deepPat += 0.0797 * dp;
    deepCol *= 0.7624;
    deepPat = pow(clamp(deepPat * 0.5899, 0.0, 1.0), 1.6198);

    // [layer:mid:lavaflow_none_pulse]
    ...
    vec2 auv = vec2(baseUV.x * 5.0000 + time * -0.058333, (baseUV.y - 0.5) * 5.0000 * breathe + 2.5000 + time * 0.093333);
    vec2 wuv = auv;
    vec2 lavaUV = wuv + vec2(0.0, time * 0.151667);
    float molten = fbm2(lavaUV, midPer);
    float crust = smoothstep(0.3609, 0.6799, fbm2(lavaUV * 2.0 + vec2(9.1, 3.7), midPer * 2.0));
    float vein = pow(clamp(1.0 - abs(2.0 * fbm2(lavaUV + vec2(4.3, 7.9), midPer) - 1.0), 0.0, 1.0), 9.5497);
    float throb = 0.8 + 0.2 * sin(time * 0.785398 + molten * 2.9128);
    float mid = clamp(molten * (1.0 - crust * 0.6288) * throb + vein * 0.7706 * (1.0 - crust * 0.5), 0.0, 1.3);
    ...
    float flourish = 0.1715 * caustic(wuv + vec2(3.1, 8.7), vec2(0.145833, -0.122500) * time, midPer);
    float grain = 0.0718 * (cellHash(floor(wuv * 64.0000) + vec2(floor(time * 6.0), 0.0), 320.0000) - 0.5);
    ...
    float pattern = clamp(0.3060 * deepPat + 0.9307 * mid + 0.6531 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.2962 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0685 + 0.3041 * presence + 0.3970 * pattern, 0.8501);
