...
void main() {
    ...
    mid = clamp(mid * (0.6869 + 0.5068 * texDetail) + 0.3497 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3967), 0.0, 1.0);
    float emit = atlas.a * (0.6071 + 0.4784 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1300 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.8357 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3541 + 0.4148 * presence + 0.4198 * pattern + 0.0843 * (1.0 - deepTrans) + 0.1328 * emit, 0.9112);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2589), 0.9112);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7037 + 0.2963 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0671), 0.0, 1.0);
