...
void main() {
    ...
    mid = clamp(mid * (0.7384 + 0.6384 * texDetail) + 0.2760 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4305), 0.0, 1.0);
    float emit = atlas.a * (0.7689 + 0.4611 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1676 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.8173 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3349 + 0.2621 * presence + 0.4813 * pattern + 0.1460 * (1.0 - deepTrans) + 0.1721 * emit, 0.8774);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1852), 0.8774);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0606), 0.0, 1.0);
