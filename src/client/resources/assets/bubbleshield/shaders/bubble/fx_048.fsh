...
// stack: deep=fbm x3 taps | mid=starfield+warp2+rotate | rim=graze_sparkle | flourish=glint | tex=starfield
// fbm: standard x3 octaves | seed 3fbbad7aa2a508c7
// v8 realism: refr 0.2651+0.3164*fres/dist (cap 0.1131) | energy 0.5517+0.3822*p | flow 0.090000 c/s | soft 0.8601m
...
void main() {
    ...
    float refrAmp = min((0.2651 + 0.3164 * fresnel) / max(length(worldPos), 1.0), 0.1131);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0308, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9692, vec2(0.001), vec2(0.999))).b);
