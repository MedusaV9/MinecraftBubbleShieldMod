...
void main() {
    ...
    mid = clamp(mid * (0.7365 + 0.5488 * texDetail) + 0.2100 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5055), 0.0, 1.0);
    float emit = atlas.a * (0.7211 + 0.4324 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1597 * (0.5 + 0.5 * sin(time * 0.109956)));
    ...
    float alpha = vertexColor.a * min(0.3975 + 0.5048 * presence + 0.3795 * pattern + 0.1473 * (1.0 - deepTrans) + 0.1566 * emit, 0.9409);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3495), 0.9409);
    // [layer:v5:ghostalpha]
