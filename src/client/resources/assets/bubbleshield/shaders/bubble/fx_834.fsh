...
// stack: deep=fbm x4 taps | mid=deepice+warp2+flicker | rim=lat | flourish=shimmer
...
void main() {
    ...
    rgb = mix(rgb, mix(secCol, deepStop, 0.5), clamp(crackB * (1.0 - crackA), 0.0, 1.0) * 0.4615);
    ...
    float alpha = vertexColor.a * min(0.0581 + 0.3648 * presence + 0.3795 * pattern + 0.1473 * (1.0 - deepTrans), 0.8809);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3495), 0.8809);
    // [layer:v5:ghostalpha]
