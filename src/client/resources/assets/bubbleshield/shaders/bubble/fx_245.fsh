...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8155, 0.0, 1.0);
    ...
    deepCol += 0.4342 * dp * mix(baseCol, deepStop, 0.2127);
    deepPat += 0.4342 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 3.9763) + par * 0.1233, time);
    deepCol += 0.1885 * dp * mix(baseCol, deepStop, 0.4254);
    deepPat += 0.1885 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 4.9822) + par * 0.1850, time);
    deepCol += 0.0819 * dp * mix(baseCol, deepStop, 0.6381);
    deepPat += 0.0819 * dp;
    deepCol *= 0.6526;
    deepPat = pow(clamp(deepPat * 0.5866, 0.0, 1.0), 1.2601);
    ...
    rgb += hotStop * 0.1923 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0410 + 0.2884 * presence + 0.4066 * pattern, 0.8480);
