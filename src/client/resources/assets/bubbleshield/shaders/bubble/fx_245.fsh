...
void main() {
    ...
    mid = clamp(mid * (0.6424 + 0.6828 * texDetail) + 0.3197 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5136), 0.0, 1.0);
    float emit = atlas.a * (0.6707 + 0.4702 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0949 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6842 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3310 + 0.4284 * presence + 0.4066 * pattern + 0.1261 * (1.0 - deepTrans) + 0.1528 * emit, 0.9080);
