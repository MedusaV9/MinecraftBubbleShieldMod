...
void main() {
    ...
    mid = clamp(mid * (0.6498 + 0.5999 * texDetail) + 0.2416 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4957), 0.0, 1.0);
    float emit = atlas.a * (0.7392 + 0.3311 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1664 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.8672 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3659 + 0.4474 * presence + 0.4534 * pattern + 0.1483 * (1.0 - deepTrans) + 0.2197 * emit, 0.8590);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1766), 0.8590);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6351 + 0.3649 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1127), 0.0, 1.0);
