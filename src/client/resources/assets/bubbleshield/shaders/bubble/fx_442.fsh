...
// stack: deep=ridge x3 taps | mid=prismdisperse+curl+flicker | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    float flourish = 0.2879 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0544 + 0.3074 * presence + 0.4534 * pattern + 0.1483 * (1.0 - deepTrans), 0.7990);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1766), 0.7990);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.3961 + 0.6039 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
