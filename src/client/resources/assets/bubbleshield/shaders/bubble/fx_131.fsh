...
void main() {
    ...
    mid = clamp(mid * (0.7038 + 0.4607 * texDetail) + 0.2270 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4208), 0.0, 1.0);
    float emit = atlas.a * (0.5814 + 0.3914 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1542 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.9657 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4235 + 0.4044 * presence + 0.3918 * pattern + 0.1078 * (1.0 - deepTrans) + 0.1476 * emit, 0.8616);
