...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0026) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0026);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.030000, 0.017500) * time, time);
    deep += 0.5196 * deepField(duvBase + rimDir * 0.0601 + vec2(0.046667, 0.027500) * time, time);
    deep = pow(clamp(deep * 0.6581, 0.0, 1.0), 1.0383);
    ...
    float n = fbm2(wuv + vec2(0.0, time * 0.395000), midPer);
