...
void main() {
    ...
    mid = clamp(mid * (0.6886 + 0.4969 * texDetail) + 0.1937 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5059), 0.0, 1.0);
    float emit = atlas.a * (0.7692 + 0.3252 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1176 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.9666 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2799 + 0.2568 * presence + 0.5858 * pattern + 0.1221 * (1.0 - deepTrans) + 0.1702 * emit, 0.8750);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3042), 0.8750);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6275 + 0.3725 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1234), 0.0, 1.0);
