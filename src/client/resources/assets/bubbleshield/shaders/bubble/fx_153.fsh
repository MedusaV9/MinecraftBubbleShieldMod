...
void main() {
    ...
    mid = clamp(mid * (0.7129 + 0.6046 * texDetail) + 0.1853 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7937 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5082), 0.0, 1.0);
    float emit = atlas.a * (0.6256 + 0.3038 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0804 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.8792 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3944 + 0.3845 * presence + 0.3508 * pattern + 0.0827 * (1.0 - deepTrans) + 0.1607 * emit, 0.8874);
