...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5739, 0.0, 1.0);
    ...
    deepCol += 0.4389 * dp * mix(baseCol, deepStop, 0.2691);
    deepPat += 0.4389 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.5815) + par * 0.1255, time);
    deepCol += 0.1927 * dp * mix(baseCol, deepStop, 0.5382);
    deepPat += 0.1927 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.9997) + par * 0.1882, time);
    deepCol += 0.0846 * dp * mix(baseCol, deepStop, 0.8073);
    deepPat += 0.0846 * dp;
    deepCol *= 0.6473;
    deepPat = pow(clamp(deepPat * 0.5827, 0.0, 1.0), 1.0665);
    ...
    rgb += hotStop * 0.2097 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0624 + 0.2445 * presence + 0.3508 * pattern, 0.8274);
