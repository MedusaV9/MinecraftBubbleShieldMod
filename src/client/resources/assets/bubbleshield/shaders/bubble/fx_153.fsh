...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1000) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1000);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.033333, -0.032500) * time, time);
    deep += 0.4389 * deepField(duvBase + rimDir * 0.0627 + vec2(0.051667, -0.050000) * time, time);
    deep += 0.1927 * deepField(duvBase + rimDir * 0.1255 + vec2(0.070000, -0.067500) * time, time);
    deep += 0.0846 * deepField(duvBase + rimDir * 0.1882 + vec2(0.088333, -0.085000) * time, time);
    deep = pow(clamp(deep * 0.5827, 0.0, 1.0), 1.0665);
