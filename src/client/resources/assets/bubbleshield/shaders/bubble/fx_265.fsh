...
void main() {
    ...
    mid = clamp(mid * (0.6353 + 0.4751 * texDetail) + 0.3028 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4725), 0.0, 1.0);
    float emit = atlas.a * (0.5539 + 0.3049 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0971 * (0.5 + 0.5 * sin(time * 0.120428)));
    rgb += emit * 0.9875 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3673 + 0.5032 * presence + 0.4927 * pattern + 0.0914 * (1.0 - deepTrans) + 0.1326 * emit, 0.8847);
