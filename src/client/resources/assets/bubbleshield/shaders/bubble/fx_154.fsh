...
void main() {
    ...
    mid = clamp(mid * (0.6518 + 0.5019 * texDetail) + 0.2452 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4785), 0.0, 1.0);
    float emit = atlas.a * (0.7765 + 0.4494 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0854 * (0.5 + 0.5 * sin(time * 0.204204)));
    rgb += emit * 0.7867 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3348 + 0.4726 * presence + 0.4326 * pattern + 0.1399 * (1.0 - deepTrans) + 0.1633 * emit, 0.9141);
