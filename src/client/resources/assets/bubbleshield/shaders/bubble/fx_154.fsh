...
void main() {
    ...
    deep += 0.4562 * deepField(duvBase + rimDir * 0.0535 + vec2(0.038333, 0.055000) * time, time);
    deep += 0.2081 * deepField(duvBase + rimDir * 0.1070 + vec2(0.051667, 0.075000) * time, time);
    deep = pow(clamp(deep * 0.6008, 0.0, 1.0), 1.5920);
    ...
    float pattern = clamp(0.3996 * deep + 0.9748 * mid + 0.5165 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.6645 + pattern * 0.6298);
    vec3 rgb = vertexColor.rgb * (0.5487 + 0.7574 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2449 + 0.7941 * pattern, 0.0, 1.0);
