...
void main() {
    ...
    mid = clamp(mid * (0.6553 + 0.4772 * texDetail) + 0.1944 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4968), 0.0, 1.0);
    float emit = atlas.a * (0.7855 + 0.2970 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1544 * (0.5 + 0.5 * sin(time * 0.130900)));
    ...
    float alpha = vertexColor.a * min(0.3594 + 0.4368 * presence + 0.4195 * pattern + 0.1137 * (1.0 - deepTrans) + 0.1279 * emit, 0.8902);
