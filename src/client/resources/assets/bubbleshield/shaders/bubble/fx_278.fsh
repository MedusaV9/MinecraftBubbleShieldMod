...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6731, 0.0, 1.0);
    ...
    deepCol += 0.5361 * dp * mix(baseCol, deepStop, 0.2851);
    deepPat += 0.5361 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.3201) + par * 0.0714, time);
    deepCol += 0.2874 * dp * mix(baseCol, deepStop, 0.5702);
    deepPat += 0.2874 * dp;
    deepCol *= 0.6808;
    deepPat = pow(clamp(deepPat * 0.5484, 0.0, 1.0), 1.1827);
    ...
    rgb += hotStop * 0.2342 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0506 + 0.2968 * presence + 0.4195 * pattern, 0.8302);
