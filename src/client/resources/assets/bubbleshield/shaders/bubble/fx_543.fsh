...
void main() {
    ...
    mid = clamp(mid * (0.7268 + 0.7113 * texDetail) + 0.1934 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6459 + 0.3945 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1582 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.6605 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2893 + 0.3091 * presence + 0.5476 * pattern + 0.1485 * (1.0 - deepTrans) + 0.1858 * emit, 0.8673);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1904), 0.8673);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6810 + 0.3190 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
