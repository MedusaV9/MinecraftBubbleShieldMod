...
void main() {
    ...
    mid = clamp(mid * (0.6147 + 0.5233 * texDetail) + 0.2709 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4392), 0.0, 1.0);
    float emit = atlas.a * (0.6463 + 0.4935 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1212 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.7519 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3442 + 0.3898 * presence + 0.4757 * pattern + 0.1462 * (1.0 - deepTrans) + 0.1545 * emit, 0.8725);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1930), 0.8725);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6719 + 0.3281 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0681), 0.0, 1.0);
