...
void main() {
    ...
    mid = clamp(mid * (0.7067 + 0.6420 * texDetail) + 0.3452 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4322), 0.0, 1.0);
    float emit = atlas.a * (0.6297 + 0.2886 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0942 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6949 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3591 + 0.4320 * presence + 0.4203 * pattern + 0.0836 * (1.0 - deepTrans) + 0.1816 * emit, 0.9177);
