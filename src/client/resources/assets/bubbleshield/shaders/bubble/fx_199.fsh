...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6546, 0.0, 1.0);
    ...
    deepCol += 0.5743 * dp * mix(baseCol, deepStop, 0.3648);
    deepPat += 0.5743 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 3.0602) + par * 0.1192, time);
    deepCol += 0.3298 * dp * mix(baseCol, deepStop, 0.7295);
    deepPat += 0.3298 * dp;
    deepCol *= 0.5138;
    deepPat = pow(clamp(deepPat * 0.5252, 0.0, 1.0), 1.7439);
    ...
    rgb = satLift(rgb, 1.3439);
    rgb += hotStop * 0.2584 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0737 + 0.2783 * presence + 0.4788 * pattern, 0.8453);
