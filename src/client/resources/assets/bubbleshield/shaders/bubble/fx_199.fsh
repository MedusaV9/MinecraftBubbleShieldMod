...
void main() {
    ...
    mid = clamp(mid * (0.6796 + 0.4501 * texDetail) + 0.2601 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5254), 0.0, 1.0);
    float emit = atlas.a * (0.6791 + 0.3151 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0989 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.9453 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4436 + 0.4695 * presence + 0.4435 * pattern + 0.1597 * (1.0 - deepTrans) + 0.1686 * emit, 0.9453);
