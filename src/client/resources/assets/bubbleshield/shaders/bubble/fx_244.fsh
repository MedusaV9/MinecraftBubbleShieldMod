...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5812, 0.0, 1.0);
    ...
    deepCol += 0.4986 * dp * mix(baseCol, deepStop, 0.1926);
    deepPat += 0.4986 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 4.0473) + par * 0.0704, time);
    deepCol += 0.2486 * dp * mix(baseCol, deepStop, 0.3852);
    deepPat += 0.2486 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 5.1150) + par * 0.1055, time);
    deepCol += 0.1239 * dp * mix(baseCol, deepStop, 0.5778);
    deepPat += 0.1239 * dp;
    deepCol *= 0.6169;
    deepPat = pow(clamp(deepPat * 0.5345, 0.0, 1.0), 1.4555);
    ...
    rgb += hotStop * 0.3053 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0587 + 0.3557 * presence + 0.4237 * pattern, 0.8476);
