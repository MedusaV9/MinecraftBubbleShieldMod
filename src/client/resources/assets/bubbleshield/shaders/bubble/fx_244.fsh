...
void main() {
    ...
    mid = clamp(mid * (0.6016 + 0.4943 * texDetail) + 0.3122 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5206), 0.0, 1.0);
    float emit = atlas.a * (0.6643 + 0.2942 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1117 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.8659 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3835 + 0.4957 * presence + 0.4237 * pattern + 0.1257 * (1.0 - deepTrans) + 0.1746 * emit, 0.9076);
