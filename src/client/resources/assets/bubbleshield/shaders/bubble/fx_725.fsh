...
void main() {
    ...
    mid = clamp(mid * (0.7184 + 0.5777 * texDetail) + 0.2856 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3606), 0.0, 1.0);
    float emit = atlas.a * (0.7370 + 0.2791 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1552 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.6978 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4104 + 0.4065 * presence + 0.4533 * pattern + 0.1190 * (1.0 - deepTrans) + 0.1360 * emit, 0.8413);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3436), 0.8413);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7692 + 0.2308 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0785), 0.0, 1.0);
