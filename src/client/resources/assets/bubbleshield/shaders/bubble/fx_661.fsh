...
void main() {
    ...
    mid = clamp(mid * (0.6147 + 0.5950 * texDetail) + 0.2487 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4523), 0.0, 1.0);
    float emit = atlas.a * (0.6461 + 0.3455 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1138 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.8015 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4225 + 0.4337 * presence + 0.3327 * pattern + 0.1599 * (1.0 - deepTrans) + 0.1584 * emit, 0.9340);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2201), 0.9340);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7390 + 0.2610 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1103), 0.0, 1.0);
