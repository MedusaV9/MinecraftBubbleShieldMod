...
// stack: deep=ridge x4 taps | mid=raymarchfog+curl+flicker | rim=graze | flourish=glint
...
void main() {
    ...
    float mid = clamp(rfLight * 1.2341 + (1.0 - rfTrans) * 0.1684, 0.0, 1.15);
    ...
    float alpha = vertexColor.a * min(0.0794 + 0.3737 * presence + 0.3327 * pattern + 0.1599 * (1.0 - deepTrans), 0.8740);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2201), 0.8740);
    // [layer:v5:ghostalpha]
