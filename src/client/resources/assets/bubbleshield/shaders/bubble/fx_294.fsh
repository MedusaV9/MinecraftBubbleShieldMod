...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8192, 0.0, 1.0);
    ...
    deepCol += 0.4000 * dp * mix(baseCol, deepStop, 0.2006);
    deepPat += 0.4000 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.8566) + par * 0.1463, time);
    deepCol += 0.1600 * dp * mix(baseCol, deepStop, 0.4012);
    deepPat += 0.1600 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 7.4455) + par * 0.2195, time);
    deepCol += 0.0640 * dp * mix(baseCol, deepStop, 0.6018);
    deepPat += 0.0640 * dp;
    deepCol *= 0.6630;
    deepPat = pow(clamp(deepPat * 0.6157, 0.0, 1.0), 1.2902);
    ...
    rgb += hotStop * 0.2570 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0732 + 0.3153 * presence + 0.4164 * pattern, 0.8551);
