...
void main() {
    ...
    mid = clamp(mid * (0.7060 + 0.4627 * texDetail) + 0.2744 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5347), 0.0, 1.0);
    float emit = atlas.a * (0.6978 + 0.4440 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1652 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.9558 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4264 + 0.4553 * presence + 0.4164 * pattern + 0.1358 * (1.0 - deepTrans) + 0.1920 * emit, 0.9151);
