...
void main() {
    ...
    mid = clamp(mid * (0.7160 + 0.5424 * texDetail) + 0.2271 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4027), 0.0, 1.0);
    float emit = atlas.a * (0.7563 + 0.3851 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1425 * (0.5 + 0.5 * sin(time * 0.099484)));
    rgb += emit * 0.8498 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3204 + 0.2870 * presence + 0.5931 * pattern + 0.0872 * (1.0 - deepTrans) + 0.1998 * emit, 0.8831);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2528), 0.8831);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7017 + 0.2983 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1272), 0.0, 1.0);
