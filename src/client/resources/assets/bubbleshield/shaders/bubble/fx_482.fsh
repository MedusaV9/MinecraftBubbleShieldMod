...
void main() {
    ...
    mid = clamp(mid * (0.6069 + 0.4825 * texDetail) + 0.3277 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3881), 0.0, 1.0);
    float emit = atlas.a * (0.7332 + 0.2770 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1634 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.7169 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4083 + 0.4152 * presence + 0.3653 * pattern + 0.1511 * (1.0 - deepTrans) + 0.1479 * emit, 0.8978);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3151), 0.8978);
    // [layer:v5:ghostalpha]
