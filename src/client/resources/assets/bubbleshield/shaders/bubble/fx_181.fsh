...
void main() {
    ...
    mid = clamp(mid * (0.6067 + 0.7188 * texDetail) + 0.1952 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4086), 0.0, 1.0);
    float emit = atlas.a * (0.7996 + 0.2980 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0865 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.8850 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4048 + 0.4610 * presence + 0.3507 * pattern + 0.1014 * (1.0 - deepTrans) + 0.1717 * emit, 0.8785);
