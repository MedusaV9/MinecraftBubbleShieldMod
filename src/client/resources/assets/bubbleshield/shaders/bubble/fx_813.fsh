...
void main() {
    ...
    mid = clamp(mid * (0.6072 + 0.4695 * texDetail) + 0.2521 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4227), 0.0, 1.0);
    float emit = atlas.a * (0.5556 + 0.3684 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1742 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.8941 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3278 + 0.3095 * presence + 0.5536 * pattern + 0.1561 * (1.0 - deepTrans) + 0.2085 * emit, 0.8648);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3193), 0.8648);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7392 + 0.2608 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
