...
void main() {
    ...
    mid = clamp(mid * (0.6273 + 0.7446 * texDetail) + 0.2027 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4338), 0.0, 1.0);
    float emit = atlas.a * (0.7877 + 0.4667 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1642 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.9467 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3076 + 0.2446 * presence + 0.4927 * pattern + 0.1171 * (1.0 - deepTrans) + 0.1347 * emit, 0.8115);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1601), 0.8115);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6816 + 0.3184 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1085), 0.0, 1.0);
