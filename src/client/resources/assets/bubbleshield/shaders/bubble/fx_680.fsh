...
void main() {
    ...
    mid = clamp(mid * (0.7429 + 0.7425 * texDetail) + 0.3390 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3551), 0.0, 1.0);
    float emit = atlas.a * (0.7222 + 0.3399 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1092 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.8019 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2696 + 0.2974 * presence + 0.4965 * pattern + 0.1576 * (1.0 - deepTrans) + 0.1768 * emit, 0.8261);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2853), 0.8261);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6544 + 0.3456 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1291), 0.0, 1.0);
