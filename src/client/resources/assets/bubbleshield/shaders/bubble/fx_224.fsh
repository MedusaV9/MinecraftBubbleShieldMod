...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.045000, -0.012500) * time, time);
    deep += 0.5137 * deepField(duvBase + rimDir * 0.0361 + vec2(-0.070000, -0.020000) * time, time);
    deep += 0.2639 * deepField(duvBase + rimDir * 0.0722 + vec2(-0.095000, -0.027500) * time, time);
    deep = pow(clamp(deep * 0.5625, 0.0, 1.0), 1.5650);
    ...
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.130000, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.100000), midPer)), midPer);
