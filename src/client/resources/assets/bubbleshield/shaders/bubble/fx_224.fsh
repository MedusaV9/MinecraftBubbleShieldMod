...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7212, 0.0, 1.0);
    ...
    deepCol += 0.5137 * dp * mix(baseCol, deepStop, 0.2545);
    deepPat += 0.5137 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 4.1093) + par * 0.0722, time);
    deepCol += 0.2639 * dp * mix(baseCol, deepStop, 0.5091);
    deepPat += 0.2639 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.1497) + par * 0.1082, time);
    deepCol += 0.1356 * dp * mix(baseCol, deepStop, 0.7636);
    deepPat += 0.1356 * dp;
    deepCol *= 0.6794;
    deepPat = pow(clamp(deepPat * 0.5227, 0.0, 1.0), 1.5650);
    ...
    rgb += hotStop * 0.2715 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0716 + 0.2878 * presence + 0.4527 * pattern, 0.8339);
