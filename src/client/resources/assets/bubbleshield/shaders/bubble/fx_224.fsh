...
void main() {
    ...
    mid = clamp(mid * (0.6384 + 0.4677 * texDetail) + 0.3017 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4486), 0.0, 1.0);
    float emit = atlas.a * (0.5702 + 0.4370 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1532 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.9581 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4215 + 0.4278 * presence + 0.4527 * pattern + 0.0863 * (1.0 - deepTrans) + 0.1629 * emit, 0.8939);
