...
void main() {
    ...
    mid = clamp(mid * (0.6673 + 0.6865 * texDetail) + 0.3380 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4610), 0.0, 1.0);
    float emit = atlas.a * (0.6677 + 0.3611 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1182 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9345 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3317 + 0.3136 * presence + 0.4818 * pattern + 0.0933 * (1.0 - deepTrans) + 0.1299 * emit, 0.9197);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1764), 0.9197);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7171 + 0.2829 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
