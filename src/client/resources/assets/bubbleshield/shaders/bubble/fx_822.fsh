...
void main() {
    ...
    mid = clamp(mid * (0.6371 + 0.5239 * texDetail) + 0.1808 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4266), 0.0, 1.0);
    float emit = atlas.a * (0.7279 + 0.4312 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0912 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.9628 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3630 + 0.3861 * presence + 0.4198 * pattern + 0.1196 * (1.0 - deepTrans) + 0.2046 * emit, 0.9092);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2056), 0.9092);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7027 + 0.2973 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
