...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8141, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0458, abs(glR - 0.1060));
    float mid = clamp(glStars * (1.0 + glRing * 1.3501) + glRing * 0.4942, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6209 + 0.4577 * texDetail) + 0.2686 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.7976 + 0.4948 * texDetail;
    rgb = satLift(rgb, 1.1615);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5199), 0.0, 1.0);
    float emit = atlas.a * (0.7248 + 0.4611 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1488 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.7841 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3382 + 0.2345 * presence + 0.5475 * pattern + 0.1359 * (1.0 - deepTrans) + 0.1337 * emit, 0.8560);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2921), 0.8560);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6322 + 0.3678 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1151), 0.0, 1.0);
