...
void main() {
    ...
    deepCol += 0.3914 * dp * mix(baseCol, deepStop, 0.3463);
    deepPat += 0.3914 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 3.0949) + par * 0.0686, time);
    deepCol += 0.1532 * dp * mix(baseCol, deepStop, 0.6926);
    deepPat += 0.1532 * dp;
    deepCol *= 0.7192;
    deepPat = pow(clamp(deepPat * 0.6474, 0.0, 1.0), 1.4375);
    ...
    for (int i = 0; i < 4; i++) {
        float fi = float(i);
        float fa = fi * 1.5707963 + 0.7677 * sin(time * 0.146608 + fi * 1.9);
        ...
        float fil = pow(clamp(1.0 - dAng * 3.3192, 0.0, 1.0), 10.3200);
        net += fil * smoothstep(0.06, 0.25, trad) * (0.55 + 0.45 * sin(time * 2.146755 + fi * 2.3));
    }
    ...
    float pattern = clamp(0.3305 * deepPat + 0.8554 * mid + 0.6331 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.3055 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0562 + 0.2475 * presence + 0.4653 * pattern, 0.8182);
