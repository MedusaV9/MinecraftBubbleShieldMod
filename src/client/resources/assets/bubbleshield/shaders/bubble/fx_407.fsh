...
void main() {
    ...
    mid = clamp(mid * (0.7456 + 0.6622 * texDetail) + 0.3411 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4216), 0.0, 1.0);
    float emit = atlas.a * (0.6748 + 0.3687 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1403 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.7660 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3128 + 0.2646 * presence + 0.5892 * pattern + 0.0848 * (1.0 - deepTrans) + 0.1748 * emit, 0.8782);
