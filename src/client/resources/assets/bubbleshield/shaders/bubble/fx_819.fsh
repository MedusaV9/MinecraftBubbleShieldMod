...
void main() {
    ...
    mid = clamp(mid * (0.6957 + 0.5481 * texDetail) + 0.2382 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3876), 0.0, 1.0);
    float emit = atlas.a * (0.5712 + 0.4970 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1167 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.9383 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3808 + 0.3672 * presence + 0.5295 * pattern + 0.1409 * (1.0 - deepTrans) + 0.1813 * emit, 0.9176);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3391), 0.9176);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7446 + 0.2554 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
