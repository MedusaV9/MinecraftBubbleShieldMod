...
void main() {
    ...
    mid = clamp(mid * (0.7454 + 0.6006 * texDetail) + 0.2917 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4947), 0.0, 1.0);
    float emit = atlas.a * (0.7594 + 0.2828 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0956 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.7185 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3277 + 0.2747 * presence + 0.4619 * pattern + 0.1594 * (1.0 - deepTrans) + 0.1998 * emit, 0.7936);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2905), 0.7936);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7654 + 0.2346 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0981), 0.0, 1.0);
