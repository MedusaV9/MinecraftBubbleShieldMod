...
void main() {
    ...
    mid = clamp(mid * (0.7433 + 0.7280 * texDetail) + 0.1938 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4331), 0.0, 1.0);
    float emit = atlas.a * (0.5894 + 0.4147 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1752 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.8436 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3295 + 0.3017 * presence + 0.5673 * pattern + 0.1093 * (1.0 - deepTrans) + 0.1986 * emit, 0.8950);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2125), 0.8950);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7220 + 0.2780 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0662), 0.0, 1.0);
