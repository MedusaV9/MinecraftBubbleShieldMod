...
// stack: deep=voro x4 taps | mid=deepice+warp1+pulse | rim=graze_sparkle | flourish=swirl
...
void main() {
    ...
    rgb = mix(rgb, mix(secCol, deepStop, 0.5), clamp(crackB * (1.0 - crackA), 0.0, 1.0) * 0.4397);
    ...
    float alpha = vertexColor.a * min(0.0672 + 0.3283 * presence + 0.3641 * pattern + 0.1084 * (1.0 - deepTrans), 0.8413);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1501), 0.8413);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5100 + 0.4900 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
