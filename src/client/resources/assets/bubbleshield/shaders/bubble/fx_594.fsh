...
void main() {
    ...
    mid = clamp(mid * (0.6291 + 0.5292 * texDetail) + 0.2067 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.7036 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4614), 0.0, 1.0);
    float emit = atlas.a * (0.6291 + 0.4029 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1374 * (0.5 + 0.5 * sin(time * 0.267035)));
    ...
    float alpha = vertexColor.a * min(0.4235 + 0.4683 * presence + 0.3641 * pattern + 0.1084 * (1.0 - deepTrans) + 0.1770 * emit, 0.9013);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1501), 0.9013);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7423 + 0.2577 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0985), 0.0, 1.0);
