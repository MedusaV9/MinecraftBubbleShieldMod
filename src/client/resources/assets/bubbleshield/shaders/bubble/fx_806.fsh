...
void main() {
    ...
    mid = clamp(mid * (0.6588 + 0.6731 * texDetail) + 0.2192 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4699), 0.0, 1.0);
    float emit = atlas.a * (0.7726 + 0.3825 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1743 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6604 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4128 + 0.4065 * presence + 0.4412 * pattern + 0.1252 * (1.0 - deepTrans) + 0.1731 * emit, 0.8970);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2920), 0.8970);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7235 + 0.2765 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1209), 0.0, 1.0);
