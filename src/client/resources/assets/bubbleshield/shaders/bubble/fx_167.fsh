...
void main() {
    ...
    deepCol += 0.5633 * dp * mix(baseCol, deepStop, 0.4306);
    deepPat += 0.5633 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 3.1170) + par * 0.0827, time);
    deepCol += 0.3173 * dp * mix(baseCol, deepStop, 0.8613);
    deepPat += 0.3173 * dp;
    deepCol *= 0.6465;
    deepPat = pow(clamp(deepPat * 0.5318, 0.0, 1.0), 1.2104);
    ...
    rgb += hotStop * 0.2528 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0427 + 0.2670 * presence + 0.4614 * pattern, 0.8551);
