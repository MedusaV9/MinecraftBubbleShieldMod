...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1066) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1066);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.050000, 0.016667) * time, time);
    deep += 0.5633 * deepField(duvBase + rimDir * 0.0414 + vec2(-0.075000, 0.026667) * time, time);
    deep = pow(clamp(deep * 0.6397, 0.0, 1.0), 1.2104);

    // [layer:mid:arcs_none_scroll]
    // Signature structure of this effect, domain-warped and animated.
    vec2 auv = vec2(baseUV.x * 6.0000, baseUV.y * 6.0000) + vec2(-0.315000, 0.313333) * time;
    vec2 wuv = auv;
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.286667), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 7.0273);
    float flash = 0.6 + 0.4 * sin(time * 2.455678 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
