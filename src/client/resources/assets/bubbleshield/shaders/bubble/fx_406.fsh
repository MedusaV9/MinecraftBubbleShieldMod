...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6310, 0.0, 1.0);
    ...
    deepCol += 0.4272 * dp * mix(baseCol, deepStop, 0.2489);
    deepPat += 0.4272 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 5.6991) + par * 0.1402, time);
    deepCol += 0.1825 * dp * mix(baseCol, deepStop, 0.4978);
    deepPat += 0.1825 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 7.2228) + par * 0.2103, time);
    deepCol += 0.0780 * dp * mix(baseCol, deepStop, 0.7467);
    deepPat += 0.0780 * dp;
    deepCol *= 0.7190;
    deepPat = pow(clamp(deepPat * 0.5926, 0.0, 1.0), 1.5652);
    ...
    rgb += hotStop * 0.3306 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0439 + 0.2487 * presence + 0.4772 * pattern, 0.8320);
