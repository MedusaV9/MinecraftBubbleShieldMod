...
void main() {
    ...
    mid = clamp(mid * (0.6652 + 0.5146 * texDetail) + 0.1925 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4382), 0.0, 1.0);
    float emit = atlas.a * (0.5776 + 0.4604 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0943 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.7331 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3554 + 0.4651 * presence + 0.4102 * pattern + 0.1295 * (1.0 - deepTrans) + 0.1515 * emit, 0.8574);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2422), 0.8574);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6587 + 0.3413 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1211), 0.0, 1.0);
