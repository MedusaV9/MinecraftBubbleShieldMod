...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8022, 0.0, 1.0);
    ...
    deepCol += 0.4916 * dp * mix(baseCol, deepStop, 0.2936);
    deepPat += 0.4916 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.1286) + par * 0.0726, time);
    deepCol += 0.2417 * dp * mix(baseCol, deepStop, 0.5871);
    deepPat += 0.2417 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 6.2313) + par * 0.1088, time);
    deepCol += 0.1188 * dp * mix(baseCol, deepStop, 0.8807);
    deepPat += 0.1188 * dp;
    deepCol *= 0.6019;
    deepPat = pow(clamp(deepPat * 0.5399, 0.0, 1.0), 1.5419);
    ...
    rgb += hotStop * 0.1572 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0626 + 0.2865 * presence + 0.3614 * pattern, 0.7832);
