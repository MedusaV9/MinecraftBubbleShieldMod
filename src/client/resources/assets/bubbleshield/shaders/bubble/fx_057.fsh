...
// stack: deep=caustic x2 taps | mid=waves+curl+pulse | rim=graze_film | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.035000, 0.015000) * time, time);
    deep += 0.3904 * deepField(duvBase + rimDir * 0.0455 + vec2(-0.055000, 0.025000) * time, time);
    deep = pow(clamp(deep * 0.7192, 0.0, 1.0), 1.7146);
    ...
    float pattern = clamp(0.3426 * deep + 0.9808 * mid + 0.8438 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5833 + pattern * 0.5740);
    vec3 rgb = vertexColor.rgb * (0.3556 + 0.8462 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1157 + 0.7261 * pattern, 0.0, 1.0);
