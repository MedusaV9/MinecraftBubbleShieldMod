...
void main() {
    ...
    mid = clamp(mid * (0.6209 + 0.5478 * texDetail) + 0.3413 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4017), 0.0, 1.0);
    float emit = atlas.a * (0.5817 + 0.4687 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1784 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6982 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3902 + 0.3242 * presence + 0.3293 * pattern + 0.1049 * (1.0 - deepTrans) + 0.1983 * emit, 0.8425);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1826), 0.8425);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6747 + 0.3253 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1090), 0.0, 1.0);
