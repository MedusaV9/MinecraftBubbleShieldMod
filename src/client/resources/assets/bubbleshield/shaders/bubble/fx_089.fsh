...
// stack: deep=caustic x4 taps | mid=petals+curl+scroll | rim=lat | flourish=shimmer | tex=scales
// fbm: ridged x3 octaves | seed 023c68bc2e7bf82c
// v8 realism: refr 0.4527+0.6235*fres/dist (cap 0.1518) | energy 0.3271+0.4984*p | flow 0.100000 c/s | soft 0.8385m
...
void main() {
    ...
    float refrAmp = min((0.4527 + 0.6235 * fresnel) / max(length(worldPos), 1.0), 0.1518);
