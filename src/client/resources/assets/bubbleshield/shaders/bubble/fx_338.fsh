...
// stack: deep=ridge x2 taps | mid=petals+warp2+rotate | rim=graze_sparkle | flourish=echo
...
void main() {
    ...
    deep += 0.4223 * deepField(duvBase + rimDir * 0.0430 + vec2(0.035000, -0.007500) * time, time);
    deep = pow(clamp(deep * 0.7031, 0.0, 1.0), 1.1155);
    ...
    float pattern = clamp(0.3558 * deep + 0.8066 * mid + 0.6020 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8888 + pattern * 0.4880);
    vec3 rgb = vertexColor.rgb * (0.4020 + 0.7355 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1582 + 0.8257 * pattern, 0.0, 1.0);
