...
void main() {
    ...
    mid = clamp(mid * (0.7411 + 0.4695 * texDetail) + 0.1898 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5110), 0.0, 1.0);
    float emit = atlas.a * (0.7227 + 0.4913 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1698 * (0.5 + 0.5 * sin(time * 0.183260)));
    ...
    float alpha = vertexColor.a * min(0.3506 + 0.3976 * presence + 0.4278 * pattern + 0.1530 * (1.0 - deepTrans) + 0.2022 * emit, 0.8954);
