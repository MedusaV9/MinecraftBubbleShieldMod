...
void main() {
    ...
    mid = clamp(mid * (0.6885 + 0.7252 * texDetail) + 0.2335 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3903), 0.0, 1.0);
    float emit = atlas.a * (0.7483 + 0.3160 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1586 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7850 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4206 + 0.4680 * presence + 0.3813 * pattern + 0.1165 * (1.0 - deepTrans) + 0.1223 * emit, 0.8935);
