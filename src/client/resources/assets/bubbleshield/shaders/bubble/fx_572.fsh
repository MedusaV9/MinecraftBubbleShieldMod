...
void main() {
    ...
    mid = clamp(mid * (0.6956 + 0.7301 * texDetail) + 0.1852 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3915), 0.0, 1.0);
    float emit = atlas.a * (0.5699 + 0.3231 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1629 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.9160 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2910 + 0.3052 * presence + 0.4917 * pattern + 0.0819 * (1.0 - deepTrans) + 0.2046 * emit, 0.8954);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1619), 0.8954);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7646 + 0.2354 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1184), 0.0, 1.0);
