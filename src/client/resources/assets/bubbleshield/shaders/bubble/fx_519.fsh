...
void main() {
    ...
    mid = clamp(mid * (0.6896 + 0.6249 * texDetail) + 0.3167 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4062), 0.0, 1.0);
    float emit = atlas.a * (0.6702 + 0.4419 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1752 * (0.5 + 0.5 * sin(time * 0.172788)));
    rgb += emit * 0.6837 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3677 + 0.3019 * presence + 0.4292 * pattern + 0.1311 * (1.0 - deepTrans) + 0.1948 * emit, 0.8936);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3368), 0.8936);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7089 + 0.2911 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0840), 0.0, 1.0);
