...
void main() {
    ...
    mid = clamp(mid * (0.6882 + 0.5151 * texDetail) + 0.3408 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4881), 0.0, 1.0);
    float emit = atlas.a * (0.7046 + 0.4787 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1222 * (0.5 + 0.5 * sin(time * 0.141372)));
    ...
    float alpha = vertexColor.a * min(0.3527 + 0.4182 * presence + 0.3843 * pattern + 0.1440 * (1.0 - deepTrans) + 0.1418 * emit, 0.8675);
