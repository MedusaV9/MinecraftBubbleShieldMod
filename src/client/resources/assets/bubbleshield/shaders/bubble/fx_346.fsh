...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7407, 0.0, 1.0);
    ...
    deepCol += 0.4817 * dp * mix(baseCol, deepStop, 0.1965);
    deepPat += 0.4817 * dp;
    dp = deepField(rotA(spinAxis, time * 0.094248) * (sdir * 3.1639) + par * 0.0870, time);
    deepCol += 0.2320 * dp * mix(baseCol, deepStop, 0.3929);
    deepPat += 0.2320 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 3.8178) + par * 0.1305, time);
    deepCol += 0.1117 * dp * mix(baseCol, deepStop, 0.5894);
    deepPat += 0.1117 * dp;
    deepCol *= 0.6103;
    deepPat = pow(clamp(deepPat * 0.5478, 0.0, 1.0), 1.4869);
    ...
    rgb += hotStop * 0.1670 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0483 + 0.2782 * presence + 0.3843 * pattern, 0.8075);
