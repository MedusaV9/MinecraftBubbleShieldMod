...
void main() {
    ...
    mid = clamp(mid * (0.6254 + 0.7226 * texDetail) + 0.2234 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5108), 0.0, 1.0);
    float emit = atlas.a * (0.7529 + 0.3454 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1071 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.8438 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3741 + 0.4193 * presence + 0.4580 * pattern + 0.1296 * (1.0 - deepTrans) + 0.1532 * emit, 0.9014);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1777), 0.9014);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6432 + 0.3568 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0934), 0.0, 1.0);
