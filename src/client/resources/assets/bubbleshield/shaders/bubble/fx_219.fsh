...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6294, 0.0, 1.0);
    ...
    deepCol += 0.5430 * dp * mix(baseCol, deepStop, 0.1995);
    deepPat += 0.5430 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 4.5225) + par * 0.1329, time);
    deepCol += 0.2949 * dp * mix(baseCol, deepStop, 0.3991);
    deepPat += 0.2949 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 5.3947) + par * 0.1994, time);
    deepCol += 0.1601 * dp * mix(baseCol, deepStop, 0.5986);
    deepPat += 0.1601 * dp;
    deepCol *= 0.6453;
    deepPat = pow(clamp(deepPat * 0.5005, 0.0, 1.0), 1.0509);
    ...
    rgb += hotStop * 0.3045 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0642 + 0.2764 * presence + 0.3412 * pattern, 0.8071);
