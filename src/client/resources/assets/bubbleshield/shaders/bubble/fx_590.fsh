...
void main() {
    ...
    mid = clamp(mid * (0.7201 + 0.5783 * texDetail) + 0.1851 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4718), 0.0, 1.0);
    float emit = atlas.a * (0.7167 + 0.3094 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1128 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.8605 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2926 + 0.2695 * presence + 0.5763 * pattern + 0.0943 * (1.0 - deepTrans) + 0.2170 * emit, 0.8367);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1607), 0.8367);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1165), 0.0, 1.0);
