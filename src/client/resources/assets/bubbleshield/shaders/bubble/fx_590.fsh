...
// stack: deep=fbm x4 taps | mid=phantomecho+warp2+rotate | rim=graze_sparkle | flourish=glint | tex=foam
// fbm: ridged x4 octaves | seed 8c975828bf0274d9
// v8 realism: refr 0.4809+0.6385*fres/dist (cap 0.1202) | energy 0.3504+0.3993*p | flow 0.098333 c/s | soft 1.2009m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (0.9927 * atlasPoleW));
    ...
    float refrAmp = min((0.4809 + 0.6385 * fresnel) / max(length(worldPos), 1.0), 0.1202);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0561, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9439, vec2(0.001), vec2(0.999))).b);
