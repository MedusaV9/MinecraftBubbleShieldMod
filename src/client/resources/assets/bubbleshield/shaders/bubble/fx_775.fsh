...
void main() {
    ...
    mid = clamp(mid * (0.6102 + 0.4798 * texDetail) + 0.3086 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5407), 0.0, 1.0);
    float emit = atlas.a * (0.7182 + 0.3880 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0824 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.8717 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3512 + 0.2663 * presence + 0.5872 * pattern + 0.1176 * (1.0 - deepTrans) + 0.1416 * emit, 0.8666);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2138), 0.8666);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6305 + 0.3695 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
