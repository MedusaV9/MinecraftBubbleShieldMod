...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6527, 0.0, 1.0);
    ...
    deepCol += 0.5412 * dp * mix(baseCol, deepStop, 0.2252);
    deepPat += 0.5412 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 4.7194) + par * 0.1421, time);
    deepCol += 0.2929 * dp * mix(baseCol, deepStop, 0.4503);
    deepPat += 0.2929 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 5.7796) + par * 0.2131, time);
    deepCol += 0.1585 * dp * mix(baseCol, deepStop, 0.6755);
    deepPat += 0.1585 * dp;
    deepCol *= 0.5229;
    deepPat = pow(clamp(deepPat * 0.5019, 0.0, 1.0), 1.5871);
    ...
    rgb += hotStop * 0.3246 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0656 + 0.2450 * presence + 0.3306 * pattern, 0.7823);
