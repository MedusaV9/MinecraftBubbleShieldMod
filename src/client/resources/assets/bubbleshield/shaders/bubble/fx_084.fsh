...
void main() {
    ...
    mid = clamp(mid * (0.7046 + 0.7049 * texDetail) + 0.2657 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5113), 0.0, 1.0);
    float emit = atlas.a * (0.7113 + 0.4878 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0865 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.8101 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4039 + 0.3850 * presence + 0.3306 * pattern + 0.0814 * (1.0 - deepTrans) + 0.1774 * emit, 0.8423);
