...
void main() {
    ...
    mid = clamp(mid * (0.6221 + 0.6250 * texDetail) + 0.3347 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3835), 0.0, 1.0);
    float emit = atlas.a * (0.7315 + 0.2933 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1431 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.8393 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2803 + 0.2503 * presence + 0.4782 * pattern + 0.0943 * (1.0 - deepTrans) + 0.2109 * emit, 0.9012);
