...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.021667, 0.015000) * time, time);
    deep += 0.4241 * deepField(duvBase + rimDir * 0.0463 + vec2(-0.033333, 0.022500) * time, time);
    deep += 0.1799 * deepField(duvBase + rimDir * 0.0927 + vec2(-0.045000, 0.032500) * time, time);
    deep += 0.0763 * deepField(duvBase + rimDir * 0.1390 + vec2(-0.056667, 0.040000) * time, time);
    deep = pow(clamp(deep * 0.5951, 0.0, 1.0), 1.2181);
    ...
    float pattern = clamp(0.3147 * deep + 0.7484 * mid + 0.6265 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.4139 + pattern * 0.5877);
    vec3 rgb = vertexColor.rgb * (0.4626 + 0.6685 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2245 + 0.7708 * pattern, 0.0, 1.0);
