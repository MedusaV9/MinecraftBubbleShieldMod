...
void main() {
    ...
    mid = clamp(mid * (0.6519 + 0.5813 * texDetail) + 0.2607 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3689), 0.0, 1.0);
    float emit = atlas.a * (0.6839 + 0.4327 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0906 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.6856 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3199 + 0.3071 * presence + 0.5816 * pattern + 0.1327 * (1.0 - deepTrans) + 0.1726 * emit, 0.8926);
