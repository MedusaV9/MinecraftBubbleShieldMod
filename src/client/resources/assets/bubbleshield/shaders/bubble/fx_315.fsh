...
void main() {
    ...
    mid = clamp(mid * (0.7341 + 0.5320 * texDetail) + 0.2904 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3645), 0.0, 1.0);
    float emit = atlas.a * (0.5610 + 0.4415 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0941 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.9878 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4365 + 0.4758 * presence + 0.4926 * pattern + 0.1600 * (1.0 - deepTrans) + 0.1886 * emit, 0.9068);
