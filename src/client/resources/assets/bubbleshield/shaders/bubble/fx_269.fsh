...
void main() {
    ...
    mid = clamp(mid * (0.6249 + 0.7449 * texDetail) + 0.2706 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.5055 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5058), 0.0, 1.0);
    float emit = atlas.a * (0.7748 + 0.4802 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0838 * (0.5 + 0.5 * sin(time * 0.272271)));
    rgb += emit * 0.8070 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3548 + 0.4110 * presence + 0.4828 * pattern + 0.1147 * (1.0 - deepTrans) + 0.1520 * emit, 0.8672);
