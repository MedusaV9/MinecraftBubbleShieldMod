...
// stack: deep=caustic x3 taps | mid=arcs+warp1+flicker | rim=graze_sparkle | flourish=echo
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.027500, 0.006667) * time, time);
    deep += 0.4435 * deepField(duvBase + rimDir * 0.0513 + vec2(-0.045000, 0.013333) * time, time);
    deep += 0.1967 * deepField(duvBase + rimDir * 0.1025 + vec2(-0.060000, 0.016667) * time, time);
    deep = pow(clamp(deep * 0.6097, 0.0, 1.0), 1.2870);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.275000), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 10.4558);
    float flash = 0.6 + 0.4 * sin(time * 2.832669 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.4513 * deep + 0.7238 * mid + 0.6316 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7344 + pattern * 0.5727);
    vec3 rgb = vertexColor.rgb * (0.4856 + 0.5574 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2464 + 0.7045 * pattern, 0.0, 1.0);
