...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8184, 0.0, 1.0);
    ...
    deepCol += 0.4435 * dp * mix(baseCol, deepStop, 0.2880);
    deepPat += 0.4435 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 5.7965) + par * 0.1025, time);
    deepCol += 0.1967 * dp * mix(baseCol, deepStop, 0.5761);
    deepPat += 0.1967 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 7.3229) + par * 0.1538, time);
    deepCol += 0.0873 * dp * mix(baseCol, deepStop, 0.8641);
    deepPat += 0.0873 * dp;
    deepCol *= 0.6276;
    deepPat = pow(clamp(deepPat * 0.5789, 0.0, 1.0), 1.2870);
    ...
    rgb += hotStop * 0.3411 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0723 + 0.3191 * presence + 0.4639 * pattern, 0.8262);
