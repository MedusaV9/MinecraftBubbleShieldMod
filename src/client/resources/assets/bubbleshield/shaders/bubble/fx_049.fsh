...
void main() {
    ...
    mid = clamp(mid * (0.6393 + 0.4736 * texDetail) + 0.2297 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3605), 0.0, 1.0);
    float emit = atlas.a * (0.6542 + 0.3703 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1067 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.7684 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4235 + 0.4591 * presence + 0.4639 * pattern + 0.1455 * (1.0 - deepTrans) + 0.1386 * emit, 0.8862);
