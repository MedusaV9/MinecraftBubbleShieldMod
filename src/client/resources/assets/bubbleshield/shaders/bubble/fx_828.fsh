...
void main() {
    ...
    mid = clamp(mid * (0.6049 + 0.5675 * texDetail) + 0.2493 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3567), 0.0, 1.0);
    float emit = atlas.a * (0.6867 + 0.4363 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1089 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.7818 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3219 + 0.3769 * presence + 0.3631 * pattern + 0.1581 * (1.0 - deepTrans) + 0.1353 * emit, 0.8409);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2790), 0.8409);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6737 + 0.3263 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0864), 0.0, 1.0);
