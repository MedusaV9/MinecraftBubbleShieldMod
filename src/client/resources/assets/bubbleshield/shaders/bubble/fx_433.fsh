...
void main() {
    ...
    mid = clamp(mid * (0.6916 + 0.5313 * texDetail) + 0.2435 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5238), 0.0, 1.0);
    float emit = atlas.a * (0.6251 + 0.3315 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1329 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.7862 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2844 + 0.3143 * presence + 0.5523 * pattern + 0.1571 * (1.0 - deepTrans) + 0.1456 * emit, 0.9131);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2803), 0.9131);
    // [layer:v5:ghostalpha]
