...
void main() {
    ...
    mid = clamp(mid * (0.6993 + 0.6790 * texDetail) + 0.3237 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.5688 + 0.4225 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1637 * (0.5 + 0.5 * sin(time * 0.125664)));
    rgb += emit * 0.7349 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3409 + 0.3499 * presence + 0.4812 * pattern + 0.1407 * (1.0 - deepTrans) + 0.1226 * emit, 0.8887);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3269), 0.8887);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7272 + 0.2728 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
