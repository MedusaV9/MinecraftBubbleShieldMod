...
void main() {
    ...
    mid = clamp(mid * (0.6744 + 0.5217 * texDetail) + 0.2123 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4952), 0.0, 1.0);
    float emit = atlas.a * (0.6517 + 0.2571 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1520 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.9958 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3577 + 0.4062 * presence + 0.3716 * pattern + 0.0846 * (1.0 - deepTrans) + 0.1275 * emit, 0.8847);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2074), 0.8847);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7227 + 0.2773 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
