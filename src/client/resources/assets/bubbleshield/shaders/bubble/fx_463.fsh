...
void main() {
    ...
    mid = clamp(mid * (0.6888 + 0.6999 * texDetail) + 0.2189 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5359), 0.0, 1.0);
    float emit = atlas.a * (0.7310 + 0.2522 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0941 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.8039 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3131 + 0.3114 * presence + 0.5325 * pattern + 0.1101 * (1.0 - deepTrans) + 0.1578 * emit, 0.8444);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1648), 0.8444);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6412 + 0.3588 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0804), 0.0, 1.0);
