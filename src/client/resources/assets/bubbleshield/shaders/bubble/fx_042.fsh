...
void main() {
    ...
    mid = clamp(mid * (0.7408 + 0.6837 * texDetail) + 0.3127 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5026), 0.0, 1.0);
    float emit = atlas.a * (0.6624 + 0.4508 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0994 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.9185 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3634 + 0.4729 * presence + 0.3299 * pattern + 0.1402 * (1.0 - deepTrans) + 0.1878 * emit, 0.9039);
