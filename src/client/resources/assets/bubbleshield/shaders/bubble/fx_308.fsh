...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.043333, 0.037500) * time, time);
    deep += 0.5237 * deepField(duvBase + rimDir * 0.0713 + vec2(0.066667, 0.057500) * time, time);
    deep += 0.2742 * deepField(duvBase + rimDir * 0.1427 + vec2(0.090000, 0.080000) * time, time);
    deep += 0.1436 * deepField(duvBase + rimDir * 0.2140 + vec2(0.115000, 0.100000) * time, time);
    deep = pow(clamp(deep * 0.5151, 0.0, 1.0), 1.5995);
    ...
    float pattern = clamp(0.3665 * deep + 0.9822 * mid + 0.7194 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.0330 + pattern * 0.5573);
    vec3 rgb = vertexColor.rgb * (0.3515 + 0.8131 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1723 + 0.7475 * pattern, 0.0, 1.0);
