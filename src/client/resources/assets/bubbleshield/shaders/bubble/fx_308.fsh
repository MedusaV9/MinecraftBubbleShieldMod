...
void main() {
    ...
    mid = clamp(mid * (0.7435 + 0.6699 * texDetail) + 0.2490 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4455), 0.0, 1.0);
    float emit = atlas.a * (0.5585 + 0.3606 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1483 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.9156 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4091 + 0.5019 * presence + 0.3746 * pattern + 0.1242 * (1.0 - deepTrans) + 0.1804 * emit, 0.9045);
