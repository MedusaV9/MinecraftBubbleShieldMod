...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8734, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0398, abs(glR - 0.1210));
    float mid = clamp(glStars * (1.0 + glRing * 1.9277) + glRing * 0.4168, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.6241 + 0.7378 * texDetail) + 0.2714 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3664), 0.0, 1.0);
    float emit = atlas.a * (0.6299 + 0.4594 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1087 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.6997 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3257 + 0.2682 * presence + 0.5780 * pattern + 0.0828 * (1.0 - deepTrans) + 0.2077 * emit, 0.8681);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3484), 0.8681);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7582 + 0.2418 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
