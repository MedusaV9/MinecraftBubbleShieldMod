...
void main() {
    ...
    mid = clamp(mid * (0.7114 + 0.5236 * texDetail) + 0.2345 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4911), 0.0, 1.0);
    float emit = atlas.a * (0.7057 + 0.3461 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1696 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.9085 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4457 + 0.4402 * presence + 0.4755 * pattern + 0.1352 * (1.0 - deepTrans) + 0.1286 * emit, 0.8445);
