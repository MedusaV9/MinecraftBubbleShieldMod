...
void main() {
    ...
    mid = clamp(mid * (0.7433 + 0.6775 * texDetail) + 0.2275 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4729), 0.0, 1.0);
    float emit = atlas.a * (0.7325 + 0.3079 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1605 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.6984 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4048 + 0.4435 * presence + 0.3798 * pattern + 0.1223 * (1.0 - deepTrans) + 0.1539 * emit, 0.9261);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3186), 0.9261);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7155 + 0.2845 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0849), 0.0, 1.0);
