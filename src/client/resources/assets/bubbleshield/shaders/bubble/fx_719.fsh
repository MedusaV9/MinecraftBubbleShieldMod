...
void main() {
    ...
    mid = clamp(mid * (0.6962 + 0.6859 * texDetail) + 0.2658 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3832), 0.0, 1.0);
    float emit = atlas.a * (0.7219 + 0.3291 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0974 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.9115 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2853 + 0.2808 * presence + 0.5086 * pattern + 0.1111 * (1.0 - deepTrans) + 0.2071 * emit, 0.8519);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1707), 0.8519);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6271 + 0.3729 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0711), 0.0, 1.0);
