...
void main() {
    ...
    mid = clamp(mid * (0.6960 + 0.6615 * texDetail) + 0.1817 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5084), 0.0, 1.0);
    float emit = atlas.a * (0.5729 + 0.4605 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1401 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.8707 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4193 + 0.3988 * presence + 0.3851 * pattern + 0.1577 * (1.0 - deepTrans) + 0.1383 * emit, 0.9317);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2983), 0.9317);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7737 + 0.2263 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0834), 0.0, 1.0);
