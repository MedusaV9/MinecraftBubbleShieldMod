...
// stack: deep=caustic x4 taps | mid=raymarchfog+curl+rotate | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    float mid = clamp(rfLight * 0.9239 + (1.0 - rfTrans) * 0.2096, 0.0, 1.15);
    ...
    float flourish = 0.2720 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0782 + 0.3388 * presence + 0.3851 * pattern + 0.1577 * (1.0 - deepTrans), 0.8717);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2983), 0.8717);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.5433 + 0.4567 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
