...
// stack: deep=ridge x3 taps | mid=vortex+curl+rotate | rim=graze_film | flourish=swirl
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.030000, 0.023333) * time, time);
    deep += 0.5017 * deepField(duvBase + rimDir * 0.0740 + vec2(-0.046667, 0.036667) * time, time);
    deep += 0.2517 * deepField(duvBase + rimDir * 0.1480 + vec2(-0.062500, 0.048333) * time, time);
    deep = pow(clamp(deep * 0.5704, 0.0, 1.0), 1.0629);
    ...
    float pattern = clamp(0.3472 * deep + 0.8783 * mid + 0.5658 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.2518 + pattern * 0.5976);
    vec3 rgb = vertexColor.rgb * (0.3932 + 0.7541 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1999 + 0.7901 * pattern, 0.0, 1.0);
