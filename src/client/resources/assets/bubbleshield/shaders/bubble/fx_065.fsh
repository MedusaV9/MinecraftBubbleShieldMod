...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6402, 0.0, 1.0);
    ...
    deepCol += 0.5017 * dp * mix(baseCol, deepStop, 0.2547);
    deepPat += 0.5017 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 2.9347) + par * 0.1480, time);
    deepCol += 0.2517 * dp * mix(baseCol, deepStop, 0.5095);
    deepPat += 0.2517 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 3.5312) + par * 0.2220, time);
    deepCol += 0.1262 * dp * mix(baseCol, deepStop, 0.7642);
    deepPat += 0.1262 * dp;
    deepCol *= 0.6201;
    deepPat = pow(clamp(deepPat * 0.5320, 0.0, 1.0), 1.0629);
    ...
    rgb += hotStop * 0.3116 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0786 + 0.3441 * presence + 0.3856 * pattern, 0.8195);
