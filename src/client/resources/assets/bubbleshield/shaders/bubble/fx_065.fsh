...
void main() {
    ...
    mid = clamp(mid * (0.6720 + 0.5255 * texDetail) + 0.2234 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5425), 0.0, 1.0);
    float emit = atlas.a * (0.6946 + 0.4973 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1187 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.7180 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4423 + 0.4841 * presence + 0.3856 * pattern + 0.1423 * (1.0 - deepTrans) + 0.1419 * emit, 0.8795);
