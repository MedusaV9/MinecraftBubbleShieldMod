...
void main() {
    ...
    mid = clamp(mid * (0.7475 + 0.6011 * texDetail) + 0.3015 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3829), 0.0, 1.0);
    float emit = atlas.a * (0.6322 + 0.3275 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1453 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.7644 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3662 + 0.3215 * presence + 0.3291 * pattern + 0.1315 * (1.0 - deepTrans) + 0.1369 * emit, 0.8896);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1953), 0.8896);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7139 + 0.2861 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0739), 0.0, 1.0);
