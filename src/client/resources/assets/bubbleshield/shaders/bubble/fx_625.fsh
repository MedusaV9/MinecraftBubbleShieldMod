...
void main() {
    ...
    mid = clamp(mid * (0.6498 + 0.4848 * texDetail) + 0.3398 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3944), 0.0, 1.0);
    float emit = atlas.a * (0.6249 + 0.2941 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1146 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.8572 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3126 + 0.4233 * presence + 0.5070 * pattern + 0.1495 * (1.0 - deepTrans) + 0.1918 * emit, 0.8834);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3395), 0.8834);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6468 + 0.3532 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1013), 0.0, 1.0);
