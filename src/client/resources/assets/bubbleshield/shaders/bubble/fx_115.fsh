...
void main() {
    ...
    mid = clamp(mid * (0.7187 + 0.6443 * texDetail) + 0.2677 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4995), 0.0, 1.0);
    float emit = atlas.a * (0.6259 + 0.3549 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1003 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7520 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3627 + 0.4173 * presence + 0.4361 * pattern + 0.1278 * (1.0 - deepTrans) + 0.1747 * emit, 0.8939);
