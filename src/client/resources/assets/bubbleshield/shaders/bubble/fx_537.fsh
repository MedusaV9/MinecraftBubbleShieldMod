...
void main() {
    ...
    mid = clamp(mid * (0.7292 + 0.6770 * texDetail) + 0.3174 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4380), 0.0, 1.0);
    float emit = atlas.a * (0.7223 + 0.4496 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1553 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.9246 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3138 + 0.2060 * presence + 0.6525 * pattern + 0.1005 * (1.0 - deepTrans) + 0.2094 * emit, 0.9375);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2024), 0.9375);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1017), 0.0, 1.0);
