...
void main() {
    ...
    mid = clamp(mid * (0.6618 + 0.4745 * texDetail) + 0.3234 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5334), 0.0, 1.0);
    float emit = atlas.a * (0.7489 + 0.2878 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1602 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.9611 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3718 + 0.4078 * presence + 0.3865 * pattern + 0.1123 * (1.0 - deepTrans) + 0.1790 * emit, 0.8554);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2488), 0.8554);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6906 + 0.3094 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
