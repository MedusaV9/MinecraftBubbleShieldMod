...
void main() {
    ...
    mid = clamp(mid * (0.6866 + 0.5438 * texDetail) + 0.3434 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6090 + 0.2726 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0809 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.7857 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3456 + 0.4431 * presence + 0.3039 * pattern + 0.1089 * (1.0 - deepTrans) + 0.2007 * emit, 0.9009);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2730), 0.9009);
    // [layer:v5:ghostalpha]
