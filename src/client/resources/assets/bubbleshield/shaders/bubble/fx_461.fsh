...
// stack: deep=voro x3 taps | mid=raymarchfog+none+rotate | rim=lat | flourish=shimmer
...
void main() {
    ...
    float mid = clamp(rfLight * 1.2047 + (1.0 - rfTrans) * 0.2065, 0.0, 1.15);
    ...
    float alpha = vertexColor.a * min(0.0501 + 0.3831 * presence + 0.3039 * pattern + 0.1089 * (1.0 - deepTrans), 0.8409);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2730), 0.8409);
    // [layer:v5:ghostalpha]
