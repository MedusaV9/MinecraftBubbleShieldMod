...
// stack: deep=ridge x4 taps | mid=mycelia+warp2+rotate | rim=graze | flourish=echo | tex=filaments
// fbm: turb x6 octaves | seed 78887f677ad97909
// v8 realism: refr 0.2872+0.3997*fres/dist (cap 0.1577) | energy 0.5281+0.3421*p | flow 0.068333 c/s | soft 0.7036m
...
void main() {
    ...
    float refrAmp = min((0.2872 + 0.3997 * fresnel) / max(length(worldPos), 1.0), 0.1577);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0508, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9492, vec2(0.001), vec2(0.999))).b);
