...
void main() {
    ...
    mid = clamp(mid * (0.6623 + 0.5968 * texDetail) + 0.2415 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4159), 0.0, 1.0);
    float emit = atlas.a * (0.6171 + 0.2897 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1449 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.7178 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2856 + 0.2909 * presence + 0.4627 * pattern + 0.1599 * (1.0 - deepTrans) + 0.1389 * emit, 0.8974);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1545), 0.8974);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6832 + 0.3168 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1045), 0.0, 1.0);
