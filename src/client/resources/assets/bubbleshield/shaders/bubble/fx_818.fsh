...
void main() {
    ...
    mid = clamp(mid * (0.6350 + 0.5510 * texDetail) + 0.2874 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7130 + 0.4980 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0855 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.9911 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2951 + 0.3024 * presence + 0.5297 * pattern + 0.1195 * (1.0 - deepTrans) + 0.1898 * emit, 0.8670);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1520), 0.8670);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6557 + 0.3443 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0604), 0.0, 1.0);
