...
void main() {
    ...
    mid = clamp(mid * (0.6255 + 0.6411 * texDetail) + 0.1935 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3643), 0.0, 1.0);
    float emit = atlas.a * (0.6897 + 0.4199 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1531 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.8609 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3718 + 0.4557 * presence + 0.4390 * pattern + 0.1226 * (1.0 - deepTrans) + 0.1904 * emit, 0.8751);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3083), 0.8751);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7369 + 0.2631 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
