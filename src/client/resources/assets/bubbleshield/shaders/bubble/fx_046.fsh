...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0565) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0565);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.046667, 0.047500) * time, time);
    deep += 0.4150 * deepField(duvBase + rimDir * 0.0773 + vec2(0.073333, 0.072500) * time, time);
    deep = pow(clamp(deep * 0.7067, 0.0, 1.0), 1.1397);
