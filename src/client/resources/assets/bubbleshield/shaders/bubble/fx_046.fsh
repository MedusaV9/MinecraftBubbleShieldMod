...
void main() {
    ...
    mid = clamp(mid * (0.6528 + 0.6650 * texDetail) + 0.2831 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5002), 0.0, 1.0);
    float emit = atlas.a * (0.5553 + 0.3496 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1268 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.9771 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3709 + 0.4611 * presence + 0.4203 * pattern + 0.0923 * (1.0 - deepTrans) + 0.2121 * emit, 0.8685);
