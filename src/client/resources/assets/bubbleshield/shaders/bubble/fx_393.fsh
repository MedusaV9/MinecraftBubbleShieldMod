...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7001, 0.0, 1.0);
    ...
    deepCol += 0.4625 * dp * mix(baseCol, deepStop, 0.3230);
    deepPat += 0.4625 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 4.9787) + par * 0.1520, time);
    deepCol += 0.2139 * dp * mix(baseCol, deepStop, 0.6460);
    deepPat += 0.2139 * dp;
    deepCol *= 0.6960;
    deepPat = pow(clamp(deepPat * 0.5965, 0.0, 1.0), 1.1866);
    ...
    rgb = satLift(rgb, 1.2349);
    rgb += hotStop * 0.3148 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0526 + 0.3217 * presence + 0.4323 * pattern, 0.8227);
