...
void main() {
    ...
    mid = clamp(mid * (0.6743 + 0.5016 * texDetail) + 0.1916 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5253), 0.0, 1.0);
    float emit = atlas.a * (0.6608 + 0.2840 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1086 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.8091 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3132 + 0.3753 * presence + 0.4285 * pattern + 0.1469 * (1.0 - deepTrans) + 0.2164 * emit, 0.9070);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1899), 0.9070);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7063 + 0.2937 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
