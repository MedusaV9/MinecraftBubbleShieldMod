...
void main() {
    ...
    mid = clamp(mid * (0.6567 + 0.5851 * texDetail) + 0.2271 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4532), 0.0, 1.0);
    float emit = atlas.a * (0.5828 + 0.4415 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1345 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.9493 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3391 + 0.2898 * presence + 0.5009 * pattern + 0.1315 * (1.0 - deepTrans) + 0.2095 * emit, 0.9165);
