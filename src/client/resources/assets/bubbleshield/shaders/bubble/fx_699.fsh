...
// stack: deep=caustic x3 taps | mid=voidrift+warp1+rotate | rim=graze | flourish=echo | tex=cracked_glass
// fbm: turb x5 octaves | seed d971285f8cd51b28
// v8 realism: refr 0.3649+0.2993*fres/dist (cap 0.1534) | energy 0.4616+0.4164*p | flow 0.077500 c/s | soft 0.7058m
...
void main() {
    ...
    float refrAmp = min((0.3649 + 0.2993 * fresnel) / max(length(worldPos), 1.0), 0.1534);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0426, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9574, vec2(0.001), vec2(0.999))).b);
