...
void main() {
    ...
    mid = clamp(mid * (0.6631 + 0.5420 * texDetail) + 0.2360 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4631), 0.0, 1.0);
    float emit = atlas.a * (0.7250 + 0.3143 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0974 * (0.5 + 0.5 * sin(time * 0.240855)));
    rgb += emit * 0.7899 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4196 + 0.3841 * presence + 0.3762 * pattern + 0.0932 * (1.0 - deepTrans) + 0.1815 * emit, 0.8960);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2944), 0.8960);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7482 + 0.2518 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
