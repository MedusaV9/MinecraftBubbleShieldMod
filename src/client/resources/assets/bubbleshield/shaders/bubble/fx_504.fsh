...
void main() {
    ...
    mid = clamp(mid * (0.6279 + 0.4616 * texDetail) + 0.3167 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5047), 0.0, 1.0);
    float emit = atlas.a * (0.5869 + 0.2538 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0843 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.7801 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2876 + 0.3335 * presence + 0.4929 * pattern + 0.1436 * (1.0 - deepTrans) + 0.2151 * emit, 0.8568);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2793), 0.8568);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6819 + 0.3181 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0828), 0.0, 1.0);
