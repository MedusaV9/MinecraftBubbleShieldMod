...
void main() {
    ...
    mid = clamp(mid * (0.6205 + 0.6711 * texDetail) + 0.2009 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4328), 0.0, 1.0);
    float emit = atlas.a * (0.7709 + 0.3097 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1582 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.6697 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3403 + 0.3424 * presence + 0.4331 * pattern + 0.0855 * (1.0 - deepTrans) + 0.1337 * emit, 0.8829);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1864), 0.8829);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6267 + 0.3733 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0645), 0.0, 1.0);
