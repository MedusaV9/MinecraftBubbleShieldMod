...
void main() {
    ...
    mid = clamp(mid * (0.6977 + 0.7460 * texDetail) + 0.2014 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5146), 0.0, 1.0);
    float emit = atlas.a * (0.7783 + 0.2981 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1498 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.9074 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2963 + 0.2694 * presence + 0.5221 * pattern + 0.0918 * (1.0 - deepTrans) + 0.1535 * emit, 0.8881);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1583), 0.8881);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6888 + 0.3112 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1089), 0.0, 1.0);
