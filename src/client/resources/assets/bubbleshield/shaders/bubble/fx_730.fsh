...
// stack: deep=caustic x4 taps | mid=phantomecho+warp1+rotate | rim=graze_film | flourish=shimmer | tex=foam
// fbm: ridged x3 octaves | seed 6e66e93b7c921c12
// v8 realism: refr 0.3070+0.3622*fres/dist (cap 0.1109) | energy 0.4225+0.3927*p | flow 0.059167 c/s | soft 1.7766m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (0.6581 * atlasPoleW));
    ...
    float refrAmp = min((0.3070 + 0.3622 * fresnel) / max(length(worldPos), 1.0), 0.1109);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0407, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9593, vec2(0.001), vec2(0.999))).b);
