...
void main() {
    ...
    mid = clamp(mid * (0.7022 + 0.7238 * texDetail) + 0.2962 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5130), 0.0, 1.0);
    float emit = atlas.a * (0.7055 + 0.4296 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1438 * (0.5 + 0.5 * sin(time * 0.104720)));
    rgb += emit * 0.7124 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3471 + 0.4019 * presence + 0.4292 * pattern + 0.0844 * (1.0 - deepTrans) + 0.1562 * emit, 0.9164);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1970), 0.9164);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7795 + 0.2205 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1110), 0.0, 1.0);
