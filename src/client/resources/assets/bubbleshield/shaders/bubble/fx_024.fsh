...
void main() {
    ...
    mid = clamp(mid * (0.6376 + 0.4855 * texDetail) + 0.2629 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4599), 0.0, 1.0);
    float emit = atlas.a * (0.6160 + 0.4651 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1582 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.9866 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3571 + 0.4006 * presence + 0.3269 * pattern + 0.1017 * (1.0 - deepTrans) + 0.1228 * emit, 0.8989);
