...
// stack: deep=ridge x2 taps | mid=arcs+warp1+flicker | rim=graze_sparkle | flourish=glint
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.046667, 0.027500) * time, time);
    deep += 0.5739 * deepField(duvBase + rimDir * 0.0469 + vec2(-0.073333, 0.042500) * time, time);
    deep = pow(clamp(deep * 0.6353, 0.0, 1.0), 1.2320);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.386667), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 7.7029);
    float flash = 0.6 + 0.4 * sin(time * 2.366666 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.4658 * deep + 0.8442 * mid + 0.8214 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1968 + pattern * 0.4209);
    vec3 rgb = vertexColor.rgb * (0.4315 + 0.7985 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1099 + 0.6725 * pattern, 0.0, 1.0);
