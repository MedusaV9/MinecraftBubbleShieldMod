...
void main() {
    ...
    mid = clamp(mid * (0.6287 + 0.7293 * texDetail) + 0.2348 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3952), 0.0, 1.0);
    float emit = atlas.a * (0.5653 + 0.3705 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1574 * (0.5 + 0.5 * sin(time * 0.219911)));
    rgb += emit * 0.8781 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4099 + 0.4588 * presence + 0.4529 * pattern + 0.1576 * (1.0 - deepTrans) + 0.2144 * emit, 0.8755);
