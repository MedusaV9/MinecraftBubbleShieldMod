...
void main() {
    ...
    mid = clamp(mid * (0.6748 + 0.6224 * texDetail) + 0.2748 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4926), 0.0, 1.0);
    float emit = atlas.a * (0.7154 + 0.4519 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0983 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.7193 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3277 + 0.4218 * presence + 0.3085 * pattern + 0.1582 * (1.0 - deepTrans) + 0.2169 * emit, 0.9294);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1612), 0.9294);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7782 + 0.2218 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0635), 0.0, 1.0);
