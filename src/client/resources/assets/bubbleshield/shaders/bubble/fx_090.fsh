...
// v8 realism: refr 1.6469+2.1541*fres/dist (cap 0.1782) | energy 0.2776+0.4212*p | flow 0.089167 c/s | soft 0.8948m
...
void main() {
    ...
    rim = clamp(rim + 0.8912 * rimLine + 0.5193 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2706 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    float grain = 0.0917 * (cellHash(floor(wuv * 27.0000) + vec2(floor(time * 6.0), 0.0), 108.0000) - 0.5);
    ...
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.y * 1.0000 - -1.0 * time * 0.140833 + 0.2372)), 14.8522) * motifBand;
    ...
    float refrAmp = min((1.6469 + 2.1541 * fresnel) / max(length(worldPos), 1.0), 0.1782);
    ...
    float energyW = clamp(0.2776 + 0.4212 * clamp(pattern, 0.0, 1.0) + 0.1801 * fresnel, 0.0, 0.8175);
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.3457) * 0.8718;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3290 * (1.0 - 0.5847 * baseLuma)), 0.0, 1.0);
    float emit = atlas.a * (0.4324 + 0.6924 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0897 * (0.5 + 0.5 * sin(time * 0.272271))) * atlasPoleW;
    rgb += emit * 0.9527 * (1.0 - 0.7784 * baseLuma) * emitCol;
