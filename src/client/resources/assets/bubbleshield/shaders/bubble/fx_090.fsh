...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5747, 0.0, 1.0);
    ...
    deepCol += 0.4171 * dp * mix(baseCol, deepStop, 0.3638);
    deepPat += 0.4171 * dp;
    dp = deepField(rotA(spinAxis, time * 0.094248) * (sdir * 3.0652) + par * 0.1485, time);
    deepCol += 0.1740 * dp * mix(baseCol, deepStop, 0.7277);
    deepPat += 0.1740 * dp;
    deepCol *= 0.7155;
    deepPat = pow(clamp(deepPat * 0.6285, 0.0, 1.0), 1.6344);
    ...
    float midCover = clamp(1.0902 * mid + 0.6250 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.3083);
    rgb += hotStop * 0.1832 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0477 + 0.3525 * presence + 0.3733 * pattern, 0.8468);
