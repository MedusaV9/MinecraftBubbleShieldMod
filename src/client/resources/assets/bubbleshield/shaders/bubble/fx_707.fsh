...
void main() {
    ...
    mid = clamp(mid * (0.6949 + 0.5347 * texDetail) + 0.2520 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7504 + 0.4466 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1064 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.8485 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3784 + 0.3981 * presence + 0.4288 * pattern + 0.0963 * (1.0 - deepTrans) + 0.1293 * emit, 0.8102);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1811), 0.8102);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7246 + 0.2754 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1144), 0.0, 1.0);
