...
// stack: deep=fbm x4 taps | mid=irisfilm+warp2+pulse | rim=graze_film | flourish=shimmer | tex=chrome
// fbm: ridged x5 octaves | seed 7da020910dc1cf31
// v8 realism: refr 0.8042+0.8282*fres/dist (cap 0.1432) | energy 0.2579+0.3674*p | flow 0.070000 c/s | soft 0.8757m
...
void main() {
    ...
    float refrAmp = min((0.8042 + 0.8282 * fresnel) / max(length(worldPos), 1.0), 0.1432);
