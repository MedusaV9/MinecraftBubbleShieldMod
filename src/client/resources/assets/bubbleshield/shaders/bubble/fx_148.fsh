...
// stack: deep=caustic x2 taps | mid=thinfilm+warp2+rotate | rim=graze | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.005000, -0.047500) * time, time);
    deep += 0.4636 * deepField(duvBase + rimDir * 0.0686 + vec2(-0.008333, -0.072500) * time, time);
    deep = pow(clamp(deep * 0.6832, 0.0, 1.0), 1.1984);
    ...
    float pattern = clamp(0.4004 * deep + 0.8821 * mid + 0.5585 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9270 + pattern * 0.5829);
    vec3 rgb = vertexColor.rgb * (0.4867 + 0.6880 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2311 + 0.7705 * pattern, 0.0, 1.0);
