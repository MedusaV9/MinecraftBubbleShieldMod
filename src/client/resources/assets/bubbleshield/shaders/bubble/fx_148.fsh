...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7499, 0.0, 1.0);
    ...
    deepCol += 0.4636 * dp * mix(baseCol, deepStop, 0.3648);
    deepPat += 0.4636 * dp;
    dp = deepField(rotA(spinAxis, time * 0.099484) * (sdir * 3.2757) + par * 0.1371, time);
    deepCol += 0.2149 * dp * mix(baseCol, deepStop, 0.7296);
    deepPat += 0.2149 * dp;
    deepCol *= 0.6399;
    deepPat = pow(clamp(deepPat * 0.5958, 0.0, 1.0), 1.1984);
    ...
    rgb = satLift(rgb, 1.2197);
    rgb += hotStop * 0.1933 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0579 + 0.3433 * presence + 0.4850 * pattern, 0.8017);
