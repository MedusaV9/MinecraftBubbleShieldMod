...
// v8 realism: refr 1.6163+2.6157*fres/dist (cap 0.1855) | energy 0.2346+0.4751*p | flow 0.066667 c/s | soft 1.0095m
...
void main() {
    ...
    float jump = hash11(floor(time * 4.7705));
    ...
    vec2 tuv = vec2(baseUV.x * 8.0000, baseUV.y * 8.0000) + vec2(-0.106667, 0.150000) * time;
    vec2 wuv = auv;
    // [layer:v5:polefade]
    ...
    vec2 texUV = tuv * 1.0000 + vec2(-0.046667, -0.035833) * time;
    ...
    rim = clamp(rim + 0.7894 * rimLine + 0.4027 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1749 * caustic(wuv + vec2(3.1, 8.7), vec2(0.140000, -0.116667) * time, midPer);
    float grain = 0.0618 * (cellHash(floor(wuv * 56.0000) + vec2(floor(time * 6.0), 0.0), 448.0000) - 0.5);
    ...
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.y * 1.0000 - 1.0 * time * 0.055833 + 0.4678)), 21.3390) * motifBand;
    ...
    float refrAmp = min((1.6163 + 2.6157 * fresnel) / max(length(worldPos), 1.0), 0.1855);
    ...
    float energyW = clamp(0.2346 + 0.4751 * clamp(pattern, 0.0, 1.0) + 0.1272 * fresnel, 0.0, 0.8527);
    rgb = mix(refracted, rgb, energyW);
    rgb += fresnel * hotStop * 0.5126;
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2870) * 0.7316;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2684 * (1.0 - 0.5433 * baseLuma)), 0.0, 1.0);
    // strobe-free emission drive (see the v7 note above): only smooth,
    // non-gated terms may modulate the emissive add
    float emitDrive = clamp(0.3670 * deepPat + 0.8026 * texDetail + 0.7386 * rim + 0.1114 * v5Par, 0.0, 1.0);
    float emit = atlas.a * (0.3750 + 0.7081 * emitDrive) * (1.0 - 0.0928 * (0.5 + 0.5 * sin(time * 0.125664))) * atlasPoleW;
    rgb += emit * 0.8458 * (1.0 - 0.7361 * baseLuma) * emitCol;
    // [layer:v5:softknee]
    ...
    bodyAlpha = max(bodyAlpha, 0.9084);
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9334) * vertexColor.a;
