...
void main() {
    ...
    mid = clamp(mid * (0.6501 + 0.4837 * texDetail) + 0.2725 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3621), 0.0, 1.0);
    float emit = atlas.a * (0.6750 + 0.4651 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0928 * (0.5 + 0.5 * sin(time * 0.125664)));
    ...
    float alpha = vertexColor.a * min(0.4214 + 0.5284 * presence + 0.3548 * pattern + 0.1146 * (1.0 - deepTrans) + 0.1680 * emit, 0.8858);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1653), 0.8858);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7055 + 0.2945 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0844), 0.0, 1.0);
