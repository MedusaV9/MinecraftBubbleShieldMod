...
// Bubble surface shader fx_694 -- family DEEPICE
// stack: deep=caustic x3 taps | mid=deepice+none+flicker | rim=graze_film | flourish=shimmer | tex=topo_contours | motif=scanx2+doublepulse
// fbm: standard x5 octaves | seed 869b8757c138333b
// v8 realism: refr 1.8069+2.9243*fres/dist (cap 0.1855) | energy 0.1837+0.4751*p | flow 0.066667 c/s | soft 1.0095m
...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0832) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0832);
        amplitude *= 0.5;
    }
    return value;
}
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3385 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.9756, 1.2488, 1.2527) * t
        + vec3(0.5766, 0.1462, 0.5053)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.4502, x));
    return mix(g, hotStop, smoothstep(0.4502, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0053, 0.0483, g);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.6241;
    vec3 spun = clamp(hueSpin(baseCol, 0.0879), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2967) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.5882, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.6520);
    ...
    vec3 spinAxis = vec3(0.2010, 0.5966, 0.7770);
    vec3 par = Vt * 1.8110;
    ...
    float deepBreath = 1.0 + 0.0161 * sin(time * 1.581268);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * (2.4553 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.4040 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * (3.8708 * deepBreath)) + par * 0.0736, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.5732), deepStop, 0.2102);
    deepTrans *= 1.0 - 0.4040 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * (5.2863 * deepBreath)) + par * 0.1471, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4203);
    deepTrans *= 1.0 - 0.4040 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.9093;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.2686, 0.0, 1.0), 1.3170);
    ...
    float ft = time + 0.2041 * (0.6 * sin(time * 4.696681) + 0.4 * sin(time * 9.932669 + 2.8233));
    ...
    vec2 iceOff = rimDir * (0.3818 + 0.9867 * rimGraze());
    vec3 iceB = voro2(wuv * 2.0 + iceOff + vec2(13.7, 5.9), midPer * 2.0, 0.0);
    float crackA = invsmooth(0.004, 0.0448, iceA.x);
    float crackB = invsmooth(0.004, 0.0481, iceB.x);
    ...
    vec2 texUV = wuv * 1.0000 + vec2(-0.046667, -0.035833) * time;
    ...
    vec3 hgtW = vec3(0.3782, 0.4114, 0.2104);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -2.8424;
    ...
    float texDetail = clamp(atlas.r * 0.4612 + atlas.g * 0.5017 + atlas.b * 0.2566, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6501 + 0.4837 * texDetail, atlasPoleW) + 0.1544 * texDetail * (0.3396 + 0.6604 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.8526 * atlasPoleW));
    ...
    // thickness rho = 0.0881 (crystalline material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.8209.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9119 * 0.9119 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.8209);
    float chordN = min(chord / 0.0881, 3.0);
    ...
    float rimLine = smoothstep(0.0552, 0.0943, rimSlope);
    float rim = rimGraze() * 0.8990;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.7894 * rimLine + 0.6132 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.1749 * caustic(wuv + vec2(3.1, 8.7), vec2(0.140000, -0.116667) * time, midPer);
    float grain = 0.0618 * (cellHash(floor(wuv * 56.0000) + vec2(floor(time * 2.0), 0.0), 448.0000) - 0.5);
    ...
    float motifP = fract(time * 0.287500 + 0.8377);
    ...
    float motifM = pow(0.5 + 0.5 * cos(6.2831853 * (baseUV.y * 1.0000 - 1.0 * time * 0.055833 + 0.4678)), 14.3703) * motifBand;
    ...
    float v5Par = deepField(sdir * 2.2540 + par * 0.0733, time) * 0.5;
    v5Par += deepField(sdir * 3.0429 + par * 0.1466, time) * 0.3;
    v5Par += deepField(sdir * 3.9444 + par * 0.2199, time) * 0.2;
    float pattern = clamp(0.3670 * deepPat * mix(1.0, chordN, 0.2497) + 0.8026 * mid + 0.7386 * rim + flourish + grain + 0.1114 * v5Par + 0.1389 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.8364 + rim * 0.2706, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9699 * mid + 0.5759 * rim, 0.0, 1.0);
    ...
    rgb *= mix(1.0, texBody, atlasPoleW * (0.4355 + 0.5645 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.1359);
    ...
    vec3 softHot = 1.0 - exp(-2.6362 * hotStop * (pattern + 0.35 * rim));
    rgb = mix(rgb, softHot, 0.2673 * smoothstep(0.55, 1.10, pattern));
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 0.8863 + baseUV.y * 0.3281 + 0.3703) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1440);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.6902 + baseUV.y * 0.31 + 0.2054) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2512);
    ...
    rgb *= (1.0 - 0.3296 * washHi) * (1.0 - 0.3333 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.5679, 0.7088, -0.4184);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.5668) / 1.5668, 0.0, 1.0);
    rgb *= 0.6563 + 0.5408 * keyDiff * (1.0 - 0.5295 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 36.8875);
    rgb += hotStop * (keySpec * 0.6803 * (1.0 - 0.6765 * washHi));
    ...
    float refrAmp = min((1.8069 + 2.9243 * fresnel) / max(length(worldPos), 1.0), 0.1855);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.0990) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.1091, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.8909, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.2115);
    float energyW = clamp(0.1837 + 0.4751 * clamp(pattern, 0.0, 1.0) + 0.2118 * fresnel, 0.0, 0.8527);
    ...
    rgb += fresnel * hotStop * (0.8751 * (1.0 - 0.5858 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.2870) * 1.1043;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.2684 * max(0.0, 1.0 - 0.5433 * baseLuma - 0.5547 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3750 + 0.7081 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0928 * (0.5 + 0.5 * sin(time * 0.125664))) * atlasPoleW;
    rgb += emit * 0.8458 * (1.0 - 0.7361 * baseLuma) * (1.0 - 0.4074 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    vec3 v5Over = max(rgb - vec3(0.6436), vec3(0.0));
    rgb = min(rgb, vec3(0.6436)) + 0.3564 * (1.0 - exp(-v5Over * 2.3446));
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2292 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 4.7176));
    ...
    float bodyAlpha = 0.4214 + 0.5284 * presence + 0.3548 * pattern + 0.1146 * (1.0 - deepTrans) + 0.1680 * emit;
    ...
    rgb = mix(rgb, deepStop, v5Back * 0.3842);
    bodyAlpha *= 1.0 + v5Back * 0.1653;
    // [layer:v5:ghostalpha]
    ...
    bodyAlpha *= 0.7055 + 0.2945 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    bodyAlpha *= 1.0 + (v5Dither - 0.5) * 0.0844;
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.0610 + hgtC * 0.2518, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.3068);
    bodyAlpha *= 1.0 + innerFace * 0.1414;
    ...
    float alpha = clamp(bodyAlpha, 0.0, 0.9613) * vertexColor.a;
