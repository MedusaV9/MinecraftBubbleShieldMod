...
void main() {
    ...
    mid = clamp(mid * (0.6559 + 0.6655 * texDetail) + 0.2785 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6558 + 0.4937 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1132 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.8221 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2999 + 0.2770 * presence + 0.5586 * pattern + 0.1446 * (1.0 - deepTrans) + 0.1207 * emit, 0.8479);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3148), 0.8479);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6246 + 0.3754 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0651), 0.0, 1.0);
