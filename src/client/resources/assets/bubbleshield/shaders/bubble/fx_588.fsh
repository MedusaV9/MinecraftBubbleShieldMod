...
void main() {
    ...
    mid = clamp(mid * (0.7206 + 0.6443 * texDetail) + 0.2660 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4902), 0.0, 1.0);
    float emit = atlas.a * (0.7953 + 0.3681 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1747 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.8080 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3144 + 0.4170 * presence + 0.4152 * pattern + 0.1353 * (1.0 - deepTrans) + 0.1380 * emit, 0.9095);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3164), 0.9095);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7014 + 0.2986 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0805), 0.0, 1.0);
