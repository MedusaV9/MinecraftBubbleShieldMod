...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6242, 0.0, 1.0);
    ...
    deepCol += 0.5135 * dp * mix(baseCol, deepStop, 0.2304);
    deepPat += 0.5135 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.4443) + par * 0.0868, time);
    deepCol += 0.2637 * dp * mix(baseCol, deepStop, 0.4607);
    deepPat += 0.2637 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 6.8963) + par * 0.1302, time);
    deepCol += 0.1354 * dp * mix(baseCol, deepStop, 0.6911);
    deepPat += 0.1354 * dp;
    deepCol *= 0.6295;
    deepPat = pow(clamp(deepPat * 0.5229, 0.0, 1.0), 1.2886);
    ...
    float blob = invsmooth(0.05, 0.5325, bio.y);
    ...
    float mid = clamp(blob * (0.3351 + 0.7400 * waveGlow) + haze, 0.0, 1.2);
    ...
    float pattern = clamp(0.3718 * deepPat + 0.7179 * mid + 0.7818 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb = satLift(rgb, 1.1005);
    rgb += hotStop * 0.1818 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0586 + 0.2991 * presence + 0.3634 * pattern, 0.8198);
