...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8694, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0432, abs(glR - 0.1509));
    float mid = clamp(glStars * (1.0 + glRing * 1.4933) + glRing * 0.5125, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.7185 + 0.7450 * texDetail) + 0.3220 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4562), 0.0, 1.0);
    float emit = atlas.a * (0.6365 + 0.4063 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0940 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.8066 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2837 + 0.2776 * presence + 0.5307 * pattern + 0.1294 * (1.0 - deepTrans) + 0.1371 * emit, 0.8615);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2719), 0.8615);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6294 + 0.3706 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1175), 0.0, 1.0);
