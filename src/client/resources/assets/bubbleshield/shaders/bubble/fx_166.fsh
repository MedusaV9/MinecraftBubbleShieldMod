...
void main() {
    ...
    mid = clamp(mid * (0.7037 + 0.5815 * texDetail) + 0.2128 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5332), 0.0, 1.0);
    float emit = atlas.a * (0.7792 + 0.2775 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1305 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.7050 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4322 + 0.3808 * presence + 0.3316 * pattern + 0.1090 * (1.0 - deepTrans) + 0.1566 * emit, 0.8506);
