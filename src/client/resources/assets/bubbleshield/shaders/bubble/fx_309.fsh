...
void main() {
    ...
    mid = clamp(mid * (0.6083 + 0.6190 * texDetail) + 0.3326 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3528), 0.0, 1.0);
    float emit = atlas.a * (0.7521 + 0.2561 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1226 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.9791 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4410 + 0.3905 * presence + 0.3331 * pattern + 0.1271 * (1.0 - deepTrans) + 0.1916 * emit, 0.8642);
