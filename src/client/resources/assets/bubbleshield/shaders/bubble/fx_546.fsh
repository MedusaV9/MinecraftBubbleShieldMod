...
void main() {
    ...
    mid = clamp(mid * (0.6815 + 0.5419 * texDetail) + 0.1867 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3582), 0.0, 1.0);
    float emit = atlas.a * (0.6821 + 0.3467 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1779 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.9908 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4173 + 0.3618 * presence + 0.3470 * pattern + 0.1404 * (1.0 - deepTrans) + 0.1763 * emit, 0.8496);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2599), 0.8496);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6700 + 0.3300 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0910), 0.0, 1.0);
