...
void main() {
    ...
    mid = clamp(mid * (0.7135 + 0.6848 * texDetail) + 0.3371 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4939), 0.0, 1.0);
    float emit = atlas.a * (0.7127 + 0.2830 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1606 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.9856 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4233 + 0.4393 * presence + 0.3321 * pattern + 0.1004 * (1.0 - deepTrans) + 0.2026 * emit, 0.8829);
