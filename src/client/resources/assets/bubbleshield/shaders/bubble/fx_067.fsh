...
// stack: deep=voro x3 taps | mid=aurora+warp1+scroll | rim=graze_sparkle | flourish=swirl
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3642 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.001667, 0.000000) * time, time);
    deep += 0.4836 * deepField(duvBase + rimDir * 0.0486 + vec2(-0.003333, 0.002500) * time, time);
    deep += 0.2338 * deepField(duvBase + rimDir * 0.0973 + vec2(-0.003333, 0.002500) * time, time);
    deep = pow(clamp(deep * 0.5823, 0.0, 1.0), 1.1680);
    ...
    float pattern = clamp(0.4462 * deep + 0.9630 * mid + 0.5584 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8761 + pattern * 0.3389);
    vec3 rgb = vertexColor.rgb * (0.4335 + 0.6804 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1396 + 0.7312 * pattern, 0.0, 1.0);
