...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8481, 0.0, 1.0);
    ...
    deepCol += 0.4836 * dp * mix(baseCol, deepStop, 0.2586);
    deepPat += 0.4836 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.5175) + par * 0.0973, time);
    deepCol += 0.2338 * dp * mix(baseCol, deepStop, 0.5171);
    deepPat += 0.2338 * dp;
    dp = deepField(rotA(spinAxis, time * 0.020944) * (sdir * 6.7829) + par * 0.1459, time);
    deepCol += 0.1131 * dp * mix(baseCol, deepStop, 0.7757);
    deepPat += 0.1131 * dp;
    deepCol *= 0.6287;
    deepPat = pow(clamp(deepPat * 0.5463, 0.0, 1.0), 1.1680);
    ...
    rgb += hotStop * 0.3157 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0722 + 0.2993 * presence + 0.3321 * pattern, 0.8229);
