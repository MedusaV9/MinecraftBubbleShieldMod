...
void main() {
    ...
    mid = clamp(mid * (0.6677 + 0.5810 * texDetail) + 0.2476 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5388), 0.0, 1.0);
    float emit = atlas.a * (0.7945 + 0.4555 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1690 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.6624 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3371 + 0.3793 * presence + 0.5199 * pattern + 0.0817 * (1.0 - deepTrans) + 0.2174 * emit, 0.8444);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2879), 0.8444);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6662 + 0.3338 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0790), 0.0, 1.0);
