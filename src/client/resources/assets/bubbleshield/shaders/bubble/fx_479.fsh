...
void main() {
    ...
    mid = clamp(mid * (0.6620 + 0.5193 * texDetail) + 0.3119 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4816), 0.0, 1.0);
    float emit = atlas.a * (0.5618 + 0.3203 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0929 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.9388 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3206 + 0.2883 * presence + 0.4637 * pattern + 0.1396 * (1.0 - deepTrans) + 0.2091 * emit, 0.8938);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2266), 0.8938);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7391 + 0.2609 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
