...
void main() {
    ...
    mid = clamp(mid * (0.7040 + 0.6829 * texDetail) + 0.3110 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3976), 0.0, 1.0);
    float emit = atlas.a * (0.7553 + 0.3403 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1096 * (0.5 + 0.5 * sin(time * 0.225147)));
    rgb += emit * 0.9009 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3688 + 0.3798 * presence + 0.4991 * pattern + 0.0999 * (1.0 - deepTrans) + 0.1850 * emit, 0.8616);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2891), 0.8616);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0940), 0.0, 1.0);
