...
void main() {
    ...
    mid = clamp(mid * (0.6443 + 0.6064 * texDetail) + 0.3459 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5392), 0.0, 1.0);
    float emit = atlas.a * (0.7179 + 0.3484 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1410 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.8048 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3222 + 0.2641 * presence + 0.4349 * pattern + 0.1184 * (1.0 - deepTrans) + 0.2136 * emit, 0.7980);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1692), 0.7980);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6551 + 0.3449 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1371), 0.0, 1.0);
