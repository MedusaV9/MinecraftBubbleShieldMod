...
void main() {
    ...
    mid = clamp(mid * (0.6188 + 0.6558 * texDetail) + 0.2193 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5013), 0.0, 1.0);
    float emit = atlas.a * (0.7694 + 0.3198 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1660 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.8013 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3952 + 0.3491 * presence + 0.4752 * pattern + 0.0885 * (1.0 - deepTrans) + 0.1777 * emit, 0.8560);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1977), 0.8560);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6899 + 0.3101 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1164), 0.0, 1.0);
