...
void main() {
    ...
    mid = clamp(mid * (0.6782 + 0.7432 * texDetail) + 0.3345 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3677), 0.0, 1.0);
    float emit = atlas.a * (0.5984 + 0.3476 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1411 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.8673 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3840 + 0.3343 * presence + 0.4203 * pattern + 0.1539 * (1.0 - deepTrans) + 0.2172 * emit, 0.8931);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3035), 0.8931);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6642 + 0.3358 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0766), 0.0, 1.0);
