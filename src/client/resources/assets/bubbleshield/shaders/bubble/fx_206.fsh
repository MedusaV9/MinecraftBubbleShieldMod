...
// stack: deep=ridge x3 taps | mid=curlsmoke+warp2+scroll | rim=graze_sparkle | flourish=swirl
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.010000, 0.045000) * time, time);
    deep += 0.4918 * deepField(duvBase + rimDir * 0.0718 + vec2(0.016667, 0.070000) * time, time);
    deep += 0.2419 * deepField(duvBase + rimDir * 0.1435 + vec2(0.021667, 0.092500) * time, time);
    deep = pow(clamp(deep * 0.5768, 0.0, 1.0), 1.3328);
    ...
    vec2 adv = wuv + 0.1070 * curl2(wuv + vec2(0.0, time * 0.064167), midPer);
    float smoke = fbm2(adv + vec2(time * 0.054167, -time * 0.110833), midPer);
    ...
    float pattern = clamp(0.3931 * deep + 0.8375 * mid + 0.6147 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.3473 + pattern * 0.4144);
    vec3 rgb = vertexColor.rgb * (0.5420 + 0.6545 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1624 + 0.7857 * pattern, 0.0, 1.0);
