...
void main() {
    ...
    mid = clamp(mid * (0.6938 + 0.6942 * texDetail) + 0.2280 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3722), 0.0, 1.0);
    float emit = atlas.a * (0.5939 + 0.3333 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0897 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.7109 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3945 + 0.4835 * presence + 0.3314 * pattern + 0.1409 * (1.0 - deepTrans) + 0.2081 * emit, 0.8511);
