...
void main() {
    ...
    mid = clamp(mid * (0.6055 + 0.7280 * texDetail) + 0.2064 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4848), 0.0, 1.0);
    float emit = atlas.a * (0.7547 + 0.4665 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0893 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.7760 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3448 + 0.4115 * presence + 0.4003 * pattern + 0.0926 * (1.0 - deepTrans) + 0.2095 * emit, 0.9039);
