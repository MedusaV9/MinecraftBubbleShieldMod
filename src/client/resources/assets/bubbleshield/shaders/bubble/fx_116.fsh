...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.3778 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.408407);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.033333, -0.030000) * time, time);
    deep += 0.4173 * deepField(duvBase + rimDir * 0.0644 + vec2(0.053333, -0.045000) * time, time);
    deep += 0.1741 * deepField(duvBase + rimDir * 0.1287 + vec2(0.071667, -0.060000) * time, time);
    deep += 0.0727 * deepField(duvBase + rimDir * 0.1931 + vec2(0.090000, -0.077500) * time, time);
    deep = pow(clamp(deep * 0.6009, 0.0, 1.0), 1.1382);
    ...
    float pn = fbm2(wuv + vec2(fbm2(wuv + vec2(time * 0.140000, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.142500), midPer)), midPer);
    ...
    float pattern = clamp(0.4260 * deep + 0.7192 * mid + 0.5127 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7286 + pattern * 0.4938);
    vec3 rgb = vertexColor.rgb * (0.4026 + 0.6432 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2230 + 0.7831 * pattern, 0.0, 1.0);
