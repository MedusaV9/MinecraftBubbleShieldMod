...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1288) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1288);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.035000, -0.030000) * time, time);
    deep += 0.5554 * deepField(duvBase + rimDir * 0.0313 + vec2(0.052500, -0.043333) * time, time);
    deep += 0.3084 * deepField(duvBase + rimDir * 0.0627 + vec2(0.072500, -0.060000) * time, time);
    deep += 0.1713 * deepField(duvBase + rimDir * 0.0940 + vec2(0.090000, -0.076667) * time, time);
    deep = pow(clamp(deep * 0.4914, 0.0, 1.0), 1.7631);
