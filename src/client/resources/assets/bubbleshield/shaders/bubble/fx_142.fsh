...
void main() {
    ...
    mid = clamp(mid * (0.6769 + 0.5096 * texDetail) + 0.2170 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5321), 0.0, 1.0);
    float emit = atlas.a * (0.6167 + 0.3774 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1716 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.8005 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3297 + 0.4293 * presence + 0.4111 * pattern + 0.1023 * (1.0 - deepTrans) + 0.2157 * emit, 0.8808);
