...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6688, 0.0, 1.0);
    ...
    deepCol += 0.5554 * dp * mix(baseCol, deepStop, 0.2741);
    deepPat += 0.5554 * dp;
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * 4.4282) + par * 0.0627, time);
    deepCol += 0.3084 * dp * mix(baseCol, deepStop, 0.5482);
    deepPat += 0.3084 * dp;
    dp = deepField(rotA(spinAxis, time * 0.041888) * (sdir * 5.5969) + par * 0.0940, time);
    deepCol += 0.1713 * dp * mix(baseCol, deepStop, 0.8223);
    deepPat += 0.1713 * dp;
    deepCol *= 0.4884;
    deepPat = pow(clamp(deepPat * 0.4914, 0.0, 1.0), 1.7631);
    ...
    rgb += hotStop * 0.3156 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0406 + 0.2893 * presence + 0.4111 * pattern, 0.8208);
