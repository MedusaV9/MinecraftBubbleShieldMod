...
void main() {
    ...
    mid = clamp(mid * (0.6401 + 0.7207 * texDetail) + 0.3158 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4270), 0.0, 1.0);
    float emit = atlas.a * (0.7161 + 0.2826 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0918 * (0.5 + 0.5 * sin(time * 0.162316)));
    ...
    float alpha = vertexColor.a * min(0.2748 + 0.3148 * presence + 0.5657 * pattern + 0.1172 * (1.0 - deepTrans) + 0.1389 * emit, 0.8784);
