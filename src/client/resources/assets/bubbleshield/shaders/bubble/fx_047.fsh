...
// stack: deep=fbm x3 taps | mid=rings+none+flicker | rim=lat | flourish=glint | tex=ridged
// fbm: turb x4 octaves | seed a9ffd82455f00d8c
// v8 realism: refr 0.7407+0.7093*fres/dist (cap 0.1515) | energy 0.3358+0.4028*p | flow 0.098333 c/s | soft 0.6202m
...
void main() {
    ...
    float refrAmp = min((0.7407 + 0.7093 * fresnel) / max(length(worldPos), 1.0), 0.1515);
    ...
    float emit = atlas.a * (0.3713 + 0.6050 * emitDrive) * (1.0 - 0.0872 * (0.5 + 0.5 * sin(time * 0.235619))) * atlasPoleW;
