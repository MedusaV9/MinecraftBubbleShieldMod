...
void main() {
    ...
    mid = clamp(mid * (0.7124 + 0.7025 * texDetail) + 0.2578 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4291), 0.0, 1.0);
    float emit = atlas.a * (0.5950 + 0.4433 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0906 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.8940 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2705 + 0.2934 * presence + 0.5608 * pattern + 0.0999 * (1.0 - deepTrans) + 0.1341 * emit, 0.8897);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1760), 0.8897);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7492 + 0.2508 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1345), 0.0, 1.0);
