...
void main() {
    ...
    mid = clamp(mid * (0.6321 + 0.6218 * texDetail) + 0.2532 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5232), 0.0, 1.0);
    float emit = atlas.a * (0.7731 + 0.3971 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1243 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.6996 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3099 + 0.4184 * presence + 0.3613 * pattern + 0.1036 * (1.0 - deepTrans) + 0.1211 * emit, 0.9050);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2184), 0.9050);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6739 + 0.3261 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1205), 0.0, 1.0);
