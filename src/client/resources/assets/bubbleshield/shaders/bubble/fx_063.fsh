...
// stack: deep=ridge x3 taps | mid=rings+warp1+scroll | rim=graze | flourish=shimmer | tex=ridged
// fbm: standard x6 octaves | seed df3b349e63362899
// v8 realism: refr 0.6671+0.6200*fres/dist (cap 0.1050) | energy 0.3848+0.4423*p | flow 0.100833 c/s | soft 0.5401m
...
void main() {
    ...
    float refrAmp = min((0.6671 + 0.6200 * fresnel) / max(length(worldPos), 1.0), 0.1050);
    ...
    float emit = atlas.a * (0.3627 + 0.4542 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1632 * (0.5 + 0.5 * sin(time * 0.136136))) * atlasPoleW;
