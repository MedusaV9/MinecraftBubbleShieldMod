...
void main() {
    ...
    mid = clamp(mid * (0.6531 + 0.7332 * texDetail) + 0.3306 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3732), 0.0, 1.0);
    float emit = atlas.a * (0.6545 + 0.2535 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1632 * (0.5 + 0.5 * sin(time * 0.136136)));
    rgb += emit * 0.7884 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3762 + 0.4079 * presence + 0.4101 * pattern + 0.0848 * (1.0 - deepTrans) + 0.1751 * emit, 0.8508);
