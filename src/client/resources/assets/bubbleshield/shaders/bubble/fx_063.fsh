...
// stack: deep=ridge x2 taps | mid=rings+warp1+scroll | rim=graze | flourish=shimmer
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.005000, -0.046667) * time, time);
    deep += 0.4275 * deepField(duvBase + rimDir * 0.0547 + vec2(-0.007500, -0.071667) * time, time);
    deep = pow(clamp(deep * 0.7005, 0.0, 1.0), 1.2363);
    ...
    float pattern = clamp(0.3214 * deep + 0.8199 * mid + 0.8146 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.3678 + pattern * 0.6762);
    vec3 rgb = vertexColor.rgb * (0.4571 + 0.5935 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1038 + 0.7652 * pattern, 0.0, 1.0);
