...
void main() {
    ...
    mid = clamp(mid * (0.6667 + 0.5844 * texDetail) + 0.2270 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4753), 0.0, 1.0);
    float emit = atlas.a * (0.7061 + 0.4841 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1523 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.8362 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3011 + 0.2252 * presence + 0.6201 * pattern + 0.1473 * (1.0 - deepTrans) + 0.1548 * emit, 0.9098);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1604), 0.9098);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7039 + 0.2961 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1001), 0.0, 1.0);
