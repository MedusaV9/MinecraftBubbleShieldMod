...
void main() {
    ...
    mid = clamp(mid * (0.6427 + 0.7115 * texDetail) + 0.3244 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4357), 0.0, 1.0);
    float emit = atlas.a * (0.6637 + 0.4700 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1216 * (0.5 + 0.5 * sin(time * 0.115192)));
    rgb += emit * 0.7367 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4313 + 0.4927 * presence + 0.4075 * pattern + 0.1324 * (1.0 - deepTrans) + 0.1971 * emit, 0.8424);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1551), 0.8424);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7704 + 0.2296 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
