...
// Bubble surface shader fx_456 -- family OILSLICK
// stack: deep=ridge x3 taps | mid=oilslick+none+scroll | rim=graze_film | flourish=swirl | tex=foam | motif=sigilx2+breath
// fbm: standard x6 octaves | seed c8e3d59b83bd7d76
// v8 realism: refr 1.8854+2.5239*fres/dist (cap 0.1454) | energy 0.1905+0.4541*p | flow 0.112500 c/s | soft 0.6902m
...
vec3 accentPalette(float t) {
    return vec3(0.5) + vec3(0.5) * cos(6.2831853 * (vec3(0.4236, 0.4331, 0.7860) * t
        + vec3(0.7130, 0.7029, 0.4620)));
}
...
vec3 gradient3(vec3 deepStop, vec3 body, vec3 hotStop, float x) {
    vec3 g = mix(deepStop, body, smoothstep(0.0, 0.5620, x));
    return mix(g, hotStop, smoothstep(0.5620, 1.0, x));
}
...
float rimGraze() {
    ...
    return smoothstep(0.0060, 0.0372, g);
}
...
float deepField(vec3 p, float t) {
    float n = fbm3(p);
    return pow(1.0 - abs(2.0 * n - 1.0), 2.5928);
}
...
void main() {
    ...
    vec3 deepStop = baseCol * baseCol * 0.6254;
    vec3 spun = clamp(hueSpin(baseCol, 0.0475), 0.0, 1.0);
    vec3 hotStop = satLift(spun, 1.2149) * 1.45;
    float hotLuma = dot(hotStop, vec3(0.299, 0.587, 0.114));
    float lumaCap = max(0.4803, dot(baseCol, vec3(0.299, 0.587, 0.114)) * 1.8445);
    ...
    vec3 spinAxis = vec3(-0.3946, -0.8874, -0.2383);
    vec3 par = Vt * 2.3084;
    ...
    float deepBreath = 1.0 + 0.0114 * sin(time * 0.973894);
    float deepTrans = 1.0;
    vec3 deepCol = vec3(0.0);
    float dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * (1.9883 * deepBreath)), time);
    deepCol += deepTrans * dp * baseCol;
    deepTrans *= 1.0 - 0.5746 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.052360) * (sdir * (2.8843 * deepBreath)) + par * 0.0307, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 0.5751), deepStop, 0.2109);
    deepTrans *= 1.0 - 0.5746 * clamp(dp, 0.0, 1.0);
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * (3.7802 * deepBreath)) + par * 0.0614, time);
    deepCol += deepTrans * dp * mix(mix(baseCol, secCol, 1.0000), deepStop, 0.4217);
    deepTrans *= 1.0 - 0.5746 * clamp(dp, 0.0, 1.0);
    deepCol *= 0.9922;
    float deepPat = pow(clamp((1.0 - deepTrans) * 1.0834, 0.0, 1.0), 1.3759);

    // [layer:mid:oilslick_none_scroll]
    ...
    vec2 texUV = wuv * 2.0000 + vec2(-0.047500, 0.041667) * time;
    ...
    vec3 hgtW = vec3(0.4195, 0.3753, 0.2052);
    ...
    vec2 flowVec = vec2(atlasSlope.y, -atlasSlope.x) * -3.3970;
    ...
    float texDetail = clamp(atlas.r * 0.4180 + atlas.g * 0.3741 + atlas.b * 0.2045, 0.0, 1.0);
    ...
    mid = clamp(mid * mix(1.0, 0.6427 + 0.7115 * texDetail, atlasPoleW) + 0.1849 * texDetail * (0.3119 + 0.6881 * clamp(mid, 0.0, 1.0)) * atlasPoleW, 0.0, 1.5);
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (2.2339 * atlasPoleW));
    ...
    float fresnel = pow(1.0 - abs(dot(bumpN, viewV)), 2.8823);
    ...
    vec3 deepTexA = atlasTile(texUV + VtUV * 0.0757).rgb;
    vec3 deepTexB = atlasTile(texUV - VtUV * 0.0757).rgb;
    float deepTex = 0.5 * (dot(deepTexA, hgtW) + dot(deepTexB, hgtW));

    // [layer:thick:chord]
    // v11 volumetric thickness: the membrane is a shell of relative
    // thickness rho = 0.0931 (crystalline material group) on the unit
    ...
    // 2*sqrt(1 - (1-rho)^2) = 0.8428.
    float cosV = abs(dot(sdir, viewV));
    float chordDisc = 0.9069 * 0.9069 - (1.0 - cosV * cosV);
    float chord = chordDisc > 0.0 ? cosV - sqrt(chordDisc) : min(2.0 * cosV, 0.8428);
    float chordN = min(chord / 0.0931, 3.0);
    ...
    float rimLine = smoothstep(0.0650, 0.0896, rimSlope);
    float rim = rimGraze() * 0.8239;
    // v8: the fresnel term folds into the rim glow -- edges glow, the
    // center stays refraction-dominated (see the [layer:refract] mix).
    rim = clamp(rim + 0.8134 * rimLine + 0.7820 * fresnel, 0.0, 1.4);

    // Flourish accent + micro grain keep large areas alive up close.
    float flourish = 0.2586 * pow(clamp(fbm2(wuv + vec2(-time * 0.145833, time * 0.067500), midPer), 0.0, 1.0), 2.0);
    float grain = 0.0975 * (cellHash(floor(wuv * 43.0000) + vec2(floor(time * 2.0), 0.0), 301.0000) - 0.5);
    ...
    float motifEnv = 0.72 + 0.28 * sin(time * 0.863938 + 1.3455);
    float motifA = baseUV.x * 5.0000 + -1.0 * time * 0.075000 + 0.2043;
    float motifSig = pow(0.5 + 0.5 * cos(6.2831853 * motifA), 14.5559);
    float motifDy = (baseUV.y - 0.5529) * 12.4639;
    ...
    float v5Par = deepField(sdir * 1.5863 + par * 0.0663, time) * 0.5;
    v5Par += deepField(sdir * 2.1415 + par * 0.1325, time) * 0.3;
    v5Par += deepField(sdir * 2.7760 + par * 0.1988, time) * 0.2;
    float pattern = clamp(0.4715 * deepPat * mix(1.0, chordN, 0.1886) + 0.9653 * mid + 0.7720 * rim + flourish + grain + 0.2079 * v5Par + 0.1291 * deepTex, 0.0, 1.5);
    float gpos = clamp(pattern * 0.7753 + rim * 0.1851, 0.0, 1.0);
    vec3 rgb = gradient3(deepStop, baseCol, hotStop, gpos);
    float midCover = clamp(0.9411 * mid + 0.4586 * rim, 0.0, 1.0);
    ...
    float texBody = 0.7879 + 0.5184 * texDetail;
    rgb *= mix(1.0, texBody, atlasPoleW * (0.5116 + 0.4884 * clamp(pattern, 0.0, 1.0)));
    rgb = satLift(rgb, 1.4374);
    ...
    vec3 rimDisp = 0.5 + 0.5 * cos(vec3(1.0, 0.8065, 0.6452) * (rim * 0.9032 + baseUV.y * 0.6575 + 0.1784) * 6.2831853);
    rgb = mix(rgb, rgb * (0.72 + 0.56 * rimDisp), clamp(rim, 0.0, 1.0) * 0.1564);
    vec3 lineDisp = 0.5 + 0.5 * cos(vec3(0.6452, 0.8065, 1.0) * (rimLine * 0.8776 + baseUV.y * 0.31 + 0.5934) * 6.2831853);
    rgb = mix(rgb, hotStop * (0.62 + 0.50 * lineDisp), clamp(rimLine, 0.0, 1.0) * 0.2819);
    ...
    rgb *= (1.0 - 0.2902 * washHi) * (1.0 - 0.2713 * washHi * (1.0 - clamp(pattern, 0.0, 1.0)));
    // [layer:light:wrapkey]
    ...
    vec3 keyL = vec3(-0.3609, 0.6859, -0.6320);
    float keyDiff = clamp((dot(bumpN, keyL) + 0.5936) / 1.5936, 0.0, 1.0);
    rgb *= 0.6046 + 0.5264 * keyDiff * (1.0 - 0.5109 * washHi);
    float keySpec = pow(clamp(dot(reflect(-viewV, bumpN), keyL), 0.0, 1.0), 24.0590);
    rgb += hotStop * (keySpec * 0.6339 * (1.0 - 0.5554 * washHi));
    ...
    float refrAmp = min((1.8854 + 2.5239 * fresnel) / max(length(worldPos), 1.0), 0.1454);
    ...
    vec2 refrOff = (viewN.xy + atlasSlope * 3.4281) * refrAmp;
    vec2 refrTap = clamp(screenUV + refrOff, vec2(0.001), vec2(0.999));
    if (texture(Sampler2, refrTap).r > gl_FragCoord.z) {
        refrOff = vec2(0.0);
    }
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0937, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9063, vec2(0.001), vec2(0.999))).b);
    ...
    refracted *= exp(-0.4531 * chordN * (1.0 - baseCol));
    ...
    refracted *= mix(vec3(1.0), baseCol, 0.1830);
    float energyW = clamp(0.1905 + 0.4541 * clamp(pattern, 0.0, 1.0) + 0.3049 * fresnel, 0.0, 0.8306);
    ...
    rgb += fresnel * hotStop * (1.0778 * (1.0 - 0.7221 * washHi));
    ...
    rgb += motifGlow * mix(hotStop, secCol, 0.4095) * 1.1246;
    // [layer:emit:atlas_a]
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3200 * max(0.0, 1.0 - 0.4656 * baseLuma - 0.5385 * washHi)), 0.0, 1.0);
    float emit = atlas.a * (0.3682 + 0.7140 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1216 * (0.5 + 0.5 * sin(time * 0.115192))) * atlasPoleW;
    rgb += emit * 0.7367 * (1.0 - 0.7928 * baseLuma) * (1.0 - 0.5293 * washHi) * emitCol;
    // [layer:v5:softknee]
    ...
    vec3 v5Over = max(rgb - vec3(0.7777), vec3(0.0));
    rgb = min(rgb, vec3(0.7777)) + 0.2223 * (1.0 - exp(-v5Over * 1.7783));
    // [layer:wash:knee]
    ...
    float washKnee = 1.0 - 0.2758 * washHi;
    vec3 washOver = max(rgb - vec3(washKnee), vec3(0.0));
    rgb = min(rgb, vec3(washKnee)) + (1.0 - washKnee) * (1.0 - exp(-washOver * 3.3529));
    ...
    float bodyAlpha = 0.4313 + 0.4927 * presence + 0.4075 * pattern + 0.1324 * (1.0 - deepTrans) + 0.1971 * emit;
    // v9: the motif also firms the membrane locally (pre-clamp, pre-
    // dissolve -- vertexColor.a still always wins).
    bodyAlpha += motifGlow * 0.1804;
    // [layer:v5:backface]
    ...
    rgb = mix(rgb, deepStop, v5Back * 0.4095);
    ...
    bodyAlpha *= 0.7704 + 0.2296 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    float innerScaf = clamp(0.5 * (deepTexA.r + deepTexB.r) * 1.2253 + hgtC * 0.4835, 0.0, 1.0);
    vec3 innerCol = mix(deepStop, secCol, innerScaf);
    rgb = mix(rgb, innerCol, innerFace * 0.2618);
    ...
    bodyAlpha *= smoothstep(0.0, 0.6902, sceneVDist - fragVDist);
