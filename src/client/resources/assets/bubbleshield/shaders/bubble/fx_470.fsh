...
void main() {
    ...
    mid = clamp(mid * (0.6654 + 0.7483 * texDetail) + 0.2332 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5450), 0.0, 1.0);
    float emit = atlas.a * (0.6928 + 0.4375 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1769 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.9616 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2600 + 0.2487 * presence + 0.5126 * pattern + 0.1585 * (1.0 - deepTrans) + 0.1249 * emit, 0.8488);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2941), 0.8488);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6635 + 0.3365 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
