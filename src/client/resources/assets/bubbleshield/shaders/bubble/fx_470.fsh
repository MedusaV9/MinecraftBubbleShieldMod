...
// stack: deep=caustic x3 taps | mid=phantomecho+curl+pulse | rim=graze_sparkle | flourish=swirl | tex=foam
// fbm: ridged x6 octaves | seed da6bddcf81f9a907
// v8 realism: refr 0.3879+0.4339*fres/dist (cap 0.1073) | energy 0.4007+0.5228*p | flow 0.103333 c/s | soft 1.1543m
...
void main() {
    ...
    vec3 bumpN = normalize(sdir + (tanU * atlasSlope.x + tanV * atlasSlope.y) * (1.0429 * atlasPoleW));
    ...
    float refrAmp = min((0.3879 + 0.4339 * fresnel) / max(length(worldPos), 1.0), 0.1073);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0437, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9563, vec2(0.001), vec2(0.999))).b);
