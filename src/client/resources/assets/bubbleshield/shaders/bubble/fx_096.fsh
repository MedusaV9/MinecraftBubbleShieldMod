...
void main() {
    ...
    deep += 0.4242 * deepField(duvBase + rimDir * 0.0345 + vec2(0.033333, -0.050000) * time, time);
    deep += 0.1799 * deepField(duvBase + rimDir * 0.0690 + vec2(0.045000, -0.067500) * time, time);
    deep += 0.0763 * deepField(duvBase + rimDir * 0.1035 + vec2(0.055000, -0.085000) * time, time);
    deep = pow(clamp(deep * 0.5951, 0.0, 1.0), 1.0783);
