...
void main() {
    ...
    mid = clamp(mid * (0.6685 + 0.5002 * texDetail) + 0.3203 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4077), 0.0, 1.0);
    float emit = atlas.a * (0.7746 + 0.4718 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1479 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.7687 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3325 + 0.4354 * presence + 0.4711 * pattern + 0.1158 * (1.0 - deepTrans) + 0.1293 * emit, 0.8931);
