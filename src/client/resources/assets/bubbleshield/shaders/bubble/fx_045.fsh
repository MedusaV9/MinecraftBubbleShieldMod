...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.2804 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(2.0000, 3.0000), t * 0.376991);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.001667, 0.047500) * time, time);
    deep += 0.4809 * deepField(duvBase + rimDir * 0.0566 + vec2(0.001667, 0.075000) * time, time);
    deep += 0.2312 * deepField(duvBase + rimDir * 0.1132 + vec2(0.003333, 0.102500) * time, time);
    deep += 0.1112 * deepField(duvBase + rimDir * 0.1698 + vec2(0.003333, 0.127500) * time, time);
    deep = pow(clamp(deep * 0.5485, 0.0, 1.0), 1.0972);
    ...
    float pattern = clamp(0.3368 * deep + 0.9642 * mid + 0.8964 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9624 + pattern * 0.6012);
    vec3 rgb = vertexColor.rgb * (0.4107 + 0.8020 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2190 + 0.7644 * pattern, 0.0, 1.0);
