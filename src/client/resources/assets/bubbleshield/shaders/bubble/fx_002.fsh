...
void main() {
    ...
    mid = clamp(mid * (0.7192 + 0.5147 * texDetail) + 0.1869 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3798), 0.0, 1.0);
    float emit = atlas.a * (0.5886 + 0.3079 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1164 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.7673 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3575 + 0.4933 * presence + 0.4055 * pattern + 0.1545 * (1.0 - deepTrans) + 0.1919 * emit, 0.8743);
