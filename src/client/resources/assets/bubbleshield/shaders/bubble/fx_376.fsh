...
void main() {
    ...
    mid = clamp(mid * (0.7430 + 0.6067 * texDetail) + 0.1987 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5220), 0.0, 1.0);
    float emit = atlas.a * (0.7115 + 0.3756 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1639 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.9114 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3640 + 0.4302 * presence + 0.4685 * pattern + 0.1109 * (1.0 - deepTrans) + 0.2159 * emit, 0.8650);
