...
void main() {
    ...
    mid = clamp(mid * (0.7210 + 0.7210 * texDetail) + 0.3284 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5471), 0.0, 1.0);
    float emit = atlas.a * (0.6999 + 0.3134 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1312 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.8807 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3150 + 0.3973 * presence + 0.4321 * pattern + 0.1513 * (1.0 - deepTrans) + 0.1671 * emit, 0.8970);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2608), 0.8970);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7022 + 0.2978 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1347), 0.0, 1.0);
