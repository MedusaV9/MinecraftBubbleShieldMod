...
void main() {
    ...
    mid = clamp(mid * (0.6554 + 0.6230 * texDetail) + 0.2451 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4185), 0.0, 1.0);
    float emit = atlas.a * (0.6056 + 0.2501 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0874 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.8803 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3586 + 0.4525 * presence + 0.3548 * pattern + 0.1078 * (1.0 - deepTrans) + 0.2188 * emit, 0.8671);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2347), 0.8671);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6836 + 0.3164 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0957), 0.0, 1.0);
