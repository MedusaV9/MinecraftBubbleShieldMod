...
// stack: deep=fbm x4 taps | mid=raymarchfog+warp2+scroll | rim=graze | flourish=glint
...
void main() {
    ...
    float mid = clamp(rfLight * 1.2307 + (1.0 - rfTrans) * 0.1941, 0.0, 1.15);
    ...
    float flourish = 0.1660 * sparkle(wuv * 2.0 + 7.7, time * 1.4, midPer.x * 2.0);
    ...
    float alpha = vertexColor.a * min(0.0551 + 0.3925 * presence + 0.3548 * pattern + 0.1078 * (1.0 - deepTrans), 0.8071);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2347), 0.8071);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4476 + 0.5524 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
