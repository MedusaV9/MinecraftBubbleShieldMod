...
void main() {
    ...
    mid = clamp(mid * (0.7333 + 0.6006 * texDetail) + 0.2812 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7153 + 0.4282 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1543 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.9536 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2672 + 0.2455 * presence + 0.4527 * pattern + 0.1546 * (1.0 - deepTrans) + 0.1548 * emit, 0.8616);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2283), 0.8616);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7247 + 0.2753 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1130), 0.0, 1.0);
