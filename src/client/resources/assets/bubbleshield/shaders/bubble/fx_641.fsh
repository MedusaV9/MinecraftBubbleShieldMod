...
void main() {
    ...
    mid = clamp(mid * (0.7166 + 0.5243 * texDetail) + 0.2860 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3823), 0.0, 1.0);
    float emit = atlas.a * (0.7917 + 0.3186 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1433 * (0.5 + 0.5 * sin(time * 0.183260)));
    ...
    float alpha = vertexColor.a * min(0.3307 + 0.4332 * presence + 0.2995 * pattern + 0.0984 * (1.0 - deepTrans) + 0.1335 * emit, 0.8973);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2348), 0.8973);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7357 + 0.2643 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
