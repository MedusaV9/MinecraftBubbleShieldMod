...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8522, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0512, abs(glR - 0.1593));
    float mid = clamp(glStars * (1.0 + glRing * 1.3060) + glRing * 0.4719, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.7165 + 0.5302 * texDetail) + 0.1893 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8290 + 0.4180 * texDetail;
    rgb = satLift(rgb, 1.2322);
    ...
    float emit = atlas.a * (0.6224 + 0.4039 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1194 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.9316 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2706 + 0.2693 * presence + 0.5854 * pattern + 0.1258 * (1.0 - deepTrans) + 0.1600 * emit, 0.8756);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1633), 0.8756);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1246), 0.0, 1.0);
