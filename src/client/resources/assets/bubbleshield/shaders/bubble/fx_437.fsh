...
void main() {
    ...
    mid = clamp(mid * (0.6630 + 0.5732 * texDetail) + 0.1813 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3746), 0.0, 1.0);
    float emit = atlas.a * (0.7015 + 0.2762 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1162 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.7613 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3061 + 0.2295 * presence + 0.5972 * pattern + 0.1029 * (1.0 - deepTrans) + 0.1901 * emit, 0.9134);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2180), 0.9134);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7312 + 0.2688 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1302), 0.0, 1.0);
