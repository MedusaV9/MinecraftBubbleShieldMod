...
void main() {
    ...
    mid = clamp(mid * (0.6703 + 0.6433 * texDetail) + 0.1899 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4653), 0.0, 1.0);
    float emit = atlas.a * (0.6961 + 0.2870 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1059 * (0.5 + 0.5 * sin(time * 0.214675)));
    ...
    float alpha = vertexColor.a * min(0.4316 + 0.3820 * presence + 0.3617 * pattern + 0.1056 * (1.0 - deepTrans) + 0.1772 * emit, 0.9072);
