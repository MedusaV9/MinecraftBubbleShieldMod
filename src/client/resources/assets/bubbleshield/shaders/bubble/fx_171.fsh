...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.016667, -0.012500) * time, time);
    deep += 0.4785 * deepField(duvBase + rimDir * 0.0613 + vec2(-0.025000, -0.020000) * time, time);
    deep += 0.2289 * deepField(duvBase + rimDir * 0.1227 + vec2(-0.033333, -0.025000) * time, time);
    deep = pow(clamp(deep * 0.5857, 0.0, 1.0), 1.4674);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.449167), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 11.1992);
    float flash = 0.6 + 0.4 * sin(time * 4.309218 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
