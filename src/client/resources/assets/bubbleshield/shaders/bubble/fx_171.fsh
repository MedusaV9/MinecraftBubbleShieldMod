...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6342, 0.0, 1.0);
    ...
    deepCol += 0.4785 * dp * mix(baseCol, deepStop, 0.1924);
    deepPat += 0.4785 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 5.1524) + par * 0.1227, time);
    deepCol += 0.2289 * dp * mix(baseCol, deepStop, 0.3848);
    deepPat += 0.2289 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 6.3132) + par * 0.1840, time);
    deepCol += 0.1095 * dp * mix(baseCol, deepStop, 0.5773);
    deepPat += 0.1095 * dp;
    deepCol *= 0.6561;
    deepPat = pow(clamp(deepPat * 0.5504, 0.0, 1.0), 1.4674);
    ...
    rgb += hotStop * 0.1962 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0750 + 0.2420 * presence + 0.3617 * pattern, 0.8472);
