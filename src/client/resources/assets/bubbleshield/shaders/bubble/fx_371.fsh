...
void main() {
    ...
    float mid = clamp((lonLine + latLine) * 0.7368 * holoFlicker + scan * 0.5241 + glint * 0.9, 0.0, 1.25);
    ...
    mid = clamp(mid * (0.6480 + 0.6268 * texDetail) + 0.2973 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4996), 0.0, 1.0);
    float emit = atlas.a * (0.7379 + 0.4366 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1449 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.9964 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2815 + 0.2466 * presence + 0.5161 * pattern + 0.0978 * (1.0 - deepTrans) + 0.1590 * emit, 0.8943);
