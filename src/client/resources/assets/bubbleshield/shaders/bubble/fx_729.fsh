...
void main() {
    ...
    mid = clamp(mid * (0.7250 + 0.7334 * texDetail) + 0.3403 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.5889 + 0.3824 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1366 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.9117 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4665 + 0.4572 * presence + 0.4110 * pattern + 0.0979 * (1.0 - deepTrans) + 0.1705 * emit, 0.9175);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3233), 0.9175);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6638 + 0.3362 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0640), 0.0, 1.0);
