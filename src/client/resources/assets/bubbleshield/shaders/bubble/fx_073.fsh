...
void main() {
    ...
    deep += 0.5540 * deepField(duvBase + rimDir * 0.0749 + vec2(-0.050000, -0.042500) * time, time);
    deep += 0.3069 * deepField(duvBase + rimDir * 0.1497 + vec2(-0.066667, -0.057500) * time, time);
    deep += 0.1700 * deepField(duvBase + rimDir * 0.2246 + vec2(-0.085000, -0.075000) * time, time);
    deep = pow(clamp(deep * 0.4924, 0.0, 1.0), 1.2469);
    ...
    float pattern = clamp(0.3088 * deep + 0.9199 * mid + 0.6030 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.8170 + pattern * 0.4621);
    vec3 rgb = vertexColor.rgb * (0.4475 + 0.6434 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1952 + 0.6564 * pattern, 0.0, 1.0);
