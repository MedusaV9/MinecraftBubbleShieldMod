...
void main() {
    ...
    mid = clamp(mid * (0.6544 + 0.6909 * texDetail) + 0.1904 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4961), 0.0, 1.0);
    float emit = atlas.a * (0.6382 + 0.4581 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0994 * (0.5 + 0.5 * sin(time * 0.198968)));
    rgb += emit * 0.9361 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2912 + 0.2907 * presence + 0.5631 * pattern + 0.1280 * (1.0 - deepTrans) + 0.2041 * emit, 0.8746);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1909), 0.8746);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6450 + 0.3550 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
