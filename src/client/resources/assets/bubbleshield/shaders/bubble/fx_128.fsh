...
void main() {
    ...
    mid = clamp(mid * (0.6144 + 0.7230 * texDetail) + 0.1882 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3766), 0.0, 1.0);
    float emit = atlas.a * (0.7620 + 0.2707 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1709 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.7476 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3700 + 0.4291 * presence + 0.4614 * pattern + 0.1366 * (1.0 - deepTrans) + 0.1445 * emit, 0.8488);
