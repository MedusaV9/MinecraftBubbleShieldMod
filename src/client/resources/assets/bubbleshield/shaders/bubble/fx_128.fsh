...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.025000, 0.000000) * time, time);
    deep += 0.4163 * deepField(duvBase + rimDir * 0.0545 + vec2(0.038333, 0.000000) * time, time);
    deep = pow(clamp(deep * 0.7061, 0.0, 1.0), 1.0145);
    ...
    float rn = fbm2(wuv + vec2(time * 0.066667, time * 0.046667), midPer);
