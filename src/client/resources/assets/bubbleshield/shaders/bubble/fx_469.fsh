...
void main() {
    ...
    mid = clamp(mid * (0.7118 + 0.4546 * texDetail) + 0.2921 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5157), 0.0, 1.0);
    float emit = atlas.a * (0.7090 + 0.4948 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1205 * (0.5 + 0.5 * sin(time * 0.204204)));
    ...
    float alpha = vertexColor.a * min(0.3779 + 0.4642 * presence + 0.4400 * pattern + 0.0942 * (1.0 - deepTrans) + 0.2023 * emit, 0.9155);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1895), 0.9155);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7062 + 0.2938 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1194), 0.0, 1.0);
