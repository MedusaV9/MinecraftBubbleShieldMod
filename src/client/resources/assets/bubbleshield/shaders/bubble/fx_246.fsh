...
void main() {
    ...
    mid = clamp(mid * (0.6010 + 0.6937 * texDetail) + 0.2776 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3834), 0.0, 1.0);
    float emit = atlas.a * (0.7064 + 0.3780 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0844 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.6607 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4202 + 0.3915 * presence + 0.3437 * pattern + 0.1011 * (1.0 - deepTrans) + 0.1218 * emit, 0.8765);
