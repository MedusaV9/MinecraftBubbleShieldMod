...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.018333, -0.045000) * time, time);
    deep += 0.5200 * deepField(duvBase + rimDir * 0.0746 + vec2(0.027500, -0.070000) * time, time);
    deep += 0.2704 * deepField(duvBase + rimDir * 0.1493 + vec2(0.037500, -0.095000) * time, time);
    deep += 0.1406 * deepField(duvBase + rimDir * 0.2239 + vec2(0.047500, -0.120000) * time, time);
    deep = pow(clamp(deep * 0.5178, 0.0, 1.0), 1.3508);
    ...
    float pattern = clamp(0.4389 * deep + 0.7968 * mid + 0.8776 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5180 + pattern * 0.4143);
    vec3 rgb = vertexColor.rgb * (0.3999 + 0.6672 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1121 + 0.7282 * pattern, 0.0, 1.0);
