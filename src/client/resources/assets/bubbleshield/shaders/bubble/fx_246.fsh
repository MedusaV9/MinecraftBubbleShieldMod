...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7196, 0.0, 1.0);
    ...
    deepCol += 0.5200 * dp * mix(baseCol, deepStop, 0.2615);
    deepPat += 0.5200 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 3.3664) + par * 0.1493, time);
    deepCol += 0.2704 * dp * mix(baseCol, deepStop, 0.5231);
    deepPat += 0.2704 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 4.1631) + par * 0.2239, time);
    deepCol += 0.1406 * dp * mix(baseCol, deepStop, 0.7846);
    deepPat += 0.1406 * dp;
    deepCol *= 0.5893;
    deepPat = pow(clamp(deepPat * 0.5178, 0.0, 1.0), 1.3508);
    ...
    rgb += hotStop * 0.3120 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0711 + 0.2515 * presence + 0.3437 * pattern, 0.8165);
