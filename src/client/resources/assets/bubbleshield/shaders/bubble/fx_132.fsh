...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.005000, 0.002500) * time, time);
    deep += 0.4573 * deepField(duvBase + rimDir * 0.0562 + vec2(0.006667, 0.002500) * time, time);
    deep += 0.2091 * deepField(duvBase + rimDir * 0.1125 + vec2(0.008333, 0.005000) * time, time);
    deep = pow(clamp(deep * 0.6001, 0.0, 1.0), 1.5462);
    ...
    float rn = fbm2(wuv + vec2(time * 0.090000, time * 0.045000), midPer);
    ...
    float pattern = clamp(0.4685 * deep + 0.7599 * mid + 0.5195 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.7858 + pattern * 0.6578);
    vec3 rgb = vertexColor.rgb * (0.5097 + 0.8257 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2434 + 0.6407 * pattern, 0.0, 1.0);
