...
void main() {
    ...
    deepCol += 0.4573 * dp * mix(baseCol, deepStop, 0.4422);
    deepPat += 0.4573 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 3.0970) + par * 0.1125, time);
    deepCol += 0.2091 * dp * mix(baseCol, deepStop, 0.8845);
    deepPat += 0.2091 * dp;
    deepCol *= 0.6462;
    deepPat = pow(clamp(deepPat * 0.6001, 0.0, 1.0), 1.5462);
    ...
    rgb += hotStop * 0.3036 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0545 + 0.3293 * presence + 0.3912 * pattern, 0.7817);
