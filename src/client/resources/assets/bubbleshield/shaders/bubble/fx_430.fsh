...
void main() {
    ...
    mid = clamp(mid * (0.6462 + 0.6247 * texDetail) + 0.2934 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7514 + 0.4024 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1640 * (0.5 + 0.5 * sin(time * 0.109956)));
    rgb += emit * 0.7160 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2950 + 0.2530 * presence + 0.5722 * pattern + 0.1417 * (1.0 - deepTrans) + 0.1927 * emit, 0.8975);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1519), 0.8975);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7598 + 0.2402 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0700), 0.0, 1.0);
