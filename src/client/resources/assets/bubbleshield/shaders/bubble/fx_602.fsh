...
void main() {
    ...
    mid = clamp(mid * (0.6625 + 0.4733 * texDetail) + 0.3264 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4550), 0.0, 1.0);
    float emit = atlas.a * (0.6818 + 0.3119 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1459 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.9973 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3290 + 0.4124 * presence + 0.4288 * pattern + 0.1588 * (1.0 - deepTrans) + 0.1322 * emit, 0.8632);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1616), 0.8632);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6383 + 0.3617 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1388), 0.0, 1.0);
