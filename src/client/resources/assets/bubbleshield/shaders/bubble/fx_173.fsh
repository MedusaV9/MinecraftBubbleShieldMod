...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1342) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1342);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.036667, 0.017500) * time, time);
    deep += 0.4341 * deepField(duvBase + rimDir * 0.0541 + vec2(-0.058333, 0.027500) * time, time);
    deep += 0.1884 * deepField(duvBase + rimDir * 0.1082 + vec2(-0.078333, 0.037500) * time, time);
    deep += 0.0818 * deepField(duvBase + rimDir * 0.1622 + vec2(-0.100000, 0.045000) * time, time);
    deep = pow(clamp(deep * 0.5867, 0.0, 1.0), 1.1052);
