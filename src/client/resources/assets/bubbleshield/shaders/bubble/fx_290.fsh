...
void main() {
    ...
    mid = clamp(mid * (0.7335 + 0.6507 * texDetail) + 0.2330 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3933), 0.0, 1.0);
    float emit = atlas.a * (0.6982 + 0.3535 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1241 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.6823 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3501 + 0.4781 * presence + 0.4446 * pattern + 0.1307 * (1.0 - deepTrans) + 0.1764 * emit, 0.8638);
