...
void main() {
    ...
    mid = clamp(mid * (0.7310 + 0.4890 * texDetail) + 0.2740 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4063), 0.0, 1.0);
    float emit = atlas.a * (0.6310 + 0.2907 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1553 * (0.5 + 0.5 * sin(time * 0.251327)));
    ...
    float alpha = vertexColor.a * min(0.3277 + 0.2415 * presence + 0.6236 * pattern + 0.1236 * (1.0 - deepTrans) + 0.1706 * emit, 0.9259);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2555), 0.9259);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6994 + 0.3006 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
