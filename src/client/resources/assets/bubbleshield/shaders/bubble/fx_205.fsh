...
// stack: deep=ridge x4 taps | mid=circuit+warp1+scroll | rim=graze_sparkle | flourish=shimmer | tex=circuit
// fbm: standard x5 octaves | seed 36a39980879bc545
// v8 realism: refr 0.4955+0.7253*fres/dist (cap 0.1115) | energy 0.3486+0.4416*p | flow 0.063333 c/s | soft 1.0553m
...
void main() {
    ...
    float refrAmp = min((0.4955 + 0.7253 * fresnel) / max(length(worldPos), 1.0), 0.1115);
