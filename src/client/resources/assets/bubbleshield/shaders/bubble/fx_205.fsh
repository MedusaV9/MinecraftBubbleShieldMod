...
void main() {
    ...
    mid = clamp(mid * (0.7050 + 0.6616 * texDetail) + 0.2309 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3929), 0.0, 1.0);
    float emit = atlas.a * (0.7594 + 0.3472 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1224 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.7041 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4308 + 0.3989 * presence + 0.4952 * pattern + 0.1363 * (1.0 - deepTrans) + 0.1954 * emit, 0.9172);
