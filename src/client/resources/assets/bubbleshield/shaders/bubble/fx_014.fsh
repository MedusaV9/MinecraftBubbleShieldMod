...
void main() {
    ...
    mid = clamp(mid * (0.6392 + 0.5786 * texDetail) + 0.3415 * texDetail, 0.0, 1.5);
    ...
    rim = clamp(rim + 0.6469 * rimLine, 0.0, 1.4);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3672), 0.0, 1.0);
    float emit = atlas.a * (0.7711 + 0.3054 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1758 * (0.5 + 0.5 * sin(time * 0.267035)));
    rgb += emit * 0.7325 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4224 + 0.4334 * presence + 0.3983 * pattern + 0.0845 * (1.0 - deepTrans) + 0.1210 * emit, 0.8590);
