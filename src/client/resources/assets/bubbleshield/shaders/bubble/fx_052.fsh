...
// v8 realism: refr 0.4792+0.7169*fres/dist (cap 0.1116) | energy 0.3492+0.4479*p | flow 0.076667 c/s | soft 0.8536m
...
void main() {
    ...
    float refrAmp = min((0.4792 + 0.7169 * fresnel) / max(length(worldPos), 1.0), 0.1116);
    ...
    float emit = atlas.a * (0.4165 + 0.6945 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1699 * (0.5 + 0.5 * sin(time * 0.230383))) * atlasPoleW;
