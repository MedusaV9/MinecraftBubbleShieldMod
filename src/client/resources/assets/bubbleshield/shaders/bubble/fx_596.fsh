...
void main() {
    ...
    mid = clamp(mid * (0.6925 + 0.6373 * texDetail) + 0.3119 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5372), 0.0, 1.0);
    float emit = atlas.a * (0.6935 + 0.2917 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1534 * (0.5 + 0.5 * sin(time * 0.287979)));
    rgb += emit * 0.8646 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3923 + 0.4292 * presence + 0.4582 * pattern + 0.0904 * (1.0 - deepTrans) + 0.1492 * emit, 0.9032);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2671), 0.9032);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6610 + 0.3390 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0649), 0.0, 1.0);
