...
void main() {
    ...
    mid = clamp(mid * (0.6055 + 0.5469 * texDetail) + 0.1864 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5105), 0.0, 1.0);
    float emit = atlas.a * (0.7507 + 0.4653 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1639 * (0.5 + 0.5 * sin(time * 0.193732)));
    rgb += emit * 0.8296 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3053 + 0.3128 * presence + 0.5900 * pattern + 0.1300 * (1.0 - deepTrans) + 0.1625 * emit, 0.8547);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2683), 0.8547);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6287 + 0.3713 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
