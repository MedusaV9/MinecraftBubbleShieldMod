...
void main() {
    ...
    mid = clamp(mid * (0.6140 + 0.4753 * texDetail) + 0.3455 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3593), 0.0, 1.0);
    float emit = atlas.a * (0.5644 + 0.4179 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0881 * (0.5 + 0.5 * sin(time * 0.251327)));
    rgb += emit * 0.6793 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3405 + 0.3110 * presence + 0.3887 * pattern + 0.0946 * (1.0 - deepTrans) + 0.2117 * emit, 0.7893);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2580), 0.7893);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6960 + 0.3040 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1399), 0.0, 1.0);
