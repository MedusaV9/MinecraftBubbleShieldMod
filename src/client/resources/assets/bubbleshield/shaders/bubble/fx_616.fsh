...
void main() {
    ...
    mid = clamp(mid * (0.6870 + 0.7361 * texDetail) + 0.2996 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5134), 0.0, 1.0);
    float emit = atlas.a * (0.6914 + 0.4704 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1357 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.9137 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4028 + 0.4237 * presence + 0.3321 * pattern + 0.0890 * (1.0 - deepTrans) + 0.1696 * emit, 0.9113);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1649), 0.9113);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1358), 0.0, 1.0);
