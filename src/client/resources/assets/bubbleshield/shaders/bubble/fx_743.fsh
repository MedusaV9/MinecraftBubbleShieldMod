...
void main() {
    ...
    mid = clamp(mid * (0.6112 + 0.6089 * texDetail) + 0.2375 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5036), 0.0, 1.0);
    float emit = atlas.a * (0.6434 + 0.3235 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1687 * (0.5 + 0.5 * sin(time * 0.256563)));
    rgb += emit * 0.9900 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3263 + 0.2982 * presence + 0.6072 * pattern + 0.1087 * (1.0 - deepTrans) + 0.1931 * emit, 0.9130);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1623), 0.9130);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6512 + 0.3488 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1373), 0.0, 1.0);
