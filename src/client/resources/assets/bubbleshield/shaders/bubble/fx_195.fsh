...
// stack: deep=ridge x3 taps | mid=aurora+none+pulse | rim=graze_sparkle | flourish=echo | tex=filaments
// fbm: turb x4 octaves | seed 42a3bef5a492873d
// v8 realism: refr 0.4818+0.5084*fres/dist (cap 0.1380) | energy 0.3753+0.5264*p | flow 0.100833 c/s | soft 0.8165m
...
void main() {
    ...
    float refrAmp = min((0.4818 + 0.5084 * fresnel) / max(length(worldPos), 1.0), 0.1380);
