...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8032, 0.0, 1.0);
    ...
    deepCol += 0.5279 * dp * mix(baseCol, deepStop, 0.2841);
    deepPat += 0.5279 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 5.9059) + par * 0.0694, time);
    deepCol += 0.2787 * dp * mix(baseCol, deepStop, 0.5681);
    deepPat += 0.2787 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 7.4338) + par * 0.1041, time);
    deepCol += 0.1471 * dp * mix(baseCol, deepStop, 0.8522);
    deepPat += 0.1471 * dp;
    deepCol *= 0.5126;
    deepPat = pow(clamp(deepPat * 0.5118, 0.0, 1.0), 1.3611);
    ...
    rgb += hotStop * 0.2324 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0658 + 0.3479 * presence + 0.3239 * pattern, 0.8477);
