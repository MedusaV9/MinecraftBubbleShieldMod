...
// stack: deep=caustic x3 taps | mid=voronoi+warp1+flicker | rim=graze | flourish=glint
...
void main() {
    ...
    deep += 0.5279 * deepField(duvBase + rimDir * 0.0347 + vec2(-0.040000, -0.057500) * time, time);
    deep += 0.2787 * deepField(duvBase + rimDir * 0.0694 + vec2(-0.053333, -0.080000) * time, time);
    deep = pow(clamp(deep * 0.5535, 0.0, 1.0), 1.3611);
    ...
    float pattern = clamp(0.4289 * deep + 0.8687 * mid + 0.5636 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9095 + pattern * 0.4246);
    vec3 rgb = vertexColor.rgb * (0.5199 + 0.6327 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1512 + 0.7003 * pattern, 0.0, 1.0);
