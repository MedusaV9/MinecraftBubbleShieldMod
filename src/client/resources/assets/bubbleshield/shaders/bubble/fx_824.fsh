...
void main() {
    ...
    mid = clamp(mid * (0.6397 + 0.6244 * texDetail) + 0.2639 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4058), 0.0, 1.0);
    float emit = atlas.a * (0.6442 + 0.4553 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1424 * (0.5 + 0.5 * sin(time * 0.130900)));
    rgb += emit * 0.8644 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2806 + 0.3386 * presence + 0.4537 * pattern + 0.1347 * (1.0 - deepTrans) + 0.1730 * emit, 0.8669);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3450), 0.8669);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6227 + 0.3773 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0978), 0.0, 1.0);
