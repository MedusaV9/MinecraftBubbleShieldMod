...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7470, 0.0, 1.0);
    ...
    deepCol += 0.5128 * dp * mix(baseCol, deepStop, 0.3516);
    deepPat += 0.5128 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 4.3493) + par * 0.0837, time);
    deepCol += 0.2630 * dp * mix(baseCol, deepStop, 0.7032);
    deepPat += 0.2630 * dp;
    deepCol *= 0.7219;
    deepPat = pow(clamp(deepPat * 0.5631, 0.0, 1.0), 1.3673);
    ...
    rgb = satLift(rgb, 1.1108);
    rgb += hotStop * 0.1820 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0614 + 0.3343 * presence + 0.4563 * pattern, 0.8593);
