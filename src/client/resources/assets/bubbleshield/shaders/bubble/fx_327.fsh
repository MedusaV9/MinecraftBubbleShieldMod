...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.0684) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.0684);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.045000, -0.010000) * time, time);
    deep += 0.5128 * deepField(duvBase + rimDir * 0.0418 + vec2(0.068333, -0.015000) * time, time);
    deep = pow(clamp(deep * 0.6610, 0.0, 1.0), 1.3673);
    ...
    vec2 wuv = auv;
    float neb = fbm2(wuv + 0.9962 * vec2(fbm2(wuv + vec2(time * 0.033333, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.029167), midPer)), midPer);
    float lanes = 1.0 - pow(clamp(1.0 - abs(2.0 * fbm2(wuv * 2.0 + vec2(4.2, 1.1), midPer * 2.0) - 1.0), 0.0, 1.0), 3.0) * 0.4088;
