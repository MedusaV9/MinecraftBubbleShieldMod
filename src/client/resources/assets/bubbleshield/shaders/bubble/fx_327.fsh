...
void main() {
    ...
    mid = clamp(mid * (0.7290 + 0.5888 * texDetail) + 0.2256 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4197), 0.0, 1.0);
    float emit = atlas.a * (0.7306 + 0.4704 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1050 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.8153 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4112 + 0.5125 * presence + 0.4260 * pattern + 0.1373 * (1.0 - deepTrans) + 0.1977 * emit, 0.9593);
