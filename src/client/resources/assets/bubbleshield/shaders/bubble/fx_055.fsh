...
float fbm2(vec2 p, vec2 per) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        p = vec2(p.x * 2.0, p.y * 2.1409) + vec2(17.7, 9.2);
        per = vec2(per.x * 2.0, per.y * 2.1409);
        amplitude *= 0.5;
    }
    return value;
}
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.033333, 0.025000) * time, time);
    deep += 0.4391 * deepField(duvBase + rimDir * 0.0693 + vec2(0.051667, 0.037500) * time, time);
    deep += 0.1928 * deepField(duvBase + rimDir * 0.1385 + vec2(0.070000, 0.052500) * time, time);
    deep += 0.0846 * deepField(duvBase + rimDir * 0.2078 + vec2(0.088333, 0.065000) * time, time);
    deep = pow(clamp(deep * 0.5826, 0.0, 1.0), 1.5662);
    ...
    float ridge = 1.0 - abs(2.0 * fbm2(wuv + vec2(0.0, time * 0.320000), midPer) - 1.0);
    float bolt = pow(clamp(ridge, 0.0, 1.0), 7.4272);
    float flash = 0.6 + 0.4 * sin(time * 3.764675 + fbm2(wuv + vec2(43.1, 17.3), midPer) * 9.0);
    ...
    float pattern = clamp(0.4412 * deep + 0.9458 * mid + 0.7995 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.9793 + pattern * 0.4182);
    vec3 rgb = vertexColor.rgb * (0.4668 + 0.7766 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1479 + 0.7665 * pattern, 0.0, 1.0);
