...
void main() {
    ...
    mid = clamp(mid * (0.6120 + 0.4508 * texDetail) + 0.2563 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3991), 0.0, 1.0);
    float emit = atlas.a * (0.7673 + 0.3635 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1472 * (0.5 + 0.5 * sin(time * 0.183260)));
    rgb += emit * 0.7150 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4429 + 0.4444 * presence + 0.4581 * pattern + 0.0804 * (1.0 - deepTrans) + 0.1561 * emit, 0.8426);
