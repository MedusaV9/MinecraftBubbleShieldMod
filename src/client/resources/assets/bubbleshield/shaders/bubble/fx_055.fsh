...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7618, 0.0, 1.0);
    ...
    deepCol += 0.4391 * dp * mix(baseCol, deepStop, 0.1970);
    deepPat += 0.4391 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 4.0808) + par * 0.1385, time);
    deepCol += 0.1928 * dp * mix(baseCol, deepStop, 0.3940);
    deepPat += 0.1928 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 4.9511) + par * 0.2078, time);
    deepCol += 0.0846 * dp * mix(baseCol, deepStop, 0.5910);
    deepPat += 0.0846 * dp;
    deepCol *= 0.6050;
    deepPat = pow(clamp(deepPat * 0.5826, 0.0, 1.0), 1.5662);
    ...
    rgb += hotStop * 0.1644 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0788 + 0.3044 * presence + 0.4581 * pattern, 0.7826);
