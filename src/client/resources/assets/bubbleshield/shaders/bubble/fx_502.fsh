...
void main() {
    ...
    mid = clamp(mid * (0.6570 + 0.4904 * texDetail) + 0.2731 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4828), 0.0, 1.0);
    float emit = atlas.a * (0.7134 + 0.4428 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0861 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.9320 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3943 + 0.4457 * presence + 0.4329 * pattern + 0.1436 * (1.0 - deepTrans) + 0.1526 * emit, 0.9032);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1753), 0.9032);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6811 + 0.3189 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
