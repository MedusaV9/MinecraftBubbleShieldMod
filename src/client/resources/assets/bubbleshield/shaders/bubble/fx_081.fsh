...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7758, 0.0, 1.0);
    ...
    deepCol += 0.4512 * dp * mix(baseCol, deepStop, 0.2780);
    deepPat += 0.4512 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.2907) + par * 0.1496, time);
    deepCol += 0.2036 * dp * mix(baseCol, deepStop, 0.5561);
    deepPat += 0.2036 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 5.3696) + par * 0.2244, time);
    deepCol += 0.0918 * dp * mix(baseCol, deepStop, 0.8341);
    deepPat += 0.0918 * dp;
    deepCol *= 0.6232;
    deepPat = pow(clamp(deepPat * 0.5725, 0.0, 1.0), 1.6094);
    ...
    float midCover = clamp(0.9909 * mid + 0.6523 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.2850);
    rgb += hotStop * 0.1916 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0672 + 0.2880 * presence + 0.4757 * pattern, 0.8229);
