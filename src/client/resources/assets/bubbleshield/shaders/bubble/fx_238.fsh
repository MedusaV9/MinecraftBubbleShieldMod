...
void main() {
    ...
    mid = clamp(mid * (0.6480 + 0.4549 * texDetail) + 0.2635 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3513), 0.0, 1.0);
    float emit = atlas.a * (0.7254 + 0.3671 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1504 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.7131 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3461 + 0.3986 * presence + 0.4072 * pattern + 0.0950 * (1.0 - deepTrans) + 0.1589 * emit, 0.8592);
