...
void main() {
    ...
    mid = clamp(mid * (0.6522 + 0.5368 * texDetail) + 0.1957 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3939), 0.0, 1.0);
    float emit = atlas.a * (0.5947 + 0.4561 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1535 * (0.5 + 0.5 * sin(time * 0.214675)));
    rgb += emit * 0.9753 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3066 + 0.2534 * presence + 0.5667 * pattern + 0.1103 * (1.0 - deepTrans) + 0.1960 * emit, 0.8847);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3422), 0.8847);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1065), 0.0, 1.0);
