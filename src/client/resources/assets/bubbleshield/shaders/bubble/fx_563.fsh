...
void main() {
    ...
    mid = clamp(mid * (0.7003 + 0.4890 * texDetail) + 0.2891 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.6147 + 0.3777 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1068 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.7609 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2829 + 0.2450 * presence + 0.4846 * pattern + 0.1031 * (1.0 - deepTrans) + 0.1841 * emit, 0.8779);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3274), 0.8779);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6346 + 0.3654 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1054), 0.0, 1.0);
