...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7167, 0.0, 1.0);
    ...
    deepCol += 0.4802 * dp * mix(baseCol, deepStop, 0.3879);
    deepPat += 0.4802 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 3.2862) + par * 0.1435, time);
    deepCol += 0.2306 * dp * mix(baseCol, deepStop, 0.7757);
    deepPat += 0.2306 * dp;
    deepCol *= 0.7294;
    deepPat = pow(clamp(deepPat * 0.5845, 0.0, 1.0), 1.4934);
    ...
    rgb += hotStop * 0.2953 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0672 + 0.2978 * presence + 0.3374 * pattern, 0.7930);
