...
// stack: deep=fbm x3 taps | mid=starfield+warp1+flicker | rim=graze | flourish=shimmer | tex=starfield
// fbm: turb x5 octaves | seed 78d2737a880af0d3
// v8 realism: refr 0.3853+0.4980*fres/dist (cap 0.1169) | energy 0.4325+0.4765*p | flow 0.080833 c/s | soft 0.8459m
...
void main() {
    ...
    float refrAmp = min((0.3853 + 0.4980 * fresnel) / max(length(worldPos), 1.0), 0.1169);
    ...
    vec3 refracted = vec3(
        texture(Sampler1, clamp(screenUV + refrOff * 1.0420, vec2(0.001), vec2(0.999))).r,
        texture(Sampler1, clamp(screenUV + refrOff, vec2(0.001), vec2(0.999))).g,
        texture(Sampler1, clamp(screenUV + refrOff * 0.9580, vec2(0.001), vec2(0.999))).b);
