...
void main() {
    ...
    mid = clamp(mid * (0.6899 + 0.6421 * texDetail) + 0.3460 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4109), 0.0, 1.0);
    float emit = atlas.a * (0.5876 + 0.3591 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1458 * (0.5 + 0.5 * sin(time * 0.277507)));
    rgb += emit * 0.8600 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3304 + 0.2756 * presence + 0.4645 * pattern + 0.1237 * (1.0 - deepTrans) + 0.1539 * emit, 0.8530);
