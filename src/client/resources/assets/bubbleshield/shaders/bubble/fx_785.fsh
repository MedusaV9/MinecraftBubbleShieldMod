...
void main() {
    ...
    mid = clamp(mid * (0.6165 + 0.7430 * texDetail) + 0.1854 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5069), 0.0, 1.0);
    float emit = atlas.a * (0.5962 + 0.4408 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0873 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.7588 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3290 + 0.4149 * presence + 0.4047 * pattern + 0.1214 * (1.0 - deepTrans) + 0.1689 * emit, 0.8875);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3340), 0.8875);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6806 + 0.3194 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
