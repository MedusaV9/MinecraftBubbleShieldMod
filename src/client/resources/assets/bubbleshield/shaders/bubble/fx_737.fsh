...
void main() {
    ...
    mid = clamp(mid * (0.7026 + 0.4603 * texDetail) + 0.2593 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4417), 0.0, 1.0);
    float emit = atlas.a * (0.6444 + 0.4934 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1121 * (0.5 + 0.5 * sin(time * 0.104720)));
    ...
    float alpha = vertexColor.a * min(0.2625 + 0.2388 * presence + 0.5701 * pattern + 0.1415 * (1.0 - deepTrans) + 0.1657 * emit, 0.8675);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3304), 0.8675);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6401 + 0.3599 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1195), 0.0, 1.0);
