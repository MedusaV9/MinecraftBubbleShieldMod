...
void main() {
    ...
    mid = clamp(mid * (0.6597 + 0.5279 * texDetail) + 0.2082 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4661), 0.0, 1.0);
    float emit = atlas.a * (0.7591 + 0.4161 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1256 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.8289 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4359 + 0.4463 * presence + 0.4602 * pattern + 0.1351 * (1.0 - deepTrans) + 0.2033 * emit, 0.8644);
