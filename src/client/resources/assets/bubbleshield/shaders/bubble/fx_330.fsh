...
// stack: deep=ridge x2 taps | mid=curlsmoke+warp1+rotate | rim=lat | flourish=glint
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(0.008333, 0.022500) * time, time);
    deep += 0.3888 * deepField(duvBase + rimDir * 0.0655 + vec2(0.013333, 0.035000) * time, time);
    deep = pow(clamp(deep * 0.7200, 0.0, 1.0), 1.2015);
    ...
    vec2 adv = wuv + 0.1513 * curl2(wuv + vec2(0.0, time * 0.055000), midPer);
    float smoke = fbm2(adv + vec2(time * 0.043333, -time * 0.095000), midPer);
    ...
    float pattern = clamp(0.4454 * deep + 0.8634 * mid + 0.6534 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.1565 + pattern * 0.4650);
    vec3 rgb = vertexColor.rgb * (0.3798 + 0.6868 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2105 + 0.8318 * pattern, 0.0, 1.0);
