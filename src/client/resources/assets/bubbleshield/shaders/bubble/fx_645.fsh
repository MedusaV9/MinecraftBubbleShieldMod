...
void main() {
    ...
    mid = clamp(mid * (0.6743 + 0.6232 * texDetail) + 0.1872 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3535), 0.0, 1.0);
    float emit = atlas.a * (0.6969 + 0.3770 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0996 * (0.5 + 0.5 * sin(time * 0.146608)));
    rgb += emit * 0.9212 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3274 + 0.3579 * presence + 0.3845 * pattern + 0.1539 * (1.0 - deepTrans) + 0.2077 * emit, 0.8616);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2886), 0.8616);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7347 + 0.2653 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0730), 0.0, 1.0);
