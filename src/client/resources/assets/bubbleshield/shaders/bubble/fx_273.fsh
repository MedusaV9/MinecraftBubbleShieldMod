...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6538, 0.0, 1.0);
    ...
    deepCol += 0.4124 * dp * mix(baseCol, deepStop, 0.1862);
    deepPat += 0.4124 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 5.5575) + par * 0.1244, time);
    deepCol += 0.1701 * dp * mix(baseCol, deepStop, 0.3725);
    deepPat += 0.1701 * dp;
    dp = deepField(rotA(spinAxis, time * 0.036652) * (sdir * 7.0391) + par * 0.1865, time);
    deepCol += 0.0701 * dp * mix(baseCol, deepStop, 0.5587);
    deepPat += 0.0701 * dp;
    deepCol *= 0.6999;
    deepPat = pow(clamp(deepPat * 0.6051, 0.0, 1.0), 1.3942);
    ...
    rgb += hotStop * 0.2309 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0738 + 0.3444 * presence + 0.3563 * pattern, 0.7925);
