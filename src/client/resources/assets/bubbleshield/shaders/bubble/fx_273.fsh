...
void main() {
    ...
    mid = clamp(mid * (0.7079 + 0.7383 * texDetail) + 0.2959 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4240), 0.0, 1.0);
    float emit = atlas.a * (0.6918 + 0.3868 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1766 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.7371 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4280 + 0.4844 * presence + 0.3563 * pattern + 0.1399 * (1.0 - deepTrans) + 0.1708 * emit, 0.8525);
