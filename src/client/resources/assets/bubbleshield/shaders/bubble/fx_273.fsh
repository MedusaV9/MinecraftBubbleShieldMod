...
// stack: deep=ridge x4 taps | mid=moire+curl+pulse | rim=lat | flourish=glint | tex=chrome
// fbm: turb x6 octaves | seed a6a5ff54a70335cd
// v8 realism: refr 0.9891+1.0936*fres/dist (cap 0.1595) | energy 0.2164+0.4630*p | flow 0.107500 c/s | soft 0.9562m
...
void main() {
    ...
    float refrAmp = min((0.9891 + 1.0936 * fresnel) / max(length(worldPos), 1.0), 0.1595);
