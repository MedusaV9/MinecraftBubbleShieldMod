...
void main() {
    ...
    mid = clamp(mid * (0.6943 + 0.4872 * texDetail) + 0.1838 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4064), 0.0, 1.0);
    float emit = atlas.a * (0.7336 + 0.4863 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1237 * (0.5 + 0.5 * sin(time * 0.157080)));
    ...
    float alpha = vertexColor.a * min(0.3544 + 0.4413 * presence + 0.3425 * pattern + 0.0908 * (1.0 - deepTrans) + 0.1514 * emit, 0.8812);
