...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.7954, 0.0, 1.0);
    ...
    deepCol += 0.4447 * dp * mix(baseCol, deepStop, 0.2289);
    deepPat += 0.4447 * dp;
    dp = deepField(rotA(spinAxis, time * 0.068068) * (sdir * 5.2169) + par * 0.0938, time);
    deepCol += 0.1978 * dp * mix(baseCol, deepStop, 0.4577);
    deepPat += 0.1978 * dp;
    dp = deepField(rotA(spinAxis, time * 0.057596) * (sdir * 6.4938) + par * 0.1406, time);
    deepCol += 0.0879 * dp * mix(baseCol, deepStop, 0.6866);
    deepPat += 0.0879 * dp;
    deepCol *= 0.6083;
    deepPat = pow(clamp(deepPat * 0.5779, 0.0, 1.0), 1.3641);
    ...
    rgb += hotStop * 0.2412 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0489 + 0.3013 * presence + 0.3425 * pattern, 0.8212);
