...
// stack: deep=caustic x3 taps | mid=caustic+none+pulse | rim=lat | flourish=swirl
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.045000, -0.030000) * time, time);
    deep += 0.4447 * deepField(duvBase + rimDir * 0.0469 + vec2(-0.071667, -0.047500) * time, time);
    deep += 0.1978 * deepField(duvBase + rimDir * 0.0938 + vec2(-0.096667, -0.065000) * time, time);
    deep = pow(clamp(deep * 0.6088, 0.0, 1.0), 1.3641);
    ...
    float pattern = clamp(0.3151 * deep + 0.8490 * mid + 0.8711 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.5194 + pattern * 0.4889);
    vec3 rgb = vertexColor.rgb * (0.4896 + 0.7215 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.2391 + 0.7449 * pattern, 0.0, 1.0);
