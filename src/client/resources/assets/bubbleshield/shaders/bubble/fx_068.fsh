...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6285, 0.0, 1.0);
    ...
    deepCol += 0.5762 * dp * mix(baseCol, deepStop, 0.2811);
    deepPat += 0.5762 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 4.7352) + par * 0.1529, time);
    deepCol += 0.3320 * dp * mix(baseCol, deepStop, 0.5622);
    deepPat += 0.3320 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.7087) + par * 0.2294, time);
    deepCol += 0.1913 * dp * mix(baseCol, deepStop, 0.8433);
    deepPat += 0.1913 * dp;
    deepCol *= 0.5226;
    deepPat = pow(clamp(deepPat * 0.4763, 0.0, 1.0), 1.7035);
    ...
    rgb += hotStop * 0.3451 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0703 + 0.3531 * presence + 0.4627 * pattern, 0.8444);
