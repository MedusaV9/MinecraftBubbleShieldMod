...
void main() {
    ...
    mid = clamp(mid * (0.6885 + 0.5195 * texDetail) + 0.2389 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4810), 0.0, 1.0);
    float emit = atlas.a * (0.7843 + 0.3217 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1049 * (0.5 + 0.5 * sin(time * 0.157080)));
    rgb += emit * 0.9120 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4176 + 0.4931 * presence + 0.4627 * pattern + 0.0892 * (1.0 - deepTrans) + 0.1347 * emit, 0.9044);
