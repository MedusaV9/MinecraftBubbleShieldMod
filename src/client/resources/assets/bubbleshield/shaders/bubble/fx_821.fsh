...
void main() {
    ...
    mid = clamp(mid * (0.7460 + 0.7008 * texDetail) + 0.1989 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4660), 0.0, 1.0);
    float emit = atlas.a * (0.6958 + 0.2818 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1699 * (0.5 + 0.5 * sin(time * 0.246091)));
    rgb += emit * 0.6539 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3251 + 0.4330 * presence + 0.3701 * pattern + 0.0897 * (1.0 - deepTrans) + 0.1730 * emit, 0.8834);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2059), 0.8834);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6779 + 0.3221 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0736), 0.0, 1.0);
