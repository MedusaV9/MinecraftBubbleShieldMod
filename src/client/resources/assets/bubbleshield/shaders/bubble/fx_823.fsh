...
void main() {
    ...
    mid = clamp(mid * (0.7175 + 0.5924 * texDetail) + 0.2289 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4182), 0.0, 1.0);
    float emit = atlas.a * (0.7236 + 0.2713 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0843 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.9988 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3088 + 0.2416 * presence + 0.5950 * pattern + 0.1268 * (1.0 - deepTrans) + 0.1652 * emit, 0.9041);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2419), 0.9041);
    // [layer:v5:ghostalpha]
