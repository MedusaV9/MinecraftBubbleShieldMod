...
void main() {
    ...
    mid = clamp(mid * (0.6038 + 0.5384 * texDetail) + 0.2653 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3666), 0.0, 1.0);
    float emit = atlas.a * (0.5607 + 0.3650 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0805 * (0.5 + 0.5 * sin(time * 0.261799)));
    rgb += emit * 0.9201 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.2867 + 0.3545 * presence + 0.5433 * pattern + 0.0912 * (1.0 - deepTrans) + 0.1585 * emit, 0.8931);
