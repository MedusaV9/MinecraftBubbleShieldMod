...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8096, 0.0, 1.0);
    ...
    deepCol += 0.4112 * dp * mix(baseCol, deepStop, 0.3413);
    deepPat += 0.4112 * dp;
    dp = deepField(rotA(spinAxis, time * 0.031416) * (sdir * 4.5952) + par * 0.1397, time);
    deepCol += 0.1691 * dp * mix(baseCol, deepStop, 0.6826);
    deepPat += 0.1691 * dp;
    deepCol *= 0.6085;
    deepPat = pow(clamp(deepPat * 0.6328, 0.0, 1.0), 1.0071);
    ...
    float accretion = invsmooth(0.0, 0.0741, abs(prad - 0.4202));
    ...
    float pattern = clamp(0.3599 * deepPat + 0.8878 * mid + 0.6364 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb += hotStop * 0.2439 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0415 + 0.3611 * presence + 0.4587 * pattern, 0.7931);
