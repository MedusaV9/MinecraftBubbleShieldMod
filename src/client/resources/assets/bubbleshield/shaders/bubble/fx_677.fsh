...
void main() {
    ...
    mid = clamp(mid * (0.6592 + 0.6896 * texDetail) + 0.2235 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4679), 0.0, 1.0);
    float emit = atlas.a * (0.7951 + 0.3846 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1584 * (0.5 + 0.5 * sin(time * 0.209440)));
    rgb += emit * 0.7872 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2833 + 0.2255 * presence + 0.6484 * pattern + 0.0927 * (1.0 - deepTrans) + 0.2115 * emit, 0.8682);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2500), 0.8682);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1221), 0.0, 1.0);
