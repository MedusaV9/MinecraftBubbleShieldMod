...
void main() {
    ...
    mid = clamp(mid * (0.7162 + 0.4628 * texDetail) + 0.2294 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4717), 0.0, 1.0);
    float emit = atlas.a * (0.6271 + 0.4234 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1354 * (0.5 + 0.5 * sin(time * 0.162316)));
    rgb += emit * 0.8399 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3100 + 0.2400 * presence + 0.6182 * pattern + 0.1013 * (1.0 - deepTrans) + 0.2130 * emit, 0.9000);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2546), 0.9000);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6944 + 0.3056 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
