...
// stack: deep=caustic x3 taps | mid=gravlens+warp1+rotate | rim=graze | flourish=shimmer | tex=starfield
// fbm: standard x3 octaves | seed 69fd0d6f4c317f72
// v8 realism: refr 1.3360+2.0070*fres/dist (cap 0.1169) | energy 0.2231+0.4671*p | flow 0.066667 c/s | soft 0.9989m
...
void main() {
    ...
    texDetail = clamp(texDetail * 2.5889 + 0.2150, 0.0, 1.0);
    ...
    float refrAmp = min((1.3360 + 2.0070 * fresnel) / max(length(worldPos), 1.0), 0.1169);
    ...
    float emitMask = clamp(atlas.a * 1.8070, 0.0, 1.0);
    float emit = emitMask * (0.4267 + 0.5321 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1506 * (0.5 + 0.5 * sin(time * 0.298451))) * atlasPoleW;
