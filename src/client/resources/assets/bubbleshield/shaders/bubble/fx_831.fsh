...
void main() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glStars += step(0.8060, sh) * invsmooth(0.03, 0.16, length(sf - (vec2(cellHash(sc + 4.7, layerPx), cellHash(sc + 9.3, layerPx)) - 0.5) * 0.55)) * tw;
    }
    float glRing = invsmooth(0.0, 0.0302, abs(glR - 0.1764));
    float mid = clamp(glStars * (1.0 + glRing * 1.7830) + glRing * 0.3551, 0.0, 1.3);
    ...
    mid = clamp(mid * (0.7074 + 0.6571 * texDetail) + 0.2379 * texDetail, 0.0, 1.5);
    ...
    rgb *= 0.8419 + 0.3866 * texDetail;
    rgb = satLift(rgb, 1.1176);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4768), 0.0, 1.0);
    float emit = atlas.a * (0.7611 + 0.3184 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1506 * (0.5 + 0.5 * sin(time * 0.298451)));
    rgb += emit * 0.6537 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3226 + 0.2740 * presence + 0.5257 * pattern + 0.1191 * (1.0 - deepTrans) + 0.1953 * emit, 0.8790);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2224), 0.8790);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6651 + 0.3349 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
