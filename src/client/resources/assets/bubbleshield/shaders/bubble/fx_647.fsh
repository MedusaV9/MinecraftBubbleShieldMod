...
void main() {
    ...
    mid = clamp(mid * (0.6568 + 0.6434 * texDetail) + 0.1997 * texDetail, 0.0, 1.5);
    ...
    float emit = atlas.a * (0.7530 + 0.4308 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1481 * (0.5 + 0.5 * sin(time * 0.230383)));
    rgb += emit * 0.6932 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3537 + 0.3392 * presence + 0.4786 * pattern + 0.1009 * (1.0 - deepTrans) + 0.1285 * emit, 0.8104);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2941), 0.8104);
    // [layer:v5:ghostalpha]
