...
// stack: deep=caustic x4 taps | mid=irisfilm+warp2+flicker | rim=graze_sparkle | flourish=swirl
...
void main() {
    ...
    rgb = mix(rgb, rgb * (0.5 + 1.4156 * clamp(ifFilm, 0.0, 1.0)), 0.3689 * ifGraze);
    ...
    float alpha = vertexColor.a * min(0.0635 + 0.1992 * presence + 0.4786 * pattern + 0.1009 * (1.0 - deepTrans), 0.7504);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2941), 0.7504);
    // [layer:v5:ghostalpha]
