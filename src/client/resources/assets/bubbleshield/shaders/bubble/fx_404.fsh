...
void main() {
    ...
    mid = clamp(mid * (0.6027 + 0.4590 * texDetail) + 0.2152 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5354), 0.0, 1.0);
    float emit = atlas.a * (0.6600 + 0.4771 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1686 * (0.5 + 0.5 * sin(time * 0.188496)));
    rgb += emit * 0.7416 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.3268 + 0.3276 * presence + 0.4421 * pattern + 0.1358 * (1.0 - deepTrans) + 0.1633 * emit, 0.8961);
