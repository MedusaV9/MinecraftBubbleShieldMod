...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8209, 0.0, 1.0);
    ...
    deepCol += 0.4841 * dp * mix(baseCol, deepStop, 0.1923);
    deepPat += 0.4841 * dp;
    dp = deepField(rotA(spinAxis, time * 0.078540) * (sdir * 4.0963) + par * 0.1139, time);
    deepCol += 0.2344 * dp * mix(baseCol, deepStop, 0.3846);
    deepPat += 0.2344 * dp;
    dp = deepField(rotA(spinAxis, time * 0.062832) * (sdir * 5.0697) + par * 0.1708, time);
    deepCol += 0.1135 * dp * mix(baseCol, deepStop, 0.5769);
    deepPat += 0.1135 * dp;
    deepCol *= 0.5297;
    deepPat = pow(clamp(deepPat * 0.5459, 0.0, 1.0), 1.5150);
    ...
    float nest = pow(clamp(kacc * 0.1769 - 0.0896, 0.0, 1.0), 1.5363);
    float mid = nest * (0.72 + 0.28 * sin(time * 0.712094 + kacc * 2.8308));
    ...
    float pattern = clamp(0.3588 * deepPat + 0.7965 * mid + 0.6405 * rim + flourish + grain, 0.0, 1.5);
    ...
    rgb = satLift(rgb, 1.3102);
    rgb += hotStop * 0.2382 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0583 + 0.3174 * presence + 0.3449 * pattern, 0.8361);
