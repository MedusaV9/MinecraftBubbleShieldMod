...
// stack: deep=voro x4 taps | mid=holoparallax+warp2+flicker | rim=graze_film | flourish=shimmer
...
void main() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        hpAcc += (hpGrid * 0.6846 + hpScan * 0.1266) * (1.0 - fi * 0.1810);
    }
    ...
    float mid = clamp(hpAcc * hpFlick + hpSync * 0.4026, 0.0, 1.25);
    ...
    float alpha = vertexColor.a * min(0.0500 + 0.1675 * presence + 0.6125 * pattern + 0.1445 * (1.0 - deepTrans), 0.8130);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3479), 0.8130);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.4310 + 0.5690 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
