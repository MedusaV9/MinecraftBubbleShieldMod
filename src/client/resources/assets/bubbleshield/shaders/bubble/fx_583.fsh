...
void main() {
    ...
    mid = clamp(mid * (0.6718 + 0.6000 * texDetail) + 0.1932 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.5468), 0.0, 1.0);
    float emit = atlas.a * (0.6055 + 0.4710 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1461 * (0.5 + 0.5 * sin(time * 0.282743)));
    rgb += emit * 0.9516 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2890 + 0.3075 * presence + 0.6125 * pattern + 0.1445 * (1.0 - deepTrans) + 0.1529 * emit, 0.8730);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.3479), 0.8730);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6680 + 0.3320 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.0954), 0.0, 1.0);
