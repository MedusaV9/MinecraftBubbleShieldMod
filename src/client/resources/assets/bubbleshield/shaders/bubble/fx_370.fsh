...
// stack: deep=caustic x3 taps | mid=biolume+none+pulse | rim=graze_sparkle | flourish=shimmer | tex=honeycomb
// fbm: standard x6 octaves | seed e6cbc1d37ee04070
// v8 realism: refr 0.3574+0.4725*fres/dist (cap 0.1168) | energy 0.5455+0.4032*p | flow 0.065833 c/s | soft 0.6750m
...
void main() {
    ...
    float refrAmp = min((0.3574 + 0.4725 * fresnel) / max(length(worldPos), 1.0), 0.1168);
