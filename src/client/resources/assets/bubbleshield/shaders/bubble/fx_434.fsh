...
void main() {
    ...
    mid = clamp(mid * (0.6627 + 0.6015 * texDetail) + 0.3437 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3631), 0.0, 1.0);
    float emit = atlas.a * (0.5619 + 0.4598 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1009 * (0.5 + 0.5 * sin(time * 0.167552)));
    rgb += emit * 0.8453 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.4272 + 0.4343 * presence + 0.3957 * pattern + 0.1288 * (1.0 - deepTrans) + 0.1374 * emit, 0.8874);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2504), 0.8874);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7643 + 0.2357 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
