...
// v8 realism: refr 0.5992+0.7914*fres/dist (cap 0.1473) | energy 0.3421+0.4461*p | flow 0.051667 c/s | soft 0.6591m
...
void main() {
    ...
    float refrAmp = min((0.5992 + 0.7914 * fresnel) / max(length(worldPos), 1.0), 0.1473);
    ...
    float emit = atlas.a * (0.3301 + 0.6735 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1642 * (0.5 + 0.5 * sin(time * 0.130900))) * atlasPoleW;
