...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.5906, 0.0, 1.0);
    ...
    deepCol += 0.4737 * dp * mix(baseCol, deepStop, 0.3193);
    deepPat += 0.4737 * dp;
    dp = deepField(rotA(spinAxis, time * 0.073304) * (sdir * 3.2654) + par * 0.1278, time);
    deepCol += 0.2244 * dp * mix(baseCol, deepStop, 0.6387);
    deepPat += 0.2244 * dp;
    deepCol *= 0.7309;
    deepPat = pow(clamp(deepPat * 0.5889, 0.0, 1.0), 1.2220);
    ...
    rgb = satLift(rgb, 1.2388);
    rgb += hotStop * 0.3060 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0709 + 0.3631 * presence + 0.3239 * pattern, 0.8075);
