...
// stack: deep=voro x2 taps | mid=nebula+warp1+flicker | rim=graze_film | flourish=swirl
...
vec3 voro2(vec2 p, vec2 per, float t) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            o = 0.5 + 0.2873 * sin(t + 6.2831853 * o);
            vec2 r = g + o - f;
            float d = dot(r, r);
            if (d < md) {
                md = d;
...
float deepField(vec2 p, float t) {
    vec3 v = voro2(p, vec2(1.0000, 2.0000), t * 0.429351);
...
void main() {
    ...
    float deep = deepField(duvBase + vec2(-0.012500, -0.023333) * time, time);
    deep += 0.4737 * deepField(duvBase + rimDir * 0.0639 + vec2(-0.019167, -0.036667) * time, time);
    deep = pow(clamp(deep * 0.6785, 0.0, 1.0), 1.2220);
    ...
    float neb = fbm2(wuv + 1.0284 * vec2(fbm2(wuv + vec2(time * 0.027500, 0.0), midPer), fbm2(wuv - vec2(0.0, time * 0.033333), midPer)), midPer);
    float lanes = 1.0 - pow(clamp(1.0 - abs(2.0 * fbm2(wuv * 2.0 + vec2(4.2, 1.1), midPer * 2.0) - 1.0), 0.0, 1.0), 3.0) * 0.4229;
    ...
    float pattern = clamp(0.4506 * deep + 0.8388 * mid + 0.6095 * rim + flourish + grain, 0.0, 1.5);
    vec3 accent = accentPalette(0.3012 + pattern * 0.4154);
    vec3 rgb = vertexColor.rgb * (0.4399 + 0.8412 * pattern);
    ...
    float alpha = vertexColor.a * clamp(0.1372 + 0.7199 * pattern, 0.0, 1.0);
