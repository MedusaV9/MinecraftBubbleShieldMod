...
void main() {
    ...
    mid = clamp(mid * (0.7355 + 0.4646 * texDetail) + 0.2585 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3804), 0.0, 1.0);
    float emit = atlas.a * (0.6341 + 0.3040 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0816 * (0.5 + 0.5 * sin(time * 0.293215)));
    rgb += emit * 0.9324 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3857 + 0.4163 * presence + 0.5036 * pattern + 0.1306 * (1.0 - deepTrans) + 0.1563 * emit, 0.8666);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.1551), 0.8666);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.7573 + 0.2427 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1375), 0.0, 1.0);
