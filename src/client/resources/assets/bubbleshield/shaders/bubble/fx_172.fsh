...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.8005, 0.0, 1.0);
    ...
    deepCol += 0.4349 * dp * mix(baseCol, deepStop, 0.3479);
    deepPat += 0.4349 * dp;
    dp = deepField(rotA(spinAxis, time * 0.047124) * (sdir * 4.2278) + par * 0.1010, time);
    deepCol += 0.1891 * dp * mix(baseCol, deepStop, 0.6958);
    deepPat += 0.1891 * dp;
    deepCol *= 0.7780;
    deepPat = pow(clamp(deepPat * 0.6158, 0.0, 1.0), 1.0264);
    ...
    float midCover = clamp(0.8584 * mid + 0.6528 * rim, 0.0, 1.0);
    rgb = mix(deepCol, rgb, midCover);
    rgb = satLift(rgb, 1.2962);
    rgb += hotStop * 0.2849 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0656 + 0.2879 * presence + 0.4799 * pattern, 0.7918);
