...
void main() {
    ...
    mid = clamp(mid * (0.6580 + 0.7222 * texDetail) + 0.2841 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.3814), 0.0, 1.0);
    float emit = atlas.a * (0.7434 + 0.3591 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1249 * (0.5 + 0.5 * sin(time * 0.151844)));
    rgb += emit * 0.9168 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.3080 + 0.3091 * presence + 0.4557 * pattern + 0.0956 * (1.0 - deepTrans) + 0.1211 * emit, 0.8691);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2158), 0.8691);
    // [layer:v5:ghostalpha]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1286), 0.0, 1.0);
