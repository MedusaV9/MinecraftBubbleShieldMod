...
void main() {
    ...
    vec3 hotStop = clamp(1.0 - (1.0 - spun) * (1.0 - spun) * 0.6531, 0.0, 1.0);
    ...
    deepCol += 0.4326 * dp * mix(baseCol, deepStop, 0.3158);
    deepPat += 0.4326 * dp;
    dp = deepField(rotA(spinAxis, time * 0.026180) * (sdir * 5.5780) + par * 0.1288, time);
    deepCol += 0.1872 * dp * mix(baseCol, deepStop, 0.6315);
    deepPat += 0.1872 * dp;
    deepCol *= 0.6714;
    deepPat = pow(clamp(deepPat * 0.6174, 0.0, 1.0), 1.4565);
    ...
    rgb = satLift(rgb, 1.1890);
    rgb += hotStop * 0.2239 * smoothstep(0.72, 1.0, pattern);
    ...
    float alpha = vertexColor.a * min(0.0657 + 0.2728 * presence + 0.4531 * pattern, 0.7845);
