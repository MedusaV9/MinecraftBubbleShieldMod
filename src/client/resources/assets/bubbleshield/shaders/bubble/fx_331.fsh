...
void main() {
    ...
    mid = clamp(mid * (0.7015 + 0.6395 * texDetail) + 0.3103 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4359), 0.0, 1.0);
    float emit = atlas.a * (0.7280 + 0.3356 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.1646 * (0.5 + 0.5 * sin(time * 0.178024)));
    rgb += emit * 0.7046 * emitCol;
    ...
    float alpha = vertexColor.a * min(0.4225 + 0.4652 * presence + 0.4235 * pattern + 0.1386 * (1.0 - deepTrans) + 0.1396 * emit, 0.8845);
