...
void main() {
    ...
    mid = clamp(mid * (0.6976 + 0.7295 * texDetail) + 0.2071 * texDetail, 0.0, 1.5);
    ...
    vec3 emitCol = clamp(mix(baseCol, vec3(1.0), 0.4776), 0.0, 1.0);
    float emit = atlas.a * (0.5840 + 0.2570 * clamp(pattern, 0.0, 1.0)) * (1.0 - 0.0919 * (0.5 + 0.5 * sin(time * 0.235619)));
    rgb += emit * 0.8566 * emitCol;
    // [layer:v5:softknee]
    ...
    float alpha = vertexColor.a * min(0.2916 + 0.2776 * presence + 0.5529 * pattern + 0.1172 * (1.0 - deepTrans) + 0.2072 * emit, 0.8852);
    // [layer:v5:backface]
    ...
    alpha = min(alpha * (1.0 + v5Back * 0.2981), 0.8852);
    // [layer:v5:ghostalpha]
    ...
    alpha *= 0.6427 + 0.3573 * smoothstep(0.03, 0.42, v5Luma);
    // [layer:v5:dither]
    ...
    alpha = clamp(alpha * (1.0 + (v5Dither - 0.5) * 0.1179), 0.0, 1.0);
