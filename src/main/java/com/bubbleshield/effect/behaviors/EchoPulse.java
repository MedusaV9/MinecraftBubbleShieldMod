	...
	public void tick(ServerLevel level, Vec3 center, float radius, ShieldShape shape, EffectDefinition def, long gameTime, ContextState ctx) {
		...
		if (variant == 3) {
			...
			for (int i = 0; i < points; i++) {
				...
				level.sendParticles(ParticleTypes.SCULK_CHARGE_POP, true, false,
		...
		if (variant == 5) {
			...
			sendRing(level, center, radius, def, ctx, 3L - phase);
			return;
		}
		...
		if (variant == 1) {
			...
			sendRing(level, center, radius, def, ctx, (phase + 1L) % 4L);
			if (gameTime % 40L == 0L) {
				level.sendParticles(ParticleTypes.SONIC_BOOM, true, false, center.x, center.y + 1.0, center.z, 1, 0.0, 0.0, 0.0, 0.0);
			}
		}
