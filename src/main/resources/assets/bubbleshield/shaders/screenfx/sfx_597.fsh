...
// for effect 597. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:duo:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.9749 * sin(animRaw * 0.1542) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8593 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4561, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0994;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1742);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0954), 0.0, 1.5);
