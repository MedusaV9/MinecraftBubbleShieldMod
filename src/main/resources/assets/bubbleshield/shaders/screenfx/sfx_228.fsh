...
// for effect 228. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:pulse:steady:grain]
...
void main() {
    ...
    float breath = 0.5807 + 0.3754 * sin(anim * 1.5040 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(anim * 8.7091), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1150);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2117), 0.0, 1.5);
