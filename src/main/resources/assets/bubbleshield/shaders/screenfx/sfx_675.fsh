...
// for effect 675. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:zoned:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.2757 * sin(animRaw * 0.1401) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Ring-zoned fringe: concentric zones alternate their separation strength.
    float zone = 0.5 + 0.5 * sin(centerDist * 17.4305 - anim);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1539), 0.0, 1.5);
