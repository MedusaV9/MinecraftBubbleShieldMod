...
// for effect 584. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:ink:surge:sparkle]
...
void main() {
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 4.1146, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5926 + 0.2802 * baseLuma;
    vec3 paper = vec3(flatTone) * mix(vec3(1.0), Secondary.rgb, ParamsB.z);
    vec3 inked = mix(paper, Primary.rgb * 0.1162, edge);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.7281);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8208, 1.0, sin(oClock * 7.6304 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9838, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3385;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2004);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1430), 0.0, 1.5);
