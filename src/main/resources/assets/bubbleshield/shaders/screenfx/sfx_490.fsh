...
// for effect 490. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:steady:none]
...
void main() {
    ...
    float frame = mod(floor(anim * 6.9338), 1024.0);
    float rowRoll = hash11(floor(texCoord.y * 22.1923) * 7.31 + frame * 13.7);
    float colRoll = hash11(floor(texCoord.x * 21.0870) * 5.13 + frame * 7.9);
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.4750 * glitchK * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.3690 * glitchK * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1730);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1193), 0.0, 1.5);
