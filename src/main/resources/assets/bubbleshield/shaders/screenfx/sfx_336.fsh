...
// for effect 336. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:steady:grain]
...
void main() {
    ...
    float frame = mod(floor(anim * 4.7911), 1024.0);
    vec2 cell = floor(texCoord * vec2(7.0047, 7.3768));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.8936
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0377 * ParamsA.y
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    float grainFrame = mod(floor(anim * 6.2660), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0339;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2062);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1238), 0.0, 1.5);
