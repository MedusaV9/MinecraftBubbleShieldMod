...
// for effect 270. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:duo:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7331 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(anim * 8.7303), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0310;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2138);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2040), 0.0, 1.5);
