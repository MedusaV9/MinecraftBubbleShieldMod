...
// for effect 725. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:starburst:six:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4092 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // 6-arm starburst: a dense snowflake star, 2 taps per arm.
    vec2 texel = 4.1148 / safeInSize;
    float thr = max(ParamsA.w, 0.3991);
    vec3 streak = base * max(baseLuma - thr, 0.0) * 0.3;
    for (int i = 0; i < 6; i++) {
        float armAng = float(i) * 1.0472 + 0.3468;
        vec2 arm = vec2(cos(armAng), sin(armAng)) * texel;
        streak += brightTap(texCoord + safeOffset(arm), thr) * 0.1;
        streak += brightTap(texCoord + safeOffset(arm * 2.1419), thr) * 0.05;
    }
    // Additive calibration: capped strength, attenuated on bright scenes.
    vec3 outColor = base + streak * mix(vec3(1.0), Primary.rgb, 0.7265) * min(strength, 1.0) * (1.0 - 0.3232 * baseLuma);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.7591);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8073, 1.0, sin(oClock * 7.2644 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9799, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2787;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1041);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1133), 0.0, 1.5);
