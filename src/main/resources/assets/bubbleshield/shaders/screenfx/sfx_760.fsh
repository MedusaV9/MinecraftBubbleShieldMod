...
// for effect 760. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.0722 * sin(animRaw * 0.1283) * ParamsB.y;
    float animAmp = 1.0;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.7479);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8207, 1.0, sin(oClock * 7.1839 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9798, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2950;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1737), 0.0, 1.5);
