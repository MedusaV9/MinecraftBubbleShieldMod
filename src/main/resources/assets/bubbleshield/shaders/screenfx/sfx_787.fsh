...
// for effect 787. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:zoned:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 1.5741 * sin(animRaw * 0.1506) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Ring-zoned fringe: concentric zones alternate their separation strength.
    float zone = 0.5 + 0.5 * sin(centerDist * 13.2893 - anim);
    ...
    float grainFrame = mod(floor(grainClock * 2.0920), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1229);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1504), 0.0, 1.5);
