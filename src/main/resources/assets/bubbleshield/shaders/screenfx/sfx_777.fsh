...
// for effect 777. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:barrel:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.5035 * sin(animRaw * 0.1348) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Barrel warp: the frame bulges outward; fringing grows with radius.
    float r2 = centerDist * centerDist;
    vec2 warp = centered * r2 * 0.0719 * min(strength, 1.0);
    vec2 fringe = centered * r2 * 0.0139 * min(strength, 1.0) * animAmp;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0775), 0.0, 1.5);
