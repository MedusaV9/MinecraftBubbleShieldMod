...
// for effect 181. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:pulse:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.4445 * sin(animRaw * 0.1077) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float oRim = smoothstep(0.3949, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1060;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2182), 0.0, 1.5);
