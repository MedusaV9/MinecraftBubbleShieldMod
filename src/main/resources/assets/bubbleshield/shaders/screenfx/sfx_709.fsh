...
// for effect 709. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:pulse:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0008 + ParamsB.x * 6.2831);
    ...
    float breath = 0.6392 + 0.3945 * sin(anim * 2.0279 + ParamsB.x * 6.2831);
    vec3 outColor = base + Primary.rgb * edge * glowK * breath;

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6372 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4412, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1518;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1914);
