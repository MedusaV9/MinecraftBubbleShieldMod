...
// for effect 711. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:rising:steady:sparkle]
...
void main() {
    ...
    vec2 off = vec2(
        sin(texCoord.y * 82.1476 + anim * 4.0) + sin(texCoord.y * 46.3037 - anim * 2.6),
        cos(texCoord.x * 61.4707 + anim * 3.1) * 0.4
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.2241);
    vec3 warm = scene * hazeTint;
    vec3 outColor = mix(scene, warm * mix(vec3(1.0), Primary.rgb, ParamsB.z), 0.6602);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.8452);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8520, 1.0, sin(oClock * 7.6881 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9836, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3765;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1779);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1525), 0.0, 1.5);
