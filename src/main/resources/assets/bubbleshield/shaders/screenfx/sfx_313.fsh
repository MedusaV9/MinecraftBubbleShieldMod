...
// for effect 313. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:lumaonly:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6835 + ParamsB.x * 3.1416);
    ...
    float grainFrame = mod(floor(anim * 6.3775), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2036);
