...
// for effect 699. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:gloom:dusk:steady:grain]
...
void main() {
    ...
    vec3 chilled = mix(base, coldGrey, 0.4015 * clamp(strength, 0.0, 1.0));
    float breathe = 0.5 + 0.5 * sin(anim * 0.3393 + ParamsB.x * 6.2831);
    float reach = 0.3264 + 0.1375 * breathe;
    float rim = smoothstep(reach, reach + 0.4921, centerDist);
    float swell = 1.0 - 0.1046 * (0.5 + 0.5 * sin(anim * 0.2323));
    vec3 dimmed = chilled * swell;
    vec3 outColor = mix(dimmed, Primary.rgb * 0.0832, rim * clamp(strength, 0.0, 1.0) * 0.7054);
    ...
    float grainFrame = mod(floor(grainClock * 2.3768), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0235;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1102);
