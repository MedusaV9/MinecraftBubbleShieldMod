...
// for effect 050. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.2420 * sin(animRaw * 0.1895) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float grainFrame = mod(floor(anim * 5.9696), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1090);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1436), 0.0, 1.5);
