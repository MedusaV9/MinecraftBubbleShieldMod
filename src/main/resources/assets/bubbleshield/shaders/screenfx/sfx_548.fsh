...
// for effect 548. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:kaleido:wedge6:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.8382 * sin(animRaw * 0.1501) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float local = mod(angle + anim * 0.1833, wedge) - wedge * 0.5;
    float folded = abs(local);
    vec2 dir = vec2(cos(folded + anim * 0.0778), sin(folded + anim * 0.0432));
    ...
    float seam = invsmooth(0.0, 0.0745, abs(local)) * smoothstep(0.05, 0.25, centerDist);
    vec3 outColor = scene + mix(Primary.rgb, Secondary.rgb, texCoord.y) * seam * 0.2192 * strength;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0830), 0.0, 1.5);
