...
// for effect 182. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:steady:pulseglow]
...
void main() {
    ...
    float frame = mod(floor(anim * 6.3996), 1024.0);
    vec2 cell = floor(texCoord * vec2(9.9403, 6.4431));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.9099
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0253 * ParamsA.y
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5474 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3774, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1156;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1083);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1817), 0.0, 1.5);
