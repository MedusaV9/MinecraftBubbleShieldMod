...
// for effect 485. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:duo:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7102 + ParamsB.x * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1689);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2027), 0.0, 1.5);
