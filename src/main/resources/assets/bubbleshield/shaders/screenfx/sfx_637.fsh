...
// for effect 637. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:breathing:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1111 + ParamsB.x * 6.2831);
    ...
    float lensK = sin(anim * 0.3183 + ParamsB.x * 6.2831) * 0.0684 * min(strength, 1.0);
    vec2 warp = centered * r2 * lensK;
    vec2 fringe = centered * r2 * 0.0187 * min(strength, 1.0) * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.9141), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0289;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1093);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1236), 0.0, 1.5);
