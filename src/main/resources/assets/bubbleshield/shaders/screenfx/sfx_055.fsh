...
void main() {
    ...
    float jitter = (hash11(col + frame * 47.3) - 0.5) * 0.0031 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(0.0, jitter)));
    float scan = 0.5 + 0.5 * sin((texCoord.x * lineCount + anim * 0.4319) * 6.2831);
    ...
    float grainFrame = mod(floor(anim * 8.0186), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1539);
