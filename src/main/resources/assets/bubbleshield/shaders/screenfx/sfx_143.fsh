...
// for effect 143. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:surge:none]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5077 + ParamsB.x * 3.1416);
    ...
    float jitter = (hash11(row + frame * 91.7) - 0.5) * 0.0023 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(jitter, 0.0)));
    float scan = 0.5 + 0.5 * sin((texCoord.y * ParamsA.z - anim * 0.4017) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3573 * scan * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2198);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2066), 0.0, 1.5);
