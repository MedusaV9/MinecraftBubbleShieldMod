...
// for effect 462. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:steady:pulseglow]
...
void main() {
    ...
    float frame = mod(floor(anim * 6.7051), 1024.0);
    float rowRoll = hash11(floor(texCoord.y * 28.7726) * 7.31 + frame * 13.7);
    float colRoll = hash11(floor(texCoord.x * 15.3028) * 5.13 + frame * 7.9);
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.4648 * glitchK * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.2933 * glitchK * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7603 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3662, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0957;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1823);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1694), 0.0, 1.5);
