...
// for effect 680. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.8437 * sin(animRaw * 0.1264) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float jitter = (hash11(row + frame * 91.7) - 0.5) * 0.0033 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(jitter, 0.0)));
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.6516) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3283 * scan * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8410 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3868, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0827;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1356);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1268), 0.0, 1.5);
