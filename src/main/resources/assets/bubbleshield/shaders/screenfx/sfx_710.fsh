...
// for effect 710. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9938 + ParamsB.x * 6.2831);
    ...
    float frost = clamp(0.3367 + 0.5998 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5137);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2879 * crystals);
    float glint = smoothstep(0.8400, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.2677 + crystals * 37.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 13.2022);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8700, 1.0, sin(oClock * 13.1100 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9761, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3794;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1535);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0932), 0.0, 1.5);
