...
// for effect 066. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:steady:sparkle]
...
void main() {
    ...
    vec3 outColor = base + Primary.rgb * pow(edge, 0.7381) * strength * 0.8090;

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 10.6124);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8505, 1.0, sin(anim * 1.7661 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9844, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3439;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1639);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1005), 0.0, 1.5);
