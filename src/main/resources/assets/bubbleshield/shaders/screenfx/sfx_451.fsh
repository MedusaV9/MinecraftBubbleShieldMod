...
// for effect 451. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:linear:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1420 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Fixed-axis fringe: the whole frame splits along one direction.
    vec2 shift = vec2(0.2305, 0.9731) * ParamsA.y * 0.4452 * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.6108), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1457);
