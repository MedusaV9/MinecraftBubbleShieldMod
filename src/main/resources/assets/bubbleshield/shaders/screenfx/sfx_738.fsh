...
// for effect 738. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:breath:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5343 + ParamsB.x * 3.1416);
    ...
    float reach = 0.3333 + 0.1340 * sin(anim * 0.4666 + ParamsB.x * 6.2831);
    float frost = smoothstep(reach, reach + 0.2911, centerDist + (crystals - 0.5) * 0.28) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5013);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3201 * crystals);
    float glint = smoothstep(0.8545, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 0.9953 + crystals * 37.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.4373);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8792, 1.0, sin(oClock * 14.7566 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9765, oTw);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1345);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1495), 0.0, 1.5);
