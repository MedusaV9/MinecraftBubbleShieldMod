...
// for effect 151. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:full:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1240 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Whole-frame heat shimmer, as if standing inside a furnace bloom.
    vec2 off = vec2(
        sin(texCoord.y * 84.9630 + anim * 3.4),
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2019);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1255), 0.0, 1.5);
