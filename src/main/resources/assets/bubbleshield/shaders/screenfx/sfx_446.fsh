...
// for effect 446. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:headswitch:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8648 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    float vhsK = min(ParamsA.y, 1.0);
    float frame = mod(floor(anim * 0.2592), 1024.0);
    float row = floor(texCoord.y * 114.2544);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 304.5461 + anim * 1.0899) * 0.0018 * vhsK * animAmp;
    // Head-switch tear: the bottom edge of the frame always shears.
    float switchZone = invsmooth(0.0, 0.0589, texCoord.y);
    float shear = switchZone * (sin(anim * 0.7217 + texCoord.y * 69.4656) * 0.5 + 0.7) * 0.0098 * vhsK;
    ...
    float bleed = 0.0036 * vhsK;
    float red = sampleAt(texCoord + safeOffset(baseOff + vec2(bleed, 0.0))).r;
    float green = sampleAt(texCoord + safeOffset(baseOff)).g;
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.8646, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    taped = mix(taped, vec3(luma(taped)), switchZone * 0.4185);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.9119);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8369, 1.0, sin(oClock * 8.2419 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9800, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3600;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1127);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0935), 0.0, 1.5);
