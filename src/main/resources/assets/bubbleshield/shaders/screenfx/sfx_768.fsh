...
// for effect 768. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:breathe:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.8967 * sin(animRaw * 0.1353) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Quantize to a slowly breathing number of levels.
    float levels = max(2.0, ParamsA.y + sin(anim * 0.8639) * 1.3981);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.9279);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8556, 1.0, sin(oClock * 8.7014 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9795, oTw);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1235);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1262), 0.0, 1.5);
