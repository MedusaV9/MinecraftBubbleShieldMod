...
// for effect 090. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:steady:none]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.6616) * strength;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1736), 0.0, 1.5);
