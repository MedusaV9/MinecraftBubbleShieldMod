...
// for effect 623. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:linear:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.1002 * sin(animRaw * 0.1880) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Planar wavefronts sweep across the screen in one direction.
    vec2 dir = vec2(0.7284, 0.6852);
    float ring = sin(dot(texCoord, dir) * ParamsA.z - anim * 2.6995);
    float fade = invsmooth(0.35, 1.0, centerDist);
    vec2 off = dir * ring * fade * 0.0060 * ParamsA.y * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.8431);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8384, 1.0, sin(oClock * 8.3261 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9839, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3598;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1008);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1475), 0.0, 1.5);
