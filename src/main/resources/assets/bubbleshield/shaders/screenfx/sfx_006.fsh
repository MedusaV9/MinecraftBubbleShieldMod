...
// for effect 006. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:radial:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.6688 * sin(animRaw * 0.1737) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float grainFrame = mod(floor(anim * 7.7472), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1162);
