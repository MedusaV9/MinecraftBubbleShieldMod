...
// for effect 629. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:spin:steady:none]
...
void main() {
    ...
    float arc = 0.0417 * ParamsA.y * animAmp * smoothstep(0.1, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 8; i++) {
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1736);
