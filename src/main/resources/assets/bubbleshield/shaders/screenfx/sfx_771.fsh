...
// for effect 771. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:duotone:soft:steady:grain]
...
void main() {
    ...
    float ramp = smoothstep(0.1148, 0.8911, baseLuma);
    vec3 duotone = mix(Secondary.rgb * 0.5125, Primary.rgb * (0.7876 + 0.2061 * ramp), ramp);
    ...
    float grainFrame = mod(floor(grainClock * 2.2717), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2097);
