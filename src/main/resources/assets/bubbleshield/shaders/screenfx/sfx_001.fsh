...
// for effect 001. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:steady:pulseglow]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.7020) * strength;

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8947 + ParamsB.x * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2102);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2000), 0.0, 1.5);
