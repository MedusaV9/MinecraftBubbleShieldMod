...
// for effect 563. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:radial:steady:sparkle]
...
void main() {
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.4100);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8764, 1.0, sin(oClock * 7.1359 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9753, oTw);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1886);
