...
void main() {
    ...
    float lineX = min(ParamsA.z * 0.6989, safeInSize.x * 0.25);
    float scanY = 0.5 + 0.5 * sin((texCoord.y * lineY - anim * 0.4160) * 6.2831);
    float scanX = 0.5 + 0.5 * sin((texCoord.x * lineX + anim * 0.4335) * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1652);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1158), 0.0, 1.5);
