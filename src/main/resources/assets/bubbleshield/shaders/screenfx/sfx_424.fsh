...
// for effect 424. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:wide:steady:grain]
...
void main() {
    ...
    vec2 mosaicInSize = max(safeInSize / max(ParamsA.y * vec2(2.8662, 1.0), vec2(1.0)), vec2(1.0));
    ...
    float grainFrame = mod(floor(grainClock * 1.5055), 256.0);
