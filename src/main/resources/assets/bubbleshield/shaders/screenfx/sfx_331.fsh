...
// for effect 331. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.5007 * sin(animRaw * 0.1500) * ParamsB.y;
    float animAmp = 1.0;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1065), 0.0, 1.5);
