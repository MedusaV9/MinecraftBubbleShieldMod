...
// for effect 704. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:breathe:steady:sparkle]
...
void main() {
    ...
    float mosaic = max(ParamsA.y * (1.0 + 0.2542 * sin(anim * 0.8963)), 1.0);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.0936);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8243, 1.0, sin(oClock * 8.7626 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9789, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3040;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2117);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1207), 0.0, 1.5);
