...
// for effect 466. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:dreamblur:glowdream:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8723 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Bright-lifted dream veil: the blur adds a soft luminous bloom.
    vec2 texel = 3.4268 / safeInSize;
    ...
    blurred /= 9.0;
    vec3 dream = base * 0.5944 + blurred * mix(vec3(1.0), Primary.rgb, ParamsB.z) * 0.5726;
    ...
    vec2 cellUv = floor(texCoord * safeInSize / 9.0922);
    float tw = hash21(cellUv);
    float twClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float twinkle = smoothstep(0.8254, 1.0, sin(twClock * 13.5771 + tw * 6.2831) * 0.5 + 0.5) * step(0.9827, tw);
    vec3 outColor = dream + Primary.rgb * twinkle * 0.4200 * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 2.3210), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1032);
