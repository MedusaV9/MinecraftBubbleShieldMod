...
// for effect 698. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:headswitch:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1393 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    float vhsK = min(ParamsA.y, 1.0);
    float frame = mod(floor(anim * 0.1951), 1024.0);
    float row = floor(texCoord.y * 107.4747);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 331.1901 + anim * 1.0529) * 0.0016 * vhsK * animAmp;
    // Head-switch tear: the bottom edge of the frame always shears.
    float switchZone = invsmooth(0.0, 0.0594, texCoord.y);
    float shear = switchZone * (sin(anim * 0.6101 + texCoord.y * 84.3096) * 0.5 + 0.7) * 0.0096 * vhsK;
    ...
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.7321, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    taped = mix(taped, vec3(luma(taped)), switchZone * 0.4345);
    ...
    float grainFrame = mod(floor(grainClock * 2.0837), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0383;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2038);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1235), 0.0, 1.5);
