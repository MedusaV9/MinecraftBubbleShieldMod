...
// for effect 609. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:cornered:drift:none]
...
void main() {
    ...
    float anim = animRaw + 1.9323 * sin(animRaw * 0.1688) * ParamsB.y;
    float animAmp = 1.0;
    ...
    vec2 warp = centered * r2 * 0.0592 * min(strength, 1.0);
    vec2 fringe = centered * r2 * r2 * 0.0339 * min(strength, 1.0) * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1259);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1536), 0.0, 1.5);
