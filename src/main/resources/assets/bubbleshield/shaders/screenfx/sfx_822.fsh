...
// for effect 822. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:veins:surge:none]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4713 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // Ridged ice veins crawl across the whole pane.
    float ridge = 1.0 - abs(2.0 * vnoise(texCoord * ParamsA.z + vec2(anim * 0.0266, 0.0)) - 1.0);
    float crystals = ridge * 0.7 + vnoise(texCoord * ParamsA.z * 2.3) * 0.3;
    float frost = smoothstep(0.5801, 0.8826, crystals * (0.75 + 0.25 * centerDist)) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5544);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3358 * crystals);
    float glint = smoothstep(0.8136, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 0.8139 + crystals * 37.0));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1110);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1152), 0.0, 1.5);
