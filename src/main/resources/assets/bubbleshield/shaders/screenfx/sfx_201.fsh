...
// for effect 201. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 1.6274 * sin(animRaw * 0.1145) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float grainFrame = mod(floor(anim * 8.8116), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1531), 0.0, 1.5);
