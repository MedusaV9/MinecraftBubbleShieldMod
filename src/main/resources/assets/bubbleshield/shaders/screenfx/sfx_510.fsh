...
// for effect 510. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:diag:surge:sparkle]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.36
        ...
        + brightTap(texCoord + safeOffset(vec2(-texel.x, texel.y)), ParamsA.w) * 0.16;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.6554) * strength;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.6887);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8386, 1.0, sin(oClock * 7.5642 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9852, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3707;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0960), 0.0, 1.5);
