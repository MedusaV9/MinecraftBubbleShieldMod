...
// for effect 545. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:pulsezoom:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6606 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // 10-tap zoom streak whose reach surges on GameTime.
    float surgeK = 0.6977 + 0.3842 * sin(anim * 1.4067 + ParamsB.x * 6.2831);
    float blur = 0.0373 * ParamsA.y * animAmp * surgeK * smoothstep(0.05, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 10; i++) {
    ...
    float grainFrame = mod(floor(grainClock * 2.4386), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1731), 0.0, 1.5);
