...
// for effect 619. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:radial:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.9495 * sin(animRaw * 0.1465) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8818 + ParamsB.x * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1729);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1874), 0.0, 1.5);
