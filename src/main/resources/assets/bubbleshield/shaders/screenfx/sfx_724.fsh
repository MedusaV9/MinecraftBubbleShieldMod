...
// for effect 724. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:charcoal:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.4184 * sin(animRaw * 0.1273) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 4-tap cross-difference Sobel over scene luma.
    vec2 texel = 1.5720 / safeInSize;
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 3.6844, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5592 + 0.2963 * baseLuma;
    vec3 paper = vec3(flatTone) * mix(vec3(1.0), Secondary.rgb, ParamsB.z);
    // Charcoal: soft wide strokes broken up by a static grain mask.
    float soft = pow(edge, 0.6705);
    float grain = vnoise(texCoord * safeInSize * 0.0793);
    vec3 inked = mix(paper, Secondary.rgb * 0.1803, clamp(soft * (0.55 + 0.45 * grain), 0.0, 1.0));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1293);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1241), 0.0, 1.5);
