...
// for effect 566. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:surge:pulseglow]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.7207) * strength;

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8086 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4804, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1134;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1666);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1954), 0.0, 1.5);
