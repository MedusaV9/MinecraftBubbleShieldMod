...
// for effect 352. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.1166 * sin(animRaw * 0.1352) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 5-tap cross blur of the bright pass, weighted toward the center.
    vec2 texel = 5.6555 / safeInSize;
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.7338) * strength;
