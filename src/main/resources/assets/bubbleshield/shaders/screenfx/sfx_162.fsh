...
// for effect 162. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:tunnel:steady:grain]
...
void main() {
    ...
    float mask = smoothstep(0.1313, 0.7301, centerDist);
    ...
    float grainFrame = mod(floor(anim * 8.7989), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1617);
