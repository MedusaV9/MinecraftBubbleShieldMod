...
// for effect 757. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:sincos:steady:grain]
...
void main() {
    ...
    vec2 off = vec2(
        ...
        cos(texCoord.x * ParamsA.z * 0.9721 + anim)
    ) * ParamsA.y * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 2.2837), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1473);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2090), 0.0, 1.5);
