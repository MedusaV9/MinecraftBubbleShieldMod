...
// for effect 703. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:zoned:steady:grain]
...
void main() {
    ...
    float zone = 0.5 + 0.5 * sin(centerDist * 9.9398 - anim);
    ...
    float grainFrame = mod(floor(grainClock * 1.8847), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2105), 0.0, 1.5);
