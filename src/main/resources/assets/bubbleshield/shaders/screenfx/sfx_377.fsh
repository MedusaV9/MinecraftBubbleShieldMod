...
// for effect 377. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:starburst:surge:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(anim * 8.0496), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0356;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2135), 0.0, 1.5);
