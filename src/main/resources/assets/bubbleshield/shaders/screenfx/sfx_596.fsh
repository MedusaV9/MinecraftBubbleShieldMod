...
// for effect 596. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:steady:sparkle]
...
void main() {
    ...
    float jitter = (hash11(row + frame * 91.7) - 0.5) * 0.0022 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(jitter, 0.0)));
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.5463) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3803 * scan * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.6660);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8299, 1.0, sin(oClock * 9.5818 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9764, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3966;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1302), 0.0, 1.5);
