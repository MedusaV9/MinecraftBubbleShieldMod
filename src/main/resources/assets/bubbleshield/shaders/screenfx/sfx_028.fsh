...
// for effect 028. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:steady:sparkle]
...
void main() {
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.4834);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8664, 1.0, sin(anim * 1.5967 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9801, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2686;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1176), 0.0, 1.5);
