...
// for effect 574. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4970 + ParamsB.x * 3.1416);
    ...
    float frame = mod(floor(anim * 6.3873), 1024.0);
    float band = floor(texCoord.y * 25.9424);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0705 * glitchK * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.2029);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8052, 1.0, sin(oClock * 11.9683 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9806, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3177;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1528);
