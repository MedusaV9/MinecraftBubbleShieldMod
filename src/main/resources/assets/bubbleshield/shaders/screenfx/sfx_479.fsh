...
// for effect 479. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:rotating:steady:grain]
...
void main() {
    ...
    float splitAngle = anim * 0.2921;
    vec2 shift = vec2(cos(splitAngle), sin(splitAngle)) * ParamsA.y * 0.5543 * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.6213), 256.0);
