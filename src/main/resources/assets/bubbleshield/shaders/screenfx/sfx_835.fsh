...
// for effect 835. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:thermal:inverted:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9528 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Drifting hotspot field biases the reading before the ramp.
    float blob = vnoise(texCoord * 5.4158 + vec2(anim * 0.0426, anim * 0.0436));
    ...
    float heat = clamp(1.0 - baseLuma + (blob - 0.5) * 0.2871 * min(strength, 1.0), 0.0, 1.0);
    // False-color ramp: cold Secondary depths through the palette to a
    // capped hot peak (never pure white -- legibility ceiling).
    vec3 coldTone = Secondary.rgb * 0.1881;
    vec3 ramped = mix(coldTone, Secondary.rgb, smoothstep(0.0, 0.4589, heat));
    ramped = mix(ramped, Primary.rgb, smoothstep(0.3601, 0.7490, heat));
    ramped = mix(ramped, vec3(0.8107), smoothstep(0.8495, 1.0, heat));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2107);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0992), 0.0, 1.5);
