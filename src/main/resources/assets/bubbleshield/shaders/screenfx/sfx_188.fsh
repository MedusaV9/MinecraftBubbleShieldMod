...
// for effect 188. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:pond:surge:pulseglow]
...
void main() {
    ...
    vec2 off = dir * ring * fade * 0.0050 * ParamsA.y * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8717 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4521, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1017;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1769);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1936), 0.0, 1.5);
