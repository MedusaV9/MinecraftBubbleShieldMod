...
// for effect 817. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:tunnel:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0918 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Tunnel vision: color survives at the center, the rim goes ashen.
    float mask = smoothstep(0.1731, 0.6901, centerDist);
    ...
    float grainFrame = mod(floor(grainClock * 1.5584), 256.0);
