...
// for effect 816. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:breathe:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.3722 * sin(animRaw * 0.1067) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // The mosaic cell size breathes on GameTime.
    float mosaic = max(ParamsA.y * (1.0 + 0.3686 * sin(anim * 0.9047)), 1.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2121);
