...
// for effect 499. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:thermal:inverted:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.6574 + ParamsB.x * 6.2831);
    ...
    float oRim = smoothstep(0.4358, 1.0, centerDist);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1598);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2010), 0.0, 1.5);
