...
// for effect 106. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rgbdrift:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6474 + ParamsB.x * 3.1416);
    ...
    float grainFrame = mod(floor(anim * 7.9249), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1031), 0.0, 1.5);
