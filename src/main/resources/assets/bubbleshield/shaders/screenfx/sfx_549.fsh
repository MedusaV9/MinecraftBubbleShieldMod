...
// for effect 549. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:global:steady:grain]
...
void main() {
    ...
    float hueAngle = sin(anim * 0.2640 + ParamsB.x * 6.2831) * 0.7926 * strength;
    ...
    float grainFrame = mod(floor(grainClock * 2.0830), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1043);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1866), 0.0, 1.5);
