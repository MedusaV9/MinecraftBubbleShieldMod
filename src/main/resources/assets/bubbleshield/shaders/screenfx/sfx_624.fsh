...
// for effect 624. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.8411 * sin(animRaw * 0.1399) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float jitter = (hash11(row + frame * 91.7) - 0.5) * 0.0028 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(jitter, 0.0)));
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.4239) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3465 * scan * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.6198);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8192, 1.0, sin(oClock * 11.1316 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9814, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2771;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2167);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0611), 0.0, 1.5);
