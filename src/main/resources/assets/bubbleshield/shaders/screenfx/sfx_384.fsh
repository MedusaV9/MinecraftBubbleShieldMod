...
// for effect 384. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:steady:pulseglow]
...
void main() {
    ...
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.6076) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3941 * scan * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7158 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3883, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0819;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1636);
