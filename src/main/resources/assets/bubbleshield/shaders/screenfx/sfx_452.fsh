...
// for effect 452. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8450 + ParamsB.x * 6.2831);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7943 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4207, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1557;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1881);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0793), 0.0, 1.5);
