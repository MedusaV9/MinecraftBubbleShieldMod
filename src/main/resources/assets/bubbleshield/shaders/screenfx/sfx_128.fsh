...
// for effect 128. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:dither:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9180 + ParamsB.x * 6.2831);
    ...
    float dframe = mod(floor(anim * 4.8821), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0653), 0.0, 1.5);
