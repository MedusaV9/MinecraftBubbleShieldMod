...
// for effect 603. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:duotone:shifting:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6327 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // The duotone knee slides on GameTime, tides of shadow and light.
    float knee = 0.4852 + 0.1403 * sin(anim * 0.5147 + ParamsB.x * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2153);
