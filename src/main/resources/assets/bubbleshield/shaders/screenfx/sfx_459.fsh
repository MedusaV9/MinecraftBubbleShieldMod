...
// for effect 459. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:embers:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6661 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // Gentle haze plus sparse embers drifting up the screen.
    vec2 off = vec2(
        sin(texCoord.y * 69.6807 + anim * 3.6),
        cos(texCoord.x * 51.5776 + anim * 2.4) * 0.4
    ) * 0.0010 * ParamsA.y * animAmp;
    ...
    vec2 emberCell = floor(vec2(texCoord.x * 48.4274, texCoord.y * 43.4359 + anim * 1.5572));
    float ember = step(0.9924, hash21(vec2(emberCell.x, mod(emberCell.y, 256.0))));
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.2372);
    vec3 warm = scene * hazeTint;
    vec3 outColor = mix(scene, warm * mix(vec3(1.0), Primary.rgb, ParamsB.z), 0.6762);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.5089);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8397, 1.0, sin(oClock * 10.1015 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9861, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2874;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1332), 0.0, 1.5);
