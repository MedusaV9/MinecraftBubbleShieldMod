...
// for effect 367. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:surge:none]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6734 + ParamsB.x * 3.1416);
    ...
    float frame = mod(floor(anim * 6.2705), 1024.0);
    float rowRoll = hash11(floor(texCoord.y * 30.2684) * 7.31 + frame * 13.7);
    float colRoll = hash11(floor(texCoord.x * 22.4905) * 5.13 + frame * 7.9);
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.3454 * ParamsA.y * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.3568 * ParamsA.y * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1323);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1256), 0.0, 1.5);
