...
// for effect 770. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.2979 * sin(animRaw * 0.1433) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frame = mod(floor(anim * 5.8662), 1024.0);
    vec2 cell = floor(texCoord * vec2(6.8210, 7.7433));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.8914
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0259 * glitchK
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1011);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0769), 0.0, 1.5);
