...
// for effect 144. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:steady:sparkle]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.6430) * strength;

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 13.5411);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8667, 1.0, sin(anim * 2.3821 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9757, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2605;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1116);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1604), 0.0, 1.5);
