...
// for effect 231. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3575 + ParamsB.x * 3.1416);
    ...
    float grainFrame = mod(floor(anim * 8.9629), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0372;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1710);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0881), 0.0, 1.5);
