...
// for effect 343. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:pulse:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.9659 * sin(animRaw * 0.1679) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float breath = 0.6355 + 0.4360 * sin(anim * 1.9813 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(anim * 8.9830), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0349;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1001);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1382), 0.0, 1.5);
