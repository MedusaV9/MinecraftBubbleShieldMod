...
// for effect 314. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:veins:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.0775 * sin(animRaw * 0.1929) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Ridged ice veins crawl across the whole pane.
    float ridge = 1.0 - abs(2.0 * vnoise(texCoord * ParamsA.z + vec2(anim * 0.0219, 0.0)) - 1.0);
    float crystals = ridge * 0.7 + vnoise(texCoord * ParamsA.z * 2.3) * 0.3;
    float frost = smoothstep(0.6554, 0.9358, crystals * (0.75 + 0.25 * centerDist)) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5642);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3278 * crystals);
    float glint = smoothstep(0.8467, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 0.8587 + crystals * 37.0));
    ...
    float grainFrame = mod(floor(anim * 6.1700), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0201;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1849);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1797), 0.0, 1.5);
