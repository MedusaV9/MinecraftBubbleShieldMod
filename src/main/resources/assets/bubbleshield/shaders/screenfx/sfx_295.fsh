...
// for effect 295. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:steady:pulseglow]
...
void main() {
    ...
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.4741, 1.0), vec2(1.0));
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6960 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4498, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1386;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1294);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1130), 0.0, 1.5);
