...
// for effect 489. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:spin:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5097 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // 12-tap tangential spin streak around the center.
    float arc = 0.0524 * ParamsA.y * animAmp * smoothstep(0.1, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 12; i++) {
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.8851);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8203, 1.0, sin(oClock * 14.2576 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9757, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2939;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1990);
