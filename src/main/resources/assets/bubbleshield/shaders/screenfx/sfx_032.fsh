...
// for effect 032. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:twin:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0407 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Two off-center ripple sources interfere across the frame.
    vec2 d1 = texCoord - vec2(0.2179, 0.3088);
    vec2 d2 = texCoord - vec2(0.6850, 0.6533);
    ...
    vec2 off = (safeNormalize(d1) + safeNormalize(d2)) * ring * fade * 0.0062 * ParamsA.y * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 16.6119);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8659, 1.0, sin(anim * 2.3288 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9807, oTw);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2160);
