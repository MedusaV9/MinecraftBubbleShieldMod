...
// for effect 037. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:linear:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4825 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // Fixed-axis fringe: the whole frame splits along one direction.
    vec2 shift = vec2(0.9968, 0.0802) * ParamsA.y * 0.5583 * animAmp;
    ...
    float grainFrame = mod(floor(anim * 8.3300), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2093), 0.0, 1.5);
