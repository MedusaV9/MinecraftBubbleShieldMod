...
// for effect 317. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:twin:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0369 + ParamsB.x * 6.2831);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9011 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4232, 1.0, centerDist);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1168), 0.0, 1.5);
