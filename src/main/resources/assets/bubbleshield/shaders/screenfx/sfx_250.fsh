...
// for effect 250. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:starburst:surge:none]
...
void main() {
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1474);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1429), 0.0, 1.5);
