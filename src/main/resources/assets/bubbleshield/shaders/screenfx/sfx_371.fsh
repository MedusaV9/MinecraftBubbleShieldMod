...
// for effect 371. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:pond:surge:grain]
...
void main() {
    ...
    vec2 off = dir * ring * fade * 0.0068 * ParamsA.y * animAmp;
    ...
    float grainFrame = mod(floor(anim * 8.9747), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0324;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1504);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1134), 0.0, 1.5);
