...
// for effect 688. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:kaleido:wedge4:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5426 + ParamsB.x * 3.1416);
    ...
    float local = mod(angle + anim * 0.1259, wedge) - wedge * 0.5;
    float folded = abs(local);
    vec2 dir = vec2(cos(folded + anim * 0.0646), sin(folded + anim * 0.0782));
    ...
    float seam = invsmooth(0.0, 0.0863, abs(local)) * smoothstep(0.05, 0.25, centerDist);
    vec3 outColor = scene + mix(Primary.rgb, Secondary.rgb, texCoord.y) * seam * 0.3385 * strength;
    ...
    float grainFrame = mod(floor(grainClock * 1.6262), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0392;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1677);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1240), 0.0, 1.5);
