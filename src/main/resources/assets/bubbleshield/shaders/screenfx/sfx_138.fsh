...
// for effect 138. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0029 + ParamsB.x * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1114);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1269), 0.0, 1.5);
