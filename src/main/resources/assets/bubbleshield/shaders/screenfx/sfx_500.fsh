...
// for effect 500. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:ink:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7580 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 4-tap cross-difference Sobel over scene luma.
    vec2 texel = 1.5480 / safeInSize;
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 3.9928, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5880 + 0.3526 * baseLuma;
    vec3 paper = vec3(flatTone) * mix(vec3(1.0), Secondary.rgb, ParamsB.z);
    vec3 inked = mix(paper, Primary.rgb * 0.1421, edge);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2160), 0.0, 1.5);
