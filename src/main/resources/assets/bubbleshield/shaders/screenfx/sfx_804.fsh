...
// for effect 804. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:spectral:orbit:steady:grain]
...
void main() {
    ...
    float orbitAng = anim * 0.1683 + ParamsB.x * 6.2831;
    vec2 orbitDir = vec2(cos(orbitAng), sin(orbitAng)) * 0.0120 * animAmp;
    vec3 g1 = sampleAt(texCoord + safeOffset(orbitDir));
    vec3 g2 = sampleAt(texCoord + safeOffset(-orbitDir * 1.6955));
    float w1 = 0.3534 * echoK;
    ...
    float pallor = 0.3446 + 0.1752 * sin(anim * 0.2177 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(grainClock * 2.1520), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1386);
