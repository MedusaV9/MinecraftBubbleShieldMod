...
// for effect 649. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:shadows:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.9056 * sin(animRaw * 0.1965) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Shadow drain: dark areas lose their color first, highlights stay alive.
    float mask = smoothstep(0.2456, 0.9117, 1.0 - baseLuma);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.7399);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8652, 1.0, sin(oClock * 7.6389 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9883, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2986;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1712);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1462), 0.0, 1.5);
