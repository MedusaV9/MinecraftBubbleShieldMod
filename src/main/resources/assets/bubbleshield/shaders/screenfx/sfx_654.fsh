...
// for effect 654. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.0320 * sin(animRaw * 0.1161) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frost = clamp(0.3301 + 0.5060 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5815);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3081 * crystals);
    float glint = smoothstep(0.8404, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.0460 + crystals * 37.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.4638);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8482, 1.0, sin(oClock * 10.7607 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9835, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2643;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1277);
