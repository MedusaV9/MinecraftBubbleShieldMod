...
// for effect 355. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:kaleido:wedge6:steady:sparkle]
...
void main() {
    ...
    float local = mod(angle + anim * 0.0871, wedge) - wedge * 0.5;
    float folded = abs(local);
    vec2 dir = vec2(cos(folded + anim * 0.0723), sin(folded + anim * 0.0743));
    ...
    float seam = invsmooth(0.0, 0.0807, abs(local)) * smoothstep(0.05, 0.25, centerDist);
    vec3 outColor = scene + mix(Primary.rgb, Secondary.rgb, texCoord.y) * seam * 0.2334 * strength;

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 13.8437);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8151, 1.0, sin(anim * 1.7828 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9773, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3352;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0816), 0.0, 1.5);
