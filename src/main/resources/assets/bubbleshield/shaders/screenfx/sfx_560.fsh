...
// for effect 560. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:tint:breathe:steady:grain]
...
void main() {
    ...
    float curve = pow(baseLuma, 0.9036 + 0.1552 * sin(anim * 0.7287));
    vec3 graded = mix(Secondary.rgb, Primary.rgb, curve) * (0.3797 + 0.6115 * curve);
    ...
    float grainFrame = mod(floor(grainClock * 2.1297), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0371;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1756);
