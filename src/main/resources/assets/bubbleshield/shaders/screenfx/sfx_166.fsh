...
void main() {
    ...
    float frame = mod(floor(anim * 6.0702), 1024.0);
    ...
    vec2 jitter = cellRoll > 0.8856
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0371 * glitchK
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1948);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2115), 0.0, 1.5);
