...
// for effect 242. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:steady:none]
...
void main() {
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1654);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1492), 0.0, 1.5);
