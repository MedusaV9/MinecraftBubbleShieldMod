...
// for effect 819. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:pond:surge:sparkle]
...
void main() {
    ...
    vec2 off = dir * ring * fade * 0.0063 * ParamsA.y * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.6723);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8405, 1.0, sin(oClock * 10.9067 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9783, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3373;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0652), 0.0, 1.5);
