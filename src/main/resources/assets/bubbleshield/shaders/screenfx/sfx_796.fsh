...
// for effect 796. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:breathe:steady:grain]
...
void main() {
    ...
    float levels = max(2.0, ParamsA.y + sin(anim * 0.7026) * 1.6656);
    ...
    float grainFrame = mod(floor(grainClock * 1.8536), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0312;
