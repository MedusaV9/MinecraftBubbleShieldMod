...
// for effect 392. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:steady:sparkle]
...
void main() {
    ...
    float frame = mod(floor(anim * 6.2330), 1024.0);
    float rowRoll = hash11(floor(texCoord.y * 26.1414) * 7.31 + frame * 13.7);
    float colRoll = hash11(floor(texCoord.x * 23.2293) * 5.13 + frame * 7.9);
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.4397 * ParamsA.y * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.3156 * ParamsA.y * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.4878);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8241, 1.0, sin(anim * 1.3444 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9879, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3388;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1961), 0.0, 1.5);
