...
// for effect 033. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:surge:pulseglow]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5291 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // 45-degree diamond cells via a skewed grid.
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.3118, 1.0), vec2(1.0));
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7375 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3613, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0937;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1517);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1445), 0.0, 1.5);
