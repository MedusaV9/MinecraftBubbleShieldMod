...
// for effect 518. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rgbdrift:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.3347 * sin(animRaw * 0.1636) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frame = mod(floor(anim * 4.6362), 1024.0);
    float wander = hash11(frame * 17.3) - 0.5;
    float split = (0.004 + abs(wander) * 0.0159) * glitchK * animAmp;
    ...
    float microTear = step(0.9723, hash11(floor(texCoord.y * 90.0) + frame)) * glitchK;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 16.2787);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8701, 1.0, sin(oClock * 8.0187 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9872, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3196;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1273);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1671), 0.0, 1.5);
