...
// for effect 083. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:diag:surge:grain]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.36
        ...
        + brightTap(texCoord + safeOffset(vec2(-texel.x, texel.y)), ParamsA.w) * 0.16;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.6224) * strength;
    ...
    float grainFrame = mod(floor(anim * 7.8305), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2155);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0616), 0.0, 1.5);
