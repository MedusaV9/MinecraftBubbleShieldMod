...
// for effect 739. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:rising:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.5500 * sin(animRaw * 0.1404) * ParamsB.y;
    float animAmp = 1.0;
    ...
    vec2 off = vec2(
        sin(texCoord.y * 71.6121 + anim * 4.0) + sin(texCoord.y * 43.6349 - anim * 2.6),
        cos(texCoord.x * 56.9229 + anim * 3.1) * 0.4
    ) * 0.0017 * ParamsA.y * animAmp * (0.4 + 0.6 * rising);
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.1649);
    vec3 warm = scene * hazeTint;
    vec3 outColor = mix(scene, warm * mix(vec3(1.0), Primary.rgb, ParamsB.z), 0.5990);
    ...
    float grainFrame = mod(floor(grainClock * 1.7586), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0210;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2137);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1650), 0.0, 1.5);
