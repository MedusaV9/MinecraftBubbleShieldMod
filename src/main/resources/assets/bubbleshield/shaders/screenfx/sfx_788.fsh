...
// for effect 788. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:wide:steady:sparkle]
...
void main() {
    ...
    vec2 mosaicInSize = max(safeInSize / max(ParamsA.y * vec2(2.2400, 1.0), vec2(1.0)), vec2(1.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 14.5620);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8152, 1.0, sin(oClock * 14.4348 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9817, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3853;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1188);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0626), 0.0, 1.5);
