...
// for effect 729. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:swirl:drift:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(grainClock * 2.4467), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1579);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1154), 0.0, 1.5);
