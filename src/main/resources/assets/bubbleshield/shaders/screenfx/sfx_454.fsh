...
// for effect 454. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:ring:surge:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(grainClock * 2.4409), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0358;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2131), 0.0, 1.5);
