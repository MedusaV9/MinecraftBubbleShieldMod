...
void main() {
    ...
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.4379 * glitchK * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.3837 * glitchK * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1074);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0884), 0.0, 1.5);
