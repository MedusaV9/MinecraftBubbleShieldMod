...
// for effect 630. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9383 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 4.7580), 1024.0);
    vec2 cell = floor(texCoord * vec2(8.2360, 5.0920));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.8860
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0367 * glitchK
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    float grainFrame = mod(floor(grainClock * 2.1887), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0297;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1724);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2038), 0.0, 1.5);
