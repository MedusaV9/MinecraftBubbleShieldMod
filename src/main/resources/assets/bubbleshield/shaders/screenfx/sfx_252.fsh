...
// for effect 252. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.9470 * sin(animRaw * 0.1451) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 45-degree diamond cells via a skewed grid.
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.5425, 1.0), vec2(1.0));
    ...
    float grainFrame = mod(floor(anim * 6.1229), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1011), 0.0, 1.5);
