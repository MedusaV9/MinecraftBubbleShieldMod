...
// for effect 445. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:starburst:spinning:steady:grain]
...
void main() {
    ...
    vec2 texel = 4.2383 / safeInSize;
    float thr = max(ParamsA.w, 0.4947);
    float wheel = anim * 0.0823;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        streak += brightTap(texCoord + safeOffset(arm * 2.1679), thr) * 0.06;
    }
    // Additive calibration: capped strength, attenuated on bright scenes.
    vec3 outColor = base + streak * mix(vec3(1.0), Primary.rgb, 0.6497) * min(strength, 1.0) * (1.0 - 0.4395 * baseLuma);
    ...
    float grainFrame = mod(floor(grainClock * 2.1321), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0217;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2165);
