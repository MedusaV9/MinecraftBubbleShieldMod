...
// for effect 154. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7446 + ParamsB.x * 6.2831);
    ...
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.5295) * 6.2831);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1719), 0.0, 1.5);
