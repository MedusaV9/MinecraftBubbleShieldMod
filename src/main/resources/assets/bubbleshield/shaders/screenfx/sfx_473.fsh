...
// for effect 473. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:starburst:four:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4171 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // 4-arm starburst: 3 decaying bright-pass taps down each arm.
    vec2 texel = 4.1649 / safeInSize;
    float thr = max(ParamsA.w, 0.3827);
    vec3 streak = base * max(baseLuma - thr, 0.0) * 0.3;
    for (int i = 0; i < 4; i++) {
        float armAng = float(i) * 1.5708 + 0.4772;
        vec2 arm = vec2(cos(armAng), sin(armAng)) * texel;
        streak += brightTap(texCoord + safeOffset(arm), thr) * 0.1;
        streak += brightTap(texCoord + safeOffset(arm * 2.3228), thr) * 0.06;
        streak += brightTap(texCoord + safeOffset(arm * 3.4680), thr) * 0.035;
    }
    // Additive calibration: capped strength, attenuated on bright scenes.
    vec3 outColor = base + streak * mix(vec3(1.0), Primary.rgb, 0.6845) * min(strength, 1.0) * (1.0 - 0.3991 * baseLuma);
    ...
    float grainFrame = mod(floor(grainClock * 2.4915), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0326;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1608);
