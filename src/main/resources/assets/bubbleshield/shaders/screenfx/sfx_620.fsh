...
// for effect 620. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7826 + ParamsB.x * 6.2831);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.4011);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8386, 1.0, sin(oClock * 14.8749 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9787, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3854;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1865);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1080), 0.0, 1.5);
