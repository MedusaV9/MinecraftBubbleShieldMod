...
// for effect 152. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:drift:none]
...
void main() {
    ...
    float anim = animRaw + 1.5093 * sin(animRaw * 0.1737) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 45-degree diamond cells via a skewed grid.
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.4666, 1.0), vec2(1.0));
