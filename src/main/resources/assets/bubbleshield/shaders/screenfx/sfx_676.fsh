...
// for effect 676. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 1.7412 * sin(animRaw * 0.1704) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 45-degree diamond cells via a skewed grid.
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.3599, 1.0), vec2(1.0));
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6743 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4687, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1306;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2029);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1442), 0.0, 1.5);
