...
// for effect 661. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:radial:steady:grain]
...
void main() {
    ...
    float reach = smoothstep(0.1231, 0.7915, centerDist);
    float hueAngle = sin(anim * 0.3302) * 0.8840 * strength * reach;
    ...
    float grainFrame = mod(floor(grainClock * 1.8840), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1674);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1973), 0.0, 1.5);
