...
// for effect 359. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:steady:sparkle]
...
void main() {
    ...
    float jitter = (hash11(col + frame * 47.3) - 0.5) * 0.0033 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(0.0, jitter)));
    float scan = 0.5 + 0.5 * sin((texCoord.x * lineCount + anim * 0.3378) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3780 * scan * animAmp;
    vec3 outColor = mix(scene * darken, scene * darken * Primary.rgb, ParamsB.z);

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 13.7051);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8453, 1.0, sin(anim * 1.7270 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9844, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2823;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1071);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1405), 0.0, 1.5);
