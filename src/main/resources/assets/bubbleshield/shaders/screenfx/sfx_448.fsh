...
// for effect 448. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:tint:splitlat:pulse:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(grainClock * 2.4468), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0388;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0798), 0.0, 1.5);
