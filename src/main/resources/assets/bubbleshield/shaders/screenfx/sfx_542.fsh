...
// for effect 542. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:steady:sparkle]
...
void main() {
    ...
    float frost = clamp(0.3231 + 0.4721 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5240);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2520 * crystals);
    float glint = smoothstep(0.7840, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.4717 + crystals * 37.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.6429);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8618, 1.0, sin(oClock * 9.9216 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9820, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2698;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1158);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1757), 0.0, 1.5);
