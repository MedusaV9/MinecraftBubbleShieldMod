...
// for effect 117. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:steady:grain]
...
void main() {
    ...
    float frame = mod(floor(anim * 7.9684), 1024.0);
    float band = floor(texCoord.y * 19.6681);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0537 * ParamsA.y * animAmp;
    ...
    float grainFrame = mod(floor(anim * 5.6003), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1085);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0635), 0.0, 1.5);
