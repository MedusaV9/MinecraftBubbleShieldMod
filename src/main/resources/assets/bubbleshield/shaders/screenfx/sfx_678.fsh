...
// for effect 678. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.4727 * sin(animRaw * 0.1053) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 5-tap cross blur of the bright pass, weighted toward the center.
    vec2 texel = 4.8254 / safeInSize;
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.6301) * strength;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 13.4429);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8606, 1.0, sin(oClock * 11.1030 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9790, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3265;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1634);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1034), 0.0, 1.5);
