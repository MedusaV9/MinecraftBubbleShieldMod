...
// for effect 702. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vignette:noisy:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0809 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Noise-eaten vignette: the creep line wanders and shimmers.
    float n = vnoise(texCoord * 7.9011 + vec2(anim * 0.0854, 0.0));
    float edge = smoothstep(0.3143, 0.7086, centerDist + (n - 0.5) * 0.1990);
    vec3 outColor = mix(base, Primary.rgb * (0.8263 + 0.3118 * n), edge * clamp(strength, 0.0, 1.0));
    ...
    float grainFrame = mod(floor(grainClock * 1.7491), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1827), 0.0, 1.5);
