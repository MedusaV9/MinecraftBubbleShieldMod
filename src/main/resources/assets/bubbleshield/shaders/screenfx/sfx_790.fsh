...
// for effect 790. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:diag:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.9997 * sin(animRaw * 0.1646) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // X-shaped bright-pass blur: diagonal streaks around hot pixels.
    vec2 texel = 3.1808 / safeInSize;
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.36
        ...
        + brightTap(texCoord + safeOffset(vec2(-texel.x, texel.y)), ParamsA.w) * 0.16;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.5226) * strength;
    ...
    float grainFrame = mod(floor(grainClock * 1.6567), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0350;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2137), 0.0, 1.5);
