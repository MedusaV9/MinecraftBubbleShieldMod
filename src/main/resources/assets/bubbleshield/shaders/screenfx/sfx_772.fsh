...
// for effect 772. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:kaleido:mirror:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4412 + ParamsB.x * 3.1416);
    ...
    float axisPos = 0.5 + 0.1826 * sin(anim * 0.2642);
    vec2 target = vec2(axisPos * 2.0 - texCoord.x, texCoord.y);
    vec3 scene = sampleAt(texCoord + safeOffset((target - texCoord) * strength));
    float seam = invsmooth(0.0, 0.0508, abs(texCoord.x - axisPos));
    vec3 outColor = scene + Primary.rgb * seam * 0.2245 * strength;
    ...
    float grainFrame = mod(floor(grainClock * 1.7460), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0292;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1134);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0601), 0.0, 1.5);
