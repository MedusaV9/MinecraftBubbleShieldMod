...
// for effect 614. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:dropout:steady:pulseglow]
...
void main() {
    ...
    float frame = mod(floor(anim * 0.2128), 1024.0);
    float row = floor(texCoord.y * 112.5613);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 321.3561 + anim * 1.3870) * 0.0016 * vhsK * animAmp;
    // Intermittent line dropouts: rare rows lose signal to pale static.
    float dropRoll = hash11(row * 7.31 + frame * 13.7);
    float drop = step(0.9570, dropRoll);
    ...
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.7364, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    float snowNoise = hash21(vec2(floor(texCoord.x * 176.5883), row + frame));
    taped = mix(taped, vec3(0.8871) * (0.6 + 0.4 * snowNoise), drop * 0.6997 * vhsK);
    vec3 outColor = mix(taped, taped * Primary.rgb, ParamsB.z);

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8707 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3901, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1540;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1117);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1432), 0.0, 1.5);
