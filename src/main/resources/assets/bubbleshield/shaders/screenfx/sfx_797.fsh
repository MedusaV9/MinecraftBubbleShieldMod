...
// for effect 797. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:spin:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7225 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 10-tap tangential spin streak around the center.
    float arc = 0.0520 * ParamsA.y * animAmp * smoothstep(0.1, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 10; i++) {
    ...
    vec2 oCell = floor(texCoord * safeInSize / 13.7279);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8173, 1.0, sin(oClock * 7.6359 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9832, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3691;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1582);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1125), 0.0, 1.5);
