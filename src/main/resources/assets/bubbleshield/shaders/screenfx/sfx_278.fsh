...
// for effect 278. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.4319 * sin(animRaw * 0.1206) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frost = clamp(0.3343 + 0.4732 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5526);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2783 * crystals);
    float glint = smoothstep(0.8078, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.0493 + crystals * 37.0));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1061);
