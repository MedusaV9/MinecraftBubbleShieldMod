...
// for effect 434. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:steady:none]
...
void main() {
    ...
    float frame = mod(floor(anim * 5.9112), 1024.0);
    vec2 cell = floor(texCoord * vec2(7.5091, 6.7028));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.8807
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0299 * glitchK
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1103);
