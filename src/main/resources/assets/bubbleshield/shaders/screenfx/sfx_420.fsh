...
// for effect 420. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:tint:splitlat:drift:sparkle]
...
void main() {
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.1705);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8723, 1.0, sin(oClock * 13.0592 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9792, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2694;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0996), 0.0, 1.5);
