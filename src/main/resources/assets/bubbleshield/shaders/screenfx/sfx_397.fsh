...
// for effect 397. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:veins:steady:pulseglow]
...
void main() {
    ...
    float frost = smoothstep(0.6410, 0.8357, crystals * (0.75 + 0.25 * centerDist)) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5760);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2590 * crystals);
    float glint = smoothstep(0.8377, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 0.8475 + crystals * 37.0));
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9276 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4778, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0868;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1809);
