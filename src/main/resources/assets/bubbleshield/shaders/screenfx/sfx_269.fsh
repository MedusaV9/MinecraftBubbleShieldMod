...
void main() {
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.2147);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.7140);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1012);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1769), 0.0, 1.5);
