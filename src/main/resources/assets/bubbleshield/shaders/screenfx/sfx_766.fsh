...
// for effect 766. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:surge:none]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3943 + ParamsB.x * 3.1416);
    ...
    float frost = clamp(0.3474 + 0.5417 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5422);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2934 * crystals);
    float glint = smoothstep(0.8084, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.1291 + crystals * 37.0));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1342);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1045), 0.0, 1.5);
