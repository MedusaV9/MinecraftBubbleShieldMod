...
// for effect 706. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.8559 * sin(animRaw * 0.1616) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 5-tap cross blur of the bright pass, weighted toward the center.
    vec2 texel = 5.3703 / safeInSize;
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.5738) * strength;

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5967 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4917, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1192;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0763), 0.0, 1.5);
