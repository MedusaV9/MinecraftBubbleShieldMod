...
// for effect 514. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3504 + ParamsB.x * 3.1416);
    ...
    float frost = clamp(0.3281 + 0.5434 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5220);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2878 * crystals);
    float glint = smoothstep(0.8588, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 0.8198 + crystals * 37.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.1262);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8632, 1.0, sin(oClock * 9.7196 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9883, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3159;
