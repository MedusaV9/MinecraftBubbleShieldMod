...
// for effect 492. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:kaleido:wedge4:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7967 + ParamsB.x * 6.2831);
    ...
    float local = mod(angle + anim * 0.1181, wedge) - wedge * 0.5;
    float folded = abs(local);
    vec2 dir = vec2(cos(folded + anim * 0.0527), sin(folded + anim * 0.0661));
    ...
    float seam = invsmooth(0.0, 0.0672, abs(local)) * smoothstep(0.05, 0.25, centerDist);
    vec3 outColor = scene + mix(Primary.rgb, Secondary.rgb, texCoord.y) * seam * 0.2435 * strength;
    ...
    float grainFrame = mod(floor(grainClock * 2.0472), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0218;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1193);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0981), 0.0, 1.5);
