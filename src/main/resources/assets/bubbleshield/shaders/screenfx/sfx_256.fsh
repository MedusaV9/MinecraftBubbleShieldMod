...
// for effect 256. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:swirl:surge:none]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6074 + ParamsB.x * 3.1416);
    ...
    float swirl = sin(angle * 2.0000 + anim - centerDist * 8.2934);
    vec2 tangent = centerDist > 0.0001 ? vec2(-aspectCentered.y, aspectCentered.x) / centerDist : vec2(0.0);
    vec2 off = tangent * swirl * ParamsA.y * animAmp * smoothstep(0.05, 0.3836, centerDist);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1338), 0.0, 1.5);
