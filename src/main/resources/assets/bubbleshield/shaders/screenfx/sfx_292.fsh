...
// for effect 292. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:radial:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9150 + ParamsB.x * 6.2831);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.2227);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8110, 1.0, sin(anim * 1.5615 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9756, oTw);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1936);
