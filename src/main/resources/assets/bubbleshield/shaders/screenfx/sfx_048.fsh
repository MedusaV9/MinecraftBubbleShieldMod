...
// for effect 048. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:steady:grain]
...
void main() {
    ...
    vec3 outColor = base + Primary.rgb * pow(edge, 0.6127) * strength * 0.7662;
    ...
    float grainFrame = mod(floor(anim * 8.0480), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0208;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1556);
