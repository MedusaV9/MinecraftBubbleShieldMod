...
// for effect 784. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:tint:breathe:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.6656 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Breathing grade curve: the luma remap exponent oscillates on GameTime.
    float curve = pow(baseLuma, 0.9761 + 0.2940 * sin(anim * 0.6213));
    vec3 graded = mix(Secondary.rgb, Primary.rgb, curve) * (0.3870 + 0.6001 * curve);
    ...
    float grainFrame = mod(floor(grainClock * 1.8876), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2128);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1767), 0.0, 1.5);
