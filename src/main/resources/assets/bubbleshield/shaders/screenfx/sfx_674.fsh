...
// for effect 674. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vignette:box:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9652 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Squared-off frame vignette, graded top-to-bottom between the palette colors.
    vec2 axed = abs(centered) * 2.0;
    float boxDist = max(axed.x, axed.y * 1.1108);
    float edge = smoothstep(0.6176, 0.9314, boxDist);
    ...
    float grainFrame = mod(floor(grainClock * 1.6399), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1584);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1230), 0.0, 1.5);
