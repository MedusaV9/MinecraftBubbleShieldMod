...
// for effect 569. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1039 + ParamsB.x * 6.2831);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.1039);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8027, 1.0, sin(oClock * 7.9322 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9772, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3460;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1065);
