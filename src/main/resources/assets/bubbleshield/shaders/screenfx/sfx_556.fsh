...
// for effect 556. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:crosshatch:steady:grain]
...
void main() {
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 3.3540, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5687 + 0.3499 * baseLuma;
    ...
    float h1 = 0.5 + 0.5 * sin((texCoord.x * 0.6765 + texCoord.y * 0.7365) * 354.7466);
    float h2 = 0.5 + 0.5 * sin((texCoord.x * -0.7365 + texCoord.y * 0.6765) * 346.6638);
    float shadowMask = smoothstep(0.3700, 0.8099, 1.0 - baseLuma);
    float hatch = clamp(smoothstep(0.6, 1.0, h1) + smoothstep(0.65, 1.0, h2), 0.0, 1.0) * shadowMask;
    vec3 inked = mix(paper, Primary.rgb * 0.1702, clamp(edge + hatch * 0.5950, 0.0, 1.0));
    ...
    float grainFrame = mod(floor(grainClock * 1.8133), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0229;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1592);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0688), 0.0, 1.5);
