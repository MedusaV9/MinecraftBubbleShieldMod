...
// for effect 528. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:ink:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1727 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 4-tap cross-difference Sobel over scene luma.
    vec2 texel = 1.4846 / safeInSize;
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 4.0869, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5661 + 0.3434 * baseLuma;
    vec3 paper = vec3(flatTone) * mix(vec3(1.0), Secondary.rgb, ParamsB.z);
    vec3 inked = mix(paper, Primary.rgb * 0.1736, edge);
    ...
    float grainFrame = mod(floor(grainClock * 2.4012), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0244;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1935);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2171), 0.0, 1.5);
