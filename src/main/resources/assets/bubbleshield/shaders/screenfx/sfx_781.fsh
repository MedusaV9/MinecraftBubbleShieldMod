...
// for effect 781. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:starburst:five:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9436 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 5-arm starburst: an off-kilter iris star, 2 taps per arm.
    vec2 texel = 4.3549 / safeInSize;
    float thr = max(ParamsA.w, 0.4360);
    vec3 streak = base * max(baseLuma - thr, 0.0) * 0.3;
    for (int i = 0; i < 5; i++) {
        float armAng = float(i) * 1.2566 + 0.5599;
        vec2 arm = vec2(cos(armAng), sin(armAng)) * texel;
        streak += brightTap(texCoord + safeOffset(arm), thr) * 0.11;
        streak += brightTap(texCoord + safeOffset(arm * 2.5160), thr) * 0.055;
    }
    // Additive calibration: capped strength, attenuated on bright scenes.
    vec3 outColor = base + streak * mix(vec3(1.0), Primary.rgb, 0.6623) * min(strength, 1.0) * (1.0 - 0.3553 * baseLuma);
    ...
    float grainFrame = mod(floor(grainClock * 2.0127), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0249;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2190);
