...
// for effect 427. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:pond:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9073 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Noise-phased pond rings: raindrop-like irregular ripples.
    float phase = vnoise(texCoord * 3.0384) * 2.7371;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2181), 0.0, 1.5);
