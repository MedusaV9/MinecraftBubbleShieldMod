...
// for effect 011. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:tint:breathe:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3932 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // Breathing grade curve: the luma remap exponent oscillates on GameTime.
    float curve = pow(baseLuma, 1.1409 + 0.1676 * sin(anim * 0.6968));
    vec3 graded = mix(Secondary.rgb, Primary.rgb, curve) * (0.3803 + 0.5772 * curve);
    ...
    float grainFrame = mod(floor(anim * 5.1422), 256.0);
