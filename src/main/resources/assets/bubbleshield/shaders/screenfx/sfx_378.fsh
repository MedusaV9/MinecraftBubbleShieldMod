...
// for effect 378. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:pulse:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6168 + ParamsB.x * 3.1416);
    ...
    float breath = 0.5818 + 0.3667 * sin(anim * 1.6973 + ParamsB.x * 6.2831);
    vec3 outColor = base + Primary.rgb * edge * glowK * breath;

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 11.2417);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8602, 1.0, sin(anim * 1.4642 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9797, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3693;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0938), 0.0, 1.5);
