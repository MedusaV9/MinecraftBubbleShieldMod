...
// for effect 244. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:linear:steady:none]
...
void main() {
    ...
    vec2 shift = vec2(0.5359, 0.8443) * ParamsA.y * 0.5950 * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1949);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1673), 0.0, 1.5);
