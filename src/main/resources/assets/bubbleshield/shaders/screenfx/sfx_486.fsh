...
// for effect 486. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 1.9224 * sin(animRaw * 0.1684) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frost = clamp(0.3452 + 0.5408 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5893);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2808 * crystals);
    float glint = smoothstep(0.7890, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.4759 + crystals * 37.0));
    ...
    float grainFrame = mod(floor(grainClock * 1.5164), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0247;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1705);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0890), 0.0, 1.5);
