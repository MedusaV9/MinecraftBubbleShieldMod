...
// for effect 430. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7917 + ParamsB.x * 6.2831);
    ...
    float frost = clamp(0.3551 + 0.5511 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5011);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2904 * crystals);
    float glint = smoothstep(0.8252, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.1554 + crystals * 37.0));
    ...
    float grainFrame = mod(floor(grainClock * 1.9251), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0293;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1156);
