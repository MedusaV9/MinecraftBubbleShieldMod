...
// for effect 677. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:twotone:steady:grain]
...
void main() {
    ...
    vec3 toned = grey * mix(Secondary.rgb, Primary.rgb, smoothstep(0.1842, 0.8000, baseLuma));
    ...
    float grainFrame = mod(floor(grainClock * 1.9657), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1484), 0.0, 1.5);
