...
// for effect 646. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vignette:box:steady:sparkle]
...
void main() {
    ...
    float boxDist = max(axed.x, axed.y * 1.1987);
    float edge = smoothstep(0.5738, 1.0193, boxDist);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 13.5934);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8438, 1.0, sin(oClock * 10.5394 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9849, oTw);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1426);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1678), 0.0, 1.5);
