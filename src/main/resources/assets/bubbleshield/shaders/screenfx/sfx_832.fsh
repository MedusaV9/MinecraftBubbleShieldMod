...
// for effect 832. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:spectral:fade:drift:pulseglow]
...
void main() {
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9595 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3990, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1508;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2023);
