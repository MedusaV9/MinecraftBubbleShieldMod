...
// for effect 800. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:kaleido:wedge8:steady:sparkle]
...
void main() {
    ...
    float local = mod(angle + anim * 0.1352, wedge) - wedge * 0.5;
    float folded = abs(local);
    vec2 dir = vec2(cos(folded + anim * 0.0363), sin(folded + anim * 0.0683));
    ...
    float seam = invsmooth(0.0, 0.0819, abs(local)) * smoothstep(0.05, 0.25, centerDist);
    vec3 outColor = scene + mix(Primary.rgb, Secondary.rgb, texCoord.y) * seam * 0.2160 * strength;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.8086);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8384, 1.0, sin(oClock * 11.7959 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9878, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2679;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1151);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1726), 0.0, 1.5);
