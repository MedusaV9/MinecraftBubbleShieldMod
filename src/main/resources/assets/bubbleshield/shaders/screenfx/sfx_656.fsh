...
// for effect 656. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:dither:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.5136 * sin(animRaw * 0.1047) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float dframe = mod(floor(anim * 3.0717), 256.0);
    ...
    float grainFrame = mod(floor(grainClock * 2.1266), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0773), 0.0, 1.5);
