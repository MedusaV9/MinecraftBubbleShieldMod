...
// for effect 360. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:dither:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.6354 + ParamsB.x * 6.2831);
    ...
    float dframe = mod(floor(anim * 5.4743), 256.0);
    ...
    float grainFrame = mod(floor(anim * 7.9605), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0304;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1632);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0760), 0.0, 1.5);
