...
// for effect 511. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:twin:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.0842 * sin(animRaw * 0.1831) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Two off-center ripple sources interfere across the frame.
    vec2 d1 = texCoord - vec2(0.2772, 0.4688);
    vec2 d2 = texCoord - vec2(0.7264, 0.5390);
    ...
    float grainFrame = mod(floor(grainClock * 1.8449), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0366;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1295), 0.0, 1.5);
