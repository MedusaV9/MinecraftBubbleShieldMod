...
void main() {
    ...
    float seam = invsmooth(0.0, 0.0918, abs(local)) * smoothstep(0.05, 0.25, centerDist);
    ...
    float grainFrame = mod(floor(anim * 6.5612), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0227;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1626), 0.0, 1.5);
