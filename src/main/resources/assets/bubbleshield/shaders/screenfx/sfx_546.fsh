...
// for effect 546. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 1.5352 * sin(animRaw * 0.1627) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frame = mod(floor(anim * 7.9396), 1024.0);
    float band = floor(texCoord.y * 26.8225);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0786 * glitchK * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5168 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4762, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1165;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1465), 0.0, 1.5);
