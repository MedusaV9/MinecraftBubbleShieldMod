...
// for effect 172. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:twin:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.6085 * sin(animRaw * 0.1974) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Two off-center ripple sources interfere across the frame.
    vec2 d1 = texCoord - vec2(0.3545, 0.4745);
    vec2 d2 = texCoord - vec2(0.6717, 0.5230);
    ...
    vec2 off = (safeNormalize(d1) + safeNormalize(d2)) * ring * fade * 0.0060 * ParamsA.y * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5096 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3547, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1368;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1763);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1457), 0.0, 1.5);
