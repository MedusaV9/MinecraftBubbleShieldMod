...
// for effect 838. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:tracking:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9676 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    float vhsK = min(ParamsA.y, 1.0);
    float frame = mod(floor(anim * 0.1861), 1024.0);
    float row = floor(texCoord.y * 100.8060);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 331.4261 + anim * 1.1979) * 0.0015 * vhsK * animAmp;
    // A tracking band crawls up the tape and shears the rows it crosses.
    float bandPos = fract(anim * 0.0411);
    float bandMask = invsmooth(0.0, 0.0510, abs(texCoord.y - bandPos));
    float shear = (hash11(row * 3.7 + frame * 17.3) - 0.5) * 0.0219 * vhsK * bandMask;
    ...
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.4194, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    taped = mix(taped, vec3(luma(taped)) * 1.1810, bandMask * 0.4191 * vhsK);
    ...
    float grainFrame = mod(floor(grainClock * 1.7638), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1564);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1291), 0.0, 1.5);
