...
// for effect 137. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:dreamblur:cross5:steady:sparkle]
...
void main() {
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1336);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2134), 0.0, 1.5);
