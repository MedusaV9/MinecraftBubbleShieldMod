...
// for effect 482. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:diag:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.7391 * sin(animRaw * 0.1436) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // X-shaped bright-pass blur: diagonal streaks around hot pixels.
    vec2 texel = 5.5252 / safeInSize;
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.36
        ...
        + brightTap(texCoord + safeOffset(vec2(-texel.x, texel.y)), ParamsA.w) * 0.16;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.7420) * strength;

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5358 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4612, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1334;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1744), 0.0, 1.5);
