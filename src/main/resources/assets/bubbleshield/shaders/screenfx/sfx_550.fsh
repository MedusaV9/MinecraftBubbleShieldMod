...
// for effect 550. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:dreamblur:cross5:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.6308 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 5-tap cross blur haze with drifting sparkle dust.
    vec2 texel = 3.4271 / safeInSize;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6678 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4310, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1181;
