...
// for effect 184. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9711 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 45-degree diamond cells via a skewed grid.
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.3806, 1.0), vec2(1.0));
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2121), 0.0, 1.5);
