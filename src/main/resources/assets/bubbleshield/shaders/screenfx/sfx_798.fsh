...
// for effect 798. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7381 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 7.3809), 1024.0);
    float band = floor(texCoord.y * 20.1976);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0637 * glitchK * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7543 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4661, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0952;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2035), 0.0, 1.5);
