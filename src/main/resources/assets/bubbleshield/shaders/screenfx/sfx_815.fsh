...
// for effect 815. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:linear:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.3209 * sin(animRaw * 0.1186) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Fixed-axis fringe: the whole frame splits along one direction.
    vec2 shift = vec2(0.3305, 0.9438) * ParamsA.y * 0.4556 * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1523);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2094), 0.0, 1.5);
