...
// for effect 648. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:breathe:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6499 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // The mosaic cell size breathes on GameTime.
    float mosaic = max(ParamsA.y * (1.0 + 0.2977 * sin(anim * 0.6138)), 1.0);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.4789);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8419, 1.0, sin(oClock * 10.8593 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9834, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3044;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1991), 0.0, 1.5);
