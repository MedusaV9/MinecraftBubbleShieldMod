...
// for effect 493. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:radial:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8211 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Hue drift grows with distance from the center; the middle stays true.
    float reach = smoothstep(0.1520, 0.7305, centerDist);
    float hueAngle = sin(anim * 0.5476) * 0.8303 * strength * reach;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 13.3577);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8456, 1.0, sin(oClock * 13.9604 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9842, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2589;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1758), 0.0, 1.5);
