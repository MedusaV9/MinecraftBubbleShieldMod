...
// for effect 664. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:spectral:fade:surge:none]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5101 + ParamsB.x * 3.1416);
    ...
    float reach = 0.7 + 0.3 * sin(anim * 0.3324 + ParamsB.x * 6.2831);
    vec2 trail = vec2(0.7544, 0.6564) * 0.0098 * reach * animAmp;
    vec3 g1 = sampleAt(texCoord + safeOffset(trail));
    vec3 g2 = sampleAt(texCoord + safeOffset(trail * 2.0));
    float w1 = 0.3414 * echoK;
    float w2 = 0.1374 * echoK;
    vec3 haunted = base * (1.0 - w1 - w2) + g1 * w1 + g2 * w2;
    // Desaturation drift: a slow spirit-world pallor swings on GameTime.
    float pallor = 0.3796 + 0.1586 * sin(anim * 0.3748 + ParamsB.x * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1402);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2155), 0.0, 1.5);
