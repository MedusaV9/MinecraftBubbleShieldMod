...
// for effect 826. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 1.7980 * sin(animRaw * 0.1201) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frame = mod(floor(anim * 7.6348), 1024.0);
    float rowRoll = hash11(floor(texCoord.y * 25.3437) * 7.31 + frame * 13.7);
    float colRoll = hash11(floor(texCoord.x * 17.2367) * 5.13 + frame * 7.9);
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.4562 * glitchK * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.3709 * glitchK * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 2.4637), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0396;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1555);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2162), 0.0, 1.5);
