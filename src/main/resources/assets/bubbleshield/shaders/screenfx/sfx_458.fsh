...
// for effect 458. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6910 + ParamsB.x * 3.1416);
    ...
    float frost = clamp(0.3895 + 0.5643 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5720);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2644 * crystals);
    float glint = smoothstep(0.8061, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.1812 + crystals * 37.0));
    ...
    float grainFrame = mod(floor(grainClock * 1.6585), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0269;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1168);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1249), 0.0, 1.5);
