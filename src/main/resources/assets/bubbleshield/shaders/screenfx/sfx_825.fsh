...
// for effect 825. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:spin:steady:grain]
...
void main() {
    ...
    float arc = 0.0479 * ParamsA.y * animAmp * smoothstep(0.1, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 12; i++) {
    ...
    float grainFrame = mod(floor(grainClock * 2.3747), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0381;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1329);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1327), 0.0, 1.5);
