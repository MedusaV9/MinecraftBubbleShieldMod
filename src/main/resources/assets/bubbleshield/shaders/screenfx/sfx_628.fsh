...
// for effect 628. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:lumaonly:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.8000 * sin(animRaw * 0.1252) * ParamsB.y;
    float animAmp = 1.0;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1820);
