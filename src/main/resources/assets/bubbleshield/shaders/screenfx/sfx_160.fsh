...
// for effect 160. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:radial:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8901 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(anim * 6.8754), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0213;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0905), 0.0, 1.5);
