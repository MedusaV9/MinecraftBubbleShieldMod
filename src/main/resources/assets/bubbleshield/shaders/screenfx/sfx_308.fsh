...
// for effect 308. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5774 + ParamsB.x * 3.1416);
    ...
    vec3 outColor = base + Primary.rgb * pow(edge, 0.7671) * strength * 0.8808;

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 14.0364);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8376, 1.0, sin(anim * 1.3942 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9827, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2813;
