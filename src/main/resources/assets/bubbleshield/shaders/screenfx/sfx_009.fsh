...
void main() {
    ...
    float grainFrame = mod(floor(anim * 7.5269), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0291;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1213), 0.0, 1.5);
