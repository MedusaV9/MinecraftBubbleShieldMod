...
// for effect 735. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:linear:steady:grain]
...
void main() {
    ...
    vec2 dir = vec2(0.8198, 0.5726);
    float ring = sin(dot(texCoord, dir) * ParamsA.z - anim * 2.5091);
    float fade = invsmooth(0.35, 1.0, centerDist);
    vec2 off = dir * ring * fade * 0.0086 * ParamsA.y * animAmp;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1076), 0.0, 1.5);
