...
// for effect 785. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:swirl:steady:pulseglow]
...
void main() {
    ...
    float swirl = sin(angle * 4.0000 + anim - centerDist * 6.5197);
    vec2 tangent = centerDist > 0.0001 ? vec2(-aspectCentered.y, aspectCentered.x) / centerDist : vec2(0.0);
    vec2 off = tangent * swirl * ParamsA.y * animAmp * smoothstep(0.05, 0.4902, centerDist);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5615 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4846, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1376;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1785);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0902), 0.0, 1.5);
