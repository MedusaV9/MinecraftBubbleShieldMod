...
// for effect 132. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6498 + ParamsB.x * 3.1416);
    ...
    float jitter = (hash11(row + frame * 91.7) - 0.5) * 0.0026 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(jitter, 0.0)));
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.4839) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3267 * scan * animAmp;
    ...
    float grainFrame = mod(floor(anim * 7.6258), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0687), 0.0, 1.5);
