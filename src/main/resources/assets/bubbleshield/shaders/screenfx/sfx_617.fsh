...
// for effect 617. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:harmonic:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9855 + ParamsB.x * 6.2831);
    ...
    vec2 off = (vec2(sin(texCoord.y * ParamsA.z + anim), cos(texCoord.x * ParamsA.z + anim))
        + 0.4782 * vec2(sin(texCoord.y * 55.1892 - anim * 1.7),
            cos(texCoord.x * 54.4264 + anim * 1.3))) * ParamsA.y * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 2.1193), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1731);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1984), 0.0, 1.5);
