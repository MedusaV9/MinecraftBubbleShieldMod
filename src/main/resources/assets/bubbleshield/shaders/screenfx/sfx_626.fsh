...
// for effect 626. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:steady:pulseglow]
...
void main() {
    ...
    float frost = clamp(0.3358 + 0.5208 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5275);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2659 * crystals);
    float glint = smoothstep(0.7895, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.0566 + crystals * 37.0));
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9474 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4140, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1548;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2151);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0899), 0.0, 1.5);
