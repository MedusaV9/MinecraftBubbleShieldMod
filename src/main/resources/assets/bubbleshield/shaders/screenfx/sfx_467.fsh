...
// for effect 467. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:moire:linring:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.4982 * sin(animRaw * 0.1347) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // A line grating against a ring grating: spiral-ish beat patterns.
    float g1 = sin((texCoord.x * 0.7802 + texCoord.y * 0.6255) * ParamsA.z + anim * 0.5968);
    float g2 = sin(centerDist * ParamsA.z * 1.0865 - anim * 0.5155);
    float inter = g1 * g2;
    float mask = smoothstep(-0.2, 1.0, inter);
    vec3 outColor = base * (1.0 - 0.3901 * strength * (1.0 - mask));
    ...
    float grainFrame = mod(floor(grainClock * 1.5024), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1509);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1169), 0.0, 1.5);
