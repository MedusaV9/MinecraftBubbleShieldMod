...
// for effect 469. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:cornered:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 1.7698 * sin(animRaw * 0.1646) * ParamsB.y;
    float animAmp = 1.0;
    ...
    vec2 warp = centered * r2 * 0.0397 * min(strength, 1.0);
    vec2 fringe = centered * r2 * r2 * 0.0310 * min(strength, 1.0) * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.6421);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8556, 1.0, sin(oClock * 9.3493 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9779, oTw);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2040);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0966), 0.0, 1.5);
