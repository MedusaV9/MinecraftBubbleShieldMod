...
// for effect 653. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:surge:pulseglow]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3987 + ParamsB.x * 3.1416);
    ...
    vec3 outColor = base + Primary.rgb * pow(edge, 0.6647) * glowK * 0.7724;

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9086 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3807, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1225;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1366), 0.0, 1.5);
