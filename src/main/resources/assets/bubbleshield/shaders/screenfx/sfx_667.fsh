...
// for effect 667. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:thermal:hotspots:steady:grain]
...
void main() {
    ...
    float blob = vnoise(texCoord * 4.9688 + vec2(anim * 0.0680, anim * 0.0453));
    // A second, tighter octave makes distinct wandering hot blobs.
    float blobB = vnoise(texCoord * 8.1556 - vec2(anim * 0.0493, anim * 0.0864));
    float thermalMix = clamp(strength, 0.0, 0.85);
    float heat = clamp(baseLuma + (blob * 0.6 + blobB * 0.4 - 0.5) * 0.3977 * min(strength, 1.0), 0.0, 1.0);
    // False-color ramp: cold Secondary depths through the palette to a
    // capped hot peak (never pure white -- legibility ceiling).
    vec3 coldTone = Secondary.rgb * 0.1958;
    vec3 ramped = mix(coldTone, Secondary.rgb, smoothstep(0.0, 0.4825, heat));
    ramped = mix(ramped, Primary.rgb, smoothstep(0.4245, 0.7550, heat));
    ramped = mix(ramped, vec3(0.8512), smoothstep(0.8628, 1.0, heat));
    ...
    float grainFrame = mod(floor(grainClock * 1.5168), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0397;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1262);
