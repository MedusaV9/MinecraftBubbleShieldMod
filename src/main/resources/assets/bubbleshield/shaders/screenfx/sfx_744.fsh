...
// for effect 744. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:kaleido:wedge8:steady:grain]
...
void main() {
    ...
    float local = mod(angle + anim * 0.1117, wedge) - wedge * 0.5;
    float folded = abs(local);
    vec2 dir = vec2(cos(folded + anim * 0.0526), sin(folded + anim * 0.0590));
    ...
    float seam = invsmooth(0.0, 0.0647, abs(local)) * smoothstep(0.05, 0.25, centerDist);
    vec3 outColor = scene + mix(Primary.rgb, Secondary.rgb, texCoord.y) * seam * 0.3289 * strength;
    ...
    float grainFrame = mod(floor(grainClock * 2.0805), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0359;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1129);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1552), 0.0, 1.5);
