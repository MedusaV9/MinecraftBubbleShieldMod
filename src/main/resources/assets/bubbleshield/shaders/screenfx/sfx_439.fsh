...
// for effect 439. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:moire:linlin:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4417 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // Two near-parallel line gratings beat into drifting moire fringes.
    float g1 = sin((texCoord.x * 0.6858 + texCoord.y * 0.7278) * ParamsA.z + anim * 0.5326);
    float g2 = sin((texCoord.x * 0.6193 + texCoord.y * 0.7852) * ParamsA.z * 1.0440 - anim * 0.4040);
    float inter = g1 * g2;
    float mask = smoothstep(-0.2, 1.0, inter);
    vec3 outColor = base * (1.0 - 0.3306 * strength * (1.0 - mask));
    ...
    float grainFrame = mod(floor(grainClock * 2.4461), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0400;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1216), 0.0, 1.5);
