...
// for effect 592. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:breathe:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4072 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // The mosaic cell size breathes on GameTime.
    float mosaic = max(ParamsA.y * (1.0 + 0.3452 * sin(anim * 0.6989)), 1.0);
    ...
    float grainFrame = mod(floor(grainClock * 1.5259), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1830);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1639), 0.0, 1.5);
