...
// for effect 679. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:twin:surge:pulseglow]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5600 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // Two off-center ripple sources interfere across the frame.
    vec2 d1 = texCoord - vec2(0.2651, 0.3831);
    vec2 d2 = texCoord - vec2(0.7386, 0.6973);
    ...
    vec2 off = (safeNormalize(d1) + safeNormalize(d2)) * ring * fade * 0.0069 * ParamsA.y * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7605 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4409, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1000;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2085), 0.0, 1.5);
