...
// for effect 769. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:zoom:drift:none]
...
void main() {
    ...
    float anim = animRaw + 1.9899 * sin(animRaw * 0.1668) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 10-tap zoom streak from the screen center outward.
    float blur = 0.0444 * ParamsA.y * animAmp * smoothstep(0.05, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 10; i++) {
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1648);
