...
// for effect 263. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:breathe:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.1778 * sin(animRaw * 0.1547) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // The mosaic cell size breathes on GameTime.
    float mosaic = max(ParamsA.y * (1.0 + 0.3566 * sin(anim * 0.7140)), 1.0);
    ...
    float grainFrame = mod(floor(anim * 6.9561), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0342;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0785), 0.0, 1.5);
