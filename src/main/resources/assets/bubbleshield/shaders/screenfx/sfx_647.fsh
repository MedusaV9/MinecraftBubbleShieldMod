...
// for effect 647. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:linear:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.2389 * sin(animRaw * 0.1606) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Fixed-axis fringe: the whole frame splits along one direction.
    vec2 shift = vec2(-0.2725, 0.9622) * ParamsA.y * 0.4677 * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.6343);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8160, 1.0, sin(oClock * 12.6085 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9791, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3821;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0774), 0.0, 1.5);
