...
// for effect 650. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:surge:sparkle]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.6535) * strength;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 13.3770);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8772, 1.0, sin(oClock * 9.5484 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9865, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3743;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1180);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1980), 0.0, 1.5);
