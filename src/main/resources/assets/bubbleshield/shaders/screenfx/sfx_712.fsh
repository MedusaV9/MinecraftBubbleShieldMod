...
// for effect 712. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:dither:steady:grain]
...
void main() {
    ...
    float dframe = mod(floor(anim * 5.9213), 256.0);
    ...
    float grainFrame = mod(floor(grainClock * 2.1180), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1694);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0926), 0.0, 1.5);
