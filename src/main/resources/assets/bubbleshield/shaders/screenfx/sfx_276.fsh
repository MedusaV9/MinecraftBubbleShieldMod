...
// for effect 276. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:pulse:drift:none]
...
void main() {
    ...
    float anim = animRaw + 1.9157 * sin(animRaw * 0.1901) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float breath = 0.5517 + 0.3002 * sin(anim * 1.9193 + ParamsB.x * 6.2831);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1357), 0.0, 1.5);
