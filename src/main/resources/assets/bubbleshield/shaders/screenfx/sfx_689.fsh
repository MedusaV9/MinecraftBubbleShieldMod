...
// for effect 689. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:split:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.0056 * sin(animRaw * 0.1306) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Shadows and highlights drift in opposite hue directions.
    float sgn = baseLuma * 2.0 - 1.0;
    float hueAngle = -sgn * sin(anim * 0.3839 + ParamsB.x * 6.2831) * 0.8463 * strength;
    ...
    float grainFrame = mod(floor(grainClock * 1.9936), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1095), 0.0, 1.5);
