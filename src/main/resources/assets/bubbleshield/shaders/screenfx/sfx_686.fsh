...
// for effect 686. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1381 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 6.2925), 1024.0);
    float band = floor(texCoord.y * 23.9549);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0694 * glitchK * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.5364);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8431, 1.0, sin(oClock * 13.7421 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9755, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3161;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2050);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1063), 0.0, 1.5);
