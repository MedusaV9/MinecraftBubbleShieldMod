...
// for effect 752. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:charcoal:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0833 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 4-tap cross-difference Sobel over scene luma.
    vec2 texel = 1.3713 / safeInSize;
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 3.2259, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5737 + 0.3006 * baseLuma;
    vec3 paper = vec3(flatTone) * mix(vec3(1.0), Secondary.rgb, ParamsB.z);
    // Charcoal: soft wide strokes broken up by a static grain mask.
    float soft = pow(edge, 0.5685);
    float grain = vnoise(texCoord * safeInSize * 0.0443);
    vec3 inked = mix(paper, Secondary.rgb * 0.2467, clamp(soft * (0.55 + 0.45 * grain), 0.0, 1.0));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1411);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2146), 0.0, 1.5);
