...
// for effect 453. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:shadows:surge:sparkle]
...
void main() {
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.5713);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8253, 1.0, sin(oClock * 14.5576 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9805, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3981;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1288);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1294), 0.0, 1.5);
