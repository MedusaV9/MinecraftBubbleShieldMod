...
// for effect 279. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9296 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 6.4750), 1024.0);
    float rowRoll = hash11(floor(texCoord.y * 29.7936) * 7.31 + frame * 13.7);
    float colRoll = hash11(floor(texCoord.x * 16.7073) * 5.13 + frame * 7.9);
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.3461 * ParamsA.y * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.2128 * ParamsA.y * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1835);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0660), 0.0, 1.5);
