...
// for effect 280. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:full:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8686 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Whole-frame heat shimmer, as if standing inside a furnace bloom.
    vec2 off = vec2(
        sin(texCoord.y * 64.0441 + anim * 3.4),
        sin(texCoord.x * 62.5272 - anim * 2.8)
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.1976);
    vec3 warm = scene * hazeTint;
    vec3 outColor = mix(scene, warm * mix(vec3(1.0), Primary.rgb, ParamsB.z), 0.5864);
    ...
    float grainFrame = mod(floor(anim * 7.8066), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0220;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0929), 0.0, 1.5);
