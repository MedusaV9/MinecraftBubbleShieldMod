...
// for effect 029. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:breathe:steady:grain]
...
void main() {
    ...
    float mosaic = max(ParamsA.y * (1.0 + 0.2589 * sin(anim * 0.5578)), 1.0);
    ...
    float grainFrame = mod(floor(anim * 6.1554), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1546);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0808), 0.0, 1.5);
