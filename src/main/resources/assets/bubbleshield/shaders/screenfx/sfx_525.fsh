...
// for effect 525. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:cornered:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9596 + ParamsB.x * 6.2831);
    ...
    vec2 warp = centered * r2 * 0.0420 * min(strength, 1.0);
    vec2 fringe = centered * r2 * r2 * 0.0367 * min(strength, 1.0) * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.9791), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1966);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1038), 0.0, 1.5);
