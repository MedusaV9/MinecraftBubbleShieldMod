...
// for effect 782. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:tracking:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0795 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    float vhsK = min(ParamsA.y, 1.0);
    float frame = mod(floor(anim * 0.2603), 1024.0);
    float row = floor(texCoord.y * 121.2321);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 292.8069 + anim * 1.1661) * 0.0012 * vhsK * animAmp;
    // A tracking band crawls up the tape and shears the rows it crosses.
    float bandPos = fract(anim * 0.0354);
    float bandMask = invsmooth(0.0, 0.0826, abs(texCoord.y - bandPos));
    float shear = (hash11(row * 3.7 + frame * 17.3) - 0.5) * 0.0274 * vhsK * bandMask;
    ...
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.6258, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    taped = mix(taped, vec3(luma(taped)) * 1.2003, bandMask * 0.3707 * vhsK);
    vec3 outColor = mix(taped, taped * Primary.rgb, ParamsB.z);

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5513 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3786, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1159;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1203);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1479), 0.0, 1.5);
