...
// for effect 513. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:steady:pulseglow]
...
void main() {
    ...
    vec3 outColor = base + Primary.rgb * pow(edge, 0.6758) * glowK * 0.7644;

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7645 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4394, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1244;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1793);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1221), 0.0, 1.5);
