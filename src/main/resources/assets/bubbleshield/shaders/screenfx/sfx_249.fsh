...
// for effect 249. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1895 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 6.8721), 1024.0);
    vec2 cell = floor(texCoord * vec2(9.6204, 4.9554));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.8807
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0395 * ParamsA.y
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1272);
