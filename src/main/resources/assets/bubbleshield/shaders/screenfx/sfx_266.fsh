...
// for effect 266. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:twin:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7292 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Two off-center ripple sources interfere across the frame.
    vec2 d1 = texCoord - vec2(0.2192, 0.4622);
    vec2 d2 = texCoord - vec2(0.6804, 0.6797);
    ...
    vec2 off = (safeNormalize(d1) + safeNormalize(d2)) * ring * fade * 0.0045 * ParamsA.y * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1630);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1672), 0.0, 1.5);
