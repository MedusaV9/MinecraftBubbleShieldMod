...
// for effect 589. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:diag:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8150 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // A single diagonal wavefront slides across the screen.
    vec2 axis = vec2(0.8389, 0.5443);
    float wave = sin(dot(texCoord, axis) * ParamsA.z * 1.4708 + anim);
    ...
    float grainFrame = mod(floor(grainClock * 1.7118), 256.0);
