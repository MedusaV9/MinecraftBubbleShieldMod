...
// for effect 820. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4078 + ParamsB.x * 3.1416);
    ...
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.5905) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3381 * scan * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 16.5909);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8067, 1.0, sin(oClock * 12.2534 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9866, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3654;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2193);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0683), 0.0, 1.5);
