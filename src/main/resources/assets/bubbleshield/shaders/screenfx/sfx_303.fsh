...
void main() {
    ...
    float frame = mod(floor(anim * 7.2415), 1024.0);
    ...
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.4150 * glitchK * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.2253 * glitchK * animAmp;
    ...
    float grainFrame = mod(floor(anim * 6.3514), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1783);
