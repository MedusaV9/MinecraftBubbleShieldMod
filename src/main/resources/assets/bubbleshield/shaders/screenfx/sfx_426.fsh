...
// for effect 426. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:diag:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.6704 * sin(animRaw * 0.1625) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // X-shaped bright-pass blur: diagonal streaks around hot pixels.
    vec2 texel = 5.3449 / safeInSize;
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.36
        ...
        + brightTap(texCoord + safeOffset(vec2(-texel.x, texel.y)), ParamsA.w) * 0.16;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.7284) * strength;
