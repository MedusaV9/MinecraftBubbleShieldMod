...
// for effect 810. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:worn:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0195 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    float vhsK = min(ParamsA.y, 1.0);
    float frame = mod(floor(anim * 0.2055), 1024.0);
    float row = floor(texCoord.y * 95.8489);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 271.4129 + anim * 1.1464) * 0.0018 * vhsK * animAmp;
    // Worn tape: per-row micro-jitter plus an all-over grain film.
    float rowJitter = (hash11(row + frame * 91.7) - 0.5) * 0.0024 * vhsK;
    ...
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.8893, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    float film = hash21(floor(texCoord * safeInSize * 0.5695) + vec2(frame, 0.0)) - 0.5;
    taped += vec3(film) * 0.0540 * vhsK;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.0944);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8394, 1.0, sin(oClock * 13.3106 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9893, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3898;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2053);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1979), 0.0, 1.5);
