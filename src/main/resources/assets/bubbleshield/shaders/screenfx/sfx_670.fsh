...
// for effect 670. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:tracking:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5543 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    float vhsK = min(ParamsA.y, 1.0);
    float frame = mod(floor(anim * 0.1822), 1024.0);
    float row = floor(texCoord.y * 113.3663);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 311.7664 + anim * 1.0239) * 0.0019 * vhsK * animAmp;
    // A tracking band crawls up the tape and shears the rows it crosses.
    float bandPos = fract(anim * 0.0428);
    float bandMask = invsmooth(0.0, 0.0675, abs(texCoord.y - bandPos));
    float shear = (hash11(row * 3.7 + frame * 17.3) - 0.5) * 0.0334 * vhsK * bandMask;
    ...
    float bleed = 0.0040 * vhsK;
    float red = sampleAt(texCoord + safeOffset(baseOff + vec2(bleed, 0.0))).r;
    float green = sampleAt(texCoord + safeOffset(baseOff)).g;
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.4940, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    taped = mix(taped, vec3(luma(taped)) * 1.1656, bandMask * 0.4096 * vhsK);
    ...
    float grainFrame = mod(floor(grainClock * 1.8985), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0311;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1360);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1489), 0.0, 1.5);
