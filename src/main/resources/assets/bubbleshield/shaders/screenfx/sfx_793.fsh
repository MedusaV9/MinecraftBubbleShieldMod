...
// for effect 793. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.1051 * sin(animRaw * 0.1323) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float grainFrame = mod(floor(grainClock * 1.9182), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1679);
