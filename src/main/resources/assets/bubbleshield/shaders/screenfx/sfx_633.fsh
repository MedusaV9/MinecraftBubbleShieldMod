...
// for effect 633. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:waves:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7450 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Bands of hue roll down the screen like slow chromatic weather.
    float hueAngle = sin(anim * 0.5776 + texCoord.y * 5.5988) * 0.8907 * strength;
    ...
    float grainFrame = mod(floor(grainClock * 2.0823), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0341;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1012), 0.0, 1.5);
