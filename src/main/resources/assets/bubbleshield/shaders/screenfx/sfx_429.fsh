...
// for effect 429. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1702 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(grainClock * 1.7898), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2047);
