...
// for effect 010. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:center:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8170 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Radial ripples from the screen center displace the sample outward.
    float ring = sin(centerDist * ParamsA.z - anim * 3.0);
    float fade = smoothstep(0.05, 0.25, centerDist) * invsmooth(0.4621, 0.9309, centerDist);
    vec2 dir = centerDist > 0.0001 ? aspectCentered / centerDist : vec2(0.0);
    vec2 off = dir * ring * fade * 0.0079 * ParamsA.y * animAmp;
