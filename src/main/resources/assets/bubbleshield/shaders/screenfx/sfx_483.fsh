...
// for effect 483. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:pond:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.5747 * sin(animRaw * 0.1889) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Noise-phased pond rings: raindrop-like irregular ripples.
    float phase = vnoise(texCoord * 6.1107) * 3.8270;
    ...
    vec2 off = dir * ring * fade * 0.0084 * ParamsA.y * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6546 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4731, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0844;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0708), 0.0, 1.5);
