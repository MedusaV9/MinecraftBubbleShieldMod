...
// for effect 665. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:pincushion:steady:sparkle]
...
void main() {
    ...
    vec2 warp = -centered * r2 * 0.0589 * min(strength, 1.0);
    vec2 fringe = centered * r2 * 0.0178 * min(strength, 1.0) * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.3155);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8262, 1.0, sin(oClock * 8.5112 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9818, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2716;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1723);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1994), 0.0, 1.5);
