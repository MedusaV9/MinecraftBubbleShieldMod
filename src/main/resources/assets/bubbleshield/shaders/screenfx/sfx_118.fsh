...
// for effect 118. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:diag:steady:grain]
...
void main() {
    ...
    vec2 axis = vec2(0.6563, 0.7545);
    float wave = sin(dot(texCoord, axis) * ParamsA.z * 1.2464 + anim);
    ...
    float grainFrame = mod(floor(anim * 6.7315), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1441), 0.0, 1.5);
