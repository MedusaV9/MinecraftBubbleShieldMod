...
// for effect 471. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:thermal:inverted:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.3591 * sin(animRaw * 0.1299) * ParamsB.y;
    float animAmp = 1.0;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1932);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1203), 0.0, 1.5);
