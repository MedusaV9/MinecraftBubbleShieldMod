...
// for effect 326. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:steady:pulseglow]
...
void main() {
    ...
    float frame = mod(floor(anim * 5.6822), 1024.0);
    float band = floor(texCoord.y * 23.8455);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0748 * ParamsA.y * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5229 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4271, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1359;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1227);
