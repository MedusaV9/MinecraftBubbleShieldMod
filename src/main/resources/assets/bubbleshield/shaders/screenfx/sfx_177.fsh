...
// for effect 177. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:swirl:drift:pulseglow]
...
void main() {
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5080 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4241, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0912;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1707);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2078), 0.0, 1.5);
