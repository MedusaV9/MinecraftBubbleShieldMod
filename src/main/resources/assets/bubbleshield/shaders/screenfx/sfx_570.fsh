...
// for effect 570. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:creep:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1899 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Crystalline frost creeps inward from the screen edges.
    float crystals = vnoise(texCoord * ParamsA.z) * 0.6 + vnoise(texCoord * ParamsA.z * 2.9881) * 0.4;
    float frost = smoothstep(0.3157, 0.6717, centerDist + (crystals - 0.5) * 0.2722) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5981);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3461 * crystals);
    float glint = smoothstep(0.8371, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 0.9486 + crystals * 37.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 16.7497);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8748, 1.0, sin(oClock * 11.9835 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9854, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2739;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1818);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1805), 0.0, 1.5);
