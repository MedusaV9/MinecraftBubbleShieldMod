...
// for effect 791. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:pond:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.9485 * sin(animRaw * 0.1919) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Noise-phased pond rings: raindrop-like irregular ripples.
    float phase = vnoise(texCoord * 5.8666) * 2.7697;
    ...
    float grainFrame = mod(floor(grainClock * 1.8791), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0278;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2046);
