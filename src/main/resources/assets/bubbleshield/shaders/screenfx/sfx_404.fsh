...
// for effect 404. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:lumaonly:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.3799 * sin(animRaw * 0.1730) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float grainFrame = mod(floor(anim * 8.3273), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1713);
