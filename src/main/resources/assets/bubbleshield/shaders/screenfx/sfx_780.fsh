...
// for effect 780. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:crosshatch:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7832 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 4-tap cross-difference Sobel over scene luma.
    vec2 texel = 1.3526 / safeInSize;
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 3.3770, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5350 + 0.3269 * baseLuma;
    ...
    float h1 = 0.5 + 0.5 * sin((texCoord.x * 0.7247 + texCoord.y * 0.6891) * 398.1672);
    float h2 = 0.5 + 0.5 * sin((texCoord.x * -0.6891 + texCoord.y * 0.7247) * 389.0331);
    float shadowMask = smoothstep(0.3807, 0.8764, 1.0 - baseLuma);
    float hatch = clamp(smoothstep(0.6, 1.0, h1) + smoothstep(0.65, 1.0, h2), 0.0, 1.0) * shadowMask;
    vec3 inked = mix(paper, Primary.rgb * 0.1532, clamp(edge + hatch * 0.5658, 0.0, 1.0));
    ...
    float grainFrame = mod(floor(grainClock * 2.1539), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0362;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2119);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1239), 0.0, 1.5);
