...
// for effect 761. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:tunnel:surge:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(grainClock * 2.1204), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0225;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1137), 0.0, 1.5);
