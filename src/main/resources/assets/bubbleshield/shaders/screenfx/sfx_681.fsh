...
// for effect 681. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.8804 * sin(animRaw * 0.1643) * ParamsB.y;
    float animAmp = 1.0;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1360), 0.0, 1.5);
