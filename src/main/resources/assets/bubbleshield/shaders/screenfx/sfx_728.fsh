...
// for effect 728. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:tint:splitlat:drift:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(grainClock * 2.0835), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1560);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1646), 0.0, 1.5);
