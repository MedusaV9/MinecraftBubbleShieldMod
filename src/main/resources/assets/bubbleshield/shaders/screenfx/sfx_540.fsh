...
// for effect 540. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6080 + ParamsB.x * 3.1416);
    ...
    float jitter = (hash11(col + frame * 47.3) - 0.5) * 0.0029 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(0.0, jitter)));
    float scan = 0.5 + 0.5 * sin((texCoord.x * lineCount + anim * 0.3290) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3289 * scan * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.6078), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0361;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0750), 0.0, 1.5);
