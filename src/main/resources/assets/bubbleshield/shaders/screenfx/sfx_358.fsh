...
// for effect 358. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:tunnel:drift:none]
...
void main() {
    ...
    float anim = animRaw + 1.7171 * sin(animRaw * 0.1590) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Tunnel vision: color survives at the center, the rim goes ashen.
    float mask = smoothstep(0.2154, 0.6472, centerDist);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1692);
