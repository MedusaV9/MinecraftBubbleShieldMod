...
// for effect 801. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:split:steady:sparkle]
...
void main() {
    ...
    float hueAngle = -sgn * sin(anim * 0.4137 + ParamsB.x * 6.2831) * 0.9267 * strength;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.8801);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8154, 1.0, sin(oClock * 12.2481 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9839, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3432;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1371);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1740), 0.0, 1.5);
