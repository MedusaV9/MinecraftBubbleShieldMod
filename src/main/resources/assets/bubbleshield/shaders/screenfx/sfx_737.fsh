...
// for effect 737. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8629 + ParamsB.x * 6.2831);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9986 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4178, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1600;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0949), 0.0, 1.5);
