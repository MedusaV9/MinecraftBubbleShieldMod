...
// for effect 580. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:spectral:triple:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4620 + ParamsB.x * 3.1416);
    ...
    float spokeAng = 0.3551;
    float ghostR = 0.0074 * animAmp;
    vec3 g1 = sampleAt(texCoord + safeOffset(vec2(cos(spokeAng), sin(spokeAng)) * ghostR));
    vec3 g2 = sampleAt(texCoord + safeOffset(vec2(cos(spokeAng + 2.0944), sin(spokeAng + 2.0944)) * ghostR * 1.5379));
    vec3 g3 = sampleAt(texCoord + safeOffset(vec2(cos(spokeAng + 4.1888), sin(spokeAng + 4.1888)) * ghostR * 2.0645));
    float w1 = 0.2397 * echoK;
    float w2 = 0.1498 * echoK;
    float w3 = 0.0720 * echoK;
    vec3 haunted = base * (1.0 - w1 - w2 - w3) + g1 * w1 + g2 * w2 + g3 * w3;
    // Desaturation drift: a slow spirit-world pallor swings on GameTime.
    float pallor = 0.2691 + 0.1964 * sin(anim * 0.2507 + ParamsB.x * 6.2831);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 14.1665);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8710, 1.0, sin(oClock * 9.9703 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9883, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3165;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1956);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0746), 0.0, 1.5);
