...
// for effect 541. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:duo:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.0689 * sin(animRaw * 0.1931) * ParamsB.y;
    float animAmp = 1.0;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1857);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0709), 0.0, 1.5);
