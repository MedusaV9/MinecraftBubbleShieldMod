...
// for effect 051. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:steady:grain]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.7489) * strength;
    ...
    float grainFrame = mod(floor(anim * 6.5440), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0382;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1085), 0.0, 1.5);
