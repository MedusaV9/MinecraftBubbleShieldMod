...
// for effect 602. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9344 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 5.4584), 1024.0);
    float band = floor(texCoord.y * 25.9070);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0569 * glitchK * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.6062), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1804);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1401), 0.0, 1.5);
