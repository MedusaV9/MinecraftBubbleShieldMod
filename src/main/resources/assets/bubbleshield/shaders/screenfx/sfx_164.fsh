...
// for effect 164. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.6955 + ParamsB.x * 6.2831);
    ...
    vec3 outColor = base + Primary.rgb * pow(edge, 0.7708) * strength * 0.8617;

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 16.9915);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8608, 1.0, sin(anim * 1.4263 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9784, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2974;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1600);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1261), 0.0, 1.5);
