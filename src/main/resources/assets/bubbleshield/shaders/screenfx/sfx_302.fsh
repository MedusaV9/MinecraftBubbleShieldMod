...
// for effect 302. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.7317 * sin(animRaw * 0.1274) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float jitter = (hash11(col + frame * 47.3) - 0.5) * 0.0020 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(0.0, jitter)));
    float scan = 0.5 + 0.5 * sin((texCoord.x * ParamsA.z + anim * 0.4709) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.2847 * scan * animAmp;
    vec3 outColor = mix(scene * darken, scene * darken * Primary.rgb, ParamsB.z);

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 10.7540);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8140, 1.0, sin(anim * 2.0084 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9874, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3561;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1029);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1636), 0.0, 1.5);
