...
// for effect 344. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:dither:steady:pulseglow]
...
void main() {
    ...
    float dframe = mod(floor(anim * 5.7235), 256.0);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6338 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3716, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1401;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1581), 0.0, 1.5);
