...
// for effect 532. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:tint:grade:steady:grain]
...
void main() {
    ...
    vec3 graded = mix(Secondary.rgb, Primary.rgb, baseLuma) * (0.9875 * baseLuma + 0.0778);
    ...
    float grainFrame = mod(floor(grainClock * 1.8568), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0202;
