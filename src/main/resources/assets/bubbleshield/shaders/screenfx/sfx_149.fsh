...
// for effect 149. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 1.7372 * sin(animRaw * 0.1748) * ParamsB.y;
    float animAmp = 1.0;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.9415);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8370, 1.0, sin(anim * 1.2879 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9867, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3299;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1866);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2020), 0.0, 1.5);
