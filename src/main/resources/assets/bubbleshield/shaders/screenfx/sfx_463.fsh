...
// for effect 463. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:duotone:soft:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0891 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Two-tone remap with a soft-knee luminance ramp.
    float ramp = smoothstep(0.0873, 0.8952, baseLuma);
    vec3 duotone = mix(Secondary.rgb * 0.5659, Primary.rgb * (0.7984 + 0.2180 * ramp), ramp);
    ...
    float grainFrame = mod(floor(grainClock * 2.0294), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1686);
