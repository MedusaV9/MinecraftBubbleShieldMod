...
// for effect 390. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:surge:pulseglow]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3516 + ParamsB.x * 3.1416);
    ...
    float jitter = (hash11(col + frame * 47.3) - 0.5) * 0.0026 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(0.0, jitter)));
    float scan = 0.5 + 0.5 * sin((texCoord.x * ParamsA.z + anim * 0.5835) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3034 * scan * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5975 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3641, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1580;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2087);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1285), 0.0, 1.5);
