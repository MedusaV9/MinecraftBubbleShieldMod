...
// for effect 213. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1341 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 4.7279), 1024.0);
    vec2 cell = floor(texCoord * vec2(7.7834, 5.6742));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.8900
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0347 * ParamsA.y
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 12.0264);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8275, 1.0, sin(anim * 1.9126 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9883, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3932;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1312);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1911), 0.0, 1.5);
