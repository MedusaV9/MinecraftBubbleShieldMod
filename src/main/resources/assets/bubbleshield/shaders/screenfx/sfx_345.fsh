...
// for effect 345. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9337 + ParamsB.x * 6.2831);
    ...
    float jitter = (hash11(row + frame * 91.7) - 0.5) * 0.0035 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(jitter, 0.0)));
    float scan = 0.5 + 0.5 * sin((texCoord.y * ParamsA.z - anim * 0.6464) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3350 * scan * animAmp;
    ...
    float grainFrame = mod(floor(anim * 8.3147), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0275;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1926);
