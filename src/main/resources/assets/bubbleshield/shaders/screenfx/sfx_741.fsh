...
// for effect 741. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:spin:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.8796 * sin(animRaw * 0.1182) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 10-tap tangential spin streak around the center.
    float arc = 0.0451 * ParamsA.y * animAmp * smoothstep(0.1, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 10; i++) {
        float t = (float(i) / 9.0000 - 0.5) * arc;
        float ca = cos(t);
        float sa = sin(t);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.5510);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8589, 1.0, sin(oClock * 12.1322 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9890, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3541;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1859);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1481), 0.0, 1.5);
