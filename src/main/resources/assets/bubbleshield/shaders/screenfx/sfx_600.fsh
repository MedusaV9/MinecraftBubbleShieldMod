...
// for effect 600. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:lumaonly:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7935 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(grainClock * 1.6659), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1708), 0.0, 1.5);
