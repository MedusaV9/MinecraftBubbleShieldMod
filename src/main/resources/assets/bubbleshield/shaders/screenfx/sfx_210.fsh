...
// for effect 210. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:surge:grain]
...
void main() {
    ...
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.6568) * strength;
    ...
    float grainFrame = mod(floor(anim * 5.6682), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0212;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1019), 0.0, 1.5);
