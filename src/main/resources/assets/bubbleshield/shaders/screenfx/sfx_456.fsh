...
// for effect 456. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1376 + ParamsB.x * 6.2831);
    ...
    float jitter = (hash11(col + frame * 47.3) - 0.5) * 0.0022 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(0.0, jitter)));
    float scan = 0.5 + 0.5 * sin((texCoord.x * lineCount + anim * 0.5465) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3659 * scan * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.6251);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8587, 1.0, sin(oClock * 11.1715 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9896, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3763;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1988);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2083), 0.0, 1.5);
