...
// for effect 693. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:breathing:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.0958 * sin(animRaw * 0.1603) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float lensK = sin(anim * 0.2701 + ParamsB.x * 6.2831) * 0.0476 * min(strength, 1.0);
    vec2 warp = centered * r2 * lensK;
    vec2 fringe = centered * r2 * 0.0194 * min(strength, 1.0) * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6810 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4896, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0853;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1545);
