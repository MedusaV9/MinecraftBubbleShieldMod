...
// for effect 041. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:twin:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0628 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Two off-center ripple sources interfere across the frame.
    vec2 d1 = texCoord - vec2(0.2599, 0.3530);
    vec2 d2 = texCoord - vec2(0.6737, 0.5069);
    ...
    float grainFrame = mod(floor(anim * 8.5015), 256.0);
