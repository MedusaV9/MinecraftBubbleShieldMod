...
// for effect 389. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6179 + ParamsB.x * 3.1416);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 14.0005);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8546, 1.0, sin(anim * 2.3582 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9821, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2763;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1716), 0.0, 1.5);
