...
// for effect 522. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:dreamblur:glowdream:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.4144 * sin(animRaw * 0.1713) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Bright-lifted dream veil: the blur adds a soft luminous bloom.
    vec2 texel = 2.9780 / safeInSize;
    ...
    blurred /= 9.0;
    vec3 dream = base * 0.5671 + blurred * mix(vec3(1.0), Primary.rgb, ParamsB.z) * 0.5124;
    ...
    vec2 cellUv = floor(texCoord * safeInSize / 11.7918);
    float tw = hash21(cellUv);
    float twClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float twinkle = smoothstep(0.8062, 1.0, sin(twClock * 11.8566 + tw * 6.2831) * 0.5 + 0.5) * step(0.9734, tw);
    vec3 outColor = dream + Primary.rgb * twinkle * 0.3573 * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.9241), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0314;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1062);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2123), 0.0, 1.5);
