...
// for effect 833. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:cornered:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3882 + ParamsB.x * 3.1416);
    ...
    vec2 warp = centered * r2 * 0.0388 * min(strength, 1.0);
    vec2 fringe = centered * r2 * r2 * 0.0316 * min(strength, 1.0) * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 14.7120);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8184, 1.0, sin(oClock * 13.9638 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9805, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3872;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2113), 0.0, 1.5);
