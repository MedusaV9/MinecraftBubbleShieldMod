...
// for effect 216. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:posterize:dither:drift:none]
...
void main() {
    ...
    float anim = animRaw + 1.5458 * sin(animRaw * 0.1621) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float dframe = mod(floor(anim * 3.3307), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1226);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1889), 0.0, 1.5);
