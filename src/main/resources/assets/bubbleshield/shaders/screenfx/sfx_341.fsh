...
// for effect 341. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:rising:steady:grain]
...
void main() {
    ...
    vec2 off = vec2(
        sin(texCoord.y * 85.9895 + anim * 4.0) + sin(texCoord.y * 35.0431 - anim * 2.6),
        cos(texCoord.x * 51.7541 + anim * 3.1) * 0.4
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.1576);
    vec3 warm = scene * hazeTint;
    vec3 outColor = mix(scene, warm * mix(vec3(1.0), Primary.rgb, ParamsB.z), 0.5151);
    ...
    float grainFrame = mod(floor(anim * 7.3655), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0216;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1881), 0.0, 1.5);
