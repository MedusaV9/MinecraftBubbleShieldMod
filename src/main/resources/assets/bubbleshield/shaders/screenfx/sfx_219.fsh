...
// for effect 219. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0839 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Vertical raster columns with per-column shimmer.
    float col = floor(texCoord.x * ParamsA.z);
    float jitter = (hash11(col + floor(anim * 6.0) * 47.3) - 0.5) * 0.0025 * ParamsA.y;
