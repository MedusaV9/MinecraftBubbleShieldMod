...
// for effect 682. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:surge:pulseglow]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6740 + ParamsB.x * 3.1416);
    ...
    float frost = clamp(0.3820 + 0.5571 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5055);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3284 * crystals);
    float glint = smoothstep(0.8037, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.4789 + crystals * 37.0));
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6347 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3687, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1460;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1887);
