...
// for effect 765. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:duo:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.4293 * sin(animRaw * 0.1578) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float grainFrame = mod(floor(grainClock * 1.6808), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0368;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1161);
