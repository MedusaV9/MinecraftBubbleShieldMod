...
// for effect 388. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:swirl:steady:sparkle]
...
void main() {
    ...
    float swirl = sin(angle * 3.0000 + anim - centerDist * 7.4006);
    vec2 tangent = centerDist > 0.0001 ? vec2(-aspectCentered.y, aspectCentered.x) / centerDist : vec2(0.0);
    vec2 off = tangent * swirl * ParamsA.y * animAmp * smoothstep(0.05, 0.3019, centerDist);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 14.7854);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8031, 1.0, sin(anim * 1.6088 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9850, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3560;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1605);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1072), 0.0, 1.5);
