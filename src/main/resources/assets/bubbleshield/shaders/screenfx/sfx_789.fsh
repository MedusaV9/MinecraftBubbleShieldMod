...
// for effect 789. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:shadows:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 1.5862 * sin(animRaw * 0.1262) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Shadow drain: dark areas lose their color first, highlights stay alive.
    float mask = smoothstep(0.2277, 0.9165, 1.0 - baseLuma);
    ...
    float grainFrame = mod(floor(grainClock * 1.7137), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0265;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1610);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2199), 0.0, 1.5);
