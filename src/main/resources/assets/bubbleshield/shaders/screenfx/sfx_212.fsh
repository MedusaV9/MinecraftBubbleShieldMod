...
// for effect 212. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:duo:steady:sparkle]
...
void main() {
    ...
    vec2 oCell = floor(texCoord * safeInSize / 14.4326);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8742, 1.0, sin(anim * 2.0123 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9813, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3730;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1960);
