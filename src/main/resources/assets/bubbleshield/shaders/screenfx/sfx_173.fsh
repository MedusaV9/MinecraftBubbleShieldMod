...
// for effect 173. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:steady:none]
...
void main() {
    ...
    float darken = 1.0 - ParamsA.y * 0.3234 * scan * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2080);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2143), 0.0, 1.5);
