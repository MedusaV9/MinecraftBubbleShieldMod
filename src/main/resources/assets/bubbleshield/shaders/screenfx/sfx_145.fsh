...
// for effect 145. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:zoned:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0744 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Ring-zoned fringe: concentric zones alternate their separation strength.
    float zone = 0.5 + 0.5 * sin(centerDist * 10.9711 - anim);
    ...
    float grainFrame = mod(floor(anim * 7.1764), 256.0);
