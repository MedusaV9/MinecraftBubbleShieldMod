...
// for effect 736. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:steady:none]
...
void main() {
    ...
    float jitter = (hash11(col + frame * 47.3) - 0.5) * 0.0023 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(0.0, jitter)));
    float scan = 0.5 + 0.5 * sin((texCoord.x * lineCount + anim * 0.5454) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3063 * scan * animAmp;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1951), 0.0, 1.5);
