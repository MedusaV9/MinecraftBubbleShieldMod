...
// for effect 307. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6555 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // 45-degree diamond cells via a skewed grid.
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.3002, 1.0), vec2(1.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.7024);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8243, 1.0, sin(anim * 1.4453 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9839, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3239;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1482);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1013), 0.0, 1.5);
