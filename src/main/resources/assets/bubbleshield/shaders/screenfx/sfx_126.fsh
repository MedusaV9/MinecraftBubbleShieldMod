...
// for effect 126. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rgbdrift:steady:grain]
...
void main() {
    ...
    float frame = mod(floor(anim * 3.8630), 1024.0);
    float wander = hash11(frame * 17.3) - 0.5;
    float split = (0.004 + abs(wander) * 0.0101) * ParamsA.y * animAmp;
    ...
    float microTear = step(0.9804, hash11(floor(texCoord.y * 90.0) + frame)) * ParamsA.y;
    ...
    float grainFrame = mod(floor(anim * 8.6441), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0305;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1344);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1854), 0.0, 1.5);
