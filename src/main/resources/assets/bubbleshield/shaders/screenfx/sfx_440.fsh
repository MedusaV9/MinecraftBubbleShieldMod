...
// for effect 440. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:spectral:twin:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.8583 * sin(animRaw * 0.1714) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Twin ghosts: two opposite decaying echoes along a fixed diagonal.
    float echoK = clamp(strength, 0.0, 1.0);
    vec2 ghostAxis = vec2(0.3936, 0.9193) * 0.0081 * animAmp;
    vec3 g1 = sampleAt(texCoord + safeOffset(ghostAxis));
    vec3 g2 = sampleAt(texCoord + safeOffset(-ghostAxis * 1.9176));
    float w1 = 0.2968 * echoK;
    float w2 = 0.1755 * echoK;
    vec3 haunted = base * (1.0 - w1 - w2) + g1 * w1 + g2 * w2;
    // Desaturation drift: a slow spirit-world pallor swings on GameTime.
    float pallor = 0.3782 + 0.1779 * sin(anim * 0.3730 + ParamsB.x * 6.2831);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1979);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1835), 0.0, 1.5);
