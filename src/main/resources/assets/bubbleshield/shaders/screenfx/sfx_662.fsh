...
// for effect 662. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:dreamblur:soft9:surge:grain]
...
void main() {
    ...
    blurred /= 9.0;
    vec3 dream = mix(base, blurred * 1.0380, clamp(strength, 0.0, 0.85));
    ...
    vec2 cellUv = floor(texCoord * safeInSize / 11.6181);
    float tw = hash21(cellUv);
    float twClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float twinkle = smoothstep(0.7907, 1.0, sin(twClock * 10.8363 + tw * 6.2831) * 0.5 + 0.5) * step(0.9828, tw);
    vec3 outColor = dream + Primary.rgb * twinkle * 0.4458 * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 2.0322), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0226;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1518);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1491), 0.0, 1.5);
