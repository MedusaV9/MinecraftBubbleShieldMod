...
// for effect 794. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:steady:none]
...
void main() {
    ...
    float frost = clamp(0.3980 + 0.5384 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.4727);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2785 * crystals);
    float glint = smoothstep(0.7922, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 0.8917 + crystals * 37.0));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1620);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2039), 0.0, 1.5);
