...
// for effect 837. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:starburst:six:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0597 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 6-arm starburst: a dense snowflake star, 2 taps per arm.
    vec2 texel = 5.7410 / safeInSize;
    float thr = max(ParamsA.w, 0.4348);
    vec3 streak = base * max(baseLuma - thr, 0.0) * 0.3;
    for (int i = 0; i < 6; i++) {
        float armAng = float(i) * 1.0472 + 0.0494;
        vec2 arm = vec2(cos(armAng), sin(armAng)) * texel;
        streak += brightTap(texCoord + safeOffset(arm), thr) * 0.1;
        streak += brightTap(texCoord + safeOffset(arm * 2.5669), thr) * 0.05;
    }
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1267);
