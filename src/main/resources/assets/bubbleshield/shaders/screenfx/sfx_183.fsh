...
void main() {
    ...
    float ember = step(0.9888, hash21(vec2(emberCell.x, mod(emberCell.y, 256.0))));
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.1878);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1005);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1584), 0.0, 1.5);
