...
void main() {
    ...
    vec2 off = (safeNormalize(d1) + safeNormalize(d2)) * ring * fade * 0.0058 * ParamsA.y * animAmp;
    ...
    float grainFrame = mod(floor(anim * 5.5134), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0245;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1264), 0.0, 1.5);
