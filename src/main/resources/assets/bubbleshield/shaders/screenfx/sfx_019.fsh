...
// for effect 019. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:chroma:radial:steady:pulseglow]
...
void main() {
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5830 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3682, 1.0, centerDist);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1667);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1613), 0.0, 1.5);
