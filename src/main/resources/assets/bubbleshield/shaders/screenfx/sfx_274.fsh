...
// for effect 274. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:duotone:soft:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.8644 * sin(animRaw * 0.1991) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Two-tone remap with a soft-knee luminance ramp.
    float ramp = smoothstep(0.0609, 0.9313, baseLuma);
    vec3 duotone = mix(Secondary.rgb * 0.5918, Primary.rgb * (0.7710 + 0.2464 * ramp), ramp);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0923), 0.0, 1.5);
