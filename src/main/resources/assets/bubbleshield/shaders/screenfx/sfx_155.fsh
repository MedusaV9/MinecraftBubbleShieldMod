...
// for effect 155. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:spin:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.9057 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 12-tap tangential spin streak around the center.
    float arc = 0.0493 * ParamsA.y * animAmp * smoothstep(0.1, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 12; i++) {
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1595), 0.0, 1.5);
