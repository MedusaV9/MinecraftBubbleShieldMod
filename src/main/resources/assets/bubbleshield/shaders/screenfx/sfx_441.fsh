...
// for effect 441. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:barrel:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8051 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // Barrel warp: the frame bulges outward; fringing grows with radius.
    float r2 = centerDist * centerDist;
    vec2 warp = centered * r2 * 0.0582 * min(strength, 1.0);
    vec2 fringe = centered * r2 * 0.0163 * min(strength, 1.0) * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1213);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0711), 0.0, 1.5);
