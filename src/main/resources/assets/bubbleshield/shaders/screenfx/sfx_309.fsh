...
// for effect 309. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:full:steady:sparkle]
...
void main() {
    ...
    vec2 off = vec2(
        sin(texCoord.y * 65.6233 + anim * 3.4),
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1890);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1092), 0.0, 1.5);
