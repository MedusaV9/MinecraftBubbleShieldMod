...
// for effect 726. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:vhs:worn:steady:pulseglow]
...
void main() {
    ...
    float frame = mod(floor(anim * 0.2932), 1024.0);
    float row = floor(texCoord.y * 117.5272);
    // Scanline-phase wobble: every row leans on a slow sine phase.
    float phaseWobble = sin(texCoord.y * 261.1880 + anim * 1.0431) * 0.0014 * vhsK * animAmp;
    ...
    float blue = sampleAt(texCoord + safeOffset(baseOff - vec2(bleed * 1.6603, 0.0))).b;
    vec3 taped = vec3(red, green, blue);
    float film = hash21(floor(texCoord * safeInSize * 0.4622) + vec2(frame, 0.0)) - 0.5;
    taped += vec3(film) * 0.0455 * vhsK;
    vec3 outColor = mix(taped, taped * Primary.rgb, ParamsB.z);

    // Overlay: a faint breathing glow of the effect color at the rim.
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7500 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4377, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1292;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1757);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1397), 0.0, 1.5);
