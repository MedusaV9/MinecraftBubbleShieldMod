...
// for effect 403. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.1099 * sin(animRaw * 0.1362) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.5016 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4629, 1.0, centerDist);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1732);
