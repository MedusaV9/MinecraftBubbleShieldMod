...
// for effect 395. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:steady:pulseglow]
...
void main() {
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9405 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3504, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0874;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0646), 0.0, 1.5);
