...
// for effect 645. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:harmonic:steady:sparkle]
...
void main() {
    ...
    vec2 off = (vec2(sin(texCoord.y * ParamsA.z + anim), cos(texCoord.x * ParamsA.z + anim))
        + 0.3514 * vec2(sin(texCoord.y * 59.2727 - anim * 1.7),
            cos(texCoord.x * 35.5158 + anim * 1.3))) * ParamsA.y * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 17.2716);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8508, 1.0, sin(oClock * 14.4945 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9814, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3653;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1073);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1522), 0.0, 1.5);
