...
// for effect 641. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:starburst:spinning:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7603 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 4-arm burst that slowly wheels on GameTime (calm, sub-strobe rate).
    vec2 texel = 5.7007 / safeInSize;
    float thr = max(ParamsA.w, 0.4030);
    float wheel = anim * 0.1300;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        streak += brightTap(texCoord + safeOffset(arm * 2.1804), thr) * 0.06;
    }
    // Additive calibration: capped strength, attenuated on bright scenes.
    vec3 outColor = base + streak * mix(vec3(1.0), Primary.rgb, 0.6026) * min(strength, 1.0) * (1.0 - 0.3123 * baseLuma);
    ...
    float grainFrame = mod(floor(grainClock * 1.8161), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0253;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1413);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1935), 0.0, 1.5);
