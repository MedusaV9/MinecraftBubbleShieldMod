...
// for effect 325. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:sobel:surge:sparkle]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.6815 + ParamsB.x * 3.1416);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.2704);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8317, 1.0, sin(anim * 1.2062 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9827, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3158;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0937), 0.0, 1.5);
