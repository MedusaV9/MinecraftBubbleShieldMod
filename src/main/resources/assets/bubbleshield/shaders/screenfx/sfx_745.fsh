...
// for effect 745. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:radial:steady:sparkle]
...
void main() {
    ...
    float reach = smoothstep(0.0843, 0.6821, centerDist);
    float hueAngle = sin(anim * 0.3217) * 0.8938 * strength * reach;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 11.5385);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8221, 1.0, sin(oClock * 6.5472 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9837, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2816;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1031);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1574), 0.0, 1.5);
