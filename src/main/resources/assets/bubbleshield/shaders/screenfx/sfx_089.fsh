...
// for effect 089. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:pulse:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 1.5406 * sin(animRaw * 0.1673) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float breath = 0.6619 + 0.4122 * sin(anim * 1.8457 + ParamsB.x * 6.2831);
    vec3 outColor = base + Primary.rgb * edge * glowK * breath;

    // Overlay: sparse twinkling motes.
    vec2 oCell = floor(texCoord * safeInSize / 13.1415);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oTwinkle = smoothstep(0.8613, 1.0, sin(anim * 1.8772 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9878, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2512;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1791);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0914), 0.0, 1.5);
