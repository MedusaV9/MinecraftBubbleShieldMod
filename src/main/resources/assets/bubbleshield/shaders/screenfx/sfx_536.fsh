...
// for effect 536. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:square:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7313 + ParamsB.x * 6.2831);
    ...
    float grainFrame = mod(floor(grainClock * 2.4918), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1444);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2173), 0.0, 1.5);
