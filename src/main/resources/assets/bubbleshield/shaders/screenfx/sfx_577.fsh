...
// for effect 577. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:huedrift:radial:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 1.8465 * sin(animRaw * 0.1090) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Hue drift grows with distance from the center; the middle stays true.
    float reach = smoothstep(0.0807, 0.7246, centerDist);
    float hueAngle = sin(anim * 0.5186) * 1.0342 * strength * reach;
    ...
    float grainFrame = mod(floor(grainClock * 1.6358), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1077), 0.0, 1.5);
