...
// for effect 564. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:diamond:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.8970 * sin(animRaw * 0.1535) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 45-degree diamond cells via a skewed grid.
    vec2 gridN = max(safeInSize / max(ParamsA.y * 1.6770, 1.0), vec2(1.0));
    ...
    vec2 oCell = floor(texCoord * safeInSize / 14.5294);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8478, 1.0, sin(oClock * 7.8748 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9798, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2724;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1561), 0.0, 1.5);
