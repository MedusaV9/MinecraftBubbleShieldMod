...
// for effect 077. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:heathaze:full:steady:pulseglow]
...
void main() {
    ...
    vec2 off = vec2(
        sin(texCoord.y * 58.3750 + anim * 3.4),
        sin(texCoord.x * 61.5835 - anim * 2.8)
    ...
    vec3 hazeTint = mix(vec3(1.0), Primary.rgb / max(max(Primary.r, max(Primary.g, Primary.b)), 0.001), 0.1997);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8160 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4788, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1196;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2052);
