...
// for effect 714. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:rowcol:steady:grain]
...
void main() {
    ...
    float frame = mod(floor(anim * 5.8949), 1024.0);
    float rowRoll = hash11(floor(texCoord.y * 29.0181) * 7.31 + frame * 13.7);
    float colRoll = hash11(floor(texCoord.x * 20.4806) * 5.13 + frame * 7.9);
    float tearX = (rowRoll > 0.88 ? rowRoll - 0.88 : 0.0) * 0.4166 * glitchK * animAmp;
    float tearY = (colRoll > 0.9 ? colRoll - 0.9 : 0.0) * 0.2498 * glitchK * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 2.0849), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0337;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2084);
