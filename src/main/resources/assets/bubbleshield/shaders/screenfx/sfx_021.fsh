...
void main() {
    ...
    float lineX = min(ParamsA.z * 0.8469, safeInSize.x * 0.25);
    float scanY = 0.5 + 0.5 * sin((texCoord.y * lineY - anim * 0.5563) * 6.2831);
    float scanX = 0.5 + 0.5 * sin((texCoord.x * lineX + anim * 0.3156) * 6.2831);
    ...
    float grainFrame = mod(floor(anim * 6.8269), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0300;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1098);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1863), 0.0, 1.5);
