...
// for effect 136. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.6070 + ParamsB.x * 6.2831);
    ...
    float frame = mod(floor(anim * 5.1401), 1024.0);
    vec2 cell = floor(texCoord * vec2(6.2819, 7.8572));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.9240
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0341 * ParamsA.y
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.8425 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3965, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.0889;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1898);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0838), 0.0, 1.5);
