...
// for effect 159. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:swirl:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.0517 * sin(animRaw * 0.1409) * ParamsB.y;
    float animAmp = 1.0;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1867);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1476), 0.0, 1.5);
