...
// for effect 697. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:starburst:four:pulse:grain]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.8435 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 4-arm starburst: 3 decaying bright-pass taps down each arm.
    vec2 texel = 4.2827 / safeInSize;
    float thr = max(ParamsA.w, 0.4403);
    vec3 streak = base * max(baseLuma - thr, 0.0) * 0.3;
    for (int i = 0; i < 4; i++) {
        float armAng = float(i) * 1.5708 + 0.0703;
        vec2 arm = vec2(cos(armAng), sin(armAng)) * texel;
        streak += brightTap(texCoord + safeOffset(arm), thr) * 0.1;
        streak += brightTap(texCoord + safeOffset(arm * 2.0662), thr) * 0.06;
        streak += brightTap(texCoord + safeOffset(arm * 3.3448), thr) * 0.035;
    }
    // Additive calibration: capped strength, attenuated on bright scenes.
    vec3 outColor = base + streak * mix(vec3(1.0), Primary.rgb, 0.6260) * min(strength, 1.0) * (1.0 - 0.3487 * baseLuma);
    ...
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0338;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2032);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1662), 0.0, 1.5);
