...
// for effect 315. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:cross:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7486 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // 5-tap cross blur of the bright pass, weighted toward the center.
    vec2 texel = 3.0564 / safeInSize;
    vec3 glow = brightTap(texCoord, ParamsA.w) * 0.4
        ...
        + brightTap(texCoord + safeOffset(vec2(0.0, -texel.y)), ParamsA.w) * 0.15;
    vec3 outColor = base + glow * mix(vec3(1.0), Primary.rgb, 0.5310) * strength;
