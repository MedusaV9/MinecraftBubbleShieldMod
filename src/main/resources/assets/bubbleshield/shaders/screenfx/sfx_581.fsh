...
// for effect 581. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:cornered:steady:sparkle]
...
void main() {
    ...
    vec2 warp = centered * r2 * 0.0515 * min(strength, 1.0);
    vec2 fringe = centered * r2 * r2 * 0.0427 * min(strength, 1.0) * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 16.8007);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8757, 1.0, sin(oClock * 13.0582 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9841, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3667;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1974);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1263), 0.0, 1.5);
