...
// for effect 734. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:ring:pulse:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(grainClock * 2.0624), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0271;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1404), 0.0, 1.5);
