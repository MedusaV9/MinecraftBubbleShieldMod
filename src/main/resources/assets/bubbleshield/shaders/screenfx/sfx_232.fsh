...
// for effect 232. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:veins:drift:none]
...
void main() {
    ...
    float anim = animRaw + 2.9381 * sin(animRaw * 0.1738) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Ridged ice veins crawl across the whole pane.
    float ridge = 1.0 - abs(2.0 * vnoise(texCoord * ParamsA.z + vec2(anim * 0.0171, 0.0)) - 1.0);
    float crystals = ridge * 0.7 + vnoise(texCoord * ParamsA.z * 2.3) * 0.3;
    float frost = smoothstep(0.5964, 0.8183, crystals * (0.75 + 0.25 * centerDist)) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.5334);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.2887 * crystals);
    float glint = smoothstep(0.8569, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.0975 + crystals * 37.0));
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1628);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1697), 0.0, 1.5);
