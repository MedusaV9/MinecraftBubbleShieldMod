...
// for effect 496. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:spectral:orbit:pulse:sparkle]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 0.7045 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // The ghost pair slowly orbits the true image on GameTime.
    float echoK = clamp(strength, 0.0, 1.0);
    float orbitAng = anim * 0.1493 + ParamsB.x * 6.2831;
    vec2 orbitDir = vec2(cos(orbitAng), sin(orbitAng)) * 0.0109 * animAmp;
    vec3 g1 = sampleAt(texCoord + safeOffset(orbitDir));
    vec3 g2 = sampleAt(texCoord + safeOffset(-orbitDir * 1.6658));
    float w1 = 0.3179 * echoK;
    float w2 = 0.1402 * echoK;
    vec3 haunted = base * (1.0 - w1 - w2) + g1 * w1 + g2 * w2;
    // Desaturation drift: a slow spirit-world pallor swings on GameTime.
    float pallor = 0.3416 + 0.1494 * sin(anim * 0.3377 + ParamsB.x * 6.2831);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.8019);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8031, 1.0, sin(oClock * 8.5843 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9770, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2846;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1354);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1387), 0.0, 1.5);
