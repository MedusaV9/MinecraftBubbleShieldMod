...
// for effect 457. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:edgeglow:thick:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5879 + ParamsB.x * 3.1416);
    ...
    vec3 outColor = base + Primary.rgb * pow(edge, 0.7355) * glowK * 0.8447;
    ...
    float grainFrame = mod(floor(grainClock * 1.7665), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1622);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1444), 0.0, 1.5);
