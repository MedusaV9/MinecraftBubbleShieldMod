...
// for effect 658. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:surge:pulseglow]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.5599 + ParamsB.x * 3.1416);
    ...
    float frame = mod(floor(anim * 7.1758), 1024.0);
    float band = floor(texCoord.y * 21.7494);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0776 * glitchK * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6553 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.3753, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1191;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0874), 0.0, 1.5);
