...
// for effect 180. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:sheet:steady:grain]
...
void main() {
    ...
    float frost = clamp(0.3897 + 0.5789 * max(corner.x, corner.y) * crystals, 0.0, 1.0) * strength;
    vec2 grain = vec2(
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3414 * crystals);
    float glint = smoothstep(0.8185, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.3905 + crystals * 37.0));
    ...
    float grainFrame = mod(floor(anim * 5.6251), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0285;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1581);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0969), 0.0, 1.5);
