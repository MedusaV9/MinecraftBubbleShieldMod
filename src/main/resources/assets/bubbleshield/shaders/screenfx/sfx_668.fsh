...
// for effect 668. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:sketch:ink:steady:grain]
...
void main() {
    ...
    float edge = clamp(length(vec2(gr - gl, gd - gu)) * 2.8329, 0.0, 1.0);
    // Paper base: contrast-flattened luma with a palette paper tint.
    float flatTone = 0.5457 + 0.2978 * baseLuma;
    vec3 paper = vec3(flatTone) * mix(vec3(1.0), Secondary.rgb, ParamsB.z);
    vec3 inked = mix(paper, Primary.rgb * 0.1913, edge);
    ...
    float grainFrame = mod(floor(grainClock * 1.8868), 256.0);
