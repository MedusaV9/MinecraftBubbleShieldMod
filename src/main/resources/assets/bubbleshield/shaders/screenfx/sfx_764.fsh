...
// for effect 764. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.2217 * sin(animRaw * 0.1536) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.5429) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3048 * scan * animAmp;
    ...
    float grainFrame = mod(floor(grainClock * 1.9216), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0262;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1527);
