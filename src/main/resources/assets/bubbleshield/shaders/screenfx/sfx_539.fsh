...
// for effect 539. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:linear:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.3269 * sin(animRaw * 0.1415) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Planar wavefronts sweep across the screen in one direction.
    vec2 dir = vec2(0.2621, 0.9650);
    float ring = sin(dot(texCoord, dir) * ParamsA.z - anim * 2.9991);
    ...
    float grainFrame = mod(floor(grainClock * 2.3511), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0221;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1815);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2164), 0.0, 1.5);
