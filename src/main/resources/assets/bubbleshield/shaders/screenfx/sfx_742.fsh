...
// for effect 742. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:blocks:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 2.7171 * sin(animRaw * 0.1721) * ParamsB.y;
    float animAmp = 1.0;
    ...
    float frame = mod(floor(anim * 4.6747), 1024.0);
    vec2 cell = floor(texCoord * vec2(6.7017, 6.7592));
    float cellRoll = hash11(cell.x * 3.7 + cell.y * 11.9 + frame * 5.3);
    vec2 jitter = cellRoll > 0.8958
        ? (vec2(hash11(cellRoll * 91.7), hash11(cellRoll * 47.3)) - 0.5) * 0.0366 * glitchK
        : vec2(0.0);
    vec2 baseOff = jitter;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 16.4796);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8746, 1.0, sin(oClock * 13.2229 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9864, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3998;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1010);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1534), 0.0, 1.5);
