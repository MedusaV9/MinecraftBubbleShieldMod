...
// for effect 349. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:pixelate:breathe:pulse:pulseglow]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.1613 + ParamsB.x * 6.2831);
    float strength = ParamsA.y * animAmp;

    // The mosaic cell size breathes on GameTime.
    float mosaic = max(ParamsA.y * (1.0 + 0.2882 * sin(anim * 1.0011)), 1.0);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.7993 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4457, 1.0, centerDist);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1606);
