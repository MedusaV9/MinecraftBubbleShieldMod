...
// for effect 593. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:shadows:steady:sparkle]
...
void main() {
    ...
    float mask = smoothstep(0.2796, 0.8051, 1.0 - baseLuma);
    ...
    vec2 oCell = floor(texCoord * safeInSize / 10.2874);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8139, 1.0, sin(oClock * 13.0454 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9797, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.2954;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1320);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1510), 0.0, 1.5);
