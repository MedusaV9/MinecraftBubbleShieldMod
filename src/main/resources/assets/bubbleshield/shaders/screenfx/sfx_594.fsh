...
// for effect 594. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:bloomglow:ring:steady:grain]
...
void main() {
    ...
    float grainFrame = mod(floor(grainClock * 1.8172), 256.0);
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1498);
