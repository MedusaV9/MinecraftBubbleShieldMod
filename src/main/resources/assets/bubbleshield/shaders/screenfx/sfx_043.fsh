...
// for effect 043. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:frostlens:creep:surge:grain]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.4063 + ParamsB.x * 3.1416);
    float animAmp = 0.7 + 0.3 * surge * surge * surge;
    float strength = ParamsA.y * animAmp;

    // Crystalline frost creeps inward from the screen edges.
    float crystals = vnoise(texCoord * ParamsA.z) * 0.6 + vnoise(texCoord * ParamsA.z * 3.0543) * 0.4;
    float frost = smoothstep(0.2765, 0.5734, centerDist + (crystals - 0.5) * 0.2565) * strength;
    vec2 grain = vec2(
    ...
    vec3 frosted = mix(scene, iceColor * (0.6 + 0.4 * crystals), 0.4736);
    ...
    frosted = mix(frosted, vec3(1.0), baseLuma * baseLuma * 0.3398 * crystals);
    float glint = smoothstep(0.8080, 1.0, crystals) * (0.5 + 0.5 * sin(anim * 1.1541 + crystals * 37.0));
    ...
    float grainFrame = mod(floor(anim * 8.3843), 256.0);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0651), 0.0, 1.5);
