...
// for effect 606. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:dreamblur:edgehalo:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.2762 * sin(animRaw * 0.1700) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // The veil thickens toward the edges: a dreamy porthole.
    vec2 texel = 2.9680 / safeInSize;
