...
// for effect 553. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:aberration:cornered:drift:pulseglow]
...
void main() {
    ...
    float anim = animRaw + 2.9879 * sin(animRaw * 0.1569) * ParamsB.y;
    float animAmp = 1.0;
    ...
    vec2 warp = centered * r2 * 0.0334 * min(strength, 1.0);
    vec2 fringe = centered * r2 * r2 * 0.0387 * min(strength, 1.0) * animAmp;
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.9741 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4190, 1.0, centerDist);
    outColor += Primary.rgb * oRim * oBreath * 0.1336;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1233);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1792), 0.0, 1.5);
