...
// for effect 413. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:wobble:swirl:steady:grain]
...
void main() {
    ...
    float swirl = sin(angle * 2.0000 + anim - centerDist * 8.6261);
    vec2 tangent = centerDist > 0.0001 ? vec2(-aspectCentered.y, aspectCentered.x) / centerDist : vec2(0.0);
    vec2 off = tangent * swirl * ParamsA.y * animAmp * smoothstep(0.05, 0.3766, centerDist);
    ...
    float grainFrame = mod(floor(anim * 7.4043), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0323;
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1265), 0.0, 1.5);
