...
// for effect 085. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:glitch:tear:steady:none]
...
void main() {
    ...
    float frame = mod(floor(anim * 5.6813), 1024.0);
    float band = floor(texCoord.y * 25.6684);
    float tearRoll = hash11(band * 7.31 + frame * 13.7);
    float tear = (tearRoll > 0.85 ? (tearRoll - 0.85) / 0.15 - 0.5 : 0.0) * 0.0648 * ParamsA.y * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1938);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1679), 0.0, 1.5);
