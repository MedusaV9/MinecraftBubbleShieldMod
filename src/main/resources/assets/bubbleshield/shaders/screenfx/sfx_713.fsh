...
// for effect 713. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:radialblur:pulsezoom:drift:grain]
...
void main() {
    ...
    float anim = animRaw + 2.5407 * sin(animRaw * 0.1768) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // 6-tap zoom streak whose reach surges on GameTime.
    float surgeK = 0.6756 + 0.3772 * sin(anim * 1.2839 + ParamsB.x * 6.2831);
    float blur = 0.0306 * ParamsA.y * animAmp * surgeK * smoothstep(0.05, 0.7, centerDist);
    vec3 accum = vec3(0.0);
    for (int i = 0; i < 6; i++) {
    ...
    float grainFrame = mod(floor(grainClock * 2.4074), 256.0);
    outColor += (hash21(floor(texCoord * safeInSize) + vec2(grainFrame, 0.0)) - 0.5) * 0.0267;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2112);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.0761), 0.0, 1.5);
