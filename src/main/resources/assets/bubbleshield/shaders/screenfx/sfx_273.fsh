...
// for effect 273. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:vertical:surge:none]
...
void main() {
    ...
    float surge = 0.5 + 0.5 * sin(anim * 0.3859 + ParamsB.x * 3.1416);
    ...
    float scan = 0.5 + 0.5 * sin((texCoord.x * lineCount + anim * 0.4932) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3122 * scan * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1420);
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.1148), 0.0, 1.5);
