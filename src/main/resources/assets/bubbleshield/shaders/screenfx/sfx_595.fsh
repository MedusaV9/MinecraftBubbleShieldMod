...
// for effect 595. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:ripple:center:drift:sparkle]
...
void main() {
    ...
    float anim = animRaw + 1.6572 * sin(animRaw * 0.1932) * ParamsB.y;
    float animAmp = 1.0;
    float strength = ParamsA.y * animAmp;

    // Radial ripples from the screen center displace the sample outward.
    float ring = sin(centerDist * ParamsA.z - anim * 3.0);
    float fade = smoothstep(0.05, 0.25, centerDist) * invsmooth(0.4641, 0.9103, centerDist);
    vec2 dir = centerDist > 0.0001 ? aspectCentered / centerDist : vec2(0.0);
    vec2 off = dir * ring * fade * 0.0087 * ParamsA.y * animAmp;
    ...
    vec2 oCell = floor(texCoord * safeInSize / 15.2534);
    float oTw = hash21(oCell + vec2(37.0, 91.0));
    float oClock = GameTime * 1200.0 + ParamsB.x * 61.8;
    float oTwinkle = smoothstep(0.8339, 1.0, sin(oClock * 13.2417 + oTw * 6.2831) * 0.5 + 0.5) * step(0.9795, oTw);
    outColor += Secondary.rgb * oTwinkle * 0.3316;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.2069);
