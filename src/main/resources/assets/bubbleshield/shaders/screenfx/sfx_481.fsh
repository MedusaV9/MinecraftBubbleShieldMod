...
// for effect 481. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:desat:shadows:surge:pulseglow]
...
void main() {
    ...
    float mask = smoothstep(0.2817, 0.8933, 1.0 - baseLuma);
    ...
    float oBreath = 0.5 + 0.5 * sin(anim * 0.6614 + ParamsB.x * 6.2831);
    float oRim = smoothstep(0.4551, 1.0, centerDist);
    ...
    outColor = clamp(mix(vec3(luma(outColor)), outColor, 1.2147), 0.0, 1.5);
