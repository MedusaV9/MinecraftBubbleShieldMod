...
// for effect 792. Edit the generator and regenerate instead
// (byte-stable, fixed seed).
// [screen:scanlines:horizontal:pulse:none]
...
void main() {
    ...
    float animAmp = 0.8 + 0.2 * sin(anim * 1.0909 + ParamsB.x * 6.2831);
    ...
    float jitter = (hash11(row + frame * 91.7) - 0.5) * 0.0034 * ParamsA.y;
    vec3 scene = sampleAt(texCoord + safeOffset(vec2(jitter, 0.0)));
    float scan = 0.5 + 0.5 * sin((texCoord.y * lineCount - anim * 0.4899) * 6.2831);
    float darken = 1.0 - ParamsA.y * 0.3614 * scan * animAmp;
    ...
    outColor = mix(outColor, curved * curved * (3.0 - 2.0 * curved), 0.1706);
