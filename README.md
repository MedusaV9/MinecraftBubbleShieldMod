...
Every shield has one of **75 selectable effects** (ids 0–74, organized as 15 color families x 5 effects),
...
2. **Surface layer** (client): one of **12** procedural animated shader templates — plasma, hex lattice,
   waves, aurora, sparkle, rings, voronoi, arcs, scales, starfield, vortex or interference.
3. **Inside layer** (server): one of **25 behaviors** x 3 variants — particle domes/spirals/orbits, regen /
   ...
   spore drift, enchantment streams, fire wards, intruder-freezing frost and purge pulses.
...
7. **Screen layer** (client): one of **12** full-screen post-processing templates applied while you stand
   inside the bubble — tint, wobble, vignette, chroma, pixelate, desat, bloomglow, ripple, scanlines,
   edgeglow, frostlens or heathaze (`assets/bubbleshield/post_effect/effect_00.json` … `effect_74.json`,
   regenerated by `tools/gen_post_effects.py`).

**Uniqueness guarantee** (machine-enforced by `EffectRegistry.validate()` and the gametests): all 75 palette
pairs are pairwise distinct, every behavior is used exactly 3 times covering variants {0, 1, 2}, no color
family repeats a surface or screen template, and every (sound, pitch, period) triple is unique. No two
effects look, sound or behave the same.
